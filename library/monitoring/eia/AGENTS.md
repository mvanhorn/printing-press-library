# Eia Printed CLI Agent Guide

This directory is a generated `eia-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
eia-pp-cli doctor --json
eia-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
eia-pp-cli which "<capability>" --json
eia-pp-cli <command> --help
```
...
```bash
eia-pp-cli <command> --help
eia-pp-cli <command> --dry-run --agent
```
