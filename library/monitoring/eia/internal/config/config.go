...
type Config struct {
	...
	Path           string            `toml:"-"`
	EiaApiKeyQuery string            `toml:"api_key_query"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("EIA_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "eia-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("EIA_API_KEY_QUERY"); v != "" {
		cfg.EiaApiKeyQuery = v
		cfg.AuthSource = "env:EIA_API_KEY_QUERY"
	}
	...
	if cfg.AuthSource == "" && cfg.EiaApiKeyQuery != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("EIA_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
