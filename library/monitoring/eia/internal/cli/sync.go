...
Resource scoping:
  ...
  eia-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  eia-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  eia-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  eia-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  eia-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/eia-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "electricity":
		return db.UpsertElectricity(data)
	case "natural-gas":
		return db.UpsertNaturalGas(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"electricity-electric-power-operational-data-data": "/electricity/electric-power-operational-data/data/",
		"electricity-rto-fuel-type-data-data":              "/electricity/rto/fuel-type-data/data/",
		"electricity-rto-region-data-data":                 "/electricity/rto/region-data/data/",
		"natural-gas":                                      "/natural-gas/pri/fut/data/",
		"natural-gas-pri-sum-data":                         "/natural-gas/pri/sum/data/",
