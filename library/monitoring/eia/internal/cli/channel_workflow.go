...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  eia-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  eia-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"electricity", "electricity-electric-power-operational-data-data", "electricity-rto-fuel-type-data-data", "electricity-rto-region-data-data", "natural-gas", "natural-gas-pri-sum-data", "petroleum", "seds", "steo"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  eia-pp-cli workflow status

  # Show status as JSON
  eia-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("eia-pp-cli")
			}
