...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  eia-pp-cli doctor
  eia-pp-cli doctor --json
  eia-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export EIA_API_KEY_QUERY=<your-key>"
					report["auth_key_url"] = "https://www.eia.gov/opendata/register.php"
				} else {
					authConfigured = true
			...
			if os.Getenv("EIA_API_KEY_QUERY") != "" {
				authEnvSet = append(authEnvSet, "EIA_API_KEY_QUERY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "EIA_API_KEY_QUERY")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("eia-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'eia-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'eia-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'eia-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'eia-pp-cli sync' to refresh."
	}
	return report
}
