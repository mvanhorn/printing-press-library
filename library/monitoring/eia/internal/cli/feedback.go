...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".eia-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("EIA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("EIA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "eia-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.eia-pp-cli/feedback.jsonl.
When ` + "`EIA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`EIA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  eia-pp-cli feedback list
  eia-pp-cli feedback list --limit 5
  eia-pp-cli feedback list --json`,
