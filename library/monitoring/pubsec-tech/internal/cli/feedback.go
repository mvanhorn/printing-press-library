...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".pubsec-tech-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PUBSEC_TECH_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PUBSEC_TECH_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "pubsec-tech-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.pubsec-tech-pp-cli/feedback.jsonl.
When ` + "`PUBSEC_TECH_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PUBSEC_TECH_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pubsec-tech-pp-cli feedback list
  pubsec-tech-pp-cli feedback list --limit 5
  pubsec-tech-pp-cli feedback list --json`,
