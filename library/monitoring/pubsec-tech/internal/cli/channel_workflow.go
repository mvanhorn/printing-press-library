...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  pubsec-tech-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  pubsec-tech-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"agencies", "awards", "entities", "recipients", "references", "references-glossary"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  pubsec-tech-pp-cli workflow status

  # Show status as JSON
  pubsec-tech-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("pubsec-tech-pp-cli")
			}
