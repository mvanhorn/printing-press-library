# pubsec-tech Printed CLI Agent Guide

This directory is a generated `pubsec-tech-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
pubsec-tech-pp-cli doctor --json
pubsec-tech-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
pubsec-tech-pp-cli which "<capability>" --json
pubsec-tech-pp-cli <command> --help
```
...
```bash
pubsec-tech-pp-cli <command> --agent
```
...
```bash
pubsec-tech-pp-cli <command> --help
pubsec-tech-pp-cli <command> --dry-run --agent
```
