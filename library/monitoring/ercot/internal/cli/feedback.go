...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".ercot-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ERCOT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ERCOT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ercot-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.ercot-pp-cli/feedback.jsonl.
When ` + "`ERCOT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ERCOT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ercot-pp-cli feedback list
  ercot-pp-cli feedback list --limit 5
  ercot-pp-cli feedback list --json`,
