# Ercot Printed CLI Agent Guide

This directory is a generated `ercot-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
ercot-pp-cli doctor --json
ercot-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
ercot-pp-cli which "<capability>" --json
ercot-pp-cli <command> --help
```
...
```bash
ercot-pp-cli <command> --help
ercot-pp-cli <command> --dry-run --agent
```
