...
**Every NinjaOne endpoint plus fleet-wide cross-org commands the org-scoped console can't give you.**
...
The recommended path installs both the `ninjaone-pp-cli` binary and the `pp-ninjaone` agent skill (Claude Code, Codex, Cursor, Gemini CLI, GitHub Copilot, and other agents supported by the upstream [`skills`](https://github.com/vercel-labs/skills) CLI) in one shot:
...
```
Install the pp-ninjaone skill from https://github.com/mvanhorn/printing-press-library/tree/main/cli-skills/pp-ninjaone. The skill defines how its required CLI can be installed.
```
...
```bash
# Confirm credentials, region, and API reachability before anything else.
ninjaone-pp-cli doctor --dry-run

# Pull core fleet state into the local store so offline and cross-org commands work.
ninjaone-pp-cli sync --resources organizations,devices,alerts,os-patches

# See every device missing a critical patch across all organizations at once.
ninjaone-pp-cli patch-gaps --severity critical

# Collapse the active alert flood into the real incidents behind it.
ninjaone-pp-cli alert-storms --window 1h

# Find devices gone quiet fleet-wide for a hygiene sweep.
ninjaone-pp-cli stale-devices --offline-days 30
...
Run `ninjaone-pp-cli --help` for the full command reference and flag list.
...
- **`ninjaone-pp-cli billing activate-product`** - Activates a previously deactivated product given its ID.
...
- **`ninjaone-pp-cli billing deactivate-product`** - Deactivates a previously activated product given its ID.
...
**Note:** This endpoint returns 204 No Content on success. No invoice data is returned.
...
- **`ninjaone-pp-cli itam delete-asset-relationships`** - Deletes one or more relationships between assets by their unique identifiers. Maximum of 100 relationships can be deleted per request.
- **`ninjaone-pp-cli itam delete-unmanaged-device-public-api`** - Delete an Unmanaged Device with the provided id
- **`ninjaone-pp-cli itam get-all-relationship-types`** - Returns a paginated list of all relationship types, including both system default and user created types, available for creating asset relationships.
...
- **`ninjaone-pp-cli itam get-entity-relations`** - Returns a paginated list of all asset relationships for a specific entity, optionally filtered by relationship type and relation side.
...
- **`ninjaone-pp-cli queries get-scoped-custom-fields-detailed-report`** - Returns report for Custom Fields defined at different scopes (device, location, organization) with additional information about each field
...
- **`ninjaone-pp-cli tab update-custom-public-api`** - Update a Custom Tab. This API can be used to either update existing tabs, or create tab 'role extensions' for existing tabs
...
- **`ninjaone-pp-cli tab update-organization-custom-order`** - Update the order of custom tabs for organizations and locations. NOTE: All tabs defined for organizations must be specified in the payload
- **`ninjaone-pp-cli tab update-role-custom-order`** - Update the order of custom tabs for a specific role. NOTE: Only tabs created on this role can be ordered. All tabs defined on the role must be specified in the payload
...
- **`ninjaone-pp-cli vulnerability update-scan-group`** - Upload CSV to an existing scan group. The uploaded CSV must contain the columns defined in the 
...
ninjaone-pp-cli activities

# JSON for scripting and agents
ninjaone-pp-cli activities --json

# Filter to specific fields
ninjaone-pp-cli activities --json --select id,name,status

# Dry run — show the request without sending
ninjaone-pp-cli activities --dry-run

# Agent mode — JSON + compact + no prompts in one flag
ninjaone-pp-cli activities --agent
```
...
| --- | --- | --- | --- |
| `NINJAONE_CLIENT_ID` | per_call | Yes | Set to your API credential. |
| `NINJAONE_CLIENT_SECRET` | per_call | Yes | Set to your API credential. |

### agentcookie (optional)

If you use agentcookie to sync secrets across machines, this CLI auto-adopts agentcookie-managed credentials with no extra setup. When the daemon writes to this CLI's config, `ninjaone-pp-cli doctor` reports `agentcookie: detected` and `auth status` labels the source as `agentcookie`. Skip this section if you don't use agentcookie - the CLI works the same as any other.

## Troubleshooting
**Authentication errors (exit code 4)**
- Run `ninjaone-pp-cli doctor` to check credentials
...
### API-specific
- **401 or missing_header on every call** — Set NINJAONE_CLIENT_ID and NINJAONE_CLIENT_SECRET, then run `ninjaone-pp-cli doctor`.
- **403 forbidden on a write command (patch-sweep, alert-clear)** — Your Client App ID lacks the management scope; recreate it with monitoring, management, and control.
- **429 Too Many Requests during a large sync or sweep** — The adaptive limiter already honors Retry-After; lower --max-scan-pages or narrow the --df filter to reduce volume.
- **Commands return empty results** — Run `ninjaone-pp-cli sync` first; cross-org commands read the local store.
- **Wrong region / 404 on token endpoint** — Set NINJAONE_INSTANCE to your shard (eu.ninjarmm.com, ca.ninjarmm.com, or oc.ninjarmm.com).

## Sources & Inspiration

This CLI was built by studying these projects and resources:

- [**homotechsual/NinjaOne**](https://github.com/homotechsual/NinjaOne) — PowerShell (131 stars)
- [**Lungshot/NinjaOneMCP**](https://github.com/Lungshot/NinjaOneMCP) — TypeScript (34 stars)
- [**wyre-technology/ninjaone-mcp**](https://github.com/wyre-technology/ninjaone-mcp) — TypeScript (17 stars)
- [**jasondsmith72/NinjaRMM-API-Gateway-MCP**](https://github.com/jasondsmith72/NinjaRMM-API-Gateway-MCP) — Python (1 stars)
- [**fredriksknese/mcp-ninjaone**](https://github.com/fredriksknese/mcp-ninjaone) — TypeScript (1 stars)
- [**marsacom/NinjaOneToolKit**](https://github.com/marsacom/NinjaOneToolKit) — Python (1 stars)
