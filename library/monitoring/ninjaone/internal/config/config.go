...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("NINJAONE_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "ninjaone-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("NINJAONE_CLIENT_ID"); v != "" {
		cfg.ClientID = v
		cfg.AuthSource = "env:NINJAONE_CLIENT_ID"
	}
	if v := os.Getenv("NINJAONE_CLIENT_SECRET"); v != "" {
		cfg.ClientSecret = v
		cfg.AuthSource = "env:NINJAONE_CLIENT_SECRET"
	}
	...
	if v := os.Getenv("NINJAONE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("NINJAONE_TOKEN_URL"); v != "" {
		cfg.TokenURL = v
	}
	return cfg, nil
}
