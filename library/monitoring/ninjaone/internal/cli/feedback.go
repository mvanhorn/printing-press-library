...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "ninjaone-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NINJAONE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NINJAONE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ninjaone-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/ninjaone-pp-cli/feedback.jsonl.
When ` + "`NINJAONE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NINJAONE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ninjaone-pp-cli feedback list
  ninjaone-pp-cli feedback list --limit 5
  ninjaone-pp-cli feedback list --json`,
