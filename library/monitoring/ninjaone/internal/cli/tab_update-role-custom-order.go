...
func newTabUpdateRoleCustomOrderCmd(flags *rootFlags) *cobra.Command {
	var flagBodyJSON string
	var stdinBody bool

	cmd := &cobra.Command{
		Use:         "update-role-custom-order <roleId>",
		Short:       "Update the order of custom tabs for a specific role. NOTE: Only tabs created on this role can be ordered.",
		Example:     "  ninjaone-pp-cli tab update-role-custom-order 42",
		Annotations: map[string]string{"pp:endpoint": "tab.update-role-custom-order", "pp:method": "PATCH", "pp:path": "/v2/tab/role/{roleId}/order"},
