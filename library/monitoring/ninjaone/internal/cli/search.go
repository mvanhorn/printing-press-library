...
In local mode: searches locally synced data only.`,
  ...
  ninjaone-pp-cli search "error timeout"

  # Force local search only
  ninjaone-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  ninjaone-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  ninjaone-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /v2/devices/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/v2/devices/search", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'ninjaone-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "billing":
				results, err = db.SearchBilling(query, limit)
			case "custom-fields":
				results, err = db.SearchCustomFields(query, limit)
			case "document-templates":
				results, err = db.SearchDocumentTemplates(query, limit)
			case "noderole":
				results, err = db.SearchNoderole(query, limit)
			case "software-license":
				results, err = db.SearchSoftwareLicense(query, limit)
			case "tab":
				results, err = db.SearchTab(query, limit)
			case "tag":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchBilling(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search billing failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchDocumentTemplates(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search document_templates failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchNoderole(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search noderole failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchSoftwareLicense(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search software_license failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchTab(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search tab failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
