# NinjaOne Printed CLI Agent Guide

This directory is a generated `ninjaone-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
ninjaone-pp-cli doctor --json
ninjaone-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
ninjaone-pp-cli which "<capability>" --json
ninjaone-pp-cli <command> --help
```
...
```bash
ninjaone-pp-cli <command> --help
ninjaone-pp-cli <command> --dry-run --agent
```
