# Dogecoin Core Printed CLI Agent Guide

This directory is a generated `dogecoin-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
dogecoin-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
dogecoin-pp-cli which "<capability>" --json
dogecoin-pp-cli <command> --help
```
...
```bash
dogecoin-pp-cli <command> --help
dogecoin-pp-cli <command> --dry-run --agent
```
