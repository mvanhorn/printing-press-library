...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "dogecoin-pp-cli",
		Short: `Dogecoin CLI — Monitor your Dogecoin node with typed exit codes, SQLite trending, and native MCP — everything bitcoin-cli lacks for au…`,
		Long: `Dogecoin CLI — Monitor your Dogecoin node with typed exit codes, SQLite trending, and native MCP — everything bitcoin-cli lacks for au…
...
Health check: run 'dogecoin-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("dogecoin-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'dogecoin-pp-cli profile list')")
	...
	rootCmd.AddCommand(newBlockchainCmd(flags))
	rootCmd.AddCommand(newMempoolCmd(flags))
	rootCmd.AddCommand(newMiningCmd(flags))
	...
	rootCmd.AddCommand(newNodePromotedCmd(flags))
...
func (f *rootFlags) newRPCClient() (*rpc.Client, error) {
	cfg, err := config.Load(f.configPath)
	if err != nil {
		return nil, configErr(err)
	}
