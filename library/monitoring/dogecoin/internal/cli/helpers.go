...
func printOutputWithFlags(w io.Writer, data json.RawMessage, flags *rootFlags) error {
	...
	// --plain: tab-separated key=value pairs for shell scripting
	if flags.plain {
...
// printPlain renders JSON as tab-separated key=value pairs for shell scripting.
// Arrays print one item per line; objects print each field on its own line.
