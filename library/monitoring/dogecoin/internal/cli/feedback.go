...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".dogecoin-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DOGECOIN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DOGECOIN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "dogecoin-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.dogecoin-pp-cli/feedback.jsonl.
When ` + "`DOGECOIN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DOGECOIN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  dogecoin-pp-cli feedback list
  dogecoin-pp-cli feedback list --limit 5
  dogecoin-pp-cli feedback list --json`,
