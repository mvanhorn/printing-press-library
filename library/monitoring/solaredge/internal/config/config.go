...
type Config struct {
	...
	SolaredgeApiKey  string          `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("SOLAREDGE_API_KEY"); v != "" {
		cfg.SolaredgeApiKey = v
		cfg.markEnvOverride("SolaredgeApiKey")
		cfg.AuthSource = "env:SOLAREDGE_API_KEY"
		cfg.CredentialSource = "env:SOLAREDGE_API_KEY"
	}
	...
	if v := os.Getenv("SOLAREDGE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("SOLAREDGE_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "solaredge-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.SolaredgeApiKey
	if token == "" {
		return ""
	}
	if c.SolaredgeApiKey == "" {
		return ""
	}
	return token
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		SolaredgeApiKey: c.SolaredgeApiKey,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.SolaredgeApiKey = creds.SolaredgeApiKey
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.SolaredgeApiKey = token
	delete(c.envOverrides, "SolaredgeApiKey")
	c.updateFileConfigField("SolaredgeApiKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["SolaredgeApiKey"] {
			out.SolaredgeApiKey = c.fileConfig.SolaredgeApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "SolaredgeApiKey":
		c.fileConfig.SolaredgeApiKey = c.SolaredgeApiKey
	}
}
