...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("morning_health_check",
			mcplib.WithDescription("One combined go/no-go status instead of cross-referencing overview, power flow, and equipment calls by hand."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Override the recipe's positional id value.")),
		),
		handleMorningHealthCheck,
	)
}

// handleMorningHealthCheck runs the morning_health_check recipe intent: One combined go/no-go status instead of cross-referencing overview, power flow, and equipment calls by hand.
func handleMorningHealthCheck(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
