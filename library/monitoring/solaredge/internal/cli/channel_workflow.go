...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  solaredge-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  solaredge-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"accounts", "sites"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  solaredge-pp-cli workflow status

  # Show status as JSON
  solaredge-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("solaredge-pp-cli")
			}
