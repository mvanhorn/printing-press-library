...
Run sync first to populate the local search index.`,
  ...
  solaredge-pp-cli search "status"

  # Force local search explicitly
  solaredge-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  solaredge-pp-cli search "status" --type accounts --data-source local
  # JSON output for piping
  solaredge-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'solaredge-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "sites":
				results, err = db.SearchSites(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchSites(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search sites failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
