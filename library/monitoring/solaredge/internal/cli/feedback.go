...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SOLAREDGE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SOLAREDGE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "solaredge-pp-cli/feedback")
...
When ` + "`SOLAREDGE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SOLAREDGE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  solaredge-pp-cli feedback list
  solaredge-pp-cli feedback list --limit 5
  solaredge-pp-cli feedback list --json`,
