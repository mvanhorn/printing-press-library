# SolarEdge Printed CLI Agent Guide

This directory is a generated `solaredge-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
solaredge-pp-cli doctor --json
solaredge-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
solaredge-pp-cli which "<capability>" --json
solaredge-pp-cli <command> --help
```
...
```bash
solaredge-pp-cli <command> --agent
```
...
```bash
solaredge-pp-cli <command> --help
solaredge-pp-cli <command> --dry-run --agent
```
