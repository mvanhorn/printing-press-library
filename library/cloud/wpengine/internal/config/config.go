...
type Config struct {
	...
	WpEngineApiUsername string          `toml:"engine_api_username"`
	WpEngineApiPassword string          `toml:"engine_api_password"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("WP_ENGINE_API_USERNAME"); v != "" {
		cfg.WpEngineApiUsername = v
		cfg.markEnvOverride("WpEngineApiUsername")
		cfg.AuthSource = "env:WP_ENGINE_API_USERNAME"
		cfg.CredentialSource = "env:WP_ENGINE_API_USERNAME"
	}
	if v := os.Getenv("WP_ENGINE_API_PASSWORD"); v != "" {
		cfg.WpEngineApiPassword = v
		cfg.markEnvOverride("WpEngineApiPassword")
		cfg.AuthSource = "env:WP_ENGINE_API_PASSWORD"
		cfg.CredentialSource = "env:WP_ENGINE_API_PASSWORD"
	}
	...
	if v := os.Getenv("WPENGINE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("WPENGINE_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "wpengine-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.WpEngineApiUsername == "" {
		return ""
	}
	if c.WpEngineApiPassword == "" {
		return ""
	}
	credentials := basicAuthPayload("Basic {username}:{password}", map[string]string{
		"engine_api_username":    c.WpEngineApiUsername,
		"WP_ENGINE_API_USERNAME": c.WpEngineApiUsername,
		"username":               c.WpEngineApiUsername,
		"user":                   c.WpEngineApiUsername,
		"engine_api_password":    c.WpEngineApiPassword,
		"WP_ENGINE_API_PASSWORD": c.WpEngineApiPassword,
		"password":               c.WpEngineApiPassword,
		"secret":                 c.WpEngineApiPassword,
		"token":                  c.WpEngineApiPassword,
		"access_token":           c.WpEngineApiPassword,
	}, false)
	if credentials == "" {
		return ""
	}
...
func (c *Config) clearCredentialFields() {
	...
	c.ClientID = ""
	c.ClientSecret = ""
	c.WpEngineApiUsername = ""
	c.WpEngineApiPassword = ""
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		WpEngineApiUsername: c.WpEngineApiUsername,
		WpEngineApiPassword: c.WpEngineApiPassword,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.WpEngineApiUsername = creds.WpEngineApiUsername
	c.WpEngineApiPassword = creds.WpEngineApiPassword
}
...
func (c *Config) SaveCredentials(value0, value1 string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.WpEngineApiUsername = value0
	delete(c.envOverrides, "WpEngineApiUsername")
	c.updateFileConfigField("WpEngineApiUsername")
	c.WpEngineApiPassword = value1
	delete(c.envOverrides, "WpEngineApiPassword")
	c.updateFileConfigField("WpEngineApiPassword")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["WpEngineApiUsername"] {
			out.WpEngineApiUsername = c.fileConfig.WpEngineApiUsername
		}
		if c.envOverrides["WpEngineApiPassword"] {
			out.WpEngineApiPassword = c.fileConfig.WpEngineApiPassword
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "WpEngineApiUsername":
		c.fileConfig.WpEngineApiUsername = c.WpEngineApiUsername
	case "WpEngineApiPassword":
		c.fileConfig.WpEngineApiPassword = c.WpEngineApiPassword
	}
}
