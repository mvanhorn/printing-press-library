...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	return home, filepath.Join(home, ".config", "wpengine-pp-cli", "config.toml")
}
...
func TestCredentialsFileWinsWhenLegacyConfigAlsoHasSecrets(t *testing.T) {
	...
	assertBasicAuthCredential(t, cfg.AuthHeader(), "data-secret")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "wpengine-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("WP_ENGINE_API_USERNAME", "env-secret")
	t.Setenv("WP_ENGINE_API_PASSWORD", testCredentialPassword("env-secret"))
	credentialsPath := filepath.Join(home, ".local", "share", "wpengine-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "wpengine-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "wpengine-pp-cli", "credentials.toml")
...
func testCredentialPassword(token string) string {
	return "password-" + token
}

func assertBasicAuthCredential(t *testing.T, header, token string) {
	t.Helper()
	const prefix = "Basic "
	...
	if err != nil {
		t.Fatalf("AuthHeader() payload is not valid base64: %v", err)
	}
	want := token + ":" + testCredentialPassword(token)
	if string(payload) != want {
		t.Fatalf("AuthHeader() payload = %q, want %q", payload, want)
	}
}

func legacyCredentialKey() string {
	return "engine_api_username"
}
...
func legacyConfigData(t *testing.T, baseURL, token string) []byte {
	...
	return []byte("base_url = \"" + baseURL + "\"\n" +
		...
		"engine_api_password = \"" + testCredentialPassword(token) + "\"\n")
}
...
func saveConfigCredential(cfg *config.Config, token string) error {
	return cfg.SaveCredentials(token, testCredentialPassword(token))
}
...
func configCredentialValue(cfg *config.Config) string {
	return cfg.WpEngineApiUsername
}
...
func credentialValue(creds *cliutil.Credentials) string {
	return creds.WpEngineApiUsername
}

func setCredentialValue(creds *cliutil.Credentials, token string) {
	creds.WpEngineApiUsername = token
	creds.WpEngineApiPassword = testCredentialPassword(token)
}
