...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: wpengine-pp-cli: ticker pattern %q failed to compile: %v\n", `^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$`, err)
	} else {
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"cache_type": {
			{Canonical: "cdn", Values: []string{"cdn cache"}},
			{Canonical: "page", Values: []string{"page cache"}},
			{Canonical: "object", Values: []string{"object cache"}},
			{Canonical: "all", Values: []string{"all caches", "everything"}},
		},
		"environment": {
			{Canonical: "production", Values: []string{"prod", "live"}},
			{Canonical: "staging", Values: []string{"stage", "stg"}},
			{Canonical: "development", Values: []string{"dev"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: wpengine-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: wpengine-pp-cli: learn init: %v\n", err)
		}
	})
}
