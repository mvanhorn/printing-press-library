...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  wpengine-pp-cli doctor
  wpengine-pp-cli doctor --json
  wpengine-pp-cli doctor --fail-on warn
  wpengine-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set Basic credentials with: export WP_ENGINE_API_USERNAME=\"your-username\" WP_ENGINE_API_PASSWORD=\"your-password\""
					report["auth_key_url"] = "https://my.wpengine.com/api_access"
				} else {
					authConfigured = true
			...
			if os.Getenv("WP_ENGINE_API_USERNAME") != "" {
				authEnvSet = append(authEnvSet, "WP_ENGINE_API_USERNAME")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "WP_ENGINE_API_USERNAME")
			}
			if os.Getenv("WP_ENGINE_API_PASSWORD") != "" {
				authEnvSet = append(authEnvSet, "WP_ENGINE_API_PASSWORD")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "WP_ENGINE_API_PASSWORD")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "wpengine-pp-cli"
						verifyPath := "/user"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'wpengine-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'wpengine-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'wpengine-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'wpengine-pp-cli sync' to refresh."
	}
	return report
}
