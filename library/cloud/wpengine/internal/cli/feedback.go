...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("WPENGINE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("WPENGINE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "wpengine-pp-cli/feedback")
...
When ` + "`WPENGINE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`WPENGINE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  wpengine-pp-cli feedback list
  wpengine-pp-cli feedback list --limit 5
  wpengine-pp-cli feedback list --json`,
