...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Wp Engine Hosting",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  wpengine-pp-cli auth setup\n  wpengine-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://my.wpengine.com/api_access")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export WP_ENGINE_API_USERNAME=\"your-username\"")
			fmt.Fprintln(w, "  export WP_ENGINE_API_PASSWORD=\"your-password\"")
			fmt.Fprintln(w, "  wpengine-pp-cli auth set-credentials your-username your-password")
			if !launch {
				return nil
			}
			launchURL := "https://my.wpengine.com/api_access"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  wpengine-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export WP_ENGINE_API_USERNAME=\"your-username\"")
				fmt.Fprintln(w, "  export WP_ENGINE_API_PASSWORD=\"your-password\"")
				fmt.Fprintf(w, "  wpengine-pp-cli auth set-credentials your-username your-password\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetCredentialsCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		Use:     "set-credentials <engine_api_username> <engine_api_password>",
		Short:   "Save Basic auth credentials to the credentials file",
		Example: "  wpengine-pp-cli auth set-credentials your-username your-password",
		Args: func(cmd *cobra.Command, args []string) error {
			if len(args) != 2 {
				return fmt.Errorf("requires exactly 2 args: <engine_api_username> <engine_api_password>")
			}
			if args[0] == "" {
				return fmt.Errorf("WP_ENGINE_API_USERNAME is required")
			}
			if args[1] == "" {
				return fmt.Errorf("WP_ENGINE_API_PASSWORD is required")
			}
			return nil
		},
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  wpengine-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("WP_ENGINE_API_USERNAME") != "" {
				envStillSet = "WP_ENGINE_API_USERNAME"
			}
			if envStillSet == "" && os.Getenv("WP_ENGINE_API_PASSWORD") != "" {
				envStillSet = "WP_ENGINE_API_PASSWORD"
			}
