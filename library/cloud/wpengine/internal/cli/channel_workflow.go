...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  wpengine-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  wpengine-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"accounts", "installs", "site-reports-sections", "sites", "ssh-keys"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  wpengine-pp-cli workflow status

  # Show status as JSON
  wpengine-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("wpengine-pp-cli")
			}
