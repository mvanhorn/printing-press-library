...
type rootFlags struct {
	...
	// invocation. Mirrors the WPENGINE_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "wpengine-pp-cli",
		Short: `Wpengine CLI — Every WP Engine Hosting Platform API operation, plus a local fleet mirror that answers cross-account audit questions…`,
		Long: `Wpengine CLI — Every WP Engine Hosting Platform API operation, plus a local fleet mirror that answers cross-account audit questions the portal cannot.
...
Health check: run 'wpengine-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("wpengine-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'wpengine-pp-cli profile list')")
	...
	rootCmd.AddCommand(newInstallsCmd(flags))
	rootCmd.AddCommand(newSiteReportsCmd(flags))
	...
	rootCmd.AddCommand(newNovelWhoisCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newInstallCopyPromotedCmd(flags))
	rootCmd.AddCommand(newStatusPromotedCmd(flags))
	rootCmd.AddCommand(newSwaggerPromotedCmd(flags))
