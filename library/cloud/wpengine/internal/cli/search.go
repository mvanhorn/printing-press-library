...
Run sync first to populate the local search index.`,
  ...
  wpengine-pp-cli search "status"

  # Force local search explicitly
  wpengine-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  wpengine-pp-cli search "status" --type accounts --data-source local
  # JSON output for piping
  wpengine-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'wpengine-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "site-reports":
				results, err = db.SearchSiteReports(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchSiteReports(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search site_reports failed: %w", searchErr)
					}
