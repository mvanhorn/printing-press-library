...
var whichIndex = []whichEntry{
	{Command: "audit certs", Description: "See every SSL certificate across your whole fleet sorted by days to expiry, before a client site goes insecure.", Group: "Fleet audits from local state", WhyItMatters: "Reach for this when asked about certificate expiry anywhere in the fleet instead of walking installs one API call at a time."},
	{Command: "audit backups", Description: "Find production installs whose latest completed backup is older than a threshold you set.", Group: "Fleet audits from local state", WhyItMatters: "Answers 'which prod installs are unprotected right now' in one call instead of N backup-list calls."},
	{Command: "audit versions", Description: "Fleet-wide PHP and WordPress version distribution with outlier filters, plus per-site environment drift detection.", Group: "Fleet audits from local state", WhyItMatters: "Use it when asked which installs run outdated PHP/WordPress or whether staging matches production."},
	{Command: "guard", Description: "Gate a deploy: create a checkpoint backup, wait until it completes, then optionally purge cache, with CI-friendly exit codes.", Group: "Deploy plumbing", WhyItMatters: "Use it before any deploy that needs a verified restore point; it blocks until the backup actually completes."},
	{Command: "audit usage", Description: "Project month-end billable visits and storage against account limits and flag accounts trending over.", Group: "Live API projections", WhyItMatters: "Use it to warn about overages before they land on an invoice, not after."},
	{Command: "audit domains", Description: "One list of everything blocked across launches: unverified domains, domains without cert coverage, dangling redirects.", Group: "Fleet audits from local state", WhyItMatters: "Use it to find every domain still blocking a launch instead of re-checking installs one by one."},
	{Command: "whois", Description: "Resolve any domain name to the install, site, and account that serve it, plus cert and redirect status.", Group: "Fleet audits from local state", WhyItMatters: "Use it first when a support ticket names a domain and you need to know which install and account own it."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  wpengine-pp-cli which "stale tickets"
  wpengine-pp-cli which "bottleneck"
  wpengine-pp-cli which --limit 1 "send message"
  wpengine-pp-cli which                                # list the full capability index`,
