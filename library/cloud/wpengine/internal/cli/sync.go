...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/cloud/wpengine/internal/learn/lookups"
...
Resource scoping:
  ...
  wpengine-pp-cli sync

  # Sync specific resources only
  wpengine-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  wpengine-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  wpengine-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  wpengine-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  wpengine-pp-cli sync --latest-only`,
...
func determinePaginationDefaults(resource string) paginationDefaults {
	...
	case "accounts/accounts_site_transfers":
		return paginationDefaults{
			cursorParam: "offset",
			cursorType:  "offset",
			limitParam:  "limit",
	...
	case "installs/ssl_certificates":
		return paginationDefaults{
			cursorParam: "offset",
			cursorType:  "offset",
			limitParam:  "limit",
	...
	case "sites/sites_site_transfers":
		return paginationDefaults{
			cursorParam: "offset",
			cursorType:  "offset",
			limitParam:  "limit",
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "account_users":
		return db.UpsertAccountUsers(data)
	case "limits":
		return db.UpsertLimits(data)
	case "accounts_site_transfers":
		return db.UpsertAccountsSiteTransfers(data)
	case "accounts_usage":
		return db.UpsertAccountsUsage(data)
	case "installs":
		return db.UpsertInstalls(data)
	case "archives":
		return db.UpsertArchives(data)
	case "backups":
		return db.UpsertBackups(data)
	case "domains":
		return db.UpsertDomains(data)
	case "offload_settings":
		return db.UpsertOffloadSettings(data)
	case "purge_cache":
		return db.UpsertPurgeCache(data)
	case "ssl_certificates":
		return db.UpsertSslCertificates(data)
	case "installs_usage":
		return db.UpsertInstallsUsage(data)
	case "site-reports":
		return db.UpsertSiteReports(data)
	case "sites":
		return db.UpsertSites(data)
	case "sites_site_transfers":
		return db.UpsertSitesSiteTransfers(data)
	case "ssh-keys":
		return db.UpsertSshKeys(data)
	case "user":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		...
		"site-reports-schedules": "/site_reports/schedules",
		"site-reports-sections":  "/site_reports/sections",
		"site-reports-templates": "/site_reports/templates",
		"sites":                  "/sites",
		"ssh-keys":               "/ssh_keys",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "accounts_site_transfers", ParentTable: "accounts", ParentIDParam: "account_id", PathTemplate: "/accounts/{account_id}/site_transfers", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.account", PathParams: []dependentPathParamDef{
			{Param: "account_id", Field: "id"},
		}},
		{Name: "accounts_usage", ParentTable: "accounts", ParentIDParam: "account_id", PathTemplate: "/accounts/{account_id}/usage", KeyField: "", ReconcileMode: "none", GenericScopeJSONPath: "", PathParams: []dependentPathParamDef{
			{Param: "account_id", Field: "id"},
		}},
		{Name: "archives", ParentTable: "installs", ParentIDParam: "install_id", PathTemplate: "/installs/{install_id}/archives", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.install", PathParams: []dependentPathParamDef{
			{Param: "install_id", Field: "id"},
		}},
		{Name: "backups", ParentTable: "installs", ParentIDParam: "install_id", PathTemplate: "/installs/{install_id}/backups", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.install", PathParams: []dependentPathParamDef{
			{Param: "install_id", Field: "id"},
		}},
		{Name: "domains", ParentTable: "installs", ParentIDParam: "install_id", PathTemplate: "/installs/{install_id}/domains", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.install", PathParams: []dependentPathParamDef{
			{Param: "install_id", Field: "id"},
		}},
		{Name: "sites_site_transfers", ParentTable: "sites", ParentIDParam: "site_id", PathTemplate: "/sites/{site_id}/site_transfers", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.site", PathParams: []dependentPathParamDef{
			{Param: "site_id", Field: "id"},
		}},
		{Name: "ssl_certificates", ParentTable: "installs", ParentIDParam: "install_id", PathTemplate: "/installs/{install_id}/ssl_certificates", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.install", PathParams: []dependentPathParamDef{
			{Param: "install_id", Field: "id"},
		}},
	}
}
...
var reconcileTypedTables = map[string]string{
	...
	"accounts":                "accounts",
	"accounts_site_transfers": "accounts_site_transfers",
	"accounts_usage":          "accounts_usage",
	"archives":                "archives",
	"backups":                 "backups",
	"domains":                 "domains",
	"installs":                "installs",
	"installs_usage":          "installs_usage",
	"limits":                  "limits",
	"offload_settings":        "offload_settings",
	"purge_cache":             "purge_cache",
	"site-reports":            "site_reports",
	"sites":                   "sites",
	"sites_site_transfers":    "sites_site_transfers",
	"ssh-keys":                "ssh_keys",
...
func responsePathForResource(resource, path string) []string {
	...
	case "accounts\x00/accounts":
		return []string{"results"}
	case "installs\x00/installs":
		return []string{"stable_ips"}
	case "installs\x00/installs/{install_id}":
		return []string{"stable_ips"}
	case "site-reports\x00/site_reports":
		return []string{"reports"}
	case "site-reports\x00/site_reports/schedules":
		return []string{"schedules"}
	case "site-reports\x00/site_reports/sections":
		return []string{"sections"}
	case "site-reports\x00/site_reports/templates":
		return []string{"templates"}
	case "sites\x00/sites":
		return []string{"results"}
	case "ssh-keys\x00/ssh_keys":
		return []string{"results"}
	}
	return nil
}
