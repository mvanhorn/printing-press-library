...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/cloud/browserbase/internal/learn/lookups"
...
Resource scoping:
  ...
  browserbase-pp-cli sync
  # Sync specific resources only
  browserbase-pp-cli sync --resources functions,agents

  # Full resync (ignore previous checkpoint)
  browserbase-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  browserbase-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  browserbase-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  browserbase-pp-cli sync --latest-only`,
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		"agents":           "/v1/agents",
		"agents-runs":      "/v1/agents/runs",
		"certificates":     "/v1/certificates",
		"downloads":        "/v1/downloads",
		"functions":        "/v1/functions",
		"functions-builds": "/v1/functions/builds",
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "versions", ParentTable: "functions", ParentIDParam: "id", PathTemplate: "/v1/functions/{id}/versions", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.function", PathParams: []dependentPathParamDef{
			{Param: "id", Field: "id"},
		}},
	}
}
...
func responsePathForResource(resource, path string) []string {
	...
	case "agents\x00/v1/agents":
		return []string{"data"}
	case "agents\x00/v1/agents/runs":
		return []string{"data"}
	case "agents\x00/v1/agents/runs/{runId}/messages":
		return []string{"data"}
	case "downloads\x00/v1/downloads":
		return []string{"downloads"}
	case "functions\x00/v1/functions/builds/{id}":
		return []string{"builtFunctions"}
	case "functions\x00/v1/functions/builds/{id}/logs":
		return []string{"logs"}
	case "functions\x00/v1/functions/builds":
		return []string{"data"}
	case "functions\x00/v1/functions/invocations/{id}/logs":
		return []string{"logs"}
	case "functions\x00/v1/functions":
		return []string{"data"}
	case "functions\x00/v1/functions/versions/{id}/invocations":
		return []string{"results"}
	case "websearch\x00/v1/search":
		return []string{"results"}
	}
	return nil
}
