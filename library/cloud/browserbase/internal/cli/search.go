...
In local mode: searches locally synced data only.`,
  ...
  browserbase-pp-cli search "error timeout"

  # Force local search only
  browserbase-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  browserbase-pp-cli search "status" --type agents --data-source local
  # JSON output for piping
  browserbase-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'browserbase-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
