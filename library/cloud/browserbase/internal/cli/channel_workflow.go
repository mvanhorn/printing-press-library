...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  browserbase-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  browserbase-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"agents", "agents-runs", "certificates", "functions", "functions-builds", "projects", "sessions"}
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  browserbase-pp-cli workflow status

  # Show status as JSON
  browserbase-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("browserbase-pp-cli")
			}
