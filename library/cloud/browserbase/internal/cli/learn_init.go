...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: browserbase-pp-cli: ticker pattern %q failed to compile: %v\n", `^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$`, err)
	} else {
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"agent": {
			{Canonical: "agent", Values: []string{"agents", "browser agent"}},
		},
		"project": {
			{Canonical: "project", Values: []string{"projects"}},
		},
		"session": {
			{Canonical: "session", Values: []string{"browser session", "sessions", "browser"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: browserbase-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: browserbase-pp-cli: learn init: %v\n", err)
		}
	})
}
