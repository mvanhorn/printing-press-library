...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BROWSERBASE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BROWSERBASE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "browserbase-pp-cli/feedback")
...
When ` + "`BROWSERBASE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BROWSERBASE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  browserbase-pp-cli feedback list
  browserbase-pp-cli feedback list --limit 5
  browserbase-pp-cli feedback list --json`,
