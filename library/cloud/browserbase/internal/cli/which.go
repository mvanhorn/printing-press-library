...
var whichIndex = []whichEntry{
	{Command: "sessions orphans", Description: "Find running sessions that were never released (keepAlive orphans) and the runtime they're burning, then optionally stop them in batch. Requires `sync` first — an unsynced store returns an empty scan.", Group: "Session lifecycle control", WhyItMatters: "Reach for this when a crashed automation may have left billable sessions running — it turns a silent cost leak into a one-command cleanup."},
	{Command: "sessions run", Description: "Create a session, print its connect URL, and guarantee it is released on completion, timeout, or interrupt.", Group: "Session lifecycle control", WhyItMatters: "Use when you need a browser session for a script or agent without leaking keepAlive sessions on failure."},
	{Command: "fetch batch", Description: "Fetch a list of URLs with rate-limit pacing and a resumable checkpoint, so large scrape jobs survive interruptions.", Group: "Agent-native fetch pipeline", WhyItMatters: "Reach for this when scraping a list of pages: it paces requests, skips already-fetched URLs, and reports per-URL status."},
	{Command: "projects digest", Description: "See everything that ran in a project this week — sessions, agent runs, and downloads grouped by day — from the local store. Requires `sync` first; an unsynced store returns an empty digest.", Group: "Local state that compounds", WhyItMatters: "Use when reviewing what a project actually did over a period, without clicking through the dashboard."},
	{Command: "agents runs diff", Description: "Compare two agent runs structurally — message sequences and final structured results — to see what changed between prompt iterations.", Group: "Agent-native fetch pipeline", WhyItMatters: "Reach for this when iterating on an agent's prompt or schema and you need to see exactly what changed between two runs."},
	{Command: "usage trend", Description: "Track per-project browserMinutes and proxyBytes over sync history and spot quota creep before the bill arrives. Requires `sync` first; an unsynced store returns an empty trend.", Group: "Local state that compounds", WhyItMatters: "Use when you own the Browserbase bill and want to see usage direction, not just a snapshot."},
	{Command: "web history", Description: "Review past fetch and search calls with cached results, and re-emit a cached response without re-hitting the API. Requires `sync` first; an unsynced store returns an empty history.", Group: "Local state that compounds", WhyItMatters: "Reach for this when you need provenance for what was fetched, or want to re-run a prior result offline."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  browserbase-pp-cli which "stale tickets"
  browserbase-pp-cli which "bottleneck"
  browserbase-pp-cli which --limit 1 "send message"
  browserbase-pp-cli which                                # list the full capability index`,
