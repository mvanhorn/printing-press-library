...
type rootFlags struct {
	...
	// invocation. Mirrors the BROWSERBASE_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "browserbase-pp-cli",
		Short: `Browserbase CLI — Every Browserbase cloud feature, plus session lifecycle control, local history, and usage analytics no other tool has.`,
		Long: `Browserbase CLI — Every Browserbase cloud feature, plus session lifecycle control, local history, and usage analytics no other tool has.
...
Health check: run 'browserbase-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("browserbase-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select agentId,createdAt,name)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'browserbase-pp-cli profile list')")
	...
	rootCmd.AddCommand(newFetchPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelWebCmd(flags))
