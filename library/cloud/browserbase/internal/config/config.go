...
type Config struct {
	...
	BrowserbaseApiKey string          `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	if v := os.Getenv("BROWSERBASE_API_KEY"); v != "" {
		...
		cfg.markEnvOverride("BrowserbaseApiKey")
		cfg.AuthSource = "env:BROWSERBASE_API_KEY"
		cfg.CredentialSource = "env:BROWSERBASE_API_KEY"
	}
	...
	if v := os.Getenv("BROWSERBASE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("BROWSERBASE_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "browserbase-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.BrowserbaseApiKey
	if token == "" {
		return ""
	}
	if c.BrowserbaseApiKey == "" {
		return ""
	}
	return token
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		BrowserbaseApiKey: c.BrowserbaseApiKey,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.BrowserbaseApiKey == "" {
		c.BrowserbaseApiKey = creds.BrowserbaseApiKey
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.BrowserbaseApiKey = token
	delete(c.envOverrides, "BrowserbaseApiKey")
	c.updateFileConfigField("BrowserbaseApiKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["BrowserbaseApiKey"] {
			out.BrowserbaseApiKey = c.fileConfig.BrowserbaseApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "BrowserbaseApiKey":
		c.fileConfig.BrowserbaseApiKey = c.BrowserbaseApiKey
	}
}
