...
This directory is a generated `servosity-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
servosity-pp-cli doctor --json
servosity-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
servosity-pp-cli which "<capability>" --json
servosity-pp-cli <command> --help
```
...
```bash
servosity-pp-cli <command> --agent
```
...
```bash
servosity-pp-cli <command> --help
servosity-pp-cli <command> --dry-run --agent
```
