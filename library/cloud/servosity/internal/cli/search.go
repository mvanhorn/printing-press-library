...
In local mode: searches locally synced data only.`,
  ...
  servosity-pp-cli search "error timeout"

  # Force local search only
  servosity-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  servosity-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  servosity-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /backup-search/
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get("/backup-search/", map[string]string{
					"search": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'servosity-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "credentials":
				results, err = db.SearchCredentials(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchCredentials(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search credentials failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
