...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  servosity-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  servosity-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"admin", "admin-attention", "admin-dirty-repos", "admin-draas-in-progress", "admin-maxio-price-points", "admin-servosity-one-worker-token", "admin-support-staff", "admin-worker-agents", "admin-worker-run", "agent-login", "backup-plans", "backup-search", "backups", "backups-mfa-codes", "companies", "companies-fully-managed", "companies-fully-managed-ng", "companies-summary", "companies-summary-ng", "company-notes", "components", "contracts", "contracts-signatures", "credentials", "current-user", "current-user-api-token", "current-user-groups", "current-user-mfa-backup-codes", "current-user-notifications", "current-user-profile", "current-user-start-mfa", "current-user-verified-mfa", "download", "dr-backups", "issues", "issues-archived", "issues-ignored", "reports", "reports-account", "reports-classic-usage", "reports-clients", "reports-dr-from-email", "reports-maxio-price-points", "reports-product", "reports-stale-backup-sets", "reports-user-profiles", "reseller-notes", "resellers", "resellers-summary", "restic-backups", "restore-queue-web-login", "stats", "stats-live", "stats-user", "users"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  servosity-pp-cli workflow status

  # Show status as JSON
  servosity-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("servosity-pp-cli")
			}
