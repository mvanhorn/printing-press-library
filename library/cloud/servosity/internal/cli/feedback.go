...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".servosity-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SERVOSITY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SERVOSITY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "servosity-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.servosity-pp-cli/feedback.jsonl.
When ` + "`SERVOSITY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SERVOSITY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  servosity-pp-cli feedback list
  servosity-pp-cli feedback list --limit 5
  servosity-pp-cli feedback list --json`,
