...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		...
		Short: `Servosity CLI — Every Servosity endpoint as a typed command, plus a local fleet mirror, snapshot history, and cross-engine rollups the …`,
		Long: `Servosity CLI — Every Servosity endpoint as a typed command, plus a local fleet mirror, snapshot history, and cross-engine rollups the …
...
Highlights (not in the official API docs):
  ...
  • stale-issues   Pull your FMDB companies, fetch active issues, classify known-safe-to-archive patterns from a shipped rule table, auto-archive the safe ones, ignore non-dashboard noise, and print unknowns for review…

Agent mode: add --agent to any command for JSON output + non-interactive mode.
Health check: run 'servosity-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("servosity-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'servosity-pp-cli profile list')")
	...
	rootCmd.AddCommand(newAgentLoginCmd(flags))
	rootCmd.AddCommand(newBackupPlansCmd(flags))
	rootCmd.AddCommand(newBackupSetsCmd(flags))
	...
	rootCmd.AddCommand(newCurrentUserCmd(flags))
	rootCmd.AddCommand(newDrBackupsCmd(flags))
	rootCmd.AddCommand(newIssueCommentsCmd(flags))
	rootCmd.AddCommand(newIssuesCmd(flags))
	rootCmd.AddCommand(newReportSubscriptionsCmd(flags))
	rootCmd.AddCommand(newReportsCmd(flags))
	rootCmd.AddCommand(newResellerNotesCmd(flags))
	rootCmd.AddCommand(newResellersCmd(flags))
	rootCmd.AddCommand(newResticBackupsCmd(flags))
	rootCmd.AddCommand(newRestoreQueueWebLoginCmd(flags))
	...
	rootCmd.AddCommand(newBackupJobReportSummaryPromotedCmd(flags))
	rootCmd.AddCommand(newBackupJobStatusPromotedCmd(flags))
	rootCmd.AddCommand(newBackupJobsPromotedCmd(flags))
	...
	rootCmd.AddCommand(newPostmarkWebhookPromotedCmd(flags))
	rootCmd.AddCommand(newScreenshotPromotedCmd(flags))
	rootCmd.AddCommand(newSmsFmMfaCallbackPromotedCmd(flags))
	rootCmd.AddCommand(newSmsMfaCallbackPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	rootCmd.AddCommand(newAttentionCmd(flags))
	rootCmd.AddCommand(newDriftCmd(flags))
	rootCmd.AddCommand(newStaleBackupsCmd(flags))
	rootCmd.AddCommand(newBackupFactsCmd(flags))
	rootCmd.AddCommand(newFindCmd(flags))
	rootCmd.AddCommand(newRestoreQueueCmd(flags))
	rootCmd.AddCommand(newCompanyCmd(flags))
	rootCmd.AddCommand(newTriageCmd(flags))
	rootCmd.AddCommand(newClearCmd(flags))
	rootCmd.AddCommand(newStaleIssuesCmd(flags))
