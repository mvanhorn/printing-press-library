...
type Config struct {
	...
	NetlifyAuthToken   string          `toml:"auth_token"`
	NetlifyNetlifyAuth string          `toml:"netlify_auth"`
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("NETLIFY_AUTH_TOKEN"); v != "" {
		cfg.NetlifyAuthToken = v
		cfg.markEnvOverride("NetlifyAuthToken")
		cfg.AuthSource = "env:NETLIFY_AUTH_TOKEN"
		cfg.CredentialSource = "env:NETLIFY_AUTH_TOKEN"
	}
	if v := os.Getenv("NETLIFY_NETLIFY_AUTH"); v != "" {
		cfg.NetlifyNetlifyAuth = v
		cfg.markEnvOverride("NetlifyNetlifyAuth")
		cfg.AuthSource = "env:NETLIFY_NETLIFY_AUTH"
		cfg.CredentialSource = "env:NETLIFY_NETLIFY_AUTH"
	}
	...
	if v := os.Getenv("NETLIFY_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("NETLIFY_AUTHORIZATION_URL"); v != "" {
	...
	if v := strings.TrimSpace(os.Getenv("NETLIFY_FORM_ID")); v != "" {
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("NETLIFY_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "netlify-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.NetlifyAuthToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:NETLIFY_AUTH_TOKEN"
		}
		return "Bearer " + c.NetlifyAuthToken
	}
	if c.NetlifyNetlifyAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:NETLIFY_NETLIFY_AUTH"
		}
		return "Bearer " + c.NetlifyNetlifyAuth
	}
	if c.AccessToken != "" {
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		NetlifyAuthToken:   c.NetlifyAuthToken,
		NetlifyNetlifyAuth: c.NetlifyNetlifyAuth,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.NetlifyAuthToken = creds.NetlifyAuthToken
	c.NetlifyNetlifyAuth = creds.NetlifyNetlifyAuth
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.NetlifyAuthToken = ""
	delete(c.envOverrides, "NetlifyAuthToken")
	c.updateFileConfigField("NetlifyAuthToken")
	c.NetlifyNetlifyAuth = ""
	delete(c.envOverrides, "NetlifyNetlifyAuth")
	c.updateFileConfigField("NetlifyNetlifyAuth")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["NetlifyAuthToken"] {
			out.NetlifyAuthToken = c.fileConfig.NetlifyAuthToken
		}
		if c.envOverrides["NetlifyNetlifyAuth"] {
			out.NetlifyNetlifyAuth = c.fileConfig.NetlifyNetlifyAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "NetlifyAuthToken":
		c.fileConfig.NetlifyAuthToken = c.NetlifyAuthToken
	case "NetlifyNetlifyAuth":
		c.fileConfig.NetlifyNetlifyAuth = c.NetlifyNetlifyAuth
	}
}
