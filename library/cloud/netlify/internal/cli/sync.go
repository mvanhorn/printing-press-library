...
Resource scoping:
  ...
  netlify-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  netlify-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  netlify-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  netlify-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  netlify-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("netlify-pp-cli")
			}
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "accounts_ai_gateway":
		return db.UpsertAccountsAiGateway(data)
	case "audit":
		return db.UpsertAudit(data)
	case "accounts_env":
		return db.UpsertAccountsEnv(data)
	case "agent-runners":
		return db.UpsertAgentRunners(data)
	case "archive":
		return db.UpsertArchive(data)
	case "commit":
		return db.UpsertCommit(data)
	case "pull_request":
		return db.UpsertPullRequest(data)
	case "sessions":
	...
	case "deploy-keys":
		return db.UpsertDeployKeys(data)
	case "deploys":
		return db.UpsertDeploys(data)
	case "cancel":
		return db.UpsertCancel(data)
	case "deploys_files":
		return db.UpsertDeploysFiles(data)
	case "deploys_functions":
		return db.UpsertDeploysFunctions(data)
	case "lock":
		return db.UpsertLock(data)
	case "deploys_plugin_runs":
		return db.UpsertDeploysPluginRuns(data)
	case "unlock":
		return db.UpsertUnlock(data)
	case "validations_report":
		return db.UpsertValidationsReport(data)
	case "dns-zones":
		return db.UpsertDnsZones(data)
	case "dns_records":
		return db.UpsertDnsRecords(data)
	case "transfer":
		return db.UpsertTransfer(data)
	case "forms_submissions":
		return db.UpsertFormsSubmissions(data)
	case "hooks":
		return db.UpsertHooks(data)
	case "hooks_enable":
		return db.UpsertHooksEnable(data)
	case "oauth":
		return db.UpsertOauth(data)
	case "services":
		return db.UpsertServices(data)
	case "manifest":
		return db.UpsertManifest(data)
	case "sites":
		return db.UpsertSites(data)
	case "agent_runner_hooks":
		return db.UpsertAgentRunnerHooks(data)
	case "sites_ai_gateway":
		return db.UpsertSitesAiGateway(data)
	case "assets":
		return db.UpsertAssets(data)
	case "build_hooks":
		return db.UpsertBuildHooks(data)
	case "sites_builds":
		return db.UpsertSitesBuilds(data)
	case "database":
		return db.UpsertDatabase(data)
	case "deployed_branches":
		return db.UpsertDeployedBranches(data)
	case "sites_deploys":
		return db.UpsertSitesDeploys(data)
	case "dev_server_hooks":
		return db.UpsertDevServerHooks(data)
	case "dev_servers":
		return db.UpsertDevServers(data)
	case "disable":
		return db.UpsertDisable(data)
	case "dns":
		return db.UpsertDns(data)
	case "sites_enable":
		return db.UpsertSitesEnable(data)
	case "sites_env":
		return db.UpsertSitesEnv(data)
	case "sites_files":
		return db.UpsertSitesFiles(data)
	case "sites_forms":
		return db.UpsertSitesForms(data)
	case "sites_functions":
		return db.UpsertSitesFunctions(data)
	case "metadata":
		return db.UpsertMetadata(data)
	case "sites_plugin_runs":
		return db.UpsertSitesPluginRuns(data)
	case "plugins":
		return db.UpsertPlugins(data)
	case "rollback":
		return db.UpsertRollback(data)
	case "service_instances":
		return db.UpsertServiceInstances(data)
	case "sites_services":
		return db.UpsertSitesServices(data)
	case "snippets":
		return db.UpsertSnippets(data)
	case "ssl":
		return db.UpsertSsl(data)
	case "sites_submissions":
		return db.UpsertSitesSubmissions(data)
	case "traffic_splits":
		return db.UpsertTrafficSplits(data)
	case "unlink_repo":
		return db.UpsertUnlinkRepo(data)
	case "submissions":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"billing":        "/billing/payment_methods",
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "audit", ParentTable: "accounts", ParentIDParam: "account_id", PathTemplate: "/accounts/{account_id}/audit", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.account", PathParams: []dependentPathParamDef{
			{Param: "account_id", Field: "id"},
		}},
		{Name: "dev_servers", ParentTable: "sites", ParentIDParam: "site_id", PathTemplate: "/sites/{site_id}/dev_servers", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.site", PathParams: []dependentPathParamDef{
			{Param: "site_id", Field: "id"},
		}},
		{Name: "sessions", ParentTable: "agent-runners", ParentIDParam: "agent_runner_id", PathTemplate: "/agent_runners/{agent_runner_id}/sessions", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.agent-runner", PathParams: []dependentPathParamDef{
			{Param: "agent_runner_id", Field: "id"},
		}},
		{Name: "sites_builds", ParentTable: "sites", ParentIDParam: "site_id", PathTemplate: "/sites/{site_id}/builds", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.site", PathParams: []dependentPathParamDef{
			{Param: "site_id", Field: "id"},
		}},
		{Name: "sites_deploys", ParentTable: "sites", ParentIDParam: "site_id", PathTemplate: "/sites/{site_id}/deploys", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.site", PathParams: []dependentPathParamDef{
			{Param: "site_id", Field: "id"},
		}},
		{Name: "sites_submissions", ParentTable: "sites", ParentIDParam: "site_id", PathTemplate: "/sites/{site_id}/submissions", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.site", PathParams: []dependentPathParamDef{
			{Param: "site_id", Field: "id"},
		}},
	}
}
...
func responsePathForResource(resource, path string) []string {
	...
	case "agent-runners\x00/agent_runners":
		return []string{"attached_file_keys"}
	case "agent-runners\x00/agent_runners/{agent_runner_id}":
		return []string{"attached_file_keys"}
	case "sites\x00/sites":
		return []string{"domain_aliases"}
	case "sites\x00/sites/{site_id}":
		return []string{"domain_aliases"}
	case "user\x00/user":
		return []string{"login_providers"}
	}
	return nil
}
