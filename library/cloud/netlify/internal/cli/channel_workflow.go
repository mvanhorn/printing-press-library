...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  netlify-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  netlify-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"accounts", "accounts-types", "billing", "deploy-keys", "dns-zones", "hooks-types", "services", "sites"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  netlify-pp-cli workflow status

  # Show status as JSON
  netlify-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("netlify-pp-cli")
			}
