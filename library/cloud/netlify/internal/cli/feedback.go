...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NETLIFY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NETLIFY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "netlify-pp-cli/feedback")
...
When ` + "`NETLIFY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NETLIFY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  netlify-pp-cli feedback list
  netlify-pp-cli feedback list --limit 5
  netlify-pp-cli feedback list --json`,
