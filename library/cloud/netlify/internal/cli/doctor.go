...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  netlify-pp-cli doctor
  netlify-pp-cli doctor --json
  netlify-pp-cli doctor --fail-on warn
  netlify-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "netlify-pp-cli login"
					report["auth_docs_url"] = "https://www.netlify.com/docs/api/"
				} else {
					authConfigured = true
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "NETLIFY_AUTH_TOKEN")
				if os.Getenv("NETLIFY_AUTH_TOKEN") != "" {
					authEnvSet = append(authEnvSet, "NETLIFY_AUTH_TOKEN")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("NETLIFY_AUTH_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "NETLIFY_AUTH_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "NETLIFY_AUTH_TOKEN optional")
			}
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "NETLIFY_NETLIFY_AUTH")
				if os.Getenv("NETLIFY_NETLIFY_AUTH") != "" {
					authEnvSet = append(authEnvSet, "NETLIFY_NETLIFY_AUTH")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("NETLIFY_NETLIFY_AUTH") != "" {
				authEnvSet = append(authEnvSet, "NETLIFY_NETLIFY_AUTH")
			} else {
				authEnvInfo = append(authEnvInfo, "NETLIFY_NETLIFY_AUTH optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "netlify-pp-cli"
						verifyPath := "/user"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'netlify-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'netlify-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'netlify-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'netlify-pp-cli sync' to refresh."
	}
	return report
}
