...
In local mode: searches locally synced data only.`,
  ...
  netlify-pp-cli search "error timeout"

  # Force local search only
  netlify-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  netlify-pp-cli search "status" --type accounts --data-source local
  # JSON output for piping
  netlify-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /sites/{site_id}/functions
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/sites/{site_id}/functions", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'netlify-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "accounts":
				results, err = db.SearchAccounts(query, limit)
			case "deploys":
				results, err = db.SearchDeploys(query, limit)
			case "services":
				results, err = db.SearchServices(query, limit)
			case "submissions":
				results, err = db.SearchSubmissions(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchAccounts(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search accounts failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchDeploys(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search deploys failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchSubmissions(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search submissions failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
