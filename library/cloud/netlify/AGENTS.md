# Netlify Printed CLI Agent Guide

This directory is a generated `netlify-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
netlify-pp-cli doctor --json
netlify-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
netlify-pp-cli which "<capability>" --json
...
```bash
netlify-pp-cli <command> --help
netlify-pp-cli <command> --dry-run --agent
```
