...
This directory is a generated `google-cloud-run-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
google-cloud-run-pp-cli doctor --json
google-cloud-run-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
google-cloud-run-pp-cli which "<capability>" --json
google-cloud-run-pp-cli <command> --help
```
...
```bash
google-cloud-run-pp-cli <command> --agent
```
...
```bash
google-cloud-run-pp-cli <command> --help
google-cloud-run-pp-cli <command> --dry-run --agent
```
