...
// StripHTMLBody detects HTML response bodies (indicated by a <!DOCTYPE or <html
// prefix, possibly preceded by whitespace) and replaces the body portion with a
// short placeholder. This prevents Google Cloud Run API HTML error pages — which
// include hundreds of lines of CSS, <style>, and <meta> tags — from flooding
// the terminal when a 404 response body is included in an error message.
//
// The error message format from the client layer is:
//
//	"HTTP 404: <status-line>: <response-body>"
//
// Only the response body is stripped; the HTTP status and API message are
// preserved so users still see the actionable part of the error.
func StripHTMLBody(msg string) string {
	lower := strings.ToLower(msg)
	for _, marker := range []string{"<!doctype", "<html"} {
		if idx := strings.Index(lower, marker); idx != -1 {
			return msg[:idx] + "[HTML response body omitted]"
		}
	}
	return msg
}
