...
type Config struct {
	...
	Path                  string `toml:"-"`
	GoogleCloudRunToken   string `toml:"cloud_run_token"`
	GoogleCloudRunOauth2c string `toml:"cloud_run_oauth2c"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("GOOGLE_CLOUD_RUN_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "google-cloud-run-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("GOOGLE_CLOUD_RUN_TOKEN"); v != "" {
		cfg.GoogleCloudRunToken = v
		cfg.AuthSource = "env:GOOGLE_CLOUD_RUN_TOKEN"
	}
	if v := os.Getenv("GOOGLE_CLOUD_RUN_OAUTH2C"); v != "" {
		cfg.GoogleCloudRunOauth2c = v
		cfg.AuthSource = "env:GOOGLE_CLOUD_RUN_OAUTH2C"
	}
	...
	if cfg.AuthSource == "" && cfg.GoogleCloudRunToken != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.GoogleCloudRunOauth2c != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("GOOGLE_CLOUD_RUN_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("GOOGLE_CLOUD_RUN_AUTHORIZATION_URL"); v != "" {
		cfg.AuthorizationURL = v
	}
	if v := os.Getenv("GOOGLE_CLOUD_RUN_TOKEN_URL"); v != "" {
		cfg.TokenURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.GoogleCloudRunToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:GOOGLE_CLOUD_RUN_TOKEN"
		}
		return "Bearer " + c.GoogleCloudRunToken
	}
	if c.GoogleCloudRunOauth2c != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:GOOGLE_CLOUD_RUN_OAUTH2C"
		}
		return "Bearer " + c.GoogleCloudRunOauth2c
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.ClientID = ""
	c.ClientSecret = ""
	c.GoogleCloudRunToken = ""
	c.GoogleCloudRunOauth2c = ""
	return c.save()
}
