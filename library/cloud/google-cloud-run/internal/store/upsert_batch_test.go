...
func TestUpsertBatch_PopulatesCloudRunAdminJobsTable(t *testing.T) {
	...
	if _, _, err := s.UpsertBatch("cloud-run-admin-jobs", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "cloud-run-admin-jobs").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
...
// TestUpsertBatch_PopulatesCloudRunAdminJobsExecutionsTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed cloud_run_admin_jobs_executions table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesCloudRunAdminJobsExecutionsTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "cloud_run_admin_jobs_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "cloud_run_admin_jobs_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "cloud_run_admin_jobs_id": "test-parent-001"}`),
	}
	if _, _, err := s.UpsertBatch("cloud_run_admin_jobs_executions", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "cloud_run_admin_jobs_executions").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "cloud_run_admin_jobs_executions")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count cloud_run_admin_jobs_executions: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("cloud_run_admin_jobs_executions count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandCloudRunAdminJobsExecutionsGeneric exercises
...
func TestUpsertBatch_TypedFailureDoesNotStrandCloudRunAdminJobsExecutionsGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// Items deliberately omit "cloud_run_admin_jobs_id" so the typed NOT NULL
	// constraint fires.
	items := []json.RawMessage{
	...
	stored, extractFailures, err := s.UpsertBatch("cloud_run_admin_jobs_executions", items)
	if err != nil {
	...
	if typed != 0 {
		t.Fatalf("cloud_run_admin_jobs_executions count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "cloud_run_admin_jobs_id")
	}
}
...
func TestUpsertBatch_TypedFailureDoesNotStrandTasksGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("tasks count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "executions_id")
	}
}
...
func TestUpsertBatch_TypedFailureDoesNotStrandRevisionsGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("revisions count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "services_id")
	}
}
