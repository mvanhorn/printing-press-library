...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  google-cloud-run-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  google-cloud-run-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  google-cloud-run-pp-cli workflow status

  # Show status as JSON
  google-cloud-run-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("google-cloud-run-pp-cli")
			}
