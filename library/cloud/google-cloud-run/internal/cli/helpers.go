...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			"\n      Set your API key: export GOOGLE_CLOUD_RUN_TOKEN=<your-key>"+
			"\n      See API docs: https://google.com"+
			"\n      Run 'google-cloud-run-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token. Set it with: google-cloud-run-pp-cli auth set-token <token>"+
			"\n      or: export GOOGLE_CLOUD_RUN_TOKEN=<your-token>"+
			"\n      See API docs: https://google.com"+
			"\n      Run 'google-cloud-run-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: permission denied. Your credentials are valid but lack access to this resource."+
			"\n      Check that your API key has the required permissions."+
			"\n      Set it with: export GOOGLE_CLOUD_RUN_TOKEN=<your-key>"+
			"\n      See API docs: https://google.com"+
			"\n      Run 'google-cloud-run-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 404"):
		// Strip HTML response bodies (Google's default 404 error page) before
		// wrapping — without this, hundreds of lines of CSS and markup appear
		// above the actionable hint whenever the API returns an HTML error page.
		strippedErr := fmt.Errorf("%s", cliutil.StripHTMLBody(msg))
		return notFoundErr(fmt.Errorf("%w\nhint: resource not found. Run the 'list' command to see available items", strippedErr))
