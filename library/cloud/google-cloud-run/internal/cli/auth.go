...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  google-cloud-run-pp-cli auth setup\n  google-cloud-run-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "See API docs for OAuth app registration: https://google.com")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then run:")
			fmt.Fprintln(w, "  google-cloud-run-pp-cli login")
			if !launch {
				return nil
			}
...
func newAuthLoginCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.Flags().StringVar(&clientID, "client-id", os.Getenv("GOOGLE_CLOUD_RUN_CLIENT_ID"), "OAuth2 client ID")
	cmd.Flags().StringVar(&clientSecret, "client-secret", os.Getenv("GOOGLE_CLOUD_RUN_CLIENT_SECRET"), "OAuth2 client secret")
...
func runOAuthLogin(cmd *cobra.Command, flags *rootFlags, clientID, clientSecret string, port int) error {
	...
	// Short-circuit BEFORE resolving credentials, binding the callback port, or
	// opening a browser, so verify / validate-narrative neither requires real
	// OAuth credentials (which would fail with "client ID required" in CI) nor
	// binds 127.0.0.1:<port> (which would EADDRINUSE on a parallel run or
	// occupied port) nor launches a browser (which would time out).
	if cliutil.IsVerifyEnv() {
...
func resolveOAuthCredentials(cmd *cobra.Command, flags *rootFlags, cfg *config.Config, clientID, clientSecret string) (string, string, error) {
	...
	if flags.noInput {
		return "", "", fmt.Errorf("OAuth2 client ID is required; pass --client-id, set GOOGLE_CLOUD_RUN_CLIENT_ID, or run 'google-cloud-run-pp-cli login' without --no-input")
	}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				fmt.Fprintf(w, "  %s Not authenticated. Run 'google-cloud-run-pp-cli login' to authenticate.\n", red("FAIL"))
				return nil
			}
			...
			if cfg.TokenExpiry.IsZero() {
				...
				if cfg.RefreshToken != "" {
					...
					fmt.Fprintf(w, "  %s Token expired. Run 'google-cloud-run-pp-cli login' to re-authenticate.\n", red("FAIL"))
				}
			}
