...
Resource scoping:
  ...
  google-cloud-run-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  google-cloud-run-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  google-cloud-run-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  google-cloud-run-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  google-cloud-run-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "google-cloud-run-pp-cli sync: no bulk-list endpoints in spec; populate the store via single-fetch commands instead.")
				} else {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/google-cloud-run-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "cloud_run_admin_jobs_executions":
		return db.UpsertCloudRunAdminJobsExecutions(data)
	case "tasks":
