...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  google-cloud-run-pp-cli doctor
  google-cloud-run-pp-cli doctor --json
  google-cloud-run-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "google-cloud-run-pp-cli login"
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "GOOGLE_CLOUD_RUN_TOKEN")
				if os.Getenv("GOOGLE_CLOUD_RUN_TOKEN") != "" {
					authEnvSet = append(authEnvSet, "GOOGLE_CLOUD_RUN_TOKEN")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("GOOGLE_CLOUD_RUN_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "GOOGLE_CLOUD_RUN_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "GOOGLE_CLOUD_RUN_TOKEN optional")
			}
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "GOOGLE_CLOUD_RUN_OAUTH2C")
				if os.Getenv("GOOGLE_CLOUD_RUN_OAUTH2C") != "" {
					authEnvSet = append(authEnvSet, "GOOGLE_CLOUD_RUN_OAUTH2C")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("GOOGLE_CLOUD_RUN_OAUTH2C") != "" {
				authEnvSet = append(authEnvSet, "GOOGLE_CLOUD_RUN_OAUTH2C")
			} else {
				authEnvInfo = append(authEnvInfo, "GOOGLE_CLOUD_RUN_OAUTH2C optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "google-cloud-run-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("google-cloud-run-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'google-cloud-run-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'google-cloud-run-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'google-cloud-run-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'google-cloud-run-pp-cli sync' to refresh."
	}
	return report
}
