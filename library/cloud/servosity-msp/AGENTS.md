# Servosity Msp Printed CLI Agent Guide

This directory is a generated `servosity-msp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
servosity-msp-cli doctor --json
servosity-msp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
servosity-msp-cli which "<capability>" --json
servosity-msp-cli <command> --help
```
...
```bash
servosity-msp-cli <command> --agent
```
...
```bash
servosity-msp-cli <command> --help
servosity-msp-cli <command> --dry-run --agent
```
