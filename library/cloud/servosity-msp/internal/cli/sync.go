...
Resource scoping:
  ...
  servosity-msp-cli sync

  # Sync specific resources only
  servosity-msp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  servosity-msp-cli sync --full

  # Incremental sync: only records from the last 7 days
  servosity-msp-cli sync --since 7d

  # Parallel sync with 8 workers
  servosity-msp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  servosity-msp-cli sync --latest-only`,
...
func resourceSupportsPagination(resource string) bool {
	...
	case "backups":
		return true
	case "backups-mfa-codes":
		return true
	case "companies":
		return true
	case "companies-fully-managed":
		return true
	case "companies-summary":
		return true
	case "companies-summary-ng":
		return true
	case "company-notes":
		return true
	case "contracts":
		return true
	case "contracts-signatures":
		return true
	case "credentials":
		return true
	case "dr-backups":
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "restic_interrupt":
		return db.UpsertResticInterrupt(data)
	case "restic_migrate":
