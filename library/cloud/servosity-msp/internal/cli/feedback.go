...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".servosity-msp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SERVOSITY_MSP_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SERVOSITY_MSP_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "servosity-msp-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.servosity-msp-cli/feedback.jsonl.
When ` + "`SERVOSITY_MSP_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SERVOSITY_MSP_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  servosity-msp-cli feedback list
  servosity-msp-cli feedback list --limit 5
  servosity-msp-cli feedback list --json`,
