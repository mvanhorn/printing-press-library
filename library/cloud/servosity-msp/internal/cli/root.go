...
Highlights (not in the official API docs):
  • attention   One screen across your whole book of clients. Merges open issues, stale backups, and in-flight DR events into a per-company ranked view, then persists the result so tomorrow's drift command can compa…
  ...
  • backup-facts   Unified view across Servosity's three backup engines (classic, restic, DR) for one company or all. Engine, ID, hostname, last_successful_at, status, size — joined from three local store tables into o…
...
Run 'servosity-msp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("servosity-msp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'servosity-msp-cli profile list')")
	...
	rootCmd.AddCommand(newStorageTrendCmd(flags))
	rootCmd.AddCommand(newAttentionCmd(flags))
	rootCmd.AddCommand(newQBRCmd(flags))
	...
	rootCmd.AddCommand(newUnprovisionedCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("servosity-msp-cli %s\n", version)
		},
	}
}
