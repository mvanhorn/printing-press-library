...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.servosity.com/api/v1",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("SERVOSITY_MSP_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "servosity-msp-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("SERVOSITY_MSP_TOKEN"); v != "" {
		cfg.ServosityMspToken = v
		cfg.AuthSource = "env:SERVOSITY_MSP_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.ServosityMspToken != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("SERVOSITY_MSP_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.ServosityMspToken
	if token == "" {
		return ""
	}
	// Servosity uses Django REST Framework's TokenAuthentication, which
	// requires the literal scheme prefix "Token " before the key in the
	// Authorization header. Without this prefix, the server returns
