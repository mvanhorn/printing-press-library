...
type Config struct {
	...
	ClickhouseCloudUsername string          `toml:"cloud_username"`
	ClickhouseCloudPassword string          `toml:"cloud_password"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("CLICKHOUSE_CLOUD_USERNAME"); v != "" {
		cfg.ClickhouseCloudUsername = v
		cfg.markEnvOverride("ClickhouseCloudUsername")
		cfg.AuthSource = "env:CLICKHOUSE_CLOUD_USERNAME"
		cfg.CredentialSource = "env:CLICKHOUSE_CLOUD_USERNAME"
	}
	if v := os.Getenv("CLICKHOUSE_CLOUD_PASSWORD"); v != "" {
		cfg.ClickhouseCloudPassword = v
		cfg.markEnvOverride("ClickhouseCloudPassword")
		cfg.AuthSource = "env:CLICKHOUSE_CLOUD_PASSWORD"
		cfg.CredentialSource = "env:CLICKHOUSE_CLOUD_PASSWORD"
	}
	...
	if v := os.Getenv("CLICKHOUSE_CLOUD_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("CLICKHOUSE_CLOUD_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "clickhouse-cloud-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.ClickhouseCloudUsername == "" || c.ClickhouseCloudPassword == "" {
		return ""
	}
	credentials := c.ClickhouseCloudUsername + ":" + c.ClickhouseCloudPassword
...
func (c *Config) hasCredentialFields() bool {
	...
	if c.ClickhouseCloudUsername != "" {
		return true
	}
	if c.ClickhouseCloudPassword != "" {
		return true
	}
	return false
}
...
func (c *Config) clearCredentialFields() {
	...
	c.ClientID = ""
	c.ClientSecret = ""
	c.ClickhouseCloudUsername = ""
	c.ClickhouseCloudPassword = ""
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		ClickhouseCloudUsername: c.ClickhouseCloudUsername,
		ClickhouseCloudPassword: c.ClickhouseCloudPassword,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.ClickhouseCloudUsername = creds.ClickhouseCloudUsername
	c.ClickhouseCloudPassword = creds.ClickhouseCloudPassword
}
...
func (c *Config) SaveCredential(_ string) error {
	return fmt.Errorf("SaveCredential is not supported for ClickHouse Cloud; use SaveBasicCredentials(keyID, keySecret)")
}
...
func (c *Config) SaveBasicCredentials(username, password string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.ClickhouseCloudUsername = username
	c.ClickhouseCloudPassword = password
	delete(c.envOverrides, "ClickhouseCloudUsername")
	delete(c.envOverrides, "ClickhouseCloudPassword")
	c.updateFileConfigField("ClickhouseCloudUsername")
	c.updateFileConfigField("ClickhouseCloudPassword")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["ClickhouseCloudUsername"] {
			out.ClickhouseCloudUsername = c.fileConfig.ClickhouseCloudUsername
		}
		if c.envOverrides["ClickhouseCloudPassword"] {
			out.ClickhouseCloudPassword = c.fileConfig.ClickhouseCloudPassword
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "ClickhouseCloudUsername":
		c.fileConfig.ClickhouseCloudUsername = c.ClickhouseCloudUsername
	case "ClickhouseCloudPassword":
		c.fileConfig.ClickhouseCloudPassword = c.ClickhouseCloudPassword
	}
}
