...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  clickhouse-cloud-pp-cli doctor
  clickhouse-cloud-pp-cli doctor --json
  clickhouse-cloud-pp-cli doctor --fail-on warn
  clickhouse-cloud-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set Basic credentials with: export CLICKHOUSE_CLOUD_USERNAME=\"your-username\" CLICKHOUSE_CLOUD_PASSWORD=\"your-password\""
					report["auth_key_url"] = "https://clickhouse.com/docs/cloud/manage/openapi"
				} else {
					authConfigured = true
			...
			if os.Getenv("CLICKHOUSE_CLOUD_USERNAME") != "" {
				authEnvSet = append(authEnvSet, "CLICKHOUSE_CLOUD_USERNAME")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "CLICKHOUSE_CLOUD_USERNAME")
			}
			if os.Getenv("CLICKHOUSE_CLOUD_PASSWORD") != "" {
				authEnvSet = append(authEnvSet, "CLICKHOUSE_CLOUD_PASSWORD")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "CLICKHOUSE_CLOUD_PASSWORD")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "clickhouse-cloud-pp-cli", suggestion)
						} else {
...
func collectCredentialsLocationReport(report map[string]any, cfg *config.Config) {
	...
	if credsPresent && len(locations) > 1 {
		if legacySecretsElsewhere != "" {
			report["credentials_location_warning"] = "WARN credentials stored in more than one location; legacy secrets remain at " + legacySecretsElsewhere + "; run auth set-token <key-id> <key-secret> or auth logout to consolidate and remove legacy secrets"
		} else {
			report["credentials_location_warning"] = "WARN credentials stored in more than one location; current reads use credentials file; run auth set-token <key-id> <key-secret> or auth logout to consolidate"
		}
	}
}
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("clickhouse-cloud-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'clickhouse-cloud-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'clickhouse-cloud-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'clickhouse-cloud-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'clickhouse-cloud-pp-cli sync' to refresh."
	}
	return report
}
