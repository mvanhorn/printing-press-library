...
Resource scoping:
  ...
  clickhouse-cloud-pp-cli sync

  # Sync specific resources only
  clickhouse-cloud-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  clickhouse-cloud-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  clickhouse-cloud-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  clickhouse-cloud-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  clickhouse-cloud-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "byoc-infrastructure":
		return db.UpsertByocInfrastructure(data)
	case "invitations":
	...
	case "postgres":
		return db.UpsertPostgres(data)
	case "ca_certificates":
		return db.UpsertCaCertificates(data)
	case "config":
		return db.UpsertConfig(data)
	case "metrics":
		return db.UpsertMetrics(data)
	case "postgres_password":
		return db.UpsertPostgresPassword(data)
	case "postgres_prometheus":
		return db.UpsertPostgresPrometheus(data)
	case "read_replica":
		return db.UpsertReadReplica(data)
	case "restored_service":
		return db.UpsertRestoredService(data)
	case "slow_query_patterns":
		return db.UpsertSlowQueryPatterns(data)
	case "postgres_state":
		return db.UpsertPostgresState(data)
	case "roles":
		return db.UpsertRoles(data)
	case "services":
		return db.UpsertServices(data)
	case "backup_bucket":
		return db.UpsertBackupBucket(data)
	case "backup_configuration":
		return db.UpsertBackupConfiguration(data)
	case "backups":
		return db.UpsertBackups(data)
	case "clickhouse_settings":
		return db.UpsertClickhouseSettings(data)
	case "clickpipes":
		return db.UpsertClickpipes(data)
	case "clickpipes_cdc_scaling":
		return db.UpsertClickpipesCdcScaling(data)
	case "clickpipes_reverse_private_endpoints":
		return db.UpsertClickpipesReversePrivateEndpoints(data)
	case "clickstack":
		return db.UpsertClickstack(data)
	case "services_password":
		return db.UpsertServicesPassword(data)
	case "private_endpoint":
		return db.UpsertPrivateEndpoint(data)
	case "services_private_endpoint_config":
		return db.UpsertServicesPrivateEndpointConfig(data)
	case "services_prometheus":
		return db.UpsertServicesPrometheus(data)
	case "replica_scaling":
		return db.UpsertReplicaScaling(data)
	case "scaling":
		return db.UpsertScaling(data)
	case "scaling_schedule":
		return db.UpsertScalingSchedule(data)
	case "service_query_endpoint":
		return db.UpsertServiceQueryEndpoint(data)
	case "services_state":
		return db.UpsertServicesState(data)
	case "upgrade_window":
		return db.UpsertUpgradeWindow(data)
	default:
