...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  clickhouse-cloud-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  clickhouse-cloud-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  clickhouse-cloud-pp-cli workflow status

  # Show status as JSON
  clickhouse-cloud-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("clickhouse-cloud-pp-cli")
			}
