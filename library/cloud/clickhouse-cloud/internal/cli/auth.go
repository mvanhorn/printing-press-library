...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Clickhouse Cloud",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  clickhouse-cloud-pp-cli auth setup\n  clickhouse-cloud-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://clickhouse.com/docs/cloud/manage/openapi")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export CLICKHOUSE_CLOUD_USERNAME=\"your-key-id\"")
			fmt.Fprintln(w, "  export CLICKHOUSE_CLOUD_PASSWORD=\"your-key-secret\"")
			if !launch {
				return nil
			}
			launchURL := "https://clickhouse.com/docs/cloud/manage/openapi"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  clickhouse-cloud-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		Use:     "set-token <key-id> <key-secret>",
		Short:   "Save ClickHouse Cloud API key credentials to the credentials file",
		Example: "  clickhouse-cloud-pp-cli auth set-token YOUR_KEY_ID YOUR_KEY_SECRET",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  clickhouse-cloud-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("CLICKHOUSE_CLOUD_USERNAME") != "" {
				envStillSet = "CLICKHOUSE_CLOUD_USERNAME"
			}
			if envStillSet == "" && os.Getenv("CLICKHOUSE_CLOUD_PASSWORD") != "" {
				envStillSet = "CLICKHOUSE_CLOUD_PASSWORD"
			}
