...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CLICKHOUSE_CLOUD_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CLICKHOUSE_CLOUD_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "clickhouse-cloud-pp-cli/feedback")
...
When ` + "`CLICKHOUSE_CLOUD_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CLICKHOUSE_CLOUD_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  clickhouse-cloud-pp-cli feedback list
  clickhouse-cloud-pp-cli feedback list --limit 5
  clickhouse-cloud-pp-cli feedback list --json`,
