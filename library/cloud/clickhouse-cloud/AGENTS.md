# Clickhouse Cloud Printed CLI Agent Guide

This directory is a generated `clickhouse-cloud-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
clickhouse-cloud-pp-cli doctor --json
clickhouse-cloud-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
clickhouse-cloud-pp-cli which "<capability>" --json
...
```bash
clickhouse-cloud-pp-cli <command> --agent
```
...
```bash
clickhouse-cloud-pp-cli <command> --help
clickhouse-cloud-pp-cli <command> --dry-run --agent
```
