...
import (
	...
	"cloudflare-pp-cli/internal/client"
	"cloudflare-pp-cli/internal/config"
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		...
		Short: `Cloudflare CLI — The unified Cloudflare CLI agents and operators both want — every product covered, with offline cache, agent-native out…`,
		Long: `Cloudflare CLI — The unified Cloudflare CLI agents and operators both want — every product covered, with offline cache, agent-native out…
...
Highlights (not in the official API docs):
  ...
  • where-is   Find every place a hostname appears across DNS records, Worker routes, Page Rules, Access apps, and Custom Hostnames in one command.
...
Health check: run 'cloudflare-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
	...
	zonesCmd := newZonesCmd(flags)
	zonesCmd.AddCommand(newZonesDiffCmd(flags))
	...
	// Transcendence commands — see internal/cli/novel_*.go.
	// `zones diff` is wired above as a child of the existing zones command.
	rootCmd.AddCommand(newDnsCmd(flags))
	rootCmd.AddCommand(newRedirectCmd(flags))
	rootCmd.AddCommand(newPropagateCmd(flags))
	rootCmd.AddCommand(newWhereIsCmd(flags))
	rootCmd.AddCommand(newWorkerCmd(flags))
	rootCmd.AddCommand(newCacheCmd(flags))
	rootCmd.AddCommand(newSetupZoneCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("cloudflare-pp-cli %s\n", version)
		},
	}
}
