  ...
  cloudflare-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				legacyEmail := cfg.LegacyAuthEmail()
				legacyKey := cfg.LegacyAuthKey()
				if header == "" && (legacyEmail == "" || legacyKey == "") {
					report["auth"] = "not configured"
					report["auth_hint"] = "export CLOUDFLARE_API_TOKEN=<your-token> (preferred) — or set CLOUDFLARE_API_EMAIL + CLOUDFLARE_API_KEY for legacy global-key auth"
				} else {
					report["auth"] = "configured"
					report["auth_source"] = cfg.AuthSource
					if header != "" {
						report["auth_mode"] = "api_token"
					} else {
						report["auth_mode"] = "legacy_email_key"
					}
				}
			}

			// Check auth environment variables — Cloudflare supports two modes:
			// CLOUDFLARE_API_TOKEN (preferred, Bearer) OR
			// CLOUDFLARE_API_EMAIL + CLOUDFLARE_API_KEY (legacy global key).
			authEnvSet := []string{}
			authEnvRequiredMissing := []string{}
			authEnvInfo := []string{}
			authEnvOptionalNames := []string{"CLOUDFLARE_API_TOKEN", "CLOUDFLARE_API_EMAIL+CLOUDFLARE_API_KEY"}
			authEnvOptionalSatisfied := false
			if os.Getenv("CLOUDFLARE_API_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "CLOUDFLARE_API_TOKEN")
				authEnvOptionalSatisfied = true
			}
			if os.Getenv("CLOUDFLARE_API_EMAIL") != "" && os.Getenv("CLOUDFLARE_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "CLOUDFLARE_API_EMAIL+CLOUDFLARE_API_KEY")
				authEnvOptionalSatisfied = true
			}
			if !authEnvOptionalSatisfied {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "CLOUDFLARE_API_TOKEN (or CLOUDFLARE_API_EMAIL+CLOUDFLARE_API_KEY)")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["X-Auth-Email"] = authHeader
