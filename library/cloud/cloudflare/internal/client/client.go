...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "cloudflare-pp-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		// Cloudflare auth: API Token (preferred) → Authorization: Bearer <token>.
		// Legacy email+key → X-Auth-Email + X-Auth-Key. Token mode wins when
		// both are set so users migrating to scoped tokens work cleanly.
		if authHeader != "" && strings.HasPrefix(authHeader, "Bearer ") {
			req.Header.Set("Authorization", authHeader)
		} else if c.Config != nil {
			if email := c.Config.LegacyAuthEmail(); email != "" {
				req.Header.Set("X-Auth-Email", email)
			}
			if key := c.Config.LegacyAuthKey(); key != "" {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/cloud/cloudflare/4.0.0")
		}
