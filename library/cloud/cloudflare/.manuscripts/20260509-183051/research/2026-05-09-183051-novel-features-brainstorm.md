...
**Persona 1 — Alex (the multi-zone solo operator)**
- **Today:** runs MakerToo on a 4-VPS Coolify fleet, mid-sprint on klinikalarm.dk DNS. Owns a stack of zones (makertoo.com, makertoo.win, beamstelbay.net, klinikalarm.dk, mitlæge.dk, maker2.com) all under one Cloudflare account. Switches between dashboard tabs constantly to copy A records between zones, set up redirects, purge cache after deploys.
- **Weekly ritual:** every new Coolify deploy needs a Cloudflare A record + sometimes a page rule + cache purge. Currently three dashboard tabs and a curl scratchpad. Re-types `157.173.102.165` from memory.
- **Frustration:** "I just shipped Klinik-Alarm and I need (a) A record, (b) page-rule redirect for mitlæge.dk → klinikalarm.dk, (c) confirm propagation, (d) purge cache. That's four tabs and 15 minutes. Why isn't this `cloudflare dns add` + `cloudflare redirect set` + done?"
...
- **Today:** an agent (often Imperium/Omni or Claude Code) is told "set up DNS for klinikalarm.dk." It doesn't know the zone_id, doesn't know if the record exists, and doesn't want to burn 1k tokens loading per-endpoint MCP tools. Cloudflare's own Code Mode MCP solved this for itself but other CLIs haven't.
...
|---|---------|---------|-------|-------------|----------|
| 1 | Idempotent DNS apply | `cloudflare dns apply --zone <z> --type <t> --name <n> --content <c> [--proxied]` | 10/10 | `GET /zones/{id}/dns_records?name=&type=` then PATCH-or-POST based on content equality; no-op if identical | Brief Top Workflow #1; flarectl errors on duplicate; user mid-sprint on klinikalarm.dk needs this exact command |
| 2 | Semantic redirect shortcut | `cloudflare redirect set "<from-pattern>" "<to-template>" --status 301` | 10/10 | Composes a `/zones/{id}/pagerules` POST with forwarding_url action + correct $1 templating, places at priority 1 | Brief User Vision section names mitlæge.dk → klinikalarm.dk redirect; Page Rules JSON shape is the friction nobody wraps |
| 3 | DNS propagation verifier | `cloudflare propagate watch <name> <type> --expect <value>` | 9/10 | Fans out DoH queries to 1.1.1.1, 8.8.8.8, 9.9.9.9 (free public no-auth resolvers); exits 0 when all match | Brief lists verification as part of klinikalarm.dk sprint; no incumbent CLI does multi-resolver verification |
