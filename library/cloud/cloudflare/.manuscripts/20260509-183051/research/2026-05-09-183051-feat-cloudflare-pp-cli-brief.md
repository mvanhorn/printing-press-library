...
## User Vision
The user runs MakerToo (consultancy, Copenhagen) on a 4-VPS Coolify fleet and is mid-sprint on klinikalarm.dk DNS configuration. Near-term need: Cloudflare DNS A-record setup for `klinikalarm.dk` → 157.173.102.165, page rule `mitlæge.dk/*` → 301 redirect, Coolify domain attach + Let's Encrypt SSL. This CLI should make that workflow trivial: one command to add DNS, one to add the page rule, one to verify propagation.
