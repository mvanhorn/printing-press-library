...
3. **Novel-feature dry-run made API calls before short-circuit**: validate-narrative caught it — examples like `setup_zone klinikalarm.dk --origin ... --dry-run` tried to look up the zone on the API and failed with "zone not found" (because verify env has no real auth). Patched all 7 mutating/composing novel commands to short-circuit `dryRunOK(flags)` BEFORE any API call.
...
5. **Python `cp1252` codec failed on `mitlæge.dk` (æ)**: Set `PYTHONUTF8=1` for shipcheck invocation.
