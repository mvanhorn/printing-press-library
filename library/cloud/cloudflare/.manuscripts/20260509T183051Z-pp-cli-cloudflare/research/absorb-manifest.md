...
|---|---------|---------|-------|-------------|----------|
| 1 | Idempotent DNS apply | `cloudflare dns apply --zone <z> --type <t> --name <n> --content <c> [--proxied]` | 10/10 | `GET /zones/{id}/dns_records?name=&type=` then PATCH-or-POST based on content equality; no-op if identical | Brief Top Workflow #1; flarectl errors on duplicate; user mid-sprint on example.com |
...
All 8 survivors >= 8/10. Buildability proofs and persona attribution recorded in `novel-features-brainstorm.md`.
