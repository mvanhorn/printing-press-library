...
**Unified Cloudflare CLI for agents and operators. Initial v1 covers the high-traffic product areas — DNS, Workers (with KV / D1 / R2), Pages, Zones, Cache, Page Rules, Rulesets, SSL/TLS, IAM, API Tokens — plus 8 transcendence commands not in any other CLI. Offline cache, agent-native JSON output, Cloudflare-pattern MCP surface.**

Wrangler is Workers-only, flarectl is dormant, Terraform is declarative-only. This CLI absorbs the high-usage parts of all three and adds cross-product transcendence: idempotent DNS apply, semantic redirects, multi-resolver propagation watch, where-is, zone drift diff. MCP exposes the surface through a thin search+execute pair (the Cloudflare pattern) so agents don't burn tokens loading per-endpoint tools. Full Cloudflare API coverage (Stream, Images, Magic Transit, Zero Trust, etc.) is intentionally deferred to follow-up PRs once the core lands.
...
cloudflare-pp-cli setup_zone example.com --origin 203.0.113.10


# verify global DNS propagation across public resolvers
cloudflare-pp-cli propagate watch example.com A --expect 203.0.113.10
