...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "stac-pp-cli",
		Short: `Stac CLI — The first Go single-binary STAC CLI: every search filter the Python tools have, plus an offline cache, scene ranking, c…`,
		Long: `Stac CLI — The first Go single-binary STAC CLI: every search filter the Python tools have, plus an offline cache, scene ranking, c…
...
Health check: run 'stac-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("stac-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'stac-pp-cli profile list')")
	...
	rootCmd.AddCommand(newAggregationsCmd(flags))
	rootCmd.AddCommand(newCollectionsCmd(flags))
	rootCmd.AddCommand(newItemsCmd(flags))
	rootCmd.AddCommand(newQueryablesCmd(flags))
	...
	rootCmd.AddCommand(newNovelCloudsCmd(flags))
	...
	rootCmd.AddCommand(newNovelScenesCmd(flags))
	rootCmd.AddCommand(newNovelStackSnippetCmd(flags))
	rootCmd.AddCommand(newNovelTimelineCmd(flags))
	rootCmd.AddCommand(newNovelWatchCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newAggregatePromotedCmd(flags))
