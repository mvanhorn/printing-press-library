...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("STAC_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("STAC_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "stac-pp-cli/feedback")
...
When ` + "`STAC_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`STAC_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  stac-pp-cli feedback list
  stac-pp-cli feedback list --limit 5
  stac-pp-cli feedback list --json`,
