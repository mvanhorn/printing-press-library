...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  stac-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  stac-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"aggregate", "aggregations", "queryables"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  stac-pp-cli workflow status

  # Show status as JSON
  stac-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("stac-pp-cli")
			}
