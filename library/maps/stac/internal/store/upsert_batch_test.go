...
// TestUpsertBatch_KeysCollectionsItemsByChildAndParent verifies dependent
...
func TestUpsertBatch_KeysCollectionsItemsByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("collections_items count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "collections_id" = ?`, "collections_items")
...
func TestUpsertBatch_SetsCollectionsItemsParentID(t *testing.T) {
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "child-001", "collections_id": "parent-A", "parent_id": "parent-A"}`),
		json.RawMessage(`{"id": "child-002", "collections_id": "parent-A", "parent_id": "parent-A"}`),
		json.RawMessage(`{"id": "child-003", "collections_id": "parent-B", "parent_id": "parent-B"}`),
	}
	if _, _, err := s.UpsertBatch("collections_items", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
...
// TestUpsertBatch_CollectionsItemsSearchableViaGenericFTS verifies that a
...
// non-FTS5 `--type collections_items` fallback), so populating it is the
...
func TestUpsertBatch_CollectionsItemsSearchableViaGenericFTS(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// field is matchable regardless of which columns collections_items declares.
	const probe = "zqxjpp2629probe"
	items := []json.RawMessage{
		json.RawMessage(`{"id": "search-001", "pp_fts_probe": "` + probe + `", "collections_id": "parent-A", "parent_id": "parent-A"}`),
	}
	stored, _, err := s.UpsertBatch("collections_items", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
