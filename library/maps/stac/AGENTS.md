# STAC Printed CLI Agent Guide

This directory is a generated `stac-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
stac-pp-cli doctor --json
stac-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
stac-pp-cli which "<capability>" --json
stac-pp-cli <command> --help
```
...
```bash
stac-pp-cli <command> --help
stac-pp-cli <command> --dry-run --agent
```
