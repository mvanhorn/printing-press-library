...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "memberstack-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MEMBERSTACK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MEMBERSTACK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "memberstack-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/memberstack-pp-cli/feedback.jsonl.
When ` + "`MEMBERSTACK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MEMBERSTACK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  memberstack-pp-cli feedback list
  memberstack-pp-cli feedback list --limit 5
  memberstack-pp-cli feedback list --json`,
