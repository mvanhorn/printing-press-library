...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  memberstack-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  memberstack-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"data-tables", "members"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  memberstack-pp-cli workflow status

  # Show status as JSON
  memberstack-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("memberstack-pp-cli")
			}
