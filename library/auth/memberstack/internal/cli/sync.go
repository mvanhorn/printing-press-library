...
Resource scoping:
  ...
  memberstack-pp-cli sync

  # Sync specific resources only
  memberstack-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  memberstack-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  memberstack-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  memberstack-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  memberstack-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/memberstack-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "add_plan":
		return db.UpsertAddPlan(data)
	case "remove_plan":
		return db.UpsertRemovePlan(data)
	default:
