...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "memberstack-pp-cli",
		Short: `Memberstack CLI — The first CLI for Memberstack — every Admin API endpoint plus stale-member detection, plan coverage, JWT decode, bulk o…`,
		Long: `Memberstack CLI — The first CLI for Memberstack — every Admin API endpoint plus stale-member detection, plan coverage, JWT decode, bulk o…
...
Health check: run 'memberstack-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("memberstack-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'memberstack-pp-cli profile list')")
	...
	rootCmd.AddCommand(newDataTablesCmd(flags))
	...
	// Hand-written novel commands (transcendence). See AGENTS.md "Hand-edits
	// must be regen-mergeable" — each lives in its own file so regen-merge
	// preserves them automatically.
	rootCmd.AddCommand(newTokenCmd(flags))
	rootCmd.AddCommand(newStaleCmd(flags))
	rootCmd.AddCommand(newPlanCoverageCmd(flags))
