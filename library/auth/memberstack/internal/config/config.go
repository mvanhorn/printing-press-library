...
type Config struct {
	...
	Path                 string            `toml:"-"`
	MemberstackSecretKey string            `toml:"secret_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("MEMBERSTACK_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "memberstack-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("MEMBERSTACK_SECRET_KEY"); v != "" {
		cfg.MemberstackSecretKey = v
		cfg.AuthSource = "env:MEMBERSTACK_SECRET_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.MemberstackSecretKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("MEMBERSTACK_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.MemberstackSecretKey
	if token == "" {
		return ""
	}
	if c.MemberstackSecretKey == "" {
		return ""
	}
	return token
}
