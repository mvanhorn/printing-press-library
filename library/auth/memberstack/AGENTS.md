# Memberstack Printed CLI Agent Guide

This directory is a generated `memberstack-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
memberstack-pp-cli doctor --json
memberstack-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
memberstack-pp-cli which "<capability>" --json
memberstack-pp-cli <command> --help
```
...
```bash
memberstack-pp-cli <command> --agent
```
...
```bash
memberstack-pp-cli <command> --help
memberstack-pp-cli <command> --dry-run --agent
```
