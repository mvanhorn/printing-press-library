...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ihatepdf-cv-pp-cli doctor
  ihatepdf-cv-pp-cli doctor --json
  ihatepdf-cv-pp-cli doctor --fail-on warn
  ihatepdf-cv-pp-cli doctor --fail-on stale`,
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "ihatepdf-cv-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context) map[string]any {
