...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("IHATEPDF_CV_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("IHATEPDF_CV_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ihatepdf-cv-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  ihatepdf-cv-pp-cli feedback \"the merge command saved me time\"\n  ihatepdf-cv-pp-cli feedback --stdin",
		Long: `Feedback is captured locally first in the CLI data directory's feedback.jsonl.
When ` + "`IHATEPDF_CV_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`IHATEPDF_CV_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ihatepdf-cv-pp-cli feedback list
  ihatepdf-cv-pp-cli feedback list --limit 5
  ihatepdf-cv-pp-cli feedback list --json`,
