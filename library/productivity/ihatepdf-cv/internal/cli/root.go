...
type rootFlags struct {
	...
	// invocation. Mirrors the IHATEPDF_CV_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "ihatepdf-cv-pp-cli",
		Short: `Ihatepdf Cv CLI — Private PDF operations for agents, scripts, and offline pipelines.`,
		Long: `Ihatepdf Cv CLI — Private PDF operations for agents, scripts, and offline pipelines.
...
See README.md or the bundled SKILL.md for recipes.`,
	...
	rootCmd.SetVersionTemplate("ihatepdf-cv-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'ihatepdf-cv-pp-cli profile list')")
	...
	rootCmd.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
		...
		autoRefreshIfStale(cmd.Context(), flags)
	...
	rootCmd.AddCommand(newMergePdfPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelFingerprintCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelInspectCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelPrivacyScanCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newSearchCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newExportCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newImportCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newTailCmd(flags))
