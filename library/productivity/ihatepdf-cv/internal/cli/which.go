...
var whichIndex = []whichEntry{
	{Command: "privacy-scan", Description: "Find content and metadata risks before a PDF leaves your machine.", Group: "Private local document pipelines", WhyItMatters: "Choose this when an agent must check a document for accidental sensitive content before delivery."},
	{Command: "fingerprint", Description: "Record SHA-256, SHA-1, and MD5 hashes for inputs and outputs.", Group: "Trust and audit", WhyItMatters: "Choose this when downstream systems need to verify that a PDF was not changed."},
	{Command: "inspect", Description: "Expose PDF structure, metadata, and hashes as compact JSON.", Group: "Agent-native plumbing", WhyItMatters: "Choose this before any transformation when an agent needs to understand the artifact."},
	{Command: "search", Description: "Search extracted text across local PDFs and return matching snippets.", Group: "Agent-native plumbing", WhyItMatters: "Choose this when an agent needs to locate a phrase across a local document set."},
	{Command: "catalog index", Description: "Index PDF paths, hashes, page counts, and extracted text in a local SQLite catalog.", Group: "Private local document pipelines", WhyItMatters: "Choose this when an agent needs a searchable, repeatable inventory of local PDFs."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  ihatepdf-cv-pp-cli which "stale tickets"
  ihatepdf-cv-pp-cli which "bottleneck"
  ihatepdf-cv-pp-cli which --limit 1 "send message"
  ihatepdf-cv-pp-cli which                                # list the full capability index`,
