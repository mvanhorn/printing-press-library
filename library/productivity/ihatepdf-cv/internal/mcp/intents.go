...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("create_an_integrity_record",
			mcplib.WithDescription("Capture hashes for a reproducible handoff."),
			mcplib.WithString("domain", mcplib.Required(), mcplib.Description("Override the recipe's positional domain value.")),
		),
		handleCreateAnIntegrityRecord,
	)
}

// handleCreateAnIntegrityRecord runs the create_an_integrity_record recipe intent: Capture hashes for a reproducible handoff.
func handleCreateAnIntegrityRecord(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
