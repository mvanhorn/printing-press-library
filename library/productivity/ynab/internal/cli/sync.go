...
Resource scoping:
  ...
  ynab-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  ynab-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  ynab-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  ynab-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  ynab-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "ynab-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/github.com/mvanhorn/printing-press-library/library/productivity/ynab/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "months":
		return db.UpsertMonths(data)
	case "payees":
		return db.UpsertPayees(data)
	case "scheduled_transactions":
		return db.UpsertScheduledTransactions(data)
	case "transactions":
