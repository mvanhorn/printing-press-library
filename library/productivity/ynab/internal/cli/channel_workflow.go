...
func newWorkflowCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.AddCommand(newWorkflowFinanceSnapshotCmd(flags))
...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  ynab-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  ynab-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  ynab-pp-cli workflow status

  # Show status as JSON
  ynab-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("ynab-pp-cli")
			}
