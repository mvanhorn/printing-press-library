...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("YNAB_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "ynab-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("YNAB_ACCESS_TOKEN"); v != "" {
		...
		cfg.AuthSource = "env:YNAB_ACCESS_TOKEN"
	}
...
func (c *Config) AuthHeader() string {
	...
	if c.AccessToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:YNAB_ACCESS_TOKEN"
		}
