# Odoo18cli Printed CLI Agent Guide

This directory is a generated `odoo18cli-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
odoo18cli-pp-cli doctor --json
odoo18cli-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
odoo18cli-pp-cli which "<capability>" --json
odoo18cli-pp-cli <command> --help
```
...
```bash
odoo18cli-pp-cli <command> --agent
```
...
```bash
odoo18cli-pp-cli <command> --help
odoo18cli-pp-cli <command> --dry-run --agent
```
