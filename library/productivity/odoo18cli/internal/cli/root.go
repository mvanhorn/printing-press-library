...
type rootFlags struct {
	...
	// Odoo connection overrides (supplement ODOO_URL/DB/USER env vars)
	odooURL string
	odooDB  string
	odooUser string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "odoo18cli-pp-cli",
		Short: "Manage odoo18cli resources via the odoo18cli API",
		Long: `Manage odoo18cli resources via the odoo18cli API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'odoo18cli-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("odoo18cli-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'odoo18cli-pp-cli profile list')")
	rootCmd.PersistentFlags().StringVar(&flags.deliverSpec, "deliver", "", "Route output to a sink: stdout (default), file:<path>, webhook:<url>")
	rootCmd.PersistentFlags().Float64Var(&flags.rateLimit, "rate-limit", 0, "Max requests per second (0 to disable)")
	rootCmd.PersistentFlags().StringVar(&flags.odooURL, "url", "", "Odoo base URL (overrides ODOO_URL)")
	rootCmd.PersistentFlags().StringVar(&flags.odooDB, "db", "", "Odoo database name (overrides ODOO_DB)")
	rootCmd.PersistentFlags().StringVar(&flags.odooUser, "user", "", "Odoo username / login (overrides ODOO_USER)")
	...
	rootCmd.AddCommand(newXmlrpcCmd(flags))
	...
	// Novel domain commands (direct XML-RPC, not generated mirrors)
	...
	rootCmd.AddCommand(newManufacturingNovelCmd(flags))
