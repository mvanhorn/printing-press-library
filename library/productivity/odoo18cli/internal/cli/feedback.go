...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".odoo18cli-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ODOO18CLI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ODOO18CLI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "odoo18cli-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.odoo18cli-pp-cli/feedback.jsonl.
When ` + "`ODOO18CLI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ODOO18CLI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  odoo18cli-pp-cli feedback list
  odoo18cli-pp-cli feedback list --limit 5
  odoo18cli-pp-cli feedback list --json`,
