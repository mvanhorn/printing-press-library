...
type Config struct {
	...
	Path          string            `toml:"-"`
	OdooApiKey    string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("ODOO18CLI_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "odoo18cli-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("ODOO_API_KEY"); v != "" {
		cfg.OdooApiKey = v
		cfg.AuthSource = "env:ODOO_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.OdooApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("ODOO18CLI_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
