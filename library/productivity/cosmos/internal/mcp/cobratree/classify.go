...
const (
	...
	// OpenWorldLocalWriteAnnotation marks non-destructive commands that write
	// only CLI-owned local state while also reading from an external service.
	OpenWorldLocalWriteAnnotation = "mcp:local-write-open-world"
...
func isMCPOpenWorldLocalWrite(cmd *cobra.Command) bool {
	return annotationIsTrue(cmd, OpenWorldLocalWriteAnnotation)
}
