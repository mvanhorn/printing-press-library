...
func RegisterAll(s *server.MCPServer, root *cobra.Command, cliPath func() (string, error)) {
	...
	walk(root, nil, func(cmd *cobra.Command, path []string) {
		...
		openWorldLocalWrite := isMCPOpenWorldLocalWrite(cmd)
		if !readOnly && (isMCPLocalWrite(cmd) || openWorldLocalWrite) {
			...
			options = append(options, mcplib.WithDestructiveHintAnnotation(false), mcplib.WithOpenWorldHintAnnotation(openWorldLocalWrite))
		}
