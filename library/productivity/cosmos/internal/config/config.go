...
type Config struct {
	...
	CosmosToken      string          `toml:"token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.cosmos.so",
	}
	...
	// Env var overrides
	if v := os.Getenv("COSMOS_TOKEN"); v != "" {
		cfg.CosmosToken = v
		cfg.markEnvOverride("CosmosToken")
		cfg.AuthSource = "env:COSMOS_TOKEN"
		cfg.CredentialSource = "env:COSMOS_TOKEN"
	}
	...
	if v := os.Getenv("COSMOS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("COSMOS_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "cosmos-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.CosmosToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:COSMOS_TOKEN"
		}
		return "Bearer " + c.CosmosToken
	}
	if c.AccessToken != "" {
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.CosmosToken == "" {
		c.CosmosToken = creds.CosmosToken
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.CosmosToken = ""
	delete(c.envOverrides, "CosmosToken")
	c.updateFileConfigField("CosmosToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["CosmosToken"] {
			out.CosmosToken = c.fileConfig.CosmosToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "CosmosToken":
		c.fileConfig.CosmosToken = c.CosmosToken
	}
}
