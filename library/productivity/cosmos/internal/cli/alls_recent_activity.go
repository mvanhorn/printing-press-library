...
func newAllsRecentActivityCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "recent-activity",
		Short:       "Fetch alls recent activity",
		Example:     "  cosmos-pp-cli alls recent-activity --operation-name example-resource",
		Annotations: map[string]string{"pp:endpoint": "alls.recent_activity", "pp:method": "POST", "pp:path": "/graphql"},
				...
				if json.Unmarshal(data, &items) == nil && len(items) > 0 {
					if err := printAutoTable(cmd.OutOrStdout(), items); err != nil {
						...
						if partialFailure != nil && !flags.allowPartialFailure {
							return partialFailureErr(fmt.Errorf("partial failure in %s response: %s", "alls", partialFailure.Message))
						}
						return nil
					}
				} else {
