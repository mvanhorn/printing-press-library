...
func TestRawAPISurfaceUnavailable(t *testing.T) {
	forbidden := []string{
		"actives",
		"activities",
		"alls",
		...
		"elements get",
		"elements get-2",
		"elements social-graph",
		"elements top-counts",
		"fors",
		"fors you-elements",
		"fors you-user-configuration",
		"globals",
		"globals clusters",
		"globals elements",
		"globals get",
		...
		"users follows-basic",
		"users for-member",
		"users public-clusters-count",
		"users public-elements-cluster-id",
		"users public-elements-count",
	}
	actual := make(map[string]struct{}, len(forbidden))
	type pendingCommand struct {
		command *cobra.Command
		path    string
	}
	queue := make([]pendingCommand, 0, len(forbidden))
	...
	for _, commandPath := range forbidden {
		if _, ok := actual[commandPath]; ok {
			exposed = append(exposed, commandPath)
		}
	}
	if len(exposed) > 0 {
		t.Fatalf("raw GraphQL command paths remain public: %s", strings.Join(exposed, ", "))
	}
}
