...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("COSMOS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("COSMOS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "cosmos-pp-cli/feedback")
...
When ` + "`COSMOS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`COSMOS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cosmos-pp-cli feedback list
  cosmos-pp-cli feedback list --limit 5
  cosmos-pp-cli feedback list --json`,
