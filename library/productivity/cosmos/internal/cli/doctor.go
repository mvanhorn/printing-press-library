...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cosmos-pp-cli doctor
  cosmos-pp-cli doctor --json
  cosmos-pp-cli doctor --fail-on warn
  cosmos-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set it with: pipe the token to cosmos-pp-cli auth set-token --stdin or export COSMOS_TOKEN=\"your-token-here\""
					report["auth_docs_url"] = "https://api.cosmos.so"
				} else {
					authConfigured = true
			...
			if os.Getenv("COSMOS_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "COSMOS_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "COSMOS_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "cosmos-pp-cli", suggestion)
						} else {
