...
func TestCrossHostRedirectStripsCustomCredentialHeaders(t *testing.T) {
	var leaked string
	...
	c := New(&config.Config{BaseURL: source.URL, Headers: map[string]string{"X-API-Key": "secret"}}, time.Second, 0)
	c.NoCache = true
	if _, err := c.Get(context.Background(), "/", nil); err != nil {
		t.Fatal(err)
	}
	if leaked != "" {
		t.Fatalf("custom credential header leaked across hosts: %q", leaked)
	}
}

func TestResponseBodySafetyLimit(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	c.NoCache = true
	if _, err := c.Get(context.Background(), "/", nil); err == nil || !strings.Contains(err.Error(), "safety limit") {
