...
type rootFlags struct {
	...
	// invocation. Mirrors the CONCUR_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "concur-pp-cli",
		Short: `Concur CLI — Every expense-report and travel workflow Concur's web app offers, plus duplicate detection and real flight/hotel…`,
		Long: `Concur CLI — Every expense-report and travel workflow Concur's web app offers, plus duplicate detection and real flight/hotel search no Concur tool has -- filed through the same session your browser already uses.

Highlights (not in the official API docs):
  • expenses scan-duplicates   Find potential double-entered charges across all of your synced expenses.
  • flights search   Search real flight availability and fares from a live shopping session against your actual corporate-negotiated rates.
  • hotels search   Search real hotel availability and rates via a live, policy-scoped search.

Agent mode: add --agent to any command for JSON output + non-interactive mode.
Health check: run 'concur-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("concur-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select id,name,code)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'concur-pp-cli profile list')")
	...
	rootCmd.AddCommand(newAttendeesCmd(flags))
	rootCmd.AddCommand(newExpenseTypesCmd(flags))
	...
	rootCmd.AddCommand(newDelegatesPromotedCmd(flags))
	rootCmd.AddCommand(newListsPromotedCmd(flags))
	rootCmd.AddCommand(newLocationsPromotedCmd(flags))
	rootCmd.AddCommand(newPaymentTypesPromotedCmd(flags))
	rootCmd.AddCommand(newReceiptsPromotedCmd(flags))
	rootCmd.AddCommand(newTravelAllowancePromotedCmd(flags))
