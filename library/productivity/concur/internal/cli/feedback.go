...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CONCUR_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CONCUR_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "concur-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  concur-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
...
When ` + "`CONCUR_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CONCUR_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  concur-pp-cli feedback list
  concur-pp-cli feedback list --limit 5
  concur-pp-cli feedback list --json`,
