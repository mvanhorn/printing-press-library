...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  concur-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  concur-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// trips omitted: no longer syncable via the generic path+params
			// interface (moved to a POST GraphQL BFF; see trips_list.go).
			resources := []string{"attendee_types", "attendees", "expense_types", "locations", "payment_types", "reports", "requests"}
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  concur-pp-cli workflow status

  # Show status as JSON
  concur-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("concur-pp-cli")
			}
