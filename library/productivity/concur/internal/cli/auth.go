...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "auth",
		Short:       "Manage authentication for Concur",
...
Use --chrome to read cookies from Chrome for concursolutions.com.
...
profile by name when the installed backend supports it.`,
		Example: `  concur-pp-cli auth login --chrome
  concur-pp-cli auth login --browser
  concur-pp-cli auth login --cookies-file storage-state.json
  concur-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !browserFlag && cookiesFile == "" {
				...
				fmt.Fprintf(cmd.OutOrStdout(), "  concur-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  concur-pp-cli auth login --browser\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  concur-pp-cli auth login --cookies-file storage-state.json\n")
				return nil
			}
			if browserFlag && cookiesFile != "" {
			...
			requiredCookies := []string{"JWT", "_csrf"}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  concur-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if !credentialConfigured {
				...
				fmt.Fprintf(w, "  concur-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: concursolutions.com\n")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  concur-pp-cli auth set-token eyJhbGciOi...",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  concur-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
...
// cookieQueryHosts returns the set of hosts to query for cookies: the bare
// domain plus any subdomain host_key discovered in the local Chrome cookies
// DB that actually holds one of the required cookies. Real browser
// cookie-jar semantics scope some cookies host-only to a specific subdomain
// -- e.g. Concur's _csrf cookie lives on the tenant's regional web subdomain
// (us2.concursolutions.com), not the bare apex domain (concursolutions.com).
// A same-apex-only query silently excludes those cookies even when they are
// genuinely present and the user is logged in. See
// .printing-press-patches/chrome-cookie-subdomain-discovery.json.
func cookieQueryHosts(domain, cookiesDBPath string, requiredCookies []string) []string {
	hosts := []string{domain}
	if cookiesDBPath == "" || len(requiredCookies) == 0 {
		return hosts
	}
	seen := map[string]bool{domain: true}
	for _, host := range discoverCookieHosts(cookiesDBPath, domain, requiredCookies) {
		if !seen[host] {
			seen[host] = true
...
// discoverCookieHosts returns the distinct host_key values (leading dot
// stripped) in the given Chrome cookies DB that hold at least one of the
// required cookies, scoped to domain and its subdomains. Uses the same
// copy-then-query approach as inspectCookiesForDomain to avoid Chrome's WAL
// lock and to include uncommitted cookie writes still in the WAL.
func discoverCookieHosts(cookiesDB, domain string, requiredCookies []string) []string {
	quotedNames := make([]string, 0, len(requiredCookies))
	for _, name := range requiredCookies {
		name = strings.TrimSpace(name)
		if name == "" {
	...
	tmpFile, err := os.CreateTemp("", "cookies-hosts-*.db")
	if err != nil {
		return nil
	}
	tmpPath := tmpFile.Name()
	...
	domainPattern := "%" + strings.TrimPrefix(domain, ".") + "%"
	query := fmt.Sprintf(
		"SELECT DISTINCT host_key FROM cookies WHERE host_key LIKE '%s' AND name IN (%s)",
		sqlQuoteLiteral(domainPattern), strings.Join(quotedNames, ","),
	)
	out, err := exec.Command("sqlite3", tmpPath, query).Output()
	if err != nil {
		return nil
...
func extractViaPycookiecheat(tool cookieTool, domain string, profile chromeProfile) (string, error) {
	...
	var lastErr error
	for _, host := range cookieQueryHosts(cleanDomain, cookiePath, requiredAuthCookies()) {
		cookies, err := pycookiecheatChromeCookies(tool, host, cookiePath)
		if err != nil {
			lastErr = err
			continue
		}
	...
	if len(merged) == 0 {
		if lastErr != nil {
			return "", fmt.Errorf("pycookiecheat failed: %w", lastErr)
		}
		return "", nil
	}
...
// pycookiecheatChromeCookies runs pycookiecheat's chrome_cookies() against a
// single host and returns the raw name->value map for that host only.
// Callers merge results across cookieQueryHosts to assemble the full set.
func pycookiecheatChromeCookies(tool cookieTool, host, cookiePath string) (map[string]string, error) {
	var script string
	if cookiePath != "" {
	...
	if err := cmd.Run(); err != nil {
		return nil, fmt.Errorf("pycookiecheat failed for %s: %w", host, err)
	}

	var cookies map[string]string
	if err := json.Unmarshal(out.Bytes(), &cookies); err != nil {
		return nil, fmt.Errorf("parsing pycookiecheat output for %s: %w", host, err)
	}
	return cookies, nil
}
...
func extractViaPycookiecheatCLI(tool cookieTool, domain string, profile chromeProfile) (string, error) {
	...
	var lastErr error
	for _, host := range cookieQueryHosts(cleanDomain, cookiePath, requiredAuthCookies()) {
		cookies, err := pycookiecheatCLIChromeCookies(tool, host, cookiePath)
		if err != nil {
			lastErr = err
			continue
		}
	...
	if len(merged) == 0 {
		if lastErr != nil {
			return "", fmt.Errorf("pycookiecheat cli failed: %w", lastErr)
		}
		return "", nil
	}
...
// pycookiecheatCLIChromeCookies runs the pycookiecheat-cli binary against a
// single host and returns the raw name->value map for that host only.
func pycookiecheatCLIChromeCookies(tool cookieTool, host, cookiePath string) (map[string]string, error) {
	var args []string
	if cookiePath != "" {
	...
	cmd.Stderr = os.Stderr
	if err := cmd.Run(); err != nil {
		return nil, fmt.Errorf("pycookiecheat cli failed for %s: %w", host, err)
	}
