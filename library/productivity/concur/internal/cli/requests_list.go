...
func newRequestsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "list",
		Short:       "List the current user's travel requests",
		Example:     "  concur-pp-cli requests list",
		Annotations: map[string]string{"pp:endpoint": "requests.list", "pp:method": "GET", "pp:path": "/travelrequest/v4/requests", "mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			data, prov, err := resolveReadWithStrategyAndResponsePath(cmd.Context(), c, flags, "auto", "requests", true, path, params, nil, "", cmd.ErrOrStderr())
			if err != nil {
			...
			if asyncJobID := ExtractJobID(data, "requestId"); asyncJobID != "" {
				...
				if flagWait {
					...
					final, werr := WaitForJob(ctx, c, "/travelrequest/v4/requests/{request_id}", asyncJobID, WaitOptions{
