...
In local mode: searches locally synced data only.`,
  ...
  concur-pp-cli search "error timeout"

  # Force local search only
  concur-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  concur-pp-cli search "status" --type account --data-source local
  # JSON output for piping
  concur-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /locations/v4/locations
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/locations/v4/locations", map[string]string{
					"query": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'concur-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "expense_types":
				results, err = db.SearchExpenseTypes(query, limit)
			case "reports":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchExpenseTypes(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search expense_types failed: %w", searchErr)
					}
