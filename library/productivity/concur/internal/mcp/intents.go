...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("check_for_duplicate_charges",
			mcplib.WithDescription("Scan the local SQLite cache for likely double-entered transactions across all your reports."),
			mcplib.WithString("slug", mcplib.Required(), mcplib.Description("Override the recipe's positional slug value.")),
		),
		handleCheckForDuplicateCharges,
	)
}

// handleCheckForDuplicateCharges runs the check_for_duplicate_charges recipe intent: Scan the local SQLite cache for likely double-entered transactions across all your reports.
func handleCheckForDuplicateCharges(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
