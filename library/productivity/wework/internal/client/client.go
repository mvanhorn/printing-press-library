...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool, mutationIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if c.Config != nil {
			if v := c.Config.WeworkMemberType; v != "" {
				if authHeaderLooksLikePlaceholderCredential(v) {
					return nil, 0, authPlaceholderCredentialErrorWithSetup(c.Config, "export WEWORK_MEMBER_TYPE=<your-token>")
				}
				req.Header.Set("weworkmembertype", v)
			}
			if v := c.Config.WeworkUuid; v != "" {
				if authHeaderLooksLikePlaceholderCredential(v) {
					return nil, 0, authPlaceholderCredentialErrorWithSetup(c.Config, "export WEWORK_UUID=<your-token>")
				}
				req.Header.Set("weworkuuid", v)
			}
		}
		if c.Config != nil {
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export WEWORK_TOKEN=<your-token> or wework-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.WeworkToken)
		addCredential(c.Config.WeworkMemberType)
		addCredential(c.Config.WeworkUuid)
	}
