...
type Config struct {
	...
	WeworkToken      string          `toml:"token"`
	WeworkMemberType string          `toml:"member_type"`
	WeworkUuid       string          `toml:"uuid"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://members.wework.com/workplaceone/api",
	}
	...
	// Env var overrides
	if v := strings.ToLower(strings.TrimSpace(os.Getenv("WEWORK_SKIP_TLS_VERIFY"))); v == "1" || v == "true" || v == "yes" || v == "on" {
		cfg.SetSkipTLSVerify(true)
	}
	if v := os.Getenv("WEWORK_TOKEN"); v != "" {
		cfg.WeworkToken = v
		cfg.markEnvOverride("WeworkToken")
		cfg.AuthSource = "env:WEWORK_TOKEN"
		cfg.CredentialSource = "env:WEWORK_TOKEN"
	}
	...
	if v := os.Getenv("WEWORK_MEMBER_TYPE"); v != "" {
		cfg.WeworkMemberType = v
		cfg.markEnvOverride("WeworkMemberType")
	}
	// Sibling-scheme per-call credential sent on every request alongside the
	// primary auth. AuthSource intentionally not stamped: the primary auth
	// remains the canonical surface for doctor/auth-status reporting.
	if v := os.Getenv("WEWORK_UUID"); v != "" {
		cfg.WeworkUuid = v
		cfg.markEnvOverride("WeworkUuid")
	}
	...
	if v := os.Getenv("WEWORK_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("WEWORK_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "wework-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.WeworkToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:WEWORK_TOKEN"
		}
		return "Bearer " + c.WeworkToken
	}
	if c.AccessToken != "" {
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		WeworkToken:      c.WeworkToken,
		WeworkMemberType: c.WeworkMemberType,
		WeworkUuid:       c.WeworkUuid,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.WeworkMemberType == "" {
		c.WeworkMemberType = creds.WeworkMemberType
	}
	if c.WeworkUuid == "" {
		c.WeworkUuid = creds.WeworkUuid
	}
}
...
func (c *Config) configForSave() Config {
	...
	if c.fileConfig != nil {
		...
		if c.envOverrides["WeworkToken"] {
			out.WeworkToken = c.fileConfig.WeworkToken
		}
		if c.envOverrides["WeworkMemberType"] {
			out.WeworkMemberType = c.fileConfig.WeworkMemberType
		}
		if c.envOverrides["WeworkUuid"] {
			out.WeworkUuid = c.fileConfig.WeworkUuid
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "WeworkToken":
		c.fileConfig.WeworkToken = c.WeworkToken
	case "WeworkMemberType":
		c.fileConfig.WeworkMemberType = c.WeworkMemberType
	case "WeworkUuid":
		c.fileConfig.WeworkUuid = c.WeworkUuid
	}
}
