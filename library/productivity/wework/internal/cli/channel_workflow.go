...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  wework-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  wework-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"common-booking", "spaces-get-affiliate-locations", "wework-yardi", "wework-yardi-booking-start-locations-for-amenities-list", "wework-yardi-ondemand-get-locations-by-geo"}
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  wework-pp-cli workflow status

  # Show status as JSON
  wework-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("wework-pp-cli")
			}
