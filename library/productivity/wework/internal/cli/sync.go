...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/productivity/wework/internal/learn/lookups"
...
Resource scoping:
  ...
  wework-pp-cli sync --resources common-booking,spaces

  # Full resync (ignore previous checkpoint)
  wework-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  wework-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  wework-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  wework-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("wework-pp-cli")
			}
...
func determinePaginationDefaults(resource string) paginationDefaults {
	...
	case "wework-yardi-booking-start-locations-for-amenities-list":
		return paginationDefaults{
			cursorParam:    "",
			cursorType:     "offset",
			nextCursorPath: "",
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "wework-yardi":
		return db.UpsertWeworkYardi(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		...
		"spaces-get-affiliate-locations": "/spaces/get-affiliate-locations",
		"wework-yardi":                   "/wework-yardi/location/get-affiliate-cities",
		"wework-yardi-booking-start-locations-for-amenities-list": "/wework-yardi/booking/start-locations-for-amenities-list",
		"wework-yardi-ondemand-get-locations-by-geo":              "/wework-yardi/ondemand/get-locations-by-geo",
	}
...
func responsePathForResource(resource, path string) []string {
	...
	case "common-booking\x00/common-booking/upcoming-bookings":
		return []string{"WeWorkBookings"}
	case "spaces\x00/spaces/get-affiliate-locations":
		return []string{"locationsByGeo"}
	case "wework-yardi\x00/wework-yardi/ondemand/get-locations-by-geo":
		return []string{"locationsByGeo"}
	}
	return nil
}
