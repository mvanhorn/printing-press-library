...
type rootFlags struct {
	...
	// invocation. Mirrors the WEWORK_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "wework-pp-cli",
		Short: `WeWork CLI — Find and book WeWork day-desks from the terminal — cities, buildings, availability, price, and your bookings — with a…`,
		Long: `WeWork CLI — Find and book WeWork day-desks from the terminal — cities, buildings, availability, price, and your bookings — with a local store for instant, agent-native queries.
...
Health check: run 'wework-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("wework-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'wework-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCommonBookingCmd(flags))
	rootCmd.AddCommand(newSpacesCmd(flags))
	rootCmd.AddCommand(newWeworkYardiCmd(flags))
