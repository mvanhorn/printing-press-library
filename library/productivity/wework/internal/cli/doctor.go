...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  wework-pp-cli doctor
  wework-pp-cli doctor --json
  wework-pp-cli doctor --fail-on warn
  wework-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if !configured {
					...
					report["auth_hint"] = "Set it with: wework-pp-cli auth set-token <token> or export WEWORK_TOKEN=\"your-token-here\""
					report["auth_docs_url"] = "https://members.wework.com"
				} else {
					authConfigured = true
			...
			if os.Getenv("WEWORK_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "WEWORK_TOKEN")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "WEWORK_TOKEN")
			}
			{
				configuredValue := ""
				if cfg != nil {
					configuredValue = cfg.WeworkMemberType
				}
				recordAdditionalAuthEnv("WEWORK_MEMBER_TYPE", configuredValue)
			}
			{
				configuredValue := ""
				if cfg != nil {
					configuredValue = cfg.WeworkUuid
				}
				recordAdditionalAuthEnv("WEWORK_UUID", configuredValue)
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "wework-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'wework-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'wework-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		...
		report["hint"] = "No sync recorded; run 'wework-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'wework-pp-cli sync' to refresh."
	}
	return report
}
