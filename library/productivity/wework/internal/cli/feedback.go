...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("WEWORK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("WEWORK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "wework-pp-cli/feedback")
...
When ` + "`WEWORK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`WEWORK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  wework-pp-cli feedback list
  wework-pp-cli feedback list --limit 5
  wework-pp-cli feedback list --json`,
