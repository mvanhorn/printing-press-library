...
var whichIndex = []whichEntry{
	{Command: "search", Description: "Full-text search across every synced WeWork city and building, offline and instant.", Group: "Local state that compounds", WhyItMatters: "Reach for this to resolve a place name to concrete city/building UUIDs without hitting the network."},
	{Command: "desks", Description: "Search bookable desks by city name and date, deriving the map bounding box the API requires from cached city geo.", Group: "Agent-native plumbing", WhyItMatters: "The headline command: turns a plain city name + date into structured desk availability with price and seats."},
	{Command: "desks", Description: "Sort desks by credits or price and filter to only those with open seats.", Group: "Local state that compounds", WhyItMatters: "Answers 'what is the cheapest available desk here tomorrow' in one call."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  wework-pp-cli which "stale tickets"
  wework-pp-cli which "bottleneck"
  wework-pp-cli which --limit 1 "send message"
  wework-pp-cli which                                # list the full capability index`,
