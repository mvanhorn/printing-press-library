...
In local mode: searches locally synced data only.`,
  ...
  wework-pp-cli search "error timeout"

  # Force local search only
  wework-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  wework-pp-cli search "status" --type common-booking --data-source local
  # JSON output for piping
  wework-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /spaces/get-spaces
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/spaces/get-spaces", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'wework-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
