...
## User Vision
This run is a reprint lifting the published Figma CLI from printing-press v4.2.0 to v4.24.0. The user accepted two spec enrichments: (1) native Personal Access Token auth (X-Figma-Token) alongside OAuth2, removing the post-publish PAT hand-patch; (2) a thin MCP code-orchestration surface (figma_search + figma_execute) replacing the 48 raw endpoint tools to fix mcp_token_efficiency 0/10. No new feature asks: carry forward the 8 prior novel features (frame extract, dev-mode dump, comments-audit, orphans, tokens diff, fingerprint, webhooks test, variables explain), re-evaluated against the current machine and personas.
