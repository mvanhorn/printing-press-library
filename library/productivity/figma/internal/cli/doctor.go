		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export FIGMA_ACCESS_TOKEN=<your-key>"
					report["auth_docs_url"] = "https://developers.figma.com/docs/rest-api/"
				} else {
					authConfigured = true
			...
			if os.Getenv("FIGMA_ACCESS_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "FIGMA_ACCESS_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "FIGMA_ACCESS_TOKEN")
			}
			if os.Getenv("FIGMA_API_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "FIGMA_API_TOKEN")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "FIGMA_API_TOKEN")
			}
			if os.Getenv("FIGMA_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "FIGMA_API_KEY")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "FIGMA_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["X-Figma-Token"] = authHeader
