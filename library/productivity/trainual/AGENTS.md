# Trainual Printed CLI Agent Guide

This directory is a generated `trainual-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
trainual-pp-cli doctor --json
trainual-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
trainual-pp-cli which "<capability>" --json
trainual-pp-cli <command> --help
```
...
```bash
trainual-pp-cli <command> --help
trainual-pp-cli <command> --dry-run --agent
```
