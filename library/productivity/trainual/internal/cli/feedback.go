...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".trainual-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TRAINUAL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TRAINUAL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "trainual-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.trainual-pp-cli/feedback.jsonl.
When ` + "`TRAINUAL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TRAINUAL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  trainual-pp-cli feedback list
  trainual-pp-cli feedback list --limit 5
  trainual-pp-cli feedback list --json`,
