...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  trainual-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  trainual-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"roles", "subjects", "users"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  trainual-pp-cli workflow status

  # Show status as JSON
  trainual-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("trainual-pp-cli")
			}
