...
In local mode: searches locally synced data only.`,
		Example: `  # Search synced data for a keyword
  trainual-pp-cli search "safety training"

  # Force local search only
  trainual-pp-cli search "onboarding" --data-source local

  # Search a specific resource type locally
  trainual-pp-cli search "front desk" --type users --data-source local

  # JSON output for piping
  trainual-pp-cli search "compliance" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'trainual-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/trainual-pp-cli/data.db)")
