...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("RAINDROP_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("RAINDROP_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "raindrop-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  raindrop-pp-cli feedback "Search output was useful"
  raindrop-pp-cli feedback list`,
		Long: `Feedback is captured locally first in the CLI data directory's feedback.jsonl.
When ` + "`RAINDROP_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`RAINDROP_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  raindrop-pp-cli feedback list
  raindrop-pp-cli feedback list --limit 5
  raindrop-pp-cli feedback list --json`,
