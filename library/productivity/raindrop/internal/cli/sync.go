...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/productivity/raindrop/internal/learn/lookups"
...
Resource scoping:
  ...
  raindrop-pp-cli sync
  # Sync specific resources only
  raindrop-pp-cli sync --resources collections,highlights

  # Full resync (ignore previous checkpoint)
  raindrop-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  raindrop-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  raindrop-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  raindrop-pp-cli sync --latest-only`,
	...
	cmd.AddCommand(newSyncStatusCmd(flags), newSyncDiffCmd(flags))
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "bookmark_transfer":
		return db.UpsertBookmarkTransfer(data)
	case "collection":
		return db.UpsertCollection(data)
	case "merge":
		return db.UpsertMerge(data)
	case "sharing":
		return db.UpsertSharing(data)
	case "stats":
		return db.UpsertStats(data)
	case "collections":
		return db.UpsertCollections(data)
	case "childrens":
		return db.UpsertChildrens(data)
	case "highlights":
		return db.UpsertHighlights(data)
	case "raindrop":
		return db.UpsertRaindrop(data)
	case "raindrop_highlights":
		return db.UpsertRaindropHighlights(data)
	case "permanent":
		return db.UpsertPermanent(data)
	case "reminder":
		return db.UpsertReminder(data)
	case "suggest":
		return db.UpsertSuggest(data)
	case "raindrops":
		return db.UpsertRaindrops(data)
	case "tags":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		...
		"raindrops-0":        "/raindrops/0",
		"raindrops-multiple": "/raindrops/multiple",
		"tags":               "/tags",
		"tags-0":             "/tags/0",
		"user":               "/user/stats",
	}
...
var reconcileTypedTables = map[string]string{
	"bookmark_transfer":   "bookmark_transfer",
	"childrens":           "childrens",
	...
	"permanent":           "permanent",
	"raindrop":            "raindrop",
	"raindrop_highlights": "raindrop_highlights",
	"raindrops":           "raindrops",
	"reminder":            "reminder",
	"sharing":             "sharing",
	"stats":               "stats",
...
func responsePathForResource(resource, path string) []string {
	...
	case "bookmark_transfer\x00/import":
		return []string{"items"}
	case "collections\x00/collections":
		return []string{"items"}
	case "highlights\x00/highlights":
		return []string{"items"}
	case "highlights\x00/highlights/{collectionId}":
		return []string{"items"}
	case "raindrops\x00/raindrops/0":
		return []string{"items"}
	case "raindrops\x00/raindrops/{collectionId}":
		return []string{"items"}
	case "raindrops\x00/raindrops/multiple":
		return []string{"items"}
	case "raindrops\x00/raindrops":
		return []string{"items"}
	case "tags\x00/tags":
		return []string{"items"}
	case "tags\x00/tags/0":
		return []string{"items"}
	case "tags\x00/tags/{collectionId}":
		return []string{"items"}
	case "user\x00/user/stats":
		return []string{"items"}
	}
	return nil
}
