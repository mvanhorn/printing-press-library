...
func newCollectionSharingCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "sharing",
		Short:       "Update sharing for collection",
		Annotations: map[string]string{"mcp:read-only": "true", "pp:parent-group": "true", "pp:typed-exit-codes": "0,2"},
		RunE:        parentNoSubcommandRunE(flags),
	}

	cmd.AddCommand(newCollectionSharingShareCollectionCmd(flags))
	return cmd
}
