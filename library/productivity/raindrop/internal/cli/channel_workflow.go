...
func newWorkflowCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.AddCommand(newWorkflowTriageCmd(flags))
...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  raindrop-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  raindrop-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"collections", "highlights", "raindrops", "raindrops-0", "tags", "tags-0"}
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  raindrop-pp-cli workflow status

  # Show status as JSON
  raindrop-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("raindrop-pp-cli")
			}
