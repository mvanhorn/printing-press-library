...
In local mode: searches locally synced data only.`,
  ...
  raindrop-pp-cli search "error timeout"

  # Force local search only
  raindrop-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  raindrop-pp-cli search "status" --type bookmark_transfer --data-source local
  # JSON output for piping
  raindrop-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// Raindrop searches all collections through collection ID 0.
			if !offline && flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/raindrops/0", map[string]string{
					"search": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'raindrop-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "highlights":
				results, err = db.SearchHighlights(query, limit)
			case "raindrops":
				results, err = db.SearchRaindrops(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchHighlights(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search highlights failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchRaindrops(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search raindrops failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
	...
	cmd.Flags().BoolVar(&offline, "offline", false, "Search only the local SQLite mirror")
