...
func newCollectionsChildrensCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "childrens",
		Short:       "Get childrens for collections",
		Annotations: map[string]string{"mcp:read-only": "true", "pp:parent-group": "true", "pp:typed-exit-codes": "0,2"},
		RunE:        parentNoSubcommandRunE(flags),
	}

	cmd.AddCommand(newCollectionsChildrensGetChildCollectionsCmd(flags))
	return cmd
}
