...
func newRaindropHighlightsCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "highlights",
		Short:       "Get highlights for raindrop",
		Annotations: map[string]string{"mcp:read-only": "true", "pp:parent-group": "true", "pp:typed-exit-codes": "0,2"},
		RunE:        parentNoSubcommandRunE(flags),
	}

	cmd.AddCommand(newRaindropHighlightsGetForBookmarkCmd(flags))
	return cmd
}
