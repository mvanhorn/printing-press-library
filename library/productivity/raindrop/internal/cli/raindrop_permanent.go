...
func newRaindropPermanentCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "permanent",
		Short:       "Delete permanent for raindrop",
		Annotations: map[string]string{"mcp:read-only": "true", "pp:parent-group": "true", "pp:typed-exit-codes": "0,2"},
		RunE:        parentNoSubcommandRunE(flags),
	}

	cmd.AddCommand(newRaindropPermanentDeleteBookmarkCmd(flags))
	return cmd
}
