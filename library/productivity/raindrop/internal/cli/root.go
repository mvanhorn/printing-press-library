...
type rootFlags struct {
	...
	// invocation. Mirrors the RAINDROP_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "raindrop",
		Short: `Raindrop CLI — Complete Raindrop.io control plus offline search, safe cleanup, and durable knowledge workflows.`,
		Long: `Raindrop CLI — Complete Raindrop.io control plus offline search, safe cleanup, and durable knowledge workflows.
...
Health check: run 'raindrop doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("raindrop {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select item,result)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'raindrop-pp-cli profile list')")
	...
	rootCmd.AddCommand(newBookmarkTransferCmd(flags))
	...
	rootCmd.AddCommand(newRaindropCmd(flags))
	rootCmd.AddCommand(newRaindropsCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelChangesCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelDuplicatesCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelInboxCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelRelatedCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelRevisitCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newReadingCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newClustersCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelTagCmd(flags))
...
func (f *rootFlags) newClient() (*client.Client, error) {
	...
	// A project-local token is convenient for source checkouts and remains
	// lower precedence than RAINDROP_TOKEN or persisted auth credentials.
	// Refuse group/world-readable files so this fallback cannot normalize an
	// accidentally exposed bearer token.
	if !cfg.CredentialConfigured() {
		const projectTokenPath = ".raindrop-token"
		if info, statErr := os.Stat(projectTokenPath); statErr == nil {
			if info.Mode().Perm()&0o077 != 0 {
				return nil, configErr(fmt.Errorf("%s must be owner-only (chmod 600 %s)", projectTokenPath, projectTokenPath))
			}
			raw, readErr := os.ReadFile(projectTokenPath)
			if readErr != nil {
				return nil, configErr(fmt.Errorf("reading %s: %w", projectTokenPath, readErr))
			}
			if token := strings.TrimSpace(string(raw)); token != "" {
				cfg.RaindropToken = token
				cfg.AuthSource = "project-token-file"
				cfg.CredentialSource = "project token file"
			}
		} else if !os.IsNotExist(statErr) {
			return nil, configErr(fmt.Errorf("checking %s: %w", projectTokenPath, statErr))
		}
	}
