...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  raindrop-pp-cli doctor
  raindrop-pp-cli doctor --json
  raindrop-pp-cli doctor --fail-on warn
  raindrop-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if !configured {
					...
					report["auth_hint"] = "Set it with: raindrop-pp-cli auth set-token <token> or export RAINDROP_TOKEN=\"your-token-here\""
					report["auth_docs_url"] = "https://developer.raindrop.io"
				} else {
					authConfigured = true
			...
			if os.Getenv("RAINDROP_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "RAINDROP_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "RAINDROP_TOKEN")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'raindrop-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'raindrop-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		...
		report["hint"] = "No sync recorded; run 'raindrop-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'raindrop-pp-cli sync' to refresh."
	}
	return report
}
