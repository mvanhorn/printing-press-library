...
type Config struct {
	...
	RaindropToken    string          `toml:"token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := strings.ToLower(strings.TrimSpace(os.Getenv("RAINDROP_SKIP_TLS_VERIFY"))); v == "1" || v == "true" || v == "yes" || v == "on" {
		cfg.SetSkipTLSVerify(true)
	}
	if v := os.Getenv("RAINDROP_TOKEN"); v != "" {
		cfg.RaindropToken = v
		cfg.markEnvOverride("RaindropToken")
		cfg.AuthSource = "env:RAINDROP_TOKEN"
		cfg.CredentialSource = "env:RAINDROP_TOKEN"
	}
	...
	if v := os.Getenv("RAINDROP_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("RAINDROP_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "raindrop-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.RaindropToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:RAINDROP_TOKEN"
		}
		return "Bearer " + c.RaindropToken
	}
	if c.AccessToken != "" {
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.RaindropToken == "" {
		c.RaindropToken = creds.RaindropToken
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.RaindropToken = ""
	delete(c.envOverrides, "RaindropToken")
	c.updateFileConfigField("RaindropToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	...
	if c.fileConfig != nil {
		...
		if c.envOverrides["RaindropToken"] {
			out.RaindropToken = c.fileConfig.RaindropToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "RaindropToken":
		c.fileConfig.RaindropToken = c.RaindropToken
	}
}
