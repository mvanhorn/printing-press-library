...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/productivity/thunderbird/internal/cli"
	"github.com/mvanhorn/printing-press-library/library/productivity/thunderbird/internal/client"
	...
	"github.com/mvanhorn/printing-press-library/library/productivity/thunderbird/internal/mcp/bound"
		...
		if err != nil {
			...
			case strings.Contains(msg, "HTTP 401"):
				return mcpToolError("authentication failed: " + msg +
					...
					"\n      Run 'thunderbird-pp-cli doctor' to check auth status."), nil
...
func openMCPReadOnlyStore(path string) (*store.Store, *mcplib.CallToolResult) {
	...
	if err != nil {
		return nil, mcplib.NewToolResultError(fmt.Sprintf("opening local data store %s: %v. Run thunderbird-pp-cli sync to refresh the store before using MCP search/sql.", path, err))
	}
	return db, nil
}

func mcpMissingStoreMessage(path string) string {
	return fmt.Sprintf("No local data store found at %s. Run thunderbird-pp-cli sync before using MCP search/sql.", path)
}
...
func mcpEmptyStoreNextStep() string {
	return "Run thunderbird-pp-cli sync to populate the local SQLite store before using MCP search/sql."
}

func mcpPartialStoreNextStep() string {
	return "The latest sync attempt is incomplete. Resume or rerun thunderbird-pp-cli sync before treating local search/sql results as a complete snapshot."
}
...
func handleContextResult(s *server.MCPServer, _ context.Context, _ mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	ctx := map[string]any{
		...
		"tool_surface": "MCP exposes typed endpoint tools plus a runtime mirror of user-facing CLI commands. Endpoint tools keep typed schemas; command-mirror tools shell out to the companion thunderbird-pp-cli binary.",
		...
		"command_mirror_capabilities": registeredCommandMirrorCapabilities(s, []map[string]string{
			{"name": "Awaiting reply", "command": "awaiting-reply", "cli_command": "awaiting-reply", "description": "See every thread where someone is waiting on your answer, across all accounts, oldest first.", "rationale": "Requires reconstructed threads joined with your identities, localized Sent folders and Thunderbird's replied flag", "via": "mcp-command-mirror"},
			{"name": "Correspondents ranking", "command": "contacts top", "cli_command": "contacts top", "description": "Rank the people you exchange mail with by volume and recency, and spot frequent ones missing from your address book.", "rationale": "Joins inbound and outbound messages with your identities and address-book cards in the local store.", "via": "mcp-command-mirror"},
			{"name": "Filters audit", "command": "filters audit", "cli_command": "filters audit", "description": "Check every message filter: how often it matches, whether it is disabled, and whether its target folder still exists.", "rationale": "Parses Thunderbird's msgFilterRules.dat and evaluates rules against the locally indexed mail.", "via": "mcp-command-mirror"},
			{"name": "Largest items", "command": "largest", "cli_command": "largest", "description": "Find the biggest messages or attachments across all accounts and folders.", "rationale": "Ranks by on-disk mbox size and decoded attachment size, which no single folder view exposes.", "via": "mcp-command-mirror"},
			{"name": "Newsletters and bulk senders", "command": "newsletters", "cli_command": "newsletters", "description": "List mailing lists and bulk senders by volume and how much of their mail you never read.", "rationale": "Combines List-Id headers with Thunderbird's read flags from the local store.", "via": "mcp-command-mirror"},
		}),
		"playbook": []map[string]string{
			{"topic": "Awaiting reply", "insight": "Requires reconstructed threads joined with your identities, localized Sent folders and Thunderbird's replied flag, which only exist together in the local store."},
			{"topic": "Correspondents ranking", "insight": "Joins inbound and outbound messages with your identities and address-book cards in the local store."},
			{"topic": "Filters audit", "insight": "Parses Thunderbird's msgFilterRules.dat and evaluates rules against the locally indexed mail."},
			{"topic": "Largest items", "insight": "Ranks by on-disk mbox size and decoded attachment size, which no single folder view exposes."},
			{"topic": "Newsletters and bulk senders", "insight": "Combines List-Id headers with Thunderbird's read flags from the local store."},
