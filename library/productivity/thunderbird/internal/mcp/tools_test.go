...
func TestMCPPathResolutionMatchesCLIResolverWithPlatformDefaults(t *testing.T) {
	...
	if want := filepath.Join(home, ".config", "thunderbird-pp-cli", "config.json"); cfg.Path != want {
	...
	if want := filepath.Join(home, ".local", "share", "thunderbird-pp-cli", "data.db"); gotDB != want {
...
func TestMCPRegisterToolsPreservesTypedSpecialTools(t *testing.T) {
	s := server.NewMCPServer("thunderbird", "test")
	RegisterTools(s)
...
func TestMCPUnmigratedCheckpointsCannotProveCompletion(t *testing.T) {
	for _, cursor := range []string{"", "resume-here"} {
		...
		t.Run(name, func(t *testing.T) {
			...
			if err := db.SaveSyncStateAt("accounts", cursor, 0, time.Now().UTC()); err != nil {
				t.Fatal(err)
			}
...
func TestMCPLocalStoreMetaIncludesOldestSyncedAt(t *testing.T) {
	...
	if _, err := db.DB().Exec(`INSERT INTO sync_state(resource_type, last_synced_at, total_count) VALUES (?, ?, ?)`, "accounts", syncedAt, 0); err != nil {
