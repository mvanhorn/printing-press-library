...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "doctor",
		Short: "Check the Thunderbird profile, its accounts and folders, and the local store",
		Example: `  thunderbird-pp-cli doctor
  thunderbird-pp-cli doctor --json
  thunderbird-pp-cli doctor --fail-on warn
  thunderbird-pp-cli doctor --fail-on stale`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			addThunderbirdDoctorChecks(cmd, flags, report)
			...
			for _, key := range []string{"config_path", "profile_path", "store_path", "last_sync", "auth_source", "auth_refusals", "credentials_location", "version"} {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'thunderbird-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'thunderbird-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny:
		...
		report["hint"] = "No sync recorded; run 'thunderbird-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'thunderbird-pp-cli sync' to refresh."
	}
	return report
}
