...
// thunderbird-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/productivity/thunderbird/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/productivity/thunderbird/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/productivity/thunderbird/internal/learn/patterns"
...
const noLearnEnvVar = "THUNDERBIRD_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("thunderbird-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  thunderbird-pp-cli teach --query "find items in category" --resource-type items --resource GROUP-category`,
...
when learnings exist.`,
		Example: `  QUERY="$(cat /path/to/question.txt)" thunderbird-pp-cli recall "$QUERY" --agent
  QUERY="$(cat /path/to/question.txt)" thunderbird-pp-cli recall "$QUERY" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  thunderbird-pp-cli learnings list --agent
  thunderbird-pp-cli learnings list --query "$QUERY"
  thunderbird-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  QUERY="$(cat /path/to/question.txt)" thunderbird-pp-cli learnings forget "$QUERY" --resource <id>
  QUERY="$(cat /path/to/question.txt)" thunderbird-pp-cli learnings forget "$QUERY" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  thunderbird-pp-cli teach-pattern --query-template "items in {entity}" --resource-template "GROUP-{entity:category}" --resource-type items --entity-kind category --strategy substitute`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  thunderbird-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
