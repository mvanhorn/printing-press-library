...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("THUNDERBIRD_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("THUNDERBIRD_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "thunderbird-pp-cli/feedback")
...
When ` + "`THUNDERBIRD_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`THUNDERBIRD_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  thunderbird-pp-cli feedback "docs said exclusive but the boundary is inclusive"
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  thunderbird-pp-cli feedback list
  thunderbird-pp-cli feedback list --limit 5
  thunderbird-pp-cli feedback list --json`,
