...
func newWorkflowCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "workflow",
		Short:       "Compound workflows over the local store (archive, status)",
...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  thunderbird-pp-cli workflow archive --full
  ...
  thunderbird-pp-cli workflow archive --timeout 0`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"accounts", "contacts", "filters", "folders", "messages"}
			if cliutil.IsDogfoodEnv() {
				if len(resources) > 3 {
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  thunderbird-pp-cli workflow status

  # Show status as JSON
  thunderbird-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("thunderbird-pp-cli")
			}
