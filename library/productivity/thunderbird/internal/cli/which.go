...
var whichIndex = []whichEntry{
	{Command: "awaiting-reply", Description: "See every thread where someone is waiting on your answer, across all accounts, oldest first.", Group: "Inbox intelligence", WhyItMatters: "Reach for this when asked what mail still needs a reply instead of scanning recent messages."},
	{Command: "contacts top", Description: "Rank the people you exchange mail with by volume and recency, and spot frequent ones missing from your address book.", Group: "Inbox intelligence", WhyItMatters: "Use it to answer who someone is to the user and when they last talked."},
	{Command: "filters audit", Description: "Check every message filter: how often it matches, whether it is disabled, and whether its target folder still exists.", Group: "Mailbox hygiene", WhyItMatters: "Use it when mail lands in the wrong folder or to clean up dead filter rules."},
	{Command: "largest", Description: "Find the biggest messages or attachments across all accounts and folders.", Group: "Mailbox hygiene", WhyItMatters: "Use it to free disk or mailbox quota."},
	{Command: "newsletters", Description: "List mailing lists and bulk senders by volume and how much of their mail you never read.", Group: "Mailbox hygiene", WhyItMatters: "Use it to pick unsubscribe candidates."},
	{Command: "accounts", Description: "List accounts and identities from prefs.js", Group: "accounts", WhyItMatters: "List accounts and identities from prefs."},                             // pp:which-promoted
	{Command: "contacts", Description: "List address book contacts", Group: "contacts", WhyItMatters: "List address book contacts"},                                                           // pp:which-promoted
	{Command: "filters", Description: "List message filter rules per account", Group: "filters", WhyItMatters: "List message filter rules per account"},                                       // pp:which-promoted
	{Command: "folders", Description: "List folders with total, unread, size and offline status", Group: "folders", WhyItMatters: "List folders with total, unread, size and offline status"}, // pp:which-promoted
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one. Machine output (--json/--csv/--plain/--quiet, or a pipe) still exits 2 and writes {"matches":[]} (or the equivalent empty table) to stdout.`,
		SilenceUsage: true,
		Example: `  thunderbird-pp-cli which "stale tickets"
  thunderbird-pp-cli which "bottleneck"
  thunderbird-pp-cli which --limit 1 "send message"
  thunderbird-pp-cli which                                # list the full capability index`,
