...
type rootFlags struct {
	...
	// invocation. Mirrors the THUNDERBIRD_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "thunderbird-pp-cli",
		Short: `Thunderbird CLI — Search, read and audit the mail Thunderbird already downloaded — offline, read-only, no credentials.`,
		Long: `Thunderbird CLI — Search, read and audit the mail Thunderbird already downloaded — offline, read-only, no credentials.
...
Health check: run 'thunderbird-pp-cli doctor' to check the Thunderbird profile and the local store.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("thunderbird-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'thunderbird-pp-cli profile list')")
	...
	addNovelCommandIfAbsent(rootCmd, newNovelAwaitingReplyCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelLargestCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelNewslettersCmd(flags))
