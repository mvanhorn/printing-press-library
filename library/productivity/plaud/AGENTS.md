# Plaud Printed CLI Agent Guide

This directory is a generated `plaud-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
plaud-pp-cli doctor --json
plaud-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
plaud-pp-cli which "<capability>" --json
plaud-pp-cli <command> --help
```
...
```bash
plaud-pp-cli <command> --help
plaud-pp-cli <command> --dry-run --agent
```
