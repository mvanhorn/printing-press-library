...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".plaud-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PLAUD_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PLAUD_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "plaud-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.plaud-pp-cli/feedback.jsonl.
When ` + "`PLAUD_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PLAUD_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  plaud-pp-cli feedback list
  plaud-pp-cli feedback list --limit 5
  plaud-pp-cli feedback list --json`,
