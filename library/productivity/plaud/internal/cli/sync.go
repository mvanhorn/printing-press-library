...
Resource scoping:
  ...
  plaud-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  plaud-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  plaud-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  plaud-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  plaud-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/plaud-pp-cli/data.db)")
