...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  plaud-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  plaud-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"filetags", "recordings"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  plaud-pp-cli workflow status

  # Show status as JSON
  plaud-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("plaud-pp-cli")
			}
