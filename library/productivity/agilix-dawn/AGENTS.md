# Agilix Dawn Printed CLI Agent Guide

This directory is a generated `agilix-dawn-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
agilix-dawn-pp-cli doctor --json
agilix-dawn-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
agilix-dawn-pp-cli which "<capability>" --json
agilix-dawn-pp-cli <command> --help
```
...
```bash
agilix-dawn-pp-cli <command> --agent
```
...
```bash
agilix-dawn-pp-cli <command> --help
agilix-dawn-pp-cli <command> --dry-run --agent
```
