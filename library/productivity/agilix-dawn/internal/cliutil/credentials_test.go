...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"AGILIX_DAWN_CONFIG",
		"AGILIX_DAWN_CONFIG_DIR",
		"AGILIX_DAWN_DATA_DIR",
		"AGILIX_DAWN_STATE_DIR",
		"AGILIX_DAWN_CACHE_DIR",
		"AGILIX_DAWN_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
	...
	return home, filepath.Join(home, ".config", "agilix-dawn-pp-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "agilix-dawn-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("AGILIX_DAWN_TOKEN", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "agilix-dawn-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "agilix-dawn-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "agilix-dawn-pp-cli", "credentials.toml")
