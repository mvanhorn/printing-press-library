...
type Config struct {
	...
	AgilixDawnToken  string          `toml:"dawn_token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://drivered.agilixdawn.com/api",
	}
	...
	// Env var overrides
	if v := os.Getenv("AGILIX_DAWN_TOKEN"); v != "" {
		cfg.AgilixDawnToken = v
		cfg.markEnvOverride("AgilixDawnToken")
		cfg.AuthSource = "env:AGILIX_DAWN_TOKEN"
		cfg.CredentialSource = "env:AGILIX_DAWN_TOKEN"
	}
	...
	if v := os.Getenv("AGILIX_DAWN_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("AGILIX_DAWN_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "agilix-dawn-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	replacements := map[string]string{
		...
		"AGILIX_DAWN_TOKEN": c.AgilixDawnToken,
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		AgilixDawnToken: c.AgilixDawnToken,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.AgilixDawnToken = creds.AgilixDawnToken
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.AgilixDawnToken = token
	delete(c.envOverrides, "AgilixDawnToken")
	c.updateFileConfigField("AgilixDawnToken")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["AgilixDawnToken"] {
			out.AgilixDawnToken = c.fileConfig.AgilixDawnToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "AgilixDawnToken":
		c.fileConfig.AgilixDawnToken = c.AgilixDawnToken
	}
}
