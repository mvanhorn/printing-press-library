...
func toolOptionForFlag(flag *pflag.Flag) mcplib.ToolOption {
	...
	case "stringSlice", "stringArray":
		// Repeatable flags: expose as a JSON array of strings so MCP callers
		// pass discrete values. cliArgsFromMCP emits one --flag per element,
		// which lets pflag stringArray (which does not comma-split) receive
		// each value intact — important where commas are legitimate content
		// (e.g. quiz option/answer text).
		return mcplib.WithArray(flag.Name, append(propOpts, mcplib.WithStringItems())...)
	default:
