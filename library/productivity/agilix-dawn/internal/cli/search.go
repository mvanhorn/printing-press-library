...
Run sync first to populate the local search index.`,
  ...
  agilix-dawn-pp-cli search "status"

  # Force local search explicitly
  agilix-dawn-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  agilix-dawn-pp-cli search "status" --type concept --data-source local
  # JSON output for piping
  agilix-dawn-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'agilix-dawn-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "concept":
				results, err = db.SearchConcept(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchConcept(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search concept failed: %w", searchErr)
					}
