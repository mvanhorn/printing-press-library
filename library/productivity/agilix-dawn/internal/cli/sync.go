...
Resource scoping:
  ...
  agilix-dawn-pp-cli sync

  # Sync specific resources only
  agilix-dawn-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  agilix-dawn-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  agilix-dawn-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  agilix-dawn-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  agilix-dawn-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "concept":
		return db.UpsertConcept(data)
	case "config":
		return db.UpsertConfig(data)
	case "organization":
		return db.UpsertOrganization(data)
	case "progress":
		return db.UpsertProgress(data)
	case "purchase":
		return db.UpsertPurchase(data)
	case "user":
...
func responsePathForResource(resource, path string) []string {
	...
	case "concept\x00/concept":
		return []string{"matches"}
	case "conversation\x00/conversation":
		return []string{"matches"}
	case "organization\x00/organization":
		return []string{"matches"}
	case "progress\x00/progress":
		return []string{"matches"}
	case "purchase\x00/purchase":
		return []string{"matches"}
	case "user\x00/user":
		return []string{"matches"}
	}
