...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  agilix-dawn-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  agilix-dawn-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"concept", "conversation", "organization", "progress", "purchase", "user"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  agilix-dawn-pp-cli workflow status

  # Show status as JSON
  agilix-dawn-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("agilix-dawn-pp-cli")
			}
