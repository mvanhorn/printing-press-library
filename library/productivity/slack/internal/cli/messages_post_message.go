...
func newMessagesPostMessageCmd(flags *rootFlags) *cobra.Command {
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if stdinBody {
				...
				if cmd.Flags().Changed("unfurl-links") {
					bodyMap["unfurl_links"] = bodyUnfurlLinks
				}
			}
	...
	cmd.Flags().StringVar(&bodyChannel, "channel", "", "Channel ID or name to send the message to")
	cmd.Flags().StringVar(&bodyText, "text", "", "Message text (supports mrkdwn formatting)")
	cmd.Flags().StringVar(&bodyThreadTs, "thread-ts", "", "Thread timestamp to reply in a thread")
	cmd.Flags().BoolVar(&bodyUnfurlLinks, "unfurl-links", false, "Enable or disable link unfurling")
