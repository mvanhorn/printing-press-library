...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Set credentials with: export SLACK_BOT_TOKEN=\"your-token-here\" SLACK_USER_TOKEN=\"your-token-here\""))
	}
	if len(trimmed) == 0 {
