...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Slack",
		RunE:  parentNoSubcommandRunE(flags),
	}

	cmd.AddCommand(newAuthSetupCmd(flags))
	cmd.AddCommand(newAuthStatusCmd(flags))
	// set-token must stay registered: doctor's credentials_location_warning
	// and the client's placeholder-credential error both instruct the user to
	// run "auth set-token <token>". Leaving it unregistered turns both of
	// those remediation hints into an "unknown command" dead end.
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  slack-pp-cli auth setup\n  slack-pp-cli auth setup --launch",
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  slack-pp-cli auth set-token YOUR_TOKEN_HERE",
		Args:    cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			// Bare invocation prints help rather than a cobra arg error: the
			// token is a secret, so the common mistake is running the command
			// first and pasting the value second.
			if len(args) == 0 {
				return cmd.Help()
			}
			cfg, err := config.Load(flags.configPath)
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("SLACK_BOT_TOKEN") != "" {
				envStillSet = "SLACK_BOT_TOKEN"
			}
			if envStillSet == "" && os.Getenv("SLACK_USER_TOKEN") != "" {
				envStillSet = "SLACK_USER_TOKEN"
			}
