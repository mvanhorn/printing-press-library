...
func newDndSetSnoozeCmd(flags *rootFlags) *cobra.Command {
	var bodyNumMinutes int
	var stdinBody bool

	cmd := &cobra.Command{
		Use:         "set-snooze",
		Short:       "Turn on Do Not Disturb for a specified number of minutes",
		Example:     "  slack-pp-cli dnd set-snooze",
		Annotations: map[string]string{"pp:endpoint": "dnd.set_snooze", "pp:method": "POST", "pp:path": "/dnd.setSnooze"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if stdinBody {
				...
				body = bodyMap
				if bodyNumMinutes != 0 {
					bodyMap["num_minutes"] = bodyNumMinutes
				}
			}
			...
			if !flags.dryRun && statusCode >= 200 && statusCode < 300 && (partialFailure == nil || flags.allowPartialFailure) {
				writeMutationResponseToStore(cmd.Context(), "dnd", data, "")
			}
				...
				if json.Unmarshal(data, &items) == nil && len(items) > 0 {
					if err := printAutoTable(cmd.OutOrStdout(), items); err != nil {
						...
						if partialFailure != nil && !flags.allowPartialFailure {
							return partialFailureErr(fmt.Errorf("partial failure in %s response: %s", "dnd", partialFailure.Message))
						}
						return nil
					}
				} else {
	...
	cmd.Flags().IntVar(&bodyNumMinutes, "num-minutes", 0, "Number of minutes to snooze")
