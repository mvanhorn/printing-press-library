...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
  ...
  slack-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set credentials with: export SLACK_BOT_TOKEN=\"your-token-here\" SLACK_USER_TOKEN=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if os.Getenv("SLACK_BOT_TOKEN") != "" {
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "SLACK_BOT_TOKEN")
			}
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "slack-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'slack-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
