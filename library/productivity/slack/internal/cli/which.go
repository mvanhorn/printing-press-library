...
var whichIndex = []whichEntry{
	{Command: "archive recall", Description: "Find messages in your local archive, including ones Slack has already hidden behind the 90-day retention wall.", Group: "Local archive that outlives Slack", WhyItMatters: "Reach for this instead of search when the answer may be older than 90 days, or when you only hold a bot token, since Slack's search endpoint rejects bot tokens outright."},
	{Command: "archive coverage", Description: "Show what date range your local mirror actually holds per channel, and where the gaps are.", Group: "Local archive that outlives Slack", WhyItMatters: "Check this before trusting an empty recall result, so you can tell a genuine absence from an unsynced range."},
	{Command: "catchup", Description: "See what happened while you were away: new volume per channel, messages that mention you, and threads still waiting on your reply.", Group: "Obligation over volume", WhyItMatters: "Use this as the single call that replaces listing channels, pulling history per channel, and resolving mentions by hand."},
	{Command: "threads stale", Description: "List threads across the archive where the last reply is not yours and nobody has answered since.", Group: "Obligation over volume", WhyItMatters: "Pick this when the question is what is unanswered, rather than what is recent."},
	{Command: "health", Description: "Compare your channels by messages per day, distinct posters, median first-reply latency, and days idle.", Group: "Workspace stewardship", WhyItMatters: "Use this for archive-candidate decisions when you are not a workspace admin and have no export or admin console."},
	{Command: "users activity", Description: "Profile where one person posts, which threads they carry, and when they were last seen.", Group: "Workspace stewardship", WhyItMatters: "Reach for this for handoff and standup context about a named teammate, not about yourself."},
	{Command: "users whois", Description: "Turn an opaque Slack ID, handle, or email into one card with shared channels, timezone, DND state, and last-seen.", Group: "Identity resolution", WhyItMatters: "Use this the moment a payload hands you a bare U-prefixed or C-prefixed ID, instead of calling users info per identifier."},
}
