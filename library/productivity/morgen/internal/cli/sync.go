...
Exit codes & warnings:
  ...
  morgen-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  morgen-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  morgen-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  morgen-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  morgen-pp-cli sync --latest-only`,
...
func syncResource(c interface {
	...
	for {
		...
		// Pagination params (limit/cursor) only apply to resources whose
		// list endpoint accepts them. Morgen rejects unknown query params
		// with HTTP 400 (strict whitelist), so calendars/integrations/events
		// must not receive limit or cursor.
		...
		// Set since filter only when this resource declares a temporal
		// filter param (empty string = no incremental filter supported).
...
// resourceSupportsPagination reports whether a resource's list endpoint
// accepts limit/cursor query params. Morgen only paginates tags and tasks;
// other list endpoints (calendars, integrations, events) take a fixed param
// set and reject unknown query params with HTTP 400.
...
// list endpoint uses for incremental (updated-after) filtering, or "" when
// the endpoint declares no temporal filter. Morgen's list endpoints accept
// no sync-level temporal filter, so this is empty for every resource.
