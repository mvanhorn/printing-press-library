		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export CAL_COM_API_KEY=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("CAL_COM_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "CAL_COM_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "CAL_COM_API_KEY")
			}
			if os.Getenv("CAL_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "CAL_API_KEY")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "CAL_API_KEY")
			}
			switch {
