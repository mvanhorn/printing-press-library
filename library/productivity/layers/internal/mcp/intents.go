...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("agenda_month",
			mcplib.WithDescription("Fetch one localized calendar month without mutating the portal."),
			mcplib.WithString("months", mcplib.Required(), mcplib.Description("Override the recipe's --months value.")),
		),
		handleAgendaMonth,
	)
}

// handleAgendaMonth runs the agenda_month recipe intent: Fetch one localized calendar month without mutating the portal.
func handleAgendaMonth(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	var missingMonths bool
	args, missingMonths = appendRecipeStringFlag(args, "months", input["months"], "", true, false)
	if missingMonths {
		return mcplib.NewToolResultError("--months is required"), nil
	}
