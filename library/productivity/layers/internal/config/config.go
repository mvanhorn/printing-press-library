...
type Config struct {
	...
	LayersToken      string          `toml:"token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.layers.digital",
	}
	...
	// Env var overrides
	if v := os.Getenv("LAYERS_TOKEN"); v != "" {
		cfg.LayersToken = v
		cfg.markEnvOverride("LayersToken")
		cfg.AuthSource = "env:LAYERS_TOKEN"
		cfg.CredentialSource = "env:LAYERS_TOKEN"
	}
	// Layers' portal API requires the active community on authenticated
	// requests. Keep it process-scoped so selecting a school does not require
	// writing configuration alongside the bearer token.
	if v := strings.TrimSpace(os.Getenv("LAYERS_COMMUNITY_ID")); v != "" {
		if cfg.Headers == nil {
	...
	if v := os.Getenv("LAYERS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("LAYERS_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "layers-pp-cli", "config.toml"), nil
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.LayersToken == "" {
		c.LayersToken = creds.LayersToken
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.LayersToken = ""
	delete(c.envOverrides, "LayersToken")
	c.updateFileConfigField("LayersToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["LayersToken"] {
			out.LayersToken = c.fileConfig.LayersToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "LayersToken":
		c.fileConfig.LayersToken = c.LayersToken
	}
}
