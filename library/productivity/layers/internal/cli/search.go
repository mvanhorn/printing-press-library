...
func newSearchCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "search <query>",
		Short: "Full-text search across locally synced Layers data",
		Long: `Search data using FTS5 full-text search on the private local index.

Layers does not expose a unified cross-service search endpoint. Auto and local
modes therefore search only data previously collected with sync. Live mode is
rejected explicitly instead of misrepresenting community lookup as search.`,
		Example: `  # Search the private local index
  layers-pp-cli search "error timeout"

  # Force local search only
  layers-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  layers-pp-cli search "status" --type communities --data-source local
  # JSON output for piping
  layers-pp-cli search "critical" --json --limit 20`,
		Annotations: map[string]string{
			"mcp:hidden":    "true",
			"pp:happy-args": "<query>=semester;--data-source=local",
		},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.dataSource == "live" {
				return fmt.Errorf("Layers has no unified live search endpoint; run 'layers-pp-cli sync' and use --data-source local")
			}
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'layers-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "post-delivery":
				results, err = db.SearchPostDelivery(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchPostDelivery(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search post_delivery failed: %w", searchErr)
					}
