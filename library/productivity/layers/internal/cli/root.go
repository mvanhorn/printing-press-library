...
type rootFlags struct {
	...
	// invocation. Mirrors the LAYERS_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "layers-pp-cli",
		Short: `Layers CLI — School announcements, agenda events, account context, and a private local search surface in one agent-ready CLI.`,
		Long: `Layers CLI — School announcements, agenda events, account context, and a private local search surface in one agent-ready CLI.
...
Health check: run 'layers-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("layers-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'layers-pp-cli profile list')")
	...
	rootCmd.AddCommand(newHomeCmd(flags))
	...
	rootCmd.AddCommand(newCommunitiesPromotedCmd(flags))
	rootCmd.AddCommand(newContextPromotedCmd(flags))
	rootCmd.AddCommand(newMePromotedCmd(flags))
	rootCmd.AddCommand(newNotificationBadgePromotedCmd(flags))
	rootCmd.AddCommand(newPostDeliveryPromotedCmd(flags))
	rootCmd.AddCommand(newSsoPromotedCmd(flags))
