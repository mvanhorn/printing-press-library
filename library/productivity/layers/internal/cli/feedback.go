...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("LAYERS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("LAYERS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "layers-pp-cli/feedback")
...
When ` + "`LAYERS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`LAYERS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  layers-pp-cli feedback list
  layers-pp-cli feedback list --limit 5
  layers-pp-cli feedback list --json`,
