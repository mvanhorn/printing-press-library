...
type Config struct {
	...
	ProductiveApiToken string          `toml:"api_token"`
	// ProductiveOrganizationId is the required X-Organization-Id header value.
	// Productive's API needs BOTH X-Auth-Token and X-Organization-Id on every
	// request; the generator's single-header auth model only covers the token,
	// so the org id is loaded here and injected into Headers (see applyOrgHeader).
	ProductiveOrganizationId string `toml:"organization_id"`
}

// orgHeaderName is the second required Productive auth header.
const orgHeaderName = "X-Organization-Id"

// applyOrgHeader injects X-Organization-Id into the Headers map so client.do()
// sends it on every request (generated endpoint commands included). Called at
// the end of Load once the org id has been resolved from config/env.
func (c *Config) applyOrgHeader() {
	if c.ProductiveOrganizationId == "" {
		return
	}
	if c.Headers == nil {
		c.Headers = map[string]string{}
	}
	c.Headers[orgHeaderName] = c.ProductiveOrganizationId
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("PRODUCTIVE_API_TOKEN"); v != "" {
		cfg.ProductiveApiToken = v
		cfg.markEnvOverride("ProductiveApiToken")
		cfg.AuthSource = "env:PRODUCTIVE_API_TOKEN"
		cfg.CredentialSource = "env:PRODUCTIVE_API_TOKEN"
	}
	if v := os.Getenv("PRODUCTIVE_ORGANIZATION_ID"); v != "" {
		cfg.ProductiveOrganizationId = v
		cfg.markEnvOverride("ProductiveOrganizationId")
	}
	...
	if v := os.Getenv("PRODUCTIVE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	cfg.applyOrgHeader()
	return cfg, nil
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("PRODUCTIVE_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "productive-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.ProductiveApiToken
	if token == "" {
		return ""
	}
	if c.ProductiveApiToken == "" {
		return ""
	}
	replacements := map[string]string{
		"token":                token,
		"api_token":            c.ProductiveApiToken,
		"PRODUCTIVE_API_TOKEN": c.ProductiveApiToken,
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		ProductiveApiToken: c.ProductiveApiToken,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.ProductiveApiToken = creds.ProductiveApiToken
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.ProductiveApiToken = token
	delete(c.envOverrides, "ProductiveApiToken")
	c.updateFileConfigField("ProductiveApiToken")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["ProductiveApiToken"] {
			out.ProductiveApiToken = c.fileConfig.ProductiveApiToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "ProductiveApiToken":
		c.fileConfig.ProductiveApiToken = c.ProductiveApiToken
	}
}
