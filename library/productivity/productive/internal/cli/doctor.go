...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  productive-pp-cli doctor
  productive-pp-cli doctor --json
  productive-pp-cli doctor --fail-on warn
  productive-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set your API key with: export PRODUCTIVE_API_TOKEN=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if os.Getenv("PRODUCTIVE_API_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "PRODUCTIVE_API_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "PRODUCTIVE_API_TOKEN")
			}
			switch {
			...
			// Productive requires a SECOND auth header, X-Organization-Id, on
			// every request. A valid token without an org id 401s on every call,
			// so surface its absence explicitly.
			if cfg.ProductiveOrganizationId != "" {
				report["organization_id"] = "configured"
			} else {
				report["organization_id"] = "MISSING — set: export PRODUCTIVE_ORGANIZATION_ID=\"your-org-id\""
			}
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					healthPath := "/organization_memberships"
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "productive-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'productive-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'productive-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'productive-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'productive-pp-cli sync' to refresh."
	}
	return report
}
