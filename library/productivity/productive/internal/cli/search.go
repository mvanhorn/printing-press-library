...
Run sync first to populate the local search index.`,
  ...
  productive-pp-cli search "status"

  # Force local search explicitly
  productive-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  productive-pp-cli search "status" --type activities --data-source local
  # JSON output for piping
  productive-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'productive-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
