...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PRODUCTIVE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PRODUCTIVE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "productive-pp-cli/feedback")
...
When ` + "`PRODUCTIVE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PRODUCTIVE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  productive-pp-cli feedback list
  productive-pp-cli feedback list --limit 5
  productive-pp-cli feedback list --json`,
