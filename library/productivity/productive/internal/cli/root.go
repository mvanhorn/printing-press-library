...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "productive-pp-cli",
		Short: `Full Productive.io v2 API client — list/get across ~48 resource types, generic create/update/delete, 11 report types, and a local sync/search/export/tail data layer, plus first-class revenue-recognition commands (recognized-revenue, invoiced, reconcile, aging).`,
		Long: `Productive CLI — a full Productive.io v2 API client.

Read (list/get) across ~48 resource types, generic create/update/delete on any
resource, 11 report types, and a local sync/search/export/tail data layer for
offline analysis.

Value-add commands (not in the official API docs):
  • recognized-revenue   Recognized (earned) revenue grouped by budget and month in one call.
  • invoiced             Net finalized invoiced amount per budget per month, drafts reported separately.
  • reconcile            Join recognized revenue vs invoiced per budget×month and flag the deltas.
  • aging                Bucket unpaid invoice amounts by age past their due date.
  • export               Stream full paginated pulls of any resource to JSONL or JSON (auto-paginated).
  • sync                 Mirror any resource into local SQLite, then query with SQL and full-text search offline.
...
Health check: run 'productive-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("productive-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'productive-pp-cli profile list')")
	...
	rootCmd.AddCommand(newContactEntriesCmd(flags))
	rootCmd.AddCommand(newCustomFieldsCmd(flags))
	rootCmd.AddCommand(newDealStatusesCmd(flags))
	...
	rootCmd.AddCommand(newHolidayCalendarsCmd(flags))
	rootCmd.AddCommand(newHolidaysCmd(flags))
	rootCmd.AddCommand(newInvoiceAttributionsCmd(flags))
	rootCmd.AddCommand(newInvoicesCmd(flags))
	rootCmd.AddCommand(newLineItemsCmd(flags))
	...
	rootCmd.AddCommand(newServiceTypesCmd(flags))
	rootCmd.AddCommand(newServicesCmd(flags))
	rootCmd.AddCommand(newSubsidiariesCmd(flags))
	rootCmd.AddCommand(newTaskListsCmd(flags))
	...
	rootCmd.AddCommand(newWorkflowStatusesCmd(flags))
	...
	rootCmd.AddCommand(newNovelAgingCmd(flags))
	rootCmd.AddCommand(newNovelInvoicedCmd(flags))
	rootCmd.AddCommand(newNovelRecognizedRevenueCmd(flags))
	rootCmd.AddCommand(newNovelReconcileCmd(flags))
	rootCmd.AddCommand(newCreateCmd(flags))
