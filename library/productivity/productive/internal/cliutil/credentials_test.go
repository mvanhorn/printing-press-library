...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"PRODUCTIVE_CONFIG",
		"PRODUCTIVE_CONFIG_DIR",
		"PRODUCTIVE_DATA_DIR",
		"PRODUCTIVE_STATE_DIR",
		"PRODUCTIVE_CACHE_DIR",
		"PRODUCTIVE_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
	...
	return home, filepath.Join(home, ".config", "productive-pp-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "productive-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("PRODUCTIVE_API_TOKEN", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "productive-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "productive-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "productive-pp-cli", "credentials.toml")
