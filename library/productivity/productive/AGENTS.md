# Productive Printed CLI Agent Guide

This directory is a generated `productive-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
productive-pp-cli doctor --json
productive-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
productive-pp-cli which "<capability>" --json
productive-pp-cli <command> --help
```
...
```bash
productive-pp-cli <command> --agent
```
...
```bash
productive-pp-cli <command> --help
productive-pp-cli <command> --dry-run --agent
```
