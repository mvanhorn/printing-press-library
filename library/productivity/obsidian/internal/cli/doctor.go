...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
  ...
  obsidian-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Check auth. Filesystem mode is the default and needs no token;
			// the REST token is only required for the opt-in `rest` subcommand.
			// Absent token = "not required" (healthy), not a failure.
			authConfigured := false
			restMode := os.Getenv("OBSIDIAN_REST_TOKEN") != ""
			if cfg != nil {
			...
			if os.Getenv("OBSIDIAN_REST_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "OBSIDIAN_REST_TOKEN")
				...
				// Filesystem mode is the default and needs no token. The token
				// is INFO, not ERROR — only --rest mode needs it.
				authEnvInfo = append(authEnvInfo, "OBSIDIAN_REST_TOKEN not set (only required for --rest mode)")
			}
			switch {
			...
			// Vault path is the primary configuration for filesystem mode.
			// Without it, every domain command fails with a config error.
			if cfg != nil {
				if cfg.VaultPath == "" {
					report["vault"] = "ERROR vault path not set (export OBSIDIAN_VAULT_PATH=/path/to/vault)"
				} else if info, statErr := osStat(cfg.VaultPath); statErr != nil {
					report["vault"] = fmt.Sprintf("ERROR vault path %s not accessible: %s", cfg.VaultPath, statErr)
				} else if !info.IsDir() {
					report["vault"] = fmt.Sprintf("ERROR vault path %s is not a directory", cfg.VaultPath)
				} else {
					srcLabel := cfg.VaultPathSource
					if srcLabel == "" {
						srcLabel = "config"
					}
					report["vault"] = fmt.Sprintf("OK %s (from %s)", cfg.VaultPath, srcLabel)
				}
				report["store_path"] = cfg.StorePath
			}

			// Check API connectivity and validate credentials. Only relevant in
			// --rest mode; filesystem-only operation passes through with the
			// vault and store status above as the canonical health signals.
			// Skip the probe entirely when no REST token is set — filesystem
			// mode is the default and a live REST server is not a prerequisite.
			if cfg != nil && cfg.BaseURL != "" && restMode {
