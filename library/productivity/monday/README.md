...
monday-pp-cli search "Q3 launch" --json --select id,board.name,group.title
...
monday-pp-cli items set 12345 --column status=Done --column owner=alice --dry-run
...
- **`complexity-budget`** — Predicts the complexity-points cost of a query and the remaining account-minute budget without executing the body.
  ...
  ```bash
  monday-pp-cli complexity-budget --query ./bulk-status.graphql --json
  ```
...
- **`reconcile`** — Joins a local board to an external CSV by a chosen column-value key; emits only-in-monday, only-in-csv, and diff sets.
  ...
  ```bash
  monday-pp-cli reconcile --against salesforce-export.csv --key sf_account_id --json
  ```
...
- **ComplexityException on a bulk read** — Run `monday-pp-cli complexity-budget --query <file>` first; if predicted points exceed the per-call 5M cap, narrow the field selection or paginate.
