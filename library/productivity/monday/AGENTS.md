# monday.com Printed CLI Agent Guide

This directory is a generated `monday-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
monday-pp-cli doctor --json
monday-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
monday-pp-cli which "<capability>" --json
monday-pp-cli <command> --help
```
...
```bash
monday-pp-cli <command> --help
monday-pp-cli <command> --dry-run --agent
```
