...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  monday-pp-cli doctor
  monday-pp-cli doctor --json
  monday-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export MONDAY_API_TOKEN=<your-key>"
					report["auth_key_url"] = "https://developer.monday.com/api-reference/docs/authentication"
				} else {
			...
			// PATCH(upstream cli-printing-press): saved config credentials satisfy auth even when the env var is not exported.
			authHeader := ""
			if cfg != nil {
				authHeader = cfg.AuthHeader()
			}
			if os.Getenv("MONDAY_API_TOKEN") != "" || authHeader != "" {
				authEnvSet = append(authEnvSet, "MONDAY_API_TOKEN")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "MONDAY_API_TOKEN")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'monday-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'monday-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'monday-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'monday-pp-cli sync' to refresh."
	}
	return report
}
