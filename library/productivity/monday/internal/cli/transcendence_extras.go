...
const queryResolveItem = `query($ids: [ID!]) {
  items(ids: $ids) {
    ...
    column_values {
      ...
      ... on MirrorValue {
        display_value
        mirrored_items { linked_board_item { id name board { id name } } }
      }
      ... on FormulaValue {
        display_value
      }
...
func newReconcileCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  monday-pp-cli reconcile --board 12345 --against salesforce-export.csv --key sf_account_id --monday-column salesforce_id --json",
		Annotations: map[string]string{
			"mcp:read-only": "true",
		},
		RunE: func(cmd *cobra.Command, args []string) error {
	...
	cmd.Flags().StringVar(&csvPath, "against", "", "Path to the external CSV (required).")
