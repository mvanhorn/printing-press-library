...
Exit codes & warnings:
  ...
  monday-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  monday-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  monday-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  monday-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  monday-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Under the live-dogfood harness, curtail a bare sync hard so the
			// matrix's per-command timeout cannot kill it on a large account:
			// one page per resource (latest-only) AND a single resource, since
			// per-resource API round-trips dominate wall time. This proves the
			// sync path works end-to-end without pulling the full dataset. A
			// real run (no PRINTING_PRESS_DOGFOOD env) and an explicit --full
			// are unaffected.
...
func syncResource(c interface {
	...
	if isMondayGraphQLSyncResource(resource) {
		return syncMondayGraphQLResource(c, db, resource, maxPages, started)
	}
...
func defaultSyncResources() []string {
	return []string{"workspaces", "boards", "users", "teams", "items", "columns", "groups", "updates", "docs", "activity_logs"}
}
