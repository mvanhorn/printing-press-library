...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".monday-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MONDAY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MONDAY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "monday-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.monday-pp-cli/feedback.jsonl.
When ` + "`MONDAY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MONDAY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  monday-pp-cli feedback list
  monday-pp-cli feedback list --limit 5
  monday-pp-cli feedback list --json`,
