...
func newComplexityBudgetCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  monday-pp-cli complexity-budget --query ./bulk-items.graphql --json",
		Annotations: map[string]string{
			"mcp:read-only": "true",
		},
		RunE: func(cmd *cobra.Command, args []string) error {
	...
	cmd.Flags().StringVar(&queryFile, "query", "", "Path to a file containing a GraphQL query to probe.")
