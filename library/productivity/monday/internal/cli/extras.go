...
const queryMeetingsList = `query($limit: Int!, $page: Int!) {
  notetaker {
    meetings(limit: $limit, page: $page) {
      id
      title
      start_time
      end_time
...
func newMeetingsCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "meetings",
		Short: "List notetaker meetings (titles, summaries, attendees).",
	}
...
func newMeetingsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:     "list",
		Short:   "List notetaker meetings with title, start time, attendees, and AI-generated summary, paginated by --limit/--page.",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			data, err := c.GraphQL(queryMeetingsList, map[string]any{"limit": limit, "page": page})
			if err != nil {
				return classifyAPIError(err, flags)
			}
			if dryRunOK(flags) {
