...
// numericIDPattern matches a monday.com object id, which is always a run of
// digits.
var numericIDPattern = regexp.MustCompile(`^[0-9]+$`)

// requireNumericID rejects a malformed id argument before any API call. monday
// ids are numeric; a non-numeric id (a typo, or an unexpanded shell glob)
// otherwise reaches the API, which returns an empty result set with no error,
// so the command prints nothing and exits 0 — silently masking bad input.
// Failing fast with a usage error (non-zero exit) surfaces the mistake. A
// numeric-but-nonexistent id is left to proceed: an empty result for a
// well-formed id is a legitimate outcome, not an error.
func requireNumericID(label, val string) error {
	if !numericIDPattern.MatchString(val) {
		return usageErr(fmt.Errorf("%s must be a numeric monday id, got %q", label, val))
	}
	return nil
}
...
// emitDryRun renders a machine-readable description of a planned (un-sent)
// write to stdout, so `--dry-run --json` yields valid JSON instead of an empty
// stream. The client also prints a human-readable request preview to stderr
// when DryRun is set; this is its structured stdout counterpart. Routed through
// printOutputWithFlags so --json/--csv/--compact/--select all apply.
func emitDryRun(cmd *cobra.Command, flags *rootFlags, operation string, vars map[string]any) error {
