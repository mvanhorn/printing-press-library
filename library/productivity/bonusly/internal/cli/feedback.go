...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BONUSLY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BONUSLY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "bonusly-pp-cli/feedback")
...
When ` + "`BONUSLY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BONUSLY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  bonusly-pp-cli feedback list
  bonusly-pp-cli feedback list --limit 5
  bonusly-pp-cli feedback list --json`,
