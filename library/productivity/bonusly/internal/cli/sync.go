...
Resource scoping:
  ...
  bonusly-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  bonusly-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  bonusly-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  bonusly-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  bonusly-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "incentives":
		return db.UpsertIncentives(data)
	case "locations":
	...
	case "recognition":
		return db.UpsertRecognition(data)
	case "redemptions":
		return db.UpsertRedemptions(data)
	case "users":
...
func knownSyncResourceNames() []string {
	...
	names := []string{
		...
		"org",
		"recognition",
		"recognition-last-given",
		"recognition-recognition-types",
		"redemptions",
		"users",
	}
	return names
...
func syncResourcePath(resource string) (string, error) {
	// pp:hand-edit bonusly-remove-broken-commands — awards, groups, meetings
	// removed: no working endpoint found for any of them. org's path is
	// still the spec-derived (broken) /users/top_level — fixing sync's own
	// copy of this to match org_top.go's live-verified /users?top_level=true
	// is out of scope for this pass (syncResourcePath returns a bare path,
	// no query-param support, and sync's paginator appends its own ?params).
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		"departments":                   "/users/departments",
		"locations":                     "/users/locations",
		"org":                           "/users/top_level",
		"recognition":                   "/bonuses",
		"recognition-last-given":        "/bonuses/last_given",
		"recognition-recognition-types": "/bonuses/recognition_types",
