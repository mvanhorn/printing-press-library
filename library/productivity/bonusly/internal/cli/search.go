...
In local mode: searches locally synced data only.`,
  ...
  bonusly-pp-cli search "error timeout"

  # Force local search only
  bonusly-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  bonusly-pp-cli search "status" --type awards --data-source local
  # JSON output for piping
  bonusly-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.dataSource != "local" {
				...
				data, getErr := c.Get(cmd.Context(), "/users/search", map[string]string{
					"query": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'bonusly-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "meetings":
				results, err = db.SearchMeetings(query, limit)
			case "recognition":
				results, err = db.SearchRecognition(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchMeetings(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search meetings failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchRecognition(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search recognition failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
