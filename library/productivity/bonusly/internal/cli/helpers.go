...
// pp:hand-edit bonusly-endpoint-fix — shared helper for the "resolve my own
// user id" pattern needed by recognition_gap, balance_history,
// recognition_search_mine, redemptions_list-mine, and recognition_audit.
// /users/me wraps the user object in {"success":...,"result":{...}}; a raw
// c.Get()+Unmarshal straight into types.User silently leaves every field
// (including Id) empty. Routes through resolveReadWithStrategyAndResponsePath
// (the same helper users_me.go uses) rather than a raw c.Get(), on the
// chance that matters for some environments invoking this binary — the two
// should be equivalent, but the raw-c.Get() call sites were the only ones
// observed failing under an otherwise-identical live session.
func resolveMyUser(ctx context.Context, c *client.Client, flags *rootFlags, errWriter io.Writer) (*types.User, error) {
	raw, _, err := resolveReadWithStrategyAndResponsePath(ctx, c, flags, "live", "users", false, "/users/me", nil, nil, "", errWriter)
	if err != nil {
		return nil, err
	}
	var envelope struct {
	...
	if err := json.Unmarshal(raw, &envelope); err != nil {
		return nil, fmt.Errorf("parsing /users/me response: %w", err)
	}
	if envelope.Result.Id == "" {
		return nil, fmt.Errorf("could not resolve the authenticated user's id from /users/me")
	}
...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'bonusly-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token."+
			"\n      Set it with: bonusly-pp-cli auth set-token <token> or export BONUSLY_API_TOKEN=\"your-token-here\""+
			"\n      Run 'bonusly-pp-cli doctor' to check auth status.", err))
...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Set it with: bonusly-pp-cli auth set-token <token> or export BONUSLY_API_TOKEN=\"your-token-here\""))
	}
	if len(trimmed) == 0 {
