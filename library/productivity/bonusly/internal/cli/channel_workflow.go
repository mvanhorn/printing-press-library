...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  bonusly-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  bonusly-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"awards", "groups", "meetings", "org", "redemptions"}
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  bonusly-pp-cli workflow status

  # Show status as JSON
  bonusly-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("bonusly-pp-cli")
			}
