...
Run sync first to populate the local search index.`,
  ...
  zoom-pp-cli search "status"

  # Force local search explicitly
  zoom-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  zoom-pp-cli search "status" --type accounts --data-source local
  # JSON output for piping
  zoom-pp-cli search "status" --json --limit 20`,
