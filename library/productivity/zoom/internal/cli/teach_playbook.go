...
agents can record playbooks without a file on disk.`,
		Example: `  zoom-pp-cli teach-playbook --query "<question that anchors the family>" --playbook-file ~/playbooks/recipe.json --notes-file ~/playbooks/recipe-notes.md`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dryRunOK(flags) {
				return writeDryRun(flags, "teach-playbook")
			}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  zoom-pp-cli playbook amend --query "<exact recall query>" --add-note "summary endpoint envelope: data lives at .results.header, not .header"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			cmd.SilenceUsage = true
			cmd.SilenceErrors = true
			if noLearnActive(flags) {
			...
			if dryRunOK(flags) {
				return writeDryRun(flags, "playbook amend")
			}
			if strings.TrimSpace(query) == "" {
...
func newPlaybookListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example:     `  zoom-pp-cli playbook list --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return writeDryRun(flags, "playbook list")
			}
			dbPath = learnDBPath(dbPath)
			s, err := store.OpenWithContext(cmd.Context(), dbPath)
			if err != nil {
