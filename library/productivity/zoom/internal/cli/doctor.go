...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
  ...
  zoom-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set your API key with: export ZOOM_S2S_ACCESS_TOKEN=\"your-token-here\""
					report["auth_key_url"] = "https://zoom.us/oauth/token"
				} else {
					authConfigured = true
			...
			if os.Getenv("ZOOM_S2S_ACCESS_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "ZOOM_S2S_ACCESS_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "ZOOM_S2S_ACCESS_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "zoom-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'zoom-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
