...
func AdoptVerifiedLegacyDatabase(ctx context.Context, session *Session, state *LegacyMigrationState) error {
	...
	if _, err := source.ExecContext(ctx, "VACUUM INTO '"+quotedTarget+"'"); err != nil { // #nosec G202 -- VACUUM INTO cannot take bind parameters; target path is app-derived and single-quote escaped
