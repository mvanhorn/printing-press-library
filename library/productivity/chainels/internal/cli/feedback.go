...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".chainels-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CHAINELS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CHAINELS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "chainels-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.chainels-pp-cli/feedback.jsonl.
When ` + "`CHAINELS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CHAINELS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  chainels-pp-cli feedback list
  chainels-pp-cli feedback list --limit 5
  chainels-pp-cli feedback list --json`,
