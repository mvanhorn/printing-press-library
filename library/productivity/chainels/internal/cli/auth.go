...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Chainels",
	}

	cmd.AddCommand(newAuthSetupCmd(flags))
	cmd.AddCommand(newAuthLoginCmd(flags))
	cmd.AddCommand(newAuthClientCredentialsCmd(flags))
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  chainels-pp-cli auth setup\n  chainels-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Register an app and copy your credentials at: https://www.chainels.com/developer")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then run:")
			fmt.Fprintln(w, "  chainels-pp-cli auth login --client-id <id> --client-secret <secret>")
			if !launch {
				return nil
			}
			launchURL := "https://www.chainels.com/developer"
...
func newAuthLoginCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if authURL == "" {
				authURL = "https://www.chainels.com/oauth/authorize"
			}
			params := url.Values{
				"client_id":     {clientID},
			...
			scopes := []string{"basic", "chat", "create.account", "manage.members", "read.access_control", "read.account.email", "read.account.notifications", "read.bans", "read.booking", "read.invoices", "read.request_forms", "read.turnover", "send.alarm", "write.access_control", "write.account", "write.booking", "write.company", "write.invoices", "write.messages", "write.request_forms", "write.turnover",  }
			if len(scopes) > 0 {
	...
	cmd.Flags().StringVar(&clientID, "client-id", os.Getenv("CHAINELS_CLIENT_ID"), "OAuth2 client ID")
	cmd.Flags().StringVar(&clientSecret, "client-secret", os.Getenv("CHAINELS_CLIENT_SECRET"), "OAuth2 client secret")
