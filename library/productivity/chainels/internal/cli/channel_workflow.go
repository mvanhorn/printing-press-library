...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  chainels-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  chainels-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"alarms", "alarms-recipients", "bans", "service-accounts", "storeban",  }
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  chainels-pp-cli workflow status

  # Show status as JSON
  chainels-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("chainels-pp-cli")
			}
