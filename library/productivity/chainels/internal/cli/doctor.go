...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  chainels-pp-cli doctor
  chainels-pp-cli doctor --json
  chainels-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export CHAINELS_OAUTH_CODE=<your-key>"
					report["auth_key_url"] = "https://www.chainels.com/developer"
				} else {
					authConfigured = true
			...
			if os.Getenv("CHAINELS_OAUTH_CODE") != "" {
				authEnvSet = append(authEnvSet, "CHAINELS_OAUTH_CODE")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "CHAINELS_OAUTH_CODE")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'chainels-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'chainels-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'chainels-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'chainels-pp-cli sync' to refresh."
	}
	return report
}
