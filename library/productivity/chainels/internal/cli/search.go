...
In local mode: searches locally synced data only.`,
  ...
  chainels-pp-cli search "error timeout"

  # Force local search only
  chainels-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  chainels-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  chainels-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'chainels-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "bans":
				results, err = db.SearchBans(query, limit)
			case "booking":
				results, err = db.SearchBooking(query, limit)
			case "discounts":
				results, err = db.SearchDiscounts(query, limit)
			case "invite_templates":
				results, err = db.SearchInviteTemplates(query, limit)
			case "issues":
				results, err = db.SearchIssues(query, limit)
			case "reporting":
				results, err = db.SearchReporting(query, limit)
			case "requests":
				results, err = db.SearchRequests(query, limit)
			case "service_accounts":
				results, err = db.SearchServiceAccounts(query, limit)
			case "spaces":
				results, err = db.SearchSpaces(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchBans(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search bans failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchBooking(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search booking failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchDiscounts(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search discounts failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchInviteTemplates(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search invite_templates failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchIssues(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search issues failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchRequests(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search requests failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchServiceAccounts(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search service_accounts failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchSpaces(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search spaces failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
