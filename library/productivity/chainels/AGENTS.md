# Chainels Printed CLI Agent Guide

This directory is a generated `chainels-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
chainels-pp-cli doctor --json
chainels-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
chainels-pp-cli which "<capability>" --json
chainels-pp-cli <command> --help
```
...
```bash
chainels-pp-cli <command> --help
chainels-pp-cli <command> --dry-run --agent
```
