...
This directory is a generated `canvas-lms-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
canvas-lms-pp-cli doctor --json
canvas-lms-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
canvas-lms-pp-cli which "<capability>" --json
canvas-lms-pp-cli <command> --help
```
...
```bash
canvas-lms-pp-cli <command> --agent
```
...
```bash
canvas-lms-pp-cli <command> --help
canvas-lms-pp-cli <command> --dry-run --agent
```
