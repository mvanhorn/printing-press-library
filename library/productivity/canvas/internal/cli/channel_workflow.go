...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  canvas-lms-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  canvas-lms-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"calendar-events", "courses", "users", "users-activity-stream", "users-self-activity-stream", "users-self-course-nicknames", "users-self-todo-item-count", "users-self-upcoming-events"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  canvas-lms-pp-cli workflow status

  # Show status as JSON
  canvas-lms-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("canvas-lms-pp-cli")
			}
