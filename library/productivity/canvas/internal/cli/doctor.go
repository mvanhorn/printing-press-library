...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  canvas-lms-pp-cli doctor
  canvas-lms-pp-cli doctor --json
  canvas-lms-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export CANVAS_API_TOKEN=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("CANVAS_API_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "CANVAS_API_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "CANVAS_API_TOKEN")
			}
			if os.Getenv("CANVAS_BASE_URL") != "" {
				authEnvSet = append(authEnvSet, "CANVAS_BASE_URL")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "CANVAS_BASE_URL")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'canvas-lms-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'canvas-lms-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'canvas-lms-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'canvas-lms-pp-cli sync' to refresh."
	}
	return report
}
