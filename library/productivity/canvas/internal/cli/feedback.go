...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".canvas-lms-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CANVAS_LMS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CANVAS_LMS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/productivity/canvas/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.canvas-lms-pp-cli/feedback.jsonl.
When ` + "`CANVAS_LMS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CANVAS_LMS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  canvas-lms-pp-cli feedback list
  canvas-lms-pp-cli feedback list --limit 5
  canvas-lms-pp-cli feedback list --json`,
