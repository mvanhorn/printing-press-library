...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  canvas-lms-pp-cli auth setup\n  canvas-lms-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "No setup URL is configured for this CLI; check the API's docs.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export CANVAS_API_TOKEN=\"<your-token>\"")
			fmt.Fprintln(w, "  export CANVAS_BASE_URL=\"<your-token>\"")
			fmt.Fprintln(w, "  canvas-lms-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  canvas-lms-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export CANVAS_API_TOKEN=\"your-token-here\"")
				fmt.Fprintln(w, "  export CANVAS_BASE_URL=\"your-token-here\"")
				fmt.Fprintf(w, "  canvas-lms-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  canvas-lms-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  canvas-lms-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("CANVAS_API_TOKEN") != "" {
				envStillSet = "CANVAS_API_TOKEN"
			}
			if envStillSet == "" && os.Getenv("CANVAS_BASE_URL") != "" {
				envStillSet = "CANVAS_BASE_URL"
			}
