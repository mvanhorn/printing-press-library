...
Exit codes & warnings:
  ...
  canvas-lms-pp-cli sync

  # Sync specific resources only
  canvas-lms-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  canvas-lms-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  canvas-lms-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  canvas-lms-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  canvas-lms-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("canvas-lms-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/canvas-lms-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "activity_stream":
		return db.UpsertActivityStream(data)
	...
	case "preview_html":
		return db.UpsertPreviewHtml(data)
	case "quizzes":
	...
	case "users_temporary_enrollment_status":
		return db.UpsertUsersTemporaryEnrollmentStatus(data)
...
func defaultSyncResources() []string {
	return []string{
		...
		"users-self-activity-stream",
		"users-self-course-nicknames",
		"users-self-todo-item-count",
		"users-self-upcoming-events",
	}
}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"calendar-events":             "/v1/calendar_events",
		"courses":                     "/v1/courses",
		"users":                       "/v1/users/self/todo",
		"users-activity-stream":       "/v1/users/activity_stream",
		"users-self-activity-stream":  "/v1/users/self/activity_stream",
		"users-self-course-nicknames": "/v1/users/self/course_nicknames",
		"users-self-todo-item-count":  "/v1/users/self/todo_item_count",
		"users-self-upcoming-events":  "/v1/users/self/upcoming_events",
	}
