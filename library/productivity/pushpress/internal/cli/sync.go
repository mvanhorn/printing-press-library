...
Exit codes & warnings:
  ...
  pushpress-pp-cli sync

  # Sync specific resources only
  pushpress-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  pushpress-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  pushpress-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  pushpress-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  pushpress-pp-cli sync --latest-only`,
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "installs", ParentTable: "apps", ParentIDParam: "app_id", PathTemplate: "/apps/{app_id}/installs", KeyField: ""},
	}
}
