...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".pushpress-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PUSHPRESS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PUSHPRESS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "pushpress-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.pushpress-pp-cli/feedback.jsonl.
When ` + "`PUSHPRESS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PUSHPRESS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pushpress-pp-cli feedback list
  pushpress-pp-cli feedback list --limit 5
  pushpress-pp-cli feedback list --json`,
