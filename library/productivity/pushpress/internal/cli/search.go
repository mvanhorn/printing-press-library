...
In local mode: searches locally synced data only.`,
  ...
  pushpress-pp-cli search "error timeout"

  # Force local search only
  pushpress-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  pushpress-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  pushpress-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'pushpress-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "apps":
				results, err = db.SearchApps(query, limit)
			case "keys":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchApps(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search apps failed: %w", searchErr)
					}
