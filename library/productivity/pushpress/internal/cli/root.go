...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "pushpress-pp-cli",
		Short: `Pushpress CLI — Every PushPress /v3 endpoint that exists, plus the going-dark report and the trainer-dashboard one-liner the API itself…`,
		Long: `Pushpress CLI — Every PushPress /v3 endpoint that exists, plus the going-dark report and the trainer-dashboard one-liner the API itself…
...
Health check: run 'pushpress-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("pushpress-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'pushpress-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCheckinsCmd(flags))
	...
	// Transcendence — hand-built from the absorb manifest.
	rootCmd.AddCommand(newGoingDarkCmd(flags))
	rootCmd.AddCommand(newRecencyCmd(flags))
	rootCmd.AddCommand(newRosterCmd(flags))
	rootCmd.AddCommand(newKPICmd(flags))
	rootCmd.AddCommand(newMemberCmd(flags))
	rootCmd.AddCommand(newClassMixCmd(flags))
	// Gap-flagged stubs — feature categories not in /v3 (per user briefing).
	rootCmd.AddCommand(newPlansStubCmd(flags))
	rootCmd.AddCommand(newMRRStubCmd(flags))
	rootCmd.AddCommand(newSignupsStubCmd(flags))
	rootCmd.AddCommand(newCancellationsStubCmd(flags))
	rootCmd.AddCommand(newClassesStubCmd(flags))
	rootCmd.AddCommand(newLeadsStubCmd(flags))
	rootCmd.AddCommand(newTasksStubCmd(flags))
	rootCmd.AddCommand(newNotesStubCmd(flags))
	rootCmd.AddCommand(newCohortStubCmd(flags))
