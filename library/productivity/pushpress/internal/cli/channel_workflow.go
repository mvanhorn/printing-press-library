...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  pushpress-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  pushpress-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"apps", "checkins", "customers", "keys", "webhooks"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  pushpress-pp-cli workflow status

  # Show status as JSON
  pushpress-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("pushpress-pp-cli")
			}
