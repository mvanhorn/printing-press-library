# PushPress Printed CLI Agent Guide

This directory is a generated `pushpress-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
pushpress-pp-cli doctor --json
pushpress-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
pushpress-pp-cli which "<capability>" --json
pushpress-pp-cli <command> --help
```
...
```bash
pushpress-pp-cli <command> --agent
```
...
```bash
pushpress-pp-cli <command> --help
pushpress-pp-cli <command> --dry-run --agent
```
