# DEVONthink Printed CLI Agent Guide

This directory is a generated `devonthink-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
devonthink-pp-cli doctor --json
devonthink-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
devonthink-pp-cli which "<capability>" --json
devonthink-pp-cli <command> --help
```
...
```bash
devonthink-pp-cli <command> --agent
```
...
```bash
devonthink-pp-cli <command> --help
devonthink-pp-cli <command> --dry-run --agent
```
