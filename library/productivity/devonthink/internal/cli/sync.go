...
Resource scoping:
  ...
  devonthink-pp-cli sync

  # Sync specific resources only
  devonthink-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  devonthink-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  devonthink-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  devonthink-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  devonthink-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "ingest":
		return db.UpsertIngest(data)
	case "ledger":
		return db.UpsertLedger(data)
	case "mcp":
		return db.UpsertMcp(data)
	case "media":
		return db.UpsertMedia(data)
	case "mirror":
		return db.UpsertMirror(data)
	case "records":
		return db.UpsertRecords(data)
	case "runtime":
		return db.UpsertRuntime(data)
	case "selection":
		return db.UpsertSelection(data)
	case "tags":
