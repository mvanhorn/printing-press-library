...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "devonthink-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DEVONTHINK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DEVONTHINK_FEEDBACK_AUTO_SEND")))
...
func appendFeedback(entry FeedbackEntry) error {
	...
	f, err := os.OpenFile(p, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0o600) // #nosec G304 -- feedback ledger path is resolved under the CLI state directory.
	if err != nil {
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "devonthink-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/devonthink-pp-cli/feedback.jsonl.
When ` + "`DEVONTHINK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DEVONTHINK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  devonthink-pp-cli feedback list
  devonthink-pp-cli feedback list --limit 5
  devonthink-pp-cli feedback list --json`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			p, err := feedbackFilePath()
			if err != nil {
				return err
			}
			data, err := os.ReadFile(p) // #nosec G304 -- feedback ledger path is resolved under the CLI state directory.
			if err != nil {
