...
large datasets as it has no memory pressure.`,
  ...
  devonthink-pp-cli export <resource> --format jsonl --output data.jsonl

  # Export with limit
  devonthink-pp-cli export <resource> --format jsonl --limit 1000

  # Pipe to another tool
  devonthink-pp-cli export <resource> --format jsonl | jq '.id'`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if outputFile != "" {
				f, err := os.Create(outputFile) // #nosec G304 -- export intentionally writes to a user-selected local path.
				if err != nil {
