...
In local mode: searches locally synced data only.`,
  ...
  devonthink-pp-cli search "error timeout"

  # Force local search only
  devonthink-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  devonthink-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  devonthink-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /mirror/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/mirror/search", map[string]string{
					"query": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'devonthink-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "mcp":
				results, err = db.SearchMcp(query, limit)
			case "media":
				results, err = db.SearchMedia(query, limit)
			case "records":
				results, err = db.SearchRecords(query, limit)
			case "selection":
				results, err = db.SearchSelection(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchMcp(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search mcp failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchRecords(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search records failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchSelection(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search selection failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/devonthink-pp-cli/data.db)")
