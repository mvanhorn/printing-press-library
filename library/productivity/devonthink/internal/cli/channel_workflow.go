...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  devonthink-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  devonthink-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"databases", "ledger", "mirror", "records", "selection"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  devonthink-pp-cli workflow status

  # Show status as JSON
  devonthink-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("devonthink-pp-cli")
			}
