...
func TestWrapWithProvenanceIncludesScope(t *testing.T) {
	t.Parallel()
	got, err := wrapWithProvenance(json.RawMessage(`[{"uuid":"rec-1","name":"Invoice"}]`), DataProvenance{
		Source: "live",
		Scope: map[string]any{
			"type":  "smart_group",
			"input": "Offene Rückerstattungen",
			"uuid":  "sg-1",
		},
	})
	if err != nil {
	...
	if envelope.Meta.Source != "live" {
		t.Fatalf("source = %q, want live", envelope.Meta.Source)
	}
	if envelope.Meta.Scope.Type != "smart_group" || envelope.Meta.Scope.Input != "Offene Rückerstattungen" || envelope.Meta.Scope.UUID != "sg-1" {
		t.Fatalf("scope = %+v", envelope.Meta.Scope)
	}
	if len(envelope.Results) != 1 || envelope.Results[0]["uuid"] != "rec-1" {
