...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("DEVONTHINK_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "devonthink-pp-cli", "config.toml")
	}
	cfg.Path = path

	// Try to load config file
	data, err := os.ReadFile(path) // #nosec G304,G703 -- config path is an explicit local CLI input/env override.
	if err == nil {
	...
	if v := os.Getenv("DEVONTHINK_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
