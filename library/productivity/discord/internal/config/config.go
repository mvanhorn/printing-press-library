...
type Config struct {
	...
	DiscordBotToken  string          `toml:"bot_token"`
...
func Load(configPath string) (*Config, error) {
	...
	if v := os.Getenv("DISCORD_BOT_TOKEN"); v != "" {
		...
		cfg.markEnvOverride("DiscordBotToken")
		cfg.AuthSource = "env:DISCORD_BOT_TOKEN"
		cfg.CredentialSource = "env:DISCORD_BOT_TOKEN"
	}
	...
	if v := os.Getenv("DISCORD_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("DISCORD_APPLICATION_ID")); v != "" {
		cfg.TemplateVars["application_id"] = v
	} else if verifyMode {
		cfg.TemplateVars["application_id"] = "application_id_placeholder"
	}
	if v := strings.TrimSpace(os.Getenv("DISCORD_CHANNEL_ID")); v != "" {
		cfg.TemplateVars["channel_id"] = v
	} else if verifyMode {
		cfg.TemplateVars["channel_id"] = "channel_id_placeholder"
	}
	if v := strings.TrimSpace(os.Getenv("DISCORD_GUILD_ID")); v != "" {
		cfg.TemplateVars["guild_id"] = v
	} else if verifyMode {
		cfg.TemplateVars["guild_id"] = "guild_id_placeholder"
	}
	if v := strings.TrimSpace(os.Getenv("DISCORD_GUILD_SCHEDULED_EVENT_ID")); v != "" {
		cfg.TemplateVars["guild_scheduled_event_id"] = v
	} else if verifyMode {
		cfg.TemplateVars["guild_scheduled_event_id"] = "guild_scheduled_event_id_placeholder"
	}
	if v := strings.TrimSpace(os.Getenv("DISCORD_SKU_ID")); v != "" {
		cfg.TemplateVars["sku_id"] = v
	} else if verifyMode {
		cfg.TemplateVars["sku_id"] = "sku_id_placeholder"
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("DISCORD_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "discord-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	return applyAuthFormat("Bot {bot_token}", replacements)
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.DiscordBotToken == "" {
		c.DiscordBotToken = creds.DiscordBotToken
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	delete(c.envOverrides, "DiscordBotToken")
	c.updateFileConfigField("DiscordBotToken")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["DiscordBotToken"] {
			out.DiscordBotToken = c.fileConfig.DiscordBotToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "DiscordBotToken":
		c.fileConfig.DiscordBotToken = c.DiscordBotToken
	}
}
