...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  discord-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  discord-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"oauth2", "oauth2-keys", "oauth2-userinfo", "soundboard-default-sounds", "sticker-packs", "users", "users-@me-connections", "users-@me-guilds", "voice"}
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  discord-pp-cli workflow status

  # Show status as JSON
  discord-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("discord-pp-cli")
			}
