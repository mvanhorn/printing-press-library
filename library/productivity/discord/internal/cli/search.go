...
In local mode: searches locally synced data only.`,
  ...
  discord-pp-cli search "error timeout"

  # Force local search only
  discord-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  discord-pp-cli search "status" --type applications --data-source local
  # JSON output for piping
  discord-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /guilds/{guild_id}/members/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/guilds/{guild_id}/members/search", map[string]string{
					"query": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'discord-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "applications":
				results, err = db.SearchApplications(query, limit)
			case "guilds":
				results, err = db.SearchGuilds(query, limit)
			case "oauth2":
				results, err = db.SearchOauth2(query, limit)
			case "sticker-packs":
				results, err = db.SearchStickerPacks(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchApplications(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search applications failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchGuilds(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search guilds failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchOauth2(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search oauth2 failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchStickerPacks(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search sticker_packs failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
