...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  discord-pp-cli doctor
  discord-pp-cli doctor --json
  discord-pp-cli doctor --fail-on warn
  discord-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if !configured {
					...
					report["auth_hint"] = "Set your API key with: export DISCORD_BOT_TOKEN=\"your-token-here\""
					report["auth_docs_url"] = "https://discord.com/developers/docs"
				} else {
					authConfigured = true
			...
			if os.Getenv("DISCORD_BOT_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "DISCORD_BOT_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "DISCORD_BOT_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "discord-pp-cli"
						verifyPath := "/users/@me"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'discord-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'discord-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		...
		report["hint"] = "No sync recorded; run 'discord-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'discord-pp-cli sync' to refresh."
	}
	return report
}
