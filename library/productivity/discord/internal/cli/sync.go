...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/productivity/discord/internal/learn/lookups"
...
Resource scoping:
  ...
  discord-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  discord-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  discord-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  discord-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  discord-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "activity_instances":
		return db.UpsertActivityInstances(data)
	case "attachment":
		return db.UpsertAttachment(data)
	case "commands":
		return db.UpsertCommands(data)
	case "applications_emojis":
		return db.UpsertApplicationsEmojis(data)
	case "entitlements":
		return db.UpsertEntitlements(data)
	case "applications_guilds":
		return db.UpsertApplicationsGuilds(data)
	case "role_connections":
		return db.UpsertRoleConnections(data)
	case "followers":
		return db.UpsertFollowers(data)
	case "channels_invites":
		return db.UpsertChannelsInvites(data)
	case "channels_messages":
		return db.UpsertChannelsMessages(data)
	case "permissions":
	...
	case "send_soundboard_sound":
		return db.UpsertSendSoundboardSound(data)
	case "thread_members":
		return db.UpsertThreadMembers(data)
	case "channels_threads":
		return db.UpsertChannelsThreads(data)
	case "typing":
		return db.UpsertTyping(data)
	case "channels_users":
		return db.UpsertChannelsUsers(data)
	case "voice_status":
		return db.UpsertVoiceStatus(data)
	case "channels_webhooks":
		return db.UpsertChannelsWebhooks(data)
	case "gateway":
		return db.UpsertGateway(data)
	case "guilds":
		return db.UpsertGuilds(data)
	case "audit_logs":
		return db.UpsertAuditLogs(data)
	case "auto_moderation":
		return db.UpsertAutoModeration(data)
	case "bans":
		return db.UpsertBans(data)
	case "bulk_ban":
		return db.UpsertBulkBan(data)
	case "guilds_channels":
		return db.UpsertGuildsChannels(data)
	case "guilds_emojis":
		return db.UpsertGuildsEmojis(data)
	case "incident_actions":
		return db.UpsertIncidentActions(data)
	case "integrations":
		return db.UpsertIntegrations(data)
	case "guilds_invites":
		return db.UpsertGuildsInvites(data)
	case "guilds_members":
		return db.UpsertGuildsMembers(data)
	case "guilds_messages":
		return db.UpsertGuildsMessages(data)
	case "new_member_welcome":
		return db.UpsertNewMemberWelcome(data)
	case "onboarding":
		return db.UpsertOnboarding(data)
	case "preview":
		return db.UpsertPreview(data)
	case "prune":
		return db.UpsertPrune(data)
	case "regions":
	...
	case "scheduled_events":
		return db.UpsertScheduledEvents(data)
	case "soundboard_sounds":
		return db.UpsertSoundboardSounds(data)
	case "guilds_stickers":
		return db.UpsertGuildsStickers(data)
	case "templates":
		return db.UpsertTemplates(data)
	case "guilds_threads":
		return db.UpsertGuildsThreads(data)
	case "vanity_url":
		return db.UpsertVanityUrl(data)
	case "voice_states":
		return db.UpsertVoiceStates(data)
	case "guilds_webhooks":
		return db.UpsertGuildsWebhooks(data)
	case "welcome_screen":
		return db.UpsertWelcomeScreen(data)
	case "widget":
		return db.UpsertWidget(data)
	case "widget_json":
		return db.UpsertWidgetJson(data)
	case "widget_png":
		return db.UpsertWidgetPng(data)
	case "callback":
		return db.UpsertCallback(data)
	case "target_users":
		return db.UpsertTargetUsers(data)
	case "lobbies":
		return db.UpsertLobbies(data)
	case "channel_linking":
		return db.UpsertChannelLinking(data)
	case "lobbies_members":
		return db.UpsertLobbiesMembers(data)
	case "lobbies_messages":
		return db.UpsertLobbiesMessages(data)
	case "oauth2":
		return db.UpsertOauth2(data)
	case "subscriptions":
		return db.UpsertSubscriptions(data)
	case "soundboard-default-sounds":
		return db.UpsertSoundboardDefaultSounds(data)
	case "stage-instances":
		return db.UpsertStageInstances(data)
	case "sticker-packs":
		return db.UpsertStickerPacks(data)
	case "users":
	...
	case "github":
		return db.UpsertGithub(data)
	case "webhooks_messages":
		return db.UpsertWebhooksMessages(data)
	case "slack":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		"audit-logs":                "/guilds/{guild_id}/audit-logs",
		"bans":                      "/guilds/{guild_id}/bans",
		"entitlements":              "/applications/{application_id}/entitlements",
		"members":                   "/guilds/{guild_id}/members",
		"messages":                  "/channels/{channel_id}/messages",
		"oauth2":                    "/oauth2/@me",
		"oauth2-keys":               "/oauth2/keys",
		"oauth2-userinfo":           "/oauth2/userinfo",
		"requests":                  "/guilds/{guild_id}/requests",
		"scheduled-events":          "/guilds/{guild_id}/scheduled-events/{guild_scheduled_event_id}/users",
		"soundboard-default-sounds": "/soundboard-default-sounds",
		"sticker-packs":             "/sticker-packs",
		"subscriptions":             "/skus/{sku_id}/subscriptions",
		"thread-members":            "/channels/{channel_id}/thread-members",
		"threads":                   "/channels/{channel_id}/threads/archived/private",
		"users":                     "/users/@me",
...
var reconcileTypedTables = map[string]string{
	...
	"applications_emojis":       "applications_emojis",
	"applications_guilds":       "applications_guilds",
	...
	"channels_invites":          "channels_invites",
	"channels_messages":         "channels_messages",
	"channels_threads":          "channels_threads",
	"channels_users":            "channels_users",
	"channels_webhooks":         "channels_webhooks",
	"commands":                  "commands",
	...
	"guilds_emojis":             "guilds_emojis",
	"guilds_invites":            "guilds_invites",
	"guilds_members":            "guilds_members",
	"guilds_messages":           "guilds_messages",
	"guilds_stickers":           "guilds_stickers",
	"guilds_threads":            "guilds_threads",
	"guilds_webhooks":           "guilds_webhooks",
	"incident_actions":          "incident_actions",
	"integrations":              "integrations",
	"lobbies":                   "lobbies",
	"lobbies_members":           "lobbies_members",
	"lobbies_messages":          "lobbies_messages",
	...
	"send_soundboard_sound":     "send_soundboard_sound",
	"slack":                     "slack",
	"soundboard-default-sounds": "soundboard_default_sounds",
	"soundboard_sounds":         "soundboard_sounds",
	"stage-instances":           "stage_instances",
	...
	"webhooks":                  "webhooks",
	"webhooks_messages":         "webhooks_messages",
...
func responsePathForResource(resource, path string) []string {
	...
	case "lobbies\x00/lobbies":
		return []string{"members"}
	case "lobbies\x00/lobbies/{lobby_id}":
		return []string{"members"}
	case "oauth2\x00/oauth2/@me":
		return []string{"scopes"}
	case "oauth2\x00/oauth2/keys":
		return []string{"keys"}
	case "partner-sdk\x00/partner-sdk/token/bot":
		return []string{"scopes"}
	case "partner-sdk\x00/partner-sdk/token":
		return []string{"scopes"}
	case "sticker-packs\x00/sticker-packs/{pack_id}":
		return []string{"stickers"}
	case "sticker-packs\x00/sticker-packs":
		return []string{"sticker_packs"}
	case "users\x00/users/@me/guilds/{guild_id}/member":
		return []string{"roles"}
	}
	return nil
}
