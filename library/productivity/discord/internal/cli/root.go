...
type rootFlags struct {
	...
	// invocation. Mirrors the DISCORD_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "discord-pp-cli",
		Short: "Manage discord resources via the discord API",
		Long: `Manage discord resources via the discord API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'discord-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("discord-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'discord-pp-cli profile list')")
	...
	rootCmd.AddCommand(newSkusCmd(flags))
	...
	rootCmd.AddCommand(newSoundboardDefaultSoundsPromotedCmd(flags))
	rootCmd.AddCommand(newStickersPromotedCmd(flags))
	rootCmd.AddCommand(newVoicePromotedCmd(flags))
