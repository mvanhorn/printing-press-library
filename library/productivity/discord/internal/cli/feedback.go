...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DISCORD_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DISCORD_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "discord-pp-cli/feedback")
...
When ` + "`DISCORD_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DISCORD_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  discord-pp-cli feedback list
  discord-pp-cli feedback list --limit 5
  discord-pp-cli feedback list --json`,
