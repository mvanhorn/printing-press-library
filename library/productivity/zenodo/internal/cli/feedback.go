...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ZENODO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ZENODO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "zenodo-pp-cli/feedback")
...
When ` + "`ZENODO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ZENODO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  zenodo-pp-cli feedback list
  zenodo-pp-cli feedback list --limit 5
  zenodo-pp-cli feedback list --json`,
