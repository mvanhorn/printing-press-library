...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  zenodo-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  zenodo-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"communities", "deposit", "funders", "grants", "licenses", "records"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  zenodo-pp-cli workflow status

  # Show status as JSON
  zenodo-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("zenodo-pp-cli")
			}
