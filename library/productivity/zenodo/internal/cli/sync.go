...
Resource scoping:
  ...
  zenodo-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  zenodo-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  zenodo-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  zenodo-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  zenodo-pp-cli sync --latest-only`,
...
func responsePathForResource(resource, path string) []string {
	...
	case "deposit\x00/deposit/depositions":
		return []string{"files"}
	case "deposit\x00/deposit/depositions/{depositId}/actions/discard":
		return []string{"files"}
	case "deposit\x00/deposit/depositions/{depositId}/actions/edit":
		return []string{"files"}
	case "deposit\x00/deposit/depositions/{depositId}":
		return []string{"files"}
	case "deposit\x00/deposit/depositions/{depositId}/actions/newversion":
		return []string{"files"}
	case "deposit\x00/deposit/depositions/{depositId}/actions/publish":
		return []string{"files"}
	}
	return nil
}
