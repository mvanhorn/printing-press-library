...
Run sync first to populate the local search index.`,
  ...
  zenodo-pp-cli search "status"

  # Force local search explicitly
  zenodo-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  zenodo-pp-cli search "status" --type communities --data-source local
  # JSON output for piping
  zenodo-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'zenodo-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
