...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  habitica-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  habitica-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"notifications", "tags", "tasks", "user", "user-in-app-rewards"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  habitica-pp-cli workflow status

  # Show status as JSON
  habitica-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("habitica-pp-cli")
			}
