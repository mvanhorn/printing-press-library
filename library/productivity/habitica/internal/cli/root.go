...
type rootFlags struct {
	...
	// invocation. Mirrors the HABITICA_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "habitica-pp-cli",
		Short: `Habitica CLI — Turn chores into quests, check off workouts, and protect your reward budget from the command line.`,
		Long: `Habitica CLI — Turn chores into quests, check off workouts, and protect your reward budget from the command line.
...
Health check: run 'habitica-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("habitica-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'habitica-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelRewardCmd(flags))
	rootCmd.AddCommand(newNovelTagCmd(flags))
	rootCmd.AddCommand(newNovelTodayCmd(flags))
	rootCmd.AddCommand(newNovelWeekCmd(flags))
