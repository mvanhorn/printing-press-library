...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  habitica-pp-cli doctor
  habitica-pp-cli doctor --json
  habitica-pp-cli doctor --fail-on warn
  habitica-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set your API key with: export HABITICA_API_TOKEN=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if os.Getenv("HABITICA_API_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "HABITICA_API_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "HABITICA_API_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["x-client"] = "habitica-pp-cli"
						authHeaders["User-Agent"] = "habitica-pp-cli"
						verifyPath := "/tasks/user"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'habitica-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'habitica-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'habitica-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'habitica-pp-cli sync' to refresh."
	}
	return report
}
