...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"task_type": {
			{Canonical: "habit", Values: []string{"habit", "routine"}},
			{Canonical: "daily", Values: []string{"daily", "recurring task"}},
			{Canonical: "todo", Values: []string{"todo", "to-do", "chore"}},
			{Canonical: "reward", Values: []string{"reward", "custom reward"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: habitica-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: habitica-pp-cli: learn init: %v\n", err)
		}
	})
}
