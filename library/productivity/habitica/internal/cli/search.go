...
Run sync first to populate the local search index.`,
  ...
  habitica-pp-cli search "status"

  # Force local search explicitly
  habitica-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  habitica-pp-cli search "status" --type notifications --data-source local
  # JSON output for piping
  habitica-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'habitica-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
