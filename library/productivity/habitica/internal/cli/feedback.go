...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("HABITICA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HABITICA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "habitica-pp-cli/feedback")
...
When ` + "`HABITICA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`HABITICA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  habitica-pp-cli feedback list
  habitica-pp-cli feedback list --limit 5
  habitica-pp-cli feedback list --json`,
