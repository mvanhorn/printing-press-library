...
type Config struct {
	...
	HabiticaApiToken string          `toml:"api_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("HABITICA_API_TOKEN"); v != "" {
		cfg.HabiticaApiToken = v
		cfg.markEnvOverride("HabiticaApiToken")
		cfg.AuthSource = "env:HABITICA_API_TOKEN"
		cfg.CredentialSource = "env:HABITICA_API_TOKEN"
	}
	...
	if v := os.Getenv("HABITICA_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("HABITICA_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "habitica-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.HabiticaApiToken
	if token == "" {
		return ""
	}
	if c.HabiticaApiToken == "" {
		return ""
	}
	replacements := map[string]string{
		"token":              token,
		"api_token":          c.HabiticaApiToken,
		"HABITICA_API_TOKEN": c.HabiticaApiToken,
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		HabiticaApiToken: c.HabiticaApiToken,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.HabiticaApiToken == "" {
		c.HabiticaApiToken = creds.HabiticaApiToken
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.HabiticaApiToken = token
	delete(c.envOverrides, "HabiticaApiToken")
	c.updateFileConfigField("HabiticaApiToken")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["HabiticaApiToken"] {
			out.HabiticaApiToken = c.fileConfig.HabiticaApiToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "HabiticaApiToken":
		c.fileConfig.HabiticaApiToken = c.HabiticaApiToken
	}
}
