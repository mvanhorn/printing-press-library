...
func AdoptVerifiedLegacyDatabase(ctx context.Context, session *Session, state *LegacyMigrationState) error {
	...
	// SQLite's VACUUM INTO takes a string literal, not a bind parameter, so
	// the destination has to be interpolated. targetPath is process-derived
	// (session.Paths.DataFile), never request or argument input, and single
	// quotes are doubled per SQLite string-literal escaping.
	quotedTarget := strings.ReplaceAll(targetPath, "'", "''")
	// #nosec G202 -- VACUUM INTO cannot be parameterized; see the note above.
