...
var readCommandResources = map[string][]string{
	"tiimo-pp-cli profiles":        {"profiles"},
	"tiimo-pp-cli profiles list":   {"profiles"},
	"tiimo-pp-cli profiles get":    {"profiles"},
	"tiimo-pp-cli profiles search": {"profiles"},
}
...
func cachePolicy() cliutil.Policy {
	...
	envOptOut := "TIIMO_NO_AUTO_REFRESH"
...
func autoRefreshIfStale(ctx context.Context, flags *rootFlags, resources []string) (meta cliutil.FreshnessMeta) {
	...
	if err := runAutoRefresh(refreshCtx, flags, db, resources); err != nil {
		fmt.Fprintf(os.Stderr, "warning: using stale tiimo-pp-cli cache (refresh failed: %v)\n", err)
