...
type rootFlags struct {
	...
	// invocation. Mirrors the TIIMO_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "tiimo-pp-cli",
		Short: `Your Tiimo plan, finally exportable, queryable, and yours.`,
		Long: `Your Tiimo plan, finally exportable, queryable, and yours.
...
Health check: run 'tiimo-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("tiimo-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select date,activities)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'tiimo-pp-cli profile list')")
	...
	rootCmd.AddCommand(newTodoTasksCmd(flags))
	...
	rootCmd.AddCommand(newRoutinesPromotedCmd(flags))
	rootCmd.AddCommand(newTagsPromotedCmd(flags))
	rootCmd.AddCommand(newTodoListsPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelAdherenceCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelBackupCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelCapacityCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelDriftCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelFeedCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelGapsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelOverlapsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelRollingCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelStallsCmd(flags))
