...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  tiimo-pp-cli doctor
  tiimo-pp-cli doctor --json
  tiimo-pp-cli doctor --fail-on warn
  tiimo-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if !configured {
					...
					report["auth_hint"] = "Set it with: tiimo-pp-cli auth set-token <token> or export TIIMO_TOKEN=\"your-token-here\""
					report["auth_docs_url"] = "https://api.tiimoapp.com"
				} else {
					authConfigured = true
			...
			if os.Getenv("TIIMO_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "TIIMO_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "TIIMO_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "tiimo-pp-cli", suggestion)
						} else {
			...
			report["cache"] = collectCacheReport(cmd.Context(), "12h")
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'tiimo-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'tiimo-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		...
		report["hint"] = "No sync recorded; run 'tiimo-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'tiimo-pp-cli sync' to refresh."
	}
	return report
}
