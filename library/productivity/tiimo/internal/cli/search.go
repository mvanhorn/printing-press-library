...
Run sync first to populate the local search index.`,
  ...
  tiimo-pp-cli search "status"

  # Force local search explicitly
  tiimo-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  tiimo-pp-cli search "status" --type activities --data-source local
  # JSON output for piping
  tiimo-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'tiimo-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "activities":
				results, err = db.SearchActivities(query, limit)
			case "todo_tasks":
				results, err = db.SearchTodoTasks(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchActivities(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search activities failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchTodoTasks(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search todo_tasks failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
