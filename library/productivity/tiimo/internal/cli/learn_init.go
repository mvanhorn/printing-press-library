...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"time_of_day": {
			{Canonical: "Morning", Values: []string{"am", "morning", "early"}},
			{Canonical: "Afternoon", Values: []string{"pm", "afternoon", "midday"}},
			{Canonical: "Evening", Values: []string{"night", "evening", "tonight"}},
			{Canonical: "Anytime", Values: []string{"unscheduled", "whenever", "anytime"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: tiimo-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: tiimo-pp-cli: learn init: %v\n", err)
		}
	})
}
