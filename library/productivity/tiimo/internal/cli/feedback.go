...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TIIMO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TIIMO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "tiimo-pp-cli/feedback")
...
When ` + "`TIIMO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TIIMO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  tiimo-pp-cli feedback list
  tiimo-pp-cli feedback list --limit 5
  tiimo-pp-cli feedback list --json`,
