...
type Config struct {
	...
	TiimoToken   string          `toml:"token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.tiimoapp.com",
	}
	...
	// Env var overrides
	if v := strings.ToLower(strings.TrimSpace(os.Getenv("TIIMO_SKIP_TLS_VERIFY"))); v == "1" || v == "true" || v == "yes" || v == "on" {
		cfg.SetSkipTLSVerify(true)
	}
	if v := os.Getenv("TIIMO_TOKEN"); v != "" {
		cfg.TiimoToken = v
		cfg.markEnvOverride("TiimoToken")
		cfg.AuthSource = "env:TIIMO_TOKEN"
		cfg.CredentialSource = "env:TIIMO_TOKEN"
	}
	...
	if v := os.Getenv("TIIMO_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("TIIMO_TOKEN_URL"); v != "" {
		cfg.TokenURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("TIIMO_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "tiimo-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.TiimoToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:TIIMO_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"token":        c.TiimoToken,
			"TIIMO_TOKEN":  c.TiimoToken,
			"access_token": c.TiimoToken,
		})
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.TiimoToken = ""
	delete(c.envOverrides, "TiimoToken")
	c.updateFileConfigField("TiimoToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	...
	if c.fileConfig != nil {
		...
		if c.envOverrides["TiimoToken"] {
			out.TiimoToken = c.fileConfig.TiimoToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "TiimoToken":
		c.fileConfig.TiimoToken = c.TiimoToken
	}
}
