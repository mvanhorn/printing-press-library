# Airtable Printed CLI Agent Guide

This directory is a generated `airtable-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
airtable-pp-cli doctor --json
airtable-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
airtable-pp-cli which "<capability>" --json
airtable-pp-cli <command> --help
```
...
```bash
airtable-pp-cli <command> --help
airtable-pp-cli <command> --dry-run --agent
```
