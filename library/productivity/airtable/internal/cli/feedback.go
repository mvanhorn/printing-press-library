...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "airtable-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AIRTABLE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AIRTABLE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "airtable-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/airtable-pp-cli/feedback.jsonl.
When ` + "`AIRTABLE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AIRTABLE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  airtable-pp-cli feedback list
  airtable-pp-cli feedback list --limit 5
  airtable-pp-cli feedback list --json`,
