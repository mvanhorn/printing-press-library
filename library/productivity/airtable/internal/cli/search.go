...
In local mode: searches locally synced data only.`,
  ...
  airtable-pp-cli search "error timeout"

  # Force local search only
  airtable-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  airtable-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  airtable-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			switch resourceType {
			case "bases":
				results, err = db.SearchBases(query, limit)
			case "tables":
				results, err = db.SearchTables(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchBases(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search bases failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchTables(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search tables failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
