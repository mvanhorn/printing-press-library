...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "airtable-pp-cli",
		Short: `Airtable CLI — Every Airtable surface a CLI should have — plus a local SQLite mirror, cross-base SQL, and webhook tooling no SDK ships.`,
		Long: `Airtable CLI — Every Airtable surface a CLI should have — plus a local SQLite mirror, cross-base SQL, and webhook tooling no SDK ships.
...
Health check: run 'airtable-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("airtable-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'airtable-pp-cli profile list')")
	...
	rootCmd.AddCommand(newBasesCmd(flags))
	rootCmd.AddCommand(newCommentsCmd(flags))
	recordsCmd := newRecordsCmd(flags)
	// Hand-built bulk upsert wrapper: adds --merge-on, --batch-progress, and
	// auto-batching on top of the generated upsert path.
	recordsCmd.AddCommand(newRecordsBulkUpsertCmd(flags))
	// Un-hide so 'records bulk-upsert' is discoverable in --help.
	recordsCmd.Hidden = false
	rootCmd.AddCommand(recordsCmd)
	rootCmd.AddCommand(newTablesCmd(flags))
	webhooksCmd := newWebhooksCmd(flags)
	// Novel webhook subcommands (Phase 3, hand-built):
	webhooksCmd.AddCommand(newWebhooksFleetCmd(flags))
	webhooksCmd.AddCommand(newWebhooksTailCmd(flags))
	// Un-hide the parent so the new children are discoverable in --help.
	webhooksCmd.Hidden = false
	...
	// Phase 3 novel transcendence commands (hand-built):
	rootCmd.AddCommand(newQueryCmd(flags))
	rootCmd.AddCommand(newChangesCmd(flags))
	rootCmd.AddCommand(newStaleCmd(flags))
	rootCmd.AddCommand(newTraverseCmd(flags))
	rootCmd.AddCommand(newXsearchCmd(flags))
	rootCmd.AddCommand(newSchemaCmd(flags))
	rootCmd.AddCommand(newFormulaCmd(flags))
