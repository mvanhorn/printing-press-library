...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("AIRTABLE_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "airtable-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("AIRTABLE_PAT"); v != "" {
		cfg.AirtablePat = v
		cfg.AuthSource = "env:AIRTABLE_PAT"
	}
	...
	if cfg.AuthSource == "" && cfg.AirtablePat != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("AIRTABLE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.AirtablePat != "" {
		c.AuthSource = "env:AIRTABLE_PAT"
		return applyAuthFormat("Bearer {token}", map[string]string{
			"pat":          c.AirtablePat,
			"AIRTABLE_PAT": c.AirtablePat,
			"token":        c.AirtablePat,
		})
	}
	if c.AccessToken != "" {
