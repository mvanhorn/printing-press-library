# Plaud Web Printed CLI Agent Guide

This directory is a generated `plaud-web-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
plaud-web-pp-cli doctor --json
plaud-web-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
plaud-web-pp-cli which "<capability>" --json
plaud-web-pp-cli <command> --help
```
...
```bash
plaud-web-pp-cli <command> --agent
```
...
```bash
plaud-web-pp-cli <command> --help
plaud-web-pp-cli <command> --dry-run --agent
```
