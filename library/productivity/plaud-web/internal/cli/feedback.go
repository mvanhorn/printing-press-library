...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "plaud-web-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PLAUD_WEB_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PLAUD_WEB_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "plaud-web-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/plaud-web-pp-cli/feedback.jsonl.
When ` + "`PLAUD_WEB_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PLAUD_WEB_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  plaud-web-pp-cli feedback list
  plaud-web-pp-cli feedback list --limit 5
  plaud-web-pp-cli feedback list --json`,
