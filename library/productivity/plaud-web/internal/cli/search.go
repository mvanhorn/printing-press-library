...
In local mode: searches locally synced data only.`,
  ...
  plaud-web-pp-cli search "error timeout"

  # Force local search only
  plaud-web-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  plaud-web-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  plaud-web-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /gsearch/v1/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/gsearch/v1/search", map[string]any{
					"query":     query,
					"date_from": nil,
					"date_to":   nil,
					"from":      "",
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'plaud-web-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
