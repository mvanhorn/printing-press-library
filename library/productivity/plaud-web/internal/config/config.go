...
type Config struct {
	...
	PlaudWebBearerAuth string            `toml:"web_bearer_auth"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("PLAUD_WEB_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "plaud-web-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("PLAUD_WEB_BEARER_AUTH"); v != "" {
		cfg.PlaudWebBearerAuth = v
		cfg.markEnvOverride("PlaudWebBearerAuth")
		cfg.AuthSource = "env:PLAUD_WEB_BEARER_AUTH"
	}
	...
	if cfg.AuthSource == "" && cfg.PlaudWebBearerAuth != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("PLAUD_WEB_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.PlaudWebBearerAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:PLAUD_WEB_BEARER_AUTH"
		}
		return "Bearer " + normalizeBearerToken(c.PlaudWebBearerAuth)
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientID")
	c.updateFileConfigField("ClientSecret")
	c.PlaudWebBearerAuth = ""
	delete(c.envOverrides, "PlaudWebBearerAuth")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["PlaudWebBearerAuth"] {
			out.PlaudWebBearerAuth = c.fileConfig.PlaudWebBearerAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "PlaudWebBearerAuth":
		c.fileConfig.PlaudWebBearerAuth = c.PlaudWebBearerAuth
	}
}
