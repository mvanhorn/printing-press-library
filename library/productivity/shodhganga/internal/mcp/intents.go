...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("expand_a_literature_review",
			mcplib.WithDescription("Find theses sharing the most subject keywords with a relevant one, ranked by overlap."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Override the recipe's positional id value.")),
		),
		handleExpandALiteratureReview,
	)
}

// handleExpandALiteratureReview runs the expand_a_literature_review recipe intent: Find theses sharing the most subject keywords with a relevant one, ranked by overlap.
func handleExpandALiteratureReview(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
