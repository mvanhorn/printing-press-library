...
type rootFlags struct {
	...
	// invocation. Mirrors the SHODHGANGA_NO_LEARN env var.
	noLearn       bool
	selectFields  string
	configPath    string
	homePath      string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "shodhganga-pp-cli",
		Short: `Structured, scriptable, agent-native access to India's national reservoir of PhD theses — the machine API Shodhganga never shipped.`,
		Long: `Structured, scriptable, agent-native access to India's national reservoir of PhD theses — the machine API Shodhganga never shipped.
...
Health check: run 'shodhganga-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("shodhganga-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'shodhganga-pp-cli profile list')")
	...
	rootCmd.AddCommand(newThesisCmd(flags))
	...
	rootCmd.AddCommand(newNovelGuideCmd(flags))
	rootCmd.AddCommand(newNovelHarvestCmd(flags))
	rootCmd.AddCommand(newNovelSimilarCmd(flags))
	rootCmd.AddCommand(newNovelTrendsCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newBrowsePromotedCmd(flags))
	rootCmd.AddCommand(newUniversityPromotedCmd(flags))
