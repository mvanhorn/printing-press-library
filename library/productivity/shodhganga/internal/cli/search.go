...
In local mode: searches locally synced data only.`,
  ...
  shodhganga-pp-cli search "error timeout"

  # Force local search only
  shodhganga-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  shodhganga-pp-cli search "status" --type browse --data-source local
  # JSON output for piping
  shodhganga-pp-cli search "critical" --json --limit 20`,
