...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SHODHGANGA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SHODHGANGA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "shodhganga-pp-cli/feedback")
...
When ` + "`SHODHGANGA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SHODHGANGA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  shodhganga-pp-cli feedback list
  shodhganga-pp-cli feedback list --limit 5
  shodhganga-pp-cli feedback list --json`,
