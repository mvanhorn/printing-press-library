...
profile by name when the installed backend supports it.`,
  ...
  mobbin-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			// PATCH: Keep browser-cookie import dry-runnable for shipcheck probes.
			if dryRunOK(flags) {
				fmt.Fprintln(cmd.OutOrStdout(), "would import Chrome cookies for mobbin.com (dry-run)")
				return nil
			}
			if !browserFlag {
...
// host_key is plaintext so no decryption is needed.
