# Ed Seykota's Trading Tribe Printed CLI Agent Guide

This directory is a generated `seykota-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
seykota-pp-cli doctor --json
seykota-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
seykota-pp-cli which "<capability>" --json
seykota-pp-cli <command> --help
```
...
```bash
seykota-pp-cli <command> --help
seykota-pp-cli <command> --dry-run --agent
```
