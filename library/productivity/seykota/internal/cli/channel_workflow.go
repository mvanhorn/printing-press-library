...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  seykota-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  seykota-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"pages", "pages-index-htm"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  seykota-pp-cli workflow status

  # Show status as JSON
  seykota-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("seykota-pp-cli")
			}
