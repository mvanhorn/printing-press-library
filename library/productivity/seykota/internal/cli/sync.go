...
Exit codes & warnings:
  ...
  seykota-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  seykota-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  seykota-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  seykota-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  seykota-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/seykota-pp-cli/data.db)")
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"pages":           "/tt/FAQ_Index/",
		"pages-index-htm": "/tribe/TSP/index.htm",
	}
