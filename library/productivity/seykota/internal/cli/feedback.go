...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".seykota-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SEYKOTA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SEYKOTA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "seykota-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.seykota-pp-cli/feedback.jsonl.
When ` + "`SEYKOTA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SEYKOTA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  seykota-pp-cli feedback list
  seykota-pp-cli feedback list --limit 5
  seykota-pp-cli feedback list --json`,
