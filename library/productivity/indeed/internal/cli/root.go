...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "indeed-pp-cli",
		Short: `Indeed CLI — Search Indeed from your terminal — with a local job history no scraper keeps, so you can ask what's new since last time…`,
		Long: `Indeed CLI — Search Indeed from your terminal — with a local job history no scraper keeps, so you can ask what's new since last time…
...
Health check: run 'indeed-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
	...
	rootCmd.AddCommand(newRelatedPromotedCmd(flags))
	// Hand-authored Indeed commands (internal/cli/indeed_commands.go).
	rootCmd.AddCommand(newSearchCmd(flags))
	rootCmd.AddCommand(newJobCmd(flags))
	...
	rootCmd.AddCommand(newUntrackCmd(flags))
	rootCmd.AddCommand(newTrackedCmd(flags))
