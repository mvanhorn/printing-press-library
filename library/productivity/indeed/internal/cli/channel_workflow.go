...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  indeed-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"related"}
			totalSynced := 0

			// Under --json, suppress syncResource's NDJSON progress events
			// (gated on !humanFriendly) so stdout carries only the single
			// summary JSON document this command emits below — otherwise the
			// interleaved event lines + summary make stdout unparseable as one
			// JSON value. The sync events are gated on the global humanFriendly,
			// so flipping it for the sync loop is the least-invasive suppression.
			if flags.asJSON {
				prevHF := humanFriendly
				humanFriendly = true
				defer func() { humanFriendly = prevHF }()
			}
