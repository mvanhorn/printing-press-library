...
var whichIndex = []whichEntry{
	{Command: "new", Description: "See only the Amazon reqs that appeared since you last synced a saved search — no more re-scanning the whole list every morning.", Group: "Local state that compounds", WhyItMatters: "Reach for this when a user tracks a role over time and wants the delta, not the full list."},
	{Command: "save", Description: "Persist a named query plus its filters and diff cursor so a search and its new-since state survive between runs.", Group: "Local state that compounds", WhyItMatters: "Use this to set up a repeatable watch that `new` and `sync` then operate on by name."},
	{Command: "stats", Description: "Count synced reqs grouped by city, state, team, or category — the aggregation Amazon's own empty facets[] never returns.", Group: "Aggregation the API can't do", WhyItMatters: "Use this to answer 'which cities/teams have the most open reqs' without paging every result."},
	{Command: "skills", Description: "Rank teams and cities by how many synced reqs demand a given skill keyword in their basic/preferred qualifications.", Group: "Aggregation the API can't do", WhyItMatters: "Reach for this for labor-market questions like 'who is hiring for X skill' rather than retrieving individual reqs."},
	{Command: "find", Description: "Filter live results by intern, manager, university, and schedule type — fields the .json endpoint silently ignores as server params.", Group: "Honest client-side filtering", WhyItMatters: "Use these flags when a user wants senior-IC, non-intern, or schedule-specific roles that Amazon can't filter server-side."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  amazon-jobs-pp-cli which "stale tickets"
  amazon-jobs-pp-cli which "bottleneck"
  amazon-jobs-pp-cli which --limit 1 "send message"
  amazon-jobs-pp-cli which                                # list the full capability index`,
