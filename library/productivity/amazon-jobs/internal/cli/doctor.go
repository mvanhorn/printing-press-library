...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  amazon-jobs-pp-cli doctor
  amazon-jobs-pp-cli doctor --json
  amazon-jobs-pp-cli doctor --fail-on warn
  amazon-jobs-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					healthPath := "/en/search.json?base_query=&result_limit=1"
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "amazon-jobs-pp-cli", suggestion)
						} else {
