...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/productivity/amazon-jobs/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/productivity/amazon-jobs/internal/learn/lookups"
...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^[0-9]{6,9}$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: amazon-jobs-pp-cli: ticker pattern %q failed to compile: %v\n", `^[0-9]{6,9}$`, err)
	} else {
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"business_category": {
			{Canonical: "aws", Values: []string{"amazon web services", "cloud"}},
			{Canonical: "alexa-and-amazon-devices", Values: []string{"alexa", "devices", "echo"}},
		},
		"country": {
			{Canonical: "USA", Values: []string{"us", "usa", "united states", "america"}},
			{Canonical: "GBR", Values: []string{"uk", "united kingdom", "britain", "england"}},
			{Canonical: "IND", Values: []string{"india"}},
			{Canonical: "DEU", Values: []string{"germany"}},
			{Canonical: "JPN", Values: []string{"japan"}},
			{Canonical: "CAN", Values: []string{"canada"}},
		},
		"job_category": {
			{Canonical: "Software Development", Values: []string{"sde", "swe", "software engineer", "software dev"}},
			{Canonical: "Solutions Architect", Values: []string{"sa", "solutions architect"}},
			{Canonical: "Operations, IT, & Support Engineering", Values: []string{"support engineer", "ops", "sysops", "it support"}},
			{Canonical: "Applied Science", Values: []string{"applied scientist", "ml", "machine learning"}},
			{Canonical: "Hardware Development", Values: []string{"hardware", "asic", "silicon"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		dbPath := defaultDBPath("amazon-jobs-pp-cli")
		s, err := store.OpenWithContext(ctx, dbPath)
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: amazon-jobs-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: amazon-jobs-pp-cli: learn init: %v\n", err)
		}
	})
}
