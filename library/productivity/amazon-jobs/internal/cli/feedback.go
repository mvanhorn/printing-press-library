...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AMAZON_JOBS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AMAZON_JOBS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "amazon-jobs-pp-cli/feedback")
...
When ` + "`AMAZON_JOBS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AMAZON_JOBS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  amazon-jobs-pp-cli feedback list
  amazon-jobs-pp-cli feedback list --limit 5
  amazon-jobs-pp-cli feedback list --json`,
