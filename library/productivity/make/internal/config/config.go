...
type Config struct {
	...
	Path          string            `toml:"-"`
	MakeApiToken  string            `toml:"api_token"`
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("MAKE_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "make-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("MAKE_API_TOKEN"); v != "" {
		cfg.MakeApiToken = v
		cfg.AuthSource = "env:MAKE_API_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.MakeApiToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("MAKE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("MAKE_ZONE")); v != "" {
		cfg.TemplateVars["zone"] = v
	} else if verifyMode {
		cfg.TemplateVars["zone"] = "zone_placeholder"
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.MakeApiToken
	if token == "" {
		return ""
	}
	// Make.com's API expects `Authorization: Token <token>` (the spec's
	// auth.prefix). The generator omitted the prefix for api_key auth, so we
	// stitch it in here.
