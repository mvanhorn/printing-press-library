...
In local mode: searches locally synced data only.`,
  ...
  make-pp-cli search "error timeout"

  # Force local search only
  make-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  make-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  make-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Local FTS search
			if dbPath == "" {
				dbPath = defaultDBPath("make-pp-cli")
			}

			db, err := store.OpenWithContext(cmd.Context(), dbPath)
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'make-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "scenarios":
				results, err = db.SearchScenarios(query, limit)
			case "sdk_apps":
				results, err = db.SearchSdkApps(query, limit)
			case "templates":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchScenarios(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search scenarios failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchSdkApps(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search sdk_apps failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/make-pp-cli/data.db)")
