...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  make-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  make-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"orgs", "scenarios"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  make-pp-cli workflow status

  # Show status as JSON
  make-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("make-pp-cli")
			}
