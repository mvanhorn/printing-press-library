...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "make-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MAKE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MAKE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "make-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/make-pp-cli/feedback.jsonl.
When ` + "`MAKE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MAKE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  make-pp-cli feedback list
  make-pp-cli feedback list --limit 5
  make-pp-cli feedback list --json`,
