...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  make-pp-cli doctor
  make-pp-cli doctor --json
  make-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export MAKE_API_TOKEN=<your-key>"
					report["auth_key_url"] = "https://www.make.com/en/help/api/api-overview"
					report["auth_instructions"] = "Make profile → API → Add token (https://www.make.com/en/help/api/api-overview)"
				} else {
					authConfigured = true
			...
			if os.Getenv("MAKE_API_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "MAKE_API_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "MAKE_API_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "make-pp-cli"
						verifyPath := "/users/me"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'make-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'make-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'make-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'make-pp-cli sync' to refresh."
	}
	return report
}
