...
Resource scoping:
  ...
  make-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  make-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  make-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  make-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  make-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "data_stores":
		return db.UpsertDataStores(data)
	case "records":
		return db.UpsertRecords(data)
	case "data_structures":
		return db.UpsertDataStructures(data)
	case "devices":
		return db.UpsertDevices(data)
	case "dlqs":
		return db.UpsertDlqs(data)
	case "executions":
	...
	case "scenarios":
		return db.UpsertScenarios(data)
	case "sdk_apps":
		return db.UpsertSdkApps(data)
	case "teams":
