# Make.com Printed CLI Agent Guide

This directory is a generated `make-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
make-pp-cli doctor --json
make-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
make-pp-cli which "<capability>" --json
make-pp-cli <command> --help
```
...
```bash
make-pp-cli <command> --help
make-pp-cli <command> --dry-run --agent
```
