...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for MyFitnessPal",
	}
...
Use --chrome to read cookies from Chrome for .myfitnesspal.com.
...
profile by name when the installed backend supports it.`,
		Example: `  myfitnesspal-pp-cli auth login --chrome
  myfitnesspal-pp-cli auth login --browser
  myfitnesspal-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !browserFlag {
				fmt.Fprintln(cmd.OutOrStdout(), "Use --chrome or --browser to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  myfitnesspal-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  myfitnesspal-pp-cli auth login --browser\n")
				return nil
			}
...
func newAuthRefreshCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  myfitnesspal-pp-cli auth refresh",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
...
func browserSessionTargetURL(cfg *config.Config) string {
	baseURL := strings.TrimRight("https://www.myfitnesspal.com", "/")
	if cfg != nil && cfg.BaseURL != "" {
		baseURL = strings.TrimRight(cfg.BaseURL, "/")
	}
	validationPath := strings.TrimSpace("/user/auth_token")
	if validationPath == "" {
		return baseURL
	}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  myfitnesspal-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  myfitnesspal-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: .myfitnesspal.com\n")
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  myfitnesspal-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
...
func browserSessionProofPath(cfg *config.Config) string {
	if cfg == nil || cfg.Path == "" {
		...
		return filepath.Join(home, ".config", "myfitnesspal-pp-cli", "browser-session-proof.json")
	}
...
func browserSessionProofStatusForAuth(cfg *config.Config, authHeader string) (bool, string) {
	...
	if proof.APIName != "myfitnesspal" || proof.CookieDomain != ".myfitnesspal.com" || proof.ValidationPath != "/user/auth_token" {
...
func validateAndWriteBrowserSessionProof(cfg *config.Config, flags *rootFlags) error {
	...
	proof := browserSessionProof{
		APIName:               "myfitnesspal",
		CookieDomain:          ".myfitnesspal.com",
