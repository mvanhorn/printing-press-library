...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		...
		"GOOGLE_CALENDAR_DATA_DIR",
		"GOOGLE_CALENDAR_STATE_DIR",
		"GOOGLE_CALENDAR_CACHE_DIR",
		"GOOGLE_CALENDAR_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
		"XDG_CACHE_HOME",
	...
	return home, filepath.Join(home, ".config", "google-calendar-pp-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "google-calendar-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("CALENDAR_OAUTH2C", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "google-calendar-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "google-calendar-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "google-calendar-pp-cli", "credentials.toml")
