...
func Load(configPath string) (*Config, error) {
	...
	if v := os.Getenv("CALENDAR_OAUTH2C"); v != "" {
		...
		cfg.markEnvOverride("CalendarOauth2c")
		cfg.AuthSource = "env:CALENDAR_OAUTH2C"
		cfg.CredentialSource = "env:CALENDAR_OAUTH2C"
	}
	...
	if v := strings.TrimSpace(os.Getenv("GOOGLE_CALENDAR_CALENDAR_ID")); v != "" {
		cfg.TemplateVars["calendarId"] = v
	} else if verifyMode {
		cfg.TemplateVars["calendarId"] = "calendarId_placeholder"
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("GOOGLE_CALENDAR_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "google-calendar-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	// A directly-held bearer in CALENDAR_OAUTH2C (a pasted access token, or a
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		CalendarOauth2c: c.CalendarOauth2c,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.CalendarOauth2c = creds.CalendarOauth2c
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.CalendarOauth2c = ""
	delete(c.envOverrides, "CalendarOauth2c")
	c.updateFileConfigField("CalendarOauth2c")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["CalendarOauth2c"] {
			out.CalendarOauth2c = c.fileConfig.CalendarOauth2c
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "CalendarOauth2c":
		c.fileConfig.CalendarOauth2c = c.CalendarOauth2c
	}
}
