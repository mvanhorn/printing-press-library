...
agents can record playbooks without a file on disk.`,
		Example: `  google-calendar-pp-cli teach-playbook --query "morning schedule sweep" --notes "run conflicts --from today --to +7d, then agenda for any conflict day"`,
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  google-calendar-pp-cli playbook amend --query "morning schedule sweep" --add-note "include events exceptions when a recurring meeting moved"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 && cmd.Flags().NFlag() == 0 {
				return cmd.Help() // verify-friendly: bare invocation shows help (exit 0)
			}
...
func newPlaybookListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example:     `  google-calendar-pp-cli playbook list --agent`,
