...
// google-calendar-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/productivity/google-calendar/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/productivity/google-calendar/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/productivity/google-calendar/internal/learn/patterns"
...
const noLearnEnvVar = "GOOGLE_CALENDAR_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("google-calendar-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  google-calendar-pp-cli teach --query "which calendar is the family calendar" --resource-type calendars --resource family00000000000000000000@group.calendar.google.com`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 && cmd.Flags().NFlag() == 0 {
				return cmd.Help() // verify-friendly: bare invocation shows help (exit 0)
			}
			...
			if noLearnActive(flags) {
				if flags.asJSON {
					return printJSONFiltered(cmd.OutOrStdout(), map[string]any{
						"recorded": false,
						"reason":   "no-learn active (--no-learn or " + noLearnEnvVar + ")",
					}, flags)
				}
				return nil
			}
			...
			// An explicit --json outranks the DEFAULT quiet=true (background
			// convention); an explicitly passed --quiet still silences.
			if flags.asJSON && !(quiet && cmd.Flags().Changed("quiet")) {
				return printJSONFiltered(cmd.OutOrStdout(), map[string]any{
					"recorded":   true,
					"query":      query,
					"normalized": normalized.NonEntityNormalized,
...
when learnings exist.`,
		Example: `  google-calendar-pp-cli recall "<question>" --agent
  google-calendar-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List recorded query-to-resource learnings, filterable by query, source, resource, action, and min confidence",
		Example: `  google-calendar-pp-cli learnings list --agent
  google-calendar-pp-cli learnings list --query "<substring>"
  google-calendar-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  google-calendar-pp-cli learnings forget "<question>" --resource <id>
  google-calendar-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  google-calendar-pp-cli teach-pattern --query-template "events on {entity}" --resource-template "{entity:calendar}" --resource-type "calendars" --entity-kind "calendar" --strategy substitute`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 && cmd.Flags().NFlag() == 0 {
				return cmd.Help() // verify-friendly: bare invocation shows help (exit 0)
			}
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  google-calendar-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 && cmd.Flags().NFlag() == 0 {
				return cmd.Help()
			}
