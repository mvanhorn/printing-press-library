...
var itemHydrationPaths = map[string]itemHydrationConfig{
	"events": {path: "/calendars/{calendarId}", idParam: "calendarId"},
}
...
func responsePathForResource(resource, path string) []string {
	...
	case "users\x00/users/me/calendarList/{calendarId}":
		return []string{"defaultReminders"}
	case "users\x00/users/me/calendarList":
		return []string{"items"}
	case "users\x00/users/me/settings":
		return []string{"items"}
	}
	return nil
}
