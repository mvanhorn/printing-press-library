...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Fprintln(w, "  google-calendar-pp-cli login")
			if !launch {
				return nil
			}
...
func newAuthLoginCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  google-calendar-pp-cli login\n" +
			"  google-calendar-pp-cli login --client-id 1234.apps.googleusercontent.com --port 8085",
...
func runOAuthLogin(cmd *cobra.Command, flags *rootFlags, clientID, clientSecret string, port int) error {
	...
	// --dry-run: describe the flow and stop BEFORE credential resolution, so a
	// preview never prompts interactively, binds the callback port, or opens a
	// browser. A missing client id renders as a placeholder instead of a prompt.
	if dryRunOK(flags) {
		id := clientID
		if id == "" && cfg != nil && cfg.ClientID != "" {
			id = cfg.ClientID
		}
		if id == "" {
			id = "<client-id>"
		}
		fmt.Fprintln(w, "dry-run: would start OAuth2 authorization-code login (PKCE when no client secret):")
		fmt.Fprintf(w, "  1. bind loopback callback server on 127.0.0.1:%d\n", port)
		fmt.Fprintf(w, "  2. open browser at https://accounts.google.com/o/oauth2/auth?client_id=%s&response_type=code&...\n", id)
		fmt.Fprintln(w, "  3. exchange the returned code for tokens and save them to config")
		return nil
	}
...
func resolveOAuthCredentials(cmd *cobra.Command, flags *rootFlags, cfg *config.Config, clientID, clientSecret string) (string, string, error) {
	...
	if flags.noInput {
		return "", "", fmt.Errorf("OAuth2 client ID is required; pass --client-id, set GOOGLE_CALENDAR_CLIENT_ID, or run 'google-calendar-pp-cli login' without --no-input")
	}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				fmt.Fprintf(w, "  %s Not authenticated. Run 'google-calendar-pp-cli login' to authenticate.\n", red("FAIL"))
				return nil
			}
			...
			if cfg.TokenExpiry.IsZero() {
				...
				if cfg.RefreshToken != "" {
					...
					fmt.Fprintf(w, "  %s Token expired. Run 'google-calendar-pp-cli login' to re-authenticate.\n", red("FAIL"))
				}
			}
