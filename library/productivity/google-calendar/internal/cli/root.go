...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/productivity/google-calendar/internal/learn"
...
type rootFlags struct {
	...
	// invocation. Mirrors the GOOGLE_CALENDAR_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		...
		Short: `Google Calendar CLI — The multi-account Google Calendar layer for agents that must never surprise a human.`,
		Long: `Google Calendar CLI — The multi-account Google Calendar layer for agents that must never surprise a human.
...
Health check: run 'google-calendar-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
	...
	rootCmd.PersistentFlags().StringVar(&flags.account, "account", "", "gauth profile name from profiles.yaml (multi-account OAuth; role decides scopes)")
	rootCmd.PersistentFlags().StringVar(&flags.authDir, "auth-dir", "", "gauth config dir holding client.json/profiles.yaml/tokens (default ~/.config/google-calendar-pp-cli/gauth or GCAL_CONFIG_DIR)")
	...
	rootCmd.AddCommand(newNovelConflictsCmd(flags))
	rootCmd.AddCommand(newNovelEventsCmd(flags))
	rootCmd.AddCommand(newNovelManifestCmd(flags))
	rootCmd.AddCommand(newNovelSlotsCmd(flags))
...
// clientFor builds a client authenticated as the named gauth profile,
// independent of the --account flag — the fan-out primitive for cross-account
// commands (agenda, conflicts, freebusy, slots, changes, manifest).
func (f *rootFlags) clientFor(profileName string) (*client.Client, error) {
	saved := f.account
	f.account = profileName
	defer func() { f.account = saved }()
	return f.newClient()
}
