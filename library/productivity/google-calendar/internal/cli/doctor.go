		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export GOOGLE_CALENDAR_CLIENT_ID=<your-key>"
			...
			if os.Getenv("GOOGLE_CALENDAR_CLIENT_ID") != "" {
				authEnvSet = append(authEnvSet, "GOOGLE_CALENDAR_CLIENT_ID")
			} else {
				authEnvInfo = append(authEnvInfo, "GOOGLE_CALENDAR_CLIENT_ID set during auth login")
			}
			if os.Getenv("GOOGLE_CALENDAR_CLIENT_SECRET") != "" {
				authEnvSet = append(authEnvSet, "GOOGLE_CALENDAR_CLIENT_SECRET")
			} else {
				authEnvInfo = append(authEnvInfo, "GOOGLE_CALENDAR_CLIENT_SECRET set during auth login")
			}
			if os.Getenv("GOOGLE_CALENDAR_TOKEN") != "" || authHeader != "" {
				authEnvSet = append(authEnvSet, "GOOGLE_CALENDAR_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "GOOGLE_CALENDAR_TOKEN populated automatically by auth login")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "google-calendar-pp-cli", suggestion)
						} else {
