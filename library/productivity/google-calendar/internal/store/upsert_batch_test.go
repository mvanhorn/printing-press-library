...
// TestUpsertBatch_TypedFailureDoesNotStrandAclGeneric exercises
...
func TestUpsertBatch_TypedFailureDoesNotStrandAclGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	stored, extractFailures, err := s.UpsertBatch("acl", items)
	if err != nil {
	...
	if typed != 0 {
		t.Fatalf("acl count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "calendars_id")
	}
}
...
func TestUpsertBatch_TypedFailureDoesNotStrandClearGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("clear count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "calendars_id")
	}
}
