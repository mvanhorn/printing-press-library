...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("quick_add_event",
			mcplib.WithDescription("Create a calendar event from a natural-language phrase and return the full created event in one call."),
			mcplib.WithString("calendar_id", mcplib.Required(), mcplib.Description("Target calendar ID, e.g. primary.")),
			mcplib.WithString("text", mcplib.Required(), mcplib.Description("Natural-language event description, e.g. Lunch with Sam tomorrow 1pm.")),
		),
		handleQuickAddEvent,
	)
}

// handleQuickAddEvent runs the quick_add_event intent: Create a calendar event from a natural-language phrase and return the full created event in one call.
func handleQuickAddEvent(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	c, err := newMCPClient()
	if err != nil {
	...
	// Step 1: calendars.events.quick-add
	{
		params := map[string]any{}
		if v, ok := resolveIntentBinding(scope, "${input.calendar_id}"); ok {
			params["calendarId"] = v
		}
		if v, ok := resolveIntentBinding(scope, "${input.text}"); ok {
			params["text"] = v
		}
		resp, err := callIntentEndpoint(ctx, c, "calendars.events.quick-add", params)
		if err != nil {
			return mcplib.NewToolResultError(fmt.Sprintf("step %d (calendars.events.quick-add) failed: %v", 1, err)), nil
		}
		scope["created"] = resp
	}

	// Step 2: calendars.events.get
	{
		params := map[string]any{}
		if v, ok := resolveIntentBinding(scope, "${input.calendar_id}"); ok {
			params["calendarId"] = v
		}
		if v, ok := resolveIntentBinding(scope, "${created.id}"); ok {
			params["eventId"] = v
		}
		resp, err := callIntentEndpoint(ctx, c, "calendars.events.get", params)
		if err != nil {
			return mcplib.NewToolResultError(fmt.Sprintf("step %d (calendars.events.get) failed: %v", 2, err)), nil
		}
		scope["event"] = resp
	}
...
var intentEndpoints = map[string]intentEndpointMeta{
	"calendars.events.quick-add": {method: "POST", path: "/calendars/{calendarId}/events/quickAdd"},
	"calendars.events.get":       {method: "GET", path: "/calendars/{calendarId}/events/{eventId}"},
}
