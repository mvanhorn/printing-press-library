...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export GOOGLE_CALENDAR_CLIENT_ID=<your-token> or google-calendar-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.GoogleCalendarClientSecret)
		addCredential(c.Config.GoogleCalendarToken)
	}
