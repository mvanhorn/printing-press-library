...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".harvest-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("HARVEST_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HARVEST_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "harvest-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.harvest-pp-cli/feedback.jsonl.
When ` + "`HARVEST_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`HARVEST_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  harvest-pp-cli feedback list
  harvest-pp-cli feedback list --limit 5
  harvest-pp-cli feedback list --json`,
