...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  harvest-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  harvest-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"clients", "contacts", "estimate-item-categories", "estimates", "expense-categories", "expenses", "invoice-item-categories", "invoices", "projects", "reports", "roles", "task-assignments", "tasks", "time-entries", "user-assignments", "users", "users-me-project-assignments"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  harvest-pp-cli workflow status

  # Show status as JSON
  harvest-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("harvest-pp-cli")
			}
