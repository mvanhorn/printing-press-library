...
Exit codes & warnings:
  ...
  harvest-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  harvest-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  harvest-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  harvest-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  harvest-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/harvest-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "estimate-item-categories":
		return db.UpsertEstimateItemCategories(data)
	case "estimates":
		return db.UpsertEstimates(data)
	case "estimates_messages":
		return db.UpsertEstimatesMessages(data)
	case "expense-categories":
		return db.UpsertExpenseCategories(data)
	case "expenses":
		return db.UpsertExpenses(data)
	case "invoice-item-categories":
		return db.UpsertInvoiceItemCategories(data)
	case "invoices":
		return db.UpsertInvoices(data)
	case "invoices_messages":
		return db.UpsertInvoicesMessages(data)
	case "payments":
		return db.UpsertPayments(data)
	case "projects":
		return db.UpsertProjects(data)
	case "projects_task_assignments":
		return db.UpsertProjectsTaskAssignments(data)
	case "projects_user_assignments":
		return db.UpsertProjectsUserAssignments(data)
	case "reports":
		return db.UpsertReports(data)
	case "roles":
		return db.UpsertRoles(data)
	case "task-assignments":
		return db.UpsertTaskAssignments(data)
	case "tasks":
		return db.UpsertTasks(data)
	case "time-entries":
		return db.UpsertTimeEntries(data)
	case "external_reference":
		return db.UpsertExternalReference(data)
	case "restart":
		return db.UpsertRestart(data)
	case "stop":
		return db.UpsertStop(data)
	case "user-assignments":
		return db.UpsertUserAssignments(data)
	case "users":
		return db.UpsertUsers(data)
	case "billable_rates":
		return db.UpsertBillableRates(data)
	case "cost_rates":
		return db.UpsertCostRates(data)
	case "project_assignments":
		return db.UpsertProjectAssignments(data)
	case "teammates":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"clients":                      "/clients",
		"contacts":                     "/contacts",
		"estimate-item-categories":     "/estimate_item_categories",
		"estimates":                    "/estimates",
		...
		"invoices":                     "/invoices",
		"projects":                     "/projects",
		"reports":                      "/reports/project_budget",
		"roles":                        "/roles",
		...
		"users":                        "/users",
		"users-me-project-assignments": "/users/me/project_assignments",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "billable_rates", ParentTable: "users", ParentIDParam: "userId", PathTemplate: "/users/{userId}/billable_rates", KeyField: ""},
		{Name: "cost_rates", ParentTable: "users", ParentIDParam: "userId", PathTemplate: "/users/{userId}/cost_rates", KeyField: ""},
		{Name: "estimates_messages", ParentTable: "estimates", ParentIDParam: "estimateId", PathTemplate: "/estimates/{estimateId}/messages", KeyField: ""},
		{Name: "invoices_messages", ParentTable: "invoices", ParentIDParam: "invoiceId", PathTemplate: "/invoices/{invoiceId}/messages", KeyField: ""},
		{Name: "payments", ParentTable: "invoices", ParentIDParam: "invoiceId", PathTemplate: "/invoices/{invoiceId}/payments", KeyField: ""},
		{Name: "project_assignments", ParentTable: "users", ParentIDParam: "userId", PathTemplate: "/users/{userId}/project_assignments", KeyField: ""},
		{Name: "projects_task_assignments", ParentTable: "projects", ParentIDParam: "projectId", PathTemplate: "/projects/{projectId}/task_assignments", KeyField: ""},
		{Name: "projects_user_assignments", ParentTable: "projects", ParentIDParam: "projectId", PathTemplate: "/projects/{projectId}/user_assignments", KeyField: ""},
		{Name: "teammates", ParentTable: "users", ParentIDParam: "userId", PathTemplate: "/users/{userId}/teammates", KeyField: ""},
	}
}
