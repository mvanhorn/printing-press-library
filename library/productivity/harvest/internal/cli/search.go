...
In local mode: searches locally synced data only.`,
  ...
  harvest-pp-cli search "error timeout"

  # Force local search only
  harvest-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  harvest-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  harvest-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'harvest-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "estimates":
				results, err = db.SearchEstimates(query, limit)
			case "invoices":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchEstimates(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search estimates failed: %w", searchErr)
					}
