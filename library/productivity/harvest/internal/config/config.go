...
type Config struct {
	...
	Path               string            `toml:"-"`
	HarvestAccountAuth string            `toml:"account_auth"`
}
...
func Load(configPath string) (*Config, error) {
	...
	if path == "" {
		...
		path = filepath.Join(home, ".config", "harvest-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("HARVEST_ACCOUNT_AUTH"); v != "" {
		cfg.HarvestAccountAuth = v
		cfg.AuthSource = "env:HARVEST_ACCOUNT_AUTH"
	}
	...
	if cfg.AuthSource == "" && cfg.HarvestAccountAuth != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("HARVEST_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.HarvestAccountAuth
	if token == "" {
		return ""
	}
	if c.HarvestAccountAuth == "" {
		return ""
	}
	return token
}
