# Harvest Printed CLI Agent Guide

This directory is a generated `harvest-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
harvest-pp-cli doctor --json
harvest-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
harvest-pp-cli which "<capability>" --json
harvest-pp-cli <command> --help
```
...
```bash
harvest-pp-cli <command> --help
harvest-pp-cli <command> --dry-run --agent
```
