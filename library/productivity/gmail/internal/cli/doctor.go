...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gmail-pp-cli doctor
  gmail-pp-cli doctor --json
  gmail-pp-cli doctor --fail-on warn
  gmail-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "gmail-pp-cli accounts auth --account <name>"
					report["auth_docs_url"] = "https://developers.google.com/gmail/api"
				} else {
					authConfigured = true
			...
			if os.Getenv("GMAIL_OAUTH2C") != "" {
				authEnvSet = append(authEnvSet, "GMAIL_OAUTH2C")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "GMAIL_OAUTH2C")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					// Probes the users.me profile — a bare "/" probe would be
					// refused by the in-binary transport allowlist before dialing.
					reachBody, reachErr := c.Get(cmd.Context(), "/gmail/v1/users/me/profile", nil)
					...
					case errors.As(reachErr, &reachAPIErr):
						...
						if vendor := looksLikeDoctorInterstitial([]byte(reachAPIErr.Body)); vendor != "" {
							...
							report["api"] = fmt.Sprintf("reachable (HTTP %d at /gmail/v1/users/me/profile)", status)
						}
					default:
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "gmail-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'gmail-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'gmail-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'gmail-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'gmail-pp-cli sync' to refresh."
	}
	return report
}
