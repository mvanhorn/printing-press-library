...
var whichIndex = []whichEntry{
	{Command: "schedule send", Description: "Write an email now and have it sent at any future time, even though the Gmail API itself has no schedule-send. Delivery requires `schedule run` from cron/launchd (or `schedule run --watch`).", Group: "What the API itself lacks", WhyItMatters: "Reach for this whenever a user asks to send an email later, at a specific time, or in another timezone; no other Gmail API tool can do it. Tell the user to wire `gmail-pp-cli schedule run` into cron, or the queue will sit undelivered."},
	{Command: "senders", Description: "See who actually fills your mailbox: per-sender message count, unread count, total size, and first/last seen.", Group: "Local mailbox intelligence", WhyItMatters: "Use this before any cleanup or unsubscribe pass to rank which senders matter; the live API cannot aggregate by sender at all. Requires a populated local mirror: run `gmail-pp-cli pull` first."},
	{Command: "storage", Description: "Rank senders and years by mailbox bytes consumed, with the exact cleanup command printed next to each group.", Group: "Local mailbox intelligence", WhyItMatters: "Use when the user is near their storage quota or asks what is taking up space in Gmail. Requires a populated local mirror: run `gmail-pp-cli pull` first."},
	{Command: "followups", Description: "List threads where the last word was theirs (you owe a reply) or yours (they never replied), aged past N days.", Group: "Local mailbox intelligence", WhyItMatters: "Use for outreach chasing (who never replied) and inbox accountability (what do I owe); no Gmail search operator can express either. Requires a populated local mirror: run `gmail-pp-cli pull` first."},
	{Command: "unsub", Description: "Rank mailing lists by volume and unread ratio, and emit each list's unsubscribe target ready to act on.", Group: "Local mailbox intelligence", WhyItMatters: "Use when the user wants to cut inbox noise; the unread ratio identifies lists they never actually read. Requires a populated local mirror: run `gmail-pp-cli pull` first."},
	{Command: "filters diff", Description: "Keep Gmail filters in a version-controlled YAML file; diff against live settings and apply the reconciliation plan.", Group: "What the API itself lacks", WhyItMatters: "Use when the user manages filters across machines or wants filter changes reviewed before they touch the account."},
}
