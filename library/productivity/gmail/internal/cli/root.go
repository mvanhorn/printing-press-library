...
type rootFlags struct {
	...
	// invocation. Mirrors the GMAIL_NO_LEARN env var.
	noLearn bool
	...
	homePath            string
	profileName         string
	// account selects a gauth multi-account profile (profiles.yaml in the
	// gauth config dir); when set, a live per-profile OAuth token is injected
	// and the config-file credential path is bypassed entirely.
	account       string
	authDir       string
	deliverSpec   string
...
func Execute() (retErr error) {
	...
	// Raw-mutation absence contract (grill R1-C1): `messages modify --help`
	// and friends must exit non-zero as unknown commands, not print the
	// parent's help at exit 0. Cobra parses --help before validating
	// positional args and swallows flag.ErrHelp inside ExecuteC (printing
	// the parent's help and returning nil), so the unknown-subcommand token
	// would otherwise be masked entirely. A parent with subcommands can only
	// reach err==nil with leftover positional args via that help
	// short-circuit — every non-help path errors in ValidateArgs first.
	if (err == nil || errors.Is(err, pflag.ErrHelp)) && executedCmd != nil && executedCmd.HasSubCommands() {
		if rest := executedCmd.Flags().Args(); len(rest) > 0 && findSubcommand(executedCmd, rest[0]) == nil {
			// Printed explicitly: this error is minted after ExecuteC
			// returned, so Cobra's own error printer never sees it.
			unknownErr := fmt.Errorf("unknown command %q for %q", rest[0], executedCmd.CommandPath())
			fmt.Fprintf(os.Stderr, "Error: %v\n", unknownErr)
			return usageErr(unknownErr)
		}
	}
...
Health check: run 'gmail-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("gmail-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.account, "account", "", "gauth profile name from profiles.yaml (multi-account OAuth; every profile holds the single gmail.modify scope)")
	rootCmd.PersistentFlags().StringVar(&flags.authDir, "auth-dir", "", "gauth config dir holding client.json/profiles.yaml/tokens (default ~/.config/gmail-pp-cli/gauth or GMAIL_CONFIG_DIR)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'gmail-pp-cli profile list')")
	...
	// newImportCmd deliberately NOT registered (grill R1-C1/R2-C2): the
	// generated import command is a generic "POST /<resource>" replay — a
	// raw mutation surface this binary must not expose. The transport
	// allowlist would refuse its requests anyway; unregistering removes
	...
	// Gmail-specific metadata sync (mail_sync.go) replaces the generated
	// generic-resource sync registration; see mail_sync.go's header comment.
	rootCmd.AddCommand(newMailSyncCmd(flags))
	rootCmd.AddCommand(newTailCmd(flags))
	rootCmd.AddCommand(newAnalyticsCmd(flags))
	rootCmd.AddCommand(newWorkflowCmd(flags))
	rootCmd.AddCommand(newNovelDeltaCmd(flags))
	// digest: the flagship per-category summary (hand-written, local store).
	rootCmd.AddCommand(newNovelDigestCmd(flags))
	rootCmd.AddCommand(newNovelRulesCmd(flags))
	rootCmd.AddCommand(newNovelScoreCmd(flags))
	rootCmd.AddCommand(newNovelSortCmd(flags))
	rootCmd.AddCommand(newNovelStorageCmd(flags))
	rootCmd.AddCommand(newNovelTrashCmd(flags))
	rootCmd.AddCommand(newNovelUnsubCmd(flags))
...
func (f *rootFlags) newClient() (*client.Client, error) {
	...
	if f.account != "" {
		// Multi-account path: inject a live per-profile token (gauth refreshes
		// and persists before returning) and disable the config-file credential
		// machinery so the generic refresh path can never fire.
		tok, err := gauth.AccessToken(context.Background(), gauth.ConfigDir(f.authDir), f.account)
		if err != nil {
			return nil, err
		}
		cfg.AccessToken = tok
