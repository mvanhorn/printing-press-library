...
// hasCommandNamed reports whether parent already registered a command with
// the given name. Root-level novel commands use it to keep their guarded
// AddCommand calls inline (statically analyzable) instead of routing
// through addNovelCommandIfAbsent's indirection.
func hasCommandNamed(parent *cobra.Command, name string) bool {
	for _, existing := range parent.Commands() {
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "gmail-pp-cli",
		Short: `Gmail CLI — Every Gmail API surface plus the things Gmail itself will not give you: scheduled sends, sender analytics, follow-up…`,
		Long: `Gmail CLI — Every Gmail API surface plus the things Gmail itself will not give you: scheduled sends, sender analytics, follow-up tracking, and an offline full-text mailbox.
...
See README.md or the bundled SKILL.md for recipes.`,
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'gmail-pp-cli profile list')")
	...
	// The root-level novel commands use direct rootCmd.AddCommand calls
	// (behind the same absent-name guard addNovelCommandIfAbsent applies)
	// so static command-tree analysis can see the root -> command edges;
	// the helper's indirection hides them, which both drops these files
	// from registered-command scans and lets leaves like schedule's "send"
	// false-match the generated drafts/messages send commands in
	// novel-feature depth checks.
	if !hasCommandNamed(rootCmd, "filters") {
		rootCmd.AddCommand(newNovelFiltersCmd(flags))
	}
	if !hasCommandNamed(rootCmd, "followups") {
		rootCmd.AddCommand(newNovelFollowupsCmd(flags))
	}
	if !hasCommandNamed(rootCmd, "schedule") {
		rootCmd.AddCommand(newNovelScheduleCmd(flags))
	}
	if !hasCommandNamed(rootCmd, "senders") {
		rootCmd.AddCommand(newNovelSendersCmd(flags))
	}
	if !hasCommandNamed(rootCmd, "storage") {
		rootCmd.AddCommand(newNovelStorageCmd(flags))
	}
	if !hasCommandNamed(rootCmd, "unsub") {
