...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".gmail-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GMAIL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GMAIL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "gmail-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.gmail-pp-cli/feedback.jsonl.
When ` + "`GMAIL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GMAIL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gmail-pp-cli feedback list
  gmail-pp-cli feedback list --limit 5
  gmail-pp-cli feedback list --json`,
