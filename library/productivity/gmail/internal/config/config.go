...
func Load(configPath string) (*Config, error) {
	...
	if v := os.Getenv("GMAIL_ACCESS_TOKEN"); v != "" {
		...
		cfg.AuthSource = "env:GMAIL_ACCESS_TOKEN"
		cfg.CredentialSource = "env:GMAIL_ACCESS_TOKEN"
	}
	...
	if v := strings.TrimSpace(os.Getenv("GMAIL_USER_ID")); v != "" {
		...
		// Gmail convention: "me" addresses the authenticated user on every
		// /users/{userId}/ path; no other value makes sense for a personal
		// OAuth token.
		cfg.TemplateVars["userId"] = "me"
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	// A directly-held bearer in GMAIL_ACCESS_TOKEN (a pasted access token, or a
...
func (c *Config) hasCompleteCredentialFields() bool {
	...
	// A refresh token alone is not a complete credential set: the access
	// token must still be merged from the credentials file (or minted).
	if c.AccessToken != "" {
		return true
	}
	return false
