...
type Config struct {
	...
	SigmaComputingClientId     string          `toml:"computing_client_id"`
	SigmaComputingClientSecret string          `toml:"computing_client_secret"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("SIGMA_COMPUTING_CLIENT_ID"); v != "" {
		cfg.SigmaComputingClientId = v
		cfg.markEnvOverride("SigmaComputingClientId")
		cfg.AuthSource = "env:SIGMA_COMPUTING_CLIENT_ID"
		cfg.CredentialSource = "env:SIGMA_COMPUTING_CLIENT_ID"
	}
	if v := os.Getenv("SIGMA_COMPUTING_CLIENT_SECRET"); v != "" {
		cfg.SigmaComputingClientSecret = v
		cfg.markEnvOverride("SigmaComputingClientSecret")
		cfg.AuthSource = "env:SIGMA_COMPUTING_CLIENT_SECRET"
		cfg.CredentialSource = "env:SIGMA_COMPUTING_CLIENT_SECRET"
	}
	...
	if v := os.Getenv("SIGMA_COMPUTING_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("SIGMA_COMPUTING_TOKEN_URL"); v != "" {
		cfg.TokenURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("SIGMA_COMPUTING_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "sigma-computing-pp-cli", "config.toml"), nil
}
...
func (c *Config) hasCredentialFields() bool {
	...
	if c.SigmaComputingClientId != "" {
		return true
	}
	if c.SigmaComputingClientSecret != "" {
		return true
	}
	return false
}
...
func (c *Config) clearCredentialFields() {
	...
	c.ClientID = ""
	c.ClientSecret = ""
	c.SigmaComputingClientId = ""
	c.SigmaComputingClientSecret = ""
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		SigmaComputingClientId:     c.SigmaComputingClientId,
		SigmaComputingClientSecret: c.SigmaComputingClientSecret,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.SigmaComputingClientId = creds.SigmaComputingClientId
	c.SigmaComputingClientSecret = creds.SigmaComputingClientSecret
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.SigmaComputingClientId = ""
	delete(c.envOverrides, "SigmaComputingClientId")
	c.updateFileConfigField("SigmaComputingClientId")
	c.SigmaComputingClientSecret = ""
	delete(c.envOverrides, "SigmaComputingClientSecret")
	c.updateFileConfigField("SigmaComputingClientSecret")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["SigmaComputingClientId"] {
			out.SigmaComputingClientId = c.fileConfig.SigmaComputingClientId
		}
		if c.envOverrides["SigmaComputingClientSecret"] {
			out.SigmaComputingClientSecret = c.fileConfig.SigmaComputingClientSecret
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "SigmaComputingClientId":
		c.fileConfig.SigmaComputingClientId = c.SigmaComputingClientId
	case "SigmaComputingClientSecret":
		c.fileConfig.SigmaComputingClientSecret = c.SigmaComputingClientSecret
	}
}
