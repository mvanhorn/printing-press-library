...
In local mode: searches locally synced data only.`,
  ...
  sigma-computing-pp-cli search "error timeout"

  # Force local search only
  sigma-computing-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  sigma-computing-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  sigma-computing-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'sigma-computing-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "api-connectors":
				results, err = db.SearchApiConnectors(query, limit)
			case "api-credentials":
				results, err = db.SearchApiCredentials(query, limit)
			case "datasets":
			...
			case "user-attributes":
				results, err = db.SearchUserAttributes(query, limit)
			case "workbooks":
				results, err = db.SearchWorkbooks(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchApiConnectors(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search api_connectors failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchApiCredentials(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search api_credentials failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchUserAttributes(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search user_attributes failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchWorkbooks(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search workbooks failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
