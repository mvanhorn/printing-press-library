...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  sigma-computing-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  sigma-computing-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"connections", "connections-paths", "data-models", "datasets", "files", "grants", "members", "reports", "shared-templates", "source-swap-policies", "tags", "teams", "templates", "translations", "user-attributes", "workbooks", "workspaces"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  sigma-computing-pp-cli workflow status

  # Show status as JSON
  sigma-computing-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("sigma-computing-pp-cli")
			}
