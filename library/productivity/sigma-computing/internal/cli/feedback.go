...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SIGMA_COMPUTING_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SIGMA_COMPUTING_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "sigma-computing-pp-cli/feedback")
...
When ` + "`SIGMA_COMPUTING_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SIGMA_COMPUTING_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  sigma-computing-pp-cli feedback list
  sigma-computing-pp-cli feedback list --limit 5
  sigma-computing-pp-cli feedback list --json`,
