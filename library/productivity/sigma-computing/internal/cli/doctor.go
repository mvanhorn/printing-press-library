...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  sigma-computing-pp-cli doctor
  sigma-computing-pp-cli doctor --json
  sigma-computing-pp-cli doctor --fail-on warn
  sigma-computing-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Run 'sigma-computing-pp-cli auth setup' for credential setup steps."
					report["auth_key_url"] = "https://help.sigmacomputing.com/reference"
				} else {
					authConfigured = true
			...
			if os.Getenv("SIGMA_COMPUTING_CLIENT_ID") != "" {
				authEnvSet = append(authEnvSet, "SIGMA_COMPUTING_CLIENT_ID")
			} else {
				authEnvInfo = append(authEnvInfo, "SIGMA_COMPUTING_CLIENT_ID set during auth login")
			}
			if os.Getenv("SIGMA_COMPUTING_CLIENT_SECRET") != "" {
				authEnvSet = append(authEnvSet, "SIGMA_COMPUTING_CLIENT_SECRET")
			} else {
				authEnvInfo = append(authEnvInfo, "SIGMA_COMPUTING_CLIENT_SECRET set during auth login")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "sigma-computing-pp-cli"
						verifyPath := "/v2/whoami"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'sigma-computing-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'sigma-computing-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'sigma-computing-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'sigma-computing-pp-cli sync' to refresh."
	}
	return report
}
