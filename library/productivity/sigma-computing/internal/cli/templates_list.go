...
func newTemplatesListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "list",
		Short:       "Returns a list of available templates. ### Usage notes - Official Sigma templates are created by SigmaSchedulerRobot.",
		Example:     "  sigma-computing-pp-cli templates list",
		Annotations: map[string]string{"pp:endpoint": "templates.list", "pp:method": "GET", "pp:path": "/v2/templates", "mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if cmd.Flags().Changed("source") {
				allowedSource := []string{"internal", "external"}
				validSource := false
	...
	cmd.Flags().StringVar(&flagSource, "source", "", "Source (one of: internal, external)")
