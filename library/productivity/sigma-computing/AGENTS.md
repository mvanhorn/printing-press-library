# Sigma Printed CLI Agent Guide

This directory is a generated `sigma-computing-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
sigma-computing-pp-cli doctor --json
sigma-computing-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
sigma-computing-pp-cli which "<capability>" --json
sigma-computing-pp-cli <command> --help
```
...
```bash
sigma-computing-pp-cli <command> --agent
```
...
```bash
sigma-computing-pp-cli <command> --help
sigma-computing-pp-cli <command> --dry-run --agent
```
