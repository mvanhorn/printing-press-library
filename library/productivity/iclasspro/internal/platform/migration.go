...
func AdoptVerifiedLegacyDatabase(ctx context.Context, session *Session, state *LegacyMigrationState) error {
	...
	// #nosec G202 -- SQLite's VACUUM INTO takes a literal filename and cannot accept
	// a bind parameter. targetPath is computed by this CLI (never user-supplied SQL),
	// and single quotes are doubled above so the literal cannot be terminated early.
