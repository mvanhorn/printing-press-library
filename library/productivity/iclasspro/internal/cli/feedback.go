...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ICLASSPRO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ICLASSPRO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "iclasspro-pp-cli/feedback")
...
When ` + "`ICLASSPRO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ICLASSPRO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  iclasspro-pp-cli feedback list
  iclasspro-pp-cli feedback list --limit 5
  iclasspro-pp-cli feedback list --json`,
