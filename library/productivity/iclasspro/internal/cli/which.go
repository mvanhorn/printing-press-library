...
var whichIndex = []whichEntry{
	{Command: "watch", Description: "Get told the moment a spot frees up in a class or camp that is currently full.", Group: "Local history the API forgets", WhyItMatters: "Reach for this instead of re-polling a list endpoint yourself: it stores every observation, so it reports the transition rather than the current value."},
	{Command: "drift", Description: "See what changed between syncs: classes and camps added, removed, retimed, or newly marked deleted.", Group: "Local history the API forgets", WhyItMatters: "Use this to answer 'what is different since last week' without re-reading the whole catalog into context."},
	{Command: "opens-soon", Description: "Find registration that has not opened yet or is about to close, across every synced class and camp.", Group: "Local history the API forgets", WhyItMatters: "This is the only way to see a camp before its registration opens, which is when popular sessions are still winnable."},
	{Command: "calendar", Description: "Export synced camps and classes to an RFC 5545 calendar file, one event per session.", Group: "Catalog hygiene and publishing", WhyItMatters: "Turns a portal catalog into something a calendar app, a website, or a newsletter can consume directly."},
	{Command: "tenant", Description: "Report which surfaces an account actually exposes: open, sign-in-gated, or plan-gated.", Group: "Multi-tenant reach", WhyItMatters: "Run this first against any new account; it prevents mistaking a sign-in gate for a gym with no classes."},
	{Command: "fill-rate", Description: "Show how fast classes are filling over time, by class or by program.", Group: "Local history the API forgets", WhyItMatters: "Answers whether a class is trending toward full or toward cancellation, which no upstream call can express."},
	{Command: "compare", Description: "Compare the same kind of program across several gyms at once.", Group: "Multi-tenant reach", WhyItMatters: "Built for franchise and agency operators who track many gyms and need one table instead of many tabs."},
	{Command: "lint", Description: "Flag catalog quality problems: missing descriptions or images, expired registration windows, deleted-but-listed programs.", Group: "Catalog hygiene and publishing", WhyItMatters: "Use before publishing a schedule to a website; it catches the records that will render blank or dead."},
	{Command: "admin", Description: "Read an explicit allow-list of authenticated staff resources without mutation, export, or response caching.", Group: "Read-only Office Portal access", WhyItMatters: "Provides staff-side visibility without giving an agent a generic request escape hatch or any write capability."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  iclasspro-pp-cli which "stale tickets"
  iclasspro-pp-cli which "bottleneck"
  iclasspro-pp-cli which --limit 1 "send message"
  iclasspro-pp-cli which                                # list the full capability index`,
