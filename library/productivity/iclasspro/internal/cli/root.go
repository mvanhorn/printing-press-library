...
type rootFlags struct {
	...
	// invocation. Mirrors the ICLASSPRO_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "iclasspro-pp-cli",
		Short: `The only iClassPro tool that remembers yesterday — a queryable local mirror of any gym's catalog, with the search and filter surface the portal keeps to itself.`,
		Long: `iClassPro CLI — The only iClassPro tool that remembers yesterday — a queryable local mirror of any gym's catalog, with the search and filter surface the portal keeps to itself.
...
Health check: run 'iclasspro-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("iclasspro-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'iclasspro-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCampsCmd(flags))
	...
	rootCmd.AddCommand(newInstructorsPromotedCmd(flags))
	rootCmd.AddCommand(newLevelsPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelFillRateCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelLintCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelOpensSoonCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelTenantCmd(flags))
