# Trello Printed CLI Agent Guide

This directory is a generated `trello-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
trello-pp-cli doctor --json
trello-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
trello-pp-cli which "<capability>" --json
trello-pp-cli <command> --help
```
...
```bash
trello-pp-cli <command> --help
trello-pp-cli <command> --dry-run --agent
```
