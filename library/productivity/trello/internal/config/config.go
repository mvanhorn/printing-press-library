...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("TRELLO_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "trello-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("TRELLO_API_KEY"); v != "" {
		cfg.TrelloApiKey = v
		cfg.AuthSource = "env:TRELLO_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.TrelloApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("TRELLO_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	// Trello uses dual query-param auth (key + token). Resolve the token here
	// so the client can append it alongside `key`. See trello_config.go.
	cfg.LoadTrelloToken()
	return cfg, nil
}
