...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "trello-pp-cli", "http")
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		// Trello dual-credential auth: the user token travels in the `token`
		// query param alongside `key`. The generated auth path only wired
		// `key`; without this, every authenticated call returns "invalid
		// token". See internal/config/trello_config.go.
		if c.Config != nil {
			if tok := c.Config.TrelloToken(); tok != "" {
				q := req.URL.Query()
				q.Set("token", tok)
				req.URL.RawQuery = q.Encode()
			}
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "trello-pp-cli/1.0")
		}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.TrelloApiKey)
	}
