# Zoho Projects Printed CLI Agent Guide

This directory is a generated `zoho-projects-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
zoho-projects-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
zoho-projects-pp-cli which "<capability>" --json
zoho-projects-pp-cli <command> --help
```
...
```bash
zoho-projects-pp-cli <command> --agent
```
...
```bash
zoho-projects-pp-cli <command> --help
zoho-projects-pp-cli <command> --dry-run --agent
```
