...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("ZOHO_PROJECTS_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "zoho-projects-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("ZOHOPROJECTS_REFRESH_TOKEN"); v != "" {
		cfg.RefreshToken = v
		cfg.AuthSource = "env:ZOHOPROJECTS_REFRESH_TOKEN"
	}
	...
	if v := os.Getenv("ZOHO_PROJECTS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
