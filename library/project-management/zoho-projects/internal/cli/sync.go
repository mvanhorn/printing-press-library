...
Exit codes & warnings:
  ...
  zoho-projects-pp-cli sync

  # Sync specific resources only
  zoho-projects-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  zoho-projects-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  zoho-projects-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  zoho-projects-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  zoho-projects-pp-cli sync --latest-only`,
