...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  zoho-projects-pp-cli doctor
  zoho-projects-pp-cli doctor --json
  zoho-projects-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export ZOHOPROJECTS_REFRESH_TOKEN=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("ZOHOPROJECTS_REFRESH_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "ZOHOPROJECTS_REFRESH_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "ZOHOPROJECTS_REFRESH_TOKEN")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'zoho-projects-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'zoho-projects-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'zoho-projects-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'zoho-projects-pp-cli sync' to refresh."
	}
	return report
}
