...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "zoho-projects-pp-cli",
		Short: `Zoho Projects CLI — Every Zoho Projects API endpoint, plus portal-wide insights (overdue, stale, workload, issue heat) no native view suppo…`,
		Long: `Zoho Projects CLI — Every Zoho Projects API endpoint, plus portal-wide insights (overdue, stale, workload, issue heat) no native view suppo…
...
Health check: run 'zoho-projects-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("zoho-projects-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'zoho-projects-pp-cli profile list')")
	...
	rootCmd.AddCommand(newPortalsPromotedCmd(flags))
	rootCmd.AddCommand(newTasksPromotedCmd(flags))
	rootCmd.AddCommand(newUsersPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	// Transcendence (novel) commands — local SQLite-backed cross-project insights.
	rootCmd.AddCommand(newOverdueCmd(flags))
	rootCmd.AddCommand(newStaleProjectsCmd(flags))
	rootCmd.AddCommand(newWorkloadCmd(flags))
	rootCmd.AddCommand(newGrepCmd(flags))
	rootCmd.AddCommand(newIssueHeatCmd(flags))
	rootCmd.AddCommand(newProjectBurnCmd(flags))
	rootCmd.AddCommand(newUnassignedCmd(flags))
	rootCmd.AddCommand(newMyFocusCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("zoho-projects-pp-cli %s\n", version)
		},
	}
}
