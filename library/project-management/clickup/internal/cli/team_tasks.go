...
// PATCH(amend-2026-05-25: add team-tasks read command wrapping GET /team/{team_id}/task)
...
Requires --assignee-id to scope the result to one person. Defaults to the
token's first workspace when --team-id is not given. Always includes subtasks
(--subtasks=true by default) and excludes closed tasks (--include-closed=false
by default).
...
Only live data is returned; --data-source local is not supported for this
command because the underlying query is team-scoped and not reproducible
from the flat local store.`,
  ...
  # Agent-friendly JSON (explicit)
  clickup-pp-cli team-tasks --assignee-id 12345678 --agent

  # Honour --data-source live explicitly
  clickup-pp-cli team-tasks --assignee-id 12345678 --data-source live
...
`, "\n"),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.dataSource == "local" {
				return usageErr(fmt.Errorf("team-tasks does not support --data-source local; use 'live' or omit the flag"))
			}
			...
			if resolvedTeamID == 0 {
				...
				if err != nil {
					return fmt.Errorf("resolving workspace: %w\nhint: set CLICKUP_AUTHORIZATION_TOKEN and run 'clickup-pp-cli doctor'", err)
				}
			...
			params := map[string]string{
				...
				"assignees[]":    strconv.Itoa(assigneeID),
			}

			// Use the live API directly. paginatedGet calls the real endpoint
			// with page-number pagination (v2 style: page=0,1,2,...).
			...
			for _, item := range envelope.Tasks {
	...
	cmd.Flags().IntVar(&assigneeID, "assignee-id", 0, "Filter tasks to this assignee's ClickUp user ID (required)")
	...
	_ = cmd.MarkFlagRequired("assignee-id") // PATCH(amend-2026-05-25: enforce --assignee-id as required per spec)
