...
// jsonString renders s as a JSON string literal (quotes included) for
// hand-assembled NDJSON event lines. Data-derived values interpolated with
// %s or %q break the stream: dependent-resource storage ids embed a NUL
// separator, which %s emits raw and %q escapes Go-style; json.Marshal
// emits the escape form JSON requires.
...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Set your API key with: export PLANE_API_KEY_AUTHENTICATION=\"your-token-here\""))
	}
	if len(trimmed) == 0 {
