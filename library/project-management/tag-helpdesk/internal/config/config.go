...
type Config struct {
	...
	Path              string            `toml:"-"`
	TagHelpdeskApiKey string            `toml:"helpdesk_api_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://tag.msg.it",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("TAG_HELPDESK_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "tag-helpdesk-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("TAG_HELPDESK_API_KEY"); v != "" {
		cfg.TagHelpdeskApiKey = v
		cfg.AuthSource = "env:TAG_HELPDESK_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.TagHelpdeskApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("TAG_HELPDESK_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.TagHelpdeskApiKey
	if token == "" {
		return ""
	}
	if c.TagHelpdeskApiKey == "" {
		return ""
	}
	return token
}
