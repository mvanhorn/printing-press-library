...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".tag-helpdesk-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TAG_HELPDESK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TAG_HELPDESK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "tag-helpdesk-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.tag-helpdesk-pp-cli/feedback.jsonl.
When ` + "`TAG_HELPDESK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TAG_HELPDESK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  tag-helpdesk-pp-cli feedback list
  tag-helpdesk-pp-cli feedback list --limit 5
  tag-helpdesk-pp-cli feedback list --json`,
