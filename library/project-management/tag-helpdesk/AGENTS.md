# Tag Helpdesk Printed CLI Agent Guide

This directory is a generated `tag-helpdesk-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
tag-helpdesk-pp-cli doctor --json
tag-helpdesk-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
tag-helpdesk-pp-cli which "<capability>" --json
tag-helpdesk-pp-cli <command> --help
```
...
```bash
tag-helpdesk-pp-cli <command> --agent
```
...
```bash
tag-helpdesk-pp-cli <command> --help
tag-helpdesk-pp-cli <command> --dry-run --agent
```
