...
This directory is a generated `clickup-2-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
clickup-2-pp-cli doctor --json
clickup-2-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
clickup-2-pp-cli which "<capability>" --json
clickup-2-pp-cli <command> --help
```
...
```bash
clickup-2-pp-cli <command> --agent
```
...
```bash
clickup-2-pp-cli <command> --help
clickup-2-pp-cli <command> --dry-run --agent
```
