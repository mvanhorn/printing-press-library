...
type Config struct {
	...
	Path            string            `toml:"-"`
	ClickupApiToken string            `toml:"api_token"`
	ClickupToken    string            `toml:"token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	if path == "" {
		...
		path = filepath.Join(home, ".config", "clickup-2-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides. CLICKUP_API_KEY is also accepted as a common alias.
	if v := os.Getenv("CLICKUP_API_TOKEN"); v != "" {
		cfg.ClickupApiToken = v
		cfg.AuthSource = "env:CLICKUP_API_TOKEN"
	}
	if v := os.Getenv("CLICKUP_TOKEN"); v != "" {
		cfg.ClickupToken = v
		cfg.AuthSource = "env:CLICKUP_TOKEN"
	}
	if v := os.Getenv("CLICKUP_API_KEY"); v != "" {
		cfg.ClickupApiToken = v
		cfg.AuthSource = "env:CLICKUP_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.ClickupApiToken != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.ClickupToken != "" {
		cfg.AuthSource = "config"
	}
...
func (c *Config) AuthHeader() string {
	...
	if c.ClickupApiToken != "" {
		return c.ClickupApiToken
	}
	if c.ClickupToken != "" {
		return c.ClickupToken
	}
	return ""
}
