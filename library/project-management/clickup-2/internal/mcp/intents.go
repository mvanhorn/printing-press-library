...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("find_the_stuck_work",
			mcplib.WithDescription("Per-status dwell time for a list, ranked worst-first, to spot bottlenecks."),
			mcplib.WithBoolean("rank", mcplib.Description("Override the recipe's --rank value.")),
		),
		handleFindTheStuckWork,
	)
}

// handleFindTheStuckWork runs the find_the_stuck_work recipe intent: Per-status dwell time for a list, ranked worst-first, to spot bottlenecks.
func handleFindTheStuckWork(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	args = appendRecipeBoolFlag(args, "rank", input["rank"], true)
