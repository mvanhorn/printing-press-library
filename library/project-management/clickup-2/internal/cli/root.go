...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "clickup-2-pp-cli",
		Short: `Clickup CLI - every ClickUp endpoint plus a local SQLite mirror with offline search and cross-task analytics no other ClickUp CLI ships.`,
		Long: `Clickup CLI - every ClickUp endpoint plus a local SQLite mirror with offline full-text search and cross-task analytics no other ClickUp CLI ships.
...
Health check: run 'clickup-2-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("clickup-2-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'clickup-2-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelChangedSinceCmd(flags))
	rootCmd.AddCommand(newNovelMyDayCmd(flags))
	rootCmd.AddCommand(newNovelResolveCmd(flags))
	rootCmd.AddCommand(newNovelStaleCmd(flags))
	rootCmd.AddCommand(newNovelTimeInStatusCmd(flags))
	rootCmd.AddCommand(newNovelUnblockedCmd(flags))
