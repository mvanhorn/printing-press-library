...
Resource scoping:
  ...
  clickup-2-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  clickup-2-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  clickup-2-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  clickup-2-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  clickup-2-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/clickup-2-pp-cli/data.db)")
...
func syncResourceFieldSelector(resource string) (string, string) {
	switch resource {
	case "team_task":
		return "order_by", "id,assignees.id,checklists.id,creator.id,custom_fields.id,custom_id,date_closed,date_created,date_done,date_updated,description,due_date,folder.id,list.id,locations.id,markdown_description,name,orderindex,parent,permission_level,points,priority,project.id,space.id,start_date,status.id,team_id,text_content,time_estimate,url,watchers.id"
	}
	return "", ""
}
...
// dependentResourceDefs chains the ClickUp hierarchy so `sync` populates the
// task store reliably. The generator emitted only team_task
// (GET /v2/team/{id}/task), which returns HTTP 500 (ECODE ITEMV2_003) on many
// workspaces. The reliable path is the list hierarchy:
//
//	team -> space -> folder -> list -> list tasks.
//
// Order matters: each child reads its parent table, populated by an earlier
// entry in this same slice (the walker runs them sequentially), so spaces must
// precede folders, folders+spaces must precede lists, and lists must precede
// tasks. Lists are stored under the single resource type "list" from both the
// folder and folderless paths so the task step iterates every list. Tasks are
// stored under "task", which is exactly what the local-store PM commands read.
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "space", ParentTable: "team", PathTemplate: "/v2/team/{team_id}/space", PathParams: []dependentPathParamDef{
			{Param: "team_id", Field: "id"},
		}},
		{Name: "folder", ParentTable: "space", PathTemplate: "/v2/space/{space_id}/folder", PathParams: []dependentPathParamDef{
			{Param: "space_id", Field: "id"},
		}},
		{Name: "list", ParentTable: "folder", PathTemplate: "/v2/folder/{folder_id}/list", PathParams: []dependentPathParamDef{
			{Param: "folder_id", Field: "id"},
		}},
		{Name: "list", ParentTable: "space", PathTemplate: "/v2/space/{space_id}/list", PathParams: []dependentPathParamDef{
			{Param: "space_id", Field: "id"},
		}},
		{Name: "task", ParentTable: "list", PathTemplate: "/v2/list/{list_id}/task", PathParams: []dependentPathParamDef{
