...
func newTaskGetBulkTimeinStatusCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Aliases:     []string{"list"},
		Short:       "View how long two or more tasks have been in each status.",
		Example:     "  clickup-2-pp-cli task get-bulk-timein-status --task-ids example-value",
	...
	cmd.Flags().StringVar(&flagTaskIds, "task-ids", "", "Include this paramater once per `task_id`. You can include up to 100 task ids per request.")
