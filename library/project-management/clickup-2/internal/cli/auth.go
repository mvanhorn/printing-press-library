...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  clickup-2-pp-cli auth setup\n  clickup-2-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://developer.clickup.com/docs/index")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export CLICKUP_API_TOKEN=\"<your-token>\"")
			fmt.Fprintln(w, "  export CLICKUP_TOKEN=\"<your-token>\"")
			fmt.Fprintln(w, "  clickup-2-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://developer.clickup.com/docs/index"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  clickup-2-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export CLICKUP_API_TOKEN=\"your-token-here\"")
				fmt.Fprintln(w, "  export CLICKUP_TOKEN=\"your-token-here\"")
				fmt.Fprintf(w, "  clickup-2-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  clickup-2-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  clickup-2-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("CLICKUP_API_TOKEN") != "" {
				envStillSet = "CLICKUP_API_TOKEN"
			}
			if envStillSet == "" && os.Getenv("CLICKUP_TOKEN") != "" {
				envStillSet = "CLICKUP_TOKEN"
			}
