...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Halo",
...
Mint an OAuth2 bearer token via POST https://{tenant}.{domain}/auth/token with
grant_type=client_credentials and cache it on disk. Subsequent commands
auto-refresh the token before expiry.

Credentials default to HALOPSA_CLIENT_ID (Client ID) and HALOPSA_CLIENT_SECRET (Client Secret) environment variables.
`),
		Example: strings.Trim(`
  # Use env vars
  halopsa-pp-cli auth login

  # Explicit credentials
  halopsa-pp-cli auth login --client-id <id> --client-secret <secret>
`, "\n"),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if clientID == "" {
				clientID = os.Getenv("HALOPSA_CLIENT_ID")
			}
			if clientSecret == "" {
				clientSecret = os.Getenv("HALOPSA_CLIENT_SECRET")
			}
			if clientID == "" || clientSecret == "" {
				return authErr(fmt.Errorf("client ID and secret required (set --client-id/--client-secret or HALOPSA_CLIENT_ID/HALOPSA_CLIENT_SECRET)"))
			}
	...
	cmd.Flags().StringVar(&clientID, "client-id", "", "OAuth2 Client ID; defaults to $HALOPSA_CLIENT_ID")
	cmd.Flags().StringVar(&clientSecret, "client-secret", "", "OAuth2 Client Secret; defaults to $HALOPSA_CLIENT_SECRET")
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  halopsa-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export HALOPSA_CLIENT_ID=\"<client-id>\"")
				fmt.Fprintln(w, "  export HALOPSA_CLIENT_SECRET=\"<client-secret>\"")
				fmt.Fprintf(w, "  halopsa-pp-cli auth login\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  halopsa-pp-cli auth set-token <bearer-jwt>",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  halopsa-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("HALOPSA_CLIENT_ID") != "" {
				envStillSet = "HALOPSA_CLIENT_ID"
			}
			if envStillSet == "" && os.Getenv("HALOPSA_CLIENT_SECRET") != "" {
				envStillSet = "HALOPSA_CLIENT_SECRET"
			}
			if envStillSet == "" && os.Getenv("HALOPSA_TENANT") != "" {
				envStillSet = "HALOPSA_TENANT"
			}
			if envStillSet == "" && os.Getenv("HALOPSA_DOMAIN") != "" {
				envStillSet = "HALOPSA_DOMAIN"
			}
			if envStillSet == "" && os.Getenv("HALOPSA_SCOPE") != "" {
				envStillSet = "HALOPSA_SCOPE"
			}
			if envStillSet == "" && os.Getenv("HALOPSA_TOKEN") != "" {
				envStillSet = "HALOPSA_TOKEN"
			}
