...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  halopsa-pp-cli doctor
  halopsa-pp-cli doctor --json
  halopsa-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export HALOPSA_CLIENT_ID=<your-key>"
					report["auth_key_url"] = "https://{tenant}.{domain}/auth/token"
				} else {
					authConfigured = true
			...
			if os.Getenv("HALOPSA_CLIENT_ID") != "" {
				authEnvSet = append(authEnvSet, "HALOPSA_CLIENT_ID")
			} else {
				authEnvInfo = append(authEnvInfo, "HALOPSA_CLIENT_ID set during auth login")
			}
			if os.Getenv("HALOPSA_CLIENT_SECRET") != "" {
				authEnvSet = append(authEnvSet, "HALOPSA_CLIENT_SECRET")
			} else {
				authEnvInfo = append(authEnvInfo, "HALOPSA_CLIENT_SECRET set during auth login")
			}
			if os.Getenv("HALOPSA_TENANT") != "" {
				authEnvSet = append(authEnvSet, "HALOPSA_TENANT")
			} else {
				authEnvInfo = append(authEnvInfo, "HALOPSA_TENANT set during auth login")
			}
			if os.Getenv("HALOPSA_DOMAIN") != "" {
				authEnvSet = append(authEnvSet, "HALOPSA_DOMAIN")
			} else {
				authEnvInfo = append(authEnvInfo, "HALOPSA_DOMAIN set during auth login")
			}
			if os.Getenv("HALOPSA_SCOPE") != "" {
				authEnvSet = append(authEnvSet, "HALOPSA_SCOPE")
			} else {
				authEnvInfo = append(authEnvInfo, "HALOPSA_SCOPE set during auth login")
			}
			if strings.Contains("Pre-issued Bearer access token. Overrides the client_credentials flow when set.", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "HALOPSA_TOKEN")
				if os.Getenv("HALOPSA_TOKEN") != "" {
					authEnvSet = append(authEnvSet, "HALOPSA_TOKEN")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("HALOPSA_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "HALOPSA_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "HALOPSA_TOKEN optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "halopsa-pp-cli"
						verifyPath := "/Users/me"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'halopsa-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'halopsa-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'halopsa-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'halopsa-pp-cli sync' to refresh."
	}
	return report
}
