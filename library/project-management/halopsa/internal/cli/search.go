...
In local mode: searches locally synced data only.`,
  ...
  halopsa-pp-cli search "error timeout"

  # Force local search only
  halopsa-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  halopsa-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  halopsa-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'halopsa-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "client":
				results, err = db.SearchClient(query, limit)
			case "field-info":
				results, err = db.SearchFieldInfo(query, limit)
			case "purchase-order":
				results, err = db.SearchPurchaseOrder(query, limit)
			case "quotation":
				results, err = db.SearchQuotation(query, limit)
			case "report":
				results, err = db.SearchReport(query, limit)
			case "sales-order":
				results, err = db.SearchSalesOrder(query, limit)
			case "service":
				results, err = db.SearchService(query, limit)
			case "site":
				results, err = db.SearchSite(query, limit)
			case "supplier":
				results, err = db.SearchSupplier(query, limit)
			case "users":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchClient(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search client failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchFieldInfo(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search field_info failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchPurchaseOrder(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search purchase_order failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchQuotation(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search quotation failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchReport(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search report failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchSalesOrder(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search sales_order failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchService(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search service failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchSite(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search site failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchSupplier(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search supplier failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
