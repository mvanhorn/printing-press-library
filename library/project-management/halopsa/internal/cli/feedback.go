...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".halopsa-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("HALOPSA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HALOPSA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "halopsa-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.halopsa-pp-cli/feedback.jsonl.
When ` + "`HALOPSA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`HALOPSA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  halopsa-pp-cli feedback list
  halopsa-pp-cli feedback list --limit 5
  halopsa-pp-cli feedback list --json`,
