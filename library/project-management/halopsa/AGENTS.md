...
This directory is a generated `halopsa-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
halopsa-pp-cli doctor --json
halopsa-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
halopsa-pp-cli which "<capability>" --json
halopsa-pp-cli <command> --help
```
...
```bash
halopsa-pp-cli <command> --help
halopsa-pp-cli <command> --dry-run --agent
```
