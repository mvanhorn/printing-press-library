...
func handleContext(_ context.Context, _ mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	ctx := map[string]any{
		...
		"query_tips": []string{
			...
			"offline_workout nests output under detail/history, and any offline command that returns a caveat wraps its result under result -- use dotted --select paths (e.g. --select detail.title, --select result.segments) to reach fields that aren't at the top level. offline_classes_filters double-nests under filters.filters (the wrapper's own \"filters\" key containing the provider's \"filters\" array) and filters.sorts -- there is no browse_categories/class_types field on this endpoint. Dotted --select paths only descend into object keys, not array indices (e.g. filters.filters.0.name does not work); arrays are matched element-wise instead.",
