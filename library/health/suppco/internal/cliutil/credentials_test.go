...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	return home, filepath.Join(home, ".config", "suppco-pp-cli", "config.toml")
}
...
func TestEnvironmentAccessTokenWinsOverSavedAuthHeader(t *testing.T) {
	resetCredentialEnv(t)
	t.Setenv("SUPPCO_ACCESS_TOKEN", "fresh-env-secret")
	if err := cliutil.SaveCredentials(&cliutil.Credentials{
		AuthHeaderVal: "Bearer stale-saved-secret",
	}); err != nil {
		t.Fatalf("SaveCredentials() error = %v", err)
	}
	...
	if got := cfg.AuthHeader(); got != "Bearer fresh-env-secret" {
		t.Fatalf("AuthHeader() = %q, want environment bearer token", got)
	}
	if cfg.AuthSource != "env:SUPPCO_ACCESS_TOKEN" {
		t.Fatalf("AuthSource = %q, want env:SUPPCO_ACCESS_TOKEN", cfg.AuthSource)
	}
}

func TestEnvironmentAccessTokenWinsOverSavedGenericAuthorizationHeader(t *testing.T) {
	_, configPath := resetCredentialEnv(t)
	if err := os.MkdirAll(filepath.Dir(configPath), 0o700); err != nil {
		t.Fatalf("mkdir config: %v", err)
	}
	configData := []byte("[headers]\naUtHoRiZaTiOn = \"Bearer stale-generic-secret\"\nX-Synthetic = \"preserved\"\n")
	...
	for name := range cfg.Headers {
		if strings.EqualFold(name, "Authorization") {
			t.Fatalf("saved generic Authorization header remained active")
		}
	}
	if got := cfg.Headers["X-Synthetic"]; got != "preserved" {
		t.Fatalf("unrelated header = %q, want preserved", got)
	}

	if err := cfg.SaveTokens("", "", "replacement-synthetic-secret", "", time.Time{}); err != nil {
	...
	if got := reloaded.Headers["aUtHoRiZaTiOn"]; got != "Bearer stale-generic-secret" {
		t.Fatalf("saved generic Authorization header = %q, want preserved file value", got)
	}
	if got := reloaded.Headers["X-Synthetic"]; got != "preserved" {
		t.Fatalf("saved unrelated header = %q, want preserved", got)
	}
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "suppco-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("SUPPCO_ACCESS_TOKEN", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "suppco-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "suppco-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "suppco-pp-cli", "credentials.toml")
