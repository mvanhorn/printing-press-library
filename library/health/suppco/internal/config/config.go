...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := strings.ToLower(strings.TrimSpace(os.Getenv("SUPPCO_SKIP_TLS_VERIFY"))); v == "1" || v == "true" || v == "yes" || v == "on" {
		cfg.SetSkipTLSVerify(true)
	}
	if v := os.Getenv("SUPPCO_ACCESS_TOKEN"); v != "" {
		// A direct environment token is the highest-precedence credential. Mask
		// any legacy saved auth_header or generic Authorization header for this
		// process without mutating the snapshotted file configuration used by
		// later saves.
		cfg.AuthHeaderVal = ""
		headersMasked := false
		...
		if headersMasked {
			cfg.markEnvOverride("Headers")
		}
		cfg.AccessToken = v
		cfg.markEnvOverride("AccessToken")
		cfg.AuthSource = "env:SUPPCO_ACCESS_TOKEN"
		cfg.CredentialSource = "env:SUPPCO_ACCESS_TOKEN"
	}
	...
	if v := os.Getenv("SUPPCO_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("SUPPCO_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "suppco-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.AccessToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:SUPPCO_ACCESS_TOKEN"
		}
		return applyAuthFormat("Bearer {access_token}", map[string]string{
			"access_token":        c.AccessToken,
			"SUPPCO_ACCESS_TOKEN": c.AccessToken,
			"token":               c.AccessToken,
		})
	}
	if c.AccessToken != "" {
		...
		return applyAuthFormat("Bearer {access_token}", map[string]string{"access_token": c.AccessToken, "token": c.AccessToken})
	}
	return ""
}
...
// PrepareForBearerTokenSave removes legacy Authorization credentials that
// would otherwise override a newly saved bearer token. Unrelated headers are
// preserved, including when an environment override caused Load to snapshot
// the original file configuration.
func (c *Config) PrepareForBearerTokenSave() {
	if c == nil {
		return
	}
	c.AuthHeaderVal = ""
	removeHeader(&c.Headers, "Authorization")
	if c.fileConfig != nil {
		c.fileConfig.AuthHeaderVal = ""
		removeHeader(&c.fileConfig.Headers, "Authorization")
	}
	delete(c.envOverrides, "AuthHeaderVal")
	delete(c.envOverrides, "Headers")
}

func removeHeader(headers *map[string]string, target string) {
	if headers == nil {
		return
	}
