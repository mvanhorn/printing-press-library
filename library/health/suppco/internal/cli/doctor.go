...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  suppco-pp-cli doctor
  suppco-pp-cli doctor --json
  suppco-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Provide SUPPCO_ACCESS_TOKEN from your shell or password manager, or pipe the token to: suppco-pp-cli auth set-token"
				} else {
					authConfigured = true
			...
			if os.Getenv("SUPPCO_ACCESS_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "SUPPCO_ACCESS_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "SUPPCO_ACCESS_TOKEN")
			}
			switch {
			...
			// Validate the provider client contract without making a probe that
			// either follows the API root's cross-origin redirect or reads a
			// private stack payload. Operators confirm token validity with one of
			// the explicit minimized read commands.
			if cfg != nil && cfg.BaseURL != "" {
				_, clientErr := flags.newClient()
				if clientErr != nil {
					report["api"] = fmt.Sprintf("client init error: %s", clientErr)
				} else {
					report["api"] = "configured (not probed; private read required for end-to-end validation)"
					if cfg.AuthHeader() != "" {
						suggestion := suggestReadCommand(cmd.Root())
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "suppco-pp-cli", suggestion)
						} else {
...
func collectPathsReport() map[string]any {
	...
	for _, resolution := range resolutions {
		if resolution.Kind == cliutil.PathKindCache {
			continue
		}
...
func renderPathsReport(w io.Writer, rep map[string]any) {
	fmt.Fprintf(w, "  Paths:\n")
	for _, kind := range []string{"config", "data", "state"} {
...
// status meets the --fail-on gate. "error" trips on failing sections and
// "warn" trips on deliberate WARN sections plus errors. The default empty
// string means never fail on status.
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("suppco-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'suppco-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'suppco-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'suppco-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'suppco-pp-cli sync' to refresh."
	}
	return report
}
