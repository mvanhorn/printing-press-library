...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "suppco-pp-cli",
		Short: "Read-only SuppCo stack facts with a deterministic, provenance-bearing regimen snapshot export.",
		Long: `Read-only SuppCo stack facts with a deterministic, provenance-bearing regimen snapshot export.
...
Run 'suppco-pp-cli doctor' to verify local configuration and credential presence.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("suppco-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.homePath, "home", "", "Root directory for config, data, and state files")
	rootCmd.PersistentFlags().DurationVar(&flags.timeout, "timeout", 60*time.Second, "Request timeout")
	rootCmd.PersistentFlags().BoolVar(&flags.dryRun, "dry-run", false, "Validate command without sending a request")
	...
	rootCmd.PersistentFlags().BoolVar(&flags.agent, "agent", false, "Set SuppCo agent-friendly defaults (--json --no-input --no-color --yes)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'suppco-pp-cli profile list')")
	rootCmd.PersistentFlags().StringVar(&flags.deliverSpec, "deliver", "", "Route output to a sink: stdout (default), file:<path>, webhook:<url>")
	rootCmd.PersistentFlags().Float64Var(&flags.rateLimit, "rate-limit", 2, "Max provider requests per second (0 to disable; default 2)")
	...
	rootCmd.AddCommand(newStackCmd(flags))
	...
	rootCmd.AddCommand(newSchedulePromotedCmd(flags))
