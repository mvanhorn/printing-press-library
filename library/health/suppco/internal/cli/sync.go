...
Resource scoping:
  ...
  suppco-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  suppco-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  suppco-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  suppco-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  suppco-pp-cli sync --latest-only`,
...
func responsePathForResource(resource, path string) []string {
	...
	case "stack\x00/api/users/me_compact/":
		return []string{"nutrients"}
	}
	return nil
}
