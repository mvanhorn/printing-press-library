...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SUPPCO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SUPPCO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "suppco-pp-cli/feedback")
...
When ` + "`SUPPCO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SUPPCO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  suppco-pp-cli feedback list
  suppco-pp-cli feedback list --limit 5
  suppco-pp-cli feedback list --json`,
