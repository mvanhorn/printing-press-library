...
Run sync first to populate the local search index.`,
  ...
  suppco-pp-cli search "status"

  # Force local search explicitly
  suppco-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  suppco-pp-cli search "status" --type schedule --data-source local
  # JSON output for piping
  suppco-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'suppco-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
