...
func TestMCPPathResolutionMatchesCLIResolverWithPlatformDefaults(t *testing.T) {
	...
	if want := filepath.Join(home, ".config", "suppco-pp-cli", "config.toml"); cfg.Path != want {
	...
	if want := filepath.Join(home, ".local", "share", "suppco-pp-cli", "data.db"); gotDB != want {
...
func TestMCPRegisterToolsExposesExactSuppCoSurface(t *testing.T) {
	s := server.NewMCPServer("suppco", "test")
	RegisterTools(s)

	tools := s.ListTools()
	want := []string{"context", "regimen_snapshot", "schedule_show", "stack_nutrients", "stack_products"}
...
func TestSuppCoMCPHandlersInvokeSharedProviderContract(t *testing.T) {
	compact, err := os.ReadFile(filepath.Join("..", "provider", "testdata", "me_compact.json"))
	if err != nil {
		t.Fatal(err)
	}
	schedule, err := os.ReadFile(filepath.Join("..", "provider", "testdata", "schedule.json"))
	if err != nil {
		t.Fatal(err)
	}
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if got := r.Header.Get("Authorization"); got != "Bearer mock-token-for-testing" {
			t.Errorf("Authorization = %q", got)
		}
		w.Header().Set("Content-Type", "application/json")
		switch r.URL.Path {
		case "/api/users/me_compact/":
			_, _ = w.Write(compact)
		case "/api/schedules/2026-07-19":
	...
	t.Setenv("PRINTING_PRESS_VERIFY", "1")
	t.Setenv("PRINTING_PRESS_VERIFY_LIVE_HTTP", "1")
	t.Setenv("SUPPCO_BASE_URL", server.URL)
	t.Setenv("SUPPCO_ACCESS_TOKEN", "mock-token-for-testing")
	dateRequest := mcplib.CallToolRequest{Params: mcplib.CallToolParams{Arguments: map[string]any{"date": "2026-07-19"}}}
	tests := []struct {
		name   string
		invoke func() (*mcplib.CallToolResult, error)
		assert func(*testing.T, map[string]any)
	}{
		{
			name: "stack_products",
			invoke: func() (*mcplib.CallToolResult, error) {
				return handleStackProducts(context.Background(), mcplib.CallToolRequest{})
			},
			assert: func(t *testing.T, value map[string]any) {
				if data, ok := value["data"].([]any); !ok || len(data) != 2 {
					t.Fatalf("products data = %#v", value["data"])
				}
			},
		},
		{
			name: "stack_nutrients",
			invoke: func() (*mcplib.CallToolResult, error) {
				return handleStackNutrients(context.Background(), mcplib.CallToolRequest{})
			},
			assert: func(t *testing.T, value map[string]any) {
				if data, ok := value["data"].([]any); !ok || len(data) != 2 {
					t.Fatalf("nutrients data = %#v", value["data"])
				}
			},
		},
		{
			name: "schedule_show",
			invoke: func() (*mcplib.CallToolResult, error) {
				return handleScheduleShow(context.Background(), dateRequest)
			},
			assert: func(t *testing.T, value map[string]any) {
				data, _ := value["data"].(map[string]any)
				if data["date"] != "2026-07-19" {
					t.Fatalf("schedule data = %#v", data)
				}
			},
		},
		{
			name: "regimen_snapshot",
			invoke: func() (*mcplib.CallToolResult, error) {
				return handleRegimenSnapshot(context.Background(), dateRequest)
			},
			assert: func(t *testing.T, value map[string]any) {
				if value["schema_version"] != "suppco.regimen_snapshot.v1" || value["effective_source"] != "provider_schedule" {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := json.Unmarshal([]byte(mcpTextContent(t, result)), &value); err != nil {
				t.Fatalf("handler result is not JSON: %v", err)
			}
			tc.assert(t, value)
		})
	}
...
func TestSuppCoMCPAuthFailureGivesSafeReplacementGuidance(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		http.Error(w, "private synthetic provider body", http.StatusUnauthorized)
	}))
	t.Cleanup(server.Close)
	resetMCPPathEnv(t)
	t.Setenv("PRINTING_PRESS_VERIFY", "1")
	...
	result, err := handleStackProducts(context.Background(), mcplib.CallToolRequest{})
	if err != nil {
		t.Fatalf("handler transport error: %v", err)
	}
	if result == nil || !result.IsError {
	...
	if !strings.Contains(text, "replace SUPPCO_ACCESS_TOKEN") || strings.Contains(text, "private synthetic provider body") {
		t.Fatalf("MCP auth error = %q", text)
	}
}
