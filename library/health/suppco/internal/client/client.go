...
// maxResponseBodyBytes bounds untrusted provider payloads before decoding.
// The minimized SuppCo reads are far smaller; the margin accommodates normal
// provider growth without allowing an unbounded allocation.
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if int64(len(respBody)) > maxResponseBodyBytes {
			return nil, resp.StatusCode, fmt.Errorf("provider response exceeds the %d-byte safety limit", maxResponseBodyBytes)
		}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "provide SUPPCO_ACCESS_TOKEN or pipe the token to: suppco-pp-cli auth set-token")
}
