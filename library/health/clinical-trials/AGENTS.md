# ClinicalTrials.gov Printed CLI Agent Guide

This directory is a generated `clinical-trials-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
clinical-trials-pp-cli doctor --json
clinical-trials-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
clinical-trials-pp-cli which "<capability>" --json
clinical-trials-pp-cli <command> --help
```
...
```bash
clinical-trials-pp-cli <command> --agent
```
...
```bash
clinical-trials-pp-cli <command> --help
clinical-trials-pp-cli <command> --dry-run --agent
```
