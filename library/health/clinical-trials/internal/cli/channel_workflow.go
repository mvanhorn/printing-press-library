...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  clinical-trials-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  clinical-trials-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"stats", "stats-field-values", "studies-enums", "studies-metadata", "studies-search-areas"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  clinical-trials-pp-cli workflow status

  # Show status as JSON
  clinical-trials-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("clinical-trials-pp-cli")
			}
