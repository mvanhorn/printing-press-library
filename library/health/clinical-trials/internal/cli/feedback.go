...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CLINICAL_TRIALS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CLINICAL_TRIALS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "clinical-trials-pp-cli/feedback")
...
When ` + "`CLINICAL_TRIALS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CLINICAL_TRIALS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  clinical-trials-pp-cli feedback list
  clinical-trials-pp-cli feedback list --limit 5
  clinical-trials-pp-cli feedback list --json`,
