# Cricapi Printed CLI Agent Guide

This directory is a generated `cricapi-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
cricapi-pp-cli doctor --json
cricapi-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
cricapi-pp-cli which "<capability>" --json
cricapi-pp-cli <command> --help
```
...
```bash
cricapi-pp-cli <command> --help
cricapi-pp-cli <command> --dry-run --agent
```
