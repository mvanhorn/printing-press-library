...
type Config struct {
	...
	Path           string `toml:"-"`
	CricapiApiKey string `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("CRICAPI_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "cricapi-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("CRICAPI_API_KEY"); v != "" {
		cfg.CricapiApiKey = v
		cfg.AuthSource = "env:CRICAPI_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.CricapiApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("CRICAPI_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
