...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".cricapi-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CRICAPI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CRICAPI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "cricapi-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.cricapi-pp-cli/feedback.jsonl.
When ` + "`CRICAPI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CRICAPI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cricapi-pp-cli feedback list
  cricapi-pp-cli feedback list --limit 5
  cricapi-pp-cli feedback list --json`,
