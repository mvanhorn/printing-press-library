...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SCRYFALL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SCRYFALL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "scryfall-pp-cli/feedback")
...
When ` + "`SCRYFALL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SCRYFALL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  scryfall-pp-cli feedback list
  scryfall-pp-cli feedback list --limit 5
  scryfall-pp-cli feedback list --json`,
