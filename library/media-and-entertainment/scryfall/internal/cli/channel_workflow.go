...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  scryfall-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  scryfall-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"bulk-data", "cards", "catalog", "catalog-artifact-types", "catalog-card-names", "catalog-creature-types", "catalog-enchantment-types", "catalog-land-types", "catalog-loyalties", "catalog-planeswalker-types", "catalog-spell-types", "catalog-toughnesses", "catalog-watermarks", "catalog-word-bank", "sets", "symbology"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  scryfall-pp-cli workflow status

  # Show status as JSON
  scryfall-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("scryfall-pp-cli")
			}
