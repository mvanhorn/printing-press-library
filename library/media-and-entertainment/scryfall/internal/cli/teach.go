...
// scryfall-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
const noLearnEnvVar = "SCRYFALL_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("scryfall-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  scryfall-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  scryfall-pp-cli recall "<question>" --agent
  scryfall-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  scryfall-pp-cli learnings list --agent
  scryfall-pp-cli learnings list --query "<substring>"
  scryfall-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  scryfall-pp-cli learnings forget "<question>" --resource <id>
  scryfall-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  scryfall-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  scryfall-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
