...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".spotify-web-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SPOTIFY_WEB_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SPOTIFY_WEB_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "spotify-web-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.spotify-web-pp-cli/feedback.jsonl.
When ` + "`SPOTIFY_WEB_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SPOTIFY_WEB_FEEDBACK_AUTO_SEND=true`" + `, the entry is
