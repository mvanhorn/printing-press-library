		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export SPOTIFY_WEB_TOKEN=<your-key>"
				} else {
			...
			authEnvSet := 0
			authEnvChecked++
			if os.Getenv("SPOTIFY_WEB_TOKEN") != "" {
				authEnvSet++
			}
			if authEnvSet == 0 {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if authHeader == "" {
					...
					authReq.Header.Set("User-Agent", "spotify-web-pp-cli")
...
func collectCacheReport(staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("spotify-web-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'spotify-web-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'spotify-web-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'spotify-web-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'spotify-web-pp-cli sync' to refresh."
	}
	return report
}
