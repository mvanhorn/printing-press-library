...
func positionalArgsForCommand(cmd *cobra.Command, blocked map[string]bool) []positionalArg {
	...
	for _, match := range positionalTokenPattern.FindAllStringSubmatch(cmd.Use, -1) {
		...
		name = schemaPropertyName(strings.TrimSpace(name))
		if name == "" {
			continue
		}
		inputName := name
...
// PATCH(amend-2026-08-19: re-apply fix-mcp-schema-property-names) — the reprint
// regenerated this file without the July sanitizer, reintroducing schema keys
// like "videoId|url" that Anthropic rejects (^[a-zA-Z0-9_.-]{1,64}$), which
// 400-fails the ENTIRE tool list of a session.

var invalidSchemaPropertyChars = regexp.MustCompile(`[^a-zA-Z0-9_.-]+`)

// schemaPropertyName converts human-facing Cobra positional labels into keys
// accepted by Anthropic's tool schema: ^[a-zA-Z0-9_.-]{1,64}$. A vertical bar
// means "or" in command help, while a leading @ is display syntax for handles.
func schemaPropertyName(name string) string {
	...
	name = invalidSchemaPropertyChars.ReplaceAllString(name, "_")
