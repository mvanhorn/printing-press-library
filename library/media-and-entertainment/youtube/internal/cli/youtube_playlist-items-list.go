...
func newYoutubePlaylistItemsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example:     "  youtube-pp-cli youtube playlist-items-list --part snippet --playlist-id UU_x5XG1OV2P6uZZ5FSM9Ttw --max-results 5",
		Annotations: map[string]string{"pp:happy-args": "--part=snippet;--playlist-id=UU_x5XG1OV2P6uZZ5FSM9Ttw;--max-results=5", "pp:endpoint": "youtube.playlist-items-list", "pp:method": "GET", "pp:path": "/youtube/v3/playlistItems", "mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			data, prov, err := resolvePaginatedReadWithStrategy(cmd.Context(), c, flags, "auto", "youtube", path, map[string]string{
				...
				"playlistId":             formatCLIParamValue(flagPlaylistId),
			...
			if flags.asJSON || (!isTerminal(cmd.OutOrStdout()) && !flags.csv && !flags.quiet && !flags.plain) {
				...
				if flags.selectFields != "" {
					...
					filtered = compactFields(filtered, map[string]bool{"contentDetails": true, "etag": true, "id": true, "kind": true, "snippet": true, "status": true})
				}
			...
			return printOutputWithFlagsMeta(cmd.OutOrStdout(), formatData, flags, map[string]any{"source": "live"}, map[string]bool{"contentDetails": true, "etag": true, "id": true, "kind": true, "snippet": true, "status": true})
		},
	}
