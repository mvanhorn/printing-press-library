...
alter command behavior or recall's verified…`,
		Example: `  youtube-pp-cli learnings candidates
  youtube-pp-cli learnings candidates --class flag_alias
  youtube-pp-cli learnings candidates --status rejected`,
...
func newLearningsConfirmCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  youtube-pp-cli learnings confirm 3`,
...
candidate, and recall never resurfaces it.`,
		Example: `  youtube-pp-cli learnings reject 3`,
		// Usage errors here exit 2 by design; declare it so
		// `cli-printing-press verify` scores the Execute cell honestly.
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if errors.Is(err, store.ErrConfirmedFlagAliasReject) {
				return usageErr(fmt.Errorf("learnings reject: candidate %d is a confirmed flag correction with no reject path; repair the guidance with \"youtube-pp-cli playbook amend\" instead", id))
			}
			if err != nil {
				return usageErr(fmt.Errorf("learnings reject: %w", err))
			}
...
--tombstones to remove them too, which re-allows…`,
		Example: `  youtube-pp-cli learnings purge
  youtube-pp-cli learnings purge --tombstones`,
