...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
  ...
  youtube-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Check auth — YOUTUBE_API_KEY (read) is the primary credential;
			// OAuth (YOUTUBE_DATA_OAUTH2C) is only required for write/private
			// commands, so it's optional for doctor's purposes.
			if cfg != nil {
				header := cfg.AuthHeader()
				switch {
				case cfg.APIKey != "" && header != "":
					report["auth"] = "configured (api key + oauth)"
					report["auth_source"] = cfg.APIKeySource + "+" + cfg.AuthSource
				case cfg.APIKey != "":
					report["auth"] = "configured (api key, read-only)"
					report["auth_source"] = cfg.APIKeySource
				case header != "":
					report["auth"] = "configured (oauth)"
					report["auth_source"] = cfg.AuthSource
				default:
					report["auth"] = "not configured"
					report["auth_hint"] = "export YOUTUBE_API_KEY=<your-key> for read access; run 'youtube-pp-cli auth login' for write access"
				}
			}

			// Check auth environment variables — at least one of YOUTUBE_API_KEY
			// or YOUTUBE_DATA_OAUTH2C must be set; we treat them as an OR group.
			authEnvSet := []string{}
			authEnvOptionalNames := []string{"YOUTUBE_API_KEY", "YOUTUBE_DATA_OAUTH2C"}
			...
			default:
				report["env_vars"] = fmt.Sprintf("OK %d/2 available (%s)", len(authEnvSet), strings.Join(authEnvSet, ", "))
			}
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "youtube-pp-cli"
