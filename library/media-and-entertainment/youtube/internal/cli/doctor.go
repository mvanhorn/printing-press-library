...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
  ...
  youtube-pp-cli doctor --fail-on stale`,
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "Set your API key with: export YOUTUBE_API_KEY=\"your-token-here\""
						report["auth_key_url"] = "https://console.cloud.google.com"
					} else {
						authConfigured = true
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "youtube-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	case !haveAny && len(resources) == 0:
		...
		report["hint"] = "No sync recorded; run 'youtube-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
