...
large datasets as it has no memory pressure.`,
  ...
  youtube-pp-cli export <resource> --format jsonl --output data.jsonl

  # Export with limit
  youtube-pp-cli export <resource> --format jsonl --limit 1000

  # Pipe to another tool
  youtube-pp-cli export <resource> --format jsonl | jq '.id'`,
		...
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			// PATCH(amend-2026-08-19: F2 honest refusal) — the only bulk read path this
			// API offers (/youtube/v3/activities) demands a channelId/mine/home filter,
			// so a filterless export walk can only die with a live HTTP 400. Refuse
			// with the working alternative instead of surfacing the raw API error.
			if !singleItem {
				return usageErr(fmt.Errorf("the YouTube Data API has no filterless bulk-list endpoint, so 'export youtube' cannot stream the whole resource; populate the local databank with 'backfill' or 'monitor', then query it with the MCP sql/search tools or read the SQLite file shown by 'doctor'"))
			}
			if len(args) > 1 {
