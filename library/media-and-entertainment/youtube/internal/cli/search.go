...
or hit the API's search endpoint when available.`,
  ...
  youtube-pp-cli search "error timeout"

  # Force local search only
  youtube-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  youtube-pp-cli search "status" --type youtube --data-source local
  # JSON output for piping
  youtube-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.dataSource != "local" {
				...
				data, getErr := c.Get(cmd.Context(), "/youtube/v3/search", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'youtube-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "":
				...
				{
					partial, searchErr := db.SearchYoutube(query, limit)
					if searchErr != nil {
			...
			default:
				...
				results, err = db.SearchYoutube(query, limit, resourceType)
			}
			if err != nil {
