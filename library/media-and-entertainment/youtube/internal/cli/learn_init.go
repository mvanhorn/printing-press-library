...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^[A-Za-z0-9_-]{11}$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: youtube-pp-cli: ticker pattern %q failed to compile: %v\n", `^[A-Za-z0-9_-]{11}$`, err)
	} else {
		cfg.RegisterTickerPattern(re)
	}
	if re, err := regexp.Compile(`^UC[A-Za-z0-9_-]{22}$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: youtube-pp-cli: ticker pattern %q failed to compile: %v\n", `^UC[A-Za-z0-9_-]{22}$`, err)
	} else {
		cfg.RegisterTickerPattern(re)
	}
	if re, err := regexp.Compile(`^(PL|UU)[A-Za-z0-9_-]{16,}$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: youtube-pp-cli: ticker pattern %q failed to compile: %v\n", `^(PL|UU)[A-Za-z0-9_-]{16,}$`, err)
	} else {
		cfg.RegisterTickerPattern(re)
	}
	if re, err := regexp.Compile(`^@[A-Za-z0-9._-]{3,30}$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: youtube-pp-cli: ticker pattern %q failed to compile: %v\n", `^@[A-Za-z0-9._-]{3,30}$`, err)
	} else {
		cfg.RegisterTickerPattern(re)
	}
	...
	learnSynonyms := map[string]string{
		"creator uploads": "channel uploads",
		"reference clip":  "reference video",
	}
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"resource_type": {
			{Canonical: "video", Values: []string{"clip", "clips", "footage", "videos"}},
			{Canonical: "channel", Values: []string{"creator", "creators", "uploader", "uploaders"}},
			{Canonical: "playlist", Values: []string{"collection", "collections", "playlists", "series"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: youtube-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: youtube-pp-cli: learn init: %v\n", err)
		}
	})
}
