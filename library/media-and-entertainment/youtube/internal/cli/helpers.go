...
func classifyAPIErrorOnly(err error) error {
	...
	case strings.Contains(msg, "HTTP 409"):
		return apiErr(fmt.Errorf("%w\nhint: conflict — the resource already exists or clashes with current remote state", err))
...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Set your API key with: export YOUTUBE_API_KEY=\"your-token-here\""))
	}
	if len(trimmed) == 0 {
