...
var whichIndex = []whichEntry{
	{Command: "watch", Description: "Register the competitor channels your monitoring machine tracks, in a typed watchlist table you own.", Group: "Competitor monitoring machine", WhyItMatters: "Defines the tracked market once; every later monitoring command runs against it without re-specifying channels."},
	{Command: "monitor", Description: "Refresh every watched channel in one run: stats snapshot, new uploads, re-snapshot of recent video statistics.", Group: "Competitor monitoring machine", WhyItMatters: "One command keeps the databank current, so market answers are hours old instead of weeks old."},
	{Command: "velocity", Description: "See which tracked videos are gaining views fastest right now, computed from real between-snapshot deltas.", Group: "Competitor monitoring machine", WhyItMatters: "Current market movement - what is taking off today, not what took off two weeks ago."},
	{Command: "growth", Description: "Channel-level subscriber, view, and upload-count deltas between dated local snapshots.", Group: "Competitor monitoring machine", WhyItMatters: "Tells an agent whether a competitor is accelerating without any external history service."},
	{Command: "breakouts", Description: "Chain search filters into a matrix (terms x upload window x duration x region), join results to channel size, and rank fresh high-momentum videos.", Group: "Fresh market discovery", WhyItMatters: "Finds niche breakouts days after upload, weeks before they reach lagging analytics platforms."},
	{Command: "comments-mine", Description: "Sync comments into a typed full-text-searchable table and report top-liked comments, keyword frequencies, and audience questions.", Group: "Fresh market discovery", WhyItMatters: "Fast audience signal from data you own - what viewers praise, ask, and complain about across a channel."},
	{Command: "backfill", Description: "Pull a channel's complete upload history with statistics into the local databank in one command.", Group: "Competitor monitoring machine", WhyItMatters: "Run once per competitor; every later question about that channel is answered offline for free."},
	{Command: "packaging", Description: "Collect titles, thumbnails (downloaded as local image files), and hook text from transcript openings into a packaging table.", Group: "Fresh market discovery", WhyItMatters: "Hands a multimodal agent everything it needs for thumbnail and hook analysis without any scraping or manual collection."},
	{Command: "workspace", Description: "Named databanks: keep the competitor machine in one database and explore a new niche in another, switching instantly.", Group: "Competitor monitoring machine", WhyItMatters: "Lets an agent spin up a clean research sandbox per niche without risking the production watchlist databank."},
	{Command: "auth keys", Description: "Store multiple named YouTube API keys, switch between them instantly, and optionally fail over automatically via --rotate when one runs out of quota.", Group: "Competitor monitoring machine", WhyItMatters: "An agent can finish large collection jobs without human intervention when the first key's daily quota is spent."},
}
