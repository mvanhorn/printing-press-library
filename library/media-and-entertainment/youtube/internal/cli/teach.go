...
// youtube-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/youtube/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/youtube/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/youtube/internal/learn/patterns"
...
const noLearnEnvVar = "YOUTUBE_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("youtube-pp-cli")
}
...
query that matches surfaces the right resources without re-running discovery.` + noLearnEnvVar + `=true.`,
		Example: `  youtube-pp-cli teach --query "<question>" --resource-type <type> --resource <id> --resource <id> &`,
...
found=true AND results[0].entity_match == "exact" AND…` + noLearnEnvVar + `=true returns the empty shape even
when learnings exist.`,
		Example: `  youtube-pp-cli recall "<question>" --agent
  youtube-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List recorded query-to-resource learnings stored in the local databank",
		Example: `  youtube-pp-cli learnings list --agent
  youtube-pp-cli learnings list --query "<substring>"
  youtube-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  youtube-pp-cli learnings forget "<question>" --resource <id>
  youtube-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole…`,
		Example: `  youtube-pp-cli teach-pattern --query-template "items in {entity}" --resource-template "GROUP-{entity:category}" --resource-type "items" --entity-kind "category" --strategy substitute`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
cannot be taught — they are derived from the…`,
		Example: `  youtube-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
