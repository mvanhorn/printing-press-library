...
func Load(configPath string) (*Config, error) {
	...
	if v := os.Getenv("YOUTUBE_API_KEY"); v != "" {
		cfg.YoutubeApiKey = v
		cfg.markEnvOverride("YoutubeApiKey")
		cfg.AuthSource = "env:YOUTUBE_API_KEY"
		cfg.CredentialSource = "env:YOUTUBE_API_KEY"
	}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("YOUTUBE_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "youtube-pp-cli", "config.toml"), nil
}
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.YoutubeApiKey != "" {
		parts = append(parts, "api_key="+c.YoutubeApiKey)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.YoutubeApiKey == "" {
		c.YoutubeApiKey = creds.YoutubeApiKey
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	delete(c.envOverrides, "YoutubeApiKey")
	c.updateFileConfigField("YoutubeApiKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["YoutubeApiKey"] {
			out.YoutubeApiKey = c.fileConfig.YoutubeApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "YoutubeApiKey":
		c.fileConfig.YoutubeApiKey = c.YoutubeApiKey
	}
}
