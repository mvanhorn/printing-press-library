...
This CLI never mutates remote YouTube resources (read-only, api-key). Before running an unfamiliar command that writes to the local databank, inspect its help and prefer a dry run:
...
Disable the loop with `--no-learn` per-invocation or `YOUTUBE_NO_LEARN=true` for the whole session - useful for deterministic agent flows that don't want a learning row to silently change subsequent query results.
