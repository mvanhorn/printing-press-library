...
type Config struct {
	...
	Path           string            `toml:"-"`
	MagnificApiKey string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("MAGNIFIC_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "magnific-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides. Magnific (formerly Freepik) accepts the same API key
	// on both api.magnific.com and api.freepik.com. The official Freepik MCP
	// server and the public docs use FREEPIK_API_KEY; the CLI slug derives
	// MAGNIFIC_API_KEY. Accept either; MAGNIFIC_API_KEY wins when both are set.
	if v := os.Getenv("MAGNIFIC_API_KEY"); v != "" {
		cfg.MagnificApiKey = v
		cfg.AuthSource = "env:MAGNIFIC_API_KEY"
	} else if v := os.Getenv("FREEPIK_API_KEY"); v != "" {
		cfg.MagnificApiKey = v
		cfg.AuthSource = "env:FREEPIK_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.MagnificApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("MAGNIFIC_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
