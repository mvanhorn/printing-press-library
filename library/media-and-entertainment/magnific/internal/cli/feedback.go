...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".magnific-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MAGNIFIC_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MAGNIFIC_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "magnific-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.magnific-pp-cli/feedback.jsonl.
When ` + "`MAGNIFIC_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MAGNIFIC_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  magnific-pp-cli feedback list
  magnific-pp-cli feedback list --limit 5
  magnific-pp-cli feedback list --json`,
