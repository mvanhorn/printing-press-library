...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  magnific-pp-cli doctor
  magnific-pp-cli doctor --json
  magnific-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export MAGNIFIC_API_KEY=<your-key>"
					report["auth_key_url"] = "https://docs.magnific.com/quickstart"
				} else {
					authConfigured = true
			...
			// Magnific accepts MAGNIFIC_API_KEY or FREEPIK_API_KEY (same key value works
			// against both api.magnific.com and api.freepik.com hostnames).
			if v := os.Getenv("MAGNIFIC_API_KEY"); v != "" {
				authEnvSet = append(authEnvSet, "MAGNIFIC_API_KEY")
			} else if v := os.Getenv("FREEPIK_API_KEY"); v != "" {
				authEnvSet = append(authEnvSet, "FREEPIK_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "MAGNIFIC_API_KEY or FREEPIK_API_KEY")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'magnific-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'magnific-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'magnific-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'magnific-pp-cli sync' to refresh."
	}
	return report
}
