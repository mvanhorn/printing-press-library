...
// TestUpsertBatch_PopulatesGroupItemsTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed group_items table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesGroupItemsTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("group-items", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "group-items").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "group_items")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count group_items: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("group_items count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
...
// TestUpsertBatch_PopulatesYoutubeReportingJobsTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed youtube_reporting_jobs table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesYoutubeReportingJobsTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("youtube-reporting-jobs", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "youtube-reporting-jobs").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "youtube_reporting_jobs")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count youtube_reporting_jobs: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("youtube_reporting_jobs count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_PopulatesYoutubeReportingJobsReportsTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed youtube_reporting_jobs_reports table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesYoutubeReportingJobsReportsTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "youtube_reporting_jobs_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "youtube_reporting_jobs_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "youtube_reporting_jobs_id": "test-parent-001"}`),
	}
	if _, _, err := s.UpsertBatch("youtube_reporting_jobs_reports", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "youtube_reporting_jobs_reports").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "youtube_reporting_jobs_reports")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count youtube_reporting_jobs_reports: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("youtube_reporting_jobs_reports count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandYoutubeReportingJobsReportsGeneric exercises
...
func TestUpsertBatch_TypedFailureDoesNotStrandYoutubeReportingJobsReportsGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// Items deliberately omit "youtube_reporting_jobs_id" so the typed NOT NULL
	// constraint fires.
	items := []json.RawMessage{
	...
	stored, extractFailures, err := s.UpsertBatch("youtube_reporting_jobs_reports", items)
	if err != nil {
	...
	if typed != 0 {
		t.Fatalf("youtube_reporting_jobs_reports count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "youtube_reporting_jobs_id")
	}
}

// TestUpsertBatch_SetsYoutubeReportingJobsReportsParentID verifies that dependent-resource
...
func TestUpsertBatch_SetsYoutubeReportingJobsReportsParentID(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	var matchedA int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE parent_id = ?`, "youtube_reporting_jobs_reports")
