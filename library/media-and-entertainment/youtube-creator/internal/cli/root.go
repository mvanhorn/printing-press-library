...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "youtube-creator-pp-cli",
		Short: `YouTube CLI for the Data, Analytics, and Reporting APIs with offline sync, held-comment moderation, quota tracking, and n8n recipes`,
		Long: `YouTube CLI — Every YouTube Data, Analytics, and Reporting API endpoint, plus offline sync, the moderation verbs Studio hides, and a curated set of channel-management shortcuts.
...
Health check: run 'youtube-creator-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("youtube-creator-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'youtube-creator-pp-cli profile list')")
	...
	rootCmd.AddCommand(newGroupItemsCmd(flags))
	rootCmd.AddCommand(newGroupsCmd(flags))
	rootCmd.AddCommand(newYoutubeCmd(flags))
	rootCmd.AddCommand(newYoutubeReportingJobsCmd(flags))
	...
	rootCmd.AddCommand(newReportTypesPromotedCmd(flags))
	rootCmd.AddCommand(newReportsPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	// Phase 3 novel-feature commands (hand-authored)
	rootCmd.AddCommand(newQuotaCmd(flags))
	rootCmd.AddCommand(newPubsubCmd(flags))
	rootCmd.AddCommand(newModCmd(flags))
	rootCmd.AddCommand(newDigestCmd(flags))
	rootCmd.AddCommand(newBulkCmd(flags))
	rootCmd.AddCommand(newPlaylistHygieneCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("youtube-creator-pp-cli %s\n", version)
		},
	}
}
