...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  youtube-creator-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  youtube-creator-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"group-items", "groups", "report-types", "reports", "youtube-reporting-jobs"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  youtube-creator-pp-cli workflow status

  # Show status as JSON
  youtube-creator-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("youtube-creator-pp-cli")
			}
