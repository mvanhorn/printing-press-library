...
Resource scoping:
  ...
  youtube-creator-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  youtube-creator-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  youtube-creator-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  youtube-creator-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  youtube-creator-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("youtube-creator-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/youtube-creator-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "group-items":
		return db.UpsertGroupItems(data)
	case "groups":
	...
	case "youtube-reporting-jobs":
		return db.UpsertYoutubeReportingJobs(data)
	case "youtube_reporting_jobs_reports":
		return db.UpsertYoutubeReportingJobsReports(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"group-items":            "/v2/groupItems",
		"groups":                 "/v2/groups",
		"report-types":           "/v1/reportTypes",
		"reports":                "/v2/reports",
		"youtube-reporting-jobs": "/v1/jobs",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "youtube_reporting_jobs_reports", ParentTable: "youtube-reporting-jobs", ParentIDParam: "jobId", PathTemplate: "/v1/jobs/{jobId}/reports", KeyField: ""},
	}
}
