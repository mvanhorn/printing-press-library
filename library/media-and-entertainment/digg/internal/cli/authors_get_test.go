...
func TestAuthorsGet_InThousandExactMatch(t *testing.T) {
	...
	if env.Result.FollowersGap != nil {
		t.Errorf("followers_gap should be omitted on in-1000 path; got %d", *env.Result.FollowersGap)
	}
...
// populated; followers_gap must be a signed integer
// (anchor_followers - this.followers).
func TestAuthorsGet_OffThousandWithWarmCache(t *testing.T) {
	seed := withTempHome(t)
	seedRank1000(t, seed)

	srv := startMockUsersServer(t, cannedUsersMVH())
	out, _, err := runAuthorsGet(t, srv.URL, "mvanhorn")
	if err != nil {
	...
	if env.Result.NearestIn1000.FollowersCount != 9000 {
		t.Errorf("nearest_in_1000.followers_count = %d, want 9000", env.Result.NearestIn1000.FollowersCount)
	}
	if env.Result.FollowersGap == nil {
		t.Fatal("followers_gap should be set when anchor resolves")
	}
	// 9000 (anchor) - 21911 (mvanhorn) = -12911 (negative is meaningful).
	wantGap := 9000 - 21911
	if *env.Result.FollowersGap != wantGap {
		t.Errorf("followers_gap = %d, want %d (signed; anchor-followers - mvh-followers)",
			*env.Result.FollowersGap, wantGap)
	}
	if env.Meta["tier_status_resolved"] != true {
		t.Errorf("meta.tier_status_resolved should be true with warm cache; got %v", env.Meta["tier_status_resolved"])
	}
}
...
// complete payload with the anchor.
func TestAuthorsGet_CacheColdLiveFallback(t *testing.T) {
	...
	// rank-1000 row plus one rank-1 row so the parser sees a real
	// roster.
	rsc := `[{"rank":1,"target_x_id":"1","username":"sama","display_name":"sama","followers_count":1000,"score":99.9,"previousRank":null,"rankChange":null,"followed_by_count":0},{"rank":1000,"target_x_id":"1000","username":"livefallback","display_name":"Live Fallback","followers_count":7777,"score":1.0,"previousRank":null,"rankChange":null,"followed_by_count":0}]`
	...
	if env.Result.FollowersGap == nil {
		t.Fatal("followers_gap should be set after live fallback populates anchor")
	}
	if env.Meta["tier_status_resolved"] != true {
...
// nearest_in_1000 / followers_gap, and set
...
func TestAuthorsGet_OffThousandFetchFailureGraceful(t *testing.T) {
	...
	if env.Result.FollowersGap != nil {
		t.Errorf("followers_gap should be omitted on anchor failure; got %d", *env.Result.FollowersGap)
	}
