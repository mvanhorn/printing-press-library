# YouTube Studio (Kami creator analytics) Printed CLI Agent Guide

This directory is a generated `yt-studio-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
yt-studio-pp-cli doctor --json
yt-studio-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
yt-studio-pp-cli which "<capability>" --json
yt-studio-pp-cli <command> --help
```
...
```bash
yt-studio-pp-cli <command> --agent
```
...
```bash
yt-studio-pp-cli <command> --help
yt-studio-pp-cli <command> --dry-run --agent
```
