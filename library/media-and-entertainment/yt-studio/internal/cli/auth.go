...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Yt Studio",
	}
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  yt-studio-pp-cli auth setup\n  yt-studio-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Register an app and copy your credentials at: https://console.cloud.google.com/apis/credentials")
			fmt.Fprintln(w, "  Create an OAuth 2.0 Desktop Client in Google Cloud Console, then run `yt-studio-pp-cli auth login`. Enable both the YouTube Data API v3 and YouTube Analytics API on the project first.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then run:")
			fmt.Fprintln(w, "  yt-studio-pp-cli auth login --client-id <id> --client-secret <secret>")
			if !launch {
				return nil
			}
...
func newAuthLoginCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.Flags().StringVar(&clientID, "client-id", os.Getenv("YT_STUDIO_CLIENT_ID"), "OAuth2 client ID")
	cmd.Flags().StringVar(&clientSecret, "client-secret", os.Getenv("YT_STUDIO_CLIENT_SECRET"), "OAuth2 client secret")
