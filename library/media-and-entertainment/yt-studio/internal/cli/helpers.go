...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'yt-studio-pp-cli doctor' to check auth status."+
	...
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: your token may have expired. Re-run 'yt-studio-pp-cli auth login' to re-authenticate"+
			"\n      Get a key at: https://console.cloud.google.com/apis/credentials"+
			"\n      Create an OAuth 2.0 Desktop Client in Google Cloud Console, then run `yt-studio-pp-cli auth login`. Enable both the YouTube Data API v3 and YouTube Analytics API on the project first."+
			"\n      Run 'yt-studio-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: permission denied. Your token may lack required scopes. Re-run 'yt-studio-pp-cli auth login' to re-authorize"+
			"\n      Get a key at: https://console.cloud.google.com/apis/credentials"+
			"\n      Create an OAuth 2.0 Desktop Client in Google Cloud Console, then run `yt-studio-pp-cli auth login`. Enable both the YouTube Data API v3 and YouTube Analytics API on the project first."+
			"\n      Run 'yt-studio-pp-cli doctor' to check auth status.", err))
...
func printJSONFiltered(w io.Writer, v any, flags *rootFlags) error {
	// SetEscapeHTML(false) preserves literal <, >, & in prose fields so raw
	// stdout is readable (encoding/json's default emits </>/&,
	// which JSON parsers decode correctly but humans should not have to).
	...
	// json.Encoder appends a trailing newline; trim it so downstream
	// printOutputWithFlags receives the same shape json.Marshal produced.
