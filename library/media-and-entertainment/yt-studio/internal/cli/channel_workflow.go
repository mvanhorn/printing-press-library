...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  yt-studio-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  yt-studio-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"categories", "channels", "discover", "playlists"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  yt-studio-pp-cli workflow status

  # Show status as JSON
  yt-studio-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("yt-studio-pp-cli")
			}
