...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  yt-studio-pp-cli doctor
  yt-studio-pp-cli doctor --json
  yt-studio-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export YT_STUDIO_OAUTH_CLIENT_ID=<your-key>"
					report["auth_key_url"] = "https://console.cloud.google.com/apis/credentials"
					report["auth_instructions"] = "Create an OAuth 2.0 Desktop Client in Google Cloud Console, then run `yt-studio-pp-cli auth login`. Enable both the YouTube Data API v3 and YouTube Analytics API on the project first."
				} else {
					authConfigured = true
			...
			if os.Getenv("YT_STUDIO_OAUTH_CLIENT_ID") != "" {
				authEnvSet = append(authEnvSet, "YT_STUDIO_OAUTH_CLIENT_ID")
			} else {
				authEnvInfo = append(authEnvInfo, "YT_STUDIO_OAUTH_CLIENT_ID set during auth login")
			}
			if os.Getenv("YT_STUDIO_OAUTH_CLIENT_SECRET") != "" {
				authEnvSet = append(authEnvSet, "YT_STUDIO_OAUTH_CLIENT_SECRET")
			} else {
				authEnvInfo = append(authEnvInfo, "YT_STUDIO_OAUTH_CLIENT_SECRET set during auth login")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						verifyPath := "/youtube/v3/channels?part=id&mine=true"
						...
						authHeaders["User-Agent"] = "yt-studio-pp-cli"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'yt-studio-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'yt-studio-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'yt-studio-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'yt-studio-pp-cli sync' to refresh."
	}
	return report
}
