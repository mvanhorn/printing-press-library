...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".yt-studio-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("YT_STUDIO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("YT_STUDIO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "yt-studio-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.yt-studio-pp-cli/feedback.jsonl.
When ` + "`YT_STUDIO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`YT_STUDIO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  yt-studio-pp-cli feedback list
  yt-studio-pp-cli feedback list --limit 5
  yt-studio-pp-cli feedback list --json`,
