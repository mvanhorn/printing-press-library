...
func TestUpsertBatch_PopulatesHnTable(t *testing.T) {
	...
	if _, _, err := s.UpsertBatch("hn", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "hn").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "hn")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count hn: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("hn count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
