...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("VIBE_SIGNAL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("VIBE_SIGNAL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "vibe-signal-pp-cli/feedback")
...
When ` + "`VIBE_SIGNAL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`VIBE_SIGNAL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  vibe-signal-pp-cli feedback list
  vibe-signal-pp-cli feedback list --limit 5
  vibe-signal-pp-cli feedback list --json`,
