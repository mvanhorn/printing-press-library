...
Resource scoping:
  ...
  vibe-signal-pp-cli sync

  # Sync specific resources only
  vibe-signal-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  vibe-signal-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  vibe-signal-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  vibe-signal-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  vibe-signal-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "vibe-signal-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"hn":                "/search",
		"hn-search-by-date": "/search_by_date",
	}
