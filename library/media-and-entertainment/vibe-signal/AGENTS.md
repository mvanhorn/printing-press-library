# Vibe Signal Printed CLI Agent Guide

This directory is a generated `vibe-signal-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
vibe-signal-pp-cli doctor --json
vibe-signal-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
vibe-signal-pp-cli which "<capability>" --json
vibe-signal-pp-cli <command> --help
```
...
```bash
vibe-signal-pp-cli <command> --agent
```
...
```bash
vibe-signal-pp-cli <command> --help
vibe-signal-pp-cli <command> --dry-run --agent
```
