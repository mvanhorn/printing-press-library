...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  discogs-pp-cli doctor
  discogs-pp-cli doctor --json
  discogs-pp-cli doctor --fail-on warn
  discogs-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set your API key with: export DISCOGS_TOKEN=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if os.Getenv("DISCOGS_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "DISCOGS_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "DISCOGS_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "discogs-pp-cli", suggestion)
						} else {
