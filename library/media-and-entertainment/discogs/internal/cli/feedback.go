...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DISCOGS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DISCOGS_FEEDBACK_AUTO_SEND")))
...
func appendFeedback(entry FeedbackEntry) error {
	...
	f, err := os.OpenFile(p, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0o600) // #nosec G304 -- path is a config-resolved local feedback file; not attacker-controlled
	if err != nil {
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "discogs-pp-cli/feedback")
...
When ` + "`DISCOGS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DISCOGS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  discogs-pp-cli feedback list
  discogs-pp-cli feedback list --limit 5
  discogs-pp-cli feedback list --json`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			p, err := feedbackFilePath()
			if err != nil {
				return err
			}
			data, err := os.ReadFile(p) // #nosec G304 -- path is a config-resolved local feedback file; not attacker-controlled
			if err != nil {
