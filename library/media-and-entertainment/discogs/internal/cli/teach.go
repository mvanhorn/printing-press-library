...
// discogs-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/discogs/internal/learn/patterns"
...
const noLearnEnvVar = "DISCOGS_NO_LEARN"
...
func writeTeachErrLog(line string) {
	...
	f, err := os.OpenFile(p, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0o600) // #nosec G304 -- path is a config-resolved local log file, not attacker-controlled input
	if err != nil {
		return
	}
	defer f.Close()
...
func appendLearningsAudit(entry map[string]any) error {
	...
	f, err := os.OpenFile(p, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0o600) // #nosec G304 -- path is a config-resolved local audit log file, not attacker-controlled input
	if err != nil {
		return err
	}
	defer f.Close()
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("discogs-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  discogs-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  discogs-pp-cli recall "<question>" --agent
  discogs-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  discogs-pp-cli learnings list --agent
  discogs-pp-cli learnings list --query "<substring>"
  discogs-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  discogs-pp-cli learnings forget "<question>" --resource <id>
  discogs-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  discogs-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  discogs-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
