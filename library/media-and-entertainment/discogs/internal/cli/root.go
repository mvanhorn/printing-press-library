...
type rootFlags struct {
	...
	// invocation. Mirrors the DISCOGS_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "discogs-pp-cli",
		Short: `Every Discogs feature — plus a local price-history mirror the API doesn't keep, so you can watch your wantlist like a limit-order book, spot underpriced records, and track your collection's value over time.`,
		Long: `Every Discogs feature — database, collection, wantlist, marketplace — plus a local price-history mirror no other Discogs tool has.
...
Health check: run 'discogs-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("discogs-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'discogs-pp-cli profile list')")
	...
	rootCmd.AddCommand(newDatabaseCmd(flags))
	...
	rootCmd.AddCommand(newWantlistCmd(flags))
	...
	rootCmd.AddCommand(newNovelCompsCmd(flags))
	rootCmd.AddCommand(newNovelFillsCmd(flags))
	rootCmd.AddCommand(newNovelIdentifyCmd(flags))
	rootCmd.AddCommand(newNovelPortfolioCmd(flags))
	rootCmd.AddCommand(newNovelPressingsCmd(flags))
	rootCmd.AddCommand(newNovelSellPlanCmd(flags))
	rootCmd.AddCommand(newNovelUndervaluedCmd(flags))
	rootCmd.AddCommand(newSyncCmd(flags))
	rootCmd.AddCommand(newLimitCmd(flags))
