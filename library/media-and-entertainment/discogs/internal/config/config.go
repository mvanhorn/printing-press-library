...
type Config struct {
	...
	DiscogsToken     string          `toml:"token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	if v := os.Getenv("DISCOGS_TOKEN"); v != "" {
		cfg.DiscogsToken = v
		cfg.markEnvOverride("DiscogsToken")
		cfg.AuthSource = "env:DISCOGS_TOKEN"
		cfg.CredentialSource = "env:DISCOGS_TOKEN"
	}
	...
	if v := os.Getenv("DISCOGS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("DISCOGS_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "discogs-pp-cli", "config.toml"), nil
}

func readConfigFile(path string, cfg *Config, owner string) error {
	data, err := os.ReadFile(path) // #nosec G304 -- path is the user's own CLI config file, resolved from config dir; not attacker-controlled
	if err != nil {
		return err
	}
...
func (c *Config) AuthHeader() string {
	...
	replacements := map[string]string{
		"token":         token,
		"DISCOGS_TOKEN": c.DiscogsToken,
	}
	return applyAuthFormat("Discogs token={token}", replacements)
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.DiscogsToken = creds.DiscogsToken
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.DiscogsToken = token
	delete(c.envOverrides, "DiscogsToken")
	c.updateFileConfigField("DiscogsToken")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["DiscogsToken"] {
			out.DiscogsToken = c.fileConfig.DiscogsToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "DiscogsToken":
		c.fileConfig.DiscogsToken = c.DiscogsToken
	}
}
