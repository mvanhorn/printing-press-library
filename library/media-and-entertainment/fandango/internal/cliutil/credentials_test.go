...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/fandango/internal/cliutil/testenv"
...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"FANDANGO_SUBSCRIPTION_KEY",
	} {
		t.Setenv(name, "")
	}
	return home, filepath.Join(home, ".config", "fandango-pp-cli", "config.toml")
}
...
func TestExplicitConfigUsesSiblingCredentialsBeforeGlobal(t *testing.T) {
	...
	t.Setenv("FANDANGO_CONFIG", configPath)
...
func TestCorruptCredentialsDoesNotOverrideLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "fandango-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	home, _ := resetCredentialEnv(t)
	t.Setenv("FANDANGO_SUBSCRIPTION_KEY", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "fandango-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "fandango-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "fandango-pp-cli", "credentials.toml")
...
func configCredentialValue(cfg *config.Config) string {
	return cfg.FandangoSubscriptionKey
}
...
func credentialValue(creds *cliutil.Credentials) string {
	return creds.FandangoSubscriptionKey
}

func setCredentialValue(creds *cliutil.Credentials, token string) {
	creds.FandangoSubscriptionKey = token
}
