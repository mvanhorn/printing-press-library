...
type rootFlags struct {
	...
	// invocation. Mirrors the FANDANGO_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "fandango-pp-cli",
		Short: `Fandango CLI — Find and compare official Fandango showtimes, then hand users the licensed purchase link.`,
		Long: `Fandango CLI — Find and compare official Fandango showtimes, then hand users the licensed purchase link.
...
Health check: run 'fandango-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("fandango-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'fandango-pp-cli profile list')")
	...
	rootCmd.AddCommand(newFandangoCmd(flags))
	...
	rootCmd.AddCommand(newNovelFormatFindCmd(flags))
	rootCmd.AddCommand(newNovelMovieAvailabilityCmd(flags))
	rootCmd.AddCommand(newNovelMoviePlanCmd(flags))
	rootCmd.AddCommand(newNovelStartingSoonCmd(flags))
	rootCmd.AddCommand(newNovelTheaterCompareCmd(flags))
	rootCmd.AddCommand(newNovelWatchlistShowtimesCmd(flags))
