...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FANDANGO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FANDANGO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "fandango-pp-cli/feedback")
...
When ` + "`FANDANGO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FANDANGO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  fandango-pp-cli feedback list
  fandango-pp-cli feedback list --limit 5
  fandango-pp-cli feedback list --json`,
