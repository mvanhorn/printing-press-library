...
Run sync first to populate the local search index.`,
  ...
  fandango-pp-cli search "status"

  # Force local search explicitly
  fandango-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  fandango-pp-cli search "status" --type fandango --data-source local
  # JSON output for piping
  fandango-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'fandango-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
