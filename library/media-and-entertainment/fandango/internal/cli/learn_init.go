...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"id_provider": {
			{Canonical: "fandangoApi", Values: []string{"fandango api", "fandango"}},
			{Canonical: "IVA", Values: []string{"fabric origin", "iva"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: fandango-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: fandango-pp-cli: learn init: %v\n", err)
		}
	})
}
