...
type Config struct {
	...
	FandangoSubscriptionKey string          `toml:"subscription_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("FANDANGO_SUBSCRIPTION_KEY"); v != "" {
		cfg.FandangoSubscriptionKey = v
		cfg.markEnvOverride("FandangoSubscriptionKey")
		cfg.AuthSource = "env:FANDANGO_SUBSCRIPTION_KEY"
		cfg.CredentialSource = "env:FANDANGO_SUBSCRIPTION_KEY"
	}
	...
	if v := os.Getenv("FANDANGO_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("FANDANGO_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "fandango-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.FandangoSubscriptionKey
	if token == "" {
		return ""
	}
	if c.FandangoSubscriptionKey == "" {
		return ""
	}
	return token
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		FandangoSubscriptionKey: c.FandangoSubscriptionKey,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.FandangoSubscriptionKey == "" {
		c.FandangoSubscriptionKey = creds.FandangoSubscriptionKey
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.FandangoSubscriptionKey = token
	delete(c.envOverrides, "FandangoSubscriptionKey")
	c.updateFileConfigField("FandangoSubscriptionKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["FandangoSubscriptionKey"] {
			out.FandangoSubscriptionKey = c.fileConfig.FandangoSubscriptionKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "FandangoSubscriptionKey":
		c.fileConfig.FandangoSubscriptionKey = c.FandangoSubscriptionKey
	}
}
