...
type rootFlags struct {
	...
	// invocation. Mirrors the SNIPD_NO_LEARN env var.
	noLearn       bool
	selectFields  string
	configPath    string
	homePath      string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "snipd-pp-cli",
		Short: `Snipd CLI — Turn your Snipd snips into a local, ranked, full-text-searchable corpus — search a concept across every note, quote,…`,
		Long: `Snipd CLI — Turn your Snipd snips into a local, ranked, full-text-searchable corpus — search a concept across every note, quote, and transcript, pull the exact quote, and synthesize across shows from the command line, plus an MCP for agents.
...
Health check: run 'snipd-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("snipd-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'snipd-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelAggregateCmd(flags))
	rootCmd.AddCommand(newNovelFilterCmd(flags))
	rootCmd.AddCommand(newNovelPullCmd(flags))
	rootCmd.AddCommand(newNovelQuoteCmd(flags))
	rootCmd.AddCommand(newNovelSynthesizeCmd(flags))
