...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SNIPD_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SNIPD_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "snipd-pp-cli/feedback")
...
When ` + "`SNIPD_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SNIPD_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  snipd-pp-cli feedback list
  snipd-pp-cli feedback list --limit 5
  snipd-pp-cli feedback list --json`,
