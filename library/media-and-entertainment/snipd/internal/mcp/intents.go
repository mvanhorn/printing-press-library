...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("where_do_my_snips_cluster",
			mcplib.WithDescription("A one-call rollup of snip counts per show."),
			mcplib.WithString("slug", mcplib.Required(), mcplib.Description("Override the recipe's positional slug value.")),
		),
		handleWhereDoMySnipsCluster,
	)
}

// handleWhereDoMySnipsCluster runs the where_do_my_snips_cluster recipe intent: A one-call rollup of snip counts per show.
func handleWhereDoMySnipsCluster(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
