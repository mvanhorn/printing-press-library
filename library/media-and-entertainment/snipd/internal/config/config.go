...
type Config struct {
	...
	SnipdToken       string          `toml:"token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.snipd.com/v1/public/api",
	}
	...
	// Env var overrides
	if v := os.Getenv("SNIPD_TOKEN"); v != "" {
		cfg.SnipdToken = v
		cfg.markEnvOverride("SnipdToken")
		cfg.AuthSource = "env:SNIPD_TOKEN"
		cfg.CredentialSource = "env:SNIPD_TOKEN"
	}
	...
	if v := os.Getenv("SNIPD_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("SNIPD_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "snipd-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.SnipdToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:SNIPD_TOKEN"
		}
		return "Bearer " + c.SnipdToken
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.SnipdToken = ""
	delete(c.envOverrides, "SnipdToken")
	c.updateFileConfigField("SnipdToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["SnipdToken"] {
			out.SnipdToken = c.fileConfig.SnipdToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "SnipdToken":
		c.fileConfig.SnipdToken = c.SnipdToken
	}
}
