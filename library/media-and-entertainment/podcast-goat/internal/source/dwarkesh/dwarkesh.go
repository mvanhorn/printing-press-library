...
// h2RE matches an <h2> block.
var h2RE = regexp.MustCompile(`(?is)<h2[^>]*>(.*?)</h2>`)
...
func (a *Adapter) Fetch(ctx context.Context, url string) (*transcript.Transcript, error) {
	...
	// with segments while still tagging the current speaker.
	type token struct {
		kind string // "h2" | "p"
		raw  string
	}
	var tokens []token
	for _, idxs := range h2RE.FindAllStringSubmatchIndex(html, -1) {
		tokens = append(tokens, token{kind: "h2", raw: html[idxs[2]:idxs[3]]})
		_ = idxs
	}
	// Order-preserving merge. Go's RE2 has no backreferences, so we scan h2 and
	// p separately and merge by starting offset to preserve document order.
	tokens = tokens[:0]
