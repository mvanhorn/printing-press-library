...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Podcast GOAT",
	...
	// PATCH: v0.1 podcast-goat service-scoped login + per-service status.
	cmd.AddCommand(newAuthLoginChromeServiceCmd(flags))
	cmd.AddCommand(newAuthServicesCmd(flags))
	// PATCH: v0.1.1 paid-provider key persistence (closes set-and-forget UX gap).
...
profile by name when the installed backend supports it.`,
		Example: `  podcast-goat-pp-cli auth login --chrome
  podcast-goat-pp-cli auth login --browser
  podcast-goat-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !browserFlag {
				fmt.Fprintln(cmd.OutOrStdout(), "Use --chrome or --browser to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  podcast-goat-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  podcast-goat-pp-cli auth login --browser\n")
				return nil
			}
			...
			if cookieToolSupportsProfiles(tool.name) {
				...
				if err != nil {
					...
					fmt.Fprintf(w, "\n  podcast-goat-pp-cli auth login --chrome\n")
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  podcast-goat-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  podcast-goat-pp-cli auth login --chrome\n")
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  podcast-goat-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
...
func validateComposedAuth(authHeader string) error {
	testURL := "https://www.hubermanlab.com"
