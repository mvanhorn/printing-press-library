...
Things that compile and dispatch correctly but ship a typed `NotImplementedError` until v0.2:

- `huberman`, `acquired`, `founders`, `peterattia` HTML parsers — cookie capture and authenticated GET both work end-to-end (`auth login-service --service <name>` writes `~/.config/podcast-goat/cookies/cookies-<service>.json`, the adapter loads it and reaches the member surface). The HTML-to-segment parser ships in v0.2 after a first-time browser capture from a logged-in session calibrates the per-publisher shape. Until then, fall back to `--paid --provider spoken` for the free public episode.
- `spotify` adapter is **working end-to-end** as of v0.1: `auth login-service --service spotify` captures the sp_dc cookie, the adapter then auto-bootstraps a fresh Bearer via Spotify's TOTP-signed token endpoint (using the latest secret version from `internal/source/spotify/totp.go` — refreshed at install time in v0.2), and the transcript-read-along JSON is normalized to canonical markdown. Verified live 2026-05-17 against the "10 Years of Acquired (with Michael Lewis)" episode (5083 lines / 202KB output). `SPOTIFY_BEARER` env var still works as a manual override.
- `episode get --bilingual zh-Hans,en` — the dual-language aligner is reserved as a flag in v0.1 but errors out. The yt-dlp adapter ships the English path now; the bilingual pairing arrives in v0.2.
- `whisperapi` audio extraction — the provider switch (`--provider-name elevenlabs|openai|deepgram`) and key checks are live, but the yt-dlp audio extract -> upload -> diarize pipeline ships in v0.2. Use `--provider spoken` or `--provider taddy` for paid fallback today.
- `auth services` remediation for Chrome App-Bound (v10) cookies — captured cookies are written verbatim from the chromecookies helper. Chrome 127+ App-Bound v10 values need a 32-byte host-prefix strip on the CDP path; this lands in v0.2 alongside the cookie-tier HTML parsers that need it.
