...
type Config struct {
	...
	TvtimeUsername string            `toml:"username"`
	TvtimePassword string            `toml:"password"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api2.tozelabs.com/v2",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("TV_TIME_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "tv-time-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("TVTIME_USERNAME"); v != "" {
		cfg.TvtimeUsername = v
		cfg.markEnvOverride("TvtimeUsername")
		cfg.AuthSource = "env:TVTIME_USERNAME"
	}
	if v := os.Getenv("TVTIME_PASSWORD"); v != "" {
		cfg.TvtimePassword = v
		cfg.markEnvOverride("TvtimePassword")
		cfg.AuthSource = "env:TVTIME_PASSWORD"
	}
	...
	if cfg.AuthSource == "" && cfg.TvtimeUsername != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.TvtimePassword != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("TV_TIME_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.TvtimeUsername == "" || c.TvtimePassword == "" {
		return ""
	}
	credentials := c.TvtimeUsername + ":" + c.TvtimePassword
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.TvtimeUsername = token
	delete(c.envOverrides, "TvtimeUsername")
	c.updateFileConfigField("TvtimeUsername")
	return c.save()
}
...
func (c *Config) ClearTokens() error {
	...
	delete(c.envOverrides, "TvtimeUsername")
	c.TvtimePassword = ""
	delete(c.envOverrides, "TvtimePassword")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["TvtimeUsername"] {
			out.TvtimeUsername = c.fileConfig.TvtimeUsername
		}
		if c.envOverrides["TvtimePassword"] {
			out.TvtimePassword = c.fileConfig.TvtimePassword
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "TvtimeUsername":
		c.fileConfig.TvtimeUsername = c.TvtimeUsername
	case "TvtimePassword":
		c.fileConfig.TvtimePassword = c.TvtimePassword
	}
}
