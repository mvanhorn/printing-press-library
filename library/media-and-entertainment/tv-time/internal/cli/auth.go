...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Tv Time",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  tv-time-pp-cli auth setup\n  tv-time-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "See API docs: https://api2.tozelabs.com")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export TVTIME_USERNAME=\"<your-token>\"")
			fmt.Fprintln(w, "  export TVTIME_PASSWORD=\"<your-token>\"")
			fmt.Fprintln(w, "  tv-time-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://api2.tozelabs.com"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  tv-time-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export TVTIME_USERNAME=\"your-username\"")
				fmt.Fprintln(w, "  export TVTIME_PASSWORD=\"your-token-here\"")
				fmt.Fprintf(w, "  tv-time-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  tv-time-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  tv-time-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("TVTIME_USERNAME") != "" {
				envStillSet = "TVTIME_USERNAME"
			}
			if envStillSet == "" && os.Getenv("TVTIME_PASSWORD") != "" {
				envStillSet = "TVTIME_PASSWORD"
			}
