...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "tv-time-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TV_TIME_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TV_TIME_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "tv-time-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/tv-time-pp-cli/feedback.jsonl.
When ` + "`TV_TIME_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TV_TIME_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  tv-time-pp-cli feedback list
  tv-time-pp-cli feedback list --limit 5
  tv-time-pp-cli feedback list --json`,
