# TV Time Printed CLI Agent Guide

This directory is a generated `tv-time-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
tv-time-pp-cli doctor --json
tv-time-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
tv-time-pp-cli which "<capability>" --json
tv-time-pp-cli <command> --help
```
...
```bash
tv-time-pp-cli <command> --help
tv-time-pp-cli <command> --dry-run --agent
```
