...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("IMMICH_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("IMMICH_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "immich-pp-cli/feedback")
...
When ` + "`IMMICH_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`IMMICH_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  immich-pp-cli feedback list
  immich-pp-cli feedback list --limit 5
  immich-pp-cli feedback list --json`,
