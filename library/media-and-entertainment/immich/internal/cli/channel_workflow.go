...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  immich-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  immich-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"admin", "admin-database-backups", "admin-integrity-report", "admin-maintenance-detect-install", "albums", "api-keys", "api-keys-me", "checksum_mismatch", "duplicates", "immich-search", "immich-search-explore", "immich-search-suggestions", "immich-sync", "libraries", "map", "memories", "missing_file", "notifications", "partners", "people", "plugins", "plugins-methods", "plugins-templates", "queues", "server", "server-version-history", "sessions", "shared-links", "shared-links-me", "stacks", "tags", "timeline", "untracked_file", "users", "users-me-calendar-heatmap", "view-folder-unique-paths", "workflows"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  immich-pp-cli workflow status

  # Show status as JSON
  immich-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("immich-pp-cli")
			}
