...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/immich/internal/learn/lookups"
...
Resource scoping:
  ...
  immich-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  immich-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  immich-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  immich-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  immich-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("immich-pp-cli")
			}
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "albums_assets":
		return db.UpsertAlbumsAssets(data)
	case "map_markers":
		return db.UpsertMapMarkers(data)
	case "user":
		return db.UpsertUser(data)
	case "albums_users":
		return db.UpsertAlbumsUsers(data)
	case "api-keys":
		return db.UpsertApiKeys(data)
	case "assets":
		return db.UpsertAssets(data)
	case "edits":
		return db.UpsertEdits(data)
	case "metadata":
		return db.UpsertMetadata(data)
	case "ocr":
		return db.UpsertOcr(data)
	case "original":
		return db.UpsertOriginal(data)
	case "assets_thumbnail":
		return db.UpsertAssetsThumbnail(data)
	case "video":
		return db.UpsertVideo(data)
	case "download":
		return db.UpsertDownload(data)
	case "duplicates":
		return db.UpsertDuplicates(data)
	case "faces":
		return db.UpsertFaces(data)
	case "immich-auth":
		return db.UpsertImmichAuth(data)
	case "immich-search":
		return db.UpsertImmichSearch(data)
	case "immich-sync":
		return db.UpsertImmichSync(data)
	case "libraries":
		return db.UpsertLibraries(data)
	case "scan":
		return db.UpsertScan(data)
	case "libraries_statistics":
		return db.UpsertLibrariesStatistics(data)
	case "validate":
	...
	case "memories_assets":
		return db.UpsertMemoriesAssets(data)
	case "notifications":
	...
	case "people_statistics":
		return db.UpsertPeopleStatistics(data)
	case "people_thumbnail":
		return db.UpsertPeopleThumbnail(data)
	case "plugins":
	...
	case "server":
		return db.UpsertServer(data)
	case "sessions":
		return db.UpsertSessions(data)
	case "lock":
		return db.UpsertLock(data)
	case "shared-links":
		return db.UpsertSharedLinks(data)
	case "shared_links_assets":
		return db.UpsertSharedLinksAssets(data)
	case "stacks":
		return db.UpsertStacks(data)
	case "stacks_assets":
		return db.UpsertStacksAssets(data)
	case "system-metadata":
		return db.UpsertSystemMetadata(data)
	case "tags":
		return db.UpsertTags(data)
	case "tags_assets":
		return db.UpsertTagsAssets(data)
	case "timeline":
	...
	case "profile_image":
		return db.UpsertProfileImage(data)
	case "view":
...
func knownSyncResourceNames() []string {
	names := []string{
		...
		"immich-search",
		"immich-search-explore",
		"immich-search-person",
		"immich-search-places",
		"immich-search-suggestions",
		"immich-sync",
		"libraries",
		"map",
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		...
		"admin-integrity-report":           "/admin/integrity/report",
		"admin-maintenance-detect-install": "/admin/maintenance/detect-install",
		"albums":                           "/albums",
		"api-keys":                         "/api-keys",
		"api-keys-me":                      "/api-keys/me",
		"checksum_mismatch":                "/admin/integrity/report?type=checksum_mismatch",
		"duplicates":                       "/duplicates",
		"faces":                            "/faces",
		"immich-search":                    "/search/cities",
		"immich-search-explore":            "/search/explore",
		"immich-search-person":             "/search/person",
		"immich-search-places":             "/search/places",
		"immich-search-suggestions":        "/search/suggestions",
		"immich-sync":                      "/sync/ack",
		"libraries":                        "/libraries",
		"map":                              "/map/markers",
		"map-reverse-geocode":              "/map/reverse-geocode",
		"memories":                         "/memories",
		"missing_file":                     "/admin/integrity/report?type=missing_file",
		...
		"untracked_file":                   "/admin/integrity/report?type=untracked_file",
		"users":                            "/users",
		"users-me-calendar-heatmap":        "/users/me/calendar-heatmap",
		"view":                             "/view/folder",
		"view-folder-unique-paths":         "/view/folder/unique-paths",
		"workflows":                        "/workflows",
...
var itemHydrationPaths = map[string]itemHydrationConfig{
	"immich-search-suggestions": {path: "/tags/{id}", idParam: "id"},
	"view-folder-unique-paths":  {path: "/tags/{id}", idParam: "id"},
}
...
var reconcileTypedTables = map[string]string{
	...
	"admin":                "admin",
	"albums":               "albums",
	"albums_assets":        "albums_assets",
	"albums_users":         "albums_users",
	"api-keys":             "api_keys",
	"assets":               "assets",
	"assets_thumbnail":     "assets_thumbnail",
	"download":             "download",
	"duplicates":           "duplicates",
	"edits":                "edits",
	"faces":                "faces",
	"immich-auth":          "immich_auth",
	"immich-search":        "immich_search",
	"immich-sync":          "immich_sync",
	"jobs":                 "jobs",
	"libraries":            "libraries",
	"libraries_statistics": "libraries_statistics",
	"lock":                 "lock",
	"map":                  "map",
	"map_markers":          "map_markers",
	"memories":             "memories",
	"memories_assets":      "memories_assets",
	"merge":                "merge",
	"metadata":             "metadata",
	"notifications":        "notifications",
	"oauth":                "oauth",
	...
	"people_thumbnail":     "people_thumbnail",
	"plugins":              "plugins",
	...
	"shared_links_assets":  "shared_links_assets",
	"stacks":               "stacks",
	"stacks_assets":        "stacks_assets",
...
func responsePathForResource(resource, path string) []string {
	...
	case "admin\x00/admin/maintenance/detect-install":
		return []string{"storage"}
	case "admin\x00/admin/integrity/report":
		return []string{"items"}
	case "admin\x00/admin/users/{id}/calendar-heatmap":
		return []string{"series"}
	case "admin\x00/admin/database-backups":
		return []string{"backups"}
	case "api-keys\x00/api-keys/{id}":
		return []string{"permissions"}
	case "api-keys\x00/api-keys/me":
		return []string{"permissions"}
	case "assets\x00/assets/bulk-upload-check":
		return []string{"results"}
	case "download\x00/download/info":
		return []string{"archives"}
	case "memories\x00/memories":
		return []string{"assets"}
	case "memories\x00/memories/{id}":
		return []string{"assets"}
	case "people\x00/people":
		return []string{"people"}
	case "plugins\x00/plugins/{id}":
		return []string{"methods"}
	case "server\x00/server/statistics":
		return []string{"usageByUser"}
	case "shared-links\x00/shared-links":
		return []string{"assets"}
	case "shared-links\x00/shared-links/{id}":
		return []string{"assets"}
	case "shared-links\x00/shared-links/me":
		return []string{"assets"}
	case "shared-links\x00/shared-links/login":
		return []string{"assets"}
	case "stacks\x00/stacks":
		return []string{"assets"}
	case "stacks\x00/stacks/{id}":
		return []string{"assets"}
	case "users\x00/users/me/calendar-heatmap":
		return []string{"series"}
