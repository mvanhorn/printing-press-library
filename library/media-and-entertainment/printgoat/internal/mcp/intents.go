...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("resume_an_interrupted_batch_job",
			mcplib.WithDescription("Continues a multi-source download job from its last completed byte offset per file instead of restarting."),
			mcplib.WithString("slug", mcplib.Required(), mcplib.Description("Override the recipe's positional slug value.")),
		),
		handleResumeAnInterruptedBatchJob,
	)
}

// handleResumeAnInterruptedBatchJob runs the resume_an_interrupted_batch_job recipe intent: Continues a multi-source download job from its last completed byte offset per file instead of restarting.
func handleResumeAnInterruptedBatchJob(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
