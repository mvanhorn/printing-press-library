...
Resource scoping:
  ...
  printgoat-pp-cli sync

  # Sync specific resources only
  printgoat-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  printgoat-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  printgoat-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  printgoat-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  printgoat-pp-cli sync --latest-only`,
...
func syncResource(ctx context.Context, c interface {
	...
	if missingKeys := unresolvedPathKeyRE.FindAllString(path, -1); len(missingKeys) > 0 {
		if !humanFriendly {
			...
			payloadJSON, _ := marshalJSONNoEscape(payload)
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "creations":
		return db.UpsertCreations(data)
	case "models":
		return db.UpsertModels(data)
	case "search_things":
		return db.UpsertSearchThings(data)
	case "tags":
		return db.UpsertTags(data)
	case "things":
		return db.UpsertThings(data)
	case "users":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		// Absolute URL, not "/categories": this resource came from the
		// Thingiverse spec in a multi-spec merge, whose per-resource
		// base_url override (categories_list.go's "pp:path" annotation
		// carries the same absolute URL) is not preserved by this
		// generated table. A bare "/categories" would resolve against
		// the merged CLI's primary (Printables) BaseURL instead, hitting
		// the wrong host entirely. See internal/client's isAbsoluteURL
		// check in doInternal, which this relies on.
		"categories": "https://api.thingiverse.com/categories",
	}
...
func responsePathForResource(resource, path string) []string {
	...
	case "creations\x00/graphql":
		return []string{"data.model"}
	case "models\x00/graphql/":
		return []string{"data.model"}
	}
	return nil
}
