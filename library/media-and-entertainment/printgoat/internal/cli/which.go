...
var whichIndex = []whichEntry{
	{Command: "duplicates", Description: "See instantly when the same design is listed free on one site and paid on another, before you download the wrong one.", Group: "Cross-site intelligence", WhyItMatters: "Reach for this before downloading anything from a paid Cults3D listing — it tells you if the same file exists free elsewhere."},
	{Command: "history diff", Description: "Know exactly what changed on a model since you last looked — new files, price changes, license changes.", Group: "Provenance, resume & integrity", WhyItMatters: "Use this instead of re-fetching a model's full detail when you only need to know what's different since last time."},
	{Command: "license audit", Description: "Flag license conflicts across your entire local library against how you actually intend to use the files.", Group: "Provenance, resume & integrity", WhyItMatters: "Run this before a commercial print job to catch license conflicts the site's own badge won't warn you about."},
	{Command: "similar", Description: "Re-search for alternatives after a bad print, automatically excluding the model and designer that failed you.", Group: "Print outcome memory", WhyItMatters: "Use after logging a failed print (`log fail`) instead of re-running a generic search that will surface the same bad result again."},
	{Command: "job download", Description: "Batch-download across all three sites as one crash-safe job that actually resumes from where it stopped.", Group: "Provenance, resume & integrity", WhyItMatters: "Use this for any multi-file or multi-source batch pull; it survives crashes and rate-limit backoff without restarting completed files."},
	{Command: "feed", Description: "Follow a designer across all the sites they post to and see new uploads from any of them in one feed.", Group: "Cross-site intelligence", WhyItMatters: "Check this instead of manually revisiting three separate designer profile pages to see what's new."},
	{Command: "snapshot verify", Description: "Prove exactly which file version you used for a past print job, even if the upstream model has since changed.", Group: "Provenance, resume & integrity", WhyItMatters: "Use this for provenance/reproducibility on any job whose output needs to match a prior run exactly."},
	{Command: "library doctor", Description: "Find orphaned files, missing files, silent duplicates, and remote listings that have been delisted (and can never be re-fetched).", Group: "Provenance, resume & integrity", WhyItMatters: "Run periodically to catch link rot before a delisted source becomes unrecoverable if the local copy is ever lost."},
	{Command: "formats gaps", Description: "Find alternatives when a model only ships in one file format and you need another.", Group: "Cross-site intelligence", WhyItMatters: "Use when a search result is STL-only and you need 3MF or STEP — this looks for tag-similar alternatives that include the missing format."},
	{Command: "designer stats", Description: "See your own success/failure rate with a specific designer, pooled across every site they publish on.", Group: "Print outcome memory", WhyItMatters: "Check this before downloading a new model from a designer you've printed before, when site-level popularity counts don't reflect your own experience."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  printgoat-pp-cli which "stale tickets"
  printgoat-pp-cli which "bottleneck"
  printgoat-pp-cli which --limit 1 "send message"
  printgoat-pp-cli which                                # list the full capability index`,
