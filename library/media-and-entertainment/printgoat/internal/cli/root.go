...
type rootFlags struct {
	...
	// invocation. Mirrors the PRINTGOAT_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "printgoat-pp-cli",
		Short: `PrintGoat CLI — Search and reliably download 3D-print files across Printables and Thingiverse (plus search Cults3D).`,
		Long: `PrintGoat CLI — Search and reliably download 3D-print files across Printables and Thingiverse (plus search Cults3D) — with real resumable downloads and cross-site duplicate detection nothing else has.
...
Health check: run 'printgoat-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("printgoat-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'printgoat-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCreationsCmd(flags))
	rootCmd.AddCommand(newThingsCmd(flags))
	...
	rootCmd.AddCommand(newNovelDesignerCmd(flags))
	rootCmd.AddCommand(newNovelDuplicatesCmd(flags))
	rootCmd.AddCommand(newNovelFeedCmd(flags))
	rootCmd.AddCommand(newNovelFollowCmd(flags))
	...
	rootCmd.AddCommand(newNovelLibraryCmd(flags))
	rootCmd.AddCommand(newNovelLicenseCmd(flags))
	...
	rootCmd.AddCommand(newSearchThingsPromotedCmd(flags))
