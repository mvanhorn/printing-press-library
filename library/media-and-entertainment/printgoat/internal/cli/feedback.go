...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PRINTGOAT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PRINTGOAT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	body, err := marshalJSONNoEscape(entry)
	if err != nil {
		return err
	}
	...
	req.Header.Set("User-Agent", "printgoat-pp-cli/feedback")
...
When ` + "`PRINTGOAT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PRINTGOAT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  printgoat-pp-cli feedback list
  printgoat-pp-cli feedback list --limit 5
  printgoat-pp-cli feedback list --json`,
