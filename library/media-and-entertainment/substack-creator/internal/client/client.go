...
type Client struct {
	...
	DryRun     bool
	NoCache    bool
	// Subdomain, when set, routes API calls to <subdomain>.substack.com/api/v1
	// instead of substack.com/api/v1. Required for Creator-facing endpoints
	// (drafts, posts list, subscriber count, publication info, sections, notes).
	Subdomain  string
	cacheDir   string
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "substack-creator-pp-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	// Subdomain override: rewrite host for per-publication Creator endpoints.
	// substack.com/api/v1/... -> <subdomain>.substack.com/api/v1/...
	if c.Subdomain != "" && strings.HasPrefix(targetURL, "https://substack.com") {
		targetURL = "https://" + c.Subdomain + ".substack.com" + strings.TrimPrefix(targetURL, "https://substack.com")
	}
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if authHeader != "" {
			// PATCH: cookie-auth-header — Substack's session auth uses
			// Cookie: substack.sid=...; the generator default puts every
			// authHeader value into Authorization:, which returns 401. The
			// substack.sid/connect.sid detection routes cookie-shaped values
			// to the Cookie header instead. Recorded in
			// .printing-press-patches.json.
			//
			// Cookie-based auth: when the auth string contains '=' and starts with a
			// cookie name (substack.sid, connect.sid, etc.), route to the Cookie header
			// instead of Authorization. This is the runtime equivalent of auth.type:
			// cookie in the spec.
		...
		req.Header.Set("User-Agent", "substack-creator-pp-cli/0.1 (+https://github.com/JimPresting)")
