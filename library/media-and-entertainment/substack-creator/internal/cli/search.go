...
In local mode: searches locally synced data only.`,
  ...
  substack-creator-pp-cli search "error timeout"

  # Force local search only
  substack-creator-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  substack-creator-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  substack-creator-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.dataSource != "local" {
				...
				data, getErr := c.Get("/publication/search", map[string]string{
					"query": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'substack-creator-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "drafts":
				results, err = db.SearchDrafts(query, limit)
			case "me":
				results, err = db.SearchMe(query, limit)
			case "publications":
				results, err = db.SearchPublications(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchDrafts(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search drafts failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchPublications(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search publications failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/substack-creator-pp-cli/data.db)")
