...
// subscribers churn / subscribers cross-sell / grep / schedule board)
...
Run 'substack-creator-pp-cli sync --full' first to refresh the local cache.`,
		Example:     "  substack-creator-pp-cli portfolio --json",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			w := cmd.OutOrStdout()
			if len(out) == 0 {
				fmt.Fprintln(w, "No publications synced yet. Run 'substack-creator-pp-cli sync --full' after 'substack-creator-pp-cli auth login --chrome'.")
				return nil
			}
