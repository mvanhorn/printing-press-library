...
func newSubscribersCrossSellCmd(flags *rootFlags) *cobra.Command {
	var (
		dbPath string
		limit  int
	)
...
The classic cross-sell list. Substack's web UI does not ship this.`,
		Example:     "  substack-creator-pp-cli subscribers cross-sell --json --limit 100",
		Annotations: map[string]string{"mcp:read-only": "true", "pp:novel": "subscribers cross-sell"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			prows.Close()
			if len(pubs) < 2 {
				return fmt.Errorf("cross-sell needs at least 2 synced publications; found %d", len(pubs))
			}
