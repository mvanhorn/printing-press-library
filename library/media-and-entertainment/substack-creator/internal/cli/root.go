...
type rootFlags struct {
	...
	dataSource    string
	freshnessMeta any
	// PATCH: subdomain-routing — adds a persistent --subdomain flag the
	// client uses to rewrite substack.com to <sub>.substack.com for Creator
	// endpoints (drafts, posts, notes, sections, dashboard). Without it the
	// entire Creator surface 404s. Recorded in .printing-press-patches.json.
	//
	// Substack-specific: route Creator-facing API calls to a publication subdomain.
	// When set, the client rewrites substack.com/api/v1/... -> <subdomain>.substack.com/api/v1/...
	// Required for: posts list, drafts list, notes list, publication info, sections.
	subdomain string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "substack-creator-pp-cli",
		Short: `Manage every Substack you own from one terminal: posts, drafts, notes, comments, subscribers, plus cross-publication churn diffs and full-text search across your archive.`,
		Long: `Manage every Substack you own from one terminal: posts, drafts, notes, comments, subscribers, plus cross-publication churn diffs and full-text search across your archive.
...
Health check: run 'substack-creator-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("substack-creator-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'substack-creator-pp-cli profile list')")
	rootCmd.PersistentFlags().StringVar(&flags.deliverSpec, "deliver", "", "Route output to a sink: stdout (default), file:<path>, webhook:<url>")
	rootCmd.PersistentFlags().Float64Var(&flags.rateLimit, "rate-limit", 0, "Max requests per second (0 to disable)")
	rootCmd.PersistentFlags().StringVar(&flags.subdomain, "subdomain", "", "Substack publication subdomain (e.g. 'mypub-paid') — required for Creator endpoints (drafts, posts, notes, sections per pub)")
	...
	// Transcendence top-level commands (Phase 3, hand-built)
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("substack-creator-pp-cli %s\n", version)
		},
	}
}
