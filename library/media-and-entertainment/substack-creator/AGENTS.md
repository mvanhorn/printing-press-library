...
This directory is a generated `substack-creator-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
substack-creator-pp-cli doctor --json
substack-creator-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
substack-creator-pp-cli which "<capability>" --json
substack-creator-pp-cli <command> --help
```
...
```bash
substack-creator-pp-cli <command> --agent
```
...
```bash
substack-creator-pp-cli <command> --help
substack-creator-pp-cli <command> --dry-run --agent
```
