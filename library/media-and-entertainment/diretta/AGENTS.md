# Diretta.it Printed CLI Agent Guide

This directory is a generated `diretta-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
diretta-pp-cli doctor --json
diretta-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
diretta-pp-cli which "<capability>" --json
diretta-pp-cli <command> --help
```
...
```bash
diretta-pp-cli <command> --help
diretta-pp-cli <command> --dry-run --agent
```
