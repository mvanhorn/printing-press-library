...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".diretta-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DIRETTA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DIRETTA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "diretta-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.diretta-pp-cli/feedback.jsonl.
When ` + "`DIRETTA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DIRETTA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  diretta-pp-cli feedback list
  diretta-pp-cli feedback list --limit 5
  diretta-pp-cli feedback list --json`,
