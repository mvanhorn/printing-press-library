...
func newMatchesTomorrowCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "tomorrow",
		Short:       "Tomorrow's football fixtures.",
		Example:     "  diretta-pp-cli matches tomorrow",
		Annotations: map[string]string{"pp:endpoint": "matches.tomorrow", "pp:method": "GET", "pp:path": "/x/feed/f_1_1_3_it_1", "mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			c, err := flags.newClient()
			if err != nil {
				return err
			}
			raw, prov, err := resolveRead(cmd.Context(), c, flags, "matches", false, "/x/feed/f_1_1_3_it_1", map[string]string{}, nil)
			if err != nil {
