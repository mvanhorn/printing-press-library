...
func buildAgentContext(rootCmd *cobra.Command) agentContext {
	...
	return agentContext{
		...
		CLI: agentContextCLI{
			Name:        "diretta-pp-cli",
			Description: "The only Go CLI for Diretta.it — live scores, standings, and H2H with offline SQLite cache, MCP server, and 30+...",
...
func buildAgentDiscoveryContext() *agentContextDiscovery {
	return &agentContextDiscovery{
		...
		Protocols: []string{
			"custom_delimited (99% confidence)",
			"rest_json (0% confidence)",
		},
		AuthCandidates: []string{},
		Protections:    []string{},
		GenerationHints: []string{
			"custom_delimiter_parser_required",
			"no_auth_needed",
			"static_fsign_token",
		},
		Warnings:          []string{},
