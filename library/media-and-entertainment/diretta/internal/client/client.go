...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/diretta/internal/cliutil"
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "diretta-pp-cli", "http")
...
// cacheTTL returns the appropriate cache duration for a given API path.
// Live feeds expire quickly; historical feeds can cache for much longer.
func cacheTTL(path string) time.Duration {
	switch {
	// Live match detail/events/stats — expire fast during a game
	case strings.Contains(path, "/dc_1_") || strings.Contains(path, "/df_sui_1_") || strings.Contains(path, "/df_st_1_"):
		return 60 * time.Second
	// Today's matches and multi-sport live feed
	case strings.Contains(path, "/f_1_0_") || strings.Contains(path, "/f_2_"):
		return 2 * time.Minute
	// Tomorrow's fixtures — check more frequently for new additions
	case strings.Contains(path, "/f_1_1_"):
		return 10 * time.Minute
	// H2H history — changes only when a new match is played
	case strings.Contains(path, "/df_hh_1_"):
		return 24 * time.Hour
	// Lineups — published hours before kickoff, stable once set
	case strings.Contains(path, "/df_li_1_"):
		return 30 * time.Minute
	// Standings — updated daily
	case strings.Contains(path, "/tr_"):
		return 30 * time.Minute
	// Yesterday's results — done, won't change
	case strings.Contains(path, "/f_1_-1_"):
		return 2 * time.Hour
	default:
...
func (c *Client) readCache(path string, params map[string]string) (json.RawMessage, bool) {
	...
	if err != nil || time.Since(info.ModTime()) > cacheTTL(path) {
		return nil, false
	}
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		req.Header.Set("Referer", "https://www.diretta.it/")
