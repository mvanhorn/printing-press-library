...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/movieglu/internal/learn/lookups"
...
Resource scoping:
  ...
  movieglu-pp-cli sync

  # Sync specific resources only
  movieglu-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  movieglu-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  movieglu-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  movieglu-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  movieglu-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("movieglu-pp-cli")
			}
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "movieglu-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "cinema-show-times":
		return db.UpsertCinemaShowTimes(data)
	case "cinemas-nearby":
		return db.UpsertCinemasNearby(data)
	case "closest-showing":
		return db.UpsertClosestShowing(data)
	case "film-show-times":
		return db.UpsertFilmShowTimes(data)
	case "films-now-showing":
		return db.UpsertFilmsNowShowing(data)
	case "purchase-confirmation":
		return db.UpsertPurchaseConfirmation(data)
	default:
...
var reconcileTypedTables = map[string]string{
	"cinema-show-times":     "cinema_show_times",
	"cinemas-nearby":        "cinemas_nearby",
	"closest-showing":       "closest_showing",
	"film-show-times":       "film_show_times",
	"films-now-showing":     "films_now_showing",
	"purchase-confirmation": "purchase_confirmation",
}
...
func responsePathForResource(resource, path string) []string {
	...
	case "cinema-show-times\x00/cinemaShowTimes/":
		return []string{"films"}
	case "cinemas-nearby\x00/cinemasNearby/":
		return []string{"cinemas"}
	case "closest-showing\x00/closestShowing/":
		return []string{"cinemas"}
	case "film-show-times\x00/filmShowTimes/":
		return []string{"cinemas"}
	case "films-now-showing\x00/filmsNowShowing/":
		return []string{"films"}
	}
	return nil
}
