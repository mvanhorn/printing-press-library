...
Run sync first to populate the local search index.`,
  ...
  movieglu-pp-cli search "status"

  # Force local search explicitly
  movieglu-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  movieglu-pp-cli search "status" --type cinema-show-times --data-source local
  # JSON output for piping
  movieglu-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'movieglu-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
