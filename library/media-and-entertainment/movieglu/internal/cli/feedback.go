...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MOVIEGLU_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MOVIEGLU_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "movieglu-pp-cli/feedback")
...
When ` + "`MOVIEGLU_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MOVIEGLU_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  movieglu-pp-cli feedback list
  movieglu-pp-cli feedback list --limit 5
  movieglu-pp-cli feedback list --json`,
