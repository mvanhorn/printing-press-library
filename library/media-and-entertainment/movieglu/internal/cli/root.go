...
type rootFlags struct {
	...
	// invocation. Mirrors the MOVIEGLU_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "movieglu-pp-cli",
		Short: "Manage movieglu resources via the movieglu API",
		Long: `Manage movieglu resources via the movieglu API.
...
Run 'movieglu-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("movieglu-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'movieglu-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCinemaShowTimesPromotedCmd(flags))
	rootCmd.AddCommand(newCinemasNearbyPromotedCmd(flags))
	rootCmd.AddCommand(newClosestShowingPromotedCmd(flags))
	rootCmd.AddCommand(newFilmShowTimesPromotedCmd(flags))
	rootCmd.AddCommand(newFilmsNowShowingPromotedCmd(flags))
	rootCmd.AddCommand(newPurchaseConfirmationPromotedCmd(flags))
