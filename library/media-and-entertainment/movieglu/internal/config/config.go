...
type Config struct {
	...
	MoviegluCredentials string          `toml:"credentials"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("MOVIEGLU_CREDENTIALS"); v != "" {
		cfg.MoviegluCredentials = v
		cfg.markEnvOverride("MoviegluCredentials")
		cfg.AuthSource = "env:MOVIEGLU_CREDENTIALS"
		cfg.CredentialSource = "env:MOVIEGLU_CREDENTIALS"
	}
	...
	if v := os.Getenv("MOVIEGLU_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("MOVIEGLU_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "movieglu-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.MoviegluCredentials
	if token == "" {
		return ""
	}
	if c.MoviegluCredentials == "" {
		return ""
	}
	return token
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		MoviegluCredentials: c.MoviegluCredentials,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.MoviegluCredentials == "" {
		c.MoviegluCredentials = creds.MoviegluCredentials
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.MoviegluCredentials = token
	delete(c.envOverrides, "MoviegluCredentials")
	c.updateFileConfigField("MoviegluCredentials")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["MoviegluCredentials"] {
			out.MoviegluCredentials = c.fileConfig.MoviegluCredentials
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "MoviegluCredentials":
		c.fileConfig.MoviegluCredentials = c.MoviegluCredentials
	}
}
