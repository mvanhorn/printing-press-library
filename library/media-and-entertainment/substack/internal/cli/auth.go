...
profile by name when the installed backend supports it.`,
  ...
  substack-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Verifier short-circuit — no keychain/cookie reads in mock-mode runs.
			if cliutil.IsVerifyEnv() {
				if flags.asJSON {
					return printJSONFiltered(cmd.OutOrStdout(), map[string]any{
						"status":                   "verify_env_short_circuit",
						"verify_env_short_circuit": true,
					}, flags)
				}
				fmt.Fprintln(cmd.OutOrStdout(), "verify mode short-circuit — would have read Chrome cookies")
				return nil
			}
