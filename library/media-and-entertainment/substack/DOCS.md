# substack-pp-cli — Vollständige Befehls-Dokumentation
...
#   ~/.config/substack-pp-cli/config.toml:
#   access_token = "substack.sid=<value>"

substack-pp-cli auth status     # Auth-Status prüfen
substack-pp-cli doctor          # Health-Check: Auth + DB + Reachability
substack-pp-cli auth logout     # Cookie löschen
```
...
```bash
substack-pp-cli posts list --subdomain mypub
substack-pp-cli drafts create --subdomain mypub --title "..."
```
...
```bash
substack-pp-cli posts list --subdomain mypub --type podcast --json
substack-pp-cli posts get my-post-slug --subdomain mypub --json
