...
substack-pp-cli subscribers churn --since 7d --json


# FTS5 over years of posts + notes + comments
substack-pp-cli grep "yield curve" --scope all --since 2024-01-01
...
- **`substack-pp-cli dashboard stats`** - Aggregate dashboard stats for a publication you own.
...
- **`substack-pp-cli feed list`** - Get your feed.

### images

Upload images to Substack's CDN.

- **`substack-pp-cli images upload`** - Upload an image.
...
- **`substack-pp-cli sections list`** - List sections for a publication.
...
- **`auth status` reports session expired** — Log into Substack in your Chrome browser and rerun `substack-pp-cli auth login --chrome` to refresh cookies
...
- **Rate-limited with HTTP 429 from list endpoints** — Lower the request rate: set `[network] rate_per_sec = 1` in `~/.config/substack-pp-cli/config.toml` — Substack's empirical limit is around 2/s
