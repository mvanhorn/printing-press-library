---
name: pp-atom-tickets
description: "Find bookable Atom movie tickets, compare real offers, and hand users the official checkout link. Trigger phrases: `plan a movie night with Atom`, `find cheap Atom tickets`, `find accessible movie showtimes`, `find movie preorders`, `what movies can I still make`, `use Atom Tickets`."
author: "avanderheyde"
license: "Apache-2.0"
...
metadata:
  openclaw:
    ...
    install:
      - kind: go
        bins: [atom-tickets-pp-cli]
        module: github.com/mvanhorn/printing-press-library/library/media-and-entertainment/atom-tickets/cmd/atom-tickets-pp-cli
---

# Atom Tickets — Printing Press CLI

## Prerequisites: Install the CLI

This skill drives the `atom-tickets-pp-cli` binary. **You must verify the CLI is installed before invoking any command from this skill.** If it is missing, install it first:

1. Install via the Printing Press installer. It defaults binaries to `$HOME/.local/bin` on macOS/Linux and `%LOCALAPPDATA%\Programs\PrintingPress\bin` on Windows:
   ```bash
   npx -y @mvanhorn/printing-press-library install atom-tickets --cli-only
   ```
2. Verify: `atom-tickets-pp-cli --version`
...
Use this CLI for official Atom venue, production, showtime, offer, accessibility, preorder, inventory, and checkout-link workflows.

## Anti-triggers

Do not use this CLI for:
- Do not use this CLI to scrape atomtickets.com.
- Do not claim availableInventory proves adjacent seats.
- Do not claim the CLI completes payment or seat selection.
...
```bash
atom-tickets-pp-cli which "<capability in your own words>"
```
...
Run `atom-tickets-pp-cli doctor` to verify setup.
...
- **Filterable** — `--select` keeps a subset of fields. Dotted paths descend into nested structures; arrays traverse element-wise. Critical for keeping context small on verbose APIs:
  ...
  ```bash
  atom-tickets-pp-cli partner get-production-by-vendor-id mock-value --agent --select id,name,status
  ```
...
- Use `--home <dir>` for one invocation, or set `ATOM_TICKETS_HOME=<dir>` to relocate all four path kinds under one root.
- Use per-kind env vars only when a specific kind must diverge: `ATOM_TICKETS_CONFIG_DIR`, `ATOM_TICKETS_DATA_DIR`, `ATOM_TICKETS_STATE_DIR`, `ATOM_TICKETS_CACHE_DIR`.
- Resolution order is per-kind env var, `--home`, `ATOM_TICKETS_HOME`, XDG (`XDG_CONFIG_HOME`, `XDG_DATA_HOME`, `XDG_STATE_HOME`, `XDG_CACHE_HOME`), then platform defaults.
- `config` contains settings like `config.toml` and profiles. `data` contains `credentials.toml`, `data.db`, cookies, and auth sidecars. `state` contains persisted queries, jobs, and `teach.log`. `cache` contains regenerable HTTP/cache files.
- Stored secrets live in `credentials.toml` under the data dir. Existing legacy `config.toml` secrets are read for compatibility and leave `config.toml` on the first auth write.
- Run `atom-tickets-pp-cli doctor --fail-on warn` to surface path and credential-location warnings. `agent-context` exposes a schema v4 `paths` block for agents that need the resolved dirs.
...
Fleet precedence: an inherited per-kind env var overrides an explicit `--home` for that kind. Use `ATOM_TICKETS_HOME` or per-kind vars as durable fleet levers, and use `--home` only for a single invocation. Relocation is not reversible by unsetting env vars; move files manually before clearing `ATOM_TICKETS_HOME`, or `doctor` will not find credentials left under the former root.
...
```bash
atom-tickets-pp-cli recall "<user's question>" --agent
```
...
```json
{
  ...
  "candidates": [
    { "id": 12, "class": "flag_alias | playbook_candidate",
      ...
      "rationale": "...",
      "next_action": ["<trial command>", "atom-tickets-pp-cli learnings confirm 12"] }
  ],
  "playbook": {
    "query_family": "...",
    "playbook": {
...
if Playbook present:
    ...
    -> the Playbook's expected_tool_calls is a budget; if you find yourself running
       materially more, record the divergence via `atom-tickets-pp-cli playbook amend`
       at end-of-session.
...
- `lookup_refresh_available` (top-level): an entity in the query has no lookup row yet, but synced data could provide one. Run `atom-tickets-pp-cli sync` to refresh entity lookups.
...
```bash
atom-tickets-pp-cli teach --query "<user's question>" --resource-type <type> --resource <id1> --resource <id2>
...
`atom-tickets-pp-cli learnings stats` reports recall hit rate, teach-to-reuse, playbook resolution rate, and candidate confirm/reject counts from the local `learn_events` table. Rates are null until they have a denominator; everything stays on this machine. Use it to check whether the loop is earning its keep for this CLI.

### Disabling learning

- `--no-learn` on a single command short-circuits both `recall` and the `teach` write path. Use for deterministic agent flows or tests that must not be affected by accumulated learnings.
- `ATOM_TICKETS_NO_LEARN=true` in the environment globally disables the pipeline.
...
```
atom-tickets-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
atom-tickets-pp-cli feedback --stdin < notes.txt
atom-tickets-pp-cli feedback list --json --limit 10
```

Entries are stored locally as `feedback.jsonl` under the resolved data dir. They are never POSTed unless `ATOM_TICKETS_FEEDBACK_ENDPOINT` is set AND either `--send` is passed or `ATOM_TICKETS_FEEDBACK_AUTO_SEND=true`. Default behavior is local-only.
...
```
atom-tickets-pp-cli profile save briefing --json
atom-tickets-pp-cli --profile briefing partner get-production-by-vendor-id mock-value
atom-tickets-pp-cli profile list --json
atom-tickets-pp-cli profile show briefing
atom-tickets-pp-cli profile delete briefing --yes
```
...
1. **Empty, `help`, or `--help`** → show `atom-tickets-pp-cli --help` output
...
2. Register with Claude Code:
   ```bash
   claude mcp add atom-tickets-pp-mcp -- atom-tickets-pp-mcp
   ```
3. Verify: `claude mcp list`

## Direct Use

1. Check if installed: `which atom-tickets-pp-cli`
   If not found, offer to install (see Prerequisites at the top of this skill).
2. Match the user query to the best command from the Unique Capabilities and Command Reference above.
3. Execute with the `--agent` flag:
   ```bash
   atom-tickets-pp-cli <command> [subcommand] [args] --agent
   ```
4. If ambiguous, drill into subcommand help: `atom-tickets-pp-cli <command> --help`.
