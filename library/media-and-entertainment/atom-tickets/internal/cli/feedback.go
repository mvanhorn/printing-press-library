...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ATOM_TICKETS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ATOM_TICKETS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "atom-tickets-pp-cli/feedback")
...
When ` + "`ATOM_TICKETS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ATOM_TICKETS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  atom-tickets-pp-cli feedback list
  atom-tickets-pp-cli feedback list --limit 5
  atom-tickets-pp-cli feedback list --json`,
