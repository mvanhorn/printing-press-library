...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  atom-tickets-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  atom-tickets-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  atom-tickets-pp-cli workflow status

  # Show status as JSON
  atom-tickets-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("atom-tickets-pp-cli")
			}
