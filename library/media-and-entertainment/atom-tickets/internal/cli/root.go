...
type rootFlags struct {
	...
	// invocation. Mirrors the ATOM_TICKETS_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "atom-tickets-pp-cli",
		Short: `Atom Tickets CLI — Find bookable Atom movie tickets, compare real offers, and hand users the official checkout link.`,
		Long: `Atom Tickets CLI — Find bookable Atom movie tickets, compare real offers, and hand users the official checkout link.
...
Health check: run 'atom-tickets-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("atom-tickets-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'atom-tickets-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelAccessibleShowtimesCmd(flags))
	rootCmd.AddCommand(newNovelDealFinderCmd(flags))
	rootCmd.AddCommand(newNovelFamilyFitCmd(flags))
	rootCmd.AddCommand(newNovelLastCallCmd(flags))
	rootCmd.AddCommand(newNovelMoviePlanCmd(flags))
	rootCmd.AddCommand(newNovelPreorderRadarCmd(flags))
