...
In local mode: searches locally synced data only.`,
  ...
  atom-tickets-pp-cli search "error timeout"

  # Force local search only
  atom-tickets-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  atom-tickets-pp-cli search "status" --type partner --data-source local
  # JSON output for piping
  atom-tickets-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /partner/v1/venue/details/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/partner/v1/venue/details/search", map[string]string{
					"term": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'atom-tickets-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
