...
type Config struct {
	...
	AtomTicketsApiKey string          `toml:"tickets_api_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api-beta.atomtickets.com",
	}
	...
	// Env var overrides
	if v := os.Getenv("ATOM_TICKETS_API_KEY"); v != "" {
		cfg.AtomTicketsApiKey = v
		cfg.markEnvOverride("AtomTicketsApiKey")
		cfg.AuthSource = "env:ATOM_TICKETS_API_KEY"
		cfg.CredentialSource = "env:ATOM_TICKETS_API_KEY"
	}
	...
	if v := os.Getenv("ATOM_TICKETS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("ATOM_TICKETS_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "atom-tickets-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.AtomTicketsApiKey
	if token == "" {
		return ""
	}
	if c.AtomTicketsApiKey == "" {
		return ""
	}
	return token
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		AtomTicketsApiKey: c.AtomTicketsApiKey,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.AtomTicketsApiKey == "" {
		c.AtomTicketsApiKey = creds.AtomTicketsApiKey
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.AtomTicketsApiKey = token
	delete(c.envOverrides, "AtomTicketsApiKey")
	c.updateFileConfigField("AtomTicketsApiKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["AtomTicketsApiKey"] {
			out.AtomTicketsApiKey = c.fileConfig.AtomTicketsApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "AtomTicketsApiKey":
		c.fileConfig.AtomTicketsApiKey = c.AtomTicketsApiKey
	}
}
