...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("find_missed_episodes",
			mcplib.WithDescription("Calculates the exact watched-versus-aired gap for current shows."),
			mcplib.WithString("slug", mcplib.Required(), mcplib.Description("Override the recipe's positional slug value.")),
		),
		handleFindMissedEpisodes,
	)
}

// handleFindMissedEpisodes runs the find_missed_episodes recipe intent: Calculates the exact watched-versus-aired gap for current shows.
func handleFindMissedEpisodes(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
