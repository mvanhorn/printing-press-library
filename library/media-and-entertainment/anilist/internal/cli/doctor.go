...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  anilist-pp-cli doctor
  anilist-pp-cli doctor --json
  anilist-pp-cli doctor --fail-on warn
  anilist-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set it with: anilist-pp-cli auth set-token <token> or export ANILIST_TOKEN=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if os.Getenv("ANILIST_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "ANILIST_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "ANILIST_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "anilist-pp-cli", suggestion)
						} else {
