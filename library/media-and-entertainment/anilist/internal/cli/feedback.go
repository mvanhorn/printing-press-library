...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ANILIST_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ANILIST_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "anilist-pp-cli/feedback")
...
When ` + "`ANILIST_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ANILIST_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  anilist-pp-cli feedback list
  anilist-pp-cli feedback list --limit 5
  anilist-pp-cli feedback list --json`,
