...
// anilist-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/anilist/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/anilist/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/anilist/internal/learn/patterns"
...
const noLearnEnvVar = "ANILIST_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("anilist-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  anilist-pp-cli teach --query "<question>" --resource-type <type> \
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  anilist-pp-cli learnings list --agent
  anilist-pp-cli learnings list --query "<substring>"
  anilist-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  anilist-pp-cli learnings forget "<question>" --resource <id>
  anilist-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  anilist-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  anilist-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
