...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".rocket-league-tracker-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ROCKET_LEAGUE_TRACKER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ROCKET_LEAGUE_TRACKER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "rocket-league-tracker-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.rocket-league-tracker-pp-cli/feedback.jsonl.
When ` + "`ROCKET_LEAGUE_TRACKER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ROCKET_LEAGUE_TRACKER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "rocket-league-tracker-pp-cli",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  rocket-league-tracker-pp-cli feedback list
  rocket-league-tracker-pp-cli feedback list --limit 5
  rocket-league-tracker-pp-cli feedback list --json`,
