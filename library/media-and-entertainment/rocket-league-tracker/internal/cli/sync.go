...
Exit codes & warnings:
  ...
  rocket-league-tracker-pp-cli sync

  # Sync specific resources only
  rocket-league-tracker-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  rocket-league-tracker-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  rocket-league-tracker-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  rocket-league-tracker-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  rocket-league-tracker-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "directory":
		return db.UpsertDirectory(data)
	case "player":
		return db.UpsertPlayer(data)
	case "rank":
		return db.UpsertRank(data)
	case "shop":
	...
	case "tournaments":
		return db.UpsertTournaments(data)
	default:
