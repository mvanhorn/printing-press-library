...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "rocket-league-tracker-pp-cli",
		Short: `Rocket League Tracker CLI — Every Rocket League rank lookup, plus a local SQLite store that turns 'right now' into 'last 30 days' — without a websi…`,
		Long: `Rocket League Tracker CLI — Every Rocket League rank lookup, plus a local SQLite store that turns 'right now' into 'last 30 days' — without a websi…
...
Health check: run 'rocket-league-tracker-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("rocket-league-tracker-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'rocket-league-tracker-pp-cli profile list')")
	...
	rootCmd.AddCommand(newDirectoryPromotedCmd(flags))
	rootCmd.AddCommand(newPlayerPromotedCmd(flags))
	rootCmd.AddCommand(newPopulationPromotedCmd(flags))
	rootCmd.AddCommand(newRankPromotedCmd(flags))
	rootCmd.AddCommand(newShopPromotedCmd(flags))
	rootCmd.AddCommand(newStatPromotedCmd(flags))
	rootCmd.AddCommand(newTitlesPromotedCmd(flags))
	rootCmd.AddCommand(newTournamentsPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	// Hand-built novel commands (Phase 3 transcendence features).
	rootCmd.AddCommand(newGroupCmd(flags))
	rootCmd.AddCommand(newImportCollectorSnapshotCmd(flags))
	rootCmd.AddCommand(newLiarCheckCmd(flags))
	rootCmd.AddCommand(newMMRCurveCmd(flags))
	rootCmd.AddCommand(newPeekCmd(flags))
	rootCmd.AddCommand(newPlayerContextCmd(flags))
	rootCmd.AddCommand(newPopulationBestTimeCmd(flags))
	rootCmd.AddCommand(newPromoCmd(flags))
	rootCmd.AddCommand(newSessionSummaryCmd(flags))
	rootCmd.AddCommand(newTournamentFitCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("rocket-league-tracker-pp-cli %s\n", version)
		},
	}
}
