...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("ROCKET_LEAGUE_TRACKER_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "rocket-league-tracker-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("RAPIDAPI_KEY"); v != "" {
		cfg.RapidapiKey = v
		cfg.AuthSource = "env:RAPIDAPI_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.RapidapiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("ROCKET_LEAGUE_TRACKER_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
