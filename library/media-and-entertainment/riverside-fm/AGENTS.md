...
This directory is a generated `riverside-fm-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
riverside-fm-pp-cli doctor --json
riverside-fm-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
riverside-fm-pp-cli which "<capability>" --json
riverside-fm-pp-cli <command> --help
```
...
```bash
riverside-fm-pp-cli <command> --agent
```
...
```bash
riverside-fm-pp-cli <command> --help
riverside-fm-pp-cli <command> --dry-run --agent
```
