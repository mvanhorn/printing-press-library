...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  youtube-creator-analytics-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  youtube-creator-analytics-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"group-items", "groups", "reports"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  youtube-creator-analytics-pp-cli workflow status

  # Show status as JSON
  youtube-creator-analytics-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("youtube-creator-analytics-pp-cli")
			}
