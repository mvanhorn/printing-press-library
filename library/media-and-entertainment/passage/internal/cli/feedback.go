...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BOOK_GOAT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BOOK_GOAT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "passage-pp-cli/feedback")
...
When ` + "`BOOK_GOAT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BOOK_GOAT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  passage-pp-cli feedback list
  passage-pp-cli feedback list --limit 5
  passage-pp-cli feedback list --json`,
