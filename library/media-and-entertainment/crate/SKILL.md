---
name: pp-crate
description: "Your record collection as a local database: pick tonight's record, subtract your shelf from a crate-digging search, and price your wantlist without a token. Trigger phrases: `what record should I play`, `what should I buy from my wantlist`, `what is my record collection worth`, `what am I missing from this label`, `what does my vinyl collection look like`, `search discogs`, `use crate`, `run crate`."
author: "justinwfu"
license: "Apache-2.0"
...
This skill drives the `crate-pp-cli` binary. **You must verify the CLI is installed before invoking any command from this skill.** If it is missing, install it first:

1. Install via the Printing Press installer. It defaults binaries to `$HOME/.local/bin` on macOS/Linux and `%LOCALAPPDATA%\Programs\PrintingPress\bin` on Windows:
   ```bash
   npx -y @mvanhorn/printing-press-library install crate --cli-only
   ```
2. Verify: `crate-pp-cli --version`
...
Use this for anything involving a specific person's record collection or wantlist — what to play, what to buy, what is missing, what it is worth. It also covers the full Discogs database (search, releases, masters, artists, labels) when a task just needs discography facts.
...
- `crate-pp-cli collection releases` — Folder 0 is the special 'All' folder containing every record in the collection.

**database** — Manage database

- `crate-pp-cli database` — The main discovery endpoint.
...
- `crate-pp-cli marketplace stats` — Keyless. Returns num_for_sale and lowest_price.
...
```bash
crate-pp-cli which "<capability in your own words>"
```
...
Run `crate-pp-cli doctor` to verify setup.
...
- **Filterable** — `--select` keeps a subset of fields. Dotted paths descend into nested structures; arrays traverse element-wise. Critical for keeping context small on verbose APIs:
  ...
  ```bash
  crate-pp-cli artists get mock-value --agent --select id,name,status
  ```
...
- Use `--home <dir>` for one invocation, or set `CRATE_HOME=<dir>` to relocate all four path kinds under one root.
- Use per-kind env vars only when a specific kind must diverge: `CRATE_CONFIG_DIR`, `CRATE_DATA_DIR`, `CRATE_STATE_DIR`, `CRATE_CACHE_DIR`.
- Resolution order is per-kind env var, `--home`, `CRATE_HOME`, XDG (`XDG_CONFIG_HOME`, `XDG_DATA_HOME`, `XDG_STATE_HOME`, `XDG_CACHE_HOME`), then platform defaults.
- `config` contains settings like `config.toml` and profiles. `data` contains `credentials.toml`, `data.db`, cookies, and auth sidecars. `state` contains persisted queries, jobs, and `teach.log`. `cache` contains regenerable HTTP/cache files.
- Stored secrets live in `credentials.toml` under the data dir. Existing legacy `config.toml` secrets are read for compatibility and leave `config.toml` on the first auth write.
- Run `crate-pp-cli doctor --fail-on warn` to surface path and credential-location warnings. `agent-context` exposes a schema v4 `paths` block for agents that need the resolved dirs.
...
Fleet precedence: an inherited per-kind env var overrides an explicit `--home` for that kind. Use `CRATE_HOME` or per-kind vars as durable fleet levers, and use `--home` only for a single invocation. Relocation is not reversible by unsetting env vars; move files manually before clearing `CRATE_HOME`, or `doctor` will not find credentials left under the former root.
...
```bash
crate-pp-cli recall "<user's question>" --agent
```
...
```json
{
  ...
  "candidates": [
    { "id": 12, "class": "flag_alias | playbook_candidate",
      ...
      "rationale": "...",
      "next_action": ["<trial command>", "crate-pp-cli learnings confirm 12"] }
  ],
  "playbook": {
    "query_family": "...",
    "playbook": {
...
if Playbook present:
    ...
    -> the Playbook's expected_tool_calls is a budget; if you find yourself running
       materially more, record the divergence via `crate-pp-cli playbook amend`
       at end-of-session.
...
- `lookup_refresh_available` (top-level): an entity in the query has no lookup row yet, but synced data could provide one. Run `crate-pp-cli sync` to refresh entity lookups.
...
```bash
crate-pp-cli teach --query "<user's question>" --resource-type <type> --resource <id1> --resource <id2>
...
`crate-pp-cli learnings stats` reports recall hit rate, teach-to-reuse, playbook resolution rate, and candidate confirm/reject counts from the local `learn_events` table. Rates are null until they have a denominator; everything stays on this machine. Use it to check whether the loop is earning its keep for this CLI.

### Disabling learning

- `--no-learn` on a single command short-circuits both `recall` and the `teach` write path. Use for deterministic agent flows or tests that must not be affected by accumulated learnings.
- `CRATE_NO_LEARN=true` in the environment globally disables the pipeline.
...
```
crate-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
crate-pp-cli feedback --stdin < notes.txt
crate-pp-cli feedback list --json --limit 10
```

Entries are stored locally as `feedback.jsonl` under the resolved data dir. They are never POSTed unless `CRATE_FEEDBACK_ENDPOINT` is set AND either `--send` is passed or `CRATE_FEEDBACK_AUTO_SEND=true`. Default behavior is local-only.
...
```
crate-pp-cli profile save briefing --json
crate-pp-cli --profile briefing artists get mock-value
crate-pp-cli profile list --json
crate-pp-cli profile show briefing
crate-pp-cli profile delete briefing --yes
```
...
1. **Empty, `help`, or `--help`** → show `crate-pp-cli --help` output
...
```bash
claude mcp add crate-pp-mcp -- crate-pp-mcp
```
...
1. Check if installed: `which crate-pp-cli`
   If not found, offer to install (see Prerequisites at the top of this skill).
2. Match the user query to the best command from the Unique Capabilities and Command Reference above.
3. Execute with the `--agent` flag:
   ```bash
   crate-pp-cli <command> [subcommand] [args] --agent
   ```
4. If ambiguous, drill into subcommand help: `crate-pp-cli <command> --help`.
