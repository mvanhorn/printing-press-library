...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/crate/internal/learn"
...
type rootFlags struct {
	...
	// invocation. Mirrors the CRATE_NO_LEARN env var.
	noLearn       bool
	selectFields  string
	configPath    string
	homePath      string
...
// defaultRateLimitPerSecond is 24 requests per minute, one under the keyless
// ceiling Discogs enforces.
const defaultRateLimitPerSecond = 0.4

func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "crate-pp-cli",
		Short: `Crate CLI — Your record collection as a local database: pick tonight's record, subtract your shelf from a crate-digging search,…`,
		Long: `Crate CLI — Your record collection as a local database: pick tonight's record, subtract your shelf from a crate-digging search, and price your wantlist without a token.
...
Health check: run 'crate-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("crate-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'crate-pp-cli profile list')")
	rootCmd.PersistentFlags().StringVar(&flags.deliverSpec, "deliver", "", "Route output to a sink: stdout (default), file:<path>, webhook:<url>")
	// Discogs documents 25 requests per minute without a token and 60 with
	// one. Defaulting to just under the keyless ceiling means ordinary use
	// paces itself instead of walking into HTTP 429 on the third command;
	// set --rate-limit 1 (60/min) when DISCOGS_TOKEN is configured.
	rootCmd.PersistentFlags().Float64Var(&flags.rateLimit, "rate-limit", defaultRateLimitPerSecond,
		"Max requests per second (0 to disable). Default paces under Discogs' keyless 25-per-minute limit")
	...
	rootCmd.AddCommand(newMastersCmd(flags))
	...
	rootCmd.AddCommand(newNovelDigCmd(flags))
	rootCmd.AddCommand(newNovelFloorCmd(flags))
	rootCmd.AddCommand(newNovelShelfCmd(flags))
	rootCmd.AddCommand(newNovelSpinCmd(flags))
	rootCmd.AddCommand(newCrateSyncCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newDatabasePromotedCmd(flags))
	rootCmd.AddCommand(newIdentityPromotedCmd(flags))
	rootCmd.AddCommand(newUsersPromotedCmd(flags))
	rootCmd.AddCommand(newWantlistPromotedCmd(flags))
