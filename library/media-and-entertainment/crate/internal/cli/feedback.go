...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CRATE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CRATE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "crate-pp-cli/feedback")
...
When ` + "`CRATE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CRATE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  crate-pp-cli feedback list
  crate-pp-cli feedback list --limit 5
  crate-pp-cli feedback list --json`,
