...
Resource scoping:
  ...
  crate-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  crate-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  crate-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  crate-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  crate-pp-cli sync --latest-only`,
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		"artists":    "/artists/{artist_id}/releases",
		"collection": "/users/{username}/collection/folders/{folder_id}/releases",
		"identity":   "/oauth/identity",
		"labels":     "/labels/{label_id}/releases",
		"masters":    "/masters/{master_id}/versions",
