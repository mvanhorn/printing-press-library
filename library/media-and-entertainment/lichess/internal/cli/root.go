...
type rootFlags struct {
	...
	// invocation. Mirrors the LICHESS_NO_LEARN env var.
	noLearn       bool
	selectFields  string
	configPath    string
	homePath      string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "lichess-pp-cli",
		Short: "Manage lichess resources via the lichess API",
		Long: `Lichess CLI
...
Health check: run 'lichess-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("lichess-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'lichess-pp-cli profile list')")
	...
	rootCmd.AddCommand(newPuzzleCmd(flags))
	...
	rootCmd.AddCommand(newNovelLossPatternsCmd(flags))
	rootCmd.AddCommand(newNovelTrainingBriefCmd(flags))
	rootCmd.AddCommand(newAccountPromotedCmd(flags))
	rootCmd.AddCommand(newChallengeCmd(flags))
