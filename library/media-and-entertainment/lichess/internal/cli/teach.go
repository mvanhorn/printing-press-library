...
// lichess-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
const noLearnEnvVar = "LICHESS_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("lichess-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  lichess-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  lichess-pp-cli recall "<question>" --agent
  lichess-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  lichess-pp-cli learnings list --agent
  lichess-pp-cli learnings list --query "<substring>"
  lichess-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  lichess-pp-cli learnings forget "<question>" --resource <id>
  lichess-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  lichess-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  lichess-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
