...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Lichess Org Reference",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  lichess-pp-cli auth setup\n  lichess-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Register an app and copy your credentials at: https://discord.gg/lichess")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then run:")
			fmt.Fprintln(w, "  lichess-pp-cli login")
			if !launch {
				return nil
			}
			launchURL := "https://discord.gg/lichess"
...
func newAuthLoginCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  lichess-pp-cli auth login --dry-run",
	...
	cmd.Flags().StringVar(&clientID, "client-id", os.Getenv("LICHESS_CLIENT_ID"), "OAuth2 client ID")
	cmd.Flags().StringVar(&clientSecret, "client-secret", os.Getenv("LICHESS_CLIENT_SECRET"), "OAuth2 client secret")
...
func runOAuthLogin(cmd *cobra.Command, flags *rootFlags, clientID, clientSecret string, port int) error {
	...
	// This CLI only challenges a named player after --send and reads the
	// signed-in user's puzzle dashboard. Keep OAuth consent to those actions;
	// in particular never request Board/Bot play, engine, message, or bulk
	// challenge scopes.
	scopes := []string{"challenge:write", "puzzle:read"}
	if len(scopes) > 0 {
...
func resolveOAuthCredentials(cmd *cobra.Command, flags *rootFlags, cfg *config.Config, clientID, clientSecret string) (string, string, error) {
	...
	if flags.noInput {
		return "", "", fmt.Errorf("OAuth2 client ID is required; pass --client-id, set LICHESS_CLIENT_ID, or run 'lichess-pp-cli login' without --no-input")
	}
...
func printOAuthCredentialHint(w io.Writer) {
	fmt.Fprintln(w, "Create OAuth credentials at: https://discord.gg/lichess")
}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				fmt.Fprintf(w, "  %s Not authenticated. Run 'lichess-pp-cli login' to authenticate.\n", red("FAIL"))
				return nil
			}
			...
			if cfg.TokenExpiry.IsZero() {
				...
				if cfg.RefreshToken != "" {
					...
					fmt.Fprintf(w, "  %s Token expired. Run 'lichess-pp-cli login' to re-authenticate.\n", red("FAIL"))
				}
			}
