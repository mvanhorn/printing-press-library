...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("LICHESS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("LICHESS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "lichess-pp-cli/feedback")
...
When ` + "`LICHESS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`LICHESS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  lichess-pp-cli feedback list
  lichess-pp-cli feedback list --limit 5
  lichess-pp-cli feedback list --json`,
