...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  lichess-pp-cli doctor
  lichess-pp-cli doctor --json
  lichess-pp-cli doctor --fail-on warn
  lichess-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "lichess-pp-cli login"
					report["auth_key_url"] = "https://lichess.org/account/oauth/token"
				} else {
					authConfigured = true
			...
			if os.Getenv("LICHESS_OAUTH2") != "" {
				authEnvSet = append(authEnvSet, "LICHESS_OAUTH2")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "LICHESS_OAUTH2")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "lichess-pp-cli"
						verifyPath := "/api/account"
