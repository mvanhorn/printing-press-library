...
// PostForm submits form fields through the normal authenticated, rate-limited
// mutation path. Some APIs intentionally use form-encoded writes rather than
// JSON bodies.
func (c *Client) PostForm(ctx context.Context, path string, form url.Values) (json.RawMessage, int, error) {
	return c.do(ctx, "POST", path, nil, form, nil)
}
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "lichess-pp-cli/2.0.153")
		}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "lichess-pp-cli auth login")
}
...
func isBinaryResponseContentType(ct string) bool {
	...
	switch {
	case mt == "application/json", mt == "text/json", mt == "application/x-ndjson", mt == "application/ndjson", mt == "*/*":
		return false
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.LichessOauth2)
	}
