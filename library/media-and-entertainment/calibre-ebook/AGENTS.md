# Calibre Ebook Printed CLI Agent Guide

This directory is a generated `calibre-ebook-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
calibre-ebook-pp-cli doctor --json
calibre-ebook-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
calibre-ebook-pp-cli which "<capability>" --json
calibre-ebook-pp-cli <command> --help
```
...
```bash
calibre-ebook-pp-cli <command> --agent
```
...
```bash
calibre-ebook-pp-cli <command> --help
calibre-ebook-pp-cli <command> --dry-run --agent
```
