...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".calibre-ebook-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CALIBRE_EBOOK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CALIBRE_EBOOK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/media-and-entertainment/calibre-ebook/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.calibre-ebook-pp-cli/feedback.jsonl.
When ` + "`CALIBRE_EBOOK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CALIBRE_EBOOK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  calibre-ebook-pp-cli feedback list
  calibre-ebook-pp-cli feedback list --limit 5
  calibre-ebook-pp-cli feedback list --json`,
