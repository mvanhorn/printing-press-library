...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  calibre-ebook-pp-cli doctor
  calibre-ebook-pp-cli doctor --json
  calibre-ebook-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if c, clientErr := flags.newClient(); clientErr == nil {
				...
				if found == len(binaries) {
					...
					report["calibre"] = fmt.Sprintf("partial (%d/%d binaries found)", found, len(binaries))
				}
				if c.LibraryPath != "" {
					if fi, err := os.Stat(c.LibraryPath); err == nil && fi.IsDir() {
						report["library"] = "ok"
						report["library_path"] = c.LibraryPath
					} else {
						report["library"] = fmt.Sprintf("not found: %s", c.LibraryPath)
					}
				} else {
					report["library"] = "not configured (use --library-path or CALIBRE_LIBRARY_PATH)"
				}
			}
			...
			for _, key := range []string{"config_path", "base_url", "library_path", "auth_source", "version"} {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'calibre-ebook-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'calibre-ebook-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'calibre-ebook-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'calibre-ebook-pp-cli sync' to refresh."
	}
	return report
}
