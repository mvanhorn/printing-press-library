...
func dbLockErr(err error) error    { return &cliError{code: 6, err: err} }
func calibreNotFoundErr(err error) error { return &cliError{code: 11, err: err} }
...
func classifyAPIError(err error, flags *rootFlags) error {
	msg := err.Error()
	switch {
	case strings.Contains(msg, "calibredb") && strings.Contains(msg, "not found"):
		return calibreNotFoundErr(fmt.Errorf("%w\nhint: install Calibre or set CALIBRE_BIN_DIR. Run 'doctor' to check.", err))
	case strings.Contains(msg, "is currently running") || strings.Contains(msg, "locked") || strings.Contains(msg, "database is locked"):
		return dbLockErr(fmt.Errorf("%w\nhint: close the Calibre GUI before write operations. Run 'doctor' to verify.", err))
	case strings.Contains(msg, "No book with id"):
		return notFoundErr(fmt.Errorf("%w\nhint: run 'books list --agent' to see valid IDs.", err))
	...
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API credentials."+
			"\n      Run 'calibre-ebook-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: permission denied. This API is configured without credentials, so the service may be blocking the request by rate limit, geography, bot protection, or endpoint policy."+
			"\n      Run 'calibre-ebook-pp-cli doctor' to check connectivity.", err))
