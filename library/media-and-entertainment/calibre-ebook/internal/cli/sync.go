...
Resource scoping:
  ...
  calibre-ebook-pp-cli sync

  # Sync specific resources only
  calibre-ebook-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  calibre-ebook-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  calibre-ebook-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  calibre-ebook-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  calibre-ebook-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "embed_metadata":
		return db.UpsertEmbedMetadata(data)
	case "formats":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"library-custom-columns": "/library/custom-columns",
		"library-saved-searches": "/library/saved-searches",
	}
