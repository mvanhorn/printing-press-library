...
func newWorkflowCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.AddCommand(newWorkflowHealthAuditCmd(flags))
...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  calibre-ebook-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  calibre-ebook-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"books", "fts", "library", "library-custom-columns", "library-saved-searches"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  calibre-ebook-pp-cli workflow status

  # Show status as JSON
  calibre-ebook-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("calibre-ebook-pp-cli")
			}
