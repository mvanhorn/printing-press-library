...
type rootFlags struct {
	...
	resourceTTLs map[string]string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "calibre-ebook-pp-cli",
		Short: "Manage calibre-ebook resources via the calibre-ebook API",
		Long: `Manage calibre-ebook resources via the calibre-ebook API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'calibre-ebook-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("calibre-ebook-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'calibre-ebook-pp-cli profile list')")
	rootCmd.PersistentFlags().StringVar(&flags.deliverSpec, "deliver", "", "Route output to a sink: stdout (default), file:<path>, webhook:<url>")
	rootCmd.PersistentFlags().Float64Var(&flags.rateLimit, "rate-limit", 0, "Max requests per second (0 to disable)")
	rootCmd.PersistentFlags().StringVar(&flags.libraryPath, "library-path", "", "Path to Calibre library (overrides config and CALIBRE_LIBRARY_PATH)")
	...
	rootCmd.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
		...
		flags.resourceTTLs = map[string]string{
			"books":                  "5m",
			"book_metadata":         "10m",
			"library_categories":    "30m",
			"library_check":         "15m",
			"fts_index":             "1h",
			"library_custom_columns": "1h",
			"library_saved_searches": "30m",
		}
		flags.freshnessMeta = map[string]any{
			"ttl_seconds": map[string]int{
				"books":    300,
				"metadata": 600,
				"categories": 1800,
				"check":    900,
	...
	rootCmd.AddCommand(newFileMetaPromotedCmd(flags))
	rootCmd.AddCommand(newPolishPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	rootCmd.AddCommand(newLibraryHealthScoreCmd(flags))
	rootCmd.AddCommand(newLibraryDuplicatesCmd(flags))
	rootCmd.AddCommand(newLibrarySeriesGapsCmd(flags))
	rootCmd.AddCommand(newLibraryStatsCmd(flags))
	rootCmd.AddCommand(newLibrarySimilarCmd(flags))
	rootCmd.AddCommand(newWorkflowHealthAuditCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("calibre-ebook-pp-cli %s\n", version)
		},
	}
}
