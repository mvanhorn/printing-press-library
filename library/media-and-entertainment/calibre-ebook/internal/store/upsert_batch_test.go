...
// TestUpsertBatch_PopulatesEmbedMetadataTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed embed_metadata table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesEmbedMetadataTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "books_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "books_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "books_id": "test-parent-001"}`),
	}
	if _, _, err := s.UpsertBatch("embed_metadata", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "embed_metadata").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "embed_metadata")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count embed_metadata: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("embed_metadata count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandEmbedMetadataGeneric exercises
...
func TestUpsertBatch_TypedFailureDoesNotStrandEmbedMetadataGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// Items deliberately omit "books_id" so the typed NOT NULL
	// constraint fires.
	items := []json.RawMessage{
	...
	stored, extractFailures, err := s.UpsertBatch("embed_metadata", items)
	if err != nil {
	...
	if typed != 0 {
		t.Fatalf("embed_metadata count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "books_id")
	}
}
...
// TestUpsertBatch_TypedFailureDoesNotStrandFormatsGeneric exercises
// the savepoint isolation around the typed-table dispatch. The fixture omits
// the NOT NULL parent FK column so the typed insert fails; the savepoint
// rolls back only the typed projection. The generic resources row inserted
// just before must survive. Regression for issue #1392, where a single
// outer transaction caused typed-table failures to cascade and silently
// discard every successfully fetched API row.
func TestUpsertBatch_TypedFailureDoesNotStrandFormatsGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	stored, extractFailures, err := s.UpsertBatch("formats", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v (typed-table failure must not propagate)", err)
	}
	if stored != len(items) {
	...
	if typed != 0 {
		t.Fatalf("formats count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "books_id")
	}
}
