...
// TestMigrate_AddsColumnsOnUpgrade_EmbedMetadata verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_EmbedMetadata(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "embed_metadata" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	rows, err := s.DB().Query(`PRAGMA table_info("embed_metadata")`)
	if err != nil {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from embed_metadata after migrate", want)
		}
	}
}
