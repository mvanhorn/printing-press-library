...
description: "Printing Press CLI for Drivethrurpg. Curated seed spec for public DriveThruRPG catalog search and authenticated library downloads."
author: "Jason Holt"
license: "Apache-2.0"
...
1. Install via the Printing Press installer into a user bin directory:
   ```bash
   npx -y @mvanhorn/printing-press-library install drivethrurpg --cli-only --bin-dir ~/.local/bin
   ```
...
**order-products** — Manage order products
...
## Auth Setup
Run `drivethrurpg-pp-cli auth setup` to print the URL and steps for getting a key (add `--launch` to open the URL). Then set:

```bash
export DRIVETHRURPG_DTRPG_TOKEN="<your-key>"
```

Or persist it in `~/.config/drivethrurpg-pp-cli/config.toml`.

Run `drivethrurpg-pp-cli doctor` to verify setup.
...
Entries are stored locally at `~/.local/share/drivethrurpg-pp-cli/feedback.jsonl`. They are never POSTed unless `DRIVETHRURPG_FEEDBACK_ENDPOINT` is set AND either `--send` is passed or `DRIVETHRURPG_FEEDBACK_AUTO_SEND=true`. Default behavior is local-only.
