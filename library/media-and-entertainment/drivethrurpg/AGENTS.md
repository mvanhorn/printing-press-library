# Drivethrurpg Printed CLI Agent Guide

This directory is a generated `drivethrurpg-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
drivethrurpg-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
drivethrurpg-pp-cli which "<capability>" --json
drivethrurpg-pp-cli <command> --help
```
...
```bash
drivethrurpg-pp-cli <command> --agent
```
...
```bash
drivethrurpg-pp-cli <command> --help
drivethrurpg-pp-cli <command> --dry-run --agent
```
