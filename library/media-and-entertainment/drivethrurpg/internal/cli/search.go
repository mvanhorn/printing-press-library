...
In local mode: searches locally synced data only.`,
  ...
  drivethrurpg-pp-cli search "Legend of the Five Rings core rulebook"

  # Force local search only
  drivethrurpg-pp-cli search "Cyberpunk" --data-source local

  # Search a specific resource type locally
  drivethrurpg-pp-cli search "Mothership" --type products --data-source local

  # JSON output for piping
  drivethrurpg-pp-cli search "Cyberpunk" --json --limit 20 --data-source live`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /products
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get("/products", map[string]string{
					"keyword": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'drivethrurpg-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "":
				results, err = db.SearchOrderProducts(query, limit)
				if err == nil && len(results) < limit {
					var generic []json.RawMessage
					generic, err = db.Search(query, limit-len(results))
					results = appendUniqueSearchResults(results, generic, limit)
				}
			case "library", "order-products", "order_products":
				results, err = db.SearchOrderProducts(query, limit)
			default:
				results, err = db.SearchResource(strings.ReplaceAll(resourceType, "_", "-"), query, limit)
			}
			if err != nil {
...
func appendUniqueSearchResults(base, extra []json.RawMessage, limit int) []json.RawMessage {
	if limit <= 0 {
		limit = 50
	}
	seen := map[string]bool{}
	results := make([]json.RawMessage, 0, min(limit, len(base)+len(extra)))
...
func searchResultKey(item json.RawMessage) string {
	var obj map[string]any
	if json.Unmarshal(item, &obj) != nil {
		return ""
	}
	for _, key := range []string{"id", "product_id", "order_product_id", "name", "title"} {
