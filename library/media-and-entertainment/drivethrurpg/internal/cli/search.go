...
In local mode: searches locally synced data only.`,
  ...
  drivethrurpg-pp-cli search "error timeout"

  # Force local search only
  drivethrurpg-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  drivethrurpg-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  drivethrurpg-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.dataSource != "local" {
				...
				data, getErr := c.Get(cmd.Context(), "/products", map[string]string{
					"keyword": query,
				})
				if getErr == nil {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/github.com/mvanhorn/printing-press-library/library/media-and-entertainment/drivethrurpg/data.db)")
