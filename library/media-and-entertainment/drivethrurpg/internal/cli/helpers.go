...
// isTerminal provides isatty-style detection without adding a cgo dependency.
...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			"\n      Preferred: export DRIVETHRURPG_APPLICATION_KEY=<application-key> and run 'drivethrurpg-pp-cli auth login'."+
			"\n      Advanced: export DRIVETHRURPG_DTRPG_TOKEN=<exchanged-token>."+
			"\n      Run 'drivethrurpg-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API key."+
			" Set DRIVETHRURPG_APPLICATION_KEY or log in with 'drivethrurpg-pp-cli auth login'."+
			"\n      Run 'drivethrurpg-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: permission denied. Your credentials are valid but lack access to this resource."+
			"\n      Check that your DriveThruRPG Application Key has My Library Access enabled."+
			"\n      Set DRIVETHRURPG_APPLICATION_KEY or an exchanged DRIVETHRURPG_DTRPG_TOKEN."+
			"\n      Run 'drivethrurpg-pp-cli doctor' to check auth status.", err))
