...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  drivethrurpg-pp-cli doctor
  ...
  drivethrurpg-pp-cli doctor --fail-on error`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" && cfg.ApplicationKey == "" {
					report["auth"] = "not configured"
					report["auth_hint"] = "export DRIVETHRURPG_APPLICATION_KEY=<your-application-key>"
				} else {
					report["auth"] = "configured"
					report["auth_source"] = cfg.AuthSource
					if header == "" && cfg.ApplicationKey != "" {
						report["auth_mode"] = "application_key_exchange"
					}
				}
			}
			...
			if os.Getenv("DRIVETHRURPG_DTRPG_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "DRIVETHRURPG_DTRPG_TOKEN")
			} else if os.Getenv("DRIVETHRURPG_APPLICATION_KEY") != "" {
				authEnvSet = append(authEnvSet, "DRIVETHRURPG_APPLICATION_KEY")
				authEnvInfo = append(authEnvInfo, "DRIVETHRURPG_APPLICATION_KEY set; token will be exchanged on demand")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "DRIVETHRURPG_APPLICATION_KEY or DRIVETHRURPG_DTRPG_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" && cfg.ApplicationKey != "" {
						authHeader = "application-key-configured"
					}
					if authHeader == "" {
						// No auth configured — skip credential validation
					} else if cfg.AuthHeader() == "" && cfg.ApplicationKey != "" {
						report["credentials"] = "skipped (application key exchange is validated by auth login or first authenticated command)"
						...
						authHeaders["User-Agent"] = "drivethrurpg-pp-cli"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'drivethrurpg-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'drivethrurpg-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'drivethrurpg-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'drivethrurpg-pp-cli sync' to refresh."
	}
	return report
}
