...
func TestUpsertBatch_PopulatesOrderProductsTable(t *testing.T) {
	...
	if _, _, err := s.UpsertBatch("order_products", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "order_products").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
...
func TestUpsertBatch_PopulatesOrderProductsTableFromSyncResourceName(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "name": "Legend of the Five Rings Core Rulebook"}`),
		json.RawMessage(`{"id": "test-002", "name": "Cyberpunk RED"}`),
	}
	...
	var typed int
	if err := s.DB().QueryRow(`SELECT COUNT(*) FROM order_products`).Scan(&typed); err != nil {
		t.Fatalf("count order_products: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("order_products count = %d, want %d", typed, len(items))
	}

	results, err := s.SearchOrderProducts("five rings", 10)
	if err != nil {
		t.Fatalf("SearchOrderProducts: %v", err)
	}
	if len(results) != 1 {
		t.Fatalf("SearchOrderProducts returned %d results, want 1", len(results))
	}
}
