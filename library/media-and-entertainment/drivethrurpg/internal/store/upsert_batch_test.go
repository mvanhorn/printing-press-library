...
// TestUpsertBatch_PopulatesOrderProductsTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed order_products table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesOrderProductsTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("order-products", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "order-products").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "order_products")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count order_products: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("order_products count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
...
func TestUpsertBatch_PopulatesCheckTable(t *testing.T) {
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "order_products_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "order_products_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "order_products_id": "test-parent-001"}`),
	}
...
// TestUpsertBatch_TypedFailureDoesNotStrandCheckGeneric exercises
...
func TestUpsertBatch_TypedFailureDoesNotStrandCheckGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// Items deliberately omit "order_products_id" so the typed NOT NULL
	// constraint fires.
	items := []json.RawMessage{
	...
	stored, extractFailures, err := s.UpsertBatch("check", items)
	if err != nil {
	...
	if typed != 0 {
		t.Fatalf("check count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "order_products_id")
	}
}

// TestUpsertBatch_PopulatesPrepareTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed prepare table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesPrepareTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "prepare").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "prepare")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count prepare: %v", err)
	}
	if typed != len(items) {
...
// TestUpsertBatch_TypedFailureDoesNotStrandPrepareGeneric exercises
// the savepoint isolation around the typed-table dispatch. The fixture omits
// the NOT NULL parent FK column so the typed insert fails; the savepoint
// rolls back only the typed projection. The generic resources row inserted
// just before must survive. Regression for issue #1392, where a single
// outer transaction caused typed-table failures to cascade and silently
// discard every successfully fetched API row.
func TestUpsertBatch_TypedFailureDoesNotStrandPrepareGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	stored, extractFailures, err := s.UpsertBatch("prepare", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v (typed-table failure must not propagate)", err)
	}
	if stored != len(items) {
	...
	if typed != 0 {
		t.Fatalf("prepare count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "order_products_id")
	}
}
