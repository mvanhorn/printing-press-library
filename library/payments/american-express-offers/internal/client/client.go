...
func newHTTPClient(timeout time.Duration, jar http.CookieJar) *http.Client {
	// Use http.DefaultTransport unmodified so HTTP/2 (ALPN) is negotiated normally.
	// The Amex API requires h2; disabling TLSNextProto causes the server to send
	// HTTP/2 SETTINGS frames on what the client thinks is an HTTP/1.1 connection.
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "american-express-offers-pp-cli", "http")
	...
	httpClient.CheckRedirect = func(req *http.Request, via []*http.Request) error {
		...
		if req.URL.Host == via[0].URL.Host {
			if h, err := c.authHeader(req.Context()); err == nil && h != "" {
				payload := strings.TrimPrefix(h, "Bearer ")
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if authHeader != "" {
			// Cookie auth stores a cookie jar string (name=val; name2=val2...) in
			// AccessToken. Detect by semicolons and send as Cookie header instead
			// of Authorization so the Amex API receives it correctly.
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "american-express-offers-pp-cli auth set-token <token>")
}
