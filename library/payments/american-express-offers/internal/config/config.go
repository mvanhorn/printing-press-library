...
type Config struct {
	...
	AccountProxy  string            `toml:"account_proxy"`
	Path          string            `toml:"-"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://functions.americanexpress.com",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("AMERICAN_EXPRESS_OFFERS_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "american-express-offers-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("AMERICAN_EXPRESS_OFFERS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) SaveAccountProxy(proxy string) error {
	c.AccountProxy = proxy
	return c.save()
}

func (c *Config) SaveHeader(key, value string) error {
	if c.Headers == nil {
