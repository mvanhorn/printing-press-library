...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AMERICAN_EXPRESS_OFFERS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AMERICAN_EXPRESS_OFFERS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "american-express-offers-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.american-express-offers-pp-cli/feedback.jsonl.
When ` + "`AMERICAN_EXPRESS_OFFERS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AMERICAN_EXPRESS_OFFERS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "american-express-offers-pp-cli",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  american-express-offers-pp-cli feedback list
  american-express-offers-pp-cli feedback list --limit 5
  american-express-offers-pp-cli feedback list --json`,
