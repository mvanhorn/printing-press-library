...
This directory is a generated `stripe-pp-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
stripe-pp-pp-cli doctor --json
stripe-pp-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
stripe-pp-pp-cli which "<capability>" --json
stripe-pp-pp-cli <command> --help
```
...
```bash
stripe-pp-pp-cli <command> --agent
```
...
```bash
stripe-pp-pp-cli <command> --help
stripe-pp-pp-cli <command> --dry-run --agent
```
