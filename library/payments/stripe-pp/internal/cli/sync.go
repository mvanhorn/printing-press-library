...
Exit codes & warnings:
  ...
  stripe-pp-pp-cli sync

  # Sync specific resources only
  stripe-pp-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  stripe-pp-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  stripe-pp-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  stripe-pp-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  stripe-pp-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("stripe-pp-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/stripe-pp-pp-cli/data.db)")
...
//
// Stripe's list endpoints accept the bracket-style parameter `created[gte]`
// (and friends) for temporal filtering. The generator's OpenAPI inspector
// doesn't recognize bracket-suffixed parameter names as a temporal filter,
// so the switch is populated explicitly here for the resources that the
// `sync` command operates on. See output-review finding from polish.
