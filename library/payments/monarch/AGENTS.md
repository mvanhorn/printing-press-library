...
This directory is a generated `monarch-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
monarch-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
monarch-pp-cli which "<capability>" --json
monarch-pp-cli <command> --help
```
...
```bash
monarch-pp-cli <command> --help
monarch-pp-cli <command> --dry-run --agent
```
