...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".monarch-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MONARCH_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MONARCH_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "monarch-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.monarch-pp-cli/feedback.jsonl.
When ` + "`MONARCH_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MONARCH_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  monarch-pp-cli feedback list
  monarch-pp-cli feedback list --limit 5
  monarch-pp-cli feedback list --json`,
