...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Monarch",
	}
...
// newAuthSetTokenCmd persists a manually-pasted bearer token to the config
// file. Kept alongside the chrome-driven `auth login` flow so users with a
// pre-captured token (env var, headless CI, manual DevTools paste) don't have
// to drive a browser to authenticate.
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		Use:     "set-token <token>",
		Short:   "Save an API token to the config file",
		Example: "  monarch-pp-cli auth set-token YOUR_TOKEN_HERE",
...
Use --chrome to read cookies from Chrome for .monarch.com.
...
profile by name when the installed backend supports it.`,
		Example: `  monarch-pp-cli auth login --chrome
  monarch-pp-cli auth login --browser
  monarch-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !browserFlag {
				fmt.Fprintln(cmd.OutOrStdout(), "Use --chrome or --browser to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  monarch-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  monarch-pp-cli auth login --browser\n")
				return nil
			}
			// Check if already authenticated via env var
			if v := os.Getenv("MONARCH_TOKEN"); v != "" {
			...
			if cookieToolSupportsProfiles(tool) {
				...
				if err != nil {
					...
					fmt.Fprintf(w, "\n  monarch-pp-cli auth login --chrome\n")
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  monarch-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  monarch-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: .monarch.com\n")
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  monarch-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
...
func detectCookieTool() (string, error) {
	// Modern pycookiecheat installs (pipx, uv, system pip with PEP 668)
	// expose a CLI binary on PATH but aren't importable from the system
	// `python3`. The standalone-binary check has to come before the
	// `python3 -c "import pycookiecheat"` probe so isolated installs are
	// detected; otherwise users see "no cookie tool found" despite a
	// working install.
	tools := []struct {
		name  string
		check []string
	}{
		{"pycookiecheat", []string{"pycookiecheat", "--version"}},
	...
	return "", fmt.Errorf("no cookie extraction tool found; install one: pipx install pycookiecheat")
}
...
func extractViaPycookiecheat(domain, profileDir string) (string, error) {
	...
	// Prefer the standalone CLI binary so pipx/uv/PEP-668 isolated installs
	// work. Fall back to the in-process Python import path for legacy installs.
	if _, err := exec.LookPath("pycookiecheat"); err == nil {
		args := []string{"-b", "chrome", "https://" + cleanDomain}
		if cookiePath != "" {
			args = []string{"-b", "chrome", "-c", cookiePath, "https://" + cleanDomain}
		}
...
func validateComposedAuth(authHeader string) error {
	...
	if req.Header.Get("User-Agent") == "" {
		req.Header.Set("User-Agent", "monarch-pp-cli/0.1.0")
	}
