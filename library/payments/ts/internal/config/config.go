...
type Config struct {
	...
	TsBearerToken string            `toml:"bearer_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("TS_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "ts-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("TS_BEARER_TOKEN"); v != "" {
		cfg.TsBearerToken = v
		cfg.markEnvOverride("TsBearerToken")
		cfg.AuthSource = "env:TS_BEARER_TOKEN"
	}
	if v := os.Getenv("TS_CLIENT_ID"); v != "" {
		cfg.ClientID = v
		cfg.markEnvOverride("ClientID")
		cfg.AuthSource = "env:TS_CLIENT_ID"
	}
	if v := os.Getenv("TS_CLIENT_SECRET"); v != "" {
		cfg.ClientSecret = v
		cfg.markEnvOverride("ClientSecret")
		cfg.AuthSource = "env:TS_CLIENT_SECRET"
	}
	...
	if cfg.AuthSource == "" && cfg.TsBearerToken != "" {
		cfg.AuthSource = "config"
	}
...
func (c *Config) AuthHeader() string {
	...
	if c.TsBearerToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:TS_BEARER_TOKEN"
		}
		return "Bearer " + c.TsBearerToken
	}
	return ""
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.TsBearerToken = ""
	delete(c.envOverrides, "TsBearerToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["TsBearerToken"] {
			out.TsBearerToken = c.fileConfig.TsBearerToken
		}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "TsBearerToken":
		c.fileConfig.TsBearerToken = c.TsBearerToken
	}
}
