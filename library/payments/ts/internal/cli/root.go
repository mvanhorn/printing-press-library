...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "ts-pp-cli",
		Short: `Ts CLI — Every TreasurySpring read endpoint, plus a local SQLite mirror that answers portfolio-level questions no single API cal…`,
		Long: `Ts CLI — Every TreasurySpring read endpoint, plus a local SQLite mirror that answers portfolio-level questions no single API cal…
...
Health check: run 'ts-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("ts-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'ts-pp-cli profile list')")
	...
	rootCmd.AddCommand(newHoldingCmd(flags))
	...
	rootCmd.AddCommand(newNovelConcentrationCmd(flags))
	rootCmd.AddCommand(newNovelLadderCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newCellPromotedCmd(flags))
	rootCmd.AddCommand(newHealthPromotedCmd(flags))
	rootCmd.AddCommand(newHolidaysPromotedCmd(flags))
	rootCmd.AddCommand(newIndicationPromotedCmd(flags))
	rootCmd.AddCommand(newOauthPromotedCmd(flags))
	rootCmd.AddCommand(newObligorExposurePromotedCmd(flags))
	rootCmd.AddCommand(newSubscribePromotedCmd(flags))
	rootCmd.AddCommand(newSubscriptionPromotedCmd(flags))
