...
func newEventCheckpointCheckpoint3Cmd(flags *rootFlags) *cobra.Command {
	var flagBodyJSON string
	var stdinBody bool

	cmd := &cobra.Command{
		Use:         "checkpoint-checkpoint-3 <name>",
		Short:       "Create a new named event checkpoint, or return the existing one if it already exists.",
		Example:     "  ts-pp-cli event checkpoint-checkpoint-3 example-resource",
		Annotations: map[string]string{"pp:endpoint": "event.checkpoint-checkpoint-3", "pp:method": "PUT", "pp:path": "/event/checkpoint/{name}"},
