...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  ts-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  ts-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"entity", "event", "event-checkpoint", "holding", "subscription", "task"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  ts-pp-cli workflow status

  # Show status as JSON
  ts-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("ts-pp-cli")
			}
