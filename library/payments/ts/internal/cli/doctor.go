...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ts-pp-cli doctor
  ts-pp-cli doctor --json
  ts-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export TS_BEARER_TOKEN=<your-key>"
					report["auth_docs_url"] = "https://treasuryspring.com/"
				} else {
					authConfigured = true
			...
			if strings.Contains("Pre-minted bearer token. OR set TS_CLIENT_ID + TS_CLIENT_SECRET and run `ts auth login`.", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "TS_BEARER_TOKEN")
				if os.Getenv("TS_BEARER_TOKEN") != "" {
					authEnvSet = append(authEnvSet, "TS_BEARER_TOKEN")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("TS_BEARER_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "TS_BEARER_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "TS_BEARER_TOKEN optional")
			}
			if os.Getenv("TS_CLIENT_ID") != "" {
				authEnvSet = append(authEnvSet, "TS_CLIENT_ID")
			} else {
				authEnvInfo = append(authEnvInfo, "TS_CLIENT_ID set during auth login")
			}
			if os.Getenv("TS_CLIENT_SECRET") != "" {
				authEnvSet = append(authEnvSet, "TS_CLIENT_SECRET")
			} else {
				authEnvInfo = append(authEnvInfo, "TS_CLIENT_SECRET set during auth login")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "ts-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'ts-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'ts-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'ts-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'ts-pp-cli sync' to refresh."
	}
	return report
}
