...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "ts-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ts-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/ts-pp-cli/feedback.jsonl.
When ` + "`TS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ts-pp-cli feedback list
  ts-pp-cli feedback list --limit 5
  ts-pp-cli feedback list --json`,
