...
In local mode: searches locally synced data only.`,
  ...
  ts-pp-cli search "error timeout"

  # Force local search only
  ts-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  ts-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  ts-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'ts-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "obligor-exposure":
				results, err = db.SearchObligorExposure(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchObligorExposure(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search obligor_exposure failed: %w", searchErr)
					}
