...
Resource scoping:
  ...
  ts-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  ts-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  ts-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  ts-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  ts-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/ts-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "holding":
		return db.UpsertHolding(data)
	case "maturity_action":
		return db.UpsertMaturityAction(data)
	case "holidays":
		return db.UpsertHolidays(data)
	case "indication":
		return db.UpsertIndication(data)
	case "obligor-exposure":
		return db.UpsertObligorExposure(data)
	case "subscribe":
		return db.UpsertSubscribe(data)
	case "subscription":
