...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Treasuryspring Public",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  ts-pp-cli auth setup\n  ts-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "See API docs: https://treasuryspring.com/")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export TS_BEARER_TOKEN=\"<your-token>\"")
			fmt.Fprintln(w, "  ts-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://treasuryspring.com/"
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export TS_BEARER_TOKEN=\"your-token-here\" # Pre-minted bearer token. OR set TS_CLIENT_ID + TS_CLIENT_SECRET and run `ts auth login`.")
				fmt.Fprintf(w, "  ts-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  ts-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("TS_BEARER_TOKEN") != "" {
				envStillSet = "TS_BEARER_TOKEN"
			}
			if envStillSet == "" && os.Getenv("TS_CLIENT_ID") != "" {
				envStillSet = "TS_CLIENT_ID"
			}
			if envStillSet == "" && os.Getenv("TS_CLIENT_SECRET") != "" {
				envStillSet = "TS_CLIENT_SECRET"
			}
