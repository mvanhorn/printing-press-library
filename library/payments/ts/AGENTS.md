# TreasurySpring Printed CLI Agent Guide

This directory is a generated `ts-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
ts-pp-cli doctor --json
ts-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
ts-pp-cli which "<capability>" --json
ts-pp-cli <command> --help
```
...
```bash
ts-pp-cli <command> --help
ts-pp-cli <command> --dry-run --agent
```
