...
Authenticate with a bearer token from https://my.lunchmoney.app/developers. The CLI accepts the token via five env-var aliases — LUNCHMONEY_ACCESS_TOKEN (canonical, used by the official docs and the Python lunchable client), plus LUNCHMONEY_API_KEY, LUNCHMONEY_TOKEN, LUNCHMONEY_API_TOKEN (lunchtui), and LUNCH_MONEY_API_KEY (robshox MCP) — to match whichever spelling your ecosystem already uses. The same token works against the live API at api.lunchmoney.dev/v2 and the mock server at mock.lunchmoney.dev/v2.
...
```bash
claude mcp add lunch-money lunch-money-pp-mcp -e LUNCHMONEY_ACCESS_TOKEN=<your-token>
```
...
3. Fill in `LUNCHMONEY_ACCESS_TOKEN` when Claude Desktop prompts you.
...
```json
{
  "mcpServers": {
    "lunch-money": {
      ...
      "env": {
        "LUNCHMONEY_ACCESS_TOKEN": "<your-key>"
      }
    }
  }
}
...
| --- | --- | --- | --- |
| `LUNCHMONEY_ACCESS_TOKEN` | per_call | Yes | Set to your API credential. |
| `LUNCHMONEY_API_KEY` | per_call | Yes | Set to your API credential. |
| `LUNCHMONEY_TOKEN` | per_call | Yes | Set to your API credential. |
| `LUNCHMONEY_API_TOKEN` | per_call | Yes | Set to your API credential. |
| `LUNCH_MONEY_API_KEY` | per_call | Yes | Set to your API credential. |
...
- Verify the environment variable is set: `echo $LUNCHMONEY_ACCESS_TOKEN`
...
- **401 Unauthorized on any command** — Run `lunch-money-pp-cli auth status` to see which env var is in scope; if none, paste your token via `lunch-money-pp-cli auth set-token` or export LUNCHMONEY_ACCESS_TOKEN.
...
- **Multiple LUNCHMONEY_* env vars set** — The CLI picks LUNCHMONEY_ACCESS_TOKEN first, then the four aliases in fallback order; `auth status` prints which was used.
