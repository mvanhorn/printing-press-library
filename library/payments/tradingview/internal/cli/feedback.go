...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TRADINGVIEW_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TRADINGVIEW_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "tradingview-pp-cli/feedback")
...
When ` + "`TRADINGVIEW_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TRADINGVIEW_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  tradingview-pp-cli feedback list
  tradingview-pp-cli feedback list --limit 5
  tradingview-pp-cli feedback list --json`,
