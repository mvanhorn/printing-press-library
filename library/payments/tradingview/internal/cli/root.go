...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "tradingview-pp-cli",
		Short: `Tradingview CLI — Resolve any ticker and see its price in USD and EUR in one command — stocks, crypto, and forex, no API key.`,
		Long: `Tradingview CLI — Resolve any ticker and see its price in USD and EUR in one command — stocks, crypto, and forex, no API key.
...
Health check: run 'tradingview-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("tradingview-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'tradingview-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelConvertCmd(flags))
	...
	// Trim the MCP tool surface for this focused CLI. The generated MCP server
	// mirrors the whole Cobra tree; for a price/convert/watchlist CLI, the raw
	// endpoint mirrors (symbols, market) duplicate search/quote, the generic
	// sync is a no-op (no bulk-list endpoints), and workflow/api add tokens
	// without agent value. Hiding them keeps the agent-facing tool set lean
	// (search, quote, convert, watchlist, doctor) and cuts MCP startup tokens.
	for _, child := range rootCmd.Commands() {
		switch child.Name() {
		case "symbols", "market", "sync", "workflow", "api":
			if child.Annotations == nil {
				child.Annotations = map[string]string{}
			}
			child.Annotations["mcp:hidden"] = "true"
		}
	}
