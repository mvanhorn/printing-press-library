...
Resource scoping:
  ...
  tradingview-pp-cli sync

  # Sync specific resources only
  tradingview-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  tradingview-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  tradingview-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  tradingview-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  tradingview-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "tradingview-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
