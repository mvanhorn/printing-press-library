# TradingView Printed CLI Agent Guide

This directory is a generated `tradingview-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
tradingview-pp-cli doctor --json
tradingview-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
tradingview-pp-cli which "<capability>" --json
tradingview-pp-cli <command> --help
```
...
```bash
tradingview-pp-cli <command> --agent
```
...
```bash
tradingview-pp-cli <command> --help
tradingview-pp-cli <command> --dry-run --agent
```
