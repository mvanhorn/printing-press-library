...
In local mode: searches locally synced data only.`,
  ...
  partitaiva24-pp-cli search "error timeout"

  # Force local search only
  partitaiva24-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  partitaiva24-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  partitaiva24-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /tools/search-pa/{coduni}
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post("/tools/search-pa/{coduni}", map[string]any{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'partitaiva24-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/partitaiva24-pp-cli/data.db)")
