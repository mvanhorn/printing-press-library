...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".partitaiva24-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PARTITAIVA24_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PARTITAIVA24_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "partitaiva24-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.partitaiva24-pp-cli/feedback.jsonl.
When ` + "`PARTITAIVA24_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PARTITAIVA24_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  partitaiva24-pp-cli feedback list
  partitaiva24-pp-cli feedback list --limit 5
  partitaiva24-pp-cli feedback list --json`,
