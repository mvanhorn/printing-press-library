...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Partitaiva24",
	}
...
// fetchNonce fetches the X-WP-Nonce from the Partitaiva24 homepage HTML.
// WordPress embeds it as: var rest_nonce = "XXXXXXXXXX";
func fetchNonce(cookie string) (string, error) {
	req, err := http.NewRequest("GET", "https://partitaiva24.cloud/", nil)
	if err != nil {
		return "", err
	}
	req.Header.Set("Cookie", cookie)
	req.Header.Set("User-Agent", "partitaiva24-pp-cli/0.1.0")
	...
	if err != nil {
		return "", fmt.Errorf("reading homepage: %w", err)
	}

	re := regexp.MustCompile(`var rest_nonce\s*=\s*"([^"]+)"`)
	m := re.FindSubmatch(body)
	if m == nil {
		return "", fmt.Errorf("rest_nonce not found in page (are you logged in?)")
	}
	return string(m[1]), nil
}

// newAuthRefreshCmd reads the session cookie from Chrome and fetches the
// nonce from the page automatically — zero manual steps.
func newAuthRefreshCmd(flags *rootFlags) *cobra.Command {
	var profileFlag string

	cmd := &cobra.Command{
		Use:   "refresh",
		Short: "Re-read cookie from Chrome and auto-fetch the nonce",
		Long: `Reads the session cookie from your logged-in Chrome profile and
fetches the X-WP-Nonce from the Partitaiva24 homepage automatically.

Run this whenever you see rest_cookie_invalid_nonce — no DevTools needed.`,
		Example: `  partitaiva24-pp-cli auth refresh
  partitaiva24-pp-cli auth refresh --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			domain := ".partitaiva24.cloud"

			tool, err := detectCookieTool()
			if err != nil {
				fmt.Fprintln(w, red("No cookie extraction tool found."))
				fmt.Fprintln(w, "Install one of:")
				fmt.Fprintln(w, "  pip install pycookiecheat")
				fmt.Fprintln(w, "  brew install barnardb/cookies/cookies")
				fmt.Fprintln(w, "  cargo install cookie-scoop-cli")
				return authErr(err)
			}
			...
			if cookieToolSupportsProfiles(tool) {
				...
				if err != nil {
					...
					fmt.Fprintln(w, "Log in at https://partitaiva24.cloud/ and run again.")
					return authErr(err)
				}
			}

			fmt.Fprintln(w, "Reading cookies from Chrome...")
			cookies, err := extractCookies(tool, domain, profileDir)
			if err != nil {
				return authErr(fmt.Errorf("extracting cookies: %w", err))
			}
			if cookies == "" {
				return authErr(fmt.Errorf("no cookies found for %s — log in at https://partitaiva24.cloud/", domain))
			}

			fmt.Fprintln(w, "Fetching nonce from Partitaiva24...")
			nonce, err := fetchNonce(cookies)
			if err != nil {
				return authErr(fmt.Errorf("fetching nonce: %w", err))
			}
			...
			if err := cfg.SaveNonce(nonce); err != nil {
				return configErr(fmt.Errorf("saving nonce: %w", err))
			}

			fmt.Fprintf(w, "%s Cookie and nonce refreshed\n", green("OK"))
...
Use --chrome to read cookies from Chrome for .partitaiva24.cloud.
...
profile by name when the installed backend supports it.`,
		Example: `  partitaiva24-pp-cli auth login --chrome
  partitaiva24-pp-cli auth login --browser
  partitaiva24-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !browserFlag {
				fmt.Fprintln(cmd.OutOrStdout(), "Use --chrome or --browser to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  partitaiva24-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  partitaiva24-pp-cli auth login --browser\n")
				return nil
			}
			...
			// Step 4: Auto-fetch nonce from page HTML
			fmt.Fprintln(w, "Fetching nonce from Partitaiva24...")
			nonce, nonceErr := fetchNonce(cookies)
			...
			if nonceErr == nil && nonce != "" {
				...
				fmt.Fprintf(w, "%s Cookie and nonce saved (nonce: %s)\n", green("OK"), nonce)
			} else {
				count := len(strings.Split(cookies, ";"))
				fmt.Fprintf(w, "%s Found %d cookies for %s\n", green("OK"), count, domain)
				fmt.Fprintf(w, "  nonce: could not auto-fetch (%v) — run: auth set --nonce <value>\n", nonceErr)
			}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  partitaiva24-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  partitaiva24-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: .partitaiva24.cloud\n")
			if cfg.P24Nonce != "" {
				fmt.Fprintf(w, "  X-WP-Nonce: set (%d chars)\n", len(cfg.P24Nonce))
			} else if v := os.Getenv("P24_NONCE"); v != "" {
				fmt.Fprintf(w, "  X-WP-Nonce: set via env\n")
			} else {
				fmt.Fprintf(w, "  X-WP-Nonce: %s — `auth set --nonce <value>` required for authenticated calls\n", red("missing"))
			}
			fmt.Fprintf(w, "  Config: %s\n", cfg.Path)
			return nil
		},
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  partitaiva24-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
