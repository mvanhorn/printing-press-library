...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  partitaiva24-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  partitaiva24-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"attachments", "corrispettivi", "customers", "docs", "esterometro", "events", "f24", "income", "invoices", "notifications", "notifications-broadcast", "settings", "subscriptions", "subscriptions-gateways", "tickets", "tickets-categories"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  partitaiva24-pp-cli workflow status

  # Show status as JSON
  partitaiva24-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("partitaiva24-pp-cli")
			}
