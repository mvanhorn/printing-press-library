...
type rootFlags struct {
	...
	// invocation. Mirrors the PSX_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "psx-pp-cli",
		Short: `Psx CLI — Every PSX data-portal surface in one Go binary, plus the local price history the exchange itself throws away.`,
		Long: `Psx CLI — Every PSX data-portal surface in one Go binary, plus the local price history the exchange itself throws away.
...
Health check: run 'psx-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("psx-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'psx-pp-cli profile list')")
	...
	rootCmd.AddCommand(newTimeseriesCmd(flags))
	...
	rootCmd.AddCommand(newCircuitBreakersPromotedCmd(flags))
	rootCmd.AddCommand(newDebtPromotedCmd(flags))
	rootCmd.AddCommand(newEligibleScripsPromotedCmd(flags))
	...
	rootCmd.AddCommand(newSymbolsPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelActionsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelBasisCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelDiffCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelDocsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelDriftCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelRotationCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelUnusualCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelWatchlistCmd(flags))
