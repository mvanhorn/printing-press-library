...
Resource scoping:
  ...
  psx-pp-cli sync --resources circuit-breakers,debt

  # Full resync (ignore previous checkpoint)
  psx-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  psx-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  psx-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  psx-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "calendar":
		...
		return db.UpsertCalendar(typedData)
	case "sectors":
		typedData, err := typedSingleObjectData(data, obj, id, forceTypedID)
		if err != nil {
			return err
		}
		return db.UpsertSectors(typedData)
	case "symbols":
		typedData, err := typedSingleObjectData(data, obj, id, forceTypedID)
		if err != nil {
			return err
		}
		return db.UpsertSymbols(typedData)
	default:
...
var htmlPageModeResources = map[string]bool{
	...
	"company":          true,
	"debt":             true,
	"eligible-scrips":  true,
	"eligible_scrips":  true,
	"history":          true,
	"indices":          true,
	"listings":         true,
	"market":           true,
...
func responsePathForResource(resource, path string) []string {
	...
	case "calendar\x00/calendar":
		return []string{"data"}
	case "timeseries\x00/timeseries/eod/{symbol}":
		return []string{"data"}
	case "timeseries\x00/timeseries/int/{symbol}":
		return []string{"data"}
	}
	return nil
}
