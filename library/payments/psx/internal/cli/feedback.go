...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PSX_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PSX_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "psx-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:     "feedback [text]",
		Example: "  psx-pp-cli feedback \"sync was slower than expected\"",
...
When ` + "`PSX_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PSX_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  psx-pp-cli feedback list
  psx-pp-cli feedback list --limit 5
  psx-pp-cli feedback list --json`,
