...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/payments/psx/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/payments/psx/internal/learn/lookups"
...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^[A-Z0-9]{3,12}$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: psx-pp-cli: ticker pattern %q failed to compile: %v\n", `^[A-Z0-9]{3,12}$`, err)
	} else {
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"announcement_stream": {
			{Canonical: "C", Values: []string{"companies", "company announcements", "corporate announcements"}},
			{Canonical: "E", Values: []string{"psx notices", "exchange notices"}},
			{Canonical: "B", Values: []string{"secp notices", "regulator notices"}},
			{Canonical: "D", Values: []string{"nccpl notices", "clearing notices"}},
			{Canonical: "A", Values: []string{"cdc notices", "depository notices"}},
		},
		"index": {
			{Canonical: "KSE100", Values: []string{"kse100", "kse 100", "kse-100", "the index", "benchmark index", "main index"}},
			{Canonical: "KSE30", Values: []string{"kse30", "kse 30", "kse-30"}},
			{Canonical: "KMI30", Values: []string{"kmi30", "kmi 30", "shariah index", "islamic index"}},
			{Canonical: "KMIALLSHR", Values: []string{"kmi all share", "shariah all share"}},
			{Canonical: "ALLSHR", Values: []string{"all share index", "allshare"}},
			{Canonical: "PSXDIV20", Values: []string{"dividend 20", "div20", "dividend index"}},
			{Canonical: "BKTI", Values: []string{"banking index", "bank index"}},
			{Canonical: "OGTI", Values: []string{"oil and gas index", "energy index"}},
			{Canonical: "MII30", Values: []string{"meezan index"}},
		},
		"market_board": {
			{Canonical: "REG", Values: []string{"regular", "spot", "cash market"}},
			{Canonical: "DFC", Values: []string{"deliverable futures", "futures"}},
			{Canonical: "CSF", Values: []string{"cash settled futures"}},
			{Canonical: "ODL", Values: []string{"odd lot"}},
			{Canonical: "SQR", Values: []string{"square up"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: psx-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: psx-pp-cli: learn init: %v\n", err)
		}
	})
}
