...
// TestMigrate_AddsColumnsOnUpgrade_External verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_External(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE external (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
	...
	rows, err := s.DB().Query(`PRAGMA table_info(external)`)
	if err != nil {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from external after migrate", want)
		}
	}
}
