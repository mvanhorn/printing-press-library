...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ACCOUNTING_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ACCOUNTING_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "pennylane-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.accounting-pp-cli/feedback.jsonl.
When ` + "`ACCOUNTING_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ACCOUNTING_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  accounting-pp-cli feedback list
  accounting-pp-cli feedback list --limit 5
  accounting-pp-cli feedback list --json`,
