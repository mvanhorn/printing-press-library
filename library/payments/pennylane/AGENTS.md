# Accounting Printed CLI Agent Guide

This directory is a generated `accounting-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
accounting-pp-cli doctor --json
accounting-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
accounting-pp-cli which "<capability>" --json
accounting-pp-cli <command> --help
```
...
```bash
accounting-pp-cli <command> --agent
```
...
```bash
accounting-pp-cli <command> --help
accounting-pp-cli <command> --dry-run --agent
```
