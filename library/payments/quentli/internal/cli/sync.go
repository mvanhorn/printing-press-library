...
Resource scoping:
  ...
  quentli-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  quentli-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  quentli-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  quentli-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  quentli-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "tax_profiles":
		return db.UpsertTaxProfiles(data)
	case "discounts":
		return db.UpsertDiscounts(data)
	case "invoices":
		return db.UpsertInvoices(data)
	case "mark_paid":
		return db.UpsertMarkPaid(data)
	case "payment_link":
		return db.UpsertPaymentLink(data)
	case "invoices_payments":
		return db.UpsertInvoicesPayments(data)
	case "reminders":
		return db.UpsertReminders(data)
	case "try_payment":
		return db.UpsertTryPayment(data)
	case "payment-concepts":
		return db.UpsertPaymentConcepts(data)
	case "payment-sessions":
		return db.UpsertPaymentSessions(data)
	case "payments":
		return db.UpsertPayments(data)
	case "receipt":
		return db.UpsertReceipt(data)
	case "subscriptions":
		return db.UpsertSubscriptions(data)
	case "tax-invoices":
		return db.UpsertTaxInvoices(data)
	case "cancel":
		return db.UpsertCancel(data)
	case "webhook-events":
		return db.UpsertWebhookEvents(data)
	case "retry":
...
var reconcileTypedTables = map[string]string{
	...
	"mark_paid":                 "mark_paid",
	"payment-concepts":          "payment_concepts",
...
func responsePathForResource(resource, path string) []string {
	...
	case "customers\x00/v1/customers/{id}":
		return []string{"metadata"}
	case "invoices\x00/v1/invoices/{id}":
		return []string{"items"}
	case "webhooks\x00/v1/webhooks":
		return []string{"enabledEvents"}
	case "webhooks\x00/v1/webhooks/{id}":
		return []string{"enabledEvents"}
	}
	return nil
}
