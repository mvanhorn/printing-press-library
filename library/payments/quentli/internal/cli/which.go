...
var whichIndex = []whichEntry{
	{Command: "dunning", Description: "See every customer with an outstanding or overdue invoice, how much is owed, and the next collection action (send reminder, retry payment, or resend pay link).", Group: "Collections that compound", WhyItMatters: "Reach for this to answer who owes money and what to do next without firing dozens of API calls."},
	{Command: "subs at-risk", Description: "Surface active subscriptions whose saved payment method is expired, unconfirmed, or deleted, or that have recent failed collection attempts, ranked by recovery urgency.", Group: "Churn prevention", WhyItMatters: "Pick this before renewal season to fix expiring cards instead of discovering silent churn after the fact."},
	{Command: "reconcile", Description: "Cross-check completed or refunded payments and paid invoices against SAT tax-invoice status (VALID/CANCELED/PENDING) to find timbre gaps before monthly SAT filing.", Group: "SAT tax compliance", WhyItMatters: "Use before SAT filing to catch completed payments missing a VALID CFDI instead of eyeballing two exported lists."},
	{Command: "revenue", Description: "Aggregate payments and refunds by status, type, and currency over a window; see net collected vs returned in localized minor-currency amounts.", Group: "Revenue ops", WhyItMatters: "Reach for this to answer how much money actually landed last month and what came back in refunds."},
	{Command: "webhooks health", Description: "Aggregate webhook delivery events by status per endpoint and surface PAYMENT_ATTEMPT_FAILED events as an operational alert with a retry path.", Group: "Operational alerts", WhyItMatters: "Use on-call to spot failed webhook deliveries and refund/failure signals fast instead of polling dashboards."},
	{Command: "customer balance", Description: "Render a one-screen financial snapshot for a single customer: outstanding invoices, active subscriptions, payment methods, and CFDI linkage.", Group: "Collections that compound", WhyItMatters: "Reach for this before calling a customer about money so you have their full financial picture in one command."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  quentli-pp-cli which "stale tickets"
  quentli-pp-cli which "bottleneck"
  quentli-pp-cli which --limit 1 "send message"
  quentli-pp-cli which                                # list the full capability index`,
