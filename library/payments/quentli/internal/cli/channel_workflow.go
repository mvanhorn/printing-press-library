...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  quentli-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  quentli-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"customers", "discounts", "invoices", "payment-concepts", "payments", "subscriptions", "tax-invoices", "webhook-events", "webhooks"}
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  quentli-pp-cli workflow status

  # Show status as JSON
  quentli-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("quentli-pp-cli")
			}
