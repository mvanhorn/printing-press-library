...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^cus_[a-z0-9]+$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: quentli-pp-cli: ticker pattern %q failed to compile: %v\n", `^cus_[a-z0-9]+$`, err)
	} else {
		cfg.RegisterTickerPattern(re)
	}
	if re, err := regexp.Compile(`^inv_[a-z0-9]+$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: quentli-pp-cli: ticker pattern %q failed to compile: %v\n", `^inv_[a-z0-9]+$`, err)
	} else {
		cfg.RegisterTickerPattern(re)
	}
	if re, err := regexp.Compile(`^pay_[a-z0-9]+$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: quentli-pp-cli: ticker pattern %q failed to compile: %v\n", `^pay_[a-z0-9]+$`, err)
	} else {
		cfg.RegisterTickerPattern(re)
	}
	if re, err := regexp.Compile(`^sub_[a-z0-9]+$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: quentli-pp-cli: ticker pattern %q failed to compile: %v\n", `^sub_[a-z0-9]+$`, err)
	} else {
		cfg.RegisterTickerPattern(re)
	}
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"customer": {
			{Canonical: "Juan Pérez", Values: []string{"juan perez", "juan"}},
			{Canonical: "María García", Values: []string{"maria garcia", "maria"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: quentli-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: quentli-pp-cli: learn init: %v\n", err)
		}
	})
}
