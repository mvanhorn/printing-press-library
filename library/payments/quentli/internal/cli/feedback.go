...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("QUENTLI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("QUENTLI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "quentli-pp-cli/feedback")
...
When ` + "`QUENTLI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`QUENTLI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  quentli-pp-cli feedback list
  quentli-pp-cli feedback list --limit 5
  quentli-pp-cli feedback list --json`,
