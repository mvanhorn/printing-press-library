...
type rootFlags struct {
	...
	// invocation. Mirrors the QUENTLI_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "quentli-pp-cli",
		Short: `Quentli CLI — Every Quentli payments, invoicing, and SAT tax-billing endpoint, plus offline collection, reconciliation, and…`,
		Long: `Quentli CLI — Every Quentli payments, invoicing, and SAT tax-billing endpoint, plus offline collection, reconciliation, and at-risk-subscription intelligence no other Quentli tool has.
...
Health check: run 'quentli-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("quentli-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'quentli-pp-cli profile list')")
	...
	rootCmd.AddCommand(newPaymentConceptsCmd(flags))
	rootCmd.AddCommand(newPaymentSessionsCmd(flags))
	rootCmd.AddCommand(newPaymentsCmd(flags))
	rootCmd.AddCommand(newSubscriptionsCmd(flags))
	rootCmd.AddCommand(newTaxInvoicesCmd(flags))
	...
	rootCmd.AddCommand(newAuthLinksPromotedCmd(flags))
	rootCmd.AddCommand(newCustomerPortalSessionPromotedCmd(flags))
	rootCmd.AddCommand(newPaymentMethodsPromotedCmd(flags))
	rootCmd.AddCommand(newRefundsPromotedCmd(flags))
	rootCmd.AddCommand(newSetupSessionsPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelDunningCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelReconcileCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelRevenueCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSubsCmd(flags))
