...
Run sync first to populate the local search index.`,
  ...
  quentli-pp-cli search "status"

  # Force local search explicitly
  quentli-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  quentli-pp-cli search "status" --type auth-links --data-source local
  # JSON output for piping
  quentli-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'quentli-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
