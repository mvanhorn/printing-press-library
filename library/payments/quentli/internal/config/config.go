...
type Config struct {
	...
	QuentliSecretKey string          `toml:"secret_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.quentli.com",
	}
	...
	// Env var overrides
	if v := os.Getenv("QUENTLI_SECRET_KEY"); v != "" {
		cfg.QuentliSecretKey = v
		cfg.markEnvOverride("QuentliSecretKey")
		cfg.AuthSource = "env:QUENTLI_SECRET_KEY"
		cfg.CredentialSource = "env:QUENTLI_SECRET_KEY"
	}
	...
	if v := os.Getenv("QUENTLI_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("QUENTLI_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "quentli-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.QuentliSecretKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:QUENTLI_SECRET_KEY"
		}
		return "Bearer " + c.QuentliSecretKey
	}
	if c.AccessToken != "" {
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		QuentliSecretKey: c.QuentliSecretKey,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.QuentliSecretKey == "" {
		c.QuentliSecretKey = creds.QuentliSecretKey
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.QuentliSecretKey = ""
	delete(c.envOverrides, "QuentliSecretKey")
	c.updateFileConfigField("QuentliSecretKey")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["QuentliSecretKey"] {
			out.QuentliSecretKey = c.fileConfig.QuentliSecretKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "QuentliSecretKey":
		c.fileConfig.QuentliSecretKey = c.QuentliSecretKey
	}
}
