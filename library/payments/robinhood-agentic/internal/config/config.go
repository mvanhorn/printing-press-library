...
type Config struct {
	...
	RobinhoodAgenticToken string          `toml:"agentic_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("ROBINHOOD_AGENTIC_TOKEN"); v != "" {
		cfg.RobinhoodAgenticToken = v
		cfg.markEnvOverride("RobinhoodAgenticToken")
		cfg.AuthSource = "env:ROBINHOOD_AGENTIC_TOKEN"
		cfg.CredentialSource = "env:ROBINHOOD_AGENTIC_TOKEN"
	}
	...
	if v := os.Getenv("ROBINHOOD_AGENTIC_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("ROBINHOOD_AGENTIC_AUTHORIZATION_URL"); v != "" {
		cfg.AuthorizationURL = v
	}
	if v := os.Getenv("ROBINHOOD_AGENTIC_TOKEN_URL"); v != "" {
		cfg.TokenURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("ROBINHOOD_AGENTIC_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "robinhood-agentic-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	// A directly-held bearer in ROBINHOOD_AGENTIC_TOKEN (a pasted access token, or a
	...
	if c.RobinhoodAgenticToken != "" {
		return applyAuthFormat("Bearer {token}", map[string]string{
			"access_token":  c.RobinhoodAgenticToken,
			"token":         c.RobinhoodAgenticToken,
			"agentic_token": c.RobinhoodAgenticToken,
		})
	}
	return ""
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		RobinhoodAgenticToken: c.RobinhoodAgenticToken,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.RobinhoodAgenticToken = creds.RobinhoodAgenticToken
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.RobinhoodAgenticToken = ""
	delete(c.envOverrides, "RobinhoodAgenticToken")
	c.updateFileConfigField("RobinhoodAgenticToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["RobinhoodAgenticToken"] {
			out.RobinhoodAgenticToken = c.fileConfig.RobinhoodAgenticToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "RobinhoodAgenticToken":
		c.fileConfig.RobinhoodAgenticToken = c.RobinhoodAgenticToken
	}
}
