...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Robinhood Agentic",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  robinhood-agentic-pp-cli auth setup\n  robinhood-agentic-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "No setup URL is configured for this CLI; check the API's docs.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then run:")
			fmt.Fprintln(w, "  robinhood-agentic-pp-cli login")
			if !launch {
				return nil
			}
...
func newAuthLoginCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.Flags().StringVar(&clientID, "client-id", os.Getenv("ROBINHOOD_AGENTIC_CLIENT_ID"), "OAuth2 client ID")
	cmd.Flags().StringVar(&clientSecret, "client-secret", os.Getenv("ROBINHOOD_AGENTIC_CLIENT_SECRET"), "OAuth2 client secret")
...
func runOAuthLogin(cmd *cobra.Command, flags *rootFlags, clientID, clientSecret string, port int) error {
	...
	if (dryRunOK(flags) || cliutil.IsVerifyEnv()) && clientID == "" && cfg.ClientID == "" {
		fmt.Fprintln(w, `{"status":"dry_run","action":"auth login","would":"register a public OAuth client (RFC 7591), start OAuth2 authorization-code + PKCE flow, open browser, capture loopback callback"}`)
		return nil
	}
	// Robinhood's Agentic MCP issues no client_id ahead of time — the client
	...
	if clientID == "" && cfg.ClientID == "" && clientSecret == "" {
		...
		fmt.Fprintf(os.Stderr, "Registered a public OAuth client with Robinhood.\n")
	}
	...
	if authURL == "" {
		authURL = "https://robinhood.com/oauth"
	}
	params := url.Values{
		"client_id":     {clientID},
		"response_type": {"code"},
	...
	scopes := []string{robinhoodScope}
	if len(scopes) > 0 {
		params.Set("scope", strings.Join(scopes, " "))
	}
	// RFC 8707 resource indicator: audience-scope the issued token to the MCP
	// endpoint. Robinhood's authorize server expects it alongside scope=internal.
	params.Set("resource", robinhoodResource)
	...
	tokenParams.Set("resource", robinhoodResource)
...
func resolveOAuthCredentials(cmd *cobra.Command, flags *rootFlags, cfg *config.Config, clientID, clientSecret string) (string, string, error) {
	...
	if flags.noInput {
		return "", "", fmt.Errorf("OAuth2 client ID is required; pass --client-id, set ROBINHOOD_AGENTIC_CLIENT_ID, or run 'robinhood-agentic-pp-cli login' without --no-input")
	}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				fmt.Fprintf(w, "  %s Not authenticated. Run 'robinhood-agentic-pp-cli login' to authenticate.\n", red("FAIL"))
				return nil
			}
			...
			if cfg.TokenExpiry.IsZero() {
				...
				if cfg.RefreshToken != "" {
					...
					fmt.Fprintf(w, "  %s Token expired. Run 'robinhood-agentic-pp-cli login' to re-authenticate.\n", red("FAIL"))
				}
			}
