...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ROBINHOOD_AGENTIC_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ROBINHOOD_AGENTIC_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "robinhood-agentic-pp-cli/feedback")
...
When ` + "`ROBINHOOD_AGENTIC_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ROBINHOOD_AGENTIC_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  robinhood-agentic-pp-cli feedback list
  robinhood-agentic-pp-cli feedback list --limit 5
  robinhood-agentic-pp-cli feedback list --json`,
