...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'robinhood-agentic-pp-cli doctor' to check auth status."+
	...
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: your token may have expired. Re-run 'robinhood-agentic-pp-cli auth login' to re-authenticate"+
			"\n      Run 'robinhood-agentic-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: permission denied. Your token may lack required scopes. Re-run 'robinhood-agentic-pp-cli auth login' to re-authorize"+
			"\n      Run 'robinhood-agentic-pp-cli doctor' to check auth status.", err))
...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Run 'robinhood-agentic-pp-cli auth login' to re-authenticate."))
	}
	if len(trimmed) == 0 {
