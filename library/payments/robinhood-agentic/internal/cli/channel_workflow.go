...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  robinhood-agentic-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  robinhood-agentic-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"market", "options", "options-get-option-instruments", "options-get-option-watchlist", "scans", "watchlists", "watchlists-get-popular-watchlists"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  robinhood-agentic-pp-cli workflow status

  # Show status as JSON
  robinhood-agentic-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("robinhood-agentic-pp-cli")
			}
