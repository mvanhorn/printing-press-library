...
type rootFlags struct {
	...
	// invocation. Mirrors the ROBINHOOD_AGENTIC_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "robinhood-agentic-pp-cli",
		Short: `Every Robinhood Agentic Trading MCP capability as a typed, safety-gated CLI — official OAuth (no scraped tokens), review-first orders, and an offline SQLite portfolio journal no other Robinhood tool…`,
		Long: `Every Robinhood Agentic Trading MCP capability as a typed, safety-gated CLI — official OAuth (no scraped tokens), review-first orders, and an offline SQLite portfolio journal no other Robinhood tool ships.
...
Health check: run 'robinhood-agentic-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("robinhood-agentic-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'robinhood-agentic-pp-cli profile list')")
	...
	rootCmd.AddCommand(newScansCmd(flags))
	rootCmd.AddCommand(newWatchlistsCmd(flags))
	...
	rootCmd.AddCommand(newNovelGuardCmd(flags))
	rootCmd.AddCommand(newNovelSurfaceCmd(flags))
	rootCmd.AddCommand(newNovelWheelCmd(flags))
