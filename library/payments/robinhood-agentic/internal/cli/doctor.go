...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  robinhood-agentic-pp-cli doctor
  robinhood-agentic-pp-cli doctor --json
  robinhood-agentic-pp-cli doctor --fail-on warn
  robinhood-agentic-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "robinhood-agentic-pp-cli login"
				} else {
					authConfigured = true
			...
			if os.Getenv("ROBINHOOD_AGENTIC_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "ROBINHOOD_AGENTIC_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "ROBINHOOD_AGENTIC_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "robinhood-agentic-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'robinhood-agentic-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'robinhood-agentic-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'robinhood-agentic-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'robinhood-agentic-pp-cli sync' to refresh."
	}
	return report
}
