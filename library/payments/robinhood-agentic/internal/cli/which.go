...
var whichIndex = []whichEntry{
	{Command: "portfolio history", Description: "Answer 'what was my portfolio worth on any given day' from a local time series Robinhood doesn't keep.", Group: "Local state that compounds", WhyItMatters: "Reach for this when a task needs portfolio value over time — no MCP tool can answer it."},
	{Command: "guard", Description: "Set per-trade caps, daily caps, symbol allow/denylists, and a kill switch that the CLI enforces before any order leaves the machine.", Group: "Agent safety rails", WhyItMatters: "Use this before letting any agent loop place orders — it is the only enforceable budget/kill-switch layer for the agentic account."},
	{Command: "equities settle", Description: "Resolve an order to verified terminal truth — actual fill price and state — instead of trusting the placement echo.", Group: "Agent safety rails", WhyItMatters: "Run after every place or cancel to get the real outcome, not the optimistic echo."},
	{Command: "brief", Description: "The whole pre-open check — portfolio value, day-over-day delta, open orders, positions, top movers among your holdings, and upcoming earnings for held symbols — in one command.", Group: "Agent-native rituals", WhyItMatters: "The one-command replacement for the four-round-trip morning ritual."},
	{Command: "audit", Description: "See everything the CLI (or an agent driving it) reviewed, placed, canceled, or was denied — with idempotency keys and outcomes.", Group: "Agent safety rails", WhyItMatters: "The weekly agent-accountability review: reconstruct exactly what an automation did."},
	{Command: "portfolio winrate", Description: "Round-trip win rate, average win/loss, and per-symbol stats computed from your synced trade history.", Group: "Local state that compounds", WhyItMatters: "Journaling and strategy review without exporting to a spreadsheet."},
	{Command: "surface diff", Description: "Know when Robinhood adds, removes, or reshapes MCP tools — with dates — instead of discovering breakage in production.", Group: "Agent-native rituals", WhyItMatters: "Check after any unexplained failure: the tool surface is beta and moves without notice."},
	{Command: "wheel status", Description: "Per-symbol wheel-strategy stage (cash-secured put → assigned → covered call → called away) inferred automatically.", Group: "Local state that compounds", WhyItMatters: "The Friday post-expiration answer to 'what got assigned and what stage is each position in'."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  robinhood-agentic-pp-cli which "stale tickets"
  robinhood-agentic-pp-cli which "bottleneck"
  robinhood-agentic-pp-cli which --limit 1 "send message"
  robinhood-agentic-pp-cli which                                # list the full capability index`,
