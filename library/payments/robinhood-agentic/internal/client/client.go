...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	// Robinhood's Agentic surface is a single MCP (JSON-RPC) endpoint, not a
	// REST API. Install the transport bridge that rewrites the generated
	// client's REST-shaped requests into MCP tool calls. Everything above this
	// line (auth, redirect handling, rate limiting) operates unchanged; only
	// the wire format is translated. See internal/client/mcp_transport.go.
	httpClient.Transport = newMCPTransport(httpClient.Transport, c.BaseURL)
	return c
}
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "robinhood-agentic-pp-cli/0.1.0")
		}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "robinhood-agentic-pp-cli auth login")
}
...
func (c *Client) refreshAccessToken(ctx context.Context) error {
	...
	// RFC 8707 resource indicator — Robinhood's token endpoint audience-scopes
	// refreshed tokens to the MCP endpoint, matching the initial authorization.
	params.Set("resource", "https://agent.robinhood.com/mcp/trading")
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.RobinhoodAgenticToken)
	}
