...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"ROBINHOOD_AGENTIC_CONFIG",
		"ROBINHOOD_AGENTIC_CONFIG_DIR",
		"ROBINHOOD_AGENTIC_DATA_DIR",
		"ROBINHOOD_AGENTIC_STATE_DIR",
		"ROBINHOOD_AGENTIC_CACHE_DIR",
		"ROBINHOOD_AGENTIC_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
		"XDG_CACHE_HOME",
	...
	return home, filepath.Join(home, ".config", "robinhood-agentic-pp-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "robinhood-agentic-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("ROBINHOOD_AGENTIC_TOKEN", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "robinhood-agentic-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "robinhood-agentic-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "robinhood-agentic-pp-cli", "credentials.toml")
