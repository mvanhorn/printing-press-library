# SimpleFIN Printed CLI Agent Guide

This directory is a generated `simplefin-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
simplefin-pp-cli doctor --json
simplefin-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
simplefin-pp-cli which "<capability>" --json
simplefin-pp-cli <command> --help
```
...
```bash
simplefin-pp-cli <command> --agent
```
...
```bash
simplefin-pp-cli <command> --help
simplefin-pp-cli <command> --dry-run --agent
```
