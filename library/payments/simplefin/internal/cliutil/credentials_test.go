...
// authHeaderReveal returns the credential embedded in an auth header so these
// tests can assert on the underlying value regardless of encoding. A
// "Basic <b64>" header carries base64(user:pass); other schemes carry the
// value verbatim. (Without the decode, a base64 "Basic {token}" header never
// contains the raw credential as a substring.)
func authHeaderReveal(h string) string {
...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	return home, filepath.Join(home, ".config", "simplefin-pp-cli", "config.toml")
}
...
func TestCredentialsFileWinsWhenLegacyConfigAlsoHasSecrets(t *testing.T) {
	...
	if got := authHeaderReveal(cfg.AuthHeader()); !strings.Contains(got, "data-secret") || strings.Contains(got, "legacy-secret") {
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "simplefin-pp-cli", "credentials.toml")
	...
	stderr := captureCredentialStderr(t, func() {
		...
		if got := authHeaderReveal(cfg.AuthHeader()); !strings.Contains(got, "legacy-secret") {
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("SIMPLEFIN_ACCESS_URL", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "simplefin-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
	...
	stderr := captureCredentialStderr(t, func() {
		...
		if got := authHeaderReveal(cfg.AuthHeader()); !strings.Contains(got, "env-secret") {
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "simplefin-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "simplefin-pp-cli", "credentials.toml")
