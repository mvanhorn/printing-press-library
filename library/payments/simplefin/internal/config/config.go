...
type Config struct {
	...
	SimplefinAccessUrl string          `toml:"access_url"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("SIMPLEFIN_ACCESS_URL"); v != "" {
		cfg.SimplefinAccessUrl = v
		cfg.markEnvOverride("SimplefinAccessUrl")
		cfg.AuthSource = "env:SIMPLEFIN_ACCESS_URL"
		cfg.CredentialSource = "env:SIMPLEFIN_ACCESS_URL"
	}
	...
	// SimpleFIN: derive BaseURL + Basic auth header from the Access URL
	// credential. Runs before the SIMPLEFIN_BASE_URL override below so
	// printing-press verify's mock server still wins. See simplefin_access.go.
	cfg.resolveSimpleFINAccess()

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("SIMPLEFIN_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("SIMPLEFIN_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "simplefin-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.SimplefinAccessUrl == "" {
		return ""
	}
	credentials := basicAuthPayload("Basic {token}", map[string]string{
		"access_url":           c.SimplefinAccessUrl,
		"SIMPLEFIN_ACCESS_URL": c.SimplefinAccessUrl,
		"token":                c.SimplefinAccessUrl,
		"access_token":         c.SimplefinAccessUrl,
	}, true)
	if credentials == "" {
		return ""
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		SimplefinAccessUrl: c.SimplefinAccessUrl,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.SimplefinAccessUrl = creds.SimplefinAccessUrl
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.SimplefinAccessUrl = token
	delete(c.envOverrides, "SimplefinAccessUrl")
	c.updateFileConfigField("SimplefinAccessUrl")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["SimplefinAccessUrl"] {
			out.SimplefinAccessUrl = c.fileConfig.SimplefinAccessUrl
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "SimplefinAccessUrl":
		c.fileConfig.SimplefinAccessUrl = c.SimplefinAccessUrl
	}
}
