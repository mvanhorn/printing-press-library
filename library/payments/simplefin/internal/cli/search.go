...
Run sync first to populate the local search index.`,
  ...
  simplefin-pp-cli search "status"

  # Force local search explicitly
  simplefin-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  simplefin-pp-cli search "status" --type accounts --data-source local
  # JSON output for piping
  simplefin-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'simplefin-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
