...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "simplefin-pp-cli",
		Short: `Pull every bank, card, and brokerage account through SimpleFIN into a local SQLite ledger — net worth, cash flow, recurring charges, and holdings no single bank app can show.`,
		Long: `Pull every bank, card, and brokerage account through SimpleFIN into a local SQLite ledger — net worth, cash flow, recurring charges, and holdings no single bank app can show.
...
Health check: run 'simplefin-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("simplefin-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'simplefin-pp-cli profile list')")
	...
	// SimpleFIN: attach the `claim` setup-token flow and replace the generic
	// `auth setup` (which reports "no setup URL configured") with one that
	// teaches the real claim flow + bridge URL.
	authCmd := newAuthCmd(flags)
	for _, sub := range authCmd.Commands() {
		if sub.Name() == "setup" {
			authCmd.RemoveCommand(sub)
			break
		}
	}
	authCmd.AddCommand(newSimplefinAuthSetupCmd(flags))
	authCmd.AddCommand(newAuthClaimCmd(flags))
	rootCmd.AddCommand(authCmd)
	rootCmd.AddCommand(newAgentContextCmd(rootCmd))
	rootCmd.AddCommand(newProfileCmd(flags))
	rootCmd.AddCommand(newFeedbackCmd(flags))
	rootCmd.AddCommand(newWhichCmd(flags))
	// SimpleFIN: hand-authored export (ledger/beancount/qif/csv/json) replaces
	// the generic JSONL backup export; hand-authored sync handles the nested
	// /accounts shape the generic resource-walker cannot.
	rootCmd.AddCommand(newSimplefinExportCmd(flags))
	rootCmd.AddCommand(newSearchCmd(flags))
	rootCmd.AddCommand(newSimplefinSyncCmd(flags))
	...
	rootCmd.AddCommand(newNovelCashflowCmd(flags))
	rootCmd.AddCommand(newNovelCategorizeCmd(flags))
	rootCmd.AddCommand(newNovelNetworthCmd(flags))
	rootCmd.AddCommand(newNovelPortfolioCmd(flags))
	rootCmd.AddCommand(newNovelReconcileCmd(flags))
	rootCmd.AddCommand(newNovelRecurringCmd(flags))
