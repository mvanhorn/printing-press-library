...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SIMPLEFIN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SIMPLEFIN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "simplefin-pp-cli/feedback")
...
When ` + "`SIMPLEFIN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SIMPLEFIN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  simplefin-pp-cli feedback list
  simplefin-pp-cli feedback list --limit 5
  simplefin-pp-cli feedback list --json`,
