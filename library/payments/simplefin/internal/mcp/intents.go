...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("portfolio_gain_loss",
			mcplib.WithDescription("Holdings market value vs cost basis across every brokerage."),
			mcplib.WithBoolean("gain", mcplib.Description("Override the recipe's --gain value.")),
		),
		handlePortfolioGainLoss,
	)
}

// handlePortfolioGainLoss runs the portfolio_gain_loss recipe intent: Holdings market value vs cost basis across every brokerage.
func handlePortfolioGainLoss(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	args = appendRecipeBoolFlag(args, "gain", input["gain"], true)
