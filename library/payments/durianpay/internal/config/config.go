...
type Config struct {
	...
	Path                 string            `toml:"-"`
	DurianpayApiKey      string            `toml:"api_key"`
	DurianpayApiPassword string            `toml:"api_password"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.durianpay.id/v1",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("DURIANPAY_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "durianpay-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("DURIANPAY_API_KEY"); v != "" {
		cfg.DurianpayApiKey = v
		cfg.AuthSource = "env:DURIANPAY_API_KEY"
	}
	if v := os.Getenv("DURIANPAY_API_PASSWORD"); v != "" {
		cfg.DurianpayApiPassword = v
		cfg.AuthSource = "env:DURIANPAY_API_PASSWORD"
	}
	...
	if cfg.AuthSource == "" && cfg.DurianpayApiKey != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.DurianpayApiPassword != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("DURIANPAY_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.DurianpayApiKey == "" {
		return ""
	}
	// Durianpay Basic auth uses the API key as username with a BLANK password
	// ("key:" base64-encoded); DurianpayApiPassword stays optional.
	credentials := c.DurianpayApiKey + ":" + c.DurianpayApiPassword
