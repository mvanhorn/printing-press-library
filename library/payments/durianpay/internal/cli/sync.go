...
Resource scoping:
  ...
  durianpay-pp-cli sync

  # Sync specific resources only
  durianpay-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  durianpay-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  durianpay-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  durianpay-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  durianpay-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/durianpay-pp-cli/data.db)")
...
func extractSingleObjectArraySibling(envelope map[string]json.RawMessage) ([]json.RawMessage, bool) {
	...
	for key, raw := range envelope {
		...
		// A JSON null or empty array under a non-metadata key is a valid
		// empty page (e.g. Durianpay returns {"data":{"orders":null,"total":0}}
		// for zero records). Without this, the whole envelope falls through to
		// the single-item path and fails with "missing id for <resource>".
		var rawArray []json.RawMessage
		if err := json.Unmarshal(raw, &rawArray); err == nil && (isJSONNull(raw) || len(rawArray) == 0) {
			emptyArrayCount++
			continue
		}
	}
	...
	if arrayCount == 0 && emptyArrayCount == 1 {
...
func isEmptyPageEnvelope(envelope map[string]json.RawMessage) bool {
	...
	if hasExactlyOneEmptyArray(envelope) {
		return true
	}
	return isNullListWithZeroTotal(envelope)
}

// isNullListWithZeroTotal recognizes the {"<resource>":null,"total":0} shape
// (Durianpay's zero-record list envelope). A JSON-null list key alone is NOT
// enough — single-object responses legitimately carry null fields — so this
// requires an explicit zero total/count sibling before treating the page as
// empty.
func isNullListWithZeroTotal(envelope map[string]json.RawMessage) bool {
	zeroTotal := false
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"merchants":   "/merchants/subaccount",
		"orders":      "/orders",
		"payments":    "/payments",
		"payments-va": "/payments/va",
		"refunds":     "/refunds",
	}
