...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "durianpay-pp-cli",
		Short: `Durianpay CLI — Every Durianpay API in one terminal — legacy and SNAP — with the dual-signature scheme handled for you and a local SQLi…`,
		Long: `Durianpay CLI — Every Durianpay API in one terminal — legacy and SNAP — with the dual-signature scheme handled for you and a local SQLi…
...
Health check: run 'durianpay-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("durianpay-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'durianpay-pp-cli profile list')")
	...
	rootCmd.AddCommand(newDisbursementsCmd(flags))
	...
	rootCmd.AddCommand(newNovelExplainCmd(flags))
	rootCmd.AddCommand(newNovelPayCmd(flags))
	rootCmd.AddCommand(newNovelPayoutCmd(flags))
	rootCmd.AddCommand(newNovelReconcileCmd(flags))
	rootCmd.AddCommand(newNovelRefundAuditCmd(flags))
	rootCmd.AddCommand(newNovelSandboxCmd(flags))
	rootCmd.AddCommand(newNovelSnapCmd(flags))
	rootCmd.AddCommand(newNovelStuckCmd(flags))
