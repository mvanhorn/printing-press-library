...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  durianpay-pp-cli doctor
  durianpay-pp-cli doctor --json
  durianpay-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export DURIANPAY_API_KEY=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("DURIANPAY_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "DURIANPAY_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "DURIANPAY_API_KEY")
			}
			if strings.Contains("Basic-auth password — leave unset; Durianpay uses a blank password.", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "DURIANPAY_API_PASSWORD")
				if os.Getenv("DURIANPAY_API_PASSWORD") != "" {
					authEnvSet = append(authEnvSet, "DURIANPAY_API_PASSWORD")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("DURIANPAY_API_PASSWORD") != "" {
				authEnvSet = append(authEnvSet, "DURIANPAY_API_PASSWORD")
			} else {
				authEnvInfo = append(authEnvInfo, "DURIANPAY_API_PASSWORD optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "durianpay-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'durianpay-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'durianpay-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'durianpay-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'durianpay-pp-cli sync' to refresh."
	}
	return report
}
