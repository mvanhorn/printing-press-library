...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "durianpay-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DURIANPAY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DURIANPAY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "durianpay-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/durianpay-pp-cli/feedback.jsonl.
When ` + "`DURIANPAY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DURIANPAY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  durianpay-pp-cli feedback list
  durianpay-pp-cli feedback list --limit 5
  durianpay-pp-cli feedback list --json`,
