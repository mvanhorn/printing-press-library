...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Durianpay Legacy",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  durianpay-pp-cli auth setup\n  durianpay-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "No setup URL is configured for this CLI; check the API's docs.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export DURIANPAY_API_KEY=\"<your-token>\"")
			fmt.Fprintln(w, "  export DURIANPAY_API_PASSWORD=\"<your-token>\"")
			fmt.Fprintln(w, "  durianpay-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  durianpay-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export DURIANPAY_API_KEY=\"your-token-here\" # Durianpay API key from Dashboard > Settings > API Keys (sandbox keys start dp_test_, live dp_live_). Sent as the Basic-auth username.")
				fmt.Fprintln(w, "  export DURIANPAY_API_PASSWORD=\"your-token-here\" # Basic-auth password — leave unset; Durianpay uses a blank password.")
				fmt.Fprintf(w, "  durianpay-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  durianpay-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  durianpay-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("DURIANPAY_API_KEY") != "" {
				envStillSet = "DURIANPAY_API_KEY"
			}
			if envStillSet == "" && os.Getenv("DURIANPAY_API_PASSWORD") != "" {
				envStillSet = "DURIANPAY_API_PASSWORD"
			}
