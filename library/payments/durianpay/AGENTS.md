# Durianpay Printed CLI Agent Guide

This directory is a generated `durianpay-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
durianpay-pp-cli doctor --json
durianpay-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
durianpay-pp-cli which "<capability>" --json
durianpay-pp-cli <command> --help
```
...
```bash
durianpay-pp-cli <command> --agent
```
...
```bash
durianpay-pp-cli <command> --help
durianpay-pp-cli <command> --dry-run --agent
```
