# BrainOS Printed CLI Agent Guide

This directory is a generated `brainos-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
brainos-pp-cli doctor --json
brainos-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
brainos-pp-cli which "<capability>" --json
brainos-pp-cli <command> --help
```
...
```bash
brainos-pp-cli <command> --help
brainos-pp-cli <command> --dry-run --agent
```
