...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "brainos-pp-cli", "http")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if authHeader != "" {
			// Supabase/PostgREST requires apikey header; also send Authorization for RLS bypass.
			req.Header.Set("apikey", authHeader)
			bearerVal := authHeader
			if len(bearerVal) < 7 || bearerVal[:7] != "Bearer " {
				bearerVal = "Bearer " + bearerVal
			}
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "brainos-pp-cli/14.5")
		}
