...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  brainos-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  brainos-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"active-memory", "agent-messages", "agent-messages-status-summary", "brain-config", "brain-email-capture-log", "brain-project-status", "broker-config", "cron-jobs", "documents", "executor-briefs", "margin-log", "mcp-activity-logs", "mcp-api-keys", "mcp-auth-errors", "mcp-oauth-states", "mcp-servers", "mcp-sessions", "mcp-tools", "mcpapikeys", "mcpsessions", "natureos-agent-checkpoints", "natureos-agent-memory", "natureos-allowlist", "natureos-dual-model-tasks", "natureos-task-queue", "rpc", "shared-state", "skills", "thoughts", "tool-usage-stats", "trading-calibration", "trading-postmortems", "trading-premortems", "trading-regime-log", "trading-sessions", "webauthn-credentials"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  brainos-pp-cli workflow status

  # Show status as JSON
  brainos-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("brainos-pp-cli")
			}
