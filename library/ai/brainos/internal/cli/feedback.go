...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".brainos-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BRAINOS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BRAINOS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "brainos-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.brainos-pp-cli/feedback.jsonl.
When ` + "`BRAINOS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BRAINOS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  brainos-pp-cli feedback list
  brainos-pp-cli feedback list --limit 5
  brainos-pp-cli feedback list --json`,
