# Zylos Console Printed CLI Agent Guide

This directory is a generated `zylos-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
zylos-pp-cli doctor --json
zylos-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
zylos-pp-cli which "<capability>" --json
zylos-pp-cli <command> --help
```
...
```bash
zylos-pp-cli <command> --help
zylos-pp-cli <command> --dry-run --agent
```
