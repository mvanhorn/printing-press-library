...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  zylos-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  zylos-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"conversations", "conversations-recent"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  zylos-pp-cli workflow status

  # Show status as JSON
  zylos-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("zylos-pp-cli")
			}
