...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".zylos-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ZYLOS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ZYLOS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "zylos-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.zylos-pp-cli/feedback.jsonl.
When ` + "`ZYLOS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ZYLOS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  zylos-pp-cli feedback list
  zylos-pp-cli feedback list --limit 5
  zylos-pp-cli feedback list --json`,
