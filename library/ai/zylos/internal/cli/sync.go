...
Exit codes & warnings:
  ...
  zylos-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  zylos-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  zylos-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  zylos-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  zylos-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/zylos-pp-cli/data.db)")
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"conversations":        "/api/poll",
		"conversations-recent": "/api/conversations/recent",
	}
