...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".gemini-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GEMINI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GEMINI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "gemini-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.gemini-pp-cli/feedback.jsonl.
When ` + "`GEMINI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GEMINI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gemini-pp-cli feedback list
  gemini-pp-cli feedback list --limit 5
  gemini-pp-cli feedback list --json`,
