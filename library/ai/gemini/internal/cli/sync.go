...
Exit codes & warnings:
  ...
  gemini-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  gemini-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  gemini-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  gemini-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  gemini-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/gemini-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "model_embed_content":
		return db.UpsertModelEmbedContent(data)
	case "model_generate_content":
		return db.UpsertModelGenerateContent(data)
	case "model_stream_generate_content":
		return db.UpsertModelStreamGenerateContent(data)
	case "name_update_embed_content_batch":
		return db.UpsertNameUpdateEmbedContentBatch(data)
	case "name_update_generate_content_batch":
		return db.UpsertNameUpdateGenerateContentBatch(data)
	default:
