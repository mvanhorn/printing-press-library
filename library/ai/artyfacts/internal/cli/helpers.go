...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'artyfacts-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token. Set it with: artyfacts-pp-cli auth set-token <token>"+
			"\n      or: export ARTYFACTS_API_KEY=<your-token>"+
			"\n      Run 'artyfacts-pp-cli doctor' to check auth status.", err))
...
func isNoValidFieldsError(err error) bool {
	return err != nil && strings.Contains(err.Error(), "No valid fields to update")
}
...
// truncateJSONArray caps a JSON array to n elements. No-op when n <= 0 or
// the array is already shorter. Used by list commands when the API ignores
