...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".artyfacts-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ARTYFACTS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ARTYFACTS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "artyfacts-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.artyfacts-pp-cli/feedback.jsonl.
When ` + "`ARTYFACTS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ARTYFACTS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  artyfacts-pp-cli feedback list
  artyfacts-pp-cli feedback list --limit 5
  artyfacts-pp-cli feedback list --json`,
