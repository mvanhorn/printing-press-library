# Artyfacts Printed CLI Agent Guide

This directory is a generated `artyfacts-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
artyfacts-pp-cli doctor --json
artyfacts-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
artyfacts-pp-cli which "<capability>" --json
artyfacts-pp-cli <command> --help
```
...
```bash
artyfacts-pp-cli <command> --agent
```
...
```bash
artyfacts-pp-cli <command> --help
artyfacts-pp-cli <command> --dry-run --agent
```
