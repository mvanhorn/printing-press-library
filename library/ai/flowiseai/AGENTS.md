# Flowise Printed CLI Agent Guide

This directory is a generated `flowiseai-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
flowiseai-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
flowiseai-pp-cli which "<capability>" --json
flowiseai-pp-cli <command> --help
```
...
```bash
flowiseai-pp-cli <command> --agent
```
...
```bash
flowiseai-pp-cli <command> --help
flowiseai-pp-cli <command> --dry-run --agent
```
