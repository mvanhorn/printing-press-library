...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  flowiseai-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  flowiseai-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"assistants", "chatflows", "document-store", "tools", "variables"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  flowiseai-pp-cli workflow status

  # Show status as JSON
  flowiseai-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("flowiseai-pp-cli")
			}
