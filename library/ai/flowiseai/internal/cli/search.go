...
In local mode: searches locally synced data only.`,
  ...
  flowiseai-pp-cli search "error timeout"

  # Force local search only
  flowiseai-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  flowiseai-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  flowiseai-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Local FTS search
			if dbPath == "" {
				dbPath = defaultDBPath("flowiseai-pp-cli")
			}

			db, err := store.OpenWithContext(cmd.Context(), dbPath)
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'flowiseai-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "attachments":
				results, err = db.SearchAttachments(query, limit)
			case "chatflows":
				results, err = db.SearchChatflows(query, limit)
			case "document-store":
				results, err = db.SearchDocumentStore(query, limit)
			case "tools":
				results, err = db.SearchTools(query, limit)
			case "":
				// Search all FTS-enabled tables individually to avoid duplicates.
				seen := make(map[string]bool)
				_ = seen // prevent unused error when no FTS tables exist
				{
					partial, searchErr := db.SearchAttachments(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search attachments failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchChatflows(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search chatflows failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchDocumentStore(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search document_store failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchTools(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search tools failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/flowiseai-pp-cli/data.db)")
