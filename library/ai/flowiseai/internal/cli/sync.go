...
Exit codes & warnings:
  ...
  flowiseai-pp-cli sync

  # Sync specific resources only
  flowiseai-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  flowiseai-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  flowiseai-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  flowiseai-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  flowiseai-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "chatflows":
		return db.UpsertChatflows(data)
	case "chatmessage":
		return db.UpsertChatmessage(data)
	case "document-store":
		return db.UpsertDocumentStore(data)
	case "flowiseai-feedback":
		return db.UpsertFlowiseaiFeedback(data)
	case "leads":
		return db.UpsertLeads(data)
	case "prediction":
		return db.UpsertPrediction(data)
	case "tools":
		return db.UpsertTools(data)
	case "upsert-history":
		return db.UpsertUpsertHistory(data)
	case "variables":
		return db.UpsertVariables(data)
	case "vector":
		return db.UpsertVector(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"chatflows":      "/chatflows",
		"document-store": "/document-store/store",
		"tools":          "/tools",
		"variables":      "/variables",
	}
