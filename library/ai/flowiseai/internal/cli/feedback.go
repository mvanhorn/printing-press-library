...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".flowiseai-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FLOWISEAI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FLOWISEAI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "flowiseai-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.flowiseai-pp-cli/feedback.jsonl.
When ` + "`FLOWISEAI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FLOWISEAI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  flowiseai-pp-cli feedback list
  flowiseai-pp-cli feedback list --limit 5
  flowiseai-pp-cli feedback list --json`,
