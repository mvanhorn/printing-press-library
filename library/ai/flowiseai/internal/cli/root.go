...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "flowiseai-pp-cli",
		Short: `Flowiseai CLI — Every Flowise endpoint plus newsletter-grade compound commands no MCP or SDK has — built for agents that run as marketi…`,
		Long: `Flowiseai CLI — Every Flowise endpoint plus newsletter-grade compound commands no MCP or SDK has — built for agents that run as marketi…
...
Health check: run 'flowiseai-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("flowiseai-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'flowiseai-pp-cli profile list')")
	...
	rootCmd.AddCommand(newChatflowsCmd(flags))
	rootCmd.AddCommand(newChatmessageCmd(flags))
	rootCmd.AddCommand(newDocumentStoreCmd(flags))
	rootCmd.AddCommand(newFlowiseaiFeedbackCmd(flags))
	rootCmd.AddCommand(newLeadsCmd(flags))
	rootCmd.AddCommand(newToolsCmd(flags))
	rootCmd.AddCommand(newUpsertHistoryCmd(flags))
	...
	rootCmd.AddCommand(newPredictionPromotedCmd(flags))
	rootCmd.AddCommand(newVectorPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())
	// Novel transcendence commands (hand-built, not from spec)
	rootCmd.AddCommand(newChatflowGroupCmd(flags))
	rootCmd.AddCommand(newDocstoreGroupCmd(flags))
	rootCmd.AddCommand(newNewsletterGroupCmd(flags))
