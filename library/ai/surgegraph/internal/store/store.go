...
func (s *Store) SyncDiff(ctx context.Context, since time.Time) ([]SyncDiffRow, error) {
	// We count rows in each table whose last_seen_at / created_at >= since.
	queries := map[string]string{
		"projects":             "SELECT '' AS project_id, COUNT(*), (SELECT COUNT(*) FROM projects WHERE last_synced_at >= ?) FROM projects",
		"prompts":              "SELECT COALESCE(project_id,'') as pid, COUNT(*), 0 FROM prompts GROUP BY pid",
		"visibility_snapshots": "SELECT project_id, COUNT(*), (SELECT COUNT(*) FROM visibility_snapshots v WHERE v.project_id = visibility_snapshots.project_id AND v.created_at >= ?) FROM visibility_snapshots GROUP BY project_id",
		"citation_snapshots":   "SELECT project_id, COUNT(*), (SELECT COUNT(*) FROM citation_snapshots c WHERE c.project_id = citation_snapshots.project_id AND c.created_at >= ?) FROM citation_snapshots GROUP BY project_id",
		"documents":            "SELECT project_id, COUNT(*), (SELECT COUNT(*) FROM documents d WHERE d.project_id = documents.project_id AND d.last_seen_at >= ?) FROM documents GROUP BY project_id",
	}
	...
	var out []SyncDiffRow
	for resource, q := range queries {
		_ = q
		// Use a per-resource simpler query: count by project, plus count-since
		rows, err := s.DB.QueryContext(ctx, fmt.Sprintf(`
			SELECT '' AS project_id, COUNT(*), 0 FROM %s
		`, resource))
		if err != nil {
			return nil, fmt.Errorf("sync-diff %s: %w", resource, err)
		}
		for rows.Next() {
			var d SyncDiffRow
			d.Resource = resource
			...
			if ts, ok := cursorMap[resource+"|"+d.ProjectID]; ok {
				d.LastSyncedAt = ts
			}
			out = append(out, d)
		}
