...
Run sync first to populate the local search index.`,
  ...
  v0-pp-cli search "status"

  # Force local search explicitly
  v0-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  v0-pp-cli search "status" --type chats --data-source local
  # JSON output for piping
  v0-pp-cli search "status" --json --limit 20`,
		Annotations: map[string]string{"mcp:hidden": "true", "pp:happy-args": "<query>=kanban"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'v0-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "mcp-servers":
				results, err = db.SearchMcpServers(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchMcpServers(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search mcp_servers failed: %w", searchErr)
					}
