# Higgsfield Printed CLI Agent Guide

This directory is a generated `higgsfield-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
higgsfield-pp-cli doctor --json
higgsfield-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
higgsfield-pp-cli which "<capability>" --json
higgsfield-pp-cli <command> --help
```
...
```bash
higgsfield-pp-cli <command> --agent
```
...
```bash
higgsfield-pp-cli <command> --help
higgsfield-pp-cli <command> --dry-run --agent
```
