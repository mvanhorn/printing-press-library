...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  higgsfield-pp-cli doctor
  higgsfield-pp-cli doctor --json
  higgsfield-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export HIGGSFIELD_JWT=<your-key>"
					report["auth_instructions"] = "Two auth paths are supported. The default is the cloud web backend using a raw JWT\nfrom your logged-in higgsfield CLI:\n  1. Run `higgsfield auth login` in your terminal (the official Higgsfield CLI).\n  2. Export the token: `export HIGGSFIELD_JWT=\"$(higgsfield auth token)\"`\n  3. Opt in to the web backend: `export HIGGSFIELD_ENABLE_WEB_BACKEND=1`\nThe web backend covers the headline 2026 models (Veo 3.x, Sora 2 when available,\nKling 3, Soul V2, Nano Banana 2, Seedance 2).\nFor the stable platform API (smaller model set), get a key at platform.higgsfield.ai\nand export `HIGGSFIELD_API_KEY` and `HIGGSFIELD_SECRET` (or combined HF_CREDENTIALS=KEY:SECRET).\n"
				} else {
					authConfigured = true
			...
			if os.Getenv("HIGGSFIELD_JWT") != "" {
				authEnvSet = append(authEnvSet, "HIGGSFIELD_JWT")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "HIGGSFIELD_JWT")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "higgsfield-pp-cli"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'higgsfield-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'higgsfield-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'higgsfield-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'higgsfield-pp-cli sync' to refresh."
	}
	return report
}
