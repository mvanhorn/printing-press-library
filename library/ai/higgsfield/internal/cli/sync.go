...
Resource scoping:
  ...
  higgsfield-pp-cli sync

  # Sync specific resources only
  higgsfield-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  higgsfield-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  higgsfield-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  higgsfield-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  higgsfield-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/higgsfield-pp-cli/data.db)")
