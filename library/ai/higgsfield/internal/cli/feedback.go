...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".higgsfield-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("HIGGSFIELD_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HIGGSFIELD_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "higgsfield-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.higgsfield-pp-cli/feedback.jsonl.
When ` + "`HIGGSFIELD_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`HIGGSFIELD_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  higgsfield-pp-cli feedback list
  higgsfield-pp-cli feedback list --limit 5
  higgsfield-pp-cli feedback list --json`,
