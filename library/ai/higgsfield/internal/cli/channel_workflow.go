...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  higgsfield-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  higgsfield-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"account", "generations", "models", "soul_ids", "workspaces"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  higgsfield-pp-cli workflow status

  # Show status as JSON
  higgsfield-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("higgsfield-pp-cli")
			}
