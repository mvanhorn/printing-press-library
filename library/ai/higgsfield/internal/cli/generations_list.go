...
func newGenerationsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "list",
		Short:       "List recent generation jobs from the web backend (cloud.higgsfield.ai). Returns the most-recent 100 by default.",
		Example:     "  higgsfield-pp-cli generations list",
		Annotations: map[string]string{"pp:endpoint": "generations.list", "pp:method": "GET", "pp:path": "/jobs", "mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			path := "/jobs"
			data, prov, err := resolvePaginatedRead(cmd.Context(), c, flags, "generations", path, map[string]string{
			...
			if asyncJobID := ExtractJobID(data, "request_id"); asyncJobID != "" {
				...
				if flagWait {
					...
					final, werr := WaitForJob(ctx, c, "/jobs/{request_id}", asyncJobID, WaitOptions{
	...
	cmd.Flags().IntVar(&flagSize, "size", 100, "Maximum number of jobs to return (1-100)")
	cmd.Flags().StringVar(&flagType, "type", "", "Filter by job type: image, video, or text")
