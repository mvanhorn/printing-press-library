# Cartesia Printed CLI Agent Guide

This directory is a generated `cartesia-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
cartesia-pp-cli doctor --json
cartesia-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
cartesia-pp-cli which "<capability>" --json
cartesia-pp-cli <command> --help
```
...
```bash
cartesia-pp-cli <command> --help
cartesia-pp-cli <command> --dry-run --agent
```
