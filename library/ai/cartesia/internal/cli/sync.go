...
Exit codes & warnings:
  ...
  cartesia-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  cartesia-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  cartesia-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  cartesia-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  cartesia-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "phone_numbers":
		return db.UpsertPhoneNumbers(data)
	case "datasets":
		return db.UpsertDatasets(data)
	case "files":
		return db.UpsertFiles(data)
	case "fine-tunes":
		return db.UpsertFineTunes(data)
	case "fine_tunes_voices":
		return db.UpsertFineTunesVoices(data)
	case "pronunciation-dicts":
		return db.UpsertPronunciationDicts(data)
	case "stt":
		return db.UpsertStt(data)
	case "voices":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"agents-metrics-results": "/agents/metrics/results",
		"agents-templates":       "/agents/templates",
		"datasets":               "/datasets/",
		"fine-tunes":             "/fine-tunes/",
		"pronunciation-dicts":    "/pronunciation-dicts/",
		"voices":                 "/voices",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "files", ParentTable: "datasets", ParentIDParam: "id", PathTemplate: "/datasets/{id}/files", KeyField: ""},
		{Name: "fine_tunes_voices", ParentTable: "fine-tunes", ParentIDParam: "id", PathTemplate: "/fine-tunes/{id}/voices", KeyField: ""},
	}
}
