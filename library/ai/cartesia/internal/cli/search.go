...
In local mode: searches locally synced data only.`,
  ...
  cartesia-pp-cli search "error timeout"

  # Force local search only
  cartesia-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  cartesia-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  cartesia-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'cartesia-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "agents":
				results, err = db.SearchAgents(query, limit)
			case "datasets":
				results, err = db.SearchDatasets(query, limit)
			case "fine-tunes":
				results, err = db.SearchFineTunes(query, limit)
			case "voices":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchAgents(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search agents failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchFineTunes(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search fine_tunes failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
