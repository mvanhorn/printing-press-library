...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cartesia-pp-cli doctor
  cartesia-pp-cli doctor --json
  cartesia-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export CARTESIA_API_KEY=<your-key> (or CARTESIA_ACCESS_TOKEN=<jwt>)"
				} else {
					authConfigured = true
			...
			// Check auth environment variables. CARTESIA_API_KEY (long-lived)
			// and CARTESIA_ACCESS_TOKEN (short-lived JWT) are alternatives;
			// either one satisfies auth.
			authEnvSet := []string{}
			authEnvRequiredMissing := []string{}
			authEnvInfo := []string{}
			authEnvOptionalNames := []string{"CARTESIA_API_KEY", "CARTESIA_ACCESS_TOKEN"}
			authEnvOptionalSatisfied := false
			if v := os.Getenv("CARTESIA_API_KEY"); v != "" {
				authEnvSet = append(authEnvSet, "CARTESIA_API_KEY")
				authEnvOptionalSatisfied = true
			}
			if v := os.Getenv("CARTESIA_ACCESS_TOKEN"); v != "" {
				authEnvSet = append(authEnvSet, "CARTESIA_ACCESS_TOKEN")
				authEnvOptionalSatisfied = true
			}
			if !authEnvOptionalSatisfied {
				authEnvInfo = append(authEnvInfo, "CARTESIA_API_KEY or CARTESIA_ACCESS_TOKEN")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'cartesia-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'cartesia-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'cartesia-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'cartesia-pp-cli sync' to refresh."
	}
	return report
}
