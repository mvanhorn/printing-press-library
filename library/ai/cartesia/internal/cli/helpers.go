...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			"\n      Set your API key: export CARTESIA_API_KEY=<your-key>"+
			"\n      Run 'cartesia-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token. Set it with: cartesia-pp-cli auth set-token <token>"+
			"\n      or: export CARTESIA_API_KEY=<your-key>"+
			"\n      Run 'cartesia-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: permission denied. Your credentials are valid but lack access to this resource."+
			"\n      Check that your API key has the required permissions."+
			"\n      Set it with: export CARTESIA_API_KEY=<your-key>"+
			"\n      Run 'cartesia-pp-cli doctor' to check auth status.", err))
...
// parseSince accepts either a relative window ("24h", "7d", "30m", "1w") or
// an absolute RFC3339 timestamp ("2026-05-13T00:00:00Z"). Returns the
// resolved point-in-time. Empty input returns the zero time with no error
// so callers can treat "no filter" as a value.
func parseSince(s string) (time.Time, error) {
	s = strings.TrimSpace(s)
	if s == "" {
		return time.Time{}, nil
	}
	// Try RFC3339 first because the relative-duration regex would silently
	// reject anything with a colon and we want a clear error path for
