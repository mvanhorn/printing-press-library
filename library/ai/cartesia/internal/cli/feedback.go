...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".cartesia-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CARTESIA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CARTESIA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "cartesia-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.cartesia-pp-cli/feedback.jsonl.
When ` + "`CARTESIA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CARTESIA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cartesia-pp-cli feedback list
  cartesia-pp-cli feedback list --limit 5
  cartesia-pp-cli feedback list --json`,
