...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  cartesia-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  cartesia-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"agents", "agents-metrics", "agents-metrics-results", "agents-templates", "datasets", "fine-tunes", "pronunciation-dicts", "voices"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  cartesia-pp-cli workflow status

  # Show status as JSON
  cartesia-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("cartesia-pp-cli")
			}
