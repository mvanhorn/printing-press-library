...
// TestUpsertBatch_PopulatesFindallTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed findall table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesFindallTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("findall", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "findall").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "findall")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count findall: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("findall count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

func TestSearchFindallQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if stored, failed, err := s.UpsertBatch("findall", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	} else if failed != 0 || stored != len(items) {
		t.Fatalf("UpsertBatch stored=%d failed=%d, want stored=%d failed=0", stored, failed, len(items))
	}
	...
	for _, query := range []string{
		...
		results, err := s.SearchFindall(query, 10)
		if err != nil {
			t.Fatalf("SearchFindall(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchFindall(%q) returned no results", query)
		}
	}
}
...
func TestUpsertBatch_TypedFailureDoesNotStrandCancelGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("cancel count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "monitors_id")
	}
}
...
func TestUpsertBatch_KeysCancelByChildAndParent(t *testing.T) {
	...
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "monitors_id" = ?`, "cancel")
...
func TestUpsertBatch_TypedFailureDoesNotStrandEventsGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("events count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "monitors_id")
	}
}
...
func TestUpsertBatch_KeysEventsByChildAndParent(t *testing.T) {
	...
	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "monitors_id" = ?`, "events")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by monitors_id: %v", err)
	}
	if parentMatches != 2 {
...
func TestUpsertBatch_TypedFailureDoesNotStrandUpdateGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("update count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "monitors_id")
	}
}

// TestUpsertBatch_KeysUpdateByChildAndParent verifies dependent
// sub-collection rows with the same child id but different parent ids do not
// overwrite one another in either the generic resources table or the typed
// table. Regression for issue #2471.
func TestUpsertBatch_KeysUpdateByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("update count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "monitors_id" = ?`, "update")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by monitors_id: %v", err)
	}
	if parentMatches != 2 {
