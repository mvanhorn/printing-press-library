...
type Config struct {
	...
	ParallelApiKey   string          `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("PARALLEL_API_KEY"); v != "" {
		cfg.ParallelApiKey = v
		cfg.markEnvOverride("ParallelApiKey")
		cfg.AuthSource = "env:PARALLEL_API_KEY"
		cfg.CredentialSource = "env:PARALLEL_API_KEY"
	}
	...
	if v := os.Getenv("PARALLEL_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("PARALLEL_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "parallel-pp-cli", "config.toml"), nil
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.ParallelApiKey = creds.ParallelApiKey
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.ParallelApiKey = token
	delete(c.envOverrides, "ParallelApiKey")
	c.updateFileConfigField("ParallelApiKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["ParallelApiKey"] {
			out.ParallelApiKey = c.fileConfig.ParallelApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "ParallelApiKey":
		c.fileConfig.ParallelApiKey = c.ParallelApiKey
	}
}
