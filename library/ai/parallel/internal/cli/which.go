...
var whichIndex = []whichEntry{
	{Command: "session stitch", Description: "Bind search, extract, and task/findall runs into one local session chain for agent resume.", Group: "Local state that compounds", WhyItMatters: "Use when an agent needs to resume a multi-step Parallel research loop without re-fetching."},
	{Command: "tasks guard", Description: "Refuse Task creates when prepaid balance is below a threshold.", Group: "Spend control", WhyItMatters: "Use before expensive Task Groups when you must avoid surprise credit burn."},
	{Command: "monitors digest", Description: "Mechanical per-monitor event counts and top titles since a duration window.", Group: "Local state that compounds", WhyItMatters: "Use for Monday triage of which monitors fired or went quiet."},
	{Command: "research recall", Description: "FTS across local searches, extracts, and task summaries with typed hit IDs.", Group: "Local state that compounds", WhyItMatters: "Use before paying for a live search when prior local research may already answer."},
	{Command: "findall promote", Description: "Turn FindAll candidates into a Task Group enrichment job.", Group: "Research pipelines", WhyItMatters: "Use when entity discovery should immediately become batch enrichment."},
	{Command: "balance burn", Description: "Diff local balance snapshots against local run volume over a window.", Group: "Spend control", WhyItMatters: "Use when explaining weekly credit burn without opening the dashboard."},
	{Command: "tasks lineage", Description: "Print the offline previous_interaction_id follow-up chain for a run.", Group: "Research pipelines", WhyItMatters: "Use when debugging multi-turn deep research context chains offline."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  parallel-pp-cli which "stale tickets"
  parallel-pp-cli which "bottleneck"
  parallel-pp-cli which --limit 1 "send message"
  parallel-pp-cli which                                # list the full capability index`,
