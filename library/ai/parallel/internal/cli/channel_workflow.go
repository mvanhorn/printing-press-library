...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  parallel-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  parallel-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"monitors", "service"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  parallel-pp-cli workflow status

  # Show status as JSON
  parallel-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("parallel-pp-cli")
			}
