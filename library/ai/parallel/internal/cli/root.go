...
type rootFlags struct {
	...
	// invocation. Mirrors the PARALLEL_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "parallel-pp-cli",
		Short: `Parallel CLI — Agent-native Parallel web research with a local SQLite memory no other Parallel CLI keeps.`,
		Long: `Parallel CLI — Agent-native Parallel web research with a local SQLite memory no other Parallel CLI keeps.
...
Health check: run 'parallel-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("parallel-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'parallel-pp-cli profile list')")
	...
	rootCmd.AddCommand(newFindallCmd(flags))
	...
	rootCmd.AddCommand(newNovelBalanceCmd(flags))
	rootCmd.AddCommand(newNovelResearchCmd(flags))
	rootCmd.AddCommand(newNovelSessionCmd(flags))
