...
// parallel-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/ai/parallel/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/ai/parallel/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/ai/parallel/internal/learn/patterns"
...
const noLearnEnvVar = "PARALLEL_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("parallel-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  parallel-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  parallel-pp-cli recall "<question>" --agent
  parallel-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List recorded learnings with optional query, source, and warning filters",
		Example: `  parallel-pp-cli learnings list --agent
  parallel-pp-cli learnings list --query "<substring>"
  parallel-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  parallel-pp-cli learnings forget "<question>" --resource <id>
  parallel-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  parallel-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  parallel-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
