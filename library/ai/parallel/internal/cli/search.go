...
In local mode: searches locally synced data only.`,
  ...
  parallel-pp-cli search "error timeout"

  # Force local search only
  parallel-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  parallel-pp-cli search "status" --type chat --data-source local
  # JSON output for piping
  parallel-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'parallel-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "findall":
				results, err = db.SearchFindall(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchFindall(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search findall failed: %w", searchErr)
					}
