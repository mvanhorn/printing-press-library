...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/ai/parallel/internal/learn/lookups"
...
Resource scoping:
  ...
  parallel-pp-cli sync

  # Sync specific resources only
  parallel-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  parallel-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  parallel-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  parallel-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  parallel-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("parallel-pp-cli")
			}
...
func responsePathForResource(resource, path string) []string {
	...
	case "findall\x00/v1beta/findall/runs/{findall_id}/enrich":
		return []string{"match_conditions"}
	case "findall\x00/v1beta/findall/entity-search":
		return []string{"entities"}
	case "findall\x00/v1beta/findall/runs/{findall_id}/extend":
		return []string{"match_conditions"}
	case "findall\x00/v1beta/findall/runs/{findall_id}/result":
		return []string{"candidates"}
	case "findall\x00/v1beta/findall/runs/{findall_id}/schema":
		return []string{"match_conditions"}
	case "findall\x00/v1beta/findall/ingest":
		return []string{"match_conditions"}
	case "monitors\x00/v1/monitors":
		return []string{"monitors"}
	case "service\x00/account/service/v1/apps":
		return []string{"apps"}
	case "tasks\x00/v1/tasks/groups/{taskgroup_id}/runs":
		return []string{"run_ids"}
