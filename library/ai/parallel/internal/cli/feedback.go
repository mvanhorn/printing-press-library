...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PARALLEL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PARALLEL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "parallel-pp-cli/feedback")
...
When ` + "`PARALLEL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PARALLEL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  parallel-pp-cli feedback list
  parallel-pp-cli feedback list --limit 5
  parallel-pp-cli feedback list --json`,
