...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("doctor_before_spend",
			mcplib.WithDescription("Confirm auth wiring without calling paid endpoints"),
			mcplib.WithBoolean("dry_run", mcplib.Description("Override the recipe's --dry-run value.")),
		),
		handleDoctorBeforeSpend,
	)
}

// handleDoctorBeforeSpend runs the doctor_before_spend recipe intent: Confirm auth wiring without calling paid endpoints
func handleDoctorBeforeSpend(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
