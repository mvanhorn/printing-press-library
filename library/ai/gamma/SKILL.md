...
### Pitch deck with AI images

```bash
gamma-pp-cli generations create --input-text "Seed round pitch: problem, solution, market, team, ask" --format presentation --image-source aiGenerated --sharing view-link --watch --agent
```

Generate a presentation with AI images and a shareable view link in one command.

### Document report

```bash
gamma-pp-cli generations create --input-text "Q4 2025 AI regulation summary: key developments, implications, action items" --format document --watch --download /tmp/reports/ --agent
```

Generate a document and download the export to a local directory.

### Landing page

```bash
gamma-pp-cli generations create --input-text "Landing page for a B2B developer productivity SaaS" --format webpage --watch --sharing view-link --agent
```

Generate a webpage with a shareable link for review.

### List themes first

```bash
gamma-pp-cli themes --agent --select id,name
```

List available themes before generating to pick the right visual style.

### Template-based generation

```bash
gamma-pp-cli generations create-from-template --gamma-id g_abc123 --prompt "Update with Q4 numbers and new timeline" --watch --agent
```

Adapt an existing Gamma (by its g_... ID) to new content, preserving the original structure.
