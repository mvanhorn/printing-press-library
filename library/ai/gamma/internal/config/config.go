...
type Config struct {
	...
	GammaApiKey   string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("GAMMA_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "gamma-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("GAMMA_API_KEY"); v != "" {
		cfg.GammaApiKey = v
		cfg.markEnvOverride("GammaApiKey")
		cfg.AuthSource = "env:GAMMA_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.GammaApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("GAMMA_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.GammaApiKey = token
	delete(c.envOverrides, "GammaApiKey")
	c.updateFileConfigField("GammaApiKey")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["GammaApiKey"] {
			out.GammaApiKey = c.fileConfig.GammaApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "GammaApiKey":
		c.fileConfig.GammaApiKey = c.GammaApiKey
	}
}
