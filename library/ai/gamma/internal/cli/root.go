...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "gamma-pp-cli",
		Short: "Manage gamma resources via the gamma API",
		Long: `Manage gamma resources via the gamma API.

Authentication: set GAMMA_API_KEY=<your-key> or run 'gamma-pp-cli auth set-token <token>'.
...
Run 'gamma-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("gamma-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'gamma-pp-cli profile list')")
	rootCmd.PersistentFlags().StringVar(&flags.deliverSpec, "deliver", "", "Route output to a sink: stdout (default), file:<path>, webhook:<url>")
	rootCmd.PersistentFlags().Float64Var(&flags.rateLimit, "rate-limit", 0, "Max requests per second (0 to disable)")
	rootCmd.PersistentFlags().StringVar(&flags.apiKey, "api-key", "", "API key (env: GAMMA_API_KEY)")
	_ = rootCmd.PersistentFlags().SetAnnotation("api-key", "env", []string{"GAMMA_API_KEY"})
	...
	rootCmd.AddCommand(newGammasPromotedCmd(flags))
	rootCmd.AddCommand(newThemesPromotedCmd(flags))
...
func (f *rootFlags) newClient() (*client.Client, error) {
	...
	if f.apiKey != "" {
		cfg.GammaApiKey = f.apiKey
		cfg.AuthSource = "flag:api-key"
	}
