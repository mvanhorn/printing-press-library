...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "gamma-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GAMMA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GAMMA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/ai/gamma/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/gamma-pp-cli/feedback.jsonl.
When ` + "`GAMMA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GAMMA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gamma-pp-cli feedback list
  gamma-pp-cli feedback list --limit 5
  gamma-pp-cli feedback list --json`,
