...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  gamma-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  gamma-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"folders", "themes"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  gamma-pp-cli workflow status

  # Show status as JSON
  gamma-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("gamma-pp-cli")
			}
