...
Resource scoping:
  ...
  gamma-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  gamma-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  gamma-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  gamma-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  gamma-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/gamma-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "gammas":
		return db.UpsertGammas(data)
	case "archive":
		return db.UpsertArchive(data)
	case "generations":
		return db.UpsertGenerations(data)
	case "themes":
		return db.UpsertThemes(data)
	default:
