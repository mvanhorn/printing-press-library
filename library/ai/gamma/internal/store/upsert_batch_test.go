...
// TestUpsertBatch_PopulatesGammasTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed gammas table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesGammasTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("gammas", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "gammas").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "gammas")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count gammas: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("gammas count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
...
func TestUpsertBatch_PopulatesArchiveTable(t *testing.T) {
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "gammas_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "gammas_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "gammas_id": "test-parent-001"}`),
	}
...
func TestUpsertBatch_TypedFailureDoesNotStrandArchiveGeneric(t *testing.T) {
	...
	// Items deliberately omit "gammas_id" so the typed NOT NULL
	// constraint fires.
	items := []json.RawMessage{
	...
	if typed != 0 {
		t.Fatalf("archive count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "gammas_id")
	}
}
...
func TestUpsertBatch_KeysArchiveByChildAndParent(t *testing.T) {
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "shared_child", "gammas_id": "parent_A"}`),
		json.RawMessage(`{"id": "shared", "gammas_id": "child_parent_A"}`),
		json.RawMessage(`{"id": "shared_child", "gammas_id": "parent_B"}`),
		json.RawMessage(`{"id": "other_child", "gammas_id": "parent_A"}`),
	}
	stored, extractFailures, err := s.UpsertBatch("archive", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	...
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "gammas_id" = ?`, "archive")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by gammas_id: %v", err)
	}
	if parentMatches != 2 {
		t.Fatalf("gammas_id=parent_A count = %d, want 2", parentMatches)
	}
}

// TestUpsertBatch_PopulatesGenerationsTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed generations table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesGenerationsTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("generations", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "generations").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "generations")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count generations: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("generations count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_PopulatesThemesTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed themes table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesThemesTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("themes", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "themes").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "themes")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count themes: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("themes count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
