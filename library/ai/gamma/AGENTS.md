# Gamma Printed CLI Agent Guide

This directory is a generated `gamma-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
gamma-pp-cli doctor --json
gamma-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
gamma-pp-cli which "<capability>" --json
gamma-pp-cli <command> --help
```
...
```bash
gamma-pp-cli <command> --help
gamma-pp-cli <command> --dry-run --agent
```
