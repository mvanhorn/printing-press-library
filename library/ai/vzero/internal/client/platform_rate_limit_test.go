...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/ai/vzero/internal/cliutil"
	"github.com/mvanhorn/printing-press-library/library/ai/vzero/internal/config"
	"github.com/mvanhorn/printing-press-library/library/ai/vzero/internal/platform"
)
...
func platformRateLimitTestClient(t *testing.T, server *httptest.Server) (*Client, *platform.Session) {
	...
	session, err := platform.PrepareProfileSourceFromConfig(&platform.Profile{
		...
		Sources: map[string]platform.SourceProfile{
			"v0": {CredentialRef: "fixture://Test/Synthetic/token", ExpectedAccountID: "acct-a", ExpectedOrgName: "Tenant A", ExpectedStoreID: "store-a", ExpectedStoreDomain: "tenant-a.example.test", ExpectedPropertyID: "123", ExpectedBaseURL: "https://tenant-a.example.test"},
		},
	}, "v0-pp-cli", "v0")
	if err != nil {
		t.Fatal(err)
