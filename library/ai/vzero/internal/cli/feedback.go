...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("V0_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("V0_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "v0-pp-cli/feedback")
...
When ` + "`V0_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`V0_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  v0-pp-cli feedback list
  v0-pp-cli feedback list --limit 5
  v0-pp-cli feedback list --json`,
