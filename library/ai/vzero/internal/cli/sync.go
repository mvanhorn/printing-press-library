...
Resource scoping:
  ...
  v0-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  v0-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  v0-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  v0-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  v0-pp-cli sync --latest-only`,
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "messages", ParentTable: "chats", ParentIDParam: "chatId", PathTemplate: "/chats/{chatId}/messages", KeyField: "", ReconcileMode: "none", GenericScopeJSONPath: "", PathParams: []dependentPathParamDef{
...
func responsePathForResource(resource, path string) []string {
	...
	case "chats\x00/chats/{chatId}/files":
		return []string{"files"}
	case "chats\x00/chats":
		return []string{"chats"}
	case "chats\x00/chats/{chatId}/restore-message":
		return []string{"messages"}
	case "hooks\x00/hooks":
		return []string{"items"}
	case "mcp-servers\x00/mcp-servers":
		return []string{"items"}
	case "messages\x00/chats/{chatId}/messages":
		return []string{"messages"}
	case "settings\x00/settings/preview-hosts":
		return []string{"hosts"}
	}
	return nil
}
