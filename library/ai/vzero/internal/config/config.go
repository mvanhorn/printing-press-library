...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("V0_API_KEY"); v != "" {
		cfg.V0ApiKey = v
		cfg.markEnvOverride("V0ApiKey")
		cfg.AuthSource = "env:V0_API_KEY"
		cfg.CredentialSource = "env:V0_API_KEY"
	}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("V0_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "v0-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.V0ApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:V0_API_KEY"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"api_key":      c.V0ApiKey,
			"V0_API_KEY":   c.V0ApiKey,
			"token":        c.V0ApiKey,
			"access_token": c.V0ApiKey,
		})
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.V0ApiKey = ""
	delete(c.envOverrides, "V0ApiKey")
	c.updateFileConfigField("V0ApiKey")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["V0ApiKey"] {
			out.V0ApiKey = c.fileConfig.V0ApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "V0ApiKey":
		c.fileConfig.V0ApiKey = c.V0ApiKey
	}
}
