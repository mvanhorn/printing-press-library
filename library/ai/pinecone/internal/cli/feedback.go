...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PINECONE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PINECONE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "pinecone-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pinecone-pp-cli feedback "text-query returned unexpected results for a 1536-dim index"
  pinecone-pp-cli feedback --stdin
  pinecone-pp-cli feedback list --json`,
...
When ` + "`PINECONE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PINECONE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pinecone-pp-cli feedback list
  pinecone-pp-cli feedback list --limit 5
  pinecone-pp-cli feedback list --json`,
