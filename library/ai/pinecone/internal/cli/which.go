...
var whichIndex = []whichEntry{
	{Command: "text-query", Description: "Search a dense index with a natural-language query — embeds the text via Pinecone Inference, then queries the index — without managing vectors yourself.", Group: "Search that works the way humans think", WhyItMatters: "Use when an agent needs semantic retrieval from an index but only has a text query, not a precomputed vector."},
	{Command: "snapshot", Description: "Capture a point-in-time state of any index (per-namespace vector counts, dimension, metric, host) into local SQLite, then diff against prior snapshots to see exactly what changed.", Group: "Local state that compounds", WhyItMatters: "Use when an agent needs to answer \"what changed in this index since last week?\" for drift, growth, or incident review."},
	{Command: "prune", Description: "Find vectors whose local metadata timestamps are older than a threshold and delete them in batches — dry-run by default, with --apply to commit.", Group: "Memory lifecycle", WhyItMatters: "Use when an agent maintains a long-lived memory store and needs to expire stale chunks without a full resync."},
	{Command: "cascade", Description: "Run one semantic query across multiple indexes and merge the ranked results (deduped by vector ID, best score wins) into a single agent-ready list.", Group: "Search that works the way humans think", WhyItMatters: "Use when an agent needs to search across several indexes (e.g. prod vs staging, multiple tenants) in one shot."},
	{Command: "usage", Description: "Compute vector-count growth and per-namespace distribution shifts from snapshot history, with a projection to a future horizon.", Group: "Local state that compounds", WhyItMatters: "Use when an agent needs to forecast index growth or spot a namespace ballooning before it hits quota."},
	{Command: "check-vectors", Description: "Validate a vectors JSON file against the index schema — dimension, duplicate/empty IDs, sparse/dense shape — before you upsert and burn write units on a rejected batch.", Group: "Memory lifecycle", WhyItMatters: "Use when an agent ingests data and wants to catch dimension or ID errors before a costly rejected batch."},
	{Command: "coverage", Description: "Join backups, backup schedules, and restore jobs into a per-index protection matrix showing which indexes are backed up, scheduled, and restorable.", Group: "Local state that compounds", WhyItMatters: "Use when an agent audits disaster-recovery posture across an index fleet."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  pinecone-pp-cli which "stale tickets"
  pinecone-pp-cli which "bottleneck"
  pinecone-pp-cli which --limit 1 "send message"
  pinecone-pp-cli which                                # list the full capability index`,
