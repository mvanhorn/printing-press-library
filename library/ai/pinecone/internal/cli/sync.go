...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/ai/pinecone/internal/learn/lookups"
...
Resource scoping:
  ...
  pinecone-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  pinecone-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  pinecone-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("pinecone-pp-cli")
			}
...
func determinePaginationDefaults(resource string) paginationDefaults {
	...
	case "indexes/indexes_backup_schedules":
		return paginationDefaults{
			cursorParam:    "",
			cursorType:     "offset",
			nextCursorPath: "",
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "indexes_backup_schedules":
		return db.UpsertIndexesBackupSchedules(data)
	case "indexes_backups":
		return db.UpsertIndexesBackups(data)
	case "models":
		return db.UpsertModels(data)
	case "namespaces":
		return db.UpsertNamespaces(data)
	case "operations":
		return db.UpsertOperations(data)
	case "restore-jobs":
		return db.UpsertRestoreJobs(data)
	case "vectors":
		return db.UpsertVectors(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		...
		"history":                "/backup-schedules/{schedule_id}/history",
		"indexes":                "/indexes",
		"models":                 "/models",
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "indexes_backup_schedules", ParentTable: "indexes", ParentIDParam: "index_name", PathTemplate: "/indexes/{index_name}/backup-schedules", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.index", PathParams: []dependentPathParamDef{
			{Param: "index_name", Field: "name"},
		}},
		{Name: "indexes_backups", ParentTable: "indexes", ParentIDParam: "index_name", PathTemplate: "/indexes/{index_name}/backups", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.index", PathParams: []dependentPathParamDef{
			{Param: "index_name", Field: "name"},
		}},
	}
}
...
var reconcileTypedTables = map[string]string{
	...
	"history":                  "history",
	"indexes":                  "indexes",
	"indexes_backup_schedules": "indexes_backup_schedules",
	"indexes_backups":          "indexes_backups",
	"models":                   "models",
	"namespaces":               "namespaces",
...
func responsePathForResource(resource, path string) []string {
	...
	case "admin\x00/admin/api-keys/{api_key_id}":
		return []string{"roles"}
	case "admin\x00/admin/invites":
		return []string{"data"}
	case "admin\x00/admin/organizations":
		return []string{"data"}
	case "admin\x00/admin/projects/{project_id}/api-keys":
		return []string{"data"}
	case "admin\x00/admin/projects":
		return []string{"data"}
	case "admin\x00/admin/role-bindings":
		return []string{"data"}
	case "admin\x00/admin/service-accounts":
		return []string{"data"}
	case "admin\x00/admin/users":
		return []string{"data"}
	case "assistants\x00/assistants":
		return []string{"assistants"}
	case "backups\x00/backups":
		return []string{"data"}
	case "bulk\x00/bulk/imports":
		return []string{"data"}
	case "chat\x00/chat/{assistant_name}":
		return []string{"citations"}
	case "collections\x00/collections":
		return []string{"collections"}
	case "embed\x00/embed":
		return []string{"data"}
	case "files\x00/files/{assistant_name}":
		return []string{"files"}
	case "indexes\x00/indexes":
		return []string{"indexes"}
	case "models\x00/models":
		return []string{"models"}
	case "namespaces\x00/namespaces":
		return []string{"namespaces"}
	case "operations\x00/operations/{assistant_name}":
