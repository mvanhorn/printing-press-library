...
type rootFlags struct {
	...
	// invocation. Mirrors the PINECONE_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "pinecone-pp-cli",
		Short: `Every Pinecone API feature, plus local sync, snapshot history, and text-first search no other Pinecone tool has.`,
		Long: `Every Pinecone API feature, plus local sync, snapshot history, and text-first search no other Pinecone tool has.
...
See README.md or the bundled SKILL.md for recipes.`,
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select created_at,host,instructions)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'pinecone-pp-cli profile list')")
	...
	rootCmd.AddCommand(newBackupSchedulesCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelCascadeCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelCheckVectorsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelCoverageCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelPruneCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSnapshotCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelTextQueryCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelUsageCmd(flags))
