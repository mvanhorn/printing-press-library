...
type Config struct {
	...
	PineconeApiKey   string          `toml:"api_key"`
...
func Load(configPath string) (*Config, error) {
	...
	if v := os.Getenv("PINECONE_API_KEY"); v != "" {
		cfg.PineconeApiKey = v
		cfg.markEnvOverride("PineconeApiKey")
		cfg.AuthSource = "env:PINECONE_API_KEY"
		cfg.CredentialSource = "env:PINECONE_API_KEY"
	}
	...
	if v := os.Getenv("PINECONE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("PINECONE_ASSISTANT_HOST")); v != "" {
		cfg.TemplateVars["assistant_host"] = v
	} else if verifyMode {
		cfg.TemplateVars["assistant_host"] = "assistant_host_placeholder"
	}
	if v := strings.TrimSpace(os.Getenv("PINECONE_INDEX_HOST")); v != "" {
		cfg.TemplateVars["index_host"] = v
	} else if verifyMode {
		cfg.TemplateVars["index_host"] = "index_host_placeholder"
	}
	if v := strings.TrimSpace(os.Getenv("PINECONE_SCHEDULE_ID")); v != "" {
		cfg.TemplateVars["schedule_id"] = v
	} else if verifyMode {
		cfg.TemplateVars["schedule_id"] = "schedule_id_placeholder"
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("PINECONE_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "pinecone-pp-cli", "config.toml"), nil
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.PineconeApiKey == "" {
		c.PineconeApiKey = creds.PineconeApiKey
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.PineconeApiKey = token
	delete(c.envOverrides, "PineconeApiKey")
	c.updateFileConfigField("PineconeApiKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["PineconeApiKey"] {
			out.PineconeApiKey = c.fileConfig.PineconeApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "PineconeApiKey":
		c.fileConfig.PineconeApiKey = c.PineconeApiKey
	}
}
