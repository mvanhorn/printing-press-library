...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".operon-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("OPERON_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("OPERON_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "operon-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.operon-pp-cli/feedback.jsonl.
When ` + "`OPERON_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`OPERON_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  operon-pp-cli feedback list
  operon-pp-cli feedback list --limit 5
  operon-pp-cli feedback list --json`,
