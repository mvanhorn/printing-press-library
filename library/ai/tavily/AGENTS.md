# Tavily Printed CLI Agent Guide

This directory is a generated `tavily-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
tavily-pp-cli doctor --json
tavily-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
tavily-pp-cli which "<capability>" --json
tavily-pp-cli <command> --help
```
...
```bash
tavily-pp-cli <command> --help
tavily-pp-cli <command> --dry-run --agent
```
