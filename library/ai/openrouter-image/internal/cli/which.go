...
var whichIndex = []whichEntry{
	{Command: "models rank", Description: "Rank every image model+provider combo cheapest-first under your capability and budget constraints.", Group: "Local state that compounds", WhyItMatters: "Pick the cheapest provider that meets your image constraints without paging through catalog JSON."},
	{Command: "batch", Description: "Run many generations from a CSV with a hard USD budget: estimate first, abort before any spend if over, then execute and log each cost.", Group: "Agent-native plumbing", WhyItMatters: "Cron pipelines can fire a batch with a hard spend cap and get typed exit codes instead of burning the whole balance."},
	{Command: "cost-estimate", Description: "Estimate USD cost of a generation before spending credits, computed offline from synced per-endpoint pricing.", Group: "Local state that compounds", WhyItMatters: "Agents can check the price of a planned image before spending credits."},
	{Command: "regenerate", Description: "Re-run a past generation with its exact stored parameters (model, seed, resolution, quality, references).", Group: "Local state that compounds", WhyItMatters: "Reproduce or tweak a past image without re-typing the full flag set."},
	{Command: "models diff", Description: "See newly added, retired, and price-changed image models between syncs so pinned pipelines never break silently.", Group: "Reachability mitigation", WhyItMatters: "Catch a retired model before the next scheduled batch 404s."},
	{Command: "usage digest", Description: "Period-over-period spend and volume summary: images generated, USD spent, top models, cost per image vs the prior window.", Group: "Local state that compounds", WhyItMatters: "Budget owners get a machine-readable weekly cost report from the local ledger."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  openrouter-image-pp-cli which "stale tickets"
  openrouter-image-pp-cli which "bottleneck"
  openrouter-image-pp-cli which --limit 1 "send message"
  openrouter-image-pp-cli which                                # list the full capability index`,
