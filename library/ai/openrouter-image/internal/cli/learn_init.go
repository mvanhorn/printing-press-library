...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^[a-z0-9-]+/[a-z0-9.-]+$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: openrouter-image-pp-cli: ticker pattern %q failed to compile: %v\n", `^[a-z0-9-]+/[a-z0-9.-]+$`, err)
	} else {
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"image_model": {
			{Canonical: "openai/gpt-image-1", Values: []string{"gpt-image", "gpt image", "gpt-image-1"}},
			{Canonical: "google/gemini-2.5-flash-image", Values: []string{"gemini image", "gemini flash image", "gemini-2.5-flash-image"}},
			{Canonical: "bytedance-seed/seedream-4.5", Values: []string{"seedream", "seedream 4.5"}},
			{Canonical: "black-forest-labs/flux.2-pro", Values: []string{"flux", "flux 2 pro", "bfl"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: openrouter-image-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: openrouter-image-pp-cli: learn init: %v\n", err)
		}
	})
}
