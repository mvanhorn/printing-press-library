...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  openrouter-image-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  openrouter-image-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"activity", "benchmarks", "byok", "datasets", "datasets-rankings-daily", "embeddings", "endpoints", "files", "guardrails", "guardrails-assignments-keys", "guardrails-assignments-members", "images", "keys", "models", "models-user", "observability", "organization", "presets", "providers", "scim", "scim-group-mappings", "videos", "workspaces"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  openrouter-image-pp-cli workflow status

  # Show status as JSON
  openrouter-image-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("openrouter-image-pp-cli")
			}
