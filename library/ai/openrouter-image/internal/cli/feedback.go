...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("OPENROUTER_IMAGE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("OPENROUTER_IMAGE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "openrouter-image-pp-cli/feedback")
...
When ` + "`OPENROUTER_IMAGE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`OPENROUTER_IMAGE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  openrouter-image-pp-cli feedback list
  openrouter-image-pp-cli feedback list --limit 5
  openrouter-image-pp-cli feedback list --json`,
