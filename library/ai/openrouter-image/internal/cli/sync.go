...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/ai/openrouter-image/internal/learn/lookups"
...
Resource scoping:
  ...
  openrouter-image-pp-cli sync

  # Sync specific resources only
  openrouter-image-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  openrouter-image-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  openrouter-image-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  openrouter-image-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  openrouter-image-pp-cli sync --latest-only`,
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		...
		"providers":                      "/providers",
		"scim":                           "/scim/groups",
		"scim-group-mappings":            "/scim/group-mappings",
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "assignments", ParentTable: "guardrails", ParentIDParam: "id", PathTemplate: "/guardrails/{id}/assignments/keys", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.guardrail", PathParams: []dependentPathParamDef{
			{Param: "id", Field: "id"},
		}},
		{Name: "assignments", ParentTable: "guardrails", ParentIDParam: "id", PathTemplate: "/guardrails/{id}/assignments/members", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.guardrail", PathParams: []dependentPathParamDef{
			{Param: "id", Field: "id"},
		}},
...
func responsePathForResource(resource, path string) []string {
	...
	case "activity\x00/activity":
		return []string{"data"}
	case "benchmarks\x00/benchmarks":
		return []string{"data"}
	case "byok\x00/byok":
		return []string{"data"}
	case "chat\x00/chat/completions":
		return []string{"choices"}
	case "datasets\x00/datasets/app-rankings":
		return []string{"data"}
	case "datasets\x00/datasets/rankings-daily":
		return []string{"data"}
	case "embeddings\x00/embeddings":
		return []string{"data"}
	case "embeddings\x00/embeddings/models":
		return []string{"data"}
	case "endpoints\x00/endpoints/zdr":
		return []string{"data"}
	case "guardrails\x00/guardrails":
		return []string{"data"}
	case "guardrails\x00/guardrails/assignments/keys":
		return []string{"data"}
	case "guardrails\x00/guardrails/assignments/members":
		return []string{"data"}
	case "images\x00/images":
		return []string{"data"}
	case "images\x00/images/models/{author}/{slug}/endpoints":
		return []string{"endpoints"}
	case "images\x00/images/models":
		return []string{"data"}
	case "keys\x00/keys":
		return []string{"data"}
	case "models\x00/models":
		return []string{"data"}
	case "models\x00/models/user":
		return []string{"data"}
	case "observability\x00/observability/destinations":
		return []string{"data"}
	case "organization\x00/organization/members":
		return []string{"data"}
	case "presets\x00/presets":
		return []string{"data"}
	case "providers\x00/providers":
		return []string{"data"}
	case "rerank\x00/rerank":
		return []string{"results"}
	case "responses\x00/responses/compact":
		return []string{"output"}
	case "scim\x00/scim/group-mappings":
		return []string{"data"}
	case "scim\x00/scim/groups":
		return []string{"data"}
	case "videos\x00/videos":
		return []string{"unsigned_urls"}
	case "videos\x00/videos/{jobId}":
		return []string{"unsigned_urls"}
	case "videos\x00/videos/models":
		return []string{"data"}
	case "workspaces\x00/workspaces":
		return []string{"data"}
	}
	return nil
}
