# NotebookLM Printed CLI Agent Guide

This directory is a generated `notebooklm-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
notebooklm-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
notebooklm-pp-cli <command> --help
```
...
```bash
notebooklm-pp-cli <command> --agent
```
...
```bash
notebooklm-pp-cli <command> --help
notebooklm-pp-cli <command> --dry-run --agent
```
...
NotebookLM has no public API key. Authenticate once with Chrome session cookies:
...
For offline cache commands (`search`, `sql`, `stats`, `export`), run `sync` first.
