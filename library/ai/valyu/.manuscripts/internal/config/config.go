...
type Config struct {
	...
	ValyuApiKey   string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("VALYU_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "valyu-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("VALYU_API_KEY"); v != "" {
		cfg.ValyuApiKey = v
		cfg.markEnvOverride("ValyuApiKey")
		cfg.AuthSource = "env:VALYU_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.ValyuApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("VALYU_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.ValyuApiKey = token
	delete(c.envOverrides, "ValyuApiKey")
	c.updateFileConfigField("ValyuApiKey")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["ValyuApiKey"] {
			out.ValyuApiKey = c.fileConfig.ValyuApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "ValyuApiKey":
		c.fileConfig.ValyuApiKey = c.ValyuApiKey
	}
}
