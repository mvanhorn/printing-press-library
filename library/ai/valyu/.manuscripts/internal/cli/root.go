...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "valyu-pp-cli",
		Short: "Manage valyu resources via the valyu API",
		Long: `Manage valyu resources via the valyu API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'valyu-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("valyu-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'valyu-pp-cli profile list')")
	...
	// contents + novel pipe subcommand
	contentsCmd := newContentsCmd(flags)
	contentsCmd.AddCommand(newContentsPipeCmd(flags))
	rootCmd.AddCommand(contentsCmd)

	rootCmd.AddCommand(newDatasourcesCmd(flags))

	// deepresearch + novel queue subcommand
	drCmd := newDeepresearchCmd(flags)
	drCmd.AddCommand(newResearchQueueCmd(flags))
	...
	// semantic-search + novel subcommands
	ssCmd := newSemanticSearchPromotedCmd(flags)
	ssCmd.AddCommand(newSearchCompareCmd(flags))
	ssCmd.AddCommand(newSearchBatchCmd(flags))
	ssCmd.AddCommand(newSearchSaveCmd(flags))
	ssCmd.AddCommand(newSearchRunTemplateCmd(flags))
	ssCmd.AddCommand(newSearchListSavedCmd(flags))
	ssCmd.AddCommand(newSearchDeleteSavedCmd(flags))
	...
	rootCmd.AddCommand(newCitedAnswerCmd(flags))
