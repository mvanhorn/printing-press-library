...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  valyu-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  valyu-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"datasources", "datasources-categories", "deepresearch", "deepresearch-batches", "workflows"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  valyu-pp-cli workflow status

  # Show status as JSON
  valyu-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("valyu-pp-cli")
			}
