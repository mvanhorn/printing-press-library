...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "valyu-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("VALYU_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("VALYU_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/ai/valyu/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/valyu-pp-cli/feedback.jsonl.
When ` + "`VALYU_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`VALYU_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  valyu-pp-cli feedback list
  valyu-pp-cli feedback list --limit 5
  valyu-pp-cli feedback list --json`,
