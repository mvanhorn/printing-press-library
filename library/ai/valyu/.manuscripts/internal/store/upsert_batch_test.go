...
func TestSearchContentsQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if stored, failed, err := s.UpsertBatch("contents", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	} else if failed != 0 || stored != len(items) {
		t.Fatalf("UpsertBatch stored=%d failed=%d, want stored=%d failed=0", stored, failed, len(items))
	}
	...
	for _, query := range []string{
		...
		results, err := s.SearchContents(query, 10)
		if err != nil {
			t.Fatalf("SearchContents(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchContents(%q) returned no results", query)
		}
	}
}
...
// dispatches paginated items into both the generic resources table AND the
// typed datasources table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesDatasourcesTable(t *testing.T) {
	...
	if _, _, err := s.UpsertBatch("datasources", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "datasources").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "datasources")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count datasources: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("datasources count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
...
func TestSearchDatasourcesQuotesFTSQuerySyntax(t *testing.T) {
	...
	if stored, failed, err := s.UpsertBatch("datasources", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	} else if failed != 0 || stored != len(items) {
		t.Fatalf("UpsertBatch stored=%d failed=%d, want stored=%d failed=0", stored, failed, len(items))
	}
...
// dispatches paginated items into both the generic resources table AND the
// typed deepresearch table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesDeepresearchTable(t *testing.T) {
	...
	if _, _, err := s.UpsertBatch("deepresearch", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "deepresearch").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "deepresearch")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count deepresearch: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("deepresearch count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_PopulatesSemanticSearchTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed semantic_search table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesSemanticSearchTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("semantic_search", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "semantic_search").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "semantic_search")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count semantic_search: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("semantic_search count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
...
func TestUpsertBatch_TypedFailureDoesNotStrandPreviewGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("preview count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "workflows_id")
	}
}
...
func TestUpsertBatch_KeysPreviewByChildAndParent(t *testing.T) {
	...
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "workflows_id" = ?`, "preview")
...
func TestUpsertBatch_TypedFailureDoesNotStrandVersionsGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("versions count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "workflows_id")
	}
}
...
func TestUpsertBatch_KeysVersionsByChildAndParent(t *testing.T) {
	...
	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "workflows_id" = ?`, "versions")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by workflows_id: %v", err)
	}
	if parentMatches != 2 {
