# Valyu Printed CLI Agent Guide

This directory is a generated `valyu-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
valyu-pp-cli doctor --json
valyu-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
valyu-pp-cli which "<capability>" --json
valyu-pp-cli <command> --help
```
...
```bash
valyu-pp-cli <command> --help
valyu-pp-cli <command> --dry-run --agent
```
