...
Resource scoping:
  ...
  valyu-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  valyu-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  valyu-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  valyu-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  valyu-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/valyu-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "semantic_search":
		return db.UpsertSemanticSearch(data)
	case "workflows":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"datasources-categories": "/v1/datasources/categories",
		"deepresearch":           "/v1/deepresearch/list",
		"deepresearch-batches":   "/v1/deepresearch/batches",
