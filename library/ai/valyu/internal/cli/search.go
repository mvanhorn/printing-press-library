...
In local mode: searches locally synced data only.`,
  ...
  valyu-pp-cli search "error timeout"

  # Force local search only
  valyu-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  valyu-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  valyu-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.dataSource != "local" {
				...
				data, _, getErr := c.Post(cmd.Context(), "/v1/search", map[string]any{
					...
					"fast_mode":           false,
					"historical_cache":    false,
					"included_sources":    nil,
					"instructions":        "",
					"is_tool_call":        true,
					"max_num_results":     5,
					"max_price":           nil,
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'valyu-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "contents":
				results, err = db.SearchContents(query, limit)
			case "datasources":
				results, err = db.SearchDatasources(query, limit)
			case "workflows":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchContents(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search contents failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchDatasources(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search datasources failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
