...
**Two halves:** Eleven discovery commands (`model-card`, `find-quants`, `find-feature`, `trending`, `derivatives`, `uploader-rep`, `compare-quants`, `backend-check`, `doctor`, `schema`, `local`) work for everyone with no setup. Five stack-coupled commands (`vs-current`, `bench-history`, `eval-candidates`, `watch`, `watch-poll`) light up when paired with a local config (`data/openclaw.json` shape) and a `model-eval-harness` layout — set `HF_OPENCLAW_ROOT` to point them at any compatible layout, or pass `--config-path` / `--harness` per call.
