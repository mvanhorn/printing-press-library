...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".huggingface-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("HUGGINGFACE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HUGGINGFACE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "huggingface-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.huggingface-pp-cli/feedback.jsonl.
When ` + "`HUGGINGFACE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`HUGGINGFACE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  huggingface-pp-cli feedback list
  huggingface-pp-cli feedback list --limit 5
  huggingface-pp-cli feedback list --json`,
