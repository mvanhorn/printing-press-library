...
// quality-gate: file-size shared CLI helpers; splitting is a generator-template concern, not polish-scope
...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'huggingface-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token. Set it with: huggingface-pp-cli auth set-token <token>"+
			"\n      or: export HF_TOKEN=<your-token>"+
			"\n      Get a key at: https://huggingface.co/settings/tokens"+
			"\n      Run 'huggingface-pp-cli doctor' to check auth status.", err))
