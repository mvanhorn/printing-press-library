...
Exit codes & warnings:
  ...
  huggingface-pp-cli sync

  # Sync specific resources only
  huggingface-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  huggingface-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  huggingface-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  huggingface-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  huggingface-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/huggingface-pp-cli/data.db)")
