...
var readCommandResources = map[string][]string{
	"huggingface-pp-cli models":        {"models"},
	"huggingface-pp-cli models list":   {"models"},
	"huggingface-pp-cli models get":    {"models"},
	"huggingface-pp-cli models search": {"models"},
}
...
func autoRefreshIfStale(ctx context.Context, flags *rootFlags, resources []string) (meta cliutil.FreshnessMeta) {
	...
	dbPath := defaultDBPath("huggingface-pp-cli")
	...
	if err := runAutoRefresh(refreshCtx, flags, db, resources); err != nil {
		fmt.Fprintf(os.Stderr, "warning: using stale huggingface-pp-cli cache (refresh failed: %v)\n", err)
