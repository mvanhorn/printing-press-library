...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Hugging Face",
	}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  huggingface-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export HF_TOKEN=\"your-token-here\"")
				fmt.Fprintln(w, "  export HUGGING_FACE_HUB_TOKEN=\"your-token-here\"")
				fmt.Fprintf(w, "  huggingface-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  huggingface-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  huggingface-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("HF_TOKEN") != "" {
				envStillSet = "HF_TOKEN"
			}
			if envStillSet == "" && os.Getenv("HUGGING_FACE_HUB_TOKEN") != "" {
				envStillSet = "HUGGING_FACE_HUB_TOKEN"
			}
