# Hugging Face Printed CLI Agent Guide

This directory is a generated `huggingface-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
huggingface-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
huggingface-pp-cli which "<capability>" --json
huggingface-pp-cli <command> --help
```
...
```bash
huggingface-pp-cli <command> --agent
```
...
```bash
huggingface-pp-cli <command> --help
huggingface-pp-cli <command> --dry-run --agent
```
