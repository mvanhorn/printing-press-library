...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "perplexity-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PERPLEXITY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PERPLEXITY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "perplexity-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/perplexity-pp-cli/feedback.jsonl.
When ` + "`PERPLEXITY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PERPLEXITY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  perplexity-pp-cli feedback list
  perplexity-pp-cli feedback list --limit 5
  perplexity-pp-cli feedback list --json`,
