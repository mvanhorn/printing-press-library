# Perplexity Printed CLI Agent Guide

This directory is a generated `perplexity-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
perplexity-pp-cli doctor --json
perplexity-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
perplexity-pp-cli which "<capability>" --json
perplexity-pp-cli <command> --help
```
...
```bash
perplexity-pp-cli <command> --agent
```
...
```bash
perplexity-pp-cli <command> --help
perplexity-pp-cli <command> --dry-run --agent
```
