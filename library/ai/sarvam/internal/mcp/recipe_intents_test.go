...
func TestRecipeIntentHandlerBuildsRecipeArgs(t *testing.T) {
	...
	result, err := handleReprocessFailedBatchFiles(context.Background(), req)
	if err != nil {
...
func TestRecipeIntentHandlerOverridesParams(t *testing.T) {
	...
	if strings.Contains(got, "--failed-only") {
		t.Fatalf("boolean false override should suppress --failed-only: %q", got)
	}
}
