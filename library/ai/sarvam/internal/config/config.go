...
type Config struct {
	...
	SarvamApiKey     string          `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	if v := os.Getenv("SARVAM_API_KEY"); v != "" {
		cfg.SarvamApiKey = v
		cfg.markEnvOverride("SarvamApiKey")
		cfg.AuthSource = "env:SARVAM_API_KEY"
		cfg.CredentialSource = "env:SARVAM_API_KEY"
	}
	...
	if v := os.Getenv("SARVAM_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("SARVAM_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "sarvam-pp-cli", "config.toml"), nil
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.SarvamApiKey == "" {
		c.SarvamApiKey = creds.SarvamApiKey
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.SarvamApiKey = token
	delete(c.envOverrides, "SarvamApiKey")
	c.updateFileConfigField("SarvamApiKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["SarvamApiKey"] {
			out.SarvamApiKey = c.fileConfig.SarvamApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "SarvamApiKey":
		c.fileConfig.SarvamApiKey = c.SarvamApiKey
	}
}
