...
Run sync first to populate the local search index.`,
  ...
  sarvam-pp-cli search "status"

  # Force local search explicitly
  sarvam-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  sarvam-pp-cli search "status" --type chat --data-source local
  # JSON output for piping
  sarvam-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'sarvam-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
