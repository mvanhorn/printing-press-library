...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SARVAM_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SARVAM_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "sarvam-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  sarvam-pp-cli feedback "the translate command surprised me"
...
When ` + "`SARVAM_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SARVAM_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  sarvam-pp-cli feedback list
  sarvam-pp-cli feedback list --limit 5
  sarvam-pp-cli feedback list --json`,
