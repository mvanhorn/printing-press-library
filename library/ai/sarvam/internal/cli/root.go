...
type rootFlags struct {
	...
	// invocation. Mirrors the SARVAM_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "sarvam-pp-cli",
		Short: `Sarvam CLI — Every Sarvam AI model on your terminal — translate, speak, transcribe, chat, and extract documents in 22 Indian…`,
		Long: `Sarvam CLI — Every Sarvam AI model on your terminal — translate, speak, transcribe, chat, and extract documents in 22 Indian languages, with a local history that compounds
...
Health check: run 'sarvam-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("sarvam-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select created,id,object)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'sarvam-pp-cli profile list')")
	...
	rootCmd.AddCommand(newDocAiCmd(flags))
	rootCmd.AddCommand(newSpeechToTextCmd(flags))
	rootCmd.AddCommand(newTextToSpeechCmd(flags))
	...
	rootCmd.AddCommand(newTextLidPromotedCmd(flags))
	rootCmd.AddCommand(newTranslatePromotedCmd(flags))
	rootCmd.AddCommand(newTransliteratePromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelDocaiCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelPronCheckCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSttJobCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSubsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelVoicesCmd(flags))
