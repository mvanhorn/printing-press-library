...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  sarvam-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  sarvam-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"models", "text-to-speech"}
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  sarvam-pp-cli workflow status

  # Show status as JSON
  sarvam-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("sarvam-pp-cli")
			}
