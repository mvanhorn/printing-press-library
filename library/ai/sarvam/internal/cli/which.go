...
var whichIndex = []whichEntry{
	{Command: "voices preview", Description: "Generate one sample sentence across every TTS speaker and hear them all side by side", Group: "Speech workflow", WhyItMatters: "Use when choosing a TTS voice for a new language or prompt set without burning manual API calls"},
	{Command: "chat resume", Description: "Continue a past chat thread from local history with full context", Group: "Local state that compounds", WhyItMatters: "Use to continue an assistant session without losing context, offline from the original thread"},
	{Command: "stt-job retry", Description: "Re-run only the failed files of a batch STT job with one command", Group: "Job orchestration", WhyItMatters: "Use when a batch job partially fails and you need to reprocess just the failures"},
	{Command: "pron-check", Description: "Verify a term's TTS pronunciation via a speech round-trip (TTS then STT)", Group: "Speech workflow", WhyItMatters: "Use to confirm a pronunciation dictionary edit took effect before shipping IVR prompts"},
	{Command: "subs", Description: "Emit .srt/.vtt subtitles from timestamped transcriptions in local history", Group: "Local state that compounds", WhyItMatters: "Use to turn a timestamped transcription into subtitles without a throwaway script"},
	{Command: "docai schema list", Description: "Save, list, and diff doc-ai extraction schemas locally", Group: "Document intelligence", WhyItMatters: "Use to version extraction schemas so schema changes never silently break extraction runs"},
	{Command: "docai batch", Description: "Run a saved extraction schema over a folder of documents with job pacing", Group: "Document intelligence", WhyItMatters: "Use for weekly batch document extraction (KYC, invoices) without writing orchestration code"},
	{Command: "stt-job report", Description: "Per-file digest of a batch STT job with typed exit codes for cron alerting", Group: "Job orchestration", WhyItMatters: "Use in cron to alert when a batch transcription job degrades"},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  sarvam-pp-cli which "stale tickets"
  sarvam-pp-cli which "bottleneck"
  sarvam-pp-cli which --limit 1 "send message"
  sarvam-pp-cli which                                # list the full capability index`,
