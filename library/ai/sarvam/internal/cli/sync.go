...
Resource scoping:
  ...
  sarvam-pp-cli sync --resources models,text-to-speech

  # Full resync (ignore previous checkpoint)
  sarvam-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  sarvam-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  sarvam-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  sarvam-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "doc-ai":
		return db.UpsertDocAi(data)
	case "speech-to-text":
		return db.UpsertSpeechToText(data)
	case "text-lid":
		return db.UpsertTextLid(data)
	case "text-to-speech":
		return db.UpsertTextToSpeech(data)
	case "translate":
		return db.UpsertTranslate(data)
	case "transliterate":
		return db.UpsertTransliterate(data)
	default:
...
func responsePathForResource(resource, path string) []string {
	...
	case "chat\x00/v1/chat/completions":
		return []string{"choices"}
	case "models\x00/v2/models":
		return []string{"data"}
	case "speech-to-text\x00/speech-to-text/job/v1/{job_id}/start":
		return []string{"job_details"}
	case "speech-to-text\x00/speech-to-text/job/v1/{job_id}/status":
		return []string{"job_details"}
	case "text-to-speech\x00/text-to-speech/pronunciation-dictionary":
		return []string{"dictionaries"}
	case "text-to-speech\x00/text-to-speech":
		return []string{"audios"}
	}
	return nil
}
