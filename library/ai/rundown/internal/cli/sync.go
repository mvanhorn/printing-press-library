...
Resource scoping:
  ...
  rundown-pp-cli sync --resources posts,leaderboard

  # Full resync (ignore previous checkpoint)
  rundown-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  rundown-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  rundown-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  rundown-pp-cli sync --latest-only`,
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "comments", ParentTable: "posts", ParentIDParam: "post_id", PathTemplate: "/posts/{post_id}/comments", KeyField: "", ReconcileMode: "none", GenericScopeJSONPath: "", PathParams: []dependentPathParamDef{
			{Param: "post_id", Field: "id"},
		}},
	}
}
...
func responsePathForResource(resource, path string) []string {
	...
	case "comments\x00/posts/{post_id}/comments":
		return []string{"comments"}
	case "leaderboard\x00/leaderboard":
		return []string{"entries"}
	case "posts\x00/posts?sort=new":
		return []string{"posts"}
	case "tools\x00/tools":
		return []string{"tools"}
	}
	return nil
}
