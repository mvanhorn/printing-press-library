...
type rootFlags struct {
	...
	// invocation. Mirrors the RUNDOWN_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "rundown-pp-cli",
		Short: `Rundown CLI — Search the community's AI workflows offline, and rank them by week - two things the site's own API cannot do.`,
		Long: `Rundown CLI — Search the community's AI workflows offline, and rank them by week - two things the site's own API cannot do.
...
Health check: run 'rundown-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("rundown-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select id,postId,parentCommentId)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'rundown-pp-cli profile list')")
	...
	rootCmd.AddCommand(newLeaderboardPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelShowCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelStackCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelTopCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelUseCasesCmd(flags))
