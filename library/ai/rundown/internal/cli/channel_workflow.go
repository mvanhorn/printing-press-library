...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  rundown-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  rundown-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  rundown-pp-cli workflow archive --timeout 0`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  rundown-pp-cli workflow status

  # Show status as JSON
  rundown-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("rundown-pp-cli")
			}
