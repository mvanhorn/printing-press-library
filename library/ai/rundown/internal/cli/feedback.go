...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("RUNDOWN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("RUNDOWN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "rundown-pp-cli/feedback")
...
When ` + "`RUNDOWN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`RUNDOWN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  rundown-pp-cli feedback list
  rundown-pp-cli feedback list --limit 5
  rundown-pp-cli feedback list --json`,
