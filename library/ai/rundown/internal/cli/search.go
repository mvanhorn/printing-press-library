...
Run sync first to populate the local search index.`,
  ...
  rundown-pp-cli search "status"

  # Force local search explicitly
  rundown-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  rundown-pp-cli search "status" --type comments --data-source local
  # JSON output for piping
  rundown-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'rundown-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
