...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("read_one_workflow_end_to_end",
			mcplib.WithDescription("Reassembles the post and its comments into a single readable document, including the numbered steps."),
			mcplib.WithString("slug", mcplib.Required(), mcplib.Description("Override the recipe's positional slug value.")),
		),
		handleReadOneWorkflowEndToEnd,
	)
}

// handleReadOneWorkflowEndToEnd runs the read_one_workflow_end_to_end recipe intent: Reassembles the post and its comments into a single readable document, including the numbered steps.
func handleReadOneWorkflowEndToEnd(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
