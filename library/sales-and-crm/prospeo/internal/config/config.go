...
type Config struct {
	...
	Path          string            `toml:"-"`
	ProspeoApiKey string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("PROSPEO_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "prospeo-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("PROSPEO_API_KEY"); v != "" {
		cfg.ProspeoApiKey = v
		cfg.AuthSource = "env:PROSPEO_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.ProspeoApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("PROSPEO_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
