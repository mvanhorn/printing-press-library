...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  prospeo-pp-cli doctor
  prospeo-pp-cli doctor --json
  prospeo-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export PROSPEO_API_KEY=<your-key>"
					report["auth_key_url"] = "https://app.prospeo.io/api-keys"
				} else {
					authConfigured = true
			...
			if os.Getenv("PROSPEO_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "PROSPEO_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "PROSPEO_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					healthPath := "/account-information"
					...
					if authHeader == "" {
						...
						authHeaders["x-key"] = authHeader
						authHeaders["User-Agent"] = "prospeo-pp-cli"
						verifyPath := "/account-information"
			...
			if !supa.IsConfigured() {
				report["supabase"] = "not configured (set SUPABASE_URL + SUPABASE_SERVICE_KEY for the cache features)"
			} else {
				supaCfg, cerr := supa.LoadConfig()
				if cerr != nil {
					report["supabase"] = fmt.Sprintf("config error: %s", cerr)
				} else {
					sc := supa.New(supaCfg)
					if perr := sc.Ping(cmd.Context()); perr == nil {
						report["supabase"] = "reachable (outreach schema)"
					} else {
						msg := perr.Error()
						if strings.Contains(msg, "PGRST106") {
							report["supabase"] = "reachable but `outreach` schema not exposed - set PGRST_DB_SCHEMAS=public,outreach,storage,graphql_public in your PostgREST env and restart kong"
						} else {
							report["supabase"] = fmt.Sprintf("error: %s", msg)
						}
					}
				}
			}
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("prospeo-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'prospeo-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'prospeo-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'prospeo-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'prospeo-pp-cli sync' to refresh."
	}
	return report
}
