...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".prospeo-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PROSPEO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PROSPEO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "prospeo-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.prospeo-pp-cli/feedback.jsonl.
When ` + "`PROSPEO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PROSPEO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  prospeo-pp-cli feedback list
  prospeo-pp-cli feedback list --limit 5
  prospeo-pp-cli feedback list --json`,
