...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "prospeo-pp-cli", "http")
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	// Prospeo wraps /enrich-person and /enrich-company body fields inside a
	// top-level {"data": {...}} envelope, while option flags
	// (only_verified_email, enrich_mobile, only_verified_mobile) stay at the
	// top level. /bulk-enrich-* and /search-* endpoints carry their own
	// hand-authored body shapes and pass through unchanged.
	if body != nil && (path == "/enrich-person" || path == "/enrich-company") {
		body = wrapProspeoEnrichBody(body)
	}
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if authHeader != "" {
			req.Header.Set("x-key", authHeader)
		}
		if c.Config != nil {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "prospeo-pp-cli/0.1.0")
		}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export PROSPEO_API_KEY=<your-token> or prospeo-pp-cli auth set-token <token>")
}
