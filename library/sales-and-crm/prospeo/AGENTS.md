# Prospeo Printed CLI Agent Guide

This directory is a generated `prospeo-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
prospeo-pp-cli doctor --json
prospeo-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
prospeo-pp-cli which "<capability>" --json
prospeo-pp-cli <command> --help
```
...
```bash
prospeo-pp-cli <command> --help
prospeo-pp-cli <command> --dry-run --agent
```
