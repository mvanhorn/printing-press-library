# Y Combinator Printed CLI Agent Guide

This directory is a generated `yc-companies-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
yc-companies-pp-cli doctor --json
yc-companies-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
yc-companies-pp-cli which "<capability>" --json
yc-companies-pp-cli <command> --help
```
...
```bash
yc-companies-pp-cli <command> --agent
```
...
```bash
yc-companies-pp-cli <command> --help
yc-companies-pp-cli <command> --dry-run --agent
```
