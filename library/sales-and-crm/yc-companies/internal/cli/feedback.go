...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".yc-companies-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("YC_COMPANIES_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("YC_COMPANIES_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "yc-companies-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.yc-companies-pp-cli/feedback.jsonl.
When ` + "`YC_COMPANIES_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`YC_COMPANIES_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  yc-companies-pp-cli feedback list
  yc-companies-pp-cli feedback list --limit 5
  yc-companies-pp-cli feedback list --json`,
