...
In local mode: searches locally synced data only.`,
  ...
  yc-companies-pp-cli search "error timeout"

  # Force local search only
  yc-companies-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  yc-companies-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  yc-companies-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'yc-companies-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "batches":
				results, err = db.SearchBatches(query, limit)
			case "companies":
				results, err = db.SearchCompanies(query, limit)
			case "industries":
				results, err = db.SearchIndustries(query, limit)
			case "tags":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchBatches(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search batches failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchIndustries(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search industries failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
