...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "yc-companies-pp-cli",
		Short: `Every YC-backed company in a local database, with watch lists, deltas, and cross-batch stats no scraper has.`,
		Long: `Every YC-backed company in a local database, with watch lists, deltas, and cross-batch stats no scraper has.
...
Health check: run 'yc-companies-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("yc-companies-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'yc-companies-pp-cli profile list')")
	...
	// Attach novel-feature subcommands to the existing `companies` parent.
	companiesCmd := newCompaniesCmd(flags)
	companiesCmd.Hidden = false
	companiesCmd.AddCommand(newCompaniesNewCmd(flags))
	companiesCmd.AddCommand(newCompaniesChangesCmd(flags))
	companiesCmd.AddCommand(newCompaniesSimilarCmd(flags))
	...
	// Attach `batches show <slug>` subcommand to the promoted batches parent.
	batchesCmd := newBatchesPromotedCmd(flags)
	batchesCmd.AddCommand(newBatchesShowCmd(flags))
	rootCmd.AddCommand(batchesCmd)
	rootCmd.AddCommand(newIndustriesPromotedCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("yc-companies-pp-cli %s\n", version)
		},
	}
}
