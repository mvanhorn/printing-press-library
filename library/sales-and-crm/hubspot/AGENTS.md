# HubSpot Printed CLI Agent Guide

This directory is a generated `hubspot-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
hubspot-pp-cli doctor --json
hubspot-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
hubspot-pp-cli which "<capability>" --json
hubspot-pp-cli <command> --help
```
...
```bash
hubspot-pp-cli <command> --help
hubspot-pp-cli <command> --dry-run --agent
```
