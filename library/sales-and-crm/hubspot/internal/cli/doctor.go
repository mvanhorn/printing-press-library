...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  hubspot-pp-cli doctor
  hubspot-pp-cli doctor --json
  hubspot-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "hubspot-pp-cli login"
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "HUBSPOT_PRIVATE_APP_TOKEN")
				if os.Getenv("HUBSPOT_PRIVATE_APP_TOKEN") != "" {
					authEnvSet = append(authEnvSet, "HUBSPOT_PRIVATE_APP_TOKEN")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("HUBSPOT_PRIVATE_APP_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "HUBSPOT_PRIVATE_APP_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "HUBSPOT_PRIVATE_APP_TOKEN optional")
			}
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "HUBSPOT_OAUTH2")
				if os.Getenv("HUBSPOT_OAUTH2") != "" {
					authEnvSet = append(authEnvSet, "HUBSPOT_OAUTH2")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("HUBSPOT_OAUTH2") != "" {
				authEnvSet = append(authEnvSet, "HUBSPOT_OAUTH2")
			} else {
				authEnvInfo = append(authEnvInfo, "HUBSPOT_OAUTH2 optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "hubspot-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'hubspot-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'hubspot-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'hubspot-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'hubspot-pp-cli sync' to refresh."
	}
	return report
}
