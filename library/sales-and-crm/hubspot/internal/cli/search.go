...
In local mode: searches locally synced data only.`,
  ...
  hubspot-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /crm/v3/lists/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/crm/v3/lists/search", map[string]any{
					"query":                query,
			...
			case "hubspot-properties-crm":
				results, err = db.SearchHubspotPropertiesCrm(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchHubspotPropertiesCrm(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search hubspot_properties_crm failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
