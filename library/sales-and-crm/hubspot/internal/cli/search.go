  ...
  hubspot-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.dataSource != "local" {
				...
				data, _, getErr := c.Post("/crm/v3/lists/search", map[string]any{
					"query":                query,
			...
			case "properties-crm":
				results, err = db.SearchPropertiesCrm(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchPropertiesCrm(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search properties_crm failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
