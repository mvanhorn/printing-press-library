...
Resource scoping:
  ...
  hubspot-pp-cli sync --latest-only`,
			...
			for i := 0; i < concurrency; i++ {
				...
				go func() {
					...
					for resource := range work {
						res := syncResource(cmd.Context(), c, db, resource, sinceTS, full, maxPages, effectiveLatestOnly, userParams, syncEventWriter, withHistory)
						results <- res
					}
				}()
			}
	...
	cmd.Flags().StringSliceVar(&withHistory, "with-history", nil, "Comma-separated property names to capture into hubspot_property_history table (e.g. hs_meeting_outcome). Only applies to meetings/deals/contacts/companies resources.")
...
func syncResource(ctx context.Context, c interface {
	...
	RateLimit() float64
}, db *store.Store, resource, sinceTS string, full bool, maxPages int, latestOnly bool, userParams *syncUserParams, syncEvents io.Writer, withHistory []string) syncResult {
	started := time.Now()
	if syncEvents == nil {
	...
	for {
		...
		if resourceSupportsPagination(resource) {
			...
			// HubSpot caps reads at 50 per page when propertiesWithHistory is
			// requested (HTTP 400 VALIDATION_ERROR otherwise). Cap silently so
			// the user doesn't have to know the API constraint.
			if len(withHistory) > 0 && supportsPropertyHistory(resource) && effectiveLimit > 50 {
				effectiveLimit = 50
			}
			params[pageSize.limitParam] = strconv.Itoa(effectiveLimit)
			if cursor != "" {
		...
		// --with-history: ask HubSpot to return propertiesWithHistory for the
		// named properties (meetings/deals/contacts/companies only). Reading
		// is additive; the response still includes the regular properties block
		// so existing upsert code keeps working.
		//
		// Also request the same properties as `properties=` so the regular
		// snapshot has the current value alongside the history (avoids needing
		// the user to pass --param properties=... separately).
		if len(withHistory) > 0 && supportsPropertyHistory(resource) {
			params["propertiesWithHistory"] = strings.Join(withHistory, ",")
			if _, alreadySet := params["properties"]; !alreadySet {
				params["properties"] = strings.Join(withHistory, ",")
			}
		}
		...
		// --with-history: parse the propertiesWithHistory blob HubSpot returned
		// on each item and persist into hubspot_property_history. Failures here
		// are non-fatal so a malformed history payload on one resource does not
		// stop the rest of the sync; per-item errors are written as
		// sync_warning events so callers can see them.
		if len(withHistory) > 0 && supportsPropertyHistory(resource) {
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "hubspot-calls-crm":
		return db.UpsertHubspotCallsCrm(data)
	case "hubspot-companies-crm":
		return db.UpsertHubspotCompaniesCrm(data)
	case "hubspot-contacts-crm":
		return db.UpsertHubspotContactsCrm(data)
	case "hubspot-deals-crm":
		return db.UpsertHubspotDealsCrm(data)
	case "hubspot-emails-crm":
		return db.UpsertHubspotEmailsCrm(data)
	case "hubspot-imports-crm":
		return db.UpsertHubspotImportsCrm(data)
	case "hubspot-leads-crm":
		return db.UpsertHubspotLeadsCrm(data)
	case "hubspot-line-items-crm":
		return db.UpsertHubspotLineItemsCrm(data)
	case "hubspot-lists-crm":
		return db.UpsertHubspotListsCrm(data)
	case "hubspot-meetings-crm":
		return db.UpsertHubspotMeetingsCrm(data)
	case "hubspot-notes-crm":
		return db.UpsertHubspotNotesCrm(data)
	case "hubspot-objects-crm":
		return db.UpsertHubspotObjectsCrm(data)
	case "hubspot-owners-crm":
		return db.UpsertHubspotOwnersCrm(data)
	case "hubspot-pipelines-crm":
		return db.UpsertHubspotPipelinesCrm(data)
	case "hubspot-products-crm":
		return db.UpsertHubspotProductsCrm(data)
	case "hubspot-properties-batch":
		return db.UpsertHubspotPropertiesBatch(data)
	case "hubspot-properties-crm":
		return db.UpsertHubspotPropertiesCrm(data)
	case "hubspot-quotes-crm":
		return db.UpsertHubspotQuotesCrm(data)
	case "hubspot-tasks-crm":
		return db.UpsertHubspotTasksCrm(data)
	case "hubspot-tickets-crm":
		return db.UpsertHubspotTicketsCrm(data)
	default:
...
func defaultSyncResources() []string {
	return []string{
		...
		"hubspot-lists-crm",
		"hubspot-lists-crm-folders",
		"hubspot-lists-crm-idmapping",
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"hubspot-calls-crm":           "/crm/v3/objects/calls",
		"hubspot-companies-crm":       "/crm/v3/objects/companies",
		"hubspot-contacts-crm":        "/crm/v3/objects/contacts",
		"hubspot-deals-crm":           "/crm/v3/objects/0-3",
		"hubspot-emails-crm":          "/crm/v3/objects/emails",
		"hubspot-imports-crm":         "/crm/v3/imports",
		"hubspot-leads-crm":           "/crm/v3/objects/leads",
		"hubspot-line-items-crm":      "/crm/v3/objects/line_items",
		"hubspot-lists-crm":           "/crm/v3/lists",
		"hubspot-lists-crm-folders":   "/crm/v3/lists/folders",
		"hubspot-lists-crm-idmapping": "/crm/v3/lists/idmapping",
		"hubspot-meetings-crm":        "/crm/v3/objects/meetings",
		"hubspot-notes-crm":           "/crm/v3/objects/notes",
		"hubspot-owners-crm":          "/crm/v3/owners",
		"hubspot-products-crm":        "/crm/v3/objects/products",
		"hubspot-quotes-crm":          "/crm/v3/objects/quotes",
		"hubspot-tasks-crm":           "/crm/v3/objects/tasks",
		"hubspot-tickets-crm":         "/crm/v3/objects/tickets",
	}
