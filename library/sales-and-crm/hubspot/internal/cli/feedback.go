...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "hubspot-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("HUBSPOT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HUBSPOT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "hubspot-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/hubspot-pp-cli/feedback.jsonl.
When ` + "`HUBSPOT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`HUBSPOT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  hubspot-pp-cli feedback list
  hubspot-pp-cli feedback list --limit 5
  hubspot-pp-cli feedback list --json`,
