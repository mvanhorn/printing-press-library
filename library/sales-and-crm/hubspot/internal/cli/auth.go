...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  hubspot-pp-cli auth setup\n  hubspot-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Register an app and copy your credentials at: https://app.hubspot.com/private-apps")
			fmt.Fprintln(w, "  Create a private app, grant CRM scopes, and copy the access token.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then run:")
			fmt.Fprintln(w, "  hubspot-pp-cli login")
			if !launch {
				return nil
			}
			launchURL := "https://app.hubspot.com/private-apps"
...
func newAuthLoginCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.Flags().StringVar(&clientID, "client-id", os.Getenv("HUBSPOT_CLIENT_ID"), "OAuth2 client ID")
	cmd.Flags().StringVar(&clientSecret, "client-secret", os.Getenv("HUBSPOT_CLIENT_SECRET"), "OAuth2 client secret")
...
func resolveOAuthCredentials(cmd *cobra.Command, flags *rootFlags, cfg *config.Config, clientID, clientSecret string) (string, string, error) {
	...
	if flags.noInput {
		return "", "", fmt.Errorf("OAuth2 client ID is required; pass --client-id, set HUBSPOT_CLIENT_ID, or run 'hubspot-pp-cli login' without --no-input")
	}
...
func printOAuthCredentialHint(w io.Writer) {
	fmt.Fprintln(w, "Create OAuth credentials at: https://app.hubspot.com/private-apps")
}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				fmt.Fprintf(w, "  %s Not authenticated. Run 'hubspot-pp-cli login' to authenticate.\n", red("FAIL"))
				return nil
			}
			...
			if cfg.TokenExpiry.IsZero() {
				...
				if cfg.RefreshToken != "" {
					...
					fmt.Fprintf(w, "  %s Token expired. Run 'hubspot-pp-cli login' to re-authenticate.\n", red("FAIL"))
				}
			}
