...
type Config struct {
	...
	Path                   string `toml:"-"`
	HubspotPrivateAppToken string `toml:"private_app_token"`
	HubspotOauth2          string `toml:"oauth2"`
}
...
func Load(configPath string) (*Config, error) {
	...
	if v := os.Getenv("HUBSPOT_OAUTH2"); v != "" {
		cfg.HubspotOauth2 = v
		cfg.AuthSource = "env:HUBSPOT_OAUTH2"
	}
	...
	if cfg.AuthSource == "" && cfg.HubspotPrivateAppToken != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.HubspotOauth2 != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("HUBSPOT_AUTHORIZATION_URL"); v != "" {
		cfg.AuthorizationURL = v
	}
	if v := os.Getenv("HUBSPOT_TOKEN_URL"); v != "" {
		cfg.TokenURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.HubspotPrivateAppToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:HUBSPOT_PRIVATE_APP_TOKEN"
		}
		return "Bearer " + c.HubspotPrivateAppToken
	}
	if c.HubspotOauth2 != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:HUBSPOT_OAUTH2"
		}
		return "Bearer " + c.HubspotOauth2
	}
	if c.AccessToken != "" {
