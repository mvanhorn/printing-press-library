...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("G2_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "g2-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("G2_API_KEY"); v != "" {
		cfg.G2ApiKey = v
		cfg.AuthSource = "env:G2_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.G2ApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("G2_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("G2_DEFAULT_HOST")); v != "" {
		cfg.TemplateVars["defaultHost"] = normalizeEndpointTemplateValue(v)
	} else {
		cfg.TemplateVars["defaultHost"] = "data.g2.com"
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.G2ApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:G2_API_KEY"
		}
		return "Bearer " + c.G2ApiKey
	}
	if c.AccessToken != "" {
