...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "g2-pp-cli",
		Short: `G2 CLI — Every G2 Marketplace surface, plus a credit-aware local mirror, full-text review search, and buyer-intent triage no oth…`,
		Long: `G2 CLI — Every G2 Marketplace surface, plus a credit-aware local mirror, full-text review search, and buyer-intent triage no oth…
...
Health check: run 'g2-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("g2-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'g2-pp-cli profile list')")
	...
	rootCmd.AddCommand(newProductMappingsCmd(flags))
	rootCmd.AddCommand(newProductsCmd(flags))
	rootCmd.AddCommand(newQuestionsCmd(flags))
	rootCmd.AddCommand(newScreenshotsCmd(flags))
	...
	biCmd := newBuyerIntentPromotedCmd(flags)
	biCmd.AddCommand(newBuyerIntentListCmd(flags))
	rootCmd.AddCommand(biCmd)
	rootCmd.AddCommand(newMarketSignalsPromotedCmd(flags))
	rootCmd.AddCommand(newProductRatingsPromotedCmd(flags))
	rootCmd.AddCommand(newReportsPromotedCmd(flags))
	rootCmd.AddCommand(newSandboxPromotedCmd(flags))
	rootCmd.AddCommand(newUserPromotedCmd(flags))
	rootCmd.AddCommand(newWatchCmd(flags))
	rootCmd.AddCommand(newCreditsCmd(flags))
	rootCmd.AddCommand(newAltTrackCmd(flags))
