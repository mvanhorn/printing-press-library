...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "g2-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("G2_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("G2_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "g2-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/g2-pp-cli/feedback.jsonl.
When ` + "`G2_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`G2_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  g2-pp-cli feedback list
  g2-pp-cli feedback list --limit 5
  g2-pp-cli feedback list --json`,
