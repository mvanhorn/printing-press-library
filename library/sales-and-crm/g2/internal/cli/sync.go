...
Resource scoping:
  ...
  g2-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  g2-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  g2-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  g2-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  g2-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/g2-pp-cli/data.db)")
...
func syncResourceFieldSelector(resource string) (string, string) {
	switch resource {
	case "categories":
		return "fields[categories]", "id,attributes.uuid,type"
	case "categories-features":
		return "include", "data,links"
	case "product-mappings":
		return "fields[product_mappings]", "id,type"
	case "products":
		return "fields[products]", "id,type"
	case "products-features":
		return "include", "data,links"
	case "questions":
		return "fields[questions]", "id,type"
	case "screenshots":
		return "fields[screenshots]", "id,attributes.id,type"
	case "users":
		return "fields[products]", "id,type"
	case "users-me-research-boards":
		return "fields[research_boards]", "id,type"
	case "vendors":
		return "fields[vendors]", "id,type"
	}
	return "", ""
}
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "buyer-intent":
		return db.UpsertBuyerIntent(data)
	case "categories":
		return db.UpsertCategories(data)
	case "categories_features":
		return db.UpsertCategoriesFeatures(data)
	case "credit_account":
		return db.UpsertCreditAccount(data)
	case "credit_deductions":
		return db.UpsertCreditDeductions(data)
	case "data_subscriptions":
		return db.UpsertDataSubscriptions(data)
	case "product-mappings":
		return db.UpsertProductMappings(data)
	case "products":
		return db.UpsertProducts(data)
	case "products_buyer_intent":
		return db.UpsertProductsBuyerIntent(data)
	case "competitors":
		return db.UpsertCompetitors(data)
	case "discussions":
		return db.UpsertDiscussions(data)
	case "downloads":
		return db.UpsertDownloads(data)
	case "products_features":
		return db.UpsertProductsFeatures(data)
	case "integration_reviews":
		return db.UpsertIntegrationReviews(data)
	case "reviews":
		return db.UpsertReviews(data)
	case "products_screenshots":
		return db.UpsertProductsScreenshots(data)
	case "snippets":
		return db.UpsertSnippets(data)
	case "video_reviews":
		return db.UpsertVideoReviews(data)
	case "videos":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"buyer-intent":             "/api/v2/buyer_intent",
		"categories":               "/api/v2/categories",
		"categories-features":      "/api/v2/categories/features",
		"product-mappings":         "/api/v2/product_mappings",
		"products":                 "/api/v2/products",
		"products-features":        "/api/v2/products/features",
		"questions":                "/api/v2/questions",
		"screenshots":              "/api/v2/screenshots",
		"users":                    "/api/v2/users/me/products",
		"users-me-research-boards": "/api/v2/users/me/research_boards",
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "products_buyer_intent", ParentTable: "products", ParentIDParam: "subject_product_id", PathTemplate: "/api/v2/products/{subject_product_id}/buyer_intent", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "subject_product_id", Field: "id"},
		}},
	}
}
