...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  g2-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  g2-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"buyer-intent", "categories", "categories-features", "product-mappings", "products", "products-features", "questions", "screenshots", "users", "users-me-research-boards", "vendors"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  g2-pp-cli workflow status

  # Show status as JSON
  g2-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("g2-pp-cli")
			}
