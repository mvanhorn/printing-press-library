...
In local mode: searches locally synced data only.`,
  ...
  g2-pp-cli search "error timeout"

  # Force local search only
  g2-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  g2-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  g2-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /api/v2/users/me/research_boards
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/api/v2/users/me/research_boards", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'g2-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "questions":
				results, err = db.SearchQuestions(query, limit)
			case "screenshots":
				results, err = db.SearchScreenshots(query, limit)
			case "users":
				results, err = db.SearchUsers(query, limit)
			case "vendors":
				results, err = db.SearchVendors(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchQuestions(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search questions failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchScreenshots(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search screenshots failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchVendors(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search vendors failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
