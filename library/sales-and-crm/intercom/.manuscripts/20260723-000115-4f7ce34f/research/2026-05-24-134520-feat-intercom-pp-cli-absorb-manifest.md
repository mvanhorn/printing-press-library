...
---
## Reprint Reconciliation (2026-07-23, press 4.29.0)
Novel-features subagent verdict: all 4 prior transcendence features **KEEP**.
- `conversations incident-tag` (9/10, hand-code) — region-aware guarded bulk-tag w/ default-dry-run diff; runs live search (not mirror).
- `articles pull`/`push` (9/10, hand-code) — git round-trip; manifest-diff push per locale.
- `contact 360` (8/10, hand-code) — mirror SQL join across companies/conversations/tickets/notes/tags.
- `conversations sla` (8/10, hand-code) — **reframed**: source first-response/resolution from each conversation's synced `statistics.*` object, dropping the expensive `conversation_parts` join/sync dependency. GROUP BY team aggregation still needs the mirror.
Added: none (reprint discipline). Killed: `conversations stale` (now a single search call — `waiting_since`/`statistics.last_contact_reply_at` are queryable predicates), contact dupes, filter explain, article translation gap, rate-budget meter.
