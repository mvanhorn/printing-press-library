...
// ignition-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/sales-and-crm/ignition/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/sales-and-crm/ignition/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/sales-and-crm/ignition/internal/learn/patterns"
...
const noLearnEnvVar = "IGNITION_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("ignition-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  ignition-pp-cli teach --query "<question>" --resource-type <type> --resource <id>
  # multiple resources: repeat --resource; background with a trailing & (fire-and-forget)`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// An explicit --json is a request for output: it overrides the
			// quiet-by-default background contract. Only an explicit
			// --quiet=true still suppresses the ack.
			if flags.asJSON && !(cmd.Flags().Changed("quiet") && quiet) {
...
when learnings exist.`,
		Example: `  ignition-pp-cli recall "<question>" --agent
  ignition-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ignition-pp-cli learnings list --agent
  ignition-pp-cli learnings list --query "<substring>"
  ignition-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  ignition-pp-cli learnings forget "<question>" --resource <id>
  ignition-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  ignition-pp-cli teach-pattern --query-template "items in {entity}" --resource-template "GROUP-{entity:category}" --resource-type items --entity-kind category --strategy substitute`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  ignition-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
