...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("IGNITION_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("IGNITION_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ignition-pp-cli/feedback")
...
When ` + "`IGNITION_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`IGNITION_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ignition-pp-cli feedback list
  ignition-pp-cli feedback list --limit 5
  ignition-pp-cli feedback list --json`,
