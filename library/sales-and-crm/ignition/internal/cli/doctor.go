...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ignition-pp-cli doctor
  ignition-pp-cli doctor --json
  ignition-pp-cli doctor --fail-on warn
  ignition-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			report := map[string]any{}
			transport := "cdp"
			if os.Getenv("IGNITION_TRANSPORT") == "http" {
				transport = "http"
			}
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "ignition-pp-cli auth login --chrome"
				} else {
					authConfigured = true
					report["auth"] = "configured (browser session)"
					report["auth_source"] = cfg.AuthSource
					report["auth_domain"] = ".go.ignitionapp.com"
				}
			}
			...
			if os.Getenv("IGNITION_SESSION_COOKIE") != "" {
				authEnvSet = append(authEnvSet, "IGNITION_SESSION_COOKIE")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "IGNITION_SESSION_COOKIE")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "ignition-pp-cli", suggestion)
						} else {
			...
			for _, key := range []string{"config_path", "base_url", "transport", "auth_source", "credentials_location", "version"} {
