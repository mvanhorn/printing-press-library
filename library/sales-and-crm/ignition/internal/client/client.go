...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/sales-and-crm/ignition/internal/cdp"
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	if os.Getenv("IGNITION_TRANSPORT") != "http" && method == http.MethodPost && strings.Contains(path, "graphql") {
		raw, status, err := cdp.PostGraphQL(ctx, path, bodyBytes)
		if err != nil {
			return nil, 0, fmt.Errorf("cdp transport: %w", err)
		}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export IGNITION_SESSION_COOKIE=<your-token> or ignition-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.IgnitionSessionCookie)
	}
