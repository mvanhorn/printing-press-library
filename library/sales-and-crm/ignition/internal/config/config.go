...
type Config struct {
	...
	IgnitionSessionCookie string          `toml:"session_cookie"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://go.ignitionapp.com",
	}
	...
	// Env var overrides
	if v := os.Getenv("IGNITION_SESSION_COOKIE"); v != "" {
		cfg.IgnitionSessionCookie = v
		cfg.markEnvOverride("IgnitionSessionCookie")
		cfg.AuthSource = "env:IGNITION_SESSION_COOKIE"
		cfg.CredentialSource = "env:IGNITION_SESSION_COOKIE"
	}
	...
	if v := os.Getenv("IGNITION_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}

	// Ignition requires a per-page X-CSRF-Token header alongside the session
	// cookie (see meta[name=csrf-token] on any authed Ignition page). The token
	// is minted per page-load, so it is sourced from IGNITION_CSRF_TOKEN (or the
	// [headers] "X-CSRF-Token" config value) and applied to every request via
	// Config.Headers. The env var wins over the config value when both are set.
	// See .printing-press-patches/0002-csrf-header.md.
	if v := os.Getenv("IGNITION_CSRF_TOKEN"); v != "" {
		if cfg.Headers == nil {
			cfg.Headers = map[string]string{}
		}
		cfg.Headers["X-CSRF-Token"] = v
	}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("IGNITION_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "ignition-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.IgnitionSessionCookie != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:IGNITION_SESSION_COOKIE"
		}
		return ensureAuthScheme("Bearer", c.IgnitionSessionCookie)
	}
	if c.AccessToken != "" {
		if c.AuthSource == "" {
...
func (c *Config) CookieCredential() string {
	if c.IgnitionSessionCookie != "" {
		return c.IgnitionSessionCookie
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		IgnitionSessionCookie: c.IgnitionSessionCookie,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.IgnitionSessionCookie = creds.IgnitionSessionCookie
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.IgnitionSessionCookie = ""
	delete(c.envOverrides, "IgnitionSessionCookie")
	c.updateFileConfigField("IgnitionSessionCookie")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["IgnitionSessionCookie"] {
			out.IgnitionSessionCookie = c.fileConfig.IgnitionSessionCookie
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "IgnitionSessionCookie":
		c.fileConfig.IgnitionSessionCookie = c.IgnitionSessionCookie
	}
}
