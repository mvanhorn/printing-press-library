...
// TestMigrate_AddsColumnsOnUpgrade_Layouts verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Layouts(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "layouts" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("layouts")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from layouts after migrate", want)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Lists(t *testing.T) {
	...
	for _, want := range []string{
		...
		"emoji",
		"entity",
		"last_list_evaluation_at",
		"layout",
		"list_evaluation_started_at",
		"list_evaluation_status",
		"owner_id",
		"rank",
		"state",
		"title",
...
// TestMigrate_AddsColumnsOnUpgrade_Recording verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Recording(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "recording" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("recording")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from recording after migrate", want)
		}
	}
}
...
// TestMigrate_AddsColumnsOnUpgrade_Access verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Access(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "access" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("access")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from access after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_ObjectsLists verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_ObjectsLists(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "objects_lists" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("objects_lists")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from objects_lists after migrate", want)
		}
	}
}
...
// TestMigrate_AddsColumnsOnUpgrade_FieldsOrder verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_FieldsOrder(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "fields_order" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("fields_order")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from fields_order after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_FieldsVisibility verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_FieldsVisibility(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "fields_visibility" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("fields_visibility")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from fields_visibility after migrate", want)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Settings(t *testing.T) {
	...
	for _, want := range []string{
		"advanced_enrichments_override",
		"billing_overage_spend_limit_dollars",
		"billing_overages_disabled",
		"billing_use_legacy_pricing",
		"closed_lost_deal_stage",
		"closed_won_deal_stage",
		"deal_detection_enabled",
		"deal_detection_prompt",
		"deal_field_updates_enabled",
		"deal_summary_enabled",
		"email_branding_footer",
		"enable_attachment_ingestion",
		"enable_private_meeting_ingestion",
		"enable_recorder_background",
		"enable_zero_participant_meeting_ingestion",
		"exclude_internal_ingestion",
		"external_meeting_summary_enabled",
		"fiscal_year_offset",
		"free_workspace_credit_limit_override",
		"historical_sync_period_days",
		"ingestion_record_creation",
		"lock_meeting_recording_scope",
		"max_active_campaigns_override",
		"max_connected_accounts_override",
		"max_custom_objects_override",
		"meeting_prep_scope",
		"meeting_recorder_background",
		"meeting_recorder_name",
		"meeting_recording_scope",
		"meeting_summary_enabled",
		"meeting_task_scope",
		"meeting_video_storage_disabled",
		"org_description",
		"publish_meeting_prep_to_calendar",
		"reverse_trial_end_override",
		"value",
	} {
		if !hasColumn[want] {
