...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("prep_for_your_next_call",
			mcplib.WithDescription("Attendees, their company, open deals, and past-transcript excerpts in one compact payload."),
			mcplib.WithBoolean("next", mcplib.Description("Override the recipe's --next value.")),
		),
		handlePrepForYourNextCall,
	)
}

// handlePrepForYourNextCall runs the prep_for_your_next_call recipe intent: Attendees, their company, open deals, and past-transcript excerpts in one compact payload.
func handlePrepForYourNextCall(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	args := []string{}
	args = append(args, "prep")
	args = appendRecipeBoolFlag(args, "next", input["next"], true)
