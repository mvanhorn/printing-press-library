...
var whichIndex = []whichEntry{
	{Command: "prep", Description: "One command before a call: the meeting's attendees, their company, open deals, and transcript excerpts from past meetings with that company.", Group: "Rituals the CRM knows but cannot run", WhyItMatters: "Reach for this when the task is preparing for one specific upcoming meeting rather than fetching raw records. Requires a synced local mirror (sync --resources resources --path-context object=<type> first)."},
	{Command: "brief", Description: "Start-of-day overview: today's meetings joined to their companies, open deals, and yesterday's record activity, on one screen.", Group: "Rituals the CRM knows but cannot run", WhyItMatters: "Use this for a whole-day overview; use prep for a single meeting. Requires a synced local mirror (sync --resources resources --path-context object=<type> first)."},
	{Command: "followup", Description: "The dropped-ball list: meetings with no subsequent activity, comment, or task on the linked deal or company.", Group: "Rituals the CRM knows but cannot run", WhyItMatters: "Run it after a busy week to find meetings that never got a follow-up; --no-deal also surfaces companies with meetings but no open deal. Requires a synced local mirror (sync --resources resources --path-context object=<type> first)."},
	{Command: "stale", Description: "Open deals with no activity in N days, grouped by pipeline stage.", Group: "Pipeline analytics the API does not have", WhyItMatters: "The Monday pipeline-review question answered in one command instead of a CSV export. Requires a synced local mirror (sync --resources resources --path-context object=<type> first)."},
	{Command: "dossier", Description: "A complete background bundle on any record: fields, relationships, activities, comments, and related meetings with transcript references, in one compact payload.", Group: "Agent-native plumbing", WhyItMatters: "The one-call answer to 'tell me everything about this person/company/deal' — use prep instead when the subject is a specific upcoming meeting. Requires a synced local mirror."},
	{Command: "velocity", Description: "Per-stage dwell time and stage-to-stage conversion counts, accrued from a local stage-history table across repeated runs (the first run reports the current stage distribution).", Group: "Pipeline analytics the API does not have", WhyItMatters: "Answers 'how long do deals sit in each stage' without exporting anything to a spreadsheet. Requires a synced local mirror; dwell and conversion analytics build up as you re-run sync and velocity over time."},
	{Command: "dupes", Description: "Finds likely duplicate people or companies by shared email, domain, or normalized name, and prints ready-to-run merge commands.", Group: "Pipeline analytics the API does not have", WhyItMatters: "Weekly hygiene sweep for auto-built CRM data; each finding comes with the exact merge invocation to fix it. Requires a synced local mirror (sync --resources resources --path-context object=<type> first)."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  clarify-pp-cli which "stale tickets"
  clarify-pp-cli which "bottleneck"
  clarify-pp-cli which --limit 1 "send message"
  clarify-pp-cli which                                # list the full capability index`,
