...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/sales-and-crm/clarify/internal/learn/lookups"
...
Resource scoping:
  ...
  clarify-pp-cli sync --resources activities,lists

  # Full resync (ignore previous checkpoint)
  clarify-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  clarify-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  clarify-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  clarify-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "clarify-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
...
func defaultSyncResources() []string {
	// Flat resources whose only path placeholders resolve from env-backed
	// Config.TemplateVars (CLARIFY_WORKSPACE, CLARIFY_OBJECT). "resources"
	// is the population path for the store-dependent novel commands
	// (brief, prep, stale, followup, velocity); it surfaces an actionable
	// unresolved-placeholder error when CLARIFY_OBJECT is unset instead of
	// silently no-oping.
	return []string{"lists", "resources", "users", "workflows"}
}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		...
		"records":    "/workspaces/{workspace}/objects/{object}/records/{id}/attachments",
...
func responsePathForResource(resource, path string) []string {
	...
	case "comments\x00/workspaces/{workspace}/comments":
		return []string{"message"}
	case "comments\x00/workspaces/{workspace}/comments/{id}":
		return []string{"message"}
	case "lists\x00/workspaces/{workspace}/lists":
		return []string{"data"}
	case "schemas\x00/workspaces/{workspace}/schemas":
		return []string{"data"}
	case "users\x00/workspaces/{workspace}/users":
		return []string{"data"}
	case "workflows\x00/workspaces/{workspace}/workflows":
		return []string{"data"}
	}
	return nil
}
