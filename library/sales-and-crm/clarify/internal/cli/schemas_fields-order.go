...
func newSchemasFieldsOrderCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "fields-order",
		Short:       "Update fields order for schemas",
		Annotations: map[string]string{"mcp:read-only": "true", "pp:parent-group": "true", "pp:typed-exit-codes": "0,2"},
		RunE:        parentNoSubcommandRunE(flags),
	}

	cmd.AddCommand(newSchemasFieldsOrderUpdateSchemaPropertyOrderCmd(flags))
	return cmd
}
