...
Run sync first to populate the local search index.`,
  ...
  clarify-pp-cli search "status"

  # Force local search explicitly
  clarify-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  clarify-pp-cli search "status" --type campaigns --data-source local
  # JSON output for piping
  clarify-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'clarify-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
