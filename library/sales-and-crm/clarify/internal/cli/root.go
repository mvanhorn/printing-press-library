...
type rootFlags struct {
	...
	// invocation. Mirrors the CLARIFY_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "clarify-pp-cli",
		Short: `Clarify CLI — Every Clarify API operation as a typed command, plus the morning briefing, meeting prep, and pipeline analytics the…`,
		Long: `Clarify CLI — Every Clarify API operation as a typed command, plus the morning briefing, meeting prep, and pipeline analytics the autonomous CRM knows about but cannot run.
...
Health check: run 'clarify-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("clarify-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'clarify-pp-cli profile list')")
	...
	addNovelCommandIfAbsent(rootCmd, newNovelDupesCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelFollowupCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelPrepCmd(flags))
