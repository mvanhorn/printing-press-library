...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CLARIFY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CLARIFY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "clarify-pp-cli/feedback")
...
When ` + "`CLARIFY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CLARIFY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  clarify-pp-cli feedback "the sync --path-context flag name surprised me"
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  clarify-pp-cli feedback list
  clarify-pp-cli feedback list --limit 5
  clarify-pp-cli feedback list --json`,
