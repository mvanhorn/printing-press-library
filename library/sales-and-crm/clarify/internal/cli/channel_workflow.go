...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  clarify-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  clarify-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  clarify-pp-cli workflow archive --timeout 0`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  clarify-pp-cli workflow status

  # Show status as JSON
  clarify-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("clarify-pp-cli")
			}
