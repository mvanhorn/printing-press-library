...
type Config struct {
	...
	ClarifyApiKey    string          `toml:"api_key"`
...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.clarify.ai/v1",
	}
	...
	// Env var overrides
	if v := os.Getenv("CLARIFY_API_KEY"); v != "" {
		cfg.ClarifyApiKey = v
		cfg.markEnvOverride("ClarifyApiKey")
		cfg.AuthSource = "env:CLARIFY_API_KEY"
		cfg.CredentialSource = "env:CLARIFY_API_KEY"
	}
	...
	if v := os.Getenv("CLARIFY_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("CLARIFY_CAMPAIGN_ID")); v != "" {
		cfg.TemplateVars["campaignId"] = v
	} else if verifyMode {
		cfg.TemplateVars["campaignId"] = "campaignId_placeholder"
	}
	if v := strings.TrimSpace(os.Getenv("CLARIFY_ENTITY")); v != "" {
		cfg.TemplateVars["entity"] = v
	} else if verifyMode {
		cfg.TemplateVars["entity"] = "entity_placeholder"
	}
	if v := strings.TrimSpace(os.Getenv("CLARIFY_ID")); v != "" {
		cfg.TemplateVars["id"] = v
	} else if verifyMode {
		cfg.TemplateVars["id"] = "id_placeholder"
	}
	if v := strings.TrimSpace(os.Getenv("CLARIFY_OBJECT")); v != "" {
		cfg.TemplateVars["object"] = v
	} else if verifyMode {
		cfg.TemplateVars["object"] = "object_placeholder"
	}
	if v := strings.TrimSpace(os.Getenv("CLARIFY_WORKSPACE")); v != "" {
		cfg.TemplateVars["workspace"] = v
	} else if verifyMode {
		cfg.TemplateVars["workspace"] = "workspace_placeholder"
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("CLARIFY_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "clarify-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.ClarifyApiKey
	if token == "" {
		return ""
	}
	// Clarify sends the key as `Authorization: api-key <key>` — not Bearer,
	// and not the raw key. Accept either the raw key or an already-prefixed
	// value in CLARIFY_API_KEY and normalize to the wire format here.
	if !strings.HasPrefix(strings.ToLower(token), "api-key ") {
		token = "api-key " + token
	}
	return token
}
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.ClarifyApiKey != "" {
		parts = append(parts, "api_key="+c.ClarifyApiKey)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.ClarifyApiKey == "" {
		c.ClarifyApiKey = creds.ClarifyApiKey
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.ClarifyApiKey = token
	delete(c.envOverrides, "ClarifyApiKey")
	c.updateFileConfigField("ClarifyApiKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["ClarifyApiKey"] {
			out.ClarifyApiKey = c.fileConfig.ClarifyApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "ClarifyApiKey":
		c.fileConfig.ClarifyApiKey = c.ClarifyApiKey
	}
}
