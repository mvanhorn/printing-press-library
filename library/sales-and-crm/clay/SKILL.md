---
name: pp-clay
description: "The only Clay tool that can build a table, not just read one. Trigger phrases: `create a clay table`, `add a column in clay`, `write a clay formula`, `export my clay table design`, `clone this clay table`, `use clay`, `run clay`."
author: "Ade Amos"
license: "Apache-2.0"
...
metadata:
  openclaw:
    ...
    install:
      - kind: go
        bins: [clay-pp-cli]
        module: github.com/mvanhorn/printing-press-library/library/sales-and-crm/clay/cmd/clay-pp-cli
---

# Clay — Printing Press CLI

## Prerequisites: Install the CLI

This skill drives the `clay-pp-cli` binary. **You must verify the CLI is installed before invoking any command from this skill.** If it is missing, install it first:

1. Install via the Printing Press installer. It defaults binaries to `$HOME/.local/bin` on macOS/Linux and `%LOCALAPPDATA%\Programs\PrintingPress\bin` on Windows:
   ```bash
   npx -y @mvanhorn/printing-press-library install clay --cli-only
   ```
2. Verify: `clay-pp-cli --version`
...
Use this CLI when the task is to build or reshape a Clay table rather than read one: creating tables, adding columns, wiring formulas, attaching enrichment providers, or configuring HTTP API columns. It is also the right tool for capturing a table design as YAML and reproducing it elsewhere.

## Anti-triggers

Do not use this CLI for:
- Do not use this CLI to run enrichments across many rows and wait on credits; trigger runs in the Clay app and use watch to observe.
- Do not use this CLI as a general GTM data search tool when Clay's official CLI or MCP is already installed; those cover search and routines better.
- Do not use this CLI against a workspace you do not own; table and column authoring is destructive and runs as your logged-in user.
...
**accounts** — Connected provider credentials (API keys) in the workspace
...
**columns** — Create and configure table columns: text, formula, enrichment, and HTTP API
...
**enrichments** — Search Clay's enrichment provider catalog and resolve action parameters
...
**formulas** — Generate Clay formulas from natural language

- `clay-pp-cli formulas <workspaceId>` — Turn a natural-language prompt into a Clay formula

**public** — Clay's documented Public API (requires CLAY_API_KEY, not the browser session)
...
**tables** — Create and manage Clay tables
...
**workbooks** — Workbooks group related tables
...
**workspace** — Workspace metadata, permissions, sources, and connected accounts
...
```bash
clay-pp-cli which "<capability in your own words>"
```
...
Run `clay-pp-cli doctor` to verify setup.
...
- Use `--home <dir>` for one invocation, or set `CLAY_HOME=<dir>` to relocate all four path kinds under one root.
- Use per-kind env vars only when a specific kind must diverge: `CLAY_CONFIG_DIR`, `CLAY_DATA_DIR`, `CLAY_STATE_DIR`, `CLAY_CACHE_DIR`.
- Resolution order is per-kind env var, `--home`, `CLAY_HOME`, XDG (`XDG_CONFIG_HOME`, `XDG_DATA_HOME`, `XDG_STATE_HOME`, `XDG_CACHE_HOME`), then platform defaults.
- `config` contains settings like `config.toml` and profiles. `data` contains `credentials.toml`, `data.db`, cookies, and auth sidecars. `state` contains persisted queries, jobs, and `teach.log`. `cache` contains regenerable HTTP/cache files.
- Stored secrets live in `credentials.toml` under the data dir. Existing legacy `config.toml` secrets are read for compatibility and leave `config.toml` on the first auth write.
- Run `clay-pp-cli doctor --fail-on warn` to surface path and credential-location warnings. `agent-context` exposes a schema v4 `paths` block for agents that need the resolved dirs.
...
Fleet precedence: an inherited per-kind env var overrides an explicit `--home` for that kind. Use `CLAY_HOME` or per-kind vars as durable fleet levers, and use `--home` only for a single invocation. Relocation is not reversible by unsetting env vars; move files manually before clearing `CLAY_HOME`, or `doctor` will not find credentials left under the former root.
...
```bash
clay-pp-cli recall "<user's question>" --agent
```
...
```json
{
  ...
  "candidates": [
    { "id": 12, "class": "flag_alias | playbook_candidate",
      ...
      "rationale": "...",
      "next_action": ["<trial command>", "clay-pp-cli learnings confirm 12"] }
  ],
  "playbook": {
    "query_family": "...",
    "playbook": {
...
if Playbook present:
    ...
    -> the Playbook's expected_tool_calls is a budget; if you find yourself running
       materially more, record the divergence via `clay-pp-cli playbook amend`
       at end-of-session.
...
- `lookup_refresh_available` (top-level): an entity in the query has no lookup row yet, but synced data could provide one. Run `clay-pp-cli sync` to refresh entity lookups.
...
```bash
clay-pp-cli teach --query "<user's question>" --resource-type <type> --resource <id1> --resource <id2>
...
`clay-pp-cli learnings stats` reports recall hit rate, teach-to-reuse, playbook resolution rate, and candidate confirm/reject counts from the local `learn_events` table. Rates are null until they have a denominator; everything stays on this machine. Use it to check whether the loop is earning its keep for this CLI.

### Disabling learning

- `--no-learn` on a single command short-circuits both `recall` and the `teach` write path. Use for deterministic agent flows or tests that must not be affected by accumulated learnings.
- `CLAY_NO_LEARN=true` in the environment globally disables the pipeline.
...
```
clay-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
clay-pp-cli feedback --stdin < notes.txt
clay-pp-cli feedback list --json --limit 10
```

Entries are stored locally as `feedback.jsonl` under the resolved data dir. They are never POSTed unless `CLAY_FEEDBACK_ENDPOINT` is set AND either `--send` is passed or `CLAY_FEEDBACK_AUTO_SEND=true`. Default behavior is local-only.
...
```
clay-pp-cli profile save briefing --json
clay-pp-cli --profile briefing accounts list mock-value
clay-pp-cli profile list --json
clay-pp-cli profile show briefing
clay-pp-cli profile delete briefing --yes
```
...
1. **Empty, `help`, or `--help`** → show `clay-pp-cli --help` output
...
2. Register with Claude Code:
   ```bash
   claude mcp add clay-pp-mcp -- clay-pp-mcp
   ```
3. Verify: `claude mcp list`

## Direct Use

1. Check if installed: `which clay-pp-cli`
   If not found, offer to install (see Prerequisites at the top of this skill).
2. Match the user query to the best command from the Unique Capabilities and Command Reference above.
3. Execute with the `--agent` flag:
   ```bash
   clay-pp-cli <command> [subcommand] [args] --agent
   ```
4. If ambiguous, drill into subcommand help: `clay-pp-cli <command> --help`.
