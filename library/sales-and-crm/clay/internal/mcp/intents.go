...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("check_a_table_before_spending_credits",
			mcplib.WithDescription("Reports formulas referencing deleted columns and columns nothing consumes."),
			mcplib.WithString("slug", mcplib.Required(), mcplib.Description("Override the recipe's positional slug value.")),
		),
		handleCheckATableBeforeSpendingCredits,
	)
}

// handleCheckATableBeforeSpendingCredits runs the check_a_table_before_spending_credits recipe intent: Reports formulas referencing deleted columns and columns nothing consumes.
func handleCheckATableBeforeSpendingCredits(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
