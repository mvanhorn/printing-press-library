...
var whichIndex = []whichEntry{
	{Command: "blueprint export", Description: "Snapshot a table's entire column graph to portable JSON you can commit to git.", Group: "Design as code", WhyItMatters: "Reach for this when a table design is worth keeping or reviewing; it is the only way to get Clay column configuration into version control."},
	{Command: "blueprint apply", Description: "Rebuild a captured table design in a new workbook, remapping formula references automatically.", Group: "Design as code", WhyItMatters: "Reach for this to clone a proven lead-gen table into a new market without rebuilding columns by hand."},
	{Command: "columns graph", Description: "Show which columns feed which, resolved from formula field references.", Group: "Table intelligence", WhyItMatters: "Reach for this before deleting or renaming a column, to see what breaks downstream."},
	{Command: "columns doctor", Description: "Find formulas pointing at deleted columns and columns nothing consumes.", Group: "Table intelligence", WhyItMatters: "Reach for this before a large enrichment run so broken formulas do not burn credits."},
	{Command: "enrichments compare", Description: "Rank enrichment providers for a job and flag which ones your workspace can actually run today.", Group: "Table intelligence", WhyItMatters: "Reach for this when choosing between providers; it is advisory and never spends credits."},
	{Command: "tables diff", Description: "Structurally compare two tables' column graphs, including formulas and enrichment bindings.", Group: "Table intelligence", WhyItMatters: "Reach for this to see what drifted between a template table and a copy."},
	{Command: "watch", Description: "Block until every column's enrichment runs settle, reporting per-column status counts.", Group: "Local state that compounds", WhyItMatters: "Reach for this in a script after triggering enrichment, instead of polling the UI."},
	{Command: "columns link", Description: "Connect two tables by creating a lookup column that pulls the matching row from another table on a join key.", Group: "Design as code", WhyItMatters: "Reach for this to wire one table's data into another; it is the scriptable form of Clay's Lookup enrichment."},
	{Command: "workbooks graph", Description: "Show how every table and CSV source in a workbook feeds the others, with each table's credit estimate.", Group: "Table intelligence", WhyItMatters: "Reach for this to understand an unfamiliar workbook before changing anything in it."},
	...
	{Command: "columns run", Description: "Trigger a run for action columns so their downstream formulas recompute.", Group: "Local state that compounds", WhyItMatters: "Reach for this after changing a formula or enrichment config; without it the column keeps showing stale values and status polls report settled."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  clay-pp-cli which "stale tickets"
  clay-pp-cli which "bottleneck"
  clay-pp-cli which --limit 1 "send message"
  clay-pp-cli which                                # list the full capability index`,
