...
In local mode: searches locally synced data only.`,
  ...
  clay-pp-cli search "error timeout"

  # Force local search only
  clay-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  clay-pp-cli search "status" --type accounts --data-source local
  # JSON output for piping
  clay-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /search/filters-mode
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/search/filters-mode", map[string]any{
					"q":           query,
					"source_type": "people",
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'clay-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
