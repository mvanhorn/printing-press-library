...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CLAY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CLAY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "clay-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  clay-pp-cli feedback \"columns link needs a --pull flag\"\n  clay-pp-cli feedback list",
		Long: `Feedback is captured locally first in the CLI data directory's feedback.jsonl.
When ` + "`CLAY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CLAY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  clay-pp-cli feedback list
  clay-pp-cli feedback list --limit 5
  clay-pp-cli feedback list --json`,
