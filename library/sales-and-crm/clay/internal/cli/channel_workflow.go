...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  clay-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  clay-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  clay-pp-cli workflow archive --timeout 0`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  clay-pp-cli workflow status

  # Show status as JSON
  clay-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("clay-pp-cli")
			}
