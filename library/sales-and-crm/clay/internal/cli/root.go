...
type rootFlags struct {
	...
	// invocation. Mirrors the CLAY_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "clay-pp-cli",
		Short: `Clay CLI — The only Clay tool that can build a table, not just read one.`,
		Long: `Clay CLI — The only Clay tool that can build a table, not just read one.
...
See README.md or the bundled SKILL.md for recipes.`,
	...
	rootCmd.SetVersionTemplate("clay-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'clay-pp-cli profile list')")
	...
	rootCmd.AddCommand(newEnrichmentsCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelBlueprintCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelErrorsCmd(flags))
