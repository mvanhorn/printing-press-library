...
func TestHandleSQLBoundsRows(t *testing.T) {
	...
	for i := 0; i < mcpToolResultMaxItems+10; i++ {
		items = append(items, json.RawMessage(fmt.Sprintf(`{"id":"row-%03d","name":"%s"}`, i, strings.Repeat("x", 256))))
	}
	...
	result, err := handleSQL(context.Background(), mcplib.CallToolRequest{
		Params: mcplib.CallToolParams{
			Arguments: map[string]any{
				"query": "SELECT id, data FROM resources ORDER BY id",
			},
		},
	})
	if err != nil {
		t.Fatalf("handleSQL returned error: %v", err)
	}
	...
	if len(envelope.Rows) > mcpToolResultMaxItems {
		t.Fatalf("SQL returned %d rows, want <= %d", len(envelope.Rows), mcpToolResultMaxItems)
	}
