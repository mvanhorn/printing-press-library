...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "outreach-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("OUTREACH_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("OUTREACH_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "outreach-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/outreach-pp-cli/feedback.jsonl.
When ` + "`OUTREACH_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`OUTREACH_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  outreach-pp-cli feedback list
  outreach-pp-cli feedback list --limit 5
  outreach-pp-cli feedback list --json`,
