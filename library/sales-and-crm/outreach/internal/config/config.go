...
type Config struct {
	...
	OutreachBearerAuth string            `toml:"bearer_auth"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("OUTREACH_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "outreach-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("OUTREACH_BEARER_AUTH"); v != "" {
		cfg.OutreachBearerAuth = v
		cfg.markEnvOverride("OutreachBearerAuth")
		cfg.AuthSource = "env:OUTREACH_BEARER_AUTH"
	}
	...
	if cfg.AuthSource == "" && cfg.OutreachBearerAuth != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("OUTREACH_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.OutreachBearerAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:OUTREACH_BEARER_AUTH"
		}
		return "Bearer " + c.OutreachBearerAuth
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientID")
	c.updateFileConfigField("ClientSecret")
	c.OutreachBearerAuth = ""
	delete(c.envOverrides, "OutreachBearerAuth")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["OutreachBearerAuth"] {
			out.OutreachBearerAuth = c.fileConfig.OutreachBearerAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "OutreachBearerAuth":
		c.fileConfig.OutreachBearerAuth = c.OutreachBearerAuth
	}
}
