# Outreach Printed CLI Agent Guide

This directory is a generated `outreach-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
outreach-pp-cli doctor --json
outreach-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
outreach-pp-cli which "<capability>" --json
outreach-pp-cli <command> --help
```
...
```bash
outreach-pp-cli <command> --help
outreach-pp-cli <command> --dry-run --agent
```
