...
**Outreach read workflows for agents that need sales engagement context.**

Wraps Outreach read APIs with agent defaults, local SQLite sync, search, analytics, command discovery, and MCP tools so agents can inspect prospects, sequences, tasks, mailboxes, opportunities, and activity without hand-building HTTP calls.
...
- **`search`** — Search live or locally synced Outreach records from one agent-friendly command.
