...
**Gainsight read workflows for agents that need customer success context.**

Wraps Gainsight read APIs with agent defaults, local SQLite sync, search, analytics, command discovery, and MCP tools so agents can inspect customer success data, health scores, Cockpit CTAs, success plans, and timeline activity without hand-building HTTP calls.
...
```bash
# Inspect the auth and tenant-base-url diagnostic surface.
gainsight-pp-cli doctor --help

# Review how to build the local SQLite mirror before offline analysis.
gainsight-pp-cli sync --help

# Find the exact command path for a natural-language task.
gainsight-pp-cli which --help
...
- **`search`** — Search live or locally synced Gainsight records from one agent-friendly command.
