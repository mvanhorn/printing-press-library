...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  sybill-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  sybill-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"accounts", "conversations", "deals", "documents", "health", "messages", "object-types", "rows", "sources"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  sybill-pp-cli workflow status

  # Show status as JSON
  sybill-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("sybill-pp-cli")
			}
