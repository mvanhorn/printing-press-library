...
In local mode: searches locally synced data only.`,
  ...
  sybill-pp-cli search "error timeout"

  # Force local search only
  sybill-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  sybill-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  sybill-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'sybill-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "object-types":
				results, err = db.SearchObjectTypes(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchObjectTypes(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search object_types failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/sybill-pp-cli/data.db)")
