...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "sybill-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SYBILL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SYBILL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "sybill-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/sybill-pp-cli/feedback.jsonl.
When ` + "`SYBILL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SYBILL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  sybill-pp-cli feedback list
  sybill-pp-cli feedback list --limit 5
  sybill-pp-cli feedback list --json`,
