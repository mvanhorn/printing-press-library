...
type Config struct {
	...
	Path          string            `toml:"-"`
	SybillApiKey  string            `toml:"api_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.sybill.ai",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("SYBILL_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "sybill-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("SYBILL_API_KEY"); v != "" {
		cfg.SybillApiKey = v
		cfg.AuthSource = "env:SYBILL_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.SybillApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("SYBILL_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.SybillApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:SYBILL_API_KEY"
		}
		return "Bearer " + c.SybillApiKey
	}
	if c.AccessToken != "" {
