# Sybill Printed CLI Agent Guide

This directory is a generated `sybill-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
sybill-pp-cli doctor --json
sybill-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
sybill-pp-cli which "<capability>" --json
sybill-pp-cli <command> --help
```
...
```bash
sybill-pp-cli <command> --help
sybill-pp-cli <command> --dry-run --agent
```
