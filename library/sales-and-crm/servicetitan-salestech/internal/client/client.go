...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "servicetitan-salestech-pp-cli", "http")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		// PATCH: composed-auth-apikey-wire (#1333 / #1303 apiKey half) —
		// ServiceTitan composed auth needs the static ST-App-Key header on
		// every call in addition to the OAuth2 bearer. The v4.8.0 generator
		// wired the bearer half but omits this one; without it every
		// authenticated endpoint returns 401 even with a valid token.
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "servicetitan-salestech-pp-cli/1.0")
		}
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	...
	// PATCH: composed-auth-apikey-wire (dry-run mirror) — surface ST-App-Key
	// under the same mask so dry-run output reflects what will actually be
	// sent to ServiceTitan.
	if c.Config != nil && c.Config.StAppKey != "" {
