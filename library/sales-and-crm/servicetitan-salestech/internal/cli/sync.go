...
Resource scoping:
  ...
  servicetitan-salestech-pp-cli sync

  # Sync specific resources only
  servicetitan-salestech-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  servicetitan-salestech-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  servicetitan-salestech-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  servicetitan-salestech-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  servicetitan-salestech-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "sell":
		return db.UpsertSell(data)
	case "unsell":
		return db.UpsertUnsell(data)
	default:
...
// PATCH: sync-default-resources (#1305) — the v4.8.0 generator emits
// `return []string{}` for this tenant-scoped spec, so plain `sync` silently
// exits with resources:0. Populated with the cacheable Sales/Estimates list
// resources so a no-flag `sync` iterates the full set. Estimate items and
// status changes have their own hand-built sweeps (`sync-items`,
// `sync-status-changes`) because they write to separate store tables and
// would clobber the parent `estimates` row if reused here.
...
//
// PATCH: sync-paths-registry (#1305 / #1332) — the v4.8.0 generator emits
