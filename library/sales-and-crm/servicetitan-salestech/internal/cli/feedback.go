...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".servicetitan-salestech-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SERVICETITAN_SALESTECH_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SERVICETITAN_SALESTECH_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "servicetitan-salestech-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.servicetitan-salestech-pp-cli/feedback.jsonl.
When ` + "`SERVICETITAN_SALESTECH_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SERVICETITAN_SALESTECH_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "servicetitan-salestech-pp-cli",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  servicetitan-salestech-pp-cli feedback list
  servicetitan-salestech-pp-cli feedback list --limit 5
  servicetitan-salestech-pp-cli feedback list --json`,
