# ServiceTitan Sales & Estimates Printed CLI Agent Guide

This directory is a generated `servicetitan-salestech-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
servicetitan-salestech-pp-cli doctor --json
servicetitan-salestech-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
servicetitan-salestech-pp-cli which "<capability>" --json
servicetitan-salestech-pp-cli <command> --help
```
...
```bash
servicetitan-salestech-pp-cli <command> --agent
```
...
```bash
servicetitan-salestech-pp-cli <command> --help
servicetitan-salestech-pp-cli <command> --dry-run --agent
```
