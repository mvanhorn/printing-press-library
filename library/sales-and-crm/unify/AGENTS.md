# Unify Printed CLI Agent Guide

This directory is a generated `unify-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
unify-pp-cli doctor --json
unify-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
unify-pp-cli which "<capability>" --json
unify-pp-cli <command> --help
```
...
```bash
unify-pp-cli <command> --help
unify-pp-cli <command> --dry-run --agent
```
