...
//
// The allow-list covers (1) generic record identifiers and timestamps that
// apply across most APIs and (2) Unify-specific novel-command output keys
// (search hits, watchlist rows, vet rows, audit-scores, coverage, schema
// diff). If a row has zero matching keys after filtering, the original row
// is preserved so agents do not see `[{}]` (an empty projection that
// strips ALL keys is worse than no projection — the agent loses the
// ability to follow up on the result).
func compactListFields(items []map[string]any) json.RawMessage {
	keepFields := map[string]bool{
		...
		"url": true, "email": true, "key": true, "domain": true,
		"created_at": true, "updated_at": true, "createdAt": true, "updatedAt": true,
		// Unify novel-command output keys.
		"object_name": true, "object": true, "snippet": true,
		"match_key": true, "match_value": true, "match": true,
		"added": true, "exists": true, "error": true, "record_id": true,
		"action": true, "field": true, "field_a": true, "field_b": true,
		"value_a": true, "value_b": true, "delta": true, "threshold": true,
		"bucket": true, "matched": true, "left_only": true, "right_only": true,
		"left_total": true, "right_total": true,
		"has_opportunity": true, "owner": true, "last_activity_at": true,
	}
	...
	for _, item := range items {
		...
		// Fall back to the original row when no field matched. Returning
		// `{}` would silently drop every key the caller cares about and
		// looks identical to an empty result; preserving the row keeps
		// the data agent-actionable.
		if len(compact) == 0 {
			compact = item
		}
