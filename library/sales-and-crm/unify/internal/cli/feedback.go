...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".unify-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("UNIFY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("UNIFY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "unify-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.unify-pp-cli/feedback.jsonl.
When ` + "`UNIFY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`UNIFY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  unify-pp-cli feedback list
  unify-pp-cli feedback list --limit 5
  unify-pp-cli feedback list --json`,
