...
type Config struct {
	...
	Path          string            `toml:"-"`
	UnifyApiKey   string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("UNIFY_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "unify-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("UNIFY_API_KEY"); v != "" {
		cfg.UnifyApiKey = v
		cfg.AuthSource = "env:UNIFY_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.UnifyApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("UNIFY_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
