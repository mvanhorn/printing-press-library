...
func TestUpsertBatch_PopulatesContactListsTable(t *testing.T) {
	...
	if _, _, err := s.UpsertBatch("contact_lists", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "contact_lists").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
...
func TestUpsertBatch_PopulatesOrganizationListsTable(t *testing.T) {
	...
	if _, _, err := s.UpsertBatch("organization_lists", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "organization_lists").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
