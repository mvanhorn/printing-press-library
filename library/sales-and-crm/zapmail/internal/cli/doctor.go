...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  zapmail-pp-cli doctor
  zapmail-pp-cli doctor --json
  zapmail-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export ZAPMAIL_API_KEY=<your-key>"
					report["auth_key_url"] = "https://app.zapmail.ai/"
				} else {
					authConfigured = true
			...
			if os.Getenv("ZAPMAIL_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "ZAPMAIL_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "ZAPMAIL_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["x-auth-zapmail"] = authHeader
						authHeaders["User-Agent"] = "zapmail-pp-cli"
						verifyPath := "/v2/users"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'zapmail-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'zapmail-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'zapmail-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'zapmail-pp-cli sync' to refresh."
	}
	return report
}
