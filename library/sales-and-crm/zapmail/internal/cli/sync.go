...
Resource scoping:
  ...
  zapmail-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  zapmail-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  zapmail-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  zapmail-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  zapmail-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/zapmail-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "mailboxes":
		return db.UpsertMailboxes(data)
	case "subscriptions":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"domains":                  "/v2/domains",
		"domains-assignable":       "/v2/domains/assignable",
		"inbox":                    "/v2/onebox/search-emails",
		"inbox-connected-accounts": "/v2/onebox/connected-accounts",
		"mailboxes":                "/v2/mailboxes/list",
