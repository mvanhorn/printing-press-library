...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  zapmail-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  zapmail-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"domains", "domains-assignable", "inbox", "inbox-connected-accounts", "mailboxes", "subscriptions", "workspaces"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  zapmail-pp-cli workflow status

  # Show status as JSON
  zapmail-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("zapmail-pp-cli")
			}
