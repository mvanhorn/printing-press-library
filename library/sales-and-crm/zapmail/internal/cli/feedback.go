...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "zapmail-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ZAPMAIL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ZAPMAIL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "zapmail-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/zapmail-pp-cli/feedback.jsonl.
When ` + "`ZAPMAIL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ZAPMAIL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  zapmail-pp-cli feedback list
  zapmail-pp-cli feedback list --limit 5
  zapmail-pp-cli feedback list --json`,
