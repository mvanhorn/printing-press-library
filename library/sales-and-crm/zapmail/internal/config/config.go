...
type Config struct {
	...
	Path          string            `toml:"-"`
	ZapmailApiKey string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("ZAPMAIL_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "zapmail-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("ZAPMAIL_API_KEY"); v != "" {
		cfg.ZapmailApiKey = v
		cfg.AuthSource = "env:ZAPMAIL_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.ZapmailApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("ZAPMAIL_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	replacements := map[string]string{
		"token":           token,
		"api_key":         c.ZapmailApiKey,
		"ZAPMAIL_API_KEY": c.ZapmailApiKey,
	}
