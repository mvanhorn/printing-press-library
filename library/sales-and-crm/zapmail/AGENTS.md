# Zapmail Printed CLI Agent Guide

This directory is a generated `zapmail-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
zapmail-pp-cli doctor --json
zapmail-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
zapmail-pp-cli which "<capability>" --json
zapmail-pp-cli <command> --help
```
...
```bash
zapmail-pp-cli <command> --help
zapmail-pp-cli <command> --dry-run --agent
```
