...
func compactFields(data json.RawMessage) json.RawMessage {
	...
	if err := json.Unmarshal(data, &obj); err == nil {
		// Spring-style pagination wrapper: {"content": [...], "totalElements": N, ...}
		// The "content" key is the payload here, not prose to strip. Compact the
		// inner array and keep the pagination metadata so agents see both rows
		// and totals.
		if rawContent, ok := obj["content"]; ok {
			if _, hasTotal := obj["totalElements"]; hasTotal {
				if inner, ok := rawContent.([]any); ok {
					innerItems := make([]map[string]any, 0, len(inner))
					for _, it := range inner {
						if m, ok := it.(map[string]any); ok {
							innerItems = append(innerItems, m)
						}
					}
					compactedRaw := compactListFields(innerItems)
					var compactedItems []map[string]any
					_ = json.Unmarshal(compactedRaw, &compactedItems)
					out := map[string]any{}
					for k, v := range obj {
						if compactVerboseFields[k] && k != "content" {
							continue
						}
						out[k] = v
					}
					out["content"] = compactedItems
					result, _ := json.Marshal(out)
					return result
				}
			}
