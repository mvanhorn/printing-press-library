...
This directory is a generated `ghl-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
ghl-pp-cli doctor --json
ghl-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
ghl-pp-cli which "<capability>" --json
ghl-pp-cli <command> --help
```
...
```bash
ghl-pp-cli <command> --help
ghl-pp-cli <command> --dry-run --agent
```
