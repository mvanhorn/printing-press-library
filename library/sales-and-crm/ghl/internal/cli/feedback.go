...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".ghl-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GHL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GHL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ghl-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.ghl-pp-cli/feedback.jsonl.
When ` + "`GHL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GHL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ghl-pp-cli feedback list
  ghl-pp-cli feedback list --limit 5
  ghl-pp-cli feedback list --json`,
