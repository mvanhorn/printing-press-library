...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ghl-pp-cli doctor
  ghl-pp-cli doctor --json
  ghl-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export GOHIGHLEVEL_TOKEN=<your-key>"
					report["auth_docs_url"] = "https://highlevel.stoplight.io/docs/integrations/"
				} else {
					authConfigured = true
			...
			if os.Getenv("GOHIGHLEVEL_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "GOHIGHLEVEL_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "GOHIGHLEVEL_TOKEN")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'ghl-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'ghl-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'ghl-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'ghl-pp-cli sync' to refresh."
	}
	return report
}
