...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "ghl-pp-cli",
		Short: `Ghl CLI — Every GoHighLevel surface that matters for a marketing/CRM operator — with offline SQL, kill-switch tag awareness, and …`,
		Long: `Ghl CLI — Every GoHighLevel surface that matters for a marketing/CRM operator — with offline SQL, kill-switch tag awareness, and …
...
Health check: run 'ghl-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("ghl-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'ghl-pp-cli profile list')")
	...
	// Wire novel contact subcommands into the existing contacts parent.
	contactsParent := newContactsCmd(flags)
	contactsParent.AddCommand(newContactsRecencyCmd(flags))
	rootCmd.AddCommand(contactsParent)
	...
	// Wire novel opportunities subcommands into the existing parent.
	opportunitiesParent := newOpportunitiesCmd(flags)
	opportunitiesParent.AddCommand(newOpportunitiesStaleCmd(flags))
	opportunitiesParent.AddCommand(newOpportunitiesFunnelCmd(flags))
	rootCmd.AddCommand(opportunitiesParent)
	...
	// Workflows: promoted GET shortcut plus the novel `members` subcommand.
	workflowsParent := newWorkflowsPromotedCmd(flags)
	workflowsParent.AddCommand(newWorkflowsMembersCmd(flags))
	rootCmd.AddCommand(workflowsParent)
	// Novel top-level groups.
	rootCmd.AddCommand(newKillswitchCmd(flags))
