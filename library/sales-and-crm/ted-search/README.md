...
The recommended path installs both the `ted-search-pp-cli` binary and the `pp-ted-search` agent skill in one shot:

```bash
npx -y @mvanhorn/printing-press install ted-search
```
...
Download a pre-built binary for your platform from the [latest release](https://github.com/mvanhorn/printing-press-library/releases/tag/ted-search-current). On macOS, clear the Gatekeeper quarantine: `xattr -d com.apple.quarantine <binary>`. On Unix, mark it executable: `chmod +x <binary>`.
...
```bash
hermes skills install mvanhorn/printing-press-library/cli-skills/pp-ted-search --force
```

Inside a Hermes chat session:

```bash
/skills install mvanhorn/printing-press-library/cli-skills/pp-ted-search --force
```
...
```
Install the pp-ted-search skill from https://github.com/mvanhorn/printing-press-library/tree/main/cli-skills/pp-ted-search. The skill defines how its required CLI can be installed.
```
...
eu-ted-pp-cli leads --cpv 45 --country DEU --min-value 500000 --days 30


# Find open IT contracts in Germany
eu-ted-pp-cli notices search --country DEU --cpv 72000000 --scope ACTIVE --limit 10


# Pull matching notices into local SQLite for offline analysis
eu-ted-pp-cli sync --country DEU --cpv 72000000 --since 2023-01-01


# Rank open opportunities by urgency, value, and fit
eu-ted-pp-cli score --keywords "cloud migration" --country DEU --max-days 30


# Morning briefing: highest-priority expiring tenders
eu-ted-pp-cli deadline-heat --country DEU --cpv 72 --days 14


# Is this market open? See incumbent repeat rates before writing a proposal
eu-ted-pp-cli win-rate --cpv 72000000 --country DEU --min-calls 5
...
Run `ted-search-pp-cli --help` for the full command reference and flag list.
...
- **`ted-search-pp-cli notices search`** - Search for notices using expert search query. More information about the query format and field names can be found on [this page](https://ted.europa.eu/en/search/expert-search))
...
ted-search-pp-cli notices

# JSON for scripting and agents
ted-search-pp-cli notices --json

# Filter to specific fields
ted-search-pp-cli notices --json --select id,name,status

# Dry run — show the request without sending
ted-search-pp-cli notices --dry-run

# Agent mode — JSON + compact + no prompts in one flag
ted-search-pp-cli notices --agent
```
...
```bash
npx skills add mvanhorn/printing-press-library/cli-skills/pp-ted-search -g
```

Then invoke `/pp-ted-search <query>` in Claude Code. The skill is the most efficient path — Claude Code drives the CLI directly without an MCP server in the middle.
...
```bash
claude mcp add ted-search ted-search-pp-mcp
```
...
1. Download the `.mcpb` for your platform from the [latest release](https://github.com/mvanhorn/printing-press-library/releases/tag/ted-search-current).
...
Config file: `~/.config/ted-search-pp-cli/config.toml`
...
- **CPV code not found** — Run 'eu-ted-pp-cli cpv search <keyword>' to find the right 8-digit code
