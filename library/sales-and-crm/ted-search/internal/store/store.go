...
func (s *Store) UpsertNotice(n Notice) error {
	const q = `INSERT OR REPLACE INTO notices
	...
	_, err := s.db.Exec(q,
		...
		n.RawData, n.SyncedAt,
	)
	if err != nil {
		return fmt.Errorf("upsert notice %s: %w", n.ID, err)
	}
	// Keep FTS index in sync.
	_, _ = s.db.Exec(`INSERT OR REPLACE INTO notices_fts(rowid, title, buyer_name, winner_name)
