...
Examples:
  ted-search-pp-cli concentration --country DEU --cpv 72
  ted-search-pp-cli concentration --cpv 45 --country FRA --top 10`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			q += " GROUP BY winner_name ORDER BY " + orderBy + " DESC LIMIT ?"
			...
			for rows.Next() {
				var r concRow
				r.Rank = rank
				if err := rows.Scan(&r.WinnerName, &r.Awards, &r.TotalValue, &r.MarketShare); err != nil {
					continue
				}
			...
			fmt.Fprintf(tw, "RANK\tWINNER\tAWARDS\t%s\tMARKET SHARE%%\tHHI CONTRIB\n", valueLabel)
			for _, r := range results {
				fmt.Fprintf(tw, "%d\t%s\t%d\t€%.0f\t%.1f%%\t%.1f\n",
					r.Rank,
