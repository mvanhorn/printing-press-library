...
func defaultDBPath() string {
	...
	return filepath.Join(home, ".config", "ted-search-pp-cli", "notices.db")
}
...
Examples:
  ...
  ted-search-pp-cli sync --query "buyer-country=FRA AND notice-type=can-standard" --since 2024-06-01`,
...
func syncNoticesWithClient(c clientPoster, st *store.Store, query string, limit int, queryKey string, flags *rootFlags, cmd *cobra.Command) (int, error) {
	...
	for {
		...
		if len(resp.Notices) < batchSize || (limit > 0 && count >= limit) {
			break
		}
	}
