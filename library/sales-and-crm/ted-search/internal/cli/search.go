...
Requires notices to be synced first:
  ted-search-pp-cli sync --country DEU --cpv 72000000 --since 2024-01-01

Examples:
  ted-search-pp-cli search "cloud software"
  ted-search-pp-cli search "hospital construction" --limit 50 --json`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			count, err := st.Count()
			if err == nil && count == 0 {
				fmt.Fprintf(cmd.OutOrStdout(), "No notices synced yet. Run: ted-search-pp-cli sync --country DEU --cpv 72000000 --since 2024-01-01\n")
				return nil
			}
