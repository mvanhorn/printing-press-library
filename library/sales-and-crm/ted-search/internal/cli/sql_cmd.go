...
Examples:
  ted-search-pp-cli sql "SELECT id, buyer_name, title FROM notices LIMIT 10"
  ted-search-pp-cli sql "SELECT buyer_country, COUNT(*) as n FROM notices GROUP BY buyer_country ORDER BY n DESC"`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !strings.HasPrefix(strings.ToUpper(trimmed), "SELECT") {
				return fmt.Errorf("only SELECT queries are allowed\nhint: use 'ted-search-pp-cli sync' to write data")
			}
