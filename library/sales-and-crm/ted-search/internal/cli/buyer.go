...
Examples:
  ted-search-pp-cli buyer --name "Bundesministerium" --show-winners
  ted-search-pp-cli buyer --name "Ministere" --since 2023-01-01 --json`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			count, _ := st.Count()
			if count == 0 {
				fmt.Fprintf(cmd.OutOrStdout(), "No notices synced yet. Run: ted-search-pp-cli sync --since 2024-01-01\n")
				return nil
			}
			...
			if err == nil {
				...
				for cpvRows.Next() {
					var c cpvStat
					if err := cpvRows.Scan(&c.CPVCode, &c.Count, &c.AvgValue); err != nil {
						continue
					}
			...
			if showWinners {
				...
				winQ += " GROUP BY winner_name ORDER BY awards DESC LIMIT 10"
