...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".ted-search-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TED_SEARCH_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TED_SEARCH_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ted-search-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.ted-search-pp-cli/feedback.jsonl.
When ` + "`TED_SEARCH_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TED_SEARCH_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ted-search-pp-cli feedback list
  ted-search-pp-cli feedback list --limit 5
  ted-search-pp-cli feedback list --json`,
