...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  email-bison-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  email-bison-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"campaigns", "campaigns-schedule-templates", "custom-variables", "leads", "replies", "sender-emails", "tags", "users", "workspaces"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  email-bison-pp-cli workflow status

  # Show status as JSON
  email-bison-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("email-bison-pp-cli")
			}
