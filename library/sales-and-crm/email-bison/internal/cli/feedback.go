...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "email-bison-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("EMAIL_BISON_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("EMAIL_BISON_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "email-bison-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/email-bison-pp-cli/feedback.jsonl.
When ` + "`EMAIL_BISON_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`EMAIL_BISON_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  email-bison-pp-cli feedback list
  email-bison-pp-cli feedback list --limit 5
  email-bison-pp-cli feedback list --json`,
