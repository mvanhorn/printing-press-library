...
Resource scoping:
  ...
  email-bison-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  email-bison-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  email-bison-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  email-bison-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  email-bison-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/email-bison-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "attach_sender_emails":
		return db.UpsertAttachSenderEmails(data)
	case "create_schedule_from_template":
		return db.UpsertCreateScheduleFromTemplate(data)
	case "campaigns_leads":
		return db.UpsertCampaignsLeads(data)
	case "pause":
		return db.UpsertPause(data)
	case "remove_sender_emails":
		return db.UpsertRemoveSenderEmails(data)
	case "resume":
		return db.UpsertResume(data)
	case "schedule":
		return db.UpsertSchedule(data)
	case "sequence_steps":
		return db.UpsertSequenceSteps(data)
	case "update":
		return db.UpsertUpdate(data)
	case "leads_replies":
		return db.UpsertLeadsReplies(data)
	case "sent_emails":
		return db.UpsertSentEmails(data)
	case "attach_email_to_reply":
		return db.UpsertAttachEmailToReply(data)
	case "followup_campaign":
		return db.UpsertFollowupCampaign(data)
	case "mark_as_interested":
		return db.UpsertMarkAsInterested(data)
	case "reply":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"campaigns-schedule-templates": "/api/campaigns/schedule/templates",
		"custom-variables":             "/api/custom-variables",
		"leads":                        "/api/leads",
		"replies":                      "/api/replies",
		"sender-emails":                "/api/sender-emails",
		"tags":                         "/api/tags",
		"users":                        "/api/users",
		"workspaces":                   "/api/workspaces/v1.1/workspaces",
	}
