...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "email-bison-pp-cli",
		Short: `Email Bison CLI — Every Email Bison campaign, lead, and reply on the command line, plus a local database and cross-entity analytics no ot…`,
		Long: `Email Bison CLI — Every Email Bison campaign, lead, and reply on the command line, plus a local database and cross-entity analytics no ot…
...
Health check: run 'email-bison-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("email-bison-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'email-bison-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCustomVariablesCmd(flags))
	rootCmd.AddCommand(newLeadsCmd(flags))
	senderEmailsCmd := newSenderEmailsCmd(flags)
	senderEmailsCmd.AddCommand(newNovelSendersHealthCmd(flags))
	rootCmd.AddCommand(senderEmailsCmd)
	...
	repliesPromotedCmd := newRepliesPromotedCmd(flags)
	repliesPromotedCmd.AddCommand(newNovelRepliesInterestedCmd(flags))
	repliesPromotedCmd.AddCommand(newNovelRepliesTriageCmd(flags))
	rootCmd.AddCommand(repliesPromotedCmd)
	rootCmd.AddCommand(newScheduledEmailsPromotedCmd(flags))
