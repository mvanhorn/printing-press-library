...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("launch_a_campaign",
			mcplib.WithDescription("Preview the launch call, then drop --dry-run to start sending once preflight is clean."),
			mcplib.WithBoolean("dry_run", mcplib.Description("Override the recipe's --dry-run value.")),
		),
		handleLaunchACampaign,
	)
}

// handleLaunchACampaign runs the launch_a_campaign recipe intent: Preview the launch call, then drop --dry-run to start sending once preflight is clean.
func handleLaunchACampaign(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
