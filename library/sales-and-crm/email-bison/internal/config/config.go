...
type Config struct {
	...
	Path             string            `toml:"-"`
	EmailBisonApiKey string            `toml:"bison_api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("EMAIL_BISON_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "email-bison-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("EMAIL_BISON_API_KEY"); v != "" {
		cfg.EmailBisonApiKey = v
		cfg.AuthSource = "env:EMAIL_BISON_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.EmailBisonApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("EMAIL_BISON_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.EmailBisonApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:EMAIL_BISON_API_KEY"
		}
		return "Bearer " + c.EmailBisonApiKey
	}
	if c.AccessToken != "" {
