# Email Bison Printed CLI Agent Guide

This directory is a generated `email-bison-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
email-bison-pp-cli doctor --json
email-bison-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
email-bison-pp-cli which "<capability>" --json
email-bison-pp-cli <command> --help
```
...
```bash
email-bison-pp-cli <command> --agent
```
...
```bash
email-bison-pp-cli <command> --help
email-bison-pp-cli <command> --dry-run --agent
```
