...
// TestUpsertBatch_PopulatesJobTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed job table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesJobTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "job")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count job: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("job count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
...
// dispatches paginated items into both the generic resources table AND the
// typed timeoff table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesTimeoffTable(t *testing.T) {
	...
	if _, _, err := s.UpsertBatch("timeoff", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "timeoff").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "timeoff")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count timeoff: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("timeoff count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
