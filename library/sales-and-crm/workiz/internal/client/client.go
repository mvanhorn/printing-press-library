...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "workiz-pp-cli", "http")
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	// Workiz requires auth_secret as a JSON body field on every write call,
	// not a header. Inject it here (the single shared request path for every
	// generated POST/PUT/PATCH/DELETE command) so users never have to pass
	// their secret as a per-command flag. Only applies to map-shaped bodies
	// (every generated write command builds one); a caller-supplied
	// auth_secret (e.g. via --stdin) is never overwritten.
	if bodyMap, ok := body.(map[string]any); ok && c.Config.WorkizApiSecret != "" {
		if _, exists := bodyMap["auth_secret"]; !exists {
			bodyMap["auth_secret"] = c.Config.WorkizApiSecret
		}
	}
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "workiz-pp-cli/0.1.0")
		}
	...
	if body != nil {
		...
		if json.Unmarshal(body, &pretty) == nil {
			...
			if enc.Encode(pretty) == nil {
				// Mask the same credential material as the URL/query/header
				// above. doInternal() injects Workiz's write-secret directly
				// into the request body (Workiz has no header/query mode for
				// it), so without this the raw secret would print here even
				// though every other line in this preview is masked.
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export WORKIZ_API_TOKEN=<your-token> or workiz-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.WorkizApiToken)
		addCredential(c.Config.WorkizApiSecret)
	}
