...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "workiz-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("WORKIZ_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("WORKIZ_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "workiz-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/workiz-pp-cli/feedback.jsonl.
When ` + "`WORKIZ_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`WORKIZ_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  workiz-pp-cli feedback list
  workiz-pp-cli feedback list --limit 5
  workiz-pp-cli feedback list --json`,
