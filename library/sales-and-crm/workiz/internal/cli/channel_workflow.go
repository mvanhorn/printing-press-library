...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  workiz-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  workiz-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"job", "lead", "team", "timeoff"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  workiz-pp-cli workflow status

  # Show status as JSON
  workiz-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("workiz-pp-cli")
			}
