...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  workiz-pp-cli doctor
  workiz-pp-cli doctor --json
  workiz-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				...
				report["base_url"] = cfg.DisplayBaseURL()
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export WORKIZ_API_TOKEN=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("WORKIZ_API_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "WORKIZ_API_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "WORKIZ_API_TOKEN")
			}
			if os.Getenv("WORKIZ_API_SECRET") != "" {
				authEnvSet = append(authEnvSet, "WORKIZ_API_SECRET")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "WORKIZ_API_SECRET")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "workiz-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'workiz-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'workiz-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'workiz-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'workiz-pp-cli sync' to refresh."
	}
	return report
}
