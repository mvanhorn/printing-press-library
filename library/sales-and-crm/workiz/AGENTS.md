# Workiz Printed CLI Agent Guide

This directory is a generated `workiz-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
workiz-pp-cli doctor --json
workiz-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
workiz-pp-cli which "<capability>" --json
workiz-pp-cli <command> --help
```
...
```bash
workiz-pp-cli <command> --help
workiz-pp-cli <command> --dry-run --agent
```
