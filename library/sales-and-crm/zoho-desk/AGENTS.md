# Zoho Desk Printed CLI Agent Guide

This directory is a generated `zoho-desk-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
zoho-desk-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
zoho-desk-pp-cli which "<capability>" --json
zoho-desk-pp-cli <command> --help
```
...
```bash
zoho-desk-pp-cli <command> --agent
```
...
```bash
zoho-desk-pp-cli <command> --help
zoho-desk-pp-cli <command> --dry-run --agent
```
