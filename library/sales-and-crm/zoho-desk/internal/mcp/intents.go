...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("rebalance_an_overloaded_queue",
			mcplib.WithDescription("See proposed moves from overloaded agents to idle ones before applying."),
			mcplib.WithBoolean("plan", mcplib.Description("Override the recipe's --plan value.")),
		),
		handleRebalanceAnOverloadedQueue,
	)
}

// handleRebalanceAnOverloadedQueue runs the rebalance_an_overloaded_queue recipe intent: See proposed moves from overloaded agents to idle ones before applying.
func handleRebalanceAnOverloadedQueue(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	args = appendRecipeBoolFlag(args, "plan", input["plan"], true)
