...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	return home, filepath.Join(home, ".config", "zoho-desk-pp-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "zoho-desk-pp-cli", "credentials.toml")
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "zoho-desk-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "zoho-desk-pp-cli", "credentials.toml")
