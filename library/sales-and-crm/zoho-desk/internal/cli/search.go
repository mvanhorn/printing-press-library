...
In local mode: searches locally synced data only.`,
  ...
  zoho-desk-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  zoho-desk-pp-cli search "status" --type accounts --data-source local
  ...
  zoho-desk-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /tickets/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/tickets/search", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			case "threads":
				results, err = db.SearchThreads(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchThreads(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search threads failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
