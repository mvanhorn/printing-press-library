// Copyright 2026 dan-bronson. Licensed under Apache-2.0. See LICENSE.
// Generated by CLI Printing Press (https://github.com/mvanhorn/cli-printing-press). DO NOT EDIT.

package cli

import (
	"github.com/spf13/cobra"
)

func newRolesCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:    "roles",
		Short:  "Manage roles",
		Hidden: true,
	}

	cmd.AddCommand(newRolesAddCmd(flags))
	cmd.AddCommand(newRolesGetCmd(flags))
	cmd.AddCommand(newRolesGetCountCmd(flags))
	cmd.AddCommand(newRolesGetRoleidCmd(flags))
	cmd.AddCommand(newRolesUpdateCmd(flags))
	cmd.AddCommand(newRolesAgentsCmd(flags))
	cmd.AddCommand(newRolesDeleteCmd(flags))
	cmd.AddCommand(newRolesTeamsCmd(flags))
	return cmd
}
