		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export ZOHODESK_REFRESH_TOKEN=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("ZOHODESK_REFRESH_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "ZOHODESK_REFRESH_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "ZOHODESK_REFRESH_TOKEN")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'zoho-desk-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
