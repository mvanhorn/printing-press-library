...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  zoho-desk-pp-cli doctor
  zoho-desk-pp-cli doctor --json
  zoho-desk-pp-cli doctor --fail-on warn
  zoho-desk-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				if cfg.RefreshToken == "" {
					...
					report["auth_hint"] = "export ZOHO_DESK_REFRESH_TOKEN=<your-oauth-refresh-value>"
			...
			if os.Getenv("ZOHO_DESK_CLIENT_ID") != "" {
				authEnvSet = append(authEnvSet, "ZOHO_DESK_CLIENT_ID")
			} else {
				authEnvInfo = append(authEnvInfo, "ZOHO_DESK_CLIENT_ID set during auth login")
			}
			if os.Getenv("ZOHO_DESK_CLIENT_SECRET") != "" {
				authEnvSet = append(authEnvSet, "ZOHO_DESK_CLIENT_SECRET")
			} else {
				authEnvInfo = append(authEnvInfo, "ZOHO_DESK_CLIENT_SECRET set during auth login")
			}
			if os.Getenv("ZOHO_DESK_REFRESH_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "ZOHO_DESK_REFRESH_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "ZOHO_DESK_REFRESH_TOKEN set during auth login")
			}
			switch {
			...
			// orgId is mandatory on every Zoho Desk call except the org-list
			// endpoints. Its absence is the #1 cause of 401 INVALID_OAUTH, so
			// surface it explicitly rather than letting every command fail.
			if cfg != nil {
				if strings.TrimSpace(cfg.ZohoDeskOrgId) == "" {
					report["org_id"] = "ERROR not set; export ZOHO_DESK_ORG_ID=<id> (find it with: zoho-desk-pp-cli organizations list)"
				} else {
					report["org_id"] = "OK " + cfg.ZohoDeskOrgId
				}
			}
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "zoho-desk-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'zoho-desk-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'zoho-desk-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'zoho-desk-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'zoho-desk-pp-cli sync' to refresh."
	}
	return report
}
