...
type Config struct {
	...
	ZohoDeskClientId     string          `toml:"desk_client_id"`
	ZohoDeskClientSecret string          `toml:"desk_client_secret"`
	ZohoDeskRefreshToken string          `toml:"desk_refresh_token"`
	// ZohoDeskOrgId is the portal/organization ID. Zoho Desk requires it as the
	// `orgId` header on every request except /organizations and
	// /accessibleOrganizations. Injected into Headers in Load so every client
	// (CLI, sync, doctor, MCP) sends it. Find yours with `organizations list`.
	ZohoDeskOrgId string `toml:"desk_org_id"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("ZOHO_DESK_CLIENT_ID"); v != "" {
		cfg.ZohoDeskClientId = v
		cfg.markEnvOverride("ZohoDeskClientId")
		cfg.AuthSource = "env:ZOHO_DESK_CLIENT_ID"
		cfg.CredentialSource = "env:ZOHO_DESK_CLIENT_ID"
	}
	if v := os.Getenv("ZOHO_DESK_CLIENT_SECRET"); v != "" {
		cfg.ZohoDeskClientSecret = v
		cfg.markEnvOverride("ZohoDeskClientSecret")
		cfg.AuthSource = "env:ZOHO_DESK_CLIENT_SECRET"
		cfg.CredentialSource = "env:ZOHO_DESK_CLIENT_SECRET"
	}
	if v := os.Getenv("ZOHO_DESK_REFRESH_TOKEN"); v != "" {
		cfg.ZohoDeskRefreshToken = v
		cfg.markEnvOverride("ZohoDeskRefreshToken")
		cfg.AuthSource = "env:ZOHO_DESK_REFRESH_TOKEN"
		cfg.CredentialSource = "env:ZOHO_DESK_REFRESH_TOKEN"
	}
	...
	if v := os.Getenv("ZOHO_DESK_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("ZOHO_DESK_TOKEN_URL"); v != "" {
		cfg.TokenURL = v
	}
	// The oauth2_refresh client (refreshAccessToken) reads the generic
	// ClientID/ClientSecret/RefreshToken fields, but the ZOHO_DESK_* env vars
	// and desk_* config keys land in the ZohoDesk* fields. Bridge them so both
	// env-var auth and desk_* config keys actually drive the token refresh.
	if cfg.ClientID == "" {
		cfg.ClientID = cfg.ZohoDeskClientId
	}
	if cfg.ClientSecret == "" {
		cfg.ClientSecret = cfg.ZohoDeskClientSecret
	}
	if cfg.RefreshToken == "" {
		cfg.RefreshToken = cfg.ZohoDeskRefreshToken
	}
	// orgId: env wins over config-file value. Zoho Desk requires the `orgId`
	// header on every call (except /organizations & /accessibleOrganizations),
	// so inject it into Headers here where every loaded config flows through.
	if v := os.Getenv("ZOHO_DESK_ORG_ID"); v != "" {
		cfg.ZohoDeskOrgId = v
	}
	if cfg.ZohoDeskOrgId != "" {
		if cfg.Headers == nil {
			cfg.Headers = map[string]string{}
		}
		cfg.Headers["orgId"] = cfg.ZohoDeskOrgId
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("ZOHO_DESK_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "zoho-desk-pp-cli", "config.toml"), nil
}
...
func (c *Config) hasCredentialFields() bool {
	...
	if c.ZohoDeskClientId != "" {
		return true
	}
	if c.ZohoDeskClientSecret != "" {
		return true
	}
	if c.ZohoDeskRefreshToken != "" {
		return true
	}
	return false
}
...
func (c *Config) clearCredentialFields() {
	...
	c.ClientSecret = ""
	c.ZohoDeskClientId = ""
	c.ZohoDeskClientSecret = ""
	c.ZohoDeskRefreshToken = ""
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		ZohoDeskClientId:     c.ZohoDeskClientId,
		ZohoDeskClientSecret: c.ZohoDeskClientSecret,
		ZohoDeskRefreshToken: c.ZohoDeskRefreshToken,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.ZohoDeskClientId = creds.ZohoDeskClientId
	c.ZohoDeskClientSecret = creds.ZohoDeskClientSecret
	c.ZohoDeskRefreshToken = creds.ZohoDeskRefreshToken
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.ZohoDeskClientId = ""
	delete(c.envOverrides, "ZohoDeskClientId")
	c.updateFileConfigField("ZohoDeskClientId")
	c.ZohoDeskClientSecret = ""
	delete(c.envOverrides, "ZohoDeskClientSecret")
	c.updateFileConfigField("ZohoDeskClientSecret")
	c.ZohoDeskRefreshToken = ""
	delete(c.envOverrides, "ZohoDeskRefreshToken")
	c.updateFileConfigField("ZohoDeskRefreshToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["ZohoDeskClientId"] {
			out.ZohoDeskClientId = c.fileConfig.ZohoDeskClientId
		}
		if c.envOverrides["ZohoDeskClientSecret"] {
			out.ZohoDeskClientSecret = c.fileConfig.ZohoDeskClientSecret
		}
		if c.envOverrides["ZohoDeskRefreshToken"] {
			out.ZohoDeskRefreshToken = c.fileConfig.ZohoDeskRefreshToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "ZohoDeskClientId":
		c.fileConfig.ZohoDeskClientId = c.ZohoDeskClientId
	case "ZohoDeskClientSecret":
		c.fileConfig.ZohoDeskClientSecret = c.ZohoDeskClientSecret
	case "ZohoDeskRefreshToken":
		c.fileConfig.ZohoDeskRefreshToken = c.ZohoDeskRefreshToken
	}
}
