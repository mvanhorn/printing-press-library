...
func classifyAPIError(err error) error {
	...
	case strings.Contains(msg, "HTTP 400") && looksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			"\n      Set your API key: export SALESFORCE_ACCESS_TOKEN=<your-key>"+
			"\n      Get a key at: https://help.salesforce.com/s/articleView?id=sf.connected_app_overview.htm"+
			"\n      Run 'salesforce-headless-360-pp-cli doctor' to check auth status."+
			"\n      Response: "+sanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: your token may have expired. Re-run 'salesforce-headless-360-pp-cli auth login' to re-authenticate"+
			"\n      or: export SALESFORCE_ACCESS_TOKEN=<your-token>"+
			"\n      Get a key at: https://help.salesforce.com/s/articleView?id=sf.connected_app_overview.htm"+
			"\n      Run 'salesforce-headless-360-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: permission denied. Your token may lack required scopes. Re-run 'salesforce-headless-360-pp-cli auth login' to re-authorize"+
			"\n      or: export SALESFORCE_ACCESS_TOKEN=<your-token>"+
			"\n      Get a key at: https://help.salesforce.com/s/articleView?id=sf.connected_app_overview.htm"+
			"\n      Run 'salesforce-headless-360-pp-cli doctor' to check auth status.", err))
