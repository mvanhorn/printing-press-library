...
func newSyncCmd(flags *rootFlags) *cobra.Command {
	...
	var allowBulkFLSUnsafe bool
	var includePII bool
	var includeShieldEncrypted bool
	var redactGroups []string
...
Once synced, use the 'search' command for instant full-text search.`,
  ...
  salesforce-headless-360-pp-cli sync

  # Sync specific resources only
  salesforce-headless-360-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  salesforce-headless-360-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  salesforce-headless-360-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  salesforce-headless-360-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  salesforce-headless-360-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := security.ValidateSensitiveOverride("--include-pii", includePII, flags.yes, stdinIsTTY()); err != nil {
				return err
			}
			if err := security.ValidateSensitiveOverride("--include-shield-encrypted", includeShieldEncrypted, flags.yes, stdinIsTTY()); err != nil {
				return err
			}
			...
			if accountID != "" {
				...
				if sinceTS != "" {
					parsed, err := time.Parse(time.RFC3339, sinceTS)
					if err != nil {
						return fmt.Errorf("invalid resolved --since timestamp %q: %w", sinceTS, err)
					}
					sinceTime = parsed
				} else if !full {
					_, lastSynced, _, _ := db.GetSyncState("accounts")
					sinceTime = lastSynced
				}
				prov := &security.Provenance{Redactions: map[string]int{}}
				secFilter := security.NewDefaultFilter(security.Options{
					Client:                 c,
					ComplianceStore:        db,
					UnknownFieldRecorder:   db,
					IncludePII:             includePII,
					IncludeShieldEncrypted: includeShieldEncrypted,
					RedactGroups:           redactGroups,
					OrgAlias:               ResolveOrgAlias(""),
				})
				total, err := crmsync.SyncAccount(c, db, accountID, sinceTime, crmsync.NewSecurityFilter(secFilter, prov), crmsync.NewGate())
				if err != nil {
					if crmsync.IsPartialSync(err) {
						return err
					}
					return err
				}
				if allowBulkFLSUnsafe && humanFriendly {
					fmt.Fprintln(os.Stderr, "warning: --allow-bulk-fls-unsafe was set but Bulk fallback was not needed")
				}
				fmt.Fprintf(os.Stderr, "Sync complete: %d records across account %s (%.1fs)\n",
					total, accountID, time.Since(started).Seconds())
				return nil
			}

			if full && !cmd.Flags().Changed("resources") {
				started := time.Now()
				gate := crmsync.NewGate()
				body, headers, err := c.GetWithResponseHeaders("/services/data/v63.0/query", map[string]string{
					"q": "SELECT Id FROM Account",
				})
				if err != nil {
					return fmt.Errorf("stage: sync.composite_graph.account_walk: %w", err)
				}
				if err := gate.UpdateFromHeaders(headers); err != nil {
					return err
				}
				if err := gate.Check(); err != nil {
					return &crmsync.PartialSyncError{Stage: "sync.composite_graph.account_walk", Err: err}
				}
				...
				for _, id := range accountIDs {
					...
					count, err := crmsync.SyncAccount(c, db, id, time.Time{}, crmsync.NewSecurityFilter(secFilter, prov), gate)
					if err != nil {
						return err
					}
					total += count
				}
				fmt.Fprintf(os.Stderr, "Sync complete: %d records across %d account(s) (%.1fs)\n",
					total, len(accountIDs), time.Since(started).Seconds())
				return nil
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/salesforce-headless-360-pp-cli/data.db)")
	cmd.Flags().IntVar(&maxPages, "max-pages", 10, "Maximum pages to fetch per resource (0 = unlimited)")
	cmd.Flags().BoolVar(&latestOnly, "latest-only", false, "Refresh head of each resource only; clears resume cursor and caps pages at 1. Mutually exclusive with --since (--since wins).")
	cmd.Flags().StringVar(&accountID, "account", "", "Account Id to sync through Salesforce Composite Graph")
	cmd.Flags().BoolVar(&allowBulkFLSUnsafe, "allow-bulk-fls-unsafe", false, "Allow Bulk API fallback before Unit D Apex FLS enforcement is installed")
	cmd.Flags().BoolVar(&includePII, "include-pii", false, "Include compliance-tagged PII/HIPAA/GLBA/PCI fields after explicit confirmation")
	cmd.Flags().BoolVar(&includeShieldEncrypted, "include-shield-encrypted", false, "Include Shield-encrypted fields after explicit confirmation")
	cmd.Flags().StringSliceVar(&redactGroups, "redact", nil, "Additional comma-separated compliance groups to redact")
...
func syncResourcePath(resource string) string {
	paths := map[string]string{
		"accounts":      "/services/data/v63.0/query",
		"cases":         "/services/data/v63.0/query",
		"contacts":      "/services/data/v63.0/query",
		"events":        "/services/data/v63.0/query",
		"opportunities": "/services/data/v63.0/query",
		"tasks":         "/services/data/v63.0/query",
	}
