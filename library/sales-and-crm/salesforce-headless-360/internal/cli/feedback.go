...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SALESFORCE_HEADLESS_360_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SALESFORCE_HEADLESS_360_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/sales-and-crm/salesforce-headless-360/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.salesforce-headless-360-pp-cli/feedback.jsonl.
When ` + "`SALESFORCE_HEADLESS_360_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SALESFORCE_HEADLESS_360_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
  ...
  salesforce-headless-360-pp-cli feedback "agent context needs a --since flag example"

  # Feedback from stdin (useful in pipes)
  echo "decay scoring was surprising for a quiet account" | salesforce-headless-360-pp-cli feedback --stdin

  # List prior feedback entries
  salesforce-headless-360-pp-cli feedback list

  # Opt in to upstream delivery for one entry
  salesforce-headless-360-pp-cli feedback "helpful hint text" --send`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "salesforce-headless-360-pp-cli",
				Version:   version,
