...
func newWorkflowCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: `  # Archive all syncable resources to the local store
  salesforce-headless-360-pp-cli workflow archive

  # Show archive/sync state for every resource
  salesforce-headless-360-pp-cli workflow status`,
	}
...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  salesforce-headless-360-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"accounts", "cases", "contacts", "events", "opportunities", "tasks"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  # Show status as JSON
  salesforce-headless-360-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("salesforce-headless-360-pp-cli")
			}
