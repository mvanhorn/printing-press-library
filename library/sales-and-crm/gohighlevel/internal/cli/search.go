...
In local mode: searches locally synced data only.`,
  ...
  gohighlevel-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /links/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get("/links/search", map[string]string{
					"query": query,
				})
				if getErr == nil {
			...
			case "payments":
				results, err = db.SearchPayments(query, limit)
			case "products":
				results, err = db.SearchProducts(query, limit)
			case "saas_api":
				results, err = db.SearchSaasApi(query, limit)
			case "store":
				results, err = db.SearchStore(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchPayments(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search payments failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchSaasApi(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search saas_api failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchStore(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search store failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
