# Synology Dsm Printed CLI Agent Guide

This directory is a generated `synology-nas-api-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
synology-nas-api-pp-cli doctor --json
synology-nas-api-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
synology-nas-api-pp-cli which "<capability>" --json
synology-nas-api-pp-cli <command> --help
```
...
```bash
synology-nas-api-pp-cli <command> --agent
```
...
```bash
synology-nas-api-pp-cli <command> --help
synology-nas-api-pp-cli <command> --dry-run --agent
```
