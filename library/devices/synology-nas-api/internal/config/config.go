...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("SYNOLOGY_NAS_API_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "synology-nas-api-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("SYNOLOGY_NAS_API_SIDCOOKIE"); v != "" {
		cfg.SynologyDsmSidcookie = v
		cfg.AuthSource = "env:SYNOLOGY_NAS_API_SIDCOOKIE"
	}
	if v := os.Getenv("SYNOLOGY_NAS_API_SYNOTOKEN"); v != "" {
	...
	if cfg.AuthSource == "" && cfg.SynologyDsmSidcookie != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("SYNOLOGY_NAS_API_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("SYNOLOGY_NAS_API_HOST")); v != "" {
		cfg.TemplateVars["host"] = normalizeEndpointTemplateValue(v)
	} else if os.Getenv("PRINTING_PRESS_VERIFY") == "1" {
		cfg.TemplateVars["host"] = "nas-host-placeholder"
	}
	if v := strings.TrimSpace(os.Getenv("SYNOLOGY_NAS_API_PORT")); v != "" {
		cfg.TemplateVars["port"] = normalizeEndpointTemplateValue(v)
	} else {
		cfg.TemplateVars["port"] = "5000"
	}
	return cfg, nil
}
