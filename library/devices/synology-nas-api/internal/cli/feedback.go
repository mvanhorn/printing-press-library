...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SYNOLOGY_NAS_API_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SYNOLOGY_NAS_API_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "synology-nas-api-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.synology-nas-api-pp-cli/feedback.jsonl.
When ` + "`SYNOLOGY_NAS_API_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SYNOLOGY_NAS_API_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  synology-nas-api-pp-cli feedback list
  synology-nas-api-pp-cli feedback list --limit 5
  synology-nas-api-pp-cli feedback list --json`,
	...
	cmd.Flags().StringVar(&topic, "topic", "", "Filter entries to a specific feedback topic or category")
