...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  synology-nas-api-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  synology-nas-api-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"webapi", "webapi-entry-cgi-auth-logout", "webapi-entry-cgi-filestation-info", "webapi-entry-cgi-system-utilization"}
			if scope != "" {
	...
	cmd.Flags().StringVar(&scope, "scope", "", "Resource scope filter (e.g. webapi, webapi-entry-cgi-system-utilization)")
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  synology-nas-api-pp-cli workflow status

  # Show status as JSON
  synology-nas-api-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("synology-nas-api-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Path to local SQLite database (default: ~/.local/share/synology-nas-api-pp-cli/data.db)")
