...
func profileStorePath() (string, error) {
	...
	dir := filepath.Join(home, ".synology-nas-api-pp-cli")
...
present (other than --profile and --config).`,
		Example: `  synology-nas-api-pp-cli profile save my-defaults --json --compact
  synology-nas-api-pp-cli profile save tonight-defaults --region US`,
	...
	cmd.Flags().StringVar(&description, "description", "", "Short description for this profile shown when running 'profile list'")
...
func newProfileUseCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  synology-nas-api-pp-cli profile use my-defaults
  synology-nas-api-pp-cli profile use tonight-defaults --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newProfileListCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  synology-nas-api-pp-cli profile list
...
func newProfileShowCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  synology-nas-api-pp-cli profile show my-defaults
  synology-nas-api-pp-cli profile show tonight-defaults --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			p, err := GetProfile(args[0])
			if err != nil {
...
func newProfileDeleteCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  synology-nas-api-pp-cli profile delete my-defaults --yes
  synology-nas-api-pp-cli profile delete old-profile --yes --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			name := args[0]
			s, err := loadProfileStore()
