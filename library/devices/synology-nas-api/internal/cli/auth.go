...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Synology Dsm",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  synology-nas-api-pp-cli auth setup\n  synology-nas-api-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "See API docs: https://github.com/synology-community/go-synology")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export SYNOLOGY_NAS_API_SIDCOOKIE=\"<your-token>\"")
			fmt.Fprintln(w, "  synology-nas-api-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://github.com/synology-community/go-synology"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  synology-nas-api-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export SYNOLOGY_NAS_API_SIDCOOKIE=\"your-token-here\"")
				fmt.Fprintf(w, "  synology-nas-api-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  synology-nas-api-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  synology-nas-api-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("SYNOLOGY_NAS_API_SIDCOOKIE") != "" {
				envStillSet = "SYNOLOGY_NAS_API_SIDCOOKIE"
			}
