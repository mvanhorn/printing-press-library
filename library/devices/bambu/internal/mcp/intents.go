...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("explain_the_latest_print",
			mcplib.WithDescription("Reconstruct stages, pauses, layer progress, temperature recovery, and errors."),
			mcplib.WithBoolean("latest", mcplib.Description("Override the recipe's --latest value.")),
		),
		handleExplainTheLatestPrint,
	)
}

// handleExplainTheLatestPrint runs the explain_the_latest_print recipe intent: Reconstruct stages, pauses, layer progress, temperature recovery, and errors.
func handleExplainTheLatestPrint(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	args = appendRecipeBoolFlag(args, "latest", input["latest"], true)
