...
func TestMCPPathResolutionMatchesCLIResolverWithPlatformDefaults(t *testing.T) {
	...
	if want := filepath.Join(home, ".config", "bambu-pp-cli", "config.toml"); cfg.Path != want {
	...
	if want := filepath.Join(home, ".local", "share", "bambu-pp-cli", "data.db"); gotDB != want {
...
func TestMCPRegisterToolsPreservesTypedSpecialTools(t *testing.T) {
	s := server.NewMCPServer("bambu", "test")
	RegisterTools(s)
...
func TestMCPSearchRejectsUnboundedInputsBeforeOpeningStore(t *testing.T) {
	resetMCPPathEnv(t)
	tests := []map[string]any{
		{"query": strings.Repeat("x", 4097)},
		{"query": "alpha", "limit": float64(0)},
		{"query": "alpha", "limit": float64(1001)},
		{"query": "alpha", "limit": 1.5},
		{"query": "alpha", "limit": math.Inf(1)},
	}
	for _, arguments := range tests {
		result, err := handleSearch(context.Background(), mcplib.CallToolRequest{Params: mcplib.CallToolParams{Arguments: arguments}})
		if err != nil {
			t.Fatalf("handleSearch returned transport error: %v", err)
		}
		if result == nil || !result.IsError {
			t.Fatalf("arguments %#v returned IsError=%v, want true", arguments, result != nil && result.IsError)
		}
	}
}
...
func TestMCPLocalStoreMetaIncludesOldestSyncedAt(t *testing.T) {
	...
	if _, err := db.DB().Exec(`INSERT INTO sync_state(resource_type, last_synced_at, total_count) VALUES (?, ?, ?)`, "observations", syncedAt, 0); err != nil {
