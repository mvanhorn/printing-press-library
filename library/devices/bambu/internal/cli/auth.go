...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Bambu Local",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  bambu-pp-cli auth setup\n  bambu-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "See API docs: https://printingpress.dev")
			fmt.Fprintln(w, "  Read the serial and access code from the printer's network settings. This CLI supports normal local access only; Developer Mode, LAN Only Mode, and Bambu Cloud are not used or supported.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export BAMBU_SERIAL=\"your-token-here\"")
			fmt.Fprintln(w, "  export BAMBU_ACCESS_CODE=\"your-token-here\"")
			if !launch {
				return nil
			}
			launchURL := "https://printingpress.dev"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  bambu-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export BAMBU_SERIAL=\"your-token-here\" # Bambu printer serial used for discovery filtering, MQTT topics, and certificate identity checks.")
				fmt.Fprintln(w, "  export BAMBU_ACCESS_CODE=\"your-token-here\" # Local LAN access code used to authenticate directly to the printer.")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  bambu-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  bambu-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("BAMBU_SERIAL") != "" {
				envStillSet = "BAMBU_SERIAL"
			}
			if envStillSet == "" && os.Getenv("BAMBU_ACCESS_CODE") != "" {
				envStillSet = "BAMBU_ACCESS_CODE"
			}
