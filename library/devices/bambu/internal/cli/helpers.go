...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'bambu-pp-cli doctor' to check auth status."+
	...
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API key."+
			...
			"\n      See API docs: https://printingpress.dev"+
			"\n      Read the serial and access code from the printer's network settings. This CLI supports normal local access only; Developer Mode, LAN Only Mode, and Bambu Cloud are not used or supported."+
			"\n      Run 'bambu-pp-cli doctor' to check auth status.", err))
...
func printOutputWithFlags(w io.Writer, data json.RawMessage, flags *rootFlags) error {
	source := "local"
	if flags != nil && flags.outputSource != "" {
		source = flags.outputSource
	}
	return printOutputWithFlagsMeta(w, data, flags, map[string]any{"source": source})
}
...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Set credentials with: export BAMBU_SERIAL=\"your-token-here\" BAMBU_ACCESS_CODE=\"your-token-here\""))
	}
	if len(trimmed) == 0 {
