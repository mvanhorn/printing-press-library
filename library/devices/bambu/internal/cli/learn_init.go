...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^[A-Z0-9]{15}$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: bambu-pp-cli: ticker pattern %q failed to compile: %v\n", `^[A-Z0-9]{15}$`, err)
	} else {
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"print_state": {
			{Canonical: "RUNNING", Values: []string{"running", "printing", "in progress"}},
			{Canonical: "PAUSE", Values: []string{"pause", "paused", "on hold"}},
			{Canonical: "FINISH", Values: []string{"finish", "finished", "complete", "completed"}},
			{Canonical: "FAILED", Values: []string{"failed", "error"}},
			{Canonical: "IDLE", Values: []string{"idle", "ready"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: bambu-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: bambu-pp-cli: learn init: %v\n", err)
		}
	})
}
