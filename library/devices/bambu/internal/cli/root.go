...
type rootFlags struct {
	...
	// invocation. Mirrors the BAMBU_NO_LEARN env var.
	noLearn       bool
	selectFields  string
	configPath    string
	homePath      string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "bambu-pp-cli",
		Short: `Agent-ready Bambu print monitoring with exact plate previews and start/finish payloads.`,
		Long: `Monitor Bambu prints over the local LAN and emit automation-ready start and finish payloads with exact plate metadata and previews.
...
Health check: run 'bambu-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("bambu-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.dataSource, "data-source", "auto", "Data-source request: auto, live, or local; Bambu commands reject unsupported modes")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'bambu-pp-cli profile list')")
	rootCmd.PersistentFlags().StringVar(&flags.printerName, "printer", "", "Select a named Bambu printer profile (see 'profile printer-list')")
	...
	rootCmd.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
		...
		if err := validateBambuDataSource(cmd, flags); err != nil {
			return err
		}
		if flags.agent {
	...
	rootCmd.AddCommand(newNovelAmsCmd(flags))
	rootCmd.AddCommand(newNovelHistoryCmd(flags))
	rootCmd.AddCommand(newNovelJobCmd(flags))
	rootCmd.AddCommand(newNovelPrinterCmd(flags))
	rootCmd.AddCommand(newBambuTopLevelCommands(flags)...)
	rootCmd.AddCommand(newBambuSQLCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newObservationsPromotedCmd(flags))
...
func (f *rootFlags) newClient() (*client.Client, error) {
	...
	c := client.New(cfg.WithoutHTTPAuth(), f.timeout, f.rateLimit)
	c.DryRun = f.dryRun
	c.NoCache = f.noCache
	return c, nil
}
