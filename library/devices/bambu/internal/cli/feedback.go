...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BAMBU_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BAMBU_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "bambu-pp-cli/feedback")
...
When ` + "`BAMBU_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BAMBU_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  bambu-pp-cli feedback list
  bambu-pp-cli feedback list --limit 5
  bambu-pp-cli feedback list --json`,
