...
type Config struct {
	...
	BambuSerial      string          `toml:"serial"`
	BambuAccessCode  string          `toml:"access_code"`
}
...
func Load(configPath string) (*Config, error) {
	...
	if v := os.Getenv("BAMBU_SERIAL"); v != "" {
		cfg.BambuSerial = v
		cfg.markEnvOverride("BambuSerial")
		cfg.AuthSource = "env:BAMBU_SERIAL"
		cfg.CredentialSource = "env:BAMBU_SERIAL"
	}
	if v := os.Getenv("BAMBU_ACCESS_CODE"); v != "" {
		cfg.BambuAccessCode = v
		cfg.markEnvOverride("BambuAccessCode")
		cfg.AuthSource = "env:BAMBU_ACCESS_CODE"
		cfg.CredentialSource = "env:BAMBU_ACCESS_CODE"
	}
	...
	if v := os.Getenv("BAMBU_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("BAMBU_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "bambu-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.BambuSerial != "" {
		return applyAuthFormat("{token}", map[string]string{
			"serial":       c.BambuSerial,
			"BAMBU_SERIAL": c.BambuSerial,
			"token":        c.BambuSerial,
		})
	}
	if c.BambuAccessCode != "" {
		return applyAuthFormat("{token}", map[string]string{
			"access_code":       c.BambuAccessCode,
			"BAMBU_ACCESS_CODE": c.BambuAccessCode,
			"token":             c.BambuAccessCode,
		})
	}
	return ""
}

func (c *Config) WithoutHTTPAuth() *Config {
	if c == nil {
		return nil
	}
	clone := *c
	clone.AuthHeaderVal = ""
	clone.BambuSerial = ""
	clone.BambuAccessCode = ""
	clone.Headers = make(map[string]string, len(c.Headers))
	for key, value := range c.Headers {
		if !strings.EqualFold(key, "X-Bambu-Access-Code") {
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		BambuSerial:     c.BambuSerial,
		BambuAccessCode: c.BambuAccessCode,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.BambuSerial = creds.BambuSerial
	c.BambuAccessCode = creds.BambuAccessCode
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.BambuSerial = token
	delete(c.envOverrides, "BambuSerial")
	c.updateFileConfigField("BambuSerial")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("BambuSerial")
	c.BambuAccessCode = ""
	delete(c.envOverrides, "BambuAccessCode")
	c.updateFileConfigField("BambuAccessCode")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["BambuSerial"] {
			out.BambuSerial = c.fileConfig.BambuSerial
		}
		if c.envOverrides["BambuAccessCode"] {
			out.BambuAccessCode = c.fileConfig.BambuAccessCode
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "BambuSerial":
		c.fileConfig.BambuSerial = c.BambuSerial
	case "BambuAccessCode":
		c.fileConfig.BambuAccessCode = c.BambuAccessCode
	}
}
