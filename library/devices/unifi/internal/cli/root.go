...
type rootFlags struct {
	...
	// invocation. Mirrors the UNIFI_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "unifi-pp-cli",
		Short: `Unifi CLI — Every UniFi Network API operation, plus drift detection, topology, and rule prediction no other UniFi tool has.`,
		Long: `Unifi CLI — Every UniFi Network API operation, plus drift detection, topology, and rule prediction no other UniFi tool has.
...
Health check: run 'unifi-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("unifi-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'unifi-pp-cli profile list')")
	...
	rootCmd.AddCommand(newPendingDevicesPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelGuestCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelNewcomerCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelPortAuditCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelRulePredictCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelTopologyCmd(flags))
