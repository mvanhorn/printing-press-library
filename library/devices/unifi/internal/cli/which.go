...
var whichIndex = []whichEntry{
	{Command: "topology", Description: "See the physical device tree (gateway to switches to APs) built entirely from local mirror data, no live crawl needed.", Group: "Local state that compounds", WhyItMatters: "Reach for this when an agent needs to understand physical network layout without walking every device endpoint individually."},
	{Command: "drift", Description: "Show what changed in site config (networks, firewall, wifi, DNS) since the last sync snapshot.", Group: "Local state that compounds", WhyItMatters: "Use after a suspected config change to see exactly what moved, without manually diffing the controller UI."},
	{Command: "newcomer", Description: "List devices and clients first seen since a given sync, for spotting new hardware joining the network.", Group: "Local state that compounds", WhyItMatters: "Use for periodic security review of what joined the network recently."},
	{Command: "port-audit", Description: "Review port utilization and PoE status across every switch on a site in one table.", Group: "Agent-native plumbing", WhyItMatters: "Use before adding new PoE devices to check headroom, or to find unused ports across a stack."},
	{Command: "guest report", Description: "Summarize guest network usage: active vouchers and connected guest clients, from local data.", Group: "Agent-native plumbing", WhyItMatters: "Use for a quick guest-network health check without cross-referencing three separate UI screens."},
	{Command: "rule-predict", Description: "Predict which firewall policy would match a hypothetical packet before making a live change.", Group: "Agent-native plumbing", WhyItMatters: "Use to check the effect of a proposed firewall change before applying it live."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  unifi-pp-cli which "stale tickets"
  unifi-pp-cli which "bottleneck"
  unifi-pp-cli which --limit 1 "send message"
  unifi-pp-cli which                                # list the full capability index`,
