...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("UNIFI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("UNIFI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "unifi-pp-cli/feedback")
...
When ` + "`UNIFI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`UNIFI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  unifi-pp-cli feedback list
  unifi-pp-cli feedback list --limit 5
  unifi-pp-cli feedback list --json`,
