...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  unifi-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  unifi-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"countries", "dpi", "dpi-applications", "pending-devices", "sites"}
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  unifi-pp-cli workflow status

  # Show status as JSON
  unifi-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("unifi-pp-cli")
			}
