...
Run sync first to populate the local search index.`,
  ...
  unifi-pp-cli search "status"

  # Force local search explicitly
  unifi-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  unifi-pp-cli search "status" --type countries --data-source local
  # JSON output for piping
  unifi-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'unifi-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
