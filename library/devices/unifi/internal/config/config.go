...
type Config struct {
	...
	UnifiApiKey      string          `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	if v := os.Getenv("UNIFI_API_KEY"); v != "" {
		cfg.UnifiApiKey = v
		cfg.markEnvOverride("UnifiApiKey")
		cfg.AuthSource = "env:UNIFI_API_KEY"
		cfg.CredentialSource = "env:UNIFI_API_KEY"
	}
	...
	if v := os.Getenv("UNIFI_BASE_URL"); v != "" {
		...
		// The integration spec declares its server as a bare relative path
		// ("/integration"), so there is no absolute base URL to inherit. The
		// real API lives on the gateway itself behind the UniFi OS reverse
		// proxy at /proxy/network. UNIFI_GATEWAY_HOST is the value this CLI's
		// setup docs ask users to set (e.g. "10.0.0.1"); construct the full
		// base URL from it so users don't have to know the /proxy/network
		// prefix exists.
		if host := os.Getenv("UNIFI_GATEWAY_HOST"); host != "" {
			cfg.BaseURL = "https://" + host + "/proxy/network"
		}
	}
	if v := os.Getenv("UNIFI_BASE_PATH"); v != "" {
		cfg.BasePath = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("UNIFI_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "unifi-pp-cli", "config.toml"), nil
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.UnifiApiKey == "" {
		c.UnifiApiKey = creds.UnifiApiKey
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.UnifiApiKey = token
	delete(c.envOverrides, "UnifiApiKey")
	c.updateFileConfigField("UnifiApiKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["UnifiApiKey"] {
			out.UnifiApiKey = c.fileConfig.UnifiApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "UnifiApiKey":
		c.fileConfig.UnifiApiKey = c.UnifiApiKey
	}
}
...
// skipped for this config's BaseURL. Self-hosted UniFi OS gateways serve a
// self-signed certificate by default, so every existing UniFi API tool ships
// a manual --insecure-style opt-out. This CLI auto-skips verification for
// private/loopback/link-local hosts (the gateway is always reached on the
// user's own LAN) so the common case works with zero configuration, while
// UNIFI_INSECURE_SKIP_VERIFY lets the user override the decision explicitly
// in either direction (e.g. force it on for a gateway reachable only via a
// public hostname, or force it off to require a real certificate).
//
// PATCH: this method, its BaseURL-host detection, and the transport wiring
// in internal/client/client.go are a hand patch, not generator output — see
// .printing-press-patches/tls-skip-verify-for-local-gateway.md.
func (c *Config) InsecureSkipVerify() bool {
	if v := os.Getenv("UNIFI_INSECURE_SKIP_VERIFY"); v != "" {
	...
	return isPrivateNetworkHost(c.BaseURL)
}

// isPrivateNetworkHost reports whether rawURL's host resolves to a private,
// loopback, or link-local address (RFC 1918 / RFC 4193 / RFC 3927 / ::1),
// or is the literal hostname "localhost". Any parse or lookup failure is
// treated as "not private" — the safe default is to require a valid
// certificate unless we can positively confirm a LAN-local target.
func isPrivateNetworkHost(rawURL string) bool {
	if rawURL == "" {
		return false
	}
	...
	ip := net.ParseIP(host)
	if ip == nil {
		// Not a literal IP (a DNS name) — resolving it here would add a
		// network round-trip to every config load and could itself be
		// spoofed by DNS. Treat unresolved hostnames as not private.
		return false
	}
