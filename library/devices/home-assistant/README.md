...
The recommended path installs both the `home-assistant-pp-cli` binary and the `pp-home-assistant` agent skill (Claude Code, Codex, Cursor, Gemini CLI, GitHub Copilot, and other agents supported by the upstream [`skills`](https://github.com/vercel-labs/skills) CLI) in one shot:

```bash
npx -y @mvanhorn/printing-press-library install home-assistant
```
...
```bash
npx -y @mvanhorn/printing-press-library install home-assistant --skill-only
```

To constrain the skill install to one or more specific agents (repeatable — agent names match the [`skills`](https://github.com/vercel-labs/skills) CLI):

```bash
npx -y @mvanhorn/printing-press-library install home-assistant --agent claude-code
npx -y @mvanhorn/printing-press-library install home-assistant --agent claude-code --agent codex
```
...
Download a pre-built binary for your platform from the [latest release](https://github.com/mvanhorn/printing-press-library/releases/tag/home-assistant-current). On macOS, clear the Gatekeeper quarantine: `xattr -d com.apple.quarantine <binary>`. On Unix, mark it executable: `chmod +x <binary>`.
...
```bash
hermes skills install mvanhorn/printing-press-library/cli-skills/pp-home-assistant --force
```

Inside a Hermes chat session:

```bash
/skills install mvanhorn/printing-press-library/cli-skills/pp-home-assistant --force
```
...
```bash
npx -y @mvanhorn/printing-press-library install home-assistant --agent openclaw
```
...
1. Download the `.mcpb` for your platform from the [latest release](https://github.com/mvanhorn/printing-press-library/releases/tag/home-assistant-current).
2. Double-click the `.mcpb` file. Claude Desktop opens and walks you through the install.
3. Fill in `HASS_TOKEN` when Claude Desktop prompts you.
...
1. Per-kind env var: `HOME_ASSISTANT_CONFIG_DIR`, `HOME_ASSISTANT_DATA_DIR`, `HOME_ASSISTANT_STATE_DIR`, or `HOME_ASSISTANT_CACHE_DIR`
2. `--home <dir>` for this invocation
3. `HOME_ASSISTANT_HOME` for a flat relocated root
...
- Run `home-assistant-pp-cli doctor` to check credentials
- Verify the environment variable is set: `echo $HASS_TOKEN`
...
- [**homeassistant-ai/ha-mcp**](https://github.com/homeassistant-ai/ha-mcp) — Python (4019 stars)
- [**home-assistant-ecosystem/home-assistant-cli**](https://github.com/home-assistant-ecosystem/home-assistant-cli) — Python (575 stars)
