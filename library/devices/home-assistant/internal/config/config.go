...
type Config struct {
	...
	HassToken               string          `toml:"token"`
	HomeAssistantBearerAuth string          `toml:"assistant_bearer_auth"`
...
func Load(configPath string) (*Config, error) {
	...
	if v := os.Getenv("HASS_TOKEN"); v != "" {
		cfg.HassToken = v
		cfg.markEnvOverride("HassToken")
		cfg.AuthSource = "env:HASS_TOKEN"
		cfg.CredentialSource = "env:HASS_TOKEN"
	}
	if v := os.Getenv("HOME_ASSISTANT_BEARER_AUTH"); v != "" {
		cfg.HomeAssistantBearerAuth = v
		cfg.markEnvOverride("HomeAssistantBearerAuth")
		cfg.AuthSource = "env:HOME_ASSISTANT_BEARER_AUTH"
		cfg.CredentialSource = "env:HOME_ASSISTANT_BEARER_AUTH"
	}
	...
	if v := os.Getenv("HOME_ASSISTANT_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("HOME_ASSISTANT_BASE_PATH"); v != "" {
		cfg.BasePath = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("HOME_ASSISTANT_SERVER")); v != "" {
		cfg.TemplateVars["server"] = normalizeEndpointTemplateValue(v)
	} else {
		cfg.TemplateVars["server"] = "http://homeassistant.local:8123"
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("HOME_ASSISTANT_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "home-assistant-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.HassToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:HASS_TOKEN"
		}
		return "Bearer " + c.HassToken
	}
	if c.HomeAssistantBearerAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:HOME_ASSISTANT_BEARER_AUTH"
		}
		return "Bearer " + c.HomeAssistantBearerAuth
	}
	if c.AccessToken != "" {
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		HassToken:               c.HassToken,
		HomeAssistantBearerAuth: c.HomeAssistantBearerAuth,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.HassToken = creds.HassToken
	c.HomeAssistantBearerAuth = creds.HomeAssistantBearerAuth
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.HassToken = ""
	delete(c.envOverrides, "HassToken")
	c.updateFileConfigField("HassToken")
	c.HomeAssistantBearerAuth = ""
	delete(c.envOverrides, "HomeAssistantBearerAuth")
	c.updateFileConfigField("HomeAssistantBearerAuth")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["HassToken"] {
			out.HassToken = c.fileConfig.HassToken
		}
		if c.envOverrides["HomeAssistantBearerAuth"] {
			out.HomeAssistantBearerAuth = c.fileConfig.HomeAssistantBearerAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "HassToken":
		c.fileConfig.HassToken = c.HassToken
	case "HomeAssistantBearerAuth":
		c.fileConfig.HomeAssistantBearerAuth = c.HomeAssistantBearerAuth
	}
}
