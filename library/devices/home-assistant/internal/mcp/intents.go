...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("preview_movie_night",
			mcplib.WithDescription("Resolve the household mode and inspect every action without changing any device."),
		...
		handlePreviewMovieNight,
	)
	s.AddTool(
		mcplib.NewTool("explain_a_failed_routine",
			mcplib.WithDescription("Join the relevant trace, state, logbook, and error evidence into one incident timeline."),
			mcplib.WithString("domain", mcplib.Required(), mcplib.Description("Override the recipe's positional domain value.")),
		),
		handleExplainAFailedRoutine,
	)
}

// handlePreviewMovieNight runs the preview_movie_night recipe intent: Resolve the household mode and inspect every action without changing any device.
func handlePreviewMovieNight(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
...
// handleExplainAFailedRoutine runs the explain_a_failed_routine recipe intent: Join the relevant trace, state, logbook, and error evidence into one incident timeline.
func handleExplainAFailedRoutine(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	if recipeCLIPathErr != nil {
		return mcplib.NewToolResultError(fmt.Sprintf("companion CLI binary not found: %v", recipeCLIPathErr)), nil
	}
