...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  home-assistant-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  home-assistant-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"calendars", "components", "services", "states"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  home-assistant-pp-cli workflow status

  # Show status as JSON
  home-assistant-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("home-assistant-pp-cli")
			}
