...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  home-assistant-pp-cli doctor
  home-assistant-pp-cli doctor --json
  home-assistant-pp-cli doctor --fail-on warn
  home-assistant-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set one of: export HASS_TOKEN=\"your-token-here\" or export HOME_ASSISTANT_BEARER_AUTH=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "HASS_TOKEN")
				if os.Getenv("HASS_TOKEN") != "" {
					authEnvSet = append(authEnvSet, "HASS_TOKEN")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("HASS_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "HASS_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "HASS_TOKEN optional")
			}
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "HOME_ASSISTANT_BEARER_AUTH")
				if os.Getenv("HOME_ASSISTANT_BEARER_AUTH") != "" {
					authEnvSet = append(authEnvSet, "HOME_ASSISTANT_BEARER_AUTH")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("HOME_ASSISTANT_BEARER_AUTH") != "" {
				authEnvSet = append(authEnvSet, "HOME_ASSISTANT_BEARER_AUTH")
			} else {
				authEnvInfo = append(authEnvInfo, "HOME_ASSISTANT_BEARER_AUTH optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "home-assistant-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'home-assistant-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'home-assistant-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'home-assistant-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'home-assistant-pp-cli sync' to refresh."
	}
	return report
}
