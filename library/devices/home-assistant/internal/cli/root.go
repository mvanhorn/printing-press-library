...
type rootFlags struct {
	...
	// invocation. Mirrors the HOME_ASSISTANT_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "home-assistant-pp-cli",
		Short: `Home Assistant CLI — Safe household actions, offline memory, and evidence-backed answers on top of the full Home Assistant control surface.`,
		Long: `Home Assistant CLI — Safe household actions, offline memory, and evidence-backed answers on top of the full Home Assistant control surface.
...
Health check: run 'home-assistant-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("home-assistant-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'home-assistant-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelEnergyCmd(flags))
	rootCmd.AddCommand(newNovelHealthCmd(flags))
	rootCmd.AddCommand(newNovelHouseCmd(flags))
	rootCmd.AddCommand(newNovelModeCmd(flags))
	rootCmd.AddCommand(newNovelRoutineCmd(flags))
