...
Run sync first to populate the local search index.`,
  ...
  home-assistant-pp-cli search "status"

  # Force local search explicitly
  home-assistant-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  home-assistant-pp-cli search "status" --type calendars --data-source local
  # JSON output for piping
  home-assistant-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'home-assistant-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
