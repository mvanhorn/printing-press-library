...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("HOME_ASSISTANT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HOME_ASSISTANT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "home-assistant-pp-cli/feedback")
...
When ` + "`HOME_ASSISTANT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`HOME_ASSISTANT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  home-assistant-pp-cli feedback list
  home-assistant-pp-cli feedback list --limit 5
  home-assistant-pp-cli feedback list --json`,
