...
// home-assistant-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/devices/home-assistant/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/devices/home-assistant/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/devices/home-assistant/internal/learn/patterns"
...
const noLearnEnvVar = "HOME_ASSISTANT_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("home-assistant-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  home-assistant-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  home-assistant-pp-cli recall "<question>" --agent
  home-assistant-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "list",
		Short: "Inspect recorded query-to-command learnings and confidence warnings",
		Example: `  home-assistant-pp-cli learnings list --agent
  home-assistant-pp-cli learnings list --query "<substring>"
  home-assistant-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  home-assistant-pp-cli learnings forget "<question>" --resource <id>
  home-assistant-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  home-assistant-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  home-assistant-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
