...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Home Assistant",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  home-assistant-pp-cli auth setup\n  home-assistant-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "No setup URL is configured for this CLI; check the API's docs.")
			fmt.Fprintln(w, "  home-assistant-pp-cli auth set-token <token>")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Set one of:")
			fmt.Fprintln(w, "  export HASS_TOKEN=\"your-token-here\"")
			fmt.Fprintln(w, "  export HOME_ASSISTANT_BEARER_AUTH=\"your-token-here\"")
			if !launch {
				return nil
			}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  home-assistant-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "")
				fmt.Fprintln(w, "Set one of:")
				fmt.Fprintf(w, "  home-assistant-pp-cli auth set-token <token>\n")
				fmt.Fprintln(w, "")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  home-assistant-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  home-assistant-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("HASS_TOKEN") != "" {
				envStillSet = "HASS_TOKEN"
			}
			if envStillSet == "" && os.Getenv("HOME_ASSISTANT_BEARER_AUTH") != "" {
				envStillSet = "HOME_ASSISTANT_BEARER_AUTH"
			}
