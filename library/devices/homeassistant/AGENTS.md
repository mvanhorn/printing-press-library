...
This directory is a generated `homeassistant-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
homeassistant-pp-cli doctor --json
homeassistant-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
homeassistant-pp-cli which "<capability>" --json
homeassistant-pp-cli <command> --help
```
...
```bash
homeassistant-pp-cli <command> --agent
```
...
```bash
homeassistant-pp-cli <command> --help
homeassistant-pp-cli <command> --dry-run --agent
```
