...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".homeassistant-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("HOMEASSISTANT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HOMEASSISTANT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/devices/homeassistant/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.homeassistant-pp-cli/feedback.jsonl.
When ` + "`HOMEASSISTANT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`HOMEASSISTANT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  homeassistant-pp-cli feedback list
  homeassistant-pp-cli feedback list --limit 5
  homeassistant-pp-cli feedback list --json`,
