...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  homeassistant-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  homeassistant-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"calendars", "components", "events", "services", "states",  }
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  homeassistant-pp-cli workflow status

  # Show status as JSON
  homeassistant-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("homeassistant-pp-cli")
			}
