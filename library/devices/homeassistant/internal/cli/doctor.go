...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  homeassistant-pp-cli doctor
  homeassistant-pp-cli doctor --json
  homeassistant-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export HOMEASSISTANT_TOKEN=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("HOMEASSISTANT_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "HOMEASSISTANT_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "HOMEASSISTANT_TOKEN")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("homeassistant-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'homeassistant-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'homeassistant-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'homeassistant-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'homeassistant-pp-cli sync' to refresh."
	}
	return report
}
