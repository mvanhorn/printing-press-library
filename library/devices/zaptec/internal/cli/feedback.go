...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".zaptec-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ZAPTEC_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ZAPTEC_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "zaptec-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.zaptec-pp-cli/feedback.jsonl.
When ` + "`ZAPTEC_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ZAPTEC_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  zaptec-pp-cli feedback list
  zaptec-pp-cli feedback list --limit 5
  zaptec-pp-cli feedback list --json`,
