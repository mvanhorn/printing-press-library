...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  synology-dsm-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  synology-dsm-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  synology-dsm-pp-cli workflow status

  # Show status as JSON
  synology-dsm-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("synology-dsm-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Path to local SQLite database (default: ~/.local/share/synology-dsm-pp-cli/data.db)")
