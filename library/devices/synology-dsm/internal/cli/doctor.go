...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  synology-dsm-pp-cli doctor
  synology-dsm-pp-cli doctor --json
  synology-dsm-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export SYNOLOGY_DSM_SIDCOOKIE=<your-key>"
			...
			if os.Getenv("SYNOLOGY_DSM_SIDCOOKIE") != "" {
				authEnvSet = append(authEnvSet, "SYNOLOGY_DSM_SIDCOOKIE")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "SYNOLOGY_DSM_SIDCOOKIE")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'synology-dsm-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'synology-dsm-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'synology-dsm-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'synology-dsm-pp-cli sync' to refresh."
	}
	return report
}
