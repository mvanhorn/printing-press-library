...
Resource scoping:
  ...
  synology-dsm-pp-cli sync

  # Sync specific resources only
  synology-dsm-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  synology-dsm-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  synology-dsm-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  synology-dsm-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  synology-dsm-pp-cli sync --latest-only`,
...
func defaultSyncResources() []string {
	return []string{
		"webapi",
		"webapi-entry-cgi-auth-logout",
		"webapi-entry-cgi-filestation-info",
		"webapi-entry-cgi-system-utilization",
	}
}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"webapi":                              "/webapi/entry.cgi/dsm/info",
		"webapi-entry-cgi-auth-logout":        "/webapi/entry.cgi/auth/logout",
		"webapi-entry-cgi-filestation-info":   "/webapi/entry.cgi/filestation/info",
		"webapi-entry-cgi-system-utilization": "/webapi/entry.cgi/system/utilization",
	}
