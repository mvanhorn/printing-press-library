...
func newWebapiRemoveImageCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "remove-image",
		Short:       "Remove a Docker image from the NAS",
		Example:     "  synology-dsm-pp-cli webapi remove-image --image-name adguard/adguardhome:latest",
		Annotations: map[string]string{"pp:endpoint": "webapi.remove-image", "pp:method": "POST", "pp:path": "/webapi/entry.cgi/docker/image/delete"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			path := "/webapi/entry.cgi/docker/image/delete"
	...
	cmd.Flags().StringVar(&bodyImageName, "image-name", "", "Image name with tag (e.g. adguard/adguardhome:latest) (required)")
