...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".synology-dsm-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SYNOLOGY_DSM_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SYNOLOGY_DSM_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "synology-dsm-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.synology-dsm-pp-cli/feedback.jsonl.
When ` + "`SYNOLOGY_DSM_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SYNOLOGY_DSM_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  synology-dsm-pp-cli feedback list
  synology-dsm-pp-cli feedback list --limit 5
  synology-dsm-pp-cli feedback list --json`,
