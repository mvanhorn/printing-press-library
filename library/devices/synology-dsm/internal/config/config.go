...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("SYNOLOGY_DSM_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "synology-dsm-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("SYNOLOGY_DSM_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("SYNOLOGY_DSM_HOST")); v != "" {
	...
	if v := strings.TrimSpace(os.Getenv("SYNOLOGY_DSM_PORT")); v != "" {
