...
This directory is a generated `synology-dsm-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
synology-dsm-pp-cli doctor --json
synology-dsm-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
synology-dsm-pp-cli which "<capability>" --json
synology-dsm-pp-cli <command> --help
```
...
```bash
synology-dsm-pp-cli <command> --agent
```
...
```bash
synology-dsm-pp-cli <command> --help
synology-dsm-pp-cli <command> --dry-run --agent
```
