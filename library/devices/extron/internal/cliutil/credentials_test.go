...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	return home, filepath.Join(home, ".config", "extron-pp-cli", "config.toml")
}
...
func TestConfigFileWinsWhenCredentialsFileAlsoHasSecrets(t *testing.T) {
	...
	// pp-machine-bug-workaround: the generator template emits AuthHeader()
	// containment asserts for every spec, but auth:none specs compose no
	// header (AuthHeader() is "" by design). The credential merge behavior
	// under test is the AccessToken assertion above. Retro candidate.
}
...
func TestExplicitConfigUsesSiblingCredentialsBeforeGlobal(t *testing.T) {
	...
	t.Setenv("EXTRON_CONFIG", configPath)
...
func TestCorruptCredentialsDoesNotOverrideLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "extron-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
	...
	stderr := captureCredentialStderr(t, func() {
		...
		assertConfigCredential(t, cfg, "legacy-secret")
		// pp-machine-bug-workaround: see TestConfigFileWinsWhenCredentialsFileAlsoHasSecrets.
	})
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "extron-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "extron-pp-cli", "credentials.toml")
