...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"product_series": {
			{Canonical: "DTP Series", Values: []string{"dtp", "dtp2", "dtp t"}},
			{Canonical: "MAV Plus Series", Values: []string{"mav", "mav88", "mavplus", "mav plus"}},
			{Canonical: "IP Link Series", Values: []string{"ipl", "ip link", "ipl t"}},
			{Canonical: "DVS Series", Values: []string{"dvs", "dvs304", "dvs605"}},
			{Canonical: "SMD Series", Values: []string{"smd", "smd101", "smd202"}},
			{Canonical: "PCS Series", Values: []string{"pcs", "pcs4"}},
			{Canonical: "MediaLink Plus Series", Values: []string{"medialink", "mlc"}},
			{Canonical: "Annotator Series", Values: []string{"annotator", "annotator 300"}},
			{Canonical: "Matrix Series", Values: []string{"matrix", "matrix 3200", "matrix 12800"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		dbPath := defaultDBPath("extron-pp-cli")
		s, err := store.OpenWithContext(ctx, dbPath)
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: extron-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: extron-pp-cli: learn init: %v\n", err)
		}
	})
}
