...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("EXTRON_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("EXTRON_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "extron-pp-cli/feedback")
...
When ` + "`EXTRON_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`EXTRON_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  extron-pp-cli feedback list
  extron-pp-cli feedback list --limit 5
  extron-pp-cli feedback list --json`,
