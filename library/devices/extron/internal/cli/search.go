...
Run sync first to populate the local search index.`,
  ...
  extron-pp-cli search "status"

  # Force local search explicitly
  extron-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  extron-pp-cli search "status" --type literature --data-source local
  # JSON output for piping
  extron-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'extron-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
