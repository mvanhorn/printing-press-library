...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CRESTRON_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CRESTRON_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "crestron-pp-cli/feedback")
...
When ` + "`CRESTRON_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CRESTRON_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  crestron-pp-cli feedback list
  crestron-pp-cli feedback list --limit 5
  crestron-pp-cli feedback list --json`,
