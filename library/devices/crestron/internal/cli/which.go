...
var whichIndex = []whichEntry{
	{Command: "fleet status", Description: "Check every model in your installed fleet against current firmware in one command.", Group: "Fleet lifecycle intelligence", WhyItMatters: "Reach for this instead of checking models one at a time; it also catches releases that a per-model search would miss because the release is titled under a sibling model."},
	{Command: "search", Description: "Search every firmware release note and change log at once for a term.", Group: "Firmware knowledge base", WhyItMatters: "Use this to answer 'which version fixed X' without opening a dozen version pages."},
	{Command: "firmware diff", Description: "Show what changed between two firmware versions for a model.", Group: "Firmware knowledge base", WhyItMatters: "Pick this when deciding whether an upgrade is worth scheduling on a live site."},
	{Command: "submittal", Description: "Download every documentation asset for a list of models into per-model folders with a coverage report.", Group: "Design and submittal workflow", WhyItMatters: "Use this to assemble a CSI submittal package in one step instead of hundreds of individual downloads."},
	{Command: "lifecycle", Description: "Report whether a model is still sellable and trace its replacement chain.", Group: "Fleet lifecycle intelligence", WhyItMatters: "Reach for this when triaging an as-built list to find which parts can still be ordered and what replaced the rest."},
	{Command: "specs compare", Description: "Compare two models field by field across the full specification table.", Group: "Design and submittal workflow", WhyItMatters: "Use this when choosing between sibling models in the same series, which often differ in only a few spec rows."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  crestron-pp-cli which "stale tickets"
  crestron-pp-cli which "bottleneck"
  crestron-pp-cli which --limit 1 "send message"
  crestron-pp-cli which                                # list the full capability index`,
