...
type rootFlags struct {
	...
	// invocation. Mirrors the CRESTRON_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "crestron-pp-cli",
		Short: `Crestron CLI — Every Crestron product, spec sheet, and firmware release — searchable offline, with fleet-wide currency checks the…`,
		Long: `Crestron CLI — Every Crestron product, spec sheet, and firmware release — searchable offline, with fleet-wide currency checks the website cannot do.
...
Health check: run 'crestron-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("crestron-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'crestron-pp-cli profile list')")
	...
	rootCmd.AddCommand(newAssetPromotedCmd(flags))
	rootCmd.AddCommand(newResourcePromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelLifecycleCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSearchCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSpecsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSubmittalCmd(flags))
