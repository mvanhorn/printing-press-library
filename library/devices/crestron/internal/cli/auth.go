...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Crestron",
...
func requiredAuthCookies() []string {
	raw := []string{".ASPXFORMSAUTH", ".WebApp.Cookies", "ASP.NET_SessionId", "CMSCsrfCookie"}
	out := raw[:0]
...
Use --chrome to read cookies from Chrome for www.crestron.com.
...
profile by name when the installed backend supports it.`,
		Example: `  crestron-pp-cli auth login --chrome
  crestron-pp-cli auth login --browser
  crestron-pp-cli auth login --cookies-file storage-state.json
  crestron-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			// www, not the apex: the forms-auth ticket and session cookies are scoped to
			// host www.crestron.com; the apex domain only yields the .crestron.com set.
			domain := "www.crestron.com"
			if !browserFlag && cookiesFile == "" {
				fmt.Fprintln(w, "Use --chrome, --browser, or --cookies-file to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  crestron-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  crestron-pp-cli auth login --browser\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  crestron-pp-cli auth login --cookies-file storage-state.json\n")
				return nil
			}
			if browserFlag && cookiesFile != "" {
			...
			requiredCookies := []string{".ASPXFORMSAUTH", ".WebApp.Cookies", "ASP.NET_SessionId", "CMSCsrfCookie"}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  crestron-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  crestron-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: www.crestron.com\n")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  crestron-pp-cli auth set-token eyJhbGciOi...",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  crestron-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
