...
func syncResourceSinceParam(resource string) string {
	...
	case "daily-activity":
		return "start_date"
	case "daily-cardiovascular-age":
		return "start_date"
	case "daily-readiness":
		return "start_date"
	case "daily-resilience":
		return "start_date"
	case "daily-sleep":
		return "start_date"
	case "daily-spo2":
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "daily-activity":
		return db.UpsertDailyActivity(data)
	case "daily-readiness":
		return db.UpsertDailyReadiness(data)
	case "daily-sleep":
		return db.UpsertDailySleep(data)
	case "daily-stress":
		return db.UpsertDailyStress(data)
	case "enhanced-tag":
		return db.UpsertEnhancedTag(data)
	case "personal-info":
		return db.UpsertPersonalInfo(data)
	case "ring-configuration":
		return db.UpsertRingConfiguration(data)
	case "session":
		return db.UpsertSession(data)
	case "sleep":
		return db.UpsertSleep(data)
	case "sleep-time":
		return db.UpsertSleepTime(data)
	case "webhook-subscription":
		return db.UpsertWebhookSubscription(data)
	case "workout":
		return db.UpsertWorkout(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"enhanced-tag":             "/v2/usercollection/enhanced_tag",
