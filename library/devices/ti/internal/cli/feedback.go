...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ti-pp-cli/feedback")
...
When ` + "`TI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ti-pp-cli feedback list
  ti-pp-cli feedback list --limit 5
  ti-pp-cli feedback list --json`,
