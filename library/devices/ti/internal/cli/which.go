...
var whichIndex = []whichEntry{
	{Command: "part compliance", Description: "Get a part's RoHS and REACH verdicts plus the applicable signed TI compliance statement PDFs in one JSON envelope.", Group: "Extraction-lane plumbing", WhyItMatters: "Reach for this when a supplier-extraction run needs verdicts AND evidence documents in a single deterministic call."},
	{Command: "auth check", Description: "Find out whether the deposited myTI cookie snapshot still works before running a batch of gated extractions.", Group: "Login-gated access", WhyItMatters: "Run it before any part ratings or part fmd batch; a stale result means trigger a fresh HIL login instead of burning lookups."},
	{Command: "part evidence", Description: "One command that always returns anonymous verdicts and blanket PDFs, and adds the rich ratings table and Class-D FMD XML when cookies work.", Group: "Extraction-lane plumbing", WhyItMatters: "Use when you want the most complete evidence available right now without pre-deciding whether login works."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  ti-pp-cli which "stale tickets"
  ti-pp-cli which "bottleneck"
  ti-pp-cli which --limit 1 "send message"
  ti-pp-cli which                                # list the full capability index`,
