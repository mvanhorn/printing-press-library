...
type rootFlags struct {
	...
	// invocation. Mirrors the TI_NO_LEARN env var.
	noLearn       bool
	selectFields  string
	configPath    string
	homePath      string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "ti-pp-cli",
		Short: `TI part compliance from the terminal: anonymous RoHS/REACH per orderable, and cookie-injected access to Environmental Ratings, RoHS exemptions, and IPC-1752A Class-D FMD XML.`,
		Long: `TI part compliance from the terminal: anonymous RoHS/REACH per orderable, and cookie-injected access to Environmental Ratings, RoHS exemptions, and IPC-1752A Class-D FMD XML.
...
Health check: run 'ti-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("ti-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'ti-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelAuthCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newLiteraturePromotedCmd(flags))
	rootCmd.AddCommand(newPartPromotedCmd(flags))
