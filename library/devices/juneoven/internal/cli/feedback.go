...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("JUNEOVEN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("JUNEOVEN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "juneoven-pp-cli/feedback")
...
When ` + "`JUNEOVEN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`JUNEOVEN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  juneoven-pp-cli feedback list
  juneoven-pp-cli feedback list --limit 5
  juneoven-pp-cli feedback list --json`,
