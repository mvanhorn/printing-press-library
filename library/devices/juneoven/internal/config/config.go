...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.junelife.com",
	}
	...
	// Env var overrides
	if v := os.Getenv("JUNE_ACCESS_TOKEN"); v != "" {
		cfg.AccessToken = v
		cfg.markEnvOverride("AccessToken")
		cfg.AuthSource = "env:JUNE_ACCESS_TOKEN"
		cfg.CredentialSource = "env:JUNE_ACCESS_TOKEN"
	}
	...
	if v := os.Getenv("JUNEOVEN_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("JUNEOVEN_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "juneoven-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.AccessToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:JUNE_ACCESS_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"access_token":      c.AccessToken,
			"JUNE_ACCESS_TOKEN": c.AccessToken,
			"token":             c.AccessToken,
		})
	}
	if c.AccessToken != "" {
