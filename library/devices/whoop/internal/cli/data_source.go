...
func resolvePaginatedRead(ctx context.Context, c *client.Client, flags *rootFlags, resourceType string, path string, params map[string]string, headers map[string]string, fetchAll bool, cursorParam, nextCursorPath, hasMoreField string) (json.RawMessage, DataProvenance, error) {
	// WHOOP v2 enforces limit <= 25; any larger value returns HTTP 400.
	// This is the R2 fix from the reprint brief — Greg's 1.0 CLI shipped
	// without this clamp and every backfill failed.
	if v, ok := params["limit"]; ok && v != "" {
		if n, err := strconv.Atoi(v); err == nil && n > 25 {
			fmt.Fprintf(os.Stderr, "warning: --limit %d exceeds WHOOP server max (25); clamping. Use --all to auto-paginate.\n", n)
			params["limit"] = "25"
		}
	}
	switch flags.dataSource {
...
func writeThroughCache(ctx context.Context, resourceType string, data json.RawMessage) {
	...
	if json.Unmarshal(data, &items) != nil || len(items) == 0 {
		...
		if json.Unmarshal(data, &envelope) == nil {
			for _, key := range []string{"results", "data", "items", "records"} {
