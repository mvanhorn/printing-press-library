...
// newAuthRefreshCmd exchanges the stored refresh_token for a new access_token.
// Exposes the refresh as an explicit command for two reasons: (1) doctor/agent
// pipelines that want to pre-warm credentials before a long backfill, (2)
// non-printing-press OAuth bug investigations where the user wants to force a
// refresh without waiting for the 60-second expiry window.
//
// WHOOP requires client_id+client_secret on the refresh call (confidential
// client OAuth flow). They are persisted by `auth login` into config.toml.
func newAuthRefreshCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg.RefreshToken == "" {
				return fmt.Errorf("no refresh token on disk. Run 'whoop-pp-cli auth login' (with the `offline` scope, which is requested by default)")
			}
			if cfg.ClientID == "" {
				return fmt.Errorf("no client_id on disk. Run 'whoop-pp-cli auth login --client-id <id> --client-secret <secret>' to seed credentials")
			}
			...
			params := url.Values{
				...
				"scope":         {"offline read:body_measurement read:cycles read:profile read:recovery read:sleep read:workout"},
			}
			if cfg.ClientSecret != "" {
			...
			if resp.StatusCode >= 400 {
				...
				if jerr := json.NewDecoder(resp.Body).Decode(&body); jerr == nil {
					return fmt.Errorf("refresh failed: HTTP %d: %v", resp.StatusCode, body)
				}
			...
			// WHOOP rotates refresh tokens: when a new one arrives, persist it.
			// When it doesn't, keep the existing one (otherwise we'd lock the
			// user out the next time they refresh).
			newRefresh := tokenResp.RefreshToken
			if newRefresh == "" {
				newRefresh = cfg.RefreshToken
			}
			expiry := time.Now().Add(time.Duration(tokenResp.ExpiresIn) * time.Second)
			if err := cfg.SaveTokens(cfg.ClientID, cfg.ClientSecret, tokenResp.AccessToken, newRefresh, expiry); err != nil {
			...
			fmt.Fprintf(os.Stderr, "%s Refreshed. Token expires at %s\n", green("OK"), expiry.Format(time.RFC3339))
			return nil
		},
	}
}
...
func newAuthLoginCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			scopes := []string{"offline", "read:body_measurement", "read:cycles", "read:profile", "read:recovery", "read:sleep", "read:workout"}
			if len(scopes) > 0 {
