...
Highlights (not in the official API docs):
  ...
  • sql   Execute read-only SELECT/WITH queries (or read-only PRAGMAs like table_info, table_list, foreign_key_list for schema introspection) against the local SQLite store. Accept the query as a positional ar…
  • search   FTS5 full-text search across all synced resources (cycle, sleep, recovery, workouts).

Agent mode: add --agent to any command for JSON output + non-interactive mode.
Health check: run 'whoop-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
	...
	// Hand-written transcendence layer: 7 novel commands that no live API
	// call can compute (cross-resource joins over local SQLite).
