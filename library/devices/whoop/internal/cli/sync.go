...
//
// WHOOP v2 specifics (manually patched after generation):
//   - cursor query param is `nextToken`; response cursor field is `next_token`
//   - server enforces `limit <= 25` (default 10); larger values return HTTP 400
//
// This is the R2 fix from the reprint brief — Greg's 1.0 CLI shipped limit=50
// and every backfill failed with 400. The whole reprint exists to fix this.
...
func extractPaginationFromEnvelope(envelope map[string]json.RawMessage, cursorParam string) (string, bool) {
	...
	cursorKeys := []string{
		...
		// WHOOP v2 envelope: { records: [...], next_token: "..." }.
