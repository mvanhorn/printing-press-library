...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^tsc-[0-9]+w?(-g[0-9])?$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: qsys-pp-cli: ticker pattern %q failed to compile: %v\n", `^tsc-[0-9]+w?(-g[0-9])?$`, err)
	} else {
		cfg.RegisterTickerPattern(re)
	}
	if re, err := regexp.Compile(`^cxd?-q.*$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: qsys-pp-cli: ticker pattern %q failed to compile: %v\n", `^cxd?-q.*$`, err)
	} else {
		cfg.RegisterTickerPattern(re)
	}
	if re, err := regexp.Compile(`^qio-[a-z0-9]+$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: qsys-pp-cli: ticker pattern %q failed to compile: %v\n", `^qio-[a-z0-9]+$`, err)
	} else {
		cfg.RegisterTickerPattern(re)
	}
	if re, err := regexp.Compile(`^nl-[a-z0-9]+$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: qsys-pp-cli: ticker pattern %q failed to compile: %v\n", `^nl-[a-z0-9]+$`, err)
	} else {
		cfg.RegisterTickerPattern(re)
	}
	if re, err := regexp.Compile(`^mpa-q.*$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: qsys-pp-cli: ticker pattern %q failed to compile: %v\n", `^mpa-q.*$`, err)
	} else {
		cfg.RegisterTickerPattern(re)
	}
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"product_family": {
			{Canonical: "power-amplifiers", Values: []string{"amps", "amplifiers", "amp", "power amps"}},
			{Canonical: "loudspeakers", Values: []string{"speakers", "loudspeaker", "spkr"}},
			{Canonical: "q-sys", Values: []string{"cores", "core", "processors", "qsys core"}},
			{Canonical: "mixers", Values: []string{"mixer", "mixing"}},
			{Canonical: "discontinued-products", Values: []string{"discontinued", "eol", "end of life", "obsolete"}},
		},
		"product_model": {
			{Canonical: "CX-Q", Values: []string{"cxq", "cx q", "cx-q series"}},
			{Canonical: "CXD-Q", Values: []string{"cxdq", "cxd q", "cxd-q series"}},
			{Canonical: "MPA-Q", Values: []string{"mpaq", "mpa q"}},
			{Canonical: "SPA-Qf", Values: []string{"spaqf", "spa qf", "spa-qf"}},
			{Canonical: "TSC-70-G3", Values: []string{"tsc70", "tsc 70", "tsc-70"}},
			{Canonical: "TSC-101-G3", Values: []string{"tsc101", "tsc 101", "tsc-101"}},
			{Canonical: "NL-C4", Values: []string{"nlc4", "nl c4"}},
		},
		"topic": {
			{Canonical: "Dante_Audio", Values: []string{"dante", "dante audio"}},
			{Canonical: "QoS", Values: []string{"qos", "quality of service"}},
			{Canonical: "Multicast_Traffic", Values: []string{"multicast", "igmp"}},
			{Canonical: "Network_Setup_Checklist", Values: []string{"network setup", "network checklist", "switch setup"}},
			{Canonical: "Addressing_Routing", Values: []string{"addressing", "routing", "ip setup"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: qsys-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: qsys-pp-cli: learn init: %v\n", err)
		}
	})
}
