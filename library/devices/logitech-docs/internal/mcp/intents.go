...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("preview_a_download",
			mcplib.WithDescription("see the resolved file URL before fetching"),
		...
		handlePreviewADownload,
	)
}

// handlePreviewADownload runs the preview_a_download recipe intent: see the resolved file URL before fetching
func handlePreviewADownload(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
