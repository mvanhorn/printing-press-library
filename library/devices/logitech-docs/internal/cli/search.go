...
In local mode: searches locally synced data only.`,
  ...
  logitech-docs-pp-cli search "error timeout"

  # Force local search only
  logitech-docs-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  logitech-docs-pp-cli search "status" --type articles --data-source local
  # JSON output for piping
  logitech-docs-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /api/v2/help_center/articles/search.json
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/api/v2/help_center/articles/search.json", map[string]string{
					"query": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'logitech-docs-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "sections":
				results, err = db.SearchSections(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchSections(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search sections failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
