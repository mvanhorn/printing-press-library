...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/devices/logitech-docs/internal/learn"
	"github.com/mvanhorn/printing-press-library/library/devices/logitech-docs/internal/learn/lookups"
...
Resource scoping:
  ...
  logitech-docs-pp-cli sync
  # Sync specific resources only
  logitech-docs-pp-cli sync --resources sections,articles

  # Full resync (ignore previous checkpoint)
  logitech-docs-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  logitech-docs-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  logitech-docs-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  logitech-docs-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("logitech-docs-pp-cli")
			}
...
func determinePaginationDefaults(resource string) paginationDefaults {
	...
	case "articles":
		return paginationDefaults{
			...
			cursorType:     "cursor",
			nextCursorPath: "meta.after_cursor",
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "articles", ParentTable: "sections", ParentIDParam: "section_id", PathTemplate: "/api/v2/help_center/en-us/sections/{section_id}/articles.json", KeyField: "", ReconcileMode: "none", GenericScopeJSONPath: "", PathParams: []dependentPathParamDef{
	...
	for {
		...
		if err != nil {
			...
			if w, ok := isSyncAccessWarning(err); ok {
				...
				// Record the failure so the aggregator counts this parent as
				// failed. Without this the error was only *printed*: rep.failure
				// stayed nil, the run was reported successful, and a checkpoint
				// was saved over a partially-fetched corpus. Articles are only
				// reachable by walking every section, so one failed section is
				// silent truncation — the exact failure mode this CLI was
				// patched to remove.
...
func syncDependentResource(ctx context.Context, c interface {
	...
	// Check the query error FIRST. dependentParentRows returns (nil, err) on
	// failure, so an `err != nil || len(parentRows) == 0` gate with the empty
	// case tested first swallowed real database errors and reported them as a
	// successful zero-record sync.
	if err != nil {
	...
	if len(parentRows) == 0 {
		// An empty parent table is not success. Reaching this resource requires
		// walking its parents, so zero parents means zero coverage — report it
		// as a warning with a structured event instead of exiting 0 silently.
		warn := fmt.Errorf("parent table %s is empty, so no %s were fetched; run sync for %s first", dep.ParentTable, dep.Name, dep.ParentTable)
		if humanFriendly {
			fmt.Fprintf(os.Stderr, "  %s: skipping (parent table %s is empty, sync it first)\n", dep.Name, dep.ParentTable)
		} else {
			fmt.Fprintf(syncEvents, `{"event":"sync_warning","resource":%q,"reason":"dependent_parent_table_empty","parent":%q,"message":%q}`+"\n",
				dep.Name, dep.ParentTable, warn.Error())
		}
		return syncResult{Resource: dep.Name, Warn: warn, Duration: time.Since(started)}
	}
