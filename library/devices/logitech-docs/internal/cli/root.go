...
type rootFlags struct {
	...
	// invocation. Mirrors the LOGITECH_DOCS_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "logitech-docs-pp-cli",
		Short: `Logitech Docs CLI — Every Logitech manual, spec sheet, and install guide — searchable offline, agent-native, no browser.`,
		Long: `Logitech Docs CLI — Every Logitech manual, spec sheet, and install guide — searchable offline, agent-native, no browser.
...
Health check: run 'logitech-docs-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("logitech-docs-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'logitech-docs-pp-cli profile list')")
	...
	addNovelCommandIfAbsent(rootCmd, newNovelDownloadCmd(flags))
