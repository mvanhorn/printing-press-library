# Eero Printed CLI Agent Guide

This directory is a generated `eero-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
eero-pp-cli doctor --json
eero-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
eero-pp-cli which "<capability>" --json
eero-pp-cli <command> --help
```
...
```bash
eero-pp-cli <command> --help
eero-pp-cli <command> --dry-run --agent
```
...
Disable the loop with `--no-learn` per-invocation or `EERO_NO_LEARN=true` for the whole session - useful for deterministic agent flows that don't want a learning row to silently change subsequent query results.
