...
type Config struct {
	...
	EeroSessionToken string          `toml:"session_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("EERO_SESSION_TOKEN"); v != "" {
		cfg.EeroSessionToken = v
		cfg.markEnvOverride("EeroSessionToken")
		cfg.AuthSource = "env:EERO_SESSION_TOKEN"
		cfg.CredentialSource = "env:EERO_SESSION_TOKEN"
	}
	...
	if v := os.Getenv("EERO_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("EERO_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "eero-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.EeroSessionToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:EERO_SESSION_TOKEN"
		}
		return applyAuthFormat("s={token}", map[string]string{
			"session_token":      c.EeroSessionToken,
			"EERO_SESSION_TOKEN": c.EeroSessionToken,
			"token":              c.EeroSessionToken,
			"access_token":       c.EeroSessionToken,
		})
	}
	if c.AccessToken != "" {
		if c.AuthSource == "" {
		...
		return applyAuthFormat("s={token}", map[string]string{"access_token": c.AccessToken, "token": c.AccessToken})
	}
	return ""
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		EeroSessionToken: c.EeroSessionToken,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.EeroSessionToken == "" {
		c.EeroSessionToken = creds.EeroSessionToken
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.EeroSessionToken = ""
	delete(c.envOverrides, "EeroSessionToken")
	c.updateFileConfigField("EeroSessionToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["EeroSessionToken"] {
			out.EeroSessionToken = c.fileConfig.EeroSessionToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "EeroSessionToken":
		c.fileConfig.EeroSessionToken = c.EeroSessionToken
	}
}
