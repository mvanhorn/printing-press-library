...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  eero-pp-cli doctor
  eero-pp-cli doctor --json
  eero-pp-cli doctor --fail-on warn
  eero-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !credentialConfigured {
				...
				report["auth_hint"] = "eero-pp-cli auth login --chrome"
			} else {
				authConfigured = true
				...
				if cfg != nil && cfg.CredentialConfigured() {
					...
					report["auth_domain"] = "api-user.e2ro.com"
				}
			}
			...
			if os.Getenv("EERO_SESSION_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "EERO_SESSION_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "EERO_SESSION_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "eero-pp-cli/0.1 (unofficial; community API client)"
