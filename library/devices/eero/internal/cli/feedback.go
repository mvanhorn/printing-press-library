...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("EERO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("EERO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "eero-pp-cli/feedback")
...
When ` + "`EERO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`EERO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  eero-pp-cli feedback list
  eero-pp-cli feedback list --limit 5
  eero-pp-cli feedback list --json`,
