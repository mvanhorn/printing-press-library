...
Resource scoping:
  ...
  synology-router-pp-cli sync

  # Sync specific resources only
  synology-router-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  synology-router-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  synology-router-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  synology-router-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  synology-router-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/synology-router-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "mesh":
		return db.UpsertMesh(data)
	case "session":
		return db.UpsertSession(data)
	case "smartwan":
		return db.UpsertSmartwan(data)
	case "wifi":
		return db.UpsertWifi(data)
	case "wol":
		return db.UpsertWol(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"access-control":    "/access-control/groups",
		"devices":           "/devices",
		"dns":               "/dns/ddns",
		"dns-external-ip":   "/dns/external-ip",
		"firewall":          "/firewall/rules",
		"mesh":              "/mesh/info",
		"mesh-nodes":        "/mesh/nodes",
		"qos":               "/qos/rules",
		"smartwan":          "/smartwan/config",
		"smartwan-gateways": "/smartwan/gateways",
		"system":            "/system/api-info",
		"traffic":           "/traffic",
