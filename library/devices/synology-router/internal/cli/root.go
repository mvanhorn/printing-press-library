...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "synology-router-pp-cli",
		Short: "Manage synology-router resources via the synology-router API",
		Long: `Manage synology-router resources via the synology-router API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'synology-router-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("synology-router-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.configPath, "config", "", "Path to the CLI configuration file (default: ~/.config/synology-router-pp-cli/config.toml)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'synology-router-pp-cli profile list')")
	...
	rootCmd.AddCommand(newSRMLoginCmd(flags))
	rootCmd.AddCommand(newSmartwanCmd(flags))
	rootCmd.AddCommand(newWifiCmd(flags))
	rootCmd.AddCommand(newWolCmd(flags))
	...
	rootCmd.AddCommand(newAccessControlPromotedCmd(flags))
	rootCmd.AddCommand(newDevicesPromotedCmd(flags))
	rootCmd.AddCommand(newQosPromotedCmd(flags))
	rootCmd.AddCommand(newSystemPromotedCmd(flags))
	rootCmd.AddCommand(newTrafficPromotedCmd(flags))
	rootCmd.AddCommand(newUtilizationPromotedCmd(flags))
	rootCmd.AddCommand(newWanPromotedCmd(flags))
	rootCmd.AddCommand(newHealthPromotedCmd(flags))
	rootCmd.AddCommand(newStatsPromotedCmd(flags))
	rootCmd.AddCommand(newStalePromotedCmd(flags))
	rootCmd.AddCommand(newBusiestPromotedCmd(flags))
	rootCmd.AddCommand(newTrendsPromotedCmd(flags))
	rootCmd.AddCommand(newCoveragePromotedCmd(flags))
	rootCmd.AddCommand(newBottleneckPromotedCmd(flags))
...
func (f *rootFlags) newClient() (*client.Client, error) {
	...
	c.DryRun = f.dryRun
	c.NoCache = f.noCache
	// Install the SRM transport to translate virtual REST calls to actual SRM
	// form-encoded API calls (POST /webapi/entry.cgi with api/method/version params).
	// The SID (session cookie) is read from the config's SynologyRouterCookieAuth field.
	client.NewSRMClient(c, client.SRMSessionFromConfig(c))
	return c, nil
}
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("synology-router-pp-cli %s\n", version)
		},
	}
}
