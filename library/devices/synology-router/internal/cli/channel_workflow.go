...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  synology-router-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  synology-router-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"access-control", "devices", "dns", "dns-external-ip", "firewall", "mesh", "mesh-nodes", "qos", "smartwan", "smartwan-gateways", "system", "traffic", "utilization", "wan", "wifi", "wol"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  synology-router-pp-cli workflow status

  # Show status as JSON
  synology-router-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("synology-router-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Path to the local SQLite database file for synced data")
