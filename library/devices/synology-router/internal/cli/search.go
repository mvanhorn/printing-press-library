...
In local mode: searches locally synced data only.`,
  ...
  synology-router-pp-cli search "error timeout"

  # Force local search only
  synology-router-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  synology-router-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  synology-router-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'synology-router-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "devices":
				results, err = db.SearchDevices(query, limit)
			case "traffic":
				results, err = db.SearchTraffic(query, limit)
			case "firewall":
				results, err = db.SearchFirewall(query, limit)
			default:
