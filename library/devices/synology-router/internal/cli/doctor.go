...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  synology-router-pp-cli doctor
  synology-router-pp-cli doctor --json
  synology-router-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export SYNOLOGY_ROUTER_COOKIE_AUTH=<your-key>"
					report["auth_docs_url"] = "https://www.synology.com"
				} else {
					authConfigured = true
			...
			if os.Getenv("SYNOLOGY_ROUTER_COOKIE_AUTH") != "" || authHeader != "" {
				authEnvSet = append(authEnvSet, "SYNOLOGY_ROUTER_COOKIE_AUTH")
			} else {
				authEnvInfo = append(authEnvInfo, "SYNOLOGY_ROUTER_COOKIE_AUTH set with auth set-token")
			}
			switch {
			...
			report["cache"] = collectCacheReport(cmd.Context(), "5m")
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'synology-router-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'synology-router-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'synology-router-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'synology-router-pp-cli sync' to refresh."
	}
	return report
}
