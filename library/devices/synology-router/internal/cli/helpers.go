...
// isTerminal detects whether w is a TTY using os.File.Stat (isatty equivalent).
...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'synology-router-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API key."+
			"\n      See API docs: https://www.synology.com"+
			"\n      Run 'synology-router-pp-cli doctor' to check auth status.", err))
...
func formatCellValue(v any) string {
	...
	case string:
		...
		// Color status/state strings for human-friendly output
		switch strings.ToLower(val) {
		case "online", "connected", "active", "up", "running", "enabled", "true", "success":
			return green(val)
		case "offline", "disconnected", "inactive", "down", "stopped", "disabled", "false", "error", "failed":
			return red(val)
		case "warning", "degraded", "standby", "partial":
			return yellow(val)
		}
