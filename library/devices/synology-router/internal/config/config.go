...
type Config struct {
	...
	Path                     string            `toml:"-"`
	SynologyRouterCookieAuth string            `toml:"router_cookie_auth"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://192.168.1.254:8001/webapi",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("SYNOLOGY_ROUTER_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "synology-router-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("SYNOLOGY_ROUTER_COOKIE_AUTH"); v != "" {
		cfg.SynologyRouterCookieAuth = v
		cfg.AuthSource = "env:SYNOLOGY_ROUTER_COOKIE_AUTH"
	}
	...
	if cfg.AuthSource == "" && cfg.SynologyRouterCookieAuth != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("SYNOLOGY_ROUTER_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.SynologyRouterCookieAuth
	if token == "" {
		return ""
	}
	return token
...
func (c *Config) SaveCredential(token string) error {
	c.AuthHeaderVal = ""
	c.AccessToken = ""
	c.SynologyRouterCookieAuth = token
	return c.save()
}
