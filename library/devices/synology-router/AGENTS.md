# Synology Router Printed CLI Agent Guide

This directory is a generated `synology-router-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
synology-router-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
synology-router-pp-cli which "<capability>" --json
synology-router-pp-cli <command> --help
```
...
```bash
synology-router-pp-cli <command> --agent
```
...
```bash
synology-router-pp-cli <command> --help
synology-router-pp-cli <command> --dry-run --agent
```
