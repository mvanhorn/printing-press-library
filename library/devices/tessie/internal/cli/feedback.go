...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TESSIE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TESSIE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "tessie-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  tessie-pp-cli feedback 'the use command was clear'",
		Long: `Feedback is captured locally first in the CLI data directory's feedback.jsonl.
When ` + "`TESSIE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TESSIE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  tessie-pp-cli feedback list
  tessie-pp-cli feedback list --limit 5
  tessie-pp-cli feedback list --json`,
