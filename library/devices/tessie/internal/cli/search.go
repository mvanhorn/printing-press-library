...
Run sync first to populate the local search index.`,
  ...
  tessie-pp-cli search "status"

  # Force local search explicitly
  tessie-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  tessie-pp-cli search "status" --type commands --data-source local
  # JSON output for piping
  tessie-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'tessie-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
