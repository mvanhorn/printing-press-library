...
type rootFlags struct {
	...
	// invocation. Mirrors the TESSIE_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "tessie-pp-cli",
		Short: "Manage tessie resources via the tessie API",
		Long: `Manage tessie resources via the tessie API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'tessie-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("tessie-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'tessie-pp-cli profile list')")
	...
	rootCmd.AddCommand(newUseCmd(flags))
	...
	rootCmd.AddCommand(newCommandsPromotedCmd(flags))
