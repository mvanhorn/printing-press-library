...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  tessie-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  tessie-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  tessie-pp-cli workflow archive --timeout 0`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  tessie-pp-cli workflow status

  # Show status as JSON
  tessie-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("tessie-pp-cli")
			}
