...
type Config struct {
	...
	TessieApiToken   string          `toml:"api_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("TESSIE_API_TOKEN"); v != "" {
		cfg.TessieApiToken = v
		cfg.markEnvOverride("TessieApiToken")
		cfg.AuthSource = "env:TESSIE_API_TOKEN"
		cfg.CredentialSource = "env:TESSIE_API_TOKEN"
	}
	...
	if v := os.Getenv("TESSIE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "tessie-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.TessieApiToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:TESSIE_API_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"api_token":        c.TessieApiToken,
			"TESSIE_API_TOKEN": c.TessieApiToken,
			"token":            c.TessieApiToken,
			"access_token":     c.TessieApiToken,
		})
	}
	if c.AccessToken != "" {
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.TessieApiToken != "" {
		parts = append(parts, "api_token="+c.TessieApiToken)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		TessieApiToken: c.TessieApiToken,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.TessieApiToken == "" {
		c.TessieApiToken = creds.TessieApiToken
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.TessieApiToken = ""
	delete(c.envOverrides, "TessieApiToken")
	c.updateFileConfigField("TessieApiToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["TessieApiToken"] {
			out.TessieApiToken = c.fileConfig.TessieApiToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "TessieApiToken":
		c.fileConfig.TessieApiToken = c.TessieApiToken
	}
}
