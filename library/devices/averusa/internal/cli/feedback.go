...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AVERUSA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AVERUSA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "averusa-pp-cli/feedback")
...
When ` + "`AVERUSA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AVERUSA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  averusa-pp-cli feedback list
  averusa-pp-cli feedback list --limit 5
  averusa-pp-cli feedback list --json`,
