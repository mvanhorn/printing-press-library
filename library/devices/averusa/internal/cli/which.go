...
var whichIndex = []whichEntry{
	{Command: "compare", Description: "Side-by-side spec fields for two or more AVer models from their datasheets, ready for bid comparisons and RFP tables. Spec fields come from datasheet PDFs extracted by `harvest --with-specs`.", Group: "Local catalog that compounds", WhyItMatters: "Use this instead of opening N datasheet PDFs to answer 'which model fits?' for a bid."},
	{Command: "specs", Description: "One model's full spec fields as clean text or JSON for spec-compliance tables and agent pipelines. Fields come from datasheet PDFs extracted by `harvest --with-specs`.", Group: "Local catalog that compounds", WhyItMatters: "Use this to fill an RFP spec-compliance row without opening the PDF."},
	{Command: "docs audit", Description: "HEAD-checks every document URL in the catalog and flags 404s and soft-404 shells, caching last-checked status locally.", Group: "Reachability mitigation", WhyItMatters: "Use this before pushing docs to a shared drive to catch dead or mislinked PDFs while they are still cheap to fix."},
	{Command: "docs pack", Description: "Batch-download every document for a model into one offline folder with stable <model>-<type> names, with a --dry-run preview. Downloads need entityIds, which `harvest` resolves at sync time.", Group: "Offline job-site workflow", WhyItMatters: "Use this to pre-stage a job bag of manuals before driving to a site with no signal."},
	{Command: "whats-new", Description: "List documents and products added or updated since the last sync, filterable by age.", Group: "Local catalog that compounds", WhyItMatters: "Use this to track new manuals or firmware docs across a fleet without re-checking the website."},
	{Command: "coverage", Description: "Per-model doc-type availability matrix for a category, flagging which models are missing manuals, spec sheets, or white papers.", Group: "Local catalog that compounds", WhyItMatters: "Use this before a recommendation or commissioning checklist to catch missing compliance docs."},
	{Command: "products status", Description: "Flag which models AVer lists as discontinued, filterable by category.", Group: "Service-specific intelligence", WhyItMatters: "Use this before specing a model into a school bid so a discontinued unit never ships in the quote."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  averusa-pp-cli which "stale tickets"
  averusa-pp-cli which "bottleneck"
  averusa-pp-cli which --limit 1 "send message"
  averusa-pp-cli which                                # list the full capability index`,
