...
Run sync first to populate the local search index.`,
  ...
  averusa-pp-cli search "status"

  # Force local search explicitly
  averusa-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  averusa-pp-cli search "status" --type docs --data-source local
  # JSON output for piping
  averusa-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'averusa-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
