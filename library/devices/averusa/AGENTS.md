# AVer USA Printed CLI Agent Guide

This directory is a generated `averusa-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
averusa-pp-cli doctor --json
averusa-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
averusa-pp-cli which "<capability>" --json
averusa-pp-cli <command> --help
```
...
```bash
averusa-pp-cli <command> --agent
```

This CLI is read-only with respect to AVer's sites — it harvests public pages into a local corpus and never creates, updates, or deletes anything upstream. `harvest` is the only network-heavy command. Before running an unfamiliar command, inspect its help and prefer a dry run:

```bash
averusa-pp-cli <command> --help
averusa-pp-cli <command> --dry-run --agent
```
...
3. After answering, always fire `averusa-pp-cli teach --query "<question>" --resource <id> --resource-type <type> &` in the background - teaching is unconditional and is the anchor that triggers playbook synthesis. Teach the structural question with identifiers stripped (no names, emails, phone numbers, account ids); the CLI warns on obvious PII shapes but does not block.
...
Disable the loop with `--no-learn` per-invocation or `AVERUSA_NO_LEARN=true` for the whole session - useful for deterministic agent flows that don't want a learning row to silently change subsequent query results.
