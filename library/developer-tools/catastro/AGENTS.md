# Catastro Printed CLI Agent Guide

This directory is a generated `catastro-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
catastro-pp-cli doctor --json
catastro-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
catastro-pp-cli which "<capability>" --json
catastro-pp-cli <command> --help
```
...
```bash
catastro-pp-cli <command> --help
catastro-pp-cli <command> --dry-run --agent
```
