...
Resource scoping:
  ...
  catastro-pp-cli sync

  # Sync specific resources only
  catastro-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  catastro-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  catastro-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  catastro-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  catastro-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/catastro-pp-cli/data.db)")
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"provinces": "/COVCCallejero.svc/json/ObtenerProvincias",
	}
