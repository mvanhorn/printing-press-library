...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".catastro-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CATASTRO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CATASTRO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "catastro-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.catastro-pp-cli/feedback.jsonl.
When ` + "`CATASTRO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CATASTRO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  catastro-pp-cli feedback list
  catastro-pp-cli feedback list --limit 5
  catastro-pp-cli feedback list --json`,
