...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "catastro-pp-cli",
		Short: `Every Catastro feature, plus offline SQLite, bulk RC enrichment, polygon export and change detection no other Catastro tool has.`,
		Long: `Every Catastro feature, plus offline SQLite, bulk RC enrichment, polygon export and change detection no other Catastro tool has.
...
Highlights (not in the official API docs):
  ...
  • report   Dada una RC, compone un directorio (data.json, parts.csv, coords.json, neighbors.json, map.png, map-url.txt) con la parcela, sus vecinas, sub-parts y un PNG WMS — listo para anexar a un expediente té…
...
Health check: run 'catastro-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("catastro-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'catastro-pp-cli profile list')")
	...
	rootCmd.AddCommand(newMunicipalitiesPromotedCmd(flags))
	rootCmd.AddCommand(newNumbersPromotedCmd(flags))
	rootCmd.AddCommand(newProvincesPromotedCmd(flags))
	rootCmd.AddCommand(newStreetsPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	// Hand-written novel features (geocode, reconcile, enrich, neighbors,
	// report, stale, coverage, analyze-area, watch, export, wms, srs, cache).
	registerNovelCommands(rootCmd, flags)
