# Buildalert Printed CLI Agent Guide

This directory is a generated `buildalert-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
buildalert-pp-cli doctor --json
buildalert-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
buildalert-pp-cli which "<capability>" --json
buildalert-pp-cli <command> --help
```
...
```bash
buildalert-pp-cli <command> --agent
```
...
```bash
buildalert-pp-cli <command> --help
buildalert-pp-cli <command> --dry-run --agent
```
