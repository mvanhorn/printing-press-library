...
Resource scoping:
  ...
  buildalert-pp-cli sync

  # Sync specific resources only
  buildalert-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  buildalert-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  buildalert-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  buildalert-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  buildalert-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/buildalert-pp-cli/data.db)")
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"leads":            "/dapi/leads/live-leads",
		"letter_templates": "/dapi/letter-templates",
	}
