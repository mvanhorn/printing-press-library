...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  buildalert-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  buildalert-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"leads", "letter_templates"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  buildalert-pp-cli workflow status

  # Show status as JSON
  buildalert-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("buildalert-pp-cli")
			}
