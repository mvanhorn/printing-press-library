...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".buildalert-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BUILDALERT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BUILDALERT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "buildalert-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.buildalert-pp-cli/feedback.jsonl.
When ` + "`BUILDALERT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BUILDALERT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  buildalert-pp-cli feedback list
  buildalert-pp-cli feedback list --limit 5
  buildalert-pp-cli feedback list --json`,
