# AppTweak Printed CLI Agent Guide

This directory is a generated `apptweak-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
apptweak-pp-cli doctor --json
apptweak-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
apptweak-pp-cli which "<capability>" --json
apptweak-pp-cli <command> --help
```
...
```bash
apptweak-pp-cli <command> --help
apptweak-pp-cli <command> --dry-run --agent
```
