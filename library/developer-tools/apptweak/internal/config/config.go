...
type Config struct {
	...
	Path           string            `toml:"-"`
	ApptweakApiKey string            `toml:"api_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://public-api.apptweak.com/api/public/store",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("APPTWEAK_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "apptweak-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("APPTWEAK_API_KEY"); v != "" {
		cfg.ApptweakApiKey = v
		cfg.AuthSource = "env:APPTWEAK_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.ApptweakApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("APPTWEAK_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
