...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "apptweak-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("APPTWEAK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("APPTWEAK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "apptweak-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/apptweak-pp-cli/feedback.jsonl.
When ` + "`APPTWEAK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`APPTWEAK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  apptweak-pp-cli feedback list
  apptweak-pp-cli feedback list --limit 5
  apptweak-pp-cli feedback list --json`,
