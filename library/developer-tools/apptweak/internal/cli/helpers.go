...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			"\n      Set your API key: export APPTWEAK_API_KEY=<your-key>"+
			"\n      Run 'apptweak-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API key."+
			" Set it with: export APPTWEAK_API_KEY=<your-key>"+
			"\n      Run 'apptweak-pp-cli doctor' to check auth status.", err))
...
// storeForDevice maps a --device value to AppTweak's `store` parameter.
func storeForDevice(device string) string {
	if device == "android" {
		return "android"
	}
	return "ios"
...
// appResults unwraps AppTweak's {"result": {"<app id>": {...}}} response
// envelope and returns each app's raw payload. ok is false when the response
// does not carry that envelope.
func appResults(data json.RawMessage) (map[string]json.RawMessage, bool) {
	var top struct {
		Result map[string]json.RawMessage `json:"result"`
	}
	if json.Unmarshal(data, &top) != nil || top.Result == nil {
		return nil, false
	}
