...
Resource scoping:
  ...
  apptweak-pp-cli sync

  # Sync specific resources only
  apptweak-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  apptweak-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  apptweak-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  apptweak-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  apptweak-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/apptweak-pp-cli/data.db)")
