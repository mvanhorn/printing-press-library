...
func newNodesMoveLiveCmd(flags *rootFlags) *cobra.Command {
  ...
  framer-pp-cli nodes move abc123 --parent def456 --index 0`, "\n"),
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 { return cmd.Help() }
			if parentID == "" { return usageErr(fmt.Errorf("--parent is required")) }
			if dryRunOK(flags) {
...
func newNodesSetLiveCmd(flags *rootFlags) *cobra.Command {
  ...
  framer-pp-cli nodes set abc123 --attr width=400 --attr height=300 --attr opacity=0.5`, "\n"),
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 { return cmd.Help() }
			if len(attrs) == 0 { return usageErr(fmt.Errorf("at least one --attr key=value is required")) }
			attrMap := make(map[string]interface{})
			for _, a := range attrs {
				parts := strings.SplitN(a, "=", 2)
				if len(parts) != 2 { return usageErr(fmt.Errorf("invalid --attr format: %q (expected key=value)", a)) }
