...
// looksLikeHTML reports whether a response body appears to be HTML rather than
// JSON. Used by the doctor to detect 200 responses from marketing sites or
// misconfigured base_url values that return an HTML page instead of an API
// response.
func looksLikeHTML(body []byte) bool {
	...
	prefix := strings.ToLower(strings.TrimSpace(string(body[:limit])))
  ...
  framer-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					case reachErr == nil:
						...
						} else if looksLikeHTML(reachBody) {
								report["api"] = "FAIL: API returned HTML, not JSON — check base_url and project configuration."
							} else {
					...
					case errors.As(reachErr, &reachAPIErr):
						...
						} else if looksLikeHTML([]byte(reachAPIErr.Body)) {
								report["api"] = fmt.Sprintf("FAIL: API returned HTML, not JSON (HTTP %d) — check base_url and project configuration.", status)
							} else {
			...
			// Check project_url
				...
				if projectURL == "" {
					report["project_url"] = "not configured (set FRAMER_PROJECT_URL or project_url in config)"
				} else {
					report["project_url"] = projectURL
				}
