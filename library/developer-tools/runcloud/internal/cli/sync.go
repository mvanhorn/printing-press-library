...
Resource scoping:
  ...
  runcloud-pp-cli sync

  # Sync specific resources only
  runcloud-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  runcloud-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  runcloud-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  runcloud-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  runcloud-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("runcloud-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/runcloud-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "cron_jobs":
		return db.UpsertCronJobs(data)
	case "database_users":
		return db.UpsertDatabaseUsers(data)
	case "databases":
		return db.UpsertDatabases(data)
	case "domains":
		return db.UpsertDomains(data)
	case "servers":
		return db.UpsertServers(data)
	case "supervisor":
		return db.UpsertSupervisor(data)
	case "system_users":
		return db.UpsertSystemUsers(data)
	case "webapps":
		return db.UpsertWebapps(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"agency_client_servers": "/agency/client-servers",
		"agency_clients":        "/agency/clients",
		"agency_packages":       "/agency/server-packages",
		"agency_teams":          "/agency/teams",
		"external_keys":         "/settings/externalapi",
		"servers":               "/servers",
		"servers-shared":        "/servers/shared",
		"static":                "/static/webapps/installers",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "cron_jobs", ParentTable: "servers", ParentIDParam: "serverId", PathTemplate: "/servers/{serverId}/cronjobs", KeyField: ""},
		{Name: "database_users", ParentTable: "servers", ParentIDParam: "serverId", PathTemplate: "/servers/{serverId}/databaseusers", KeyField: ""},
		{Name: "databases", ParentTable: "servers", ParentIDParam: "serverId", PathTemplate: "/servers/{serverId}/databases", KeyField: ""},
		{Name: "supervisor", ParentTable: "servers", ParentIDParam: "serverId", PathTemplate: "/servers/{serverId}/supervisors", KeyField: ""},
		{Name: "system_users", ParentTable: "servers", ParentIDParam: "serverId", PathTemplate: "/servers/{serverId}/users", KeyField: ""},
		{Name: "webapps", ParentTable: "servers", ParentIDParam: "serverId", PathTemplate: "/servers/{serverId}/webapps", KeyField: ""},
	}
}
