...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  runcloud-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  runcloud-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"agency_client_servers", "agency_clients", "agency_packages", "agency_teams", "external_keys", "servers", "servers-shared", "static"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  runcloud-pp-cli workflow status

  # Show status as JSON
  runcloud-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("runcloud-pp-cli")
			}
