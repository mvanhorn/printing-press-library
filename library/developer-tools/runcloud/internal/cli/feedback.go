...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".runcloud-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("RUNCLOUD_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("RUNCLOUD_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "runcloud-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.runcloud-pp-cli/feedback.jsonl.
When ` + "`RUNCLOUD_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`RUNCLOUD_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  runcloud-pp-cli feedback list
  runcloud-pp-cli feedback list --limit 5
  runcloud-pp-cli feedback list --json`,
