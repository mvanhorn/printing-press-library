# RunCloud Printed CLI Agent Guide

This directory is a generated `runcloud-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
runcloud-pp-cli doctor --json
runcloud-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
runcloud-pp-cli which "<capability>" --json
runcloud-pp-cli <command> --help
```
...
```bash
runcloud-pp-cli <command> --help
runcloud-pp-cli <command> --dry-run --agent
```
