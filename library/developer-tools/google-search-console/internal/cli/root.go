...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		...
		Short: `Google Search Console CLI — Every Search Console feature, plus a local SQLite store that turns one-shot API calls into time-series insights no othe…`,
		Long: `Google Search Console CLI — Every Search Console feature, plus a local SQLite store that turns one-shot API calls into time-series insights no othe…
...
See README.md or the bundled SKILL.md for recipes.`,
	...
	rootCmd.AddCommand(newUrlInspectionPromotedCmd(flags))
	rootCmd.AddCommand(newUrlTestingToolsPromotedCmd(flags))
	...
	rootCmd.AddCommand(newInspectBatchCmd(flags))

	// Transcendence: 11 commands no other GSC tool can express.
	...
	rootCmd.AddCommand(newMomentumCmd(flags))
	rootCmd.AddCommand(newNewQueriesCmd(flags))
	rootCmd.AddCommand(newTriageCmd(flags))
	rootCmd.AddCommand(newTerritoryCmd(flags))
	rootCmd.AddCommand(newAppearanceCmd(flags))
	rootCmd.AddCommand(newSitemapHealthCmd(flags))
