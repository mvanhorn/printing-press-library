...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".cre-owner-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CRE_OWNER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CRE_OWNER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "cre-owner-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.cre-owner-pp-cli/feedback.jsonl.
When ` + "`CRE_OWNER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CRE_OWNER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cre-owner-pp-cli feedback list
  cre-owner-pp-cli feedback list --limit 5
  cre-owner-pp-cli feedback list --json`,
