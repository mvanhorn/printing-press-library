...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"WEAVIATE_COLLECTIONS_CONFIG",
		"WEAVIATE_COLLECTIONS_CONFIG_DIR",
		"WEAVIATE_COLLECTIONS_DATA_DIR",
		"WEAVIATE_COLLECTIONS_STATE_DIR",
		"WEAVIATE_COLLECTIONS_CACHE_DIR",
		"WEAVIATE_COLLECTIONS_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
		"XDG_CACHE_HOME",
	...
	return home, filepath.Join(home, ".config", "weaviate-collections-pp-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "weaviate-collections-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("WEAVIATE_API_KEY", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "weaviate-collections-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "weaviate-collections-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "weaviate-collections-pp-cli", "credentials.toml")
