...
Resource scoping:
  ...
  weaviate-collections-pp-cli sync

  # Sync specific resources only
  weaviate-collections-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  weaviate-collections-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  weaviate-collections-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  weaviate-collections-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  weaviate-collections-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "weaviate-collections-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "properties_index":
		return db.UpsertPropertiesIndex(data)
	case "tokenize":
		return db.UpsertTokenize(data)
	case "schema":
		return db.UpsertSchema(data)
	case "shards":
		return db.UpsertShards(data)
	case "tenants":
		return db.UpsertTenants(data)
	case "vectors_index":
		return db.UpsertVectorsIndex(data)
	default:
...
func responsePathForResource(resource, path string) []string {
	...
	case "indexes\x00/schema/{className}/indexes":
		return []string{"properties"}
	case "schema\x00/schema":
		return []string{"classes"}
	case "schema\x00/schema/{className}":
		return []string{"properties"}
	}
	return nil
}
