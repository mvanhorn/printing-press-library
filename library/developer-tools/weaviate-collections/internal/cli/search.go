...
Run sync first to populate the local search index.`,
  ...
  weaviate-collections-pp-cli search "status"

  # Force local search explicitly
  weaviate-collections-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  weaviate-collections-pp-cli search "status" --type indexes --data-source local
  # JSON output for piping
  weaviate-collections-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'weaviate-collections-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "indexes":
				results, err = db.SearchIndexes(query, limit)
			case "properties":
				results, err = db.SearchProperties(query, limit)
			case "schema":
				results, err = db.SearchSchema(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchIndexes(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search indexes failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchProperties(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search properties failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchSchema(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search schema failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
