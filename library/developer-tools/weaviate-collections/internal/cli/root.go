...
type rootFlags struct {
	...
	// invocation. Mirrors the WEAVIATE_COLLECTIONS_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "weaviate-collections-pp-cli",
		Short: `Weaviate Collections CLI — Full collection-config control for Weaviate Cloud, plus schema history, drift diffing, and lint that no other Weaviate…`,
		Long: `Weaviate Collections CLI — Full collection-config control for Weaviate Cloud, plus schema history, drift diffing, and lint that no other Weaviate tool has.
...
Health check: run 'weaviate-collections-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("weaviate-collections-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'weaviate-collections-pp-cli profile list')")
	...
	rootCmd.AddCommand(newShardsCmd(flags))
	...
	rootCmd.AddCommand(newNovelCollectionsCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newIndexesPromotedCmd(flags))
