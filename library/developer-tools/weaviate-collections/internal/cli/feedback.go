...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("WEAVIATE_COLLECTIONS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("WEAVIATE_COLLECTIONS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "weaviate-collections-pp-cli/feedback")
...
When ` + "`WEAVIATE_COLLECTIONS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`WEAVIATE_COLLECTIONS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "weaviate-collections-pp-cli",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  weaviate-collections-pp-cli feedback list
  weaviate-collections-pp-cli feedback list --limit 5
  weaviate-collections-pp-cli feedback list --json`,
