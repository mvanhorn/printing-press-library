...
// weaviate-collections-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/developer-tools/weaviate-collections/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/developer-tools/weaviate-collections/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/developer-tools/weaviate-collections/internal/learn/patterns"
...
const noLearnEnvVar = "WEAVIATE_COLLECTIONS_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("weaviate-collections-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  weaviate-collections-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  weaviate-collections-pp-cli recall "<question>" --agent
  weaviate-collections-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  weaviate-collections-pp-cli learnings list --agent
  weaviate-collections-pp-cli learnings list --query "<substring>"
  weaviate-collections-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  weaviate-collections-pp-cli learnings forget "<question>" --resource <id>
  weaviate-collections-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  weaviate-collections-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  weaviate-collections-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
