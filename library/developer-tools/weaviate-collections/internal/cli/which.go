...
var whichIndex = []whichEntry{
	{Command: "schema snapshot", Description: "Save a point-in-time copy of every collection's config to the local store, then browse history over time.", Group: "Local state that compounds", WhyItMatters: "Use before any risky schema change so you have a rollback reference."},
	{Command: "schema diff", Description: "Diff a collection's live config against a saved snapshot or another collection.", Group: "Local state that compounds", WhyItMatters: "Catch unintended config drift between environments or over time."},
	{Command: "collections lint", Description: "Flag risky collection configs: no vectorizer set, replication factor of 1, unindexed high-cardinality properties.", Group: "Reachability mitigation", WhyItMatters: "Catch production-readiness gaps before they bite in an incident."},
	{Command: "tenants audit", Description: "See tenant counts and activity status across every collection in one view.", Group: "Agent-native plumbing", WhyItMatters: "Spot inactive or overloaded tenants across a multi-tenant deployment at a glance."},
	{Command: "schema export", Description: "Export every collection's config as one portable JSON bundle for backup or promotion to another environment.", Group: "Agent-native plumbing", WhyItMatters: "Promote a schema from staging to production, or restore after an incident, without full data backup."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  weaviate-collections-pp-cli which "stale tickets"
  weaviate-collections-pp-cli which "bottleneck"
  weaviate-collections-pp-cli which --limit 1 "send message"
  weaviate-collections-pp-cli which                                # list the full capability index`,
