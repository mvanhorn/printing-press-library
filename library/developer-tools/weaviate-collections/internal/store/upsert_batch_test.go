...
func TestSearchIndexesQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if stored, failed, err := s.UpsertBatch("indexes", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	} else if failed != 0 || stored != len(items) {
		t.Fatalf("UpsertBatch stored=%d failed=%d, want stored=%d failed=0", stored, failed, len(items))
	}
	...
	for _, query := range []string{
		...
		results, err := s.SearchIndexes(query, 10)
		if err != nil {
			t.Fatalf("SearchIndexes(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchIndexes(%q) returned no results", query)
		}
	}
}
...
func TestSearchPropertiesQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if stored, failed, err := s.UpsertBatch("properties", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	} else if failed != 0 || stored != len(items) {
		t.Fatalf("UpsertBatch stored=%d failed=%d, want stored=%d failed=0", stored, failed, len(items))
	}
	...
	for _, query := range []string{
		...
		results, err := s.SearchProperties(query, 10)
		if err != nil {
			t.Fatalf("SearchProperties(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchProperties(%q) returned no results", query)
		}
	}
}

// TestUpsertBatch_PopulatesPropertiesIndexTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed properties_index table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesPropertiesIndexTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "properties_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "properties_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "properties_id": "test-parent-001"}`),
	}
	if _, _, err := s.UpsertBatch("properties_index", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "properties_index").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "properties_index")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count properties_index: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("properties_index count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandPropertiesIndexGeneric exercises
...
func TestUpsertBatch_TypedFailureDoesNotStrandPropertiesIndexGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// Items deliberately omit "properties_id" so the typed NOT NULL
	// constraint fires.
	items := []json.RawMessage{
	...
	stored, extractFailures, err := s.UpsertBatch("properties_index", items)
	if err != nil {
	...
	if typed != 0 {
		t.Fatalf("properties_index count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "properties_id")
	}
}

// TestUpsertBatch_KeysPropertiesIndexByChildAndParent verifies dependent
...
func TestUpsertBatch_KeysPropertiesIndexByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "shared_child", "properties_id": "parent_A"}`),
		json.RawMessage(`{"id": "shared", "properties_id": "child_parent_A"}`),
		json.RawMessage(`{"id": "shared_child", "properties_id": "parent_B"}`),
		json.RawMessage(`{"id": "other_child", "properties_id": "parent_A"}`),
	}
	stored, extractFailures, err := s.UpsertBatch("properties_index", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("properties_index count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "properties_id" = ?`, "properties_index")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by properties_id: %v", err)
	}
	if parentMatches != 2 {
		t.Fatalf("properties_id=parent_A count = %d, want 2", parentMatches)
	}
}

// TestUpsertBatch_PopulatesTokenizeTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed tokenize table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesTokenizeTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("tokenize", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "tokenize").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "tokenize")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count tokenize: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("tokenize count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandTokenizeGeneric exercises
// the savepoint isolation around the typed-table dispatch. The fixture omits
// the NOT NULL parent FK column so the typed insert fails; the savepoint
// rolls back only the typed projection. The generic resources row inserted
// just before must survive. Regression for issue #1392, where a single
// outer transaction caused typed-table failures to cascade and silently
// discard every successfully fetched API row.
func TestUpsertBatch_TypedFailureDoesNotStrandTokenizeGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	stored, extractFailures, err := s.UpsertBatch("tokenize", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v (typed-table failure must not propagate)", err)
	}
	if stored != len(items) {
	...
	if typed != 0 {
		t.Fatalf("tokenize count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "properties_id")
	}
}

// TestUpsertBatch_KeysTokenizeByChildAndParent verifies dependent
// sub-collection rows with the same child id but different parent ids do not
// overwrite one another in either the generic resources table or the typed
// table. Regression for issue #2471.
func TestUpsertBatch_KeysTokenizeByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("tokenize count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "properties_id" = ?`, "tokenize")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by properties_id: %v", err)
	}
	if parentMatches != 2 {
...
func TestSearchSchemaQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if stored, failed, err := s.UpsertBatch("schema", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	} else if failed != 0 || stored != len(items) {
		t.Fatalf("UpsertBatch stored=%d failed=%d, want stored=%d failed=0", stored, failed, len(items))
	}
	...
	for _, query := range []string{
		...
		results, err := s.SearchSchema(query, 10)
		if err != nil {
			t.Fatalf("SearchSchema(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchSchema(%q) returned no results", query)
		}
	}
}

// TestUpsertBatch_PopulatesShardsTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed shards table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesShardsTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("shards", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "shards").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "shards")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count shards: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("shards count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
...
// TestUpsertBatch_PopulatesVectorsIndexTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed vectors_index table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesVectorsIndexTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "vectors_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "vectors_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "vectors_id": "test-parent-001"}`),
	}
	if _, _, err := s.UpsertBatch("vectors_index", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "vectors_index").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "vectors_index")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count vectors_index: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("vectors_index count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandVectorsIndexGeneric exercises
// the savepoint isolation around the typed-table dispatch. The fixture omits
// the NOT NULL parent FK column so the typed insert fails; the savepoint
// rolls back only the typed projection. The generic resources row inserted
// just before must survive. Regression for issue #1392, where a single
// outer transaction caused typed-table failures to cascade and silently
// discard every successfully fetched API row.
func TestUpsertBatch_TypedFailureDoesNotStrandVectorsIndexGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// Items deliberately omit "vectors_id" so the typed NOT NULL
	// constraint fires.
	items := []json.RawMessage{
		json.RawMessage(`{"id": "orphan-001"}`),
		json.RawMessage(`{"id": "orphan-002"}`),
	...
	stored, extractFailures, err := s.UpsertBatch("vectors_index", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v (typed-table failure must not propagate)", err)
	}
	if stored != len(items) {
	...
	if typed != 0 {
		t.Fatalf("vectors_index count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "vectors_id")
	}
}

// TestUpsertBatch_KeysVectorsIndexByChildAndParent verifies dependent
// sub-collection rows with the same child id but different parent ids do not
// overwrite one another in either the generic resources table or the typed
// table. Regression for issue #2471.
func TestUpsertBatch_KeysVectorsIndexByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "shared_child", "vectors_id": "parent_A"}`),
		json.RawMessage(`{"id": "shared", "vectors_id": "child_parent_A"}`),
		json.RawMessage(`{"id": "shared_child", "vectors_id": "parent_B"}`),
		json.RawMessage(`{"id": "other_child", "vectors_id": "parent_A"}`),
	}
	stored, extractFailures, err := s.UpsertBatch("vectors_index", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("vectors_index count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "vectors_id" = ?`, "vectors_index")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by vectors_id: %v", err)
	}
	if parentMatches != 2 {
		t.Fatalf("vectors_id=parent_A count = %d, want 2", parentMatches)
	}
}
