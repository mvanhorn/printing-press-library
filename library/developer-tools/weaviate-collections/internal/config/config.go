...
type Config struct {
	...
	WeaviateApiKey   string          `toml:"api_key"`
}

func Load(configPath string) (*Config, error) {
	// No default BaseURL: every Weaviate Cloud cluster has a unique
	// per-customer hostname, so there is no shared default host that would
	// be correct for more than one user. Set WEAVIATE_COLLECTIONS_BASE_URL
	// (or base_url in the config file) to your cluster's URL, found in the
	...
	if v := os.Getenv("WEAVIATE_API_KEY"); v != "" {
		cfg.WeaviateApiKey = v
		cfg.markEnvOverride("WeaviateApiKey")
		cfg.AuthSource = "env:WEAVIATE_API_KEY"
		cfg.CredentialSource = "env:WEAVIATE_API_KEY"
	}
	...
	if v := os.Getenv("WEAVIATE_COLLECTIONS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("WEAVIATE_COLLECTIONS_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "weaviate-collections-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.WeaviateApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:WEAVIATE_API_KEY"
		}
		return "Bearer " + c.WeaviateApiKey
	}
	if c.AccessToken != "" {
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		WeaviateApiKey: c.WeaviateApiKey,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.WeaviateApiKey = creds.WeaviateApiKey
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.WeaviateApiKey = ""
	delete(c.envOverrides, "WeaviateApiKey")
	c.updateFileConfigField("WeaviateApiKey")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["WeaviateApiKey"] {
			out.WeaviateApiKey = c.fileConfig.WeaviateApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "WeaviateApiKey":
		c.fileConfig.WeaviateApiKey = c.WeaviateApiKey
	}
}
