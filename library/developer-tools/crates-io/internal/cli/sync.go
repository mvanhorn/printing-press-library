...
Resource scoping:
  ...
  crates-io-pp-cli sync

  # Sync specific resources only
  crates-io-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  crates-io-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  crates-io-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  crates-io-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  crates-io-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "crates-io-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "reverse_dependencies":
		return db.UpsertReverseDependencies(data)
	case "versions":
