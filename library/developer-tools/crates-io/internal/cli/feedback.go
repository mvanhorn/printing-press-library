...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CRATES_IO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CRATES_IO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "crates-io-pp-cli/feedback")
...
When ` + "`CRATES_IO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CRATES_IO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  crates-io-pp-cli feedback list
  crates-io-pp-cli feedback list --limit 5
  crates-io-pp-cli feedback list --json`,
