...
func TestUpsertBatch_PopulatesDependenciesTable(t *testing.T) {
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "crates_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "crates_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "crates_id": "test-parent-001"}`),
	}
...
func TestUpsertBatch_TypedFailureDoesNotStrandDependenciesGeneric(t *testing.T) {
	...
	// Items deliberately omit "crates_id" so the typed NOT NULL
	// constraint fires.
	items := []json.RawMessage{
	...
	if typed != 0 {
		t.Fatalf("dependencies count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "crates_id")
	}
}
...
func TestUpsertBatch_KeysDependenciesByChildAndParent(t *testing.T) {
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "shared_child", "crates_id": "parent_A"}`),
		json.RawMessage(`{"id": "shared", "crates_id": "child_parent_A"}`),
		json.RawMessage(`{"id": "shared_child", "crates_id": "parent_B"}`),
		json.RawMessage(`{"id": "other_child", "crates_id": "parent_A"}`),
	}
	stored, extractFailures, err := s.UpsertBatch("dependencies", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	...
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "crates_id" = ?`, "dependencies")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by crates_id: %v", err)
	}
	if parentMatches != 2 {
		t.Fatalf("crates_id=parent_A count = %d, want 2", parentMatches)
	}
}
...
func TestUpsertBatch_TypedFailureDoesNotStrandDownloadsGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("downloads count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "crates_id")
	}
}

// TestUpsertBatch_KeysDownloadsByChildAndParent verifies dependent
// sub-collection rows with the same child id but different parent ids do not
// overwrite one another in either the generic resources table or the typed
// table. Regression for issue #2471.
func TestUpsertBatch_KeysDownloadsByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("downloads count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "crates_id" = ?`, "downloads")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by crates_id: %v", err)
	}
	if parentMatches != 2 {
...
// TestUpsertBatch_TypedFailureDoesNotStrandOwnersGeneric exercises
// the savepoint isolation around the typed-table dispatch. The fixture omits
// the NOT NULL parent FK column so the typed insert fails; the savepoint
// rolls back only the typed projection. The generic resources row inserted
// just before must survive. Regression for issue #1392, where a single
// outer transaction caused typed-table failures to cascade and silently
// discard every successfully fetched API row.
func TestUpsertBatch_TypedFailureDoesNotStrandOwnersGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	stored, extractFailures, err := s.UpsertBatch("owners", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v (typed-table failure must not propagate)", err)
	}
	if stored != len(items) {
	...
	if typed != 0 {
		t.Fatalf("owners count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "crates_id")
	}
}

// TestUpsertBatch_KeysOwnersByChildAndParent verifies dependent
// sub-collection rows with the same child id but different parent ids do not
// overwrite one another in either the generic resources table or the typed
// table. Regression for issue #2471.
func TestUpsertBatch_KeysOwnersByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("owners count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "crates_id" = ?`, "owners")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by crates_id: %v", err)
	}
	if parentMatches != 2 {
...
// TestUpsertBatch_PopulatesReverseDependenciesTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed reverse_dependencies table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesReverseDependenciesTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("reverse_dependencies", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "reverse_dependencies").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "reverse_dependencies")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count reverse_dependencies: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("reverse_dependencies count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandReverseDependenciesGeneric exercises
// the savepoint isolation around the typed-table dispatch. The fixture omits
// the NOT NULL parent FK column so the typed insert fails; the savepoint
// rolls back only the typed projection. The generic resources row inserted
// just before must survive. Regression for issue #1392, where a single
// outer transaction caused typed-table failures to cascade and silently
// discard every successfully fetched API row.
func TestUpsertBatch_TypedFailureDoesNotStrandReverseDependenciesGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	stored, extractFailures, err := s.UpsertBatch("reverse_dependencies", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v (typed-table failure must not propagate)", err)
	}
	if stored != len(items) {
	...
	if typed != 0 {
		t.Fatalf("reverse_dependencies count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "crates_id")
	}
}

// TestUpsertBatch_KeysReverseDependenciesByChildAndParent verifies dependent
// sub-collection rows with the same child id but different parent ids do not
// overwrite one another in either the generic resources table or the typed
// table. Regression for issue #2471.
func TestUpsertBatch_KeysReverseDependenciesByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("reverse_dependencies count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "crates_id" = ?`, "reverse_dependencies")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by crates_id: %v", err)
	}
	if parentMatches != 2 {
...
func TestUpsertBatch_TypedFailureDoesNotStrandVersionsGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("versions count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "crates_id")
	}
}
...
func TestUpsertBatch_KeysVersionsByChildAndParent(t *testing.T) {
	...
	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "crates_id" = ?`, "versions")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by crates_id: %v", err)
	}
	if parentMatches != 2 {
