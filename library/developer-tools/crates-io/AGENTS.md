# Crates Io Printed CLI Agent Guide

This directory is a generated `crates-io-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
crates-io-pp-cli doctor --json
crates-io-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
crates-io-pp-cli which "<capability>" --json
crates-io-pp-cli <command> --help
```
...
```bash
crates-io-pp-cli <command> --agent
```
...
```bash
crates-io-pp-cli <command> --help
crates-io-pp-cli <command> --dry-run --agent
```
