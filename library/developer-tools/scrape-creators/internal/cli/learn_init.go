...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"platform": {
			{Canonical: "instagram", Values: []string{"ig", "insta"}},
			{Canonical: "youtube", Values: []string{"yt"}},
			{Canonical: "tiktok", Values: []string{"tt", "tik tok"}},
			{Canonical: "twitter", Values: []string{"x", "x.com"}},
			{Canonical: "facebook", Values: []string{"fb", "meta"}},
			{Canonical: "linkedin", Values: []string{"li"}},
			{Canonical: "truthsocial", Values: []string{"truth social", "truth"}},
			{Canonical: "applemusic", Values: []string{"apple music"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: scrape-creators-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: scrape-creators-pp-cli: learn init: %v\n", err)
		}
	})
}
