...
// metadata: the comments commands return {"comments": [...]} beside scalar
...
//
// Rule: never strip the only array in the response. A blocked key is spared on
// any []any (an empty comments: [] is still the answer "zero comments"); an
// unblocked key counts as a competing payload only when
// compactObjectArrayValue accepts it (non-empty array of objects), so scalar
// or empty sidecar arrays cannot re-trigger the loss.
