...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("find_the_gaps_before_spending",
			mcplib.WithDescription("Ranks synced posts by missing-thread gap so reply credits go only where threads are incomplete."),
			mcplib.WithString("handle", mcplib.Required(), mcplib.Description("Override the recipe's positional handle value.")),
		),
		handleFindTheGapsBeforeSpending,
	)
}

// handleFindTheGapsBeforeSpending runs the find_the_gaps_before_spending recipe intent: Ranks synced posts by missing-thread gap so reply credits go only where threads are incomplete.
func handleFindTheGapsBeforeSpending(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	var missingHandle bool
	args, missingHandle = appendRecipePositional(args, input["handle"], true)
	if missingHandle {
