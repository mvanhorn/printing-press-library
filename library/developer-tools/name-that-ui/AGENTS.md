# NameThatUI Printed CLI Agent Guide

This directory is a generated `name-that-ui-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
name-that-ui-pp-cli doctor --json
name-that-ui-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
name-that-ui-pp-cli which "list UI components" --json
name-that-ui-pp-cli catalog list --help
```
...
NameThatUI remote operations are read-only. Local helpers may write only local state (for example sync/public mirrors, saved profiles, and teach/playbook/feedback records); output delivery can write a local file or POST to an explicitly selected webhook. Inspect unfamiliar local-write or delivery commands and prefer a dry run when available:

```bash
name-that-ui-pp-cli catalog list --help
name-that-ui-pp-cli catalog list --dry-run --agent
```

`--dry-run` is a no-network preview: it does not auto-refresh or write NameThatUI mirror or snapshot data.

Use `--yes --no-input` only after the target, arguments, and local or delivery side effects are clear.
...
3. After answering, always fire `name-that-ui-pp-cli teach --query "<question>" --resource <id> --resource-type <type> &` in the background - teaching is unconditional and is the anchor that triggers playbook synthesis. Teach the structural question with identifiers stripped (no names, emails, phone numbers, account ids); the CLI warns on obvious PII shapes but does not block.
...
Disable the loop with `--no-learn` per-invocation or `NAME_THAT_UI_NO_LEARN=true` for the whole session - useful for deterministic agent flows that don't want a learning row to silently change subsequent query results.
