...
func TestClientResponseBodyLimitIncludesGzipExpansion(t *testing.T) {
	tests := []struct {
		name         string
		body         []byte
		gzip         bool
		...
		{"normal", []byte(`{"ok":true}`), false, false},
		{"oversized", bytes.Repeat([]byte("x"), int(MaxResponseBodyBytes)+1), false, true},
		{"gzip expansion", bytes.Repeat([]byte("z"), int(MaxResponseBodyBytes)+1), true, true},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if got := errors.As(err, &tooLarge); got != tc.wantOversize {
				t.Fatalf("oversize = %v, error = %v", got, err)
			}
		})
	}
}
