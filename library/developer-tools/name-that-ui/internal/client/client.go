...
// MaxResponseBodyBytes is the maximum decoded HTTP response body accepted by
// the generated client. The standard transport transparently decompresses gzip
// responses, so the limit is deliberately applied while reading resp.Body,
// after decompression, rather than to Content-Length or wire bytes.
const MaxResponseBodyBytes int64 = 8 << 20

// ResponseTooLargeError is returned when a decoded response exceeds
// MaxResponseBodyBytes. Callers can recognize it with errors.As without
// relying on a brittle error string.
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "name-that-ui-pp-cli/0.1.0")
		}
