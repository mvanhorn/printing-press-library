...
func TestMCPPathResolutionMatchesCLIResolverWithHomeEnv(t *testing.T) {
	...
	t.Setenv("NAME_THAT_UI_HOME", root)
...
func TestMCPPathResolutionMatchesCLIResolverWithPlatformDefaults(t *testing.T) {
	...
	if want := filepath.Join(home, ".config", "name-that-ui-pp-cli", "config.toml"); cfg.Path != want {
	...
	if want := filepath.Join(home, ".local", "share", "name-that-ui-pp-cli", "data.db"); gotDB != want {
...
func resetMCPPathEnv(t *testing.T) string {
	...
	for _, name := range []string{
		"NAME_THAT_UI_CONFIG",
		"NAME_THAT_UI_CONFIG_DIR",
		"NAME_THAT_UI_DATA_DIR",
		"NAME_THAT_UI_STATE_DIR",
		"NAME_THAT_UI_CACHE_DIR",
		"NAME_THAT_UI_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
...
func TestMCPRegisterToolsPreservesTypedSpecialTools(t *testing.T) {
	s := server.NewMCPServer("name-that-ui", "test")
	RegisterTools(s)
...
func TestMCPSQLBoundsRowsBytesAndContext(t *testing.T) {
	resetMCPPathEnv(t)
	path, err := mcpDBPath()
	if err != nil {
		t.Fatal(err)
	...
	for i := 0; i <= mcpSQLMaxRows; i++ {
		if err := db.Upsert("catalog", fmt.Sprintf("id-%d", i), []byte(`{"name":"item"}`)); err != nil {
			t.Fatal(err)
		}
	}
	if err := db.Upsert("styles", "large", []byte(`{"body":"`+strings.Repeat("x", mcpSQLMaxBytes)+`"}`)); err != nil {
		t.Fatal(err)
	}
	if err := db.Close(); err != nil {
		t.Fatal(err)
	}
	call := func(ctx context.Context, query string) string {
		t.Helper()
		result, err := handleSQL(ctx, mcplib.CallToolRequest{Params: mcplib.CallToolParams{Arguments: map[string]any{"query": query}}})
		if err != nil {
			t.Fatal(err)
		}
		if result == nil || !result.IsError {
			t.Fatalf("query %q should be bounded error", query)
		}
		return mcpTextContent(t, result)
	}
	if text := call(context.Background(), "SELECT id FROM resources"); !strings.Contains(text, "maximum") {
		t.Fatalf("row limit error = %q", text)
	}
	if text := call(context.Background(), "SELECT data FROM resources WHERE id = 'large'"); !strings.Contains(text, "SQLite value limit") {
		t.Fatalf("database length-limit error = %q", text)
	}
	if text := call(context.Background(), fmt.Sprintf("SELECT hex(zeroblob(%d)) AS value", mcpSQLMaxBytes/2-4)); !strings.Contains(text, "byte result budget") {
		t.Fatalf("post-encoding byte limit error = %q", text)
	}
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	if text := call(ctx, "WITH RECURSIVE n(x) AS (SELECT 1 UNION ALL SELECT x + 1 FROM n) SELECT x FROM n"); !strings.Contains(strings.ToLower(text), "context canceled") {
		t.Fatalf("cancelled query error = %q", text)
	}
}

func TestMCPSQLRejectsGeneratedLargeCellAtSQLiteBoundary(t *testing.T) {
	resetMCPPathEnv(t)
	path, err := mcpDBPath()
	if err != nil {
		t.Fatal(err)
	...
	result, err := handleSQL(context.Background(), mcplib.CallToolRequest{Params: mcplib.CallToolParams{Arguments: map[string]any{
		"query": fmt.Sprintf("SELECT hex(zeroblob(%d))", mcpSQLMaxBytes),
	}}})
	if err != nil {
		t.Fatal(err)
	}
	if result == nil || !result.IsError {
		t.Fatalf("generated large cell result = %#v, want bounded error", result)
	}
	if text := mcpTextContent(t, result); !strings.Contains(text, "SQLite value limit") {
		t.Fatalf("generated large cell error = %q, want SQLite value-limit rejection", text)
	}
}

func TestMCPSQLNormalQuery(t *testing.T) {
	resetMCPPathEnv(t)
	path, err := mcpDBPath()
	if err != nil {
		t.Fatal(err)
	...
	if err := db.Upsert("catalog", "button", []byte(`{"name":"Button"}`)); err != nil {
		t.Fatal(err)
	}
	if err := db.Close(); err != nil {
		t.Fatal(err)
	}
	result, err := handleSQL(context.Background(), mcplib.CallToolRequest{Params: mcplib.CallToolParams{Arguments: map[string]any{"query": "SELECT id FROM resources WHERE resource_type = 'catalog'"}}})
	if err != nil || result == nil || result.IsError {
		t.Fatalf("normal query = %#v, %v", result, err)
	}
	if text := mcpTextContent(t, result); !strings.Contains(text, "button") {
		t.Fatalf("normal query result = %q", text)
	}
}

func TestMCPPublicAccessGuidanceDoesNotClaimCredentials(t *testing.T) {
	message := publicMCPAccessError(401, "GET / returned HTTP 401")
	if strings.Contains(message, "check your API credentials") || !strings.Contains(message, "public-site policy") || !strings.Contains(message, "connectivity and local state") {
		t.Fatalf("401 guidance = %q", message)
	}
}
