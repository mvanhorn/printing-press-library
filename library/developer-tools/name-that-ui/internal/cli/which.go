...
var whichIndex = []whichEntry{
	{Command: "context-pack", Description: "Assemble one bounded, source-backed implementation packet for a component, optional style, and target framework.", Group: "Agent-ready guidance", WhyItMatters: "Use this when an agent needs enough precise context to implement or repair a UI without hopping across reference pages."},
	{Command: "lint", Description: "Find colloquial or ambiguous UI terms in prose and return canonical, source-backed candidates without rewriting the file.", Group: "Project-aware reference", WhyItMatters: "Use this before coding when tickets, prompts, or design specs may use names that send an agent toward the wrong component."},
	{Command: "crosswalk", Description: "See one UI concept across plain language, component parts, AppKit, SwiftUI, and ARIA or HTML terminology.", Group: "Agent-ready guidance", WhyItMatters: "Use this when product language must become exact framework terminology without relying on a model's memory."},
	{Command: "inventory", Description: "Map UI API symbols in a source tree to canonical components and source references.", Group: "Project-aware reference", WhyItMatters: "Use this to ground a UI repair or design-system review in the components that actually appear in the project."},
	{Command: "impact", Description: "Show which project files may be affected by component or style guidance changes since a prior snapshot.", Group: "Guidance over time", WhyItMatters: "Use this after syncing updates to focus review on code whose source-backed guidance actually changed."},
}
...
func newWhichCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `which resolves a natural-language capability query (for example, "map UI symbols" or "source-backed implementation packet") to the best matching command from this CLI's curated feature index.

Exit codes:
  0  at least one match found
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  name-that-ui-pp-cli which "map UI symbols"
		  name-that-ui-pp-cli which "lint an interface specification"
		  name-that-ui-pp-cli which --limit 1 "component implementation packet"
  name-that-ui-pp-cli which                                # list the full capability index`,
