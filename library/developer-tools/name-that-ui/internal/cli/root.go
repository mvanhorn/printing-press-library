...
type rootFlags struct {
	...
	// invocation. Mirrors the NAME_THAT_UI_NO_LEARN env var.
	noLearn       bool
	selectFields  string
	configPath    string
	homePath      string
...
func Execute() (retErr error) {
	...
	if err == nil && flags.deliverBuf != nil {
		if derr := DeliverContext(rootCmd.Context(), flags.deliverSink, flags.deliverBuf.Bytes(), flags.compact, flags.timeout); derr != nil {
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "name-that-ui-pp-cli",
		Short: `NameThatUI CLI — Turn vague UI language into exact, source-backed component and style guidance that coding agents can use directly.`,
		Long: `NameThatUI CLI — Turn vague UI language into exact, source-backed component and style guidance that coding agents can use directly.
...
Health check: run 'name-that-ui-pp-cli doctor' to verify connectivity and local state.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("name-that-ui-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'name-that-ui-pp-cli profile list')")
	...
	rootCmd.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
		...
		if resources, isRead := readCommandResources[cmd.CommandPath()]; isRead {
			if flags.dryRun {
				// A global dry run is a no-network, no-mirror preview. In
				// particular, do not let the root freshness hook create a store
				// or invoke the generated/domain refresh paths before RunE sees
				// the command's dry-run flag.
				flags.freshnessMeta = cliutil.FreshnessMeta{Decision: "skipped", Reason: "dry_run", Resources: append([]string(nil), resources...), Source: flags.dataSource}
			} else {
				flags.freshnessMeta = autoRefreshIfStaleAtPath(cmd.Context(), flags, resources, autoRefreshDBPath(cmd))
			}
		}
	...
	rootCmd.AddCommand(newStylesCmd(flags))
	...
	rootCmd.AddCommand(newNovelContextPackCmd(flags))
	rootCmd.AddCommand(newNovelCrosswalkCmd(flags))
	rootCmd.AddCommand(newNovelImpactCmd(flags))
	rootCmd.AddCommand(newNovelInventoryCmd(flags))
	rootCmd.AddCommand(newNovelLintCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newSemanticSearchPromotedCmd(flags))
...
// autoRefreshDBPath returns the command-selected local store when a command
// exposes --db. Commands without that local override continue using the
// canonical data store.
func autoRefreshDBPath(cmd *cobra.Command) string {
	if cmd != nil {
		if flag := cmd.Flags().Lookup("db"); flag != nil {
			return componentDBPath(flag.Value.String())
		}
	}
