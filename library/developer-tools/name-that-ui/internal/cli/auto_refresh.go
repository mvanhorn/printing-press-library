...
var readCommandResources = map[string][]string{
	"name-that-ui-pp-cli catalog":      {"catalog"},
	"name-that-ui-pp-cli catalog list": {"catalog"},
	"name-that-ui-pp-cli catalog get":  {"catalog"},
	"name-that-ui-pp-cli styles":       {"styles"},
	"name-that-ui-pp-cli styles list":  {"styles"},
	"name-that-ui-pp-cli styles get":   {"styles"},
	...
	"name-that-ui-pp-cli identify": {"catalog"},
}
...
func cachePolicy() cliutil.Policy {
	...
	envOptOut := "NAME_THAT_UI_NO_AUTO_REFRESH"
...
func autoRefreshIfStale(ctx context.Context, flags *rootFlags, resources []string) (meta cliutil.FreshnessMeta) {
	return autoRefreshIfStaleAtPath(ctx, flags, resources, defaultDBPath("name-that-ui-pp-cli"))
}

// autoRefreshIfStaleAtPath is the command-aware form of auto refresh. Local
// commands may select a store with --db; the lock, freshness decision,
// generated refresh, and domain enrichment must all use that same store.
func autoRefreshIfStaleAtPath(ctx context.Context, flags *rootFlags, resources []string, dbPath string) (meta cliutil.FreshnessMeta) {
	started := time.Now()
	...
	// The lock covers opening the store, deciding staleness, and refreshing.
	// A fresh HOME has no SQLite file yet, so allowing multiple processes to
	// make the decision concurrently can race the initial migration as well as
	// duplicate the network refresh. Waiters acquire the lock only after the
	// owner has finished, then open and re-check the sync state below.
	refreshLock, err := acquireAutoRefreshLock(ctx, dbPath)
	if err != nil {
		fmt.Fprintf(os.Stderr, "warning: auto-refresh skipped (lock: %v)\n", err)
		meta.Decision = "error"
		meta.Reason = "lock_failed"
	...
	if err := runAutoRefresh(refreshCtx, flags, db, resources); err != nil {
		fmt.Fprintf(os.Stderr, "warning: using stale name-that-ui-pp-cli cache (refresh failed: %v)\n", err)
...
func runAutoRefresh(ctx context.Context, flags *rootFlags, db *store.Store, resources []string) error {
	...
	// The generated resources are only an API cache. The hand-authored
	// NameThatUI commands read a richer components/style_details mirror and
	// immutable snapshots, so auto-refresh must maintain both representations
	// just like the explicit sync command does. The mirror deliberately remains
	// disabled in Printing Press verification mode because that mode supplies
	// JSON fixtures rather than the HTML the domain parser consumes.
	...
	if (includeComponents || includeStyles) && shouldRunNameThatUIEnrichment() {
		if _, err := namethatui.Sync(ctx, c.HTTPClient, c.BaseURL, db, includeComponents, includeStyles); err != nil {
			failures = append(failures, fmt.Sprintf("NameThatUI domain mirror: %v", err))
		}
	}
	if len(failures) > 0 {
