...
Run 'api <interface>' to see that interface's methods.`,
  ...
  name-that-ui-pp-cli api

  # Inspect one supported interface
  name-that-ui-pp-cli api catalog`,
			...
			if len(args) > 0 {
				...
				for _, child := range root.Commands() {
					if child.Hidden && strings.ToLower(child.Name()) == target {
						...
						fmt.Fprintf(cmd.OutOrStdout(), "\nUse '%s-pp-cli %s <method> --help' for details.\n", "name-that-ui", child.Name())
						return nil
					}
				}
				return fmt.Errorf("interface %q not found. Run '%s-pp-cli api' to list all interfaces", args[0], "name-that-ui")
			}
