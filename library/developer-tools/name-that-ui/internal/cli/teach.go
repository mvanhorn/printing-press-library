...
// name-that-ui-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/developer-tools/name-that-ui/internal/learn/patterns"
...
const noLearnEnvVar = "NAME_THAT_UI_NO_LEARN"
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  name-that-ui-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  name-that-ui-pp-cli recall "<question>" --agent
  name-that-ui-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  name-that-ui-pp-cli learnings list --agent
  name-that-ui-pp-cli learnings list --query "<substring>"
  name-that-ui-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  name-that-ui-pp-cli learnings forget "<question>" --resource <id>
  name-that-ui-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  name-that-ui-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  name-that-ui-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
