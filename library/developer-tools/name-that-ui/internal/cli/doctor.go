...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  name-that-ui-pp-cli doctor
  name-that-ui-pp-cli doctor --json
  name-that-ui-pp-cli doctor --fail-on warn
  name-that-ui-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Check API connectivity. Public NameThatUI does not require
			// credentials; doctor reports connectivity and local state only.
			//
			...
			if flags.dryRun {
				report["api"] = "not checked (dry run)"
			...
			for _, key := range []string{"config_path", "base_url", "version"} {
	...
	cmd.Flags().StringVar(&failOn, "fail-on", "", "Exit non-zero for selected health gates. stale: cache freshness plus errors; warn: local-state warnings plus errors; error: errors only. Default is never.")
	return cmd
}
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'name-that-ui-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'name-that-ui-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'name-that-ui-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'name-that-ui-pp-cli sync' to refresh."
	}
	return report
}
