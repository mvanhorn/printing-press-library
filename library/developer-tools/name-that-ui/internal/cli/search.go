...
In local mode: searches locally synced data only.`,
  ...
  name-that-ui-pp-cli search "error timeout"

  # Force local search only
  name-that-ui-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  name-that-ui-pp-cli search "status" --type catalog --data-source local
  # JSON output for piping
  name-that-ui-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /api/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/api/search", map[string]any{
					"q":          query,
					"mode":       "retrieve",
					"local":      []any{},
					"candidates": []any{},
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'name-that-ui-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
