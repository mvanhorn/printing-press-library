...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/developer-tools/name-that-ui/internal/learn/entities"
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"ui_element": {
			{Canonical: "Combobox (Autocomplete / Typeahead)", Values: []string{"combobox", "autocomplete", "typeahead", "searchable dropdown"}},
			{Canonical: "Menu Bar Extra (Status Item)", Values: []string{"menu bar icon", "status item", "tray icon"}},
			{Canonical: "Toast (Snackbar)", Values: []string{"toast", "snackbar", "brief notification"}},
			{Canonical: "Modal Dialog vs. Drawer vs. Sheet", Values: []string{"modal", "dialog", "drawer", "bottom sheet"}},
			{Canonical: "Badge vs. Chip vs. Pill vs. Tag", Values: []string{"badge", "chip", "pill", "tag"}},
		},
		"visual_style": {
			{Canonical: "Glassmorphism", Values: []string{"frosted glass", "frosted cards"}},
			{Canonical: "Liquid Glass", Values: []string{"apple glass", "watery controls"}},
			{Canonical: "Neobrutalism", Values: []string{"neo brutalism", "hard shadows", "thick black outlines"}},
			{Canonical: "Skeuomorphism", Values: []string{"realistic interface", "real-world materials"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: name-that-ui-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: name-that-ui-pp-cli: learn init: %v\n", err)
		}
	})
}
