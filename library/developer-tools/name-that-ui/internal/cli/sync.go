...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/developer-tools/name-that-ui/internal/learn/lookups"
...
// lockedSyncWriter keeps concurrent resource workers from interleaving or
// racing their NDJSON events. Machine stdout must remain both parseable and
// race-free when sync runs with the default parallel worker count.
type lockedSyncWriter struct {
	mu sync.Mutex
	w  io.Writer
}

func (w *lockedSyncWriter) Write(p []byte) (int, error) {
	w.mu.Lock()
	defer w.mu.Unlock()
	return w.w.Write(p)
}
...
Resource scoping:
  ...
  from a prior sync.`,
		Example: `  # Sync all public resources
  name-that-ui-pp-cli sync

  # Sync the public catalog and styles
  name-that-ui-pp-cli sync --resources catalog,styles

  # Fully rebuild those public resources
  name-that-ui-pp-cli sync --resources catalog,styles --full

  # Parallel sync with 8 workers
  name-that-ui-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  name-that-ui-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("name-that-ui-pp-cli")
			}
			...
			syncEventWriter := io.Writer(&lockedSyncWriter{w: cmd.OutOrStdout()})
			...
			includeComponents := containsSyncResource(resources, "catalog")
			includeStyles := containsSyncResource(resources, "styles")
			if (includeComponents || includeStyles) && shouldRunNameThatUIEnrichment() {
				if c.DryRun {
					// Keep machine stdout entirely structured. This is diagnostic
					// prose, so human callers receive it on stderr instead.
					fmt.Fprintln(cmd.ErrOrStderr(), "NameThatUI domain mirror preview: components/styles enrichment skipped (--dry-run; no fetches or writes).")
				} else {
					report, err := namethatui.Sync(cmd.Context(), c.HTTPClient, c.BaseURL, db, includeComponents, includeStyles)
					if err != nil {
						return fmt.Errorf("syncing NameThatUI domain mirror: %w", err)
					}
					if humanFriendly {
						fmt.Fprintf(os.Stderr, "  NameThatUI mirror: %d components, %d styles synced\n", report.Components, report.Styles)
					}
				}
			}
	...
	cmd.Flags().StringVar(&since, "since", "", "Accepted duration (e.g. 7d, 24h, 1w, 30m), but has no effect for catalog or styles because their public endpoints declare no temporal filter")
...
func containsSyncResource(resources []string, target string) bool {
	for _, resource := range resources {
		if resource == target {
			return true
		}
...
// shouldRunNameThatUIEnrichment keeps the hand-authored HTML mirror out of
// Printing Press's mock-only verification environment. Generic sync still
// exercises the verifier's mock server before this enrichment would run.
func shouldRunNameThatUIEnrichment() bool {
...
func syncResource(ctx context.Context, c interface {
	...
	for {
		...
		if opts, ok := syncHTMLExtractionOptions(resource, "", c.RequestBaseURL(), c.LastContentType(), path, params); ok && !(cliutil.IsVerifyEnv() && isJSONResponse(data)) {
