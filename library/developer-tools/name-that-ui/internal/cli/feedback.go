...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NAME_THAT_UI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NAME_THAT_UI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(ctx context.Context, url string, entry FeedbackEntry, timeout time.Duration) error {
	...
	req.Header.Set("User-Agent", "name-that-ui-pp-cli/feedback")
	if timeout <= 0 {
...
When ` + "`NAME_THAT_UI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NAME_THAT_UI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			var text string
			if useStdin {
				data, err := io.ReadAll(io.LimitReader(cmd.InOrStdin(), feedbackMaxTextLen+1))
				if err != nil {
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  name-that-ui-pp-cli feedback list
  name-that-ui-pp-cli feedback list --limit 5
  name-that-ui-pp-cli feedback list --json`,
