...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: the public site may be applying endpoint policy or bot protection."+
			"\n      Run 'name-that-ui-pp-cli doctor' to check connectivity and local state.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: the public site may be applying rate limits, geography restrictions, bot protection, or endpoint policy."+
			"\n      Run 'name-that-ui-pp-cli doctor' to check connectivity.", err))
...
func printJSONFiltered(w io.Writer, v any, flags *rootFlags) error {
	return printJSONFilteredWithAgentMeta(w, v, flags, nil)
}

// printJSONFilteredWithAgentMeta is the typed-value equivalent of generated
// endpoint output: callers that know a result came from a live request must
// pass that provenance through to the --agent envelope instead of accepting
// the local default used by hand-written local commands.
func printJSONFilteredWithAgentMeta(w io.Writer, v any, flags *rootFlags, agentMeta map[string]any) error {
...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return apiErr(fmt.Errorf("API returned unexpected HTML instead of JSON; the public site may be applying endpoint policy or bot protection. " +
