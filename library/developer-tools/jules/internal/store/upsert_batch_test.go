...
func TestUpsertBatch_TypedFailureDoesNotStrandActivitiesGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("activities count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "sessions_id")
	}
}
...
func TestUpsertBatch_KeysActivitiesByChildAndParent(t *testing.T) {
	...
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "sessions_id" = ?`, "activities")
...
// TestUpsertBatch_SetsActivitiesParentID verifies that dependent-resource
...
func TestUpsertBatch_SetsActivitiesParentID(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "child-001", "sessions_id": "parent-A", "parent_id": "parent-A"}`),
		json.RawMessage(`{"id": "child-002", "sessions_id": "parent-A", "parent_id": "parent-A"}`),
		json.RawMessage(`{"id": "child-003", "sessions_id": "parent-B", "parent_id": "parent-B"}`),
	}
	if _, _, err := s.UpsertBatch("activities", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var matchedA int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE parent_id = ?`, "activities")
...
// TestUpsertBatch_ActivitiesSearchableViaGenericFTS verifies that a
...
// non-FTS5 `--type activities` fallback), so populating it is the
...
func TestUpsertBatch_ActivitiesSearchableViaGenericFTS(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// field is matchable regardless of which columns activities declares.
	const probe = "zqxjpp2629probe"
	items := []json.RawMessage{
		json.RawMessage(`{"id": "search-001", "pp_fts_probe": "` + probe + `", "sessions_id": "parent-A", "parent_id": "parent-A"}`),
	}
	stored, _, err := s.UpsertBatch("activities", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	if len(results) == 0 {
		t.Fatalf("dependent resource %q stored but not searchable via generic resources_fts (issue #2629)", "activities")
	}
}
