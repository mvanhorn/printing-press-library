...
Run sync first to populate the local search index.`,
  ...
  jules-pp-cli search "status"

  # Force local search explicitly
  jules-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  jules-pp-cli search "status" --type sessions --data-source local
  # JSON output for piping
  jules-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'jules-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
