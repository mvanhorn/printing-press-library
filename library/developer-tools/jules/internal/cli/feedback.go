...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("JULES_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("JULES_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "jules-pp-cli/feedback")
...
When ` + "`JULES_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`JULES_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  # Record feedback locally
  jules-pp-cli feedback "the --dry-run output for archive wasn't valid JSON"

  # Record feedback and forward it to the configured endpoint
  jules-pp-cli feedback "sessions create needs a --repo shortcut" --send

  # Pipe feedback text in from stdin
  echo "checkpoint restore is confusing" | jules-pp-cli feedback --stdin`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  jules-pp-cli feedback list
  jules-pp-cli feedback list --limit 5
  jules-pp-cli feedback list --json`,
