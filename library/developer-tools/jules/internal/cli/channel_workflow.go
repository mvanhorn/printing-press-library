...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  jules-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  jules-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  jules-pp-cli workflow archive --timeout 0`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"sessions", "sources"}
			if cliutil.IsDogfoodEnv() {
				if len(resources) > 3 {
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  jules-pp-cli workflow status

  # Show status as JSON
  jules-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("jules-pp-cli")
			}
