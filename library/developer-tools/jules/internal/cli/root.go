...
type rootFlags struct {
	...
	// invocation. Mirrors the JULES_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "jules-pp-cli",
		Short: "Jules CLI with session monitoring, checkpointing, and compliance gating",
		Long: `Drive Jules (async coding task) sessions, sources, and activities from the
command line, with quota-aware dispatch, continuous session monitoring,
working-tree checkpoints, and pre-submission compliance/diff gating on top.

Add --agent to any command for JSON output + non-interactive mode.
Run 'jules-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("jules-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'jules-pp-cli profile list')")
