...
type Config struct {
	...
	JulesApiKey      string          `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("JULES_API_KEY"); v != "" {
		cfg.JulesApiKey = v
		cfg.markEnvOverride("JulesApiKey")
		cfg.AuthSource = "env:JULES_API_KEY"
		cfg.CredentialSource = "env:JULES_API_KEY"
	}
	...
	if v := os.Getenv("JULES_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("JULES_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "jules-pp-cli", "config.toml"), nil
}
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.JulesApiKey != "" {
		parts = append(parts, "api_key="+c.JulesApiKey)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.JulesApiKey == "" {
		c.JulesApiKey = creds.JulesApiKey
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.JulesApiKey = token
	delete(c.envOverrides, "JulesApiKey")
	c.updateFileConfigField("JulesApiKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["JulesApiKey"] {
			out.JulesApiKey = c.fileConfig.JulesApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "JulesApiKey":
		c.fileConfig.JulesApiKey = c.JulesApiKey
	}
}
