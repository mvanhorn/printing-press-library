...
func AdoptVerifiedLegacyDatabase(ctx context.Context, session *Session, state *LegacyMigrationState) error {
	...
	// SQLite's VACUUM INTO requires the destination as a string literal in
	// the SQL text itself; it does not accept a bound parameter for the
	// filename. targetPath comes from the resolved local data directory
	// (session.Paths.DataFile), not untrusted external input, and the
	// embedded single quotes are escaped per the SQL string-literal rule
	// (doubling them) before concatenation, so a quote in the path cannot
	// break out of the literal.
	quotedTarget := strings.ReplaceAll(targetPath, "'", "''")
	if _, err := source.ExecContext(ctx, "VACUUM INTO '"+quotedTarget+"'"); err != nil { // #nosec G202 -- see comment above; filename can't be a bound parameter here and is escaped
