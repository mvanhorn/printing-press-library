...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  n8n-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  n8n-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"community-packages", "credentials", "data-tables", "discover", "executions", "insights", "projects", "tags", "users", "variables", "workflows"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  n8n-pp-cli workflow status

  # Show status as JSON
  n8n-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("n8n-pp-cli")
			}
