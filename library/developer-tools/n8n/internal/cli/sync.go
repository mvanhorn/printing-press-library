...
Exit codes & warnings:
  ...
  n8n-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  n8n-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  n8n-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  n8n-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  n8n-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/n8n-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "credentials_transfer":
		return db.UpsertCredentialsTransfer(data)
	case "columns":
	...
	case "executions_tags":
		return db.UpsertExecutionsTags(data)
	case "folders":
	...
	case "workflows_tags":
		return db.UpsertWorkflowsTags(data)
	case "workflows_transfer":
		return db.UpsertWorkflowsTransfer(data)
	case "unarchive":
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "projects_users", ParentTable: "projects", ParentIDParam: "projectId", PathTemplate: "/projects/{projectId}/users", KeyField: ""},
		{Name: "rows", ParentTable: "data-tables", ParentIDParam: "dataTableId", PathTemplate: "/data-tables/{dataTableId}/rows", KeyField: ""},
	}
}
