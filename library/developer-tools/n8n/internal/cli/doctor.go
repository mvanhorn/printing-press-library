...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  n8n-pp-cli doctor
  n8n-pp-cli doctor --json
  n8n-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export N8N_API_KEY=<your-key> N8N_BASE_URL=https://n8n.example.com"
					report["auth_docs_url"] = "https://docs.n8n.io/api/"
				} else {
					authConfigured = true
			...
			if os.Getenv("N8N_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "N8N_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "N8N_API_KEY")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'n8n-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'n8n-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'n8n-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'n8n-pp-cli sync' to refresh."
	}
	return report
}
