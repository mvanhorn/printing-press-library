...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".n8n-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("N8N_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("N8N_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "n8n-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.n8n-pp-cli/feedback.jsonl.
When ` + "`N8N_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`N8N_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  n8n-pp-cli feedback list
  n8n-pp-cli feedback list --limit 5
  n8n-pp-cli feedback list --json`,
