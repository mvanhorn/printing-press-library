...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "tavily-pp-cli",
		Short: `Search the web, extract content, crawl sites, and run deep research from the terminal.`,
		Long: `Search the web, extract content, crawl sites, and run deep research from the terminal.
...
Health check: run 'tavily-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("tavily-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'tavily-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCrawlPromotedCmd(flags))
	rootCmd.AddCommand(newExtractPromotedCmd(flags))
	rootCmd.AddCommand(newMapPromotedCmd(flags))
	rootCmd.AddCommand(newUsagePromotedCmd(flags))
	rootCmd.AddCommand(newWebPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	// PATCH: novel transcendence commands wired here
	rootCmd.AddCommand(newQnACmd(flags))
	rootCmd.AddCommand(newContextCmd(flags))
	rootCmd.AddCommand(newLocalSearchCmd(flags))
	rootCmd.AddCommand(newCorpusCmd(flags))
	rootCmd.AddCommand(newDriftDetectCmd(flags))
	rootCmd.AddCommand(newBudgetWatchCmd(flags))
	rootCmd.AddCommand(newCostReportCmd(flags))
	rootCmd.AddCommand(newBatchPlanCmd(flags))
	rootCmd.AddCommand(newReplayCmd(flags))
	rootCmd.AddCommand(newFreshnessCheckCmd(flags))
	rootCmd.AddCommand(newCrawlResumeCmd(flags))
	rootCmd.AddCommand(newResearchTrackCmd(flags))
	rootCmd.AddCommand(newSourcesRankCmd(flags))
