...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".tavily-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TAVILY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TAVILY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "tavily-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.tavily-pp-cli/feedback.jsonl.
When ` + "`TAVILY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TAVILY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  tavily-pp-cli feedback list
  tavily-pp-cli feedback list --limit 5
  tavily-pp-cli feedback list --json`,
