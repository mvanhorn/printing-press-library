# Npm Printed CLI Agent Guide

This directory is a generated `npm-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
npm-pp-cli doctor --json
npm-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
npm-pp-cli which "<capability>" --json
npm-pp-cli <command> --help
```
...
```bash
npm-pp-cli <command> --help
npm-pp-cli <command> --dry-run --agent
```
