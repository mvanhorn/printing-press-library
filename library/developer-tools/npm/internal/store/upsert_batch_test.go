...
func TestResolveByNameRejectsUnsafeJSONField(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if err := s.Upsert("packages", "safe-id", json.RawMessage(`{"name":"safe"}`)); err != nil {
		t.Fatalf("upsert: %v", err)
	}

	if _, err := s.ResolveByName("packages", "safe", "name')) OR 1=1--"); err == nil {
		t.Fatal("ResolveByName accepted unsafe JSON field")
	}

	id, err := s.ResolveByName("packages", "safe", "name")
	if err != nil {
		t.Fatalf("ResolveByName safe field: %v", err)
	}
	if id != "safe-id" {
		t.Fatalf("ResolveByName id = %q, want safe-id", id)
	}
}
