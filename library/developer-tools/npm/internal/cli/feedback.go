...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".npm-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NPM_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NPM_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/developer-tools/npm/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.npm-pp-cli/feedback.jsonl.
When ` + "`NPM_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NPM_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  npm-pp-cli feedback list
  npm-pp-cli feedback list --limit 5
  npm-pp-cli feedback list --json`,
