...
Exit codes & warnings:
  ...
  npm-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  npm-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  npm-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  npm-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  npm-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/npm-pp-cli/data.db)")
