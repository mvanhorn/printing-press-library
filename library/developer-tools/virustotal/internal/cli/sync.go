...
Exit codes & warnings:
  ...
  virustotal-pp-cli sync

  # Sync specific resources only
  virustotal-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  virustotal-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  virustotal-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  virustotal-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  virustotal-pp-cli sync --latest-only`,
