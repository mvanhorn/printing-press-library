...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "virustotal-pp-cli",
		Short: "Manage virustotal resources via the virustotal API",
		Long: `Manage virustotal resources via the virustotal API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'virustotal-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("virustotal-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().BoolVar(&flags.llm, "llm", false, "Format output for LLM consumption (human-readable structured text)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'virustotal-pp-cli profile list')")
	...
	rootCmd.AddCommand(newUrlsCmd(flags))
	...
	rootCmd.AddCommand(newIpAddressesPromotedCmd(flags))
	rootCmd.AddCommand(newVirustotalSearchPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())
	// Transcendence features
	rootCmd.AddCommand(newPivotCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("virustotal-pp-cli %s\n", version)
		},
	}
}
