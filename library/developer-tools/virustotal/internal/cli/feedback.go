...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".virustotal-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("VIRUSTOTAL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("VIRUSTOTAL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/ca7ai/pp-virustotal/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.github.com/ca7ai/pp-virustotal/feedback.jsonl.
When ` + "`VIRUSTOTAL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`VIRUSTOTAL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  virustotal-pp-cli feedback list
  virustotal-pp-cli feedback list --limit 5
  virustotal-pp-cli feedback list --json`,
