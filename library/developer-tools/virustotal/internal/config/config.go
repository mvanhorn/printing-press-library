...
type Config struct {
	...
	Path           string `toml:"-"`
	VirustotalApiKey string `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("VIRUSTOTAL_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "virustotal-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("VIRUSTOTAL_API_KEY"); v != "" {
		...
		cfg.AuthSource = "env:VIRUSTOTAL_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.VirustotalApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("VIRUSTOTAL_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
