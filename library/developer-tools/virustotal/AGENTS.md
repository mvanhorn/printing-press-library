# Virustotal Printed CLI Agent Guide

This directory is a generated `virustotal-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
virustotal-pp-cli doctor --json
virustotal-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
virustotal-pp-cli which "<capability>" --json
virustotal-pp-cli <command> --help
```
...
```bash
virustotal-pp-cli <command> --agent
```
...
```bash
virustotal-pp-cli <command> --help
virustotal-pp-cli <command> --dry-run --agent
```
