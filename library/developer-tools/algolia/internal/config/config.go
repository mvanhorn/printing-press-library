...
type Config struct {
	...
	AlgoliaApiKey        string          `toml:"api_key"`
	AlgoliaApplicationId string          `toml:"application_id"`
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("ALGOLIA_API_KEY"); v != "" {
		cfg.AlgoliaApiKey = v
		cfg.markEnvOverride("AlgoliaApiKey")
		cfg.AuthSource = "env:ALGOLIA_API_KEY"
		cfg.CredentialSource = "env:ALGOLIA_API_KEY"
	}
	...
	if v := os.Getenv("ALGOLIA_APPLICATION_ID"); v != "" {
		cfg.AlgoliaApplicationId = v
		cfg.markEnvOverride("AlgoliaApplicationId")
	}
	...
	if v := os.Getenv("ALGOLIA_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("ALGOLIA_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "algolia-pp-cli", "config.toml"), nil
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		AlgoliaApiKey:        c.AlgoliaApiKey,
		AlgoliaApplicationId: c.AlgoliaApplicationId,
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.AlgoliaApiKey = token
	delete(c.envOverrides, "AlgoliaApiKey")
	c.updateFileConfigField("AlgoliaApiKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("AlgoliaApiKey")
	c.AlgoliaApplicationId = ""
	delete(c.envOverrides, "AlgoliaApplicationId")
	c.updateFileConfigField("AlgoliaApplicationId")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["AlgoliaApiKey"] {
			out.AlgoliaApiKey = c.fileConfig.AlgoliaApiKey
		}
		if c.envOverrides["AlgoliaApplicationId"] {
			out.AlgoliaApplicationId = c.fileConfig.AlgoliaApplicationId
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "AlgoliaApiKey":
		c.fileConfig.AlgoliaApiKey = c.AlgoliaApiKey
	case "AlgoliaApplicationId":
		c.fileConfig.AlgoliaApplicationId = c.AlgoliaApplicationId
	}
}
