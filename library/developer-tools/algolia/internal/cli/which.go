...
var whichIndex = []whichEntry{
	{Command: "find", Description: "Search across every synced index in one shot, with each hit labeled by its source index.", Group: "Local state that compounds", WhyItMatters: "Use this when you need to know which of your indices contains a record, without issuing N API calls."},
	{Command: "settings diff", Description: "Field-level comparison of settings between two indices (or a settings file vs an index).", Group: "Local state that compounds", WhyItMatters: "Use this to verify prod/staging parity before a release instead of exporting and eyeballing JSON."},
	{Command: "rules stale", Description: "Find rules that reference attributes missing from the index's searchable attributes or that can never match.", Group: "Local state that compounds", WhyItMatters: "Use this to find dead-weight relevance rules before they silently corrupt search results."},
	{Command: "apikeys report", Description: "Audit all API keys: write-capable, unrestricted, and expired keys grouped by ACL, with last-use from synced logs.", Group: "Local state that compounds", WhyItMatters: "Use this for key rotation audits to catch write-capable keys that have gone unused for months."},
	{Command: "search check", Description: "Assert that a query returns expected objectIDs, with a typed exit code for CI pipelines.", Group: "Agent-native plumbing", WhyItMatters: "Use this after every rule or synonym change to catch relevance regressions before they reach users."},
	{Command: "objects gaps", Description: "List records missing attributes required by the index's searchable settings — records search can never return.", Group: "Local state that compounds", WhyItMatters: "Use this after catalog imports to find records that are silently unreachable by search."},
	{Command: "objects diff", Description: "Compare records of two indices (added/removed/changed by objectID) to verify prod/staging parity.", Group: "Local state that compounds", WhyItMatters: "Use this to reconcile dev and prod copies after a copy/move operation instead of exporting and diffing files."},
	{Command: "logs errors", Description: "Aggregate synced log entries by error code and link failed tasks for a digest of what went wrong.", Group: "Agent-native plumbing", WhyItMatters: "Use this for weekly error triage to see error spikes and affected indices at a glance."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  algolia-pp-cli which "stale tickets"
  algolia-pp-cli which "bottleneck"
  algolia-pp-cli which --limit 1 "send message"
  algolia-pp-cli which                                # list the full capability index`,
