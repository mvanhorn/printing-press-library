...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ALGOLIA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ALGOLIA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "algolia-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Annotations: map[string]string{
			"pp:no-error-path-probe": "true", // feedback accepts arbitrary text; there is no invalid-argument case
		},
		Example: `  algolia-pp-cli feedback "the sync command was slow"
  algolia-pp-cli feedback list`,
		Long: `Feedback is captured locally first in the CLI data directory's feedback.jsonl.
When ` + "`ALGOLIA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ALGOLIA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  algolia-pp-cli feedback list
  algolia-pp-cli feedback list --limit 5
  algolia-pp-cli feedback list --json`,
