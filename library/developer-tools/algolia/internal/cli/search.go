...
In local mode: searches locally synced data only.`,
  ...
  algolia-pp-cli search "error timeout"

  # Force local search only
  algolia-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  algolia-pp-cli search "status" --type clusters --data-source local
  # JSON output for piping
  algolia-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /1/indexes/*/queries
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/1/indexes/*/queries", map[string]any{
					"q":        query,
					"requests": []any{},
					"strategy": "none",
				})
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'algolia-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
