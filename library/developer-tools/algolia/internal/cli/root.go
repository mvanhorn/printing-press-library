...
type rootFlags struct {
	...
	// invocation. Mirrors the ALGOLIA_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "algolia-pp-cli",
		Short: `Algolia CLI — Every Algolia feature, plus offline search and a local database no other Algolia tool has.`,
		Long: `Algolia CLI — Every Algolia feature, plus offline search and a local database no other Algolia tool has.
...
Health check: run 'algolia-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("algolia-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'algolia-pp-cli profile list')")
	...
	rootCmd.AddCommand(newDictionariesCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelApikeysCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelFindCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelObjectsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelRulesCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSettingsCmd(flags))
