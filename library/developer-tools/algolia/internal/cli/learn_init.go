...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^[a-zA-Z0-9]{10}$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: algolia-pp-cli: ticker pattern %q failed to compile: %v\n", `^[a-zA-Z0-9]{10}$`, err)
	} else {
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"index": {
			{Canonical: "algolia_movie_sample_dataset", Values: []string{"movies", "movie dataset", "sample dataset"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: algolia-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: algolia-pp-cli: learn init: %v\n", err)
		}
	})
}
