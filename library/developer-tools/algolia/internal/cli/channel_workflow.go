...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  algolia-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  algolia-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"clusters", "clusters-mapping", "clusters-mapping-top", "dictionaries", "indexes", "keys", "logs", "security"}
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  algolia-pp-cli workflow status

  # Show status as JSON
  algolia-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("algolia-pp-cli")
			}
