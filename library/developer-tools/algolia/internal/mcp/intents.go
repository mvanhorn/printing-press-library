...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("compare_prod_and_staging_settings",
			mcplib.WithDescription("See exactly which settings differ between two indices before a release."),
		...
		handleCompareProdAndStagingSettings,
	)
}

// handleCompareProdAndStagingSettings runs the compare_prod_and_staging_settings recipe intent: See exactly which settings differ between two indices before a release.
func handleCompareProdAndStagingSettings(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
