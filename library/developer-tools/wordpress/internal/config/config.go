...
type Config struct {
	...
	WordpressUser        string          `toml:"user"`
	WordpressAppPassword string          `toml:"app_password"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://wordpress.org/news/wp-json",
	}
	...
	// Env var overrides
	if v := os.Getenv("WORDPRESS_USER"); v != "" {
		cfg.WordpressUser = v
		cfg.markEnvOverride("WordpressUser")
		cfg.AuthSource = "env:WORDPRESS_USER"
		cfg.CredentialSource = "env:WORDPRESS_USER"
	}
	if v := os.Getenv("WORDPRESS_APP_PASSWORD"); v != "" {
		cfg.WordpressAppPassword = v
		cfg.markEnvOverride("WordpressAppPassword")
		cfg.AuthSource = "env:WORDPRESS_APP_PASSWORD"
		cfg.CredentialSource = "env:WORDPRESS_APP_PASSWORD"
	}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("WORDPRESS_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "wordpress-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.WordpressAppPassword == "" {
		return ""
	}
	credentials := basicAuthPayload("Basic {username}:{password}", map[string]string{
		"user":                   c.WordpressUser,
		"WORDPRESS_USER":         c.WordpressUser,
		"username":               c.WordpressUser,
		"app_password":           c.WordpressAppPassword,
		"WORDPRESS_APP_PASSWORD": c.WordpressAppPassword,
		"password":               c.WordpressAppPassword,
		"secret":                 c.WordpressAppPassword,
		"token":                  c.WordpressAppPassword,
		"access_token":           c.WordpressAppPassword,
	}, false)
	if credentials == "" {
		return ""
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		WordpressAppPassword: c.WordpressAppPassword,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.WordpressUser = creds.WordpressUser
	c.WordpressAppPassword = creds.WordpressAppPassword
}
...
func (c *Config) SaveCredentials(value0, value1 string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.WordpressUser = value0
	delete(c.envOverrides, "WordpressUser")
	c.updateFileConfigField("WordpressUser")
	c.WordpressAppPassword = value1
	delete(c.envOverrides, "WordpressAppPassword")
	c.updateFileConfigField("WordpressAppPassword")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["WordpressUser"] {
			out.WordpressUser = c.fileConfig.WordpressUser
		}
		if c.envOverrides["WordpressAppPassword"] {
			out.WordpressAppPassword = c.fileConfig.WordpressAppPassword
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "WordpressUser":
		c.fileConfig.WordpressUser = c.WordpressUser
	case "WordpressAppPassword":
		c.fileConfig.WordpressAppPassword = c.WordpressAppPassword
	}
}
