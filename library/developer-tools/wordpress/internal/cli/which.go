...
var whichIndex = []whichEntry{
	{Command: "fleet", Description: "See core version, pending updates, admin-account count and sync age for every site you manage, in one table.", Group: "Managing many sites at once", WhyItMatters: "Reach for this when the question spans more than one site. Every other command answers for a single site."},
	{Command: "diagnose", Description: "Say in one command why a site's REST API is failing: path-based block, app-layer filter, bot challenge, renamed prefix, or stripped Authorization header.", Group: "Diagnosing a real site", WhyItMatters: "Run this first whenever a WordPress request fails. A 401 can mean bad credentials, a security plugin, or a host stripping the auth header, and the status code alone cannot tell you which."},
	{Command: "caps", Description: "Report what your credentials are actually allowed to do on each route, with zero writes.", Group: "Diagnosing a real site", WhyItMatters: "Use this before pointing an unattended job at a site, instead of discovering permissions by attempting a write in production."},
	{Command: "schema", Description: "Find out why a post type, field, or meta key is missing from the REST surface.", Group: "Diagnosing a real site", WhyItMatters: "This is the show_in_rest blind spot. Reach for it when content exists in wp-admin but never appears in an API response."},
	{Command: "queue", Description: "See drafts, pending, and scheduled posts bucketed by how long they have been sitting in that state.", Group: "Editorial hygiene from the local mirror", WhyItMatters: "Answers 'what has been stuck in pending for three weeks', which wp-admin cannot express at all."},
	{Command: "audit", Description: "Find posts and pages missing a featured image, a real category, an excerpt, or tags.", Group: "Editorial hygiene from the local mirror", WhyItMatters: "Use this for defects on content itself. It reports offending IDs, so the output feeds directly into a bulk fix."},
	{Command: "orphans", Description: "List media files that no post or page references and that are nobody's featured image.", Group: "Editorial hygiene from the local mirror", WhyItMatters: "Reach for this when auditing storage or preparing a migration; it is the only way to size unused uploads without shell access."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  wordpress-pp-cli which "stale tickets"
  wordpress-pp-cli which "bottleneck"
  wordpress-pp-cli which --limit 1 "send message"
  wordpress-pp-cli which                                # list the full capability index`,
