...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  wordpress-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  wordpress-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"abilities", "abilities-categories", "block-patterns", "block-patterns-categories", "block-types", "blocks", "categories", "comments", "font-collections", "font-families", "media", "menu-items", "menu-locations", "menus", "navigation", "pages", "pattern-categories", "pattern-directory", "plugins", "posts", "sidebars", "statuses", "tags", "taxonomies", "template-parts", "templates", "themes", "users", "widget-types", "widgets", "wp_types"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  wordpress-pp-cli workflow status

  # Show status as JSON
  wordpress-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("wordpress-pp-cli")
			}
