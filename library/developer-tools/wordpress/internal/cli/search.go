...
In local mode: searches locally synced data only.`,
  ...
  wordpress-pp-cli search "error timeout"

  # Force local search only
  wordpress-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  wordpress-pp-cli search "status" --type abilities --data-source local
  # JSON output for piping
  wordpress-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /wp/v2/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/wp/v2/search", map[string]string{
					"search": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'wordpress-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
