...
Resource scoping:
  ...
  wordpress-pp-cli sync

  # Sync specific resources only
  wordpress-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  wordpress-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  wordpress-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  wordpress-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  wordpress-pp-cli sync --latest-only`,
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		...
		"abilities-categories":      "/wp-abilities/v1/categories",
		"block-patterns":            "/wp/v2/block-patterns/patterns",
		"block-patterns-categories": "/wp/v2/block-patterns/categories",
