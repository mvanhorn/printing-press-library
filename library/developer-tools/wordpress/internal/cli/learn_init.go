...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/developer-tools/wordpress/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/developer-tools/wordpress/internal/learn/lookups"
...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^[0-9]+$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: wordpress-pp-cli: ticker pattern %q failed to compile: %v\n", `^[0-9]+$`, err)
	} else {
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"comment_status": {
			{Canonical: "approved", Values: []string{"approve", "live comments"}},
			{Canonical: "hold", Values: []string{"pending", "moderation", "unapproved", "awaiting moderation"}},
			{Canonical: "spam", Values: []string{"junk"}},
		},
		"context": {
			{Canonical: "view", Values: []string{"read", "public view"}},
			{Canonical: "edit", Values: []string{"editor", "raw", "with raw fields"}},
			{Canonical: "embed", Values: []string{"minimal", "summary"}},
		},
		"post_status": {
			{Canonical: "publish", Values: []string{"published", "live", "public"}},
			{Canonical: "draft", Values: []string{"drafts", "unpublished", "wip"}},
			{Canonical: "future", Values: []string{"scheduled", "queued"}},
			{Canonical: "pending", Values: []string{"pending review", "awaiting review", "submitted"}},
			{Canonical: "private", Values: []string{"hidden"}},
			{Canonical: "trash", Values: []string{"trashed", "deleted", "bin"}},
		},
		"post_type": {
			{Canonical: "post", Values: []string{"posts", "article", "articles", "blog post"}},
			{Canonical: "page", Values: []string{"pages", "static page"}},
			{Canonical: "attachment", Values: []string{"media", "image", "images", "upload", "uploads"}},
			{Canonical: "wp_block", Values: []string{"reusable block", "synced pattern", "block"}},
			{Canonical: "wp_template", Values: []string{"template", "site template"}},
			{Canonical: "wp_template_part", Values: []string{"template part", "header template", "footer template"}},
			{Canonical: "nav_menu_item", Values: []string{"menu item", "nav item"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: wordpress-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: wordpress-pp-cli: learn init: %v\n", err)
		}
	})
}
