...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  wordpress-pp-cli doctor
  wordpress-pp-cli doctor --json
  wordpress-pp-cli doctor --fail-on warn
  wordpress-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set Basic credentials with: export WORDPRESS_USER=\"your-token-here\" WORDPRESS_APP_PASSWORD=\"your-password\""
				} else {
					authConfigured = true
			...
			if os.Getenv("WORDPRESS_USER") != "" {
				authEnvSet = append(authEnvSet, "WORDPRESS_USER")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "WORDPRESS_USER")
			}
			if os.Getenv("WORDPRESS_APP_PASSWORD") != "" {
				authEnvSet = append(authEnvSet, "WORDPRESS_APP_PASSWORD")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "WORDPRESS_APP_PASSWORD")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "wordpress-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'wordpress-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'wordpress-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'wordpress-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'wordpress-pp-cli sync' to refresh."
	}
	return report
}
