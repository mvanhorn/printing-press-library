...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("WORDPRESS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("WORDPRESS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "wordpress-pp-cli/feedback")
...
When ` + "`WORDPRESS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`WORDPRESS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  wordpress-pp-cli feedback list
  wordpress-pp-cli feedback list --limit 5
  wordpress-pp-cli feedback list --json`,
