...
// wordpress-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/developer-tools/wordpress/internal/learn/patterns"
...
const noLearnEnvVar = "WORDPRESS_NO_LEARN"
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  wordpress-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  wordpress-pp-cli recall "<question>" --agent
  wordpress-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  wordpress-pp-cli learnings list --agent
  wordpress-pp-cli learnings list --query "<substring>"
  wordpress-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  wordpress-pp-cli learnings forget "<question>" --resource <id>
  wordpress-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  wordpress-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  wordpress-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
