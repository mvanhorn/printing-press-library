...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	return home, filepath.Join(home, ".config", "wordpress-pp-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "wordpress-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("WORDPRESS_USER", "env-secret")
	t.Setenv("WORDPRESS_APP_PASSWORD", "env-secret-password")
	credentialsPath := filepath.Join(home, ".local", "share", "wordpress-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "wordpress-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "wordpress-pp-cli", "credentials.toml")
...
func testCredentials(token string) *cliutil.Credentials {
	...
	creds.WordpressAppPassword = token + "-password"
	return creds
}
...
func saveConfigCredential(cfg *config.Config, token string) error {
	return cfg.SaveCredentials(token, token+"-password")
}

func assertBasicAuthCredential(t *testing.T, header, username string) {
	t.Helper()
	const prefix = "Basic "
	...
	if got, want := string(payload), username+":"+username+"-password"; got != want {
