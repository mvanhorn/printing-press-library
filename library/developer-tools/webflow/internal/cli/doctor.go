...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  webflow-pp-cli doctor
  webflow-pp-cli doctor --json
  webflow-pp-cli doctor --fail-on warn
  webflow-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "webflow-pp-cli login"
					report["auth_docs_url"] = "https://developers.webflow.com"
				} else {
					authConfigured = true
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "WEBFLOW_API_TOKEN")
				if os.Getenv("WEBFLOW_API_TOKEN") != "" {
					authEnvSet = append(authEnvSet, "WEBFLOW_API_TOKEN")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("WEBFLOW_API_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "WEBFLOW_API_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "WEBFLOW_API_TOKEN optional")
			}
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "WEBFLOW_OAUTH2")
				if os.Getenv("WEBFLOW_OAUTH2") != "" {
					authEnvSet = append(authEnvSet, "WEBFLOW_OAUTH2")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("WEBFLOW_OAUTH2") != "" {
				authEnvSet = append(authEnvSet, "WEBFLOW_OAUTH2")
			} else {
				authEnvInfo = append(authEnvInfo, "WEBFLOW_OAUTH2 optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "webflow-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'webflow-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'webflow-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'webflow-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'webflow-pp-cli sync' to refresh."
	}
	return report
}
