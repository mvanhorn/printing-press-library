...
// webflow-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/developer-tools/webflow/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/developer-tools/webflow/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/developer-tools/webflow/internal/learn/patterns"
...
const noLearnEnvVar = "WEBFLOW_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("webflow-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  webflow-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  webflow-pp-cli recall "<question>" --agent
  webflow-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  webflow-pp-cli learnings list --agent
  webflow-pp-cli learnings list --query "<substring>"
  webflow-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  webflow-pp-cli learnings forget "<question>" --resource <id>
  webflow-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  webflow-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  webflow-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
