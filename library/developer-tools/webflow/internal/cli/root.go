...
type rootFlags struct {
	...
	// invocation. Mirrors the WEBFLOW_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "webflow-pp-cli",
		Short: `Webflow CLI — The whole Webflow Data API, plus a local database that lets you audit SEO, diff staging against live, and preview a…`,
		Long: `Webflow CLI — The whole Webflow Data API, plus a local database that lets you audit SEO, diff staging against live, and preview a publish before you trigger it.
...
Health check: run 'webflow-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("webflow-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'webflow-pp-cli profile list')")
	...
	rootCmd.AddCommand(newFormSubmissionsCmd(flags))
	rootCmd.AddCommand(newPagesCmd(flags))
	rootCmd.AddCommand(newSitesCmd(flags))
	rootCmd.AddCommand(newTokenCmd(flags))
	...
	rootCmd.AddCommand(newNovelPublishCmd(flags))
	rootCmd.AddCommand(newNovelRedirectsCmd(flags))
	rootCmd.AddCommand(newNovelSeoCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newAssetFoldersPromotedCmd(flags))
