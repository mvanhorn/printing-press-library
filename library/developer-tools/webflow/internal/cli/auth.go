...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Webflow",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  webflow-pp-cli auth setup\n  webflow-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "See API docs for OAuth app registration: https://developers.webflow.com")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then run:")
			fmt.Fprintln(w, "  webflow-pp-cli login")
			if !launch {
				return nil
			}
			launchURL := "https://developers.webflow.com"
...
func newAuthLoginCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.Flags().StringVar(&clientID, "client-id", os.Getenv("WEBFLOW_CLIENT_ID"), "OAuth2 client ID")
	cmd.Flags().StringVar(&clientSecret, "client-secret", os.Getenv("WEBFLOW_CLIENT_SECRET"), "OAuth2 client secret")
...
func runOAuthLogin(cmd *cobra.Command, flags *rootFlags, clientID, clientSecret string, port int) error {
	...
	scopes := []string{"assets:read", "assets:write", "authorized_user:read", "cms:read", "cms:write", "comments:read", "comments:write", "components:read", "components:write", "custom_code:read", "custom_code:write", "ecommerce:read", "ecommerce:write", "forms:read", "forms:write", "pages:read", "pages:write", "site_activity:read", "site_config:read", "site_config:write", "sites:read", "sites:write", "users:read", "users:write", "workspace:read", "workspace:write"}
	if len(scopes) > 0 {
...
func resolveOAuthCredentials(cmd *cobra.Command, flags *rootFlags, cfg *config.Config, clientID, clientSecret string) (string, string, error) {
	...
	if flags.noInput {
		return "", "", fmt.Errorf("OAuth2 client ID is required; pass --client-id, set WEBFLOW_CLIENT_ID, or run 'webflow-pp-cli login' without --no-input")
	}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				fmt.Fprintf(w, "  %s Not authenticated. Run 'webflow-pp-cli login' to authenticate.\n", red("FAIL"))
				return nil
			}
			...
			if cfg.TokenExpiry.IsZero() {
				...
				if cfg.RefreshToken != "" {
					...
					fmt.Fprintf(w, "  %s Token expired. Run 'webflow-pp-cli login' to re-authenticate.\n", red("FAIL"))
				}
			}
