...
Run sync first to populate the local search index.`,
  ...
  webflow-pp-cli search "status"

  # Force local search explicitly
  webflow-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  webflow-pp-cli search "status" --type asset-folders --data-source local
  # JSON output for piping
  webflow-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'webflow-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
