...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("WEBFLOW_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("WEBFLOW_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "webflow-pp-cli/feedback")
...
When ` + "`WEBFLOW_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`WEBFLOW_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  webflow-pp-cli feedback list
  webflow-pp-cli feedback list --limit 5
  webflow-pp-cli feedback list --json`,
