...
type Config struct {
	...
	WebflowApiToken string          `toml:"api_token"`
	WebflowOauth2   string          `toml:"oauth2"`
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("WEBFLOW_API_TOKEN"); v != "" {
		cfg.WebflowApiToken = v
		cfg.markEnvOverride("WebflowApiToken")
		cfg.AuthSource = "env:WEBFLOW_API_TOKEN"
		cfg.CredentialSource = "env:WEBFLOW_API_TOKEN"
	}
	if v := os.Getenv("WEBFLOW_OAUTH2"); v != "" {
		cfg.WebflowOauth2 = v
		cfg.markEnvOverride("WebflowOauth2")
		cfg.AuthSource = "env:WEBFLOW_OAUTH2"
		cfg.CredentialSource = "env:WEBFLOW_OAUTH2"
	}
	...
	if v := os.Getenv("WEBFLOW_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("WEBFLOW_AUTHORIZATION_URL"); v != "" {
		cfg.AuthorizationURL = v
	}
	if v := os.Getenv("WEBFLOW_TOKEN_URL"); v != "" {
		cfg.TokenURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("WEBFLOW_COLLECTION_ID")); v != "" {
		cfg.TemplateVars["collection_id"] = v
	} else if verifyMode {
		cfg.TemplateVars["collection_id"] = "collection_id_placeholder"
	}
	if v := strings.TrimSpace(os.Getenv("WEBFLOW_FORM_ID")); v != "" {
		cfg.TemplateVars["form_id"] = v
	} else if verifyMode {
		cfg.TemplateVars["form_id"] = "form_id_placeholder"
	}
	if v := strings.TrimSpace(os.Getenv("WEBFLOW_PAGE_ID")); v != "" {
		cfg.TemplateVars["page_id"] = v
	} else if verifyMode {
		cfg.TemplateVars["page_id"] = "page_id_placeholder"
	}
	if v := strings.TrimSpace(os.Getenv("WEBFLOW_WORKSPACE_ID_OR_SLUG")); v != "" {
		cfg.TemplateVars["workspace_id_or_slug"] = v
	} else if verifyMode {
		cfg.TemplateVars["workspace_id_or_slug"] = "workspace_id_or_slug_placeholder"
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("WEBFLOW_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "webflow-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	// PATCH(webflow-api-token-must-reach-the-authorization-header): the spec
	// declares both an OAuth2 scheme and a bearer ApiKey scheme. The generator
	// emits WebflowApiToken for the latter and populates it from
	// WEBFLOW_API_TOKEN, but without this branch AuthHeader never returns it:
	// doctor reports credentials_location=env:WEBFLOW_API_TOKEN while every
	// request goes out anonymously and the API answers 401. That makes the
	// CLI's documented primary credential a no-op.
	if c.WebflowApiToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:WEBFLOW_API_TOKEN"
		}
		return "Bearer " + c.WebflowApiToken
	}
	return ""
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		WebflowApiToken: c.WebflowApiToken,
		WebflowOauth2:   c.WebflowOauth2,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.WebflowApiToken = creds.WebflowApiToken
	c.WebflowOauth2 = creds.WebflowOauth2
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.WebflowApiToken = ""
	delete(c.envOverrides, "WebflowApiToken")
	c.updateFileConfigField("WebflowApiToken")
	c.WebflowOauth2 = ""
	delete(c.envOverrides, "WebflowOauth2")
	c.updateFileConfigField("WebflowOauth2")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["WebflowApiToken"] {
			out.WebflowApiToken = c.fileConfig.WebflowApiToken
		}
		if c.envOverrides["WebflowOauth2"] {
			out.WebflowOauth2 = c.fileConfig.WebflowOauth2
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "WebflowApiToken":
		c.fileConfig.WebflowApiToken = c.WebflowApiToken
	case "WebflowOauth2":
		c.fileConfig.WebflowOauth2 = c.WebflowOauth2
	}
}
