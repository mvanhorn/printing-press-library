...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("audit_a_county_s_fields_before_building_a_load",
			mcplib.WithDescription("Reports which high-value fields each layer exposes."),
			mcplib.WithString("service_url", mcplib.Required(), mcplib.Description("Override the recipe's positional service_url value.")),
		),
		handleAuditACountySFieldsBeforeBuildingALoad,
	)
}

// handleAuditACountySFieldsBeforeBuildingALoad runs the audit_a_county_s_fields_before_building_a_load recipe intent: Reports which high-value fields each layer exposes.
func handleAuditACountySFieldsBeforeBuildingALoad(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	var missingServiceUrl bool
	args, missingServiceUrl = appendRecipePositional(args, input["service_url"], true)
	if missingServiceUrl {
		return mcplib.NewToolResultError("service_url is required"), nil
	}
