...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("ARCGIS_TOKEN"); v != "" {
		cfg.ArcgisToken = v
		cfg.markEnvOverride("ArcgisToken")
		cfg.AuthSource = "env:ARCGIS_TOKEN"
	}
	...
	if v := os.Getenv("ARCGIS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("ARCGIS_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "arcgis-pp-cli", "config.toml"), nil
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["ArcgisToken"] {
			out.ArcgisToken = c.fileConfig.ArcgisToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "ArcgisToken":
		c.fileConfig.ArcgisToken = c.ArcgisToken
	}
}
