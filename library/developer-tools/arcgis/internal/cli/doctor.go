...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  arcgis-pp-cli doctor
  arcgis-pp-cli doctor --json
  arcgis-pp-cli doctor --fail-on warn
  arcgis-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// ARCGIS_TOKEN is optional: public ArcGIS endpoints need no auth,
			// and a token is only used for secured layers.
			authConfigured := cfg.ArcgisToken != ""
			if os.Getenv("ARCGIS_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "ARCGIS_TOKEN")
				...
				authEnvInfo = append(authEnvInfo, "ARCGIS_TOKEN unset (optional; only needed for secured layers)")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "arcgis-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'arcgis-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'arcgis-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'arcgis-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'arcgis-pp-cli sync' to refresh."
	}
	return report
}
