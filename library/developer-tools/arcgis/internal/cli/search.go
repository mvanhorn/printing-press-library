...
Run sync first to populate the local search index.`,
  ...
  arcgis-pp-cli search "status"

  # Force local search explicitly
  arcgis-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  arcgis-pp-cli search "status" --type catalog --data-source local
  # JSON output for piping
  arcgis-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'arcgis-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
