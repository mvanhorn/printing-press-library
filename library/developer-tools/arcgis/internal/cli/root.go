...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "arcgis-pp-cli",
		Short: `Point it at any ArcGIS FeatureServer or MapServer URL and pull, discover, or spatially bind features, with correct pagination, CSV/GeoJSON output, and a local store no other ArcGIS tool keeps.`,
		Long: `Point it at any ArcGIS FeatureServer or MapServer URL and pull, discover, or spatially bind features, with correct pagination, CSV/GeoJSON output, and a local store no other ArcGIS tool keeps.
...
Health check: run 'arcgis-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("arcgis-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'arcgis-pp-cli profile list')")
	...
	// URL-taking commands: ArcGIS REST is a protocol over arbitrary endpoints,
	// so every command takes a service/layer URL rather than a fixed base API.
	rootCmd.AddCommand(newDiscoverCmd(flags))
	rootCmd.AddCommand(newFieldsCmd(flags))
	rootCmd.AddCommand(newCountCmd(flags))
	rootCmd.AddCommand(newArcSyncCmd(flags))
	rootCmd.AddCommand(newNovelAuditCmd(flags))
	rootCmd.AddCommand(newNovelDiffCmd(flags))
	rootCmd.AddCommand(newNovelLocateCmd(flags))
	rootCmd.AddCommand(newNovelQueryCmd(flags))
