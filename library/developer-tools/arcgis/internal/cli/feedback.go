...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ARCGIS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ARCGIS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "arcgis-pp-cli/feedback")
...
When ` + "`ARCGIS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ARCGIS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  arcgis-pp-cli feedback list
  arcgis-pp-cli feedback list --limit 5
  arcgis-pp-cli feedback list --json`,
