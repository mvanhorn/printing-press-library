# ArcGIS REST Printed CLI Agent Guide

This directory is a generated `arcgis-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
arcgis-pp-cli doctor --json
arcgis-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
arcgis-pp-cli which "<capability>" --json
arcgis-pp-cli <command> --help
```
...
```bash
arcgis-pp-cli <command> --help
arcgis-pp-cli <command> --dry-run --agent
```
