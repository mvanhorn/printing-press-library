...
Resource scoping:
  ...
  dbt-cloud-pp-cli sync

  # Sync specific resources only
  dbt-cloud-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  dbt-cloud-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  dbt-cloud-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  dbt-cloud-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  dbt-cloud-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "dbt-cloud-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/dbt-cloud-pp-cli/data.db)")
...
func syncResource(ctx context.Context, c interface {
	...
	for {
		...
		// Apply hand-registered default params (sync_extras.go) after
		// spec-derived params but before user overrides. This injects e.g.
		// order_by=-created_at for the dbt Cloud runs resource so sync stores
		// newest runs first. No-op for resources without registered defaults.
		applySyncExtraParams(resource, params)
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "job":
		return db.UpsertDbtCloudAdminJobs(data)
	case "dbt_cloud_admin_jobs_artifacts":
		return db.UpsertDbtCloudAdminJobsArtifacts(data)
	case "rerun":
		return db.UpsertRerun(data)
	case "run":
		return db.UpsertRun(data)
	case "runs":
		return db.UpsertRuns(data)
	case "runs_artifacts":
		return db.UpsertRunsArtifacts(data)
	case "cancel":
		return db.UpsertCancel(data)
	case "retry":
		return db.UpsertRetry(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	...
	// Check hand-wired extra paths (e.g., dbt Cloud paths that require
	// account_id, resolved in sync_extras.go from DBT_CLOUD_ACCOUNT_ID).
	if p, ok := syncExtraPaths[resource]; ok {
		return p, nil
	}
