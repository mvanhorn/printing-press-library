...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "dbt-cloud-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DBT_CLOUD_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DBT_CLOUD_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "dbt-cloud-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/dbt-cloud-pp-cli/feedback.jsonl.
When ` + "`DBT_CLOUD_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DBT_CLOUD_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  dbt-cloud-pp-cli feedback list
  dbt-cloud-pp-cli feedback list --limit 5
  dbt-cloud-pp-cli feedback list --json`,
