...
native streaming instead of polling.`,
  ...
  dbt-cloud-pp-cli tail --interval 10s

  # Tail a specific resource
  dbt-cloud-pp-cli tail messages --interval 5s

  # Pipe to jq for filtering
  dbt-cloud-pp-cli tail events --interval 30s | jq 'select(.type == "error")'`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := fetchAndEmit(cmd.Context(), c, path, enc); err != nil {
				// In one-shot mode a failed fetch is a hard error (non-zero
				// exit); in follow mode it is transient — warn and keep polling.
				if !follow {
