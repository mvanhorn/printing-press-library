...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "dbt-cloud-pp-cli",
		Short: `Dbt Cloud CLI — Operate dbt Cloud jobs and runs from one static Go binary — trigger, watch to completion with honest exit codes, and qu…`,
		Long: `Dbt Cloud CLI — Operate dbt Cloud jobs and runs from one static Go binary — trigger, watch to completion with honest exit codes, and qu…
...
Health check: run 'dbt-cloud-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("dbt-cloud-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'dbt-cloud-pp-cli profile list')")
	...
	rootCmd.AddCommand(newDbtCloudAdminJobsCmd(flags))
	...
	rootCmd.AddCommand(newNovelArtifactsCmd(flags))
	rootCmd.AddCommand(newNovelFailuresCmd(flags))
	rootCmd.AddCommand(newNovelMonitorCmd(flags))
	rootCmd.AddCommand(newNovelTriggerCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newStepsPromotedCmd(flags))
