...
// to ~/.dbt-cloud-pp-cli/jobs.jsonl; the latest row for a given JobID wins
...
~/.dbt-cloud-pp-cli/jobs.jsonl. This command lists, inspects, and prunes them.
...
func newJobsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example:     "  dbt-cloud-pp-cli jobs list\n  dbt-cloud-pp-cli jobs list --all --limit 100",
		Annotations: map[string]string{"mcp:read-only": "true"},
...
func newJobsGetCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example:     "  dbt-cloud-pp-cli jobs get 12345",
		Annotations: map[string]string{"mcp:read-only": "true"},
...
func newJobsPruneCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  dbt-cloud-pp-cli jobs prune --older-than 168h",
		RunE: func(cmd *cobra.Command, _ []string) error {
			rows, err := readJobRows()
			if err != nil {
				return err
