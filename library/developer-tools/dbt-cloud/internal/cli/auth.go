...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Dbt Cloud Admin",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  dbt-cloud-pp-cli auth setup\n  dbt-cloud-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://docs.getdbt.com/dbt-cloud/api-v3#/")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export DBT_CLOUD_TOKEN=\"<your-token>\"")
			fmt.Fprintln(w, "  dbt-cloud-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://docs.getdbt.com/dbt-cloud/api-v3#/"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  dbt-cloud-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export DBT_CLOUD_TOKEN=\"your-token-here\"")
				fmt.Fprintf(w, "  dbt-cloud-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  dbt-cloud-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  dbt-cloud-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("DBT_CLOUD_TOKEN") != "" {
				envStillSet = "DBT_CLOUD_TOKEN"
			}
