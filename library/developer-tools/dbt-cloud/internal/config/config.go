...
type Config struct {
	...
	DbtCloudToken string            `toml:"cloud_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("DBT_CLOUD_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "dbt-cloud-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("DBT_CLOUD_TOKEN"); v != "" {
		cfg.DbtCloudToken = v
		cfg.markEnvOverride("DbtCloudToken")
		cfg.AuthSource = "env:DBT_CLOUD_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.DbtCloudToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("DBT_CLOUD_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.DbtCloudToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:DBT_CLOUD_TOKEN"
		}
		return "Bearer " + c.DbtCloudToken
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.DbtCloudToken = ""
	delete(c.envOverrides, "DbtCloudToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["DbtCloudToken"] {
			out.DbtCloudToken = c.fileConfig.DbtCloudToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "DbtCloudToken":
		c.fileConfig.DbtCloudToken = c.DbtCloudToken
	}
}
