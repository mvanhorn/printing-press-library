...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage optional authentication for BLS",
	}
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  bls-pp-cli auth setup\n  bls-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://data.bls.gov/registrationEngine/")
			fmt.Fprintln(w, "  Free registration unlocks 500 queries/day (vs 25 unauth), 50 series per batch, 20-year history, and the calculations / catalog / annual-average flags.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export BLS_API_KEY=\"<your-token>\"")
			fmt.Fprintln(w, "  bls-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://data.bls.gov/registrationEngine/"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  bls-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export BLS_API_KEY=\"your-token-here\"")
				fmt.Fprintf(w, "  bls-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  bls-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  bls-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("BLS_API_KEY") != "" {
				envStillSet = "BLS_API_KEY"
			}
