...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  bls-pp-cli doctor
  bls-pp-cli doctor --json
  bls-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export BLS_API_KEY=<your-key>"
					report["auth_key_url"] = "https://data.bls.gov/registrationEngine/"
					report["auth_instructions"] = "Free registration unlocks 500 queries/day (vs 25 unauth), 50 series per batch, 20-year history, and the calculations / catalog / annual-average flags."
				} else {
					authConfigured = true
			...
			if os.Getenv("BLS_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "BLS_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "BLS_API_KEY")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'bls-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'bls-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'bls-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'bls-pp-cli sync' to refresh."
	}
	return report
}
