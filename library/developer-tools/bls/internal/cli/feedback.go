...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".bls-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BLS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BLS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "bls-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.bls-pp-cli/feedback.jsonl.
When ` + "`BLS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BLS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  bls-pp-cli feedback list
  bls-pp-cli feedback list --limit 5
  bls-pp-cli feedback list --json`,
