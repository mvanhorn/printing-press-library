...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("BLS_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "bls-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("BLS_API_KEY"); v != "" {
		cfg.BlsApiKey = v
		cfg.AuthSource = "env:BLS_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.BlsApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("BLS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
