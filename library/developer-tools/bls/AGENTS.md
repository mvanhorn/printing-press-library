# BLS Printed CLI Agent Guide

This directory is a generated `bls-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
bls-pp-cli doctor --json
bls-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
bls-pp-cli which "<capability>" --json
bls-pp-cli <command> --help
```
...
```bash
bls-pp-cli <command> --help
bls-pp-cli <command> --dry-run --agent
```
