# DNS Made Easy Printed CLI Agent Guide

This directory is a generated `dnsmadeeasy-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
dnsmadeeasy-pp-cli doctor --json
dnsmadeeasy-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
dnsmadeeasy-pp-cli which "<capability>" --json
dnsmadeeasy-pp-cli <command> --help
```
...
```bash
dnsmadeeasy-pp-cli <command> --agent
```
...
```bash
dnsmadeeasy-pp-cli <command> --help
dnsmadeeasy-pp-cli <command> --dry-run --agent
```
