...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"DNSMADEEASY_CONFIG",
		"DNSMADEEASY_CONFIG_DIR",
		"DNSMADEEASY_DATA_DIR",
		"DNSMADEEASY_STATE_DIR",
		"DNSMADEEASY_CACHE_DIR",
		"DNSMADEEASY_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
	...
	return home, filepath.Join(home, ".config", "dnsmadeeasy-pp-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "dnsmadeeasy-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("DNSMADEEASY_API_KEY", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "dnsmadeeasy-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "dnsmadeeasy-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "dnsmadeeasy-pp-cli", "credentials.toml")
