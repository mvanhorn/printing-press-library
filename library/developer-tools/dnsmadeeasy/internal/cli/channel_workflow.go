...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  dnsmadeeasy-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  dnsmadeeasy-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"acl", "contact-lists", "domains", "folders", "ipsets", "secondary", "soa", "templates", "vanity"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  dnsmadeeasy-pp-cli workflow status

  # Show status as JSON
  dnsmadeeasy-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("dnsmadeeasy-pp-cli")
			}
