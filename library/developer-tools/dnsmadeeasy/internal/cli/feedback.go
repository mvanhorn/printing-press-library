...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DNSMADEEASY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DNSMADEEASY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "dnsmadeeasy-pp-cli/feedback")
...
When ` + "`DNSMADEEASY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DNSMADEEASY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  dnsmadeeasy-pp-cli feedback list
  dnsmadeeasy-pp-cli feedback list --limit 5
  dnsmadeeasy-pp-cli feedback list --json`,
