...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  dnsmadeeasy-pp-cli doctor
  dnsmadeeasy-pp-cli doctor --json
  dnsmadeeasy-pp-cli doctor --fail-on warn
  dnsmadeeasy-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set credentials with: export DNSMADEEASY_API_KEY=\"your-token-here\" DNSMADEEASY_API_SECRET=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if os.Getenv("DNSMADEEASY_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "DNSMADEEASY_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "DNSMADEEASY_API_KEY")
			}
			if os.Getenv("DNSMADEEASY_API_SECRET") != "" {
				authEnvSet = append(authEnvSet, "DNSMADEEASY_API_SECRET")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "DNSMADEEASY_API_SECRET")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "dnsmadeeasy-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'dnsmadeeasy-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'dnsmadeeasy-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'dnsmadeeasy-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'dnsmadeeasy-pp-cli sync' to refresh."
	}
	return report
}
