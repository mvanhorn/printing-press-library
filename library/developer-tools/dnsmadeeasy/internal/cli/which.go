...
var whichIndex = []whichEntry{
	{Command: "where-used", Description: "Find every zone and record whose value or CNAME/MX/ANAME target matches an IP, hostname, or string across your entire account.", Group: "Local state that compounds", WhyItMatters: "Pick this before an IP migration or decommission to see everywhere a value still appears, instead of paginating every zone by hand."},
	{Command: "drift", Description: "Diff the two most recent local sync snapshots and report which records were added, changed, or removed.", Group: "Local state that compounds", WhyItMatters: "Use for post-deploy verification that only intended records changed, or to catch out-of-band edits the API would never surface."},
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  dnsmadeeasy-pp-cli which "stale tickets"
  dnsmadeeasy-pp-cli which "bottleneck"
  dnsmadeeasy-pp-cli which --limit 1 "send message"
  dnsmadeeasy-pp-cli which                                # list the full capability index`,
