...
Resource scoping:
  ...
  dnsmadeeasy-pp-cli sync

  # Sync specific resources only
  dnsmadeeasy-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  dnsmadeeasy-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  dnsmadeeasy-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  dnsmadeeasy-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  dnsmadeeasy-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "ipsets":
		return db.UpsertIpsets(data)
	case "monitor":
		return db.UpsertMonitor(data)
	case "records":
		return db.UpsertRecords(data)
	case "secondary":
		return db.UpsertSecondary(data)
	case "soa":
		return db.UpsertSoa(data)
	case "template-records":
		return db.UpsertTemplateRecords(data)
	case "templates":
		return db.UpsertTemplates(data)
	case "vanity":
		return db.UpsertVanity(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"domains":       "/dns/managed",
		"domains-name":  "/dns/managed/name",
		"folders":       "/security/folder",
		"ipsets":        "/dns/secondary/ipSet",
		"secondary":     "/dns/secondary",
		"soa":           "/dns/soa",
		"templates":     "/dns/template",
		"vanity":        "/dns/vanity",
	}
...
func responsePathForResource(resource, path string) []string {
	...
	case "acl\x00/dns/transferAcl":
		return []string{"data"}
	case "contact-lists\x00/contactList":
		return []string{"data"}
	case "domains\x00/dns/managed":
		return []string{"data"}
	case "folders\x00/security/folder":
		return []string{"data"}
	case "ipsets\x00/dns/secondary/ipSet":
		return []string{"data"}
	case "records\x00/dns/managed/{domainId}/records":
		return []string{"data"}
	case "secondary\x00/dns/secondary":
		return []string{"data"}
	case "soa\x00/dns/soa":
		return []string{"data"}
	case "template-records\x00/dns/template/{templateId}/records":
		return []string{"data"}
	case "templates\x00/dns/template":
		return []string{"data"}
	case "vanity\x00/dns/vanity":
		return []string{"data"}
	}
	return nil
}
