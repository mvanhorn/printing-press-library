...
Run sync first to populate the local search index.`,
  ...
  dnsmadeeasy-pp-cli search "status"

  # Force local search explicitly
  dnsmadeeasy-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  dnsmadeeasy-pp-cli search "status" --type acl --data-source local
  # JSON output for piping
  dnsmadeeasy-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'dnsmadeeasy-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "soa":
				results, err = db.SearchSoa(query, limit)
			case "template-records":
				results, err = db.SearchTemplateRecords(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchSoa(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search soa failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchTemplateRecords(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search template_records failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
