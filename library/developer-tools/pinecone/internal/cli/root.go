...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "pinecone-pp-cli",
		Short: `Pinecone CLI — Every Pinecone API endpoint, plus cross-index search, namespace drift detection, and offline cost estimation.`,
		Long: `Pinecone CLI — Every Pinecone API endpoint, plus cross-index search, namespace drift detection, and offline cost estimation.
...
Health check: run 'pinecone-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("pinecone-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'pinecone-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNamespacesCmd(flags))
	rootCmd.AddCommand(newRecordsCmd(flags))
	rootCmd.AddCommand(newRestoreJobsCmd(flags))
	rootCmd.AddCommand(newVectorsCmd(flags))
	...
	rootCmd.AddCommand(newDescribeIndexStatsPromotedCmd(flags))
	rootCmd.AddCommand(newEmbedPromotedCmd(flags))
	rootCmd.AddCommand(newQueryPromotedCmd(flags))
	rootCmd.AddCommand(newRerankPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	// Transcendence (novel) commands — cross-index operations, namespace
	// analytics, drift detection, and offline cost estimation.
	rootCmd.AddCommand(newInventoryCmd(flags))
	rootCmd.AddCommand(newNsStatsCmd(flags))
	rootCmd.AddCommand(newDriftCmd(flags))
	rootCmd.AddCommand(newXindexCmd(flags))
	rootCmd.AddCommand(newPurgeCmd(flags))
	rootCmd.AddCommand(newEstimateCmd(flags))
	rootCmd.AddCommand(newDoctorDeepCmd(flags))
	rootCmd.AddCommand(newCSearchCmd(flags))
...
// newDataPlaneClient returns a Client pointed at the given index host. Used by
// novel commands (ns-stats, xindex, purge, csearch, doctor-deep) that need to
// hit the per-index data plane, since the generated client's BaseURL is fixed
// to the control plane (https://api.pinecone.io).
func (f *rootFlags) newDataPlaneClient(host string) (*client.Client, error) {
	c, err := f.newClient()
	if err != nil {
		return nil, err
	}
	if host == "" {
		return nil, errorsNew("empty host")
	}
	// Ensure scheme + trim trailing slash
	u := host
