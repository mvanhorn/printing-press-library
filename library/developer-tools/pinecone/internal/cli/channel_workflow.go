...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  pinecone-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"backups", "bulk", "collections", "indexes", "models", "namespaces", "restore-jobs", "vectors"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  pinecone-pp-cli workflow status

  # Show status as JSON
  pinecone-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("pinecone-pp-cli")
			}
