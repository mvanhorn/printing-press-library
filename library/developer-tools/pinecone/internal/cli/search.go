...
In local mode: searches locally synced data only.`,
  ...
  pinecone-pp-cli search "error timeout"

  # Force local search only
  pinecone-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  pinecone-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  pinecone-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /records/namespaces/{namespace}/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post("/records/namespaces/{namespace}/search", map[string]any{
					"query":  query,
					"fields": []any{},
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'pinecone-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "backups":
				results, err = db.SearchBackups(query, limit)
			case "":
				// Search all FTS-enabled tables individually to avoid duplicates.
				seen := make(map[string]bool)
				_ = seen // prevent unused error when no FTS tables exist
				{
					partial, searchErr := db.SearchBackups(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search backups failed: %w", searchErr)
					}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/pinecone-pp-cli/data.db)")
