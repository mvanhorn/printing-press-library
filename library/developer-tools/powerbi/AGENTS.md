# Power BI Printed CLI Agent Guide

This directory is a generated `powerbi-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
powerbi-pp-cli doctor --json
powerbi-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
powerbi-pp-cli which "<capability>" --json
powerbi-pp-cli <command> --help
```
...
```bash
powerbi-pp-cli <command> --help
powerbi-pp-cli <command> --dry-run --agent
```
