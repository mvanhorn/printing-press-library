...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  powerbi-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  powerbi-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"apps", "dashboards", "datasets", "groups", "reports"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  powerbi-pp-cli workflow status

  # Show status as JSON
  powerbi-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("powerbi-pp-cli")
			}
