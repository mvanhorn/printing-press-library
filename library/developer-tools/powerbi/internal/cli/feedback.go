...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".powerbi-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("POWERBI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("POWERBI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "powerbi-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.powerbi-pp-cli/feedback.jsonl.
When ` + "`POWERBI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`POWERBI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  powerbi-pp-cli feedback list
  powerbi-pp-cli feedback list --limit 5
  powerbi-pp-cli feedback list --json`,
