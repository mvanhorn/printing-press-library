...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "powerbi-pp-cli",
		Short: `Powerbi CLI — Pull data from the Microsoft Power BI REST API for analysis in Claude — every read-only feature pbicli has, plus saved …`,
		Long: `Powerbi CLI — Pull data from the Microsoft Power BI REST API for analysis in Claude — every read-only feature pbicli has, plus saved …
...
Health check: run 'powerbi-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("powerbi-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'powerbi-pp-cli profile list')")
	...
	rootCmd.AddCommand(newDataflowsCmd(flags))
	...
	// Power BI novel features.
	rootCmd.AddCommand(newDAXCmd(flags))
	rootCmd.AddCommand(newReportExportCmd(flags))
	rootCmd.AddCommand(newRefreshesCmd(flags))
