...
Resource scoping:
  ...
  thelancet-pp-cli sync

  # Sync specific resources only
  thelancet-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  thelancet-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  thelancet-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  thelancet-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  thelancet-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cliutil.IsDogfoodEnv() && !cmd.Flags().Changed("max-pages") {
				// OpenAlex pages are network-bound (~1-2s each); 10 pages across
				// multiple resources exceeds the dogfood per-command timeout.
				// Cap low so the matrix command completes in time.
				maxPages = 2
			}
...
func determinePaginationDefaults() paginationDefaults {
	// OpenAlex uses page-based pagination with a `per-page` size parameter
	// (it rejects `limit` at the API edge). Max page size is 200.
