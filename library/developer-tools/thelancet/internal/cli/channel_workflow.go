...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  thelancet-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  thelancet-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"authors", "works"}
			totalSynced := 0
			...
			// OpenAlex pages are network-bound; under live-dogfood the default
			// 100-page archive far exceeds the per-command timeout. Curtail to a
			// small bounded sample so the matrix command completes.
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  thelancet-pp-cli workflow status

  # Show status as JSON
  thelancet-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("thelancet-pp-cli")
			}
