...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "thelancet-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("THELANCET_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("THELANCET_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "thelancet-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/thelancet-pp-cli/feedback.jsonl.
When ` + "`THELANCET_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`THELANCET_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  thelancet-pp-cli feedback list
  thelancet-pp-cli feedback list --limit 5
  thelancet-pp-cli feedback list --json`,
