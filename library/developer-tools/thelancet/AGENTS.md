# The Lancet Printed CLI Agent Guide

This directory is a generated `thelancet-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
thelancet-pp-cli doctor --json
thelancet-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
thelancet-pp-cli which "<capability>" --json
thelancet-pp-cli <command> --help
```
...
```bash
thelancet-pp-cli <command> --agent
```
...
```bash
thelancet-pp-cli <command> --help
thelancet-pp-cli <command> --dry-run --agent
```
