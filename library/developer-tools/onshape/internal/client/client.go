...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "onshape-pp-cli", "http")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "onshape-pp-cli/0.1.0")
		}
		...
		if authHeader == "" && c.Config != nil && c.Config.OnshapeAccessKey != "" && c.Config.OnshapeSecretKey != "" {
			signOnshapeRequest(req, c.Config.OnshapeAccessKey, c.Config.OnshapeSecretKey)
		}
...
func signOnshapeRequest(req *http.Request, accessKey, secretKey string) {
	nonce := fmt.Sprintf("%d", time.Now().UnixNano())
	date := time.Now().UTC().Format(http.TimeFormat)
	contentType := req.Header.Get("Content-Type")
	stringToSign := strings.ToLower(strings.Join([]string{
		req.Method,
		nonce,
		date,
		contentType,
	...
	req.Header.Set("On-Nonce", nonce)
	req.Header.Set("Date", date)
	req.Header.Set("Authorization", fmt.Sprintf("On %s:HmacSHA256:%s", accessKey, signature))
}
