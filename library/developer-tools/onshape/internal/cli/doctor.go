...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  onshape-pp-cli doctor
  onshape-pp-cli doctor --json
  onshape-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" && (cfg.OnshapeAccessKey == "" || cfg.OnshapeSecretKey == "") {
					report["auth"] = "not configured"
					report["auth_hint"] = "export ONSHAPE_ACCESS_KEY=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("ONSHAPE_ACCESS_KEY") != "" || os.Getenv("ONSHAPE_API_ACCESSKEY") != "" {
				authEnvSet = append(authEnvSet, "ONSHAPE_ACCESS_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "ONSHAPE_ACCESS_KEY")
			}
			if os.Getenv("ONSHAPE_SECRET_KEY") != "" || os.Getenv("ONSHAPE_API_SECRETKEY") != "" {
				authEnvSet = append(authEnvSet, "ONSHAPE_SECRET_KEY")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "ONSHAPE_SECRET_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" && (cfg.OnshapeAccessKey == "" || cfg.OnshapeSecretKey == "") {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("onshape-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'onshape-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'onshape-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'onshape-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'onshape-pp-cli sync' to refresh."
	}
	return report
}
