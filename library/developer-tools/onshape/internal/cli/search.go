...
In local mode: searches locally synced data only.`,
  ...
  onshape-pp-cli search "error timeout"

  # Force local search only
  onshape-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  onshape-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  onshape-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /documents
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get("/documents", map[string]string{
					"query": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'onshape-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "versions":
				results, err = db.SearchVersions(query, limit)
			case "workspaces":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchVersions(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search versions failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/onshape-pp-cli/data.db)")
