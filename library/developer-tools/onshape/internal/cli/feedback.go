...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".onshape-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ONSHAPE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ONSHAPE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "onshape-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.onshape-pp-cli/feedback.jsonl.
When ` + "`ONSHAPE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ONSHAPE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  onshape-pp-cli feedback list
  onshape-pp-cli feedback list --limit 5
  onshape-pp-cli feedback list --json`,
