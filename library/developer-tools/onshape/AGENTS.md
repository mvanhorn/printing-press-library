# Onshape Printed CLI Agent Guide

This directory is a generated `onshape-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
onshape-pp-cli doctor --json
onshape-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
onshape-pp-cli which "<capability>" --json
onshape-pp-cli <command> --help
```
...
```bash
onshape-pp-cli <command> --help
onshape-pp-cli <command> --dry-run --agent
```
