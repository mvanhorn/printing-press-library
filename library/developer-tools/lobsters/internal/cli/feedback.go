...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("LOBSTERS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("LOBSTERS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "lobsters-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.lobsters-pp-cli/feedback.jsonl.
When ` + "`LOBSTERS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`LOBSTERS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  lobsters-pp-cli feedback list
  lobsters-pp-cli feedback list --limit 5
  lobsters-pp-cli feedback list --json`,
