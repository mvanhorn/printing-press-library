...
import (
	...
	"math"
	"net/http"
	"github.com/mvanhorn/printing-press-library/library/developer-tools/ori/internal/a2arpc"
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "ori-pp-cli", "http")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	// A2A adapter: the spec describes a REST veneer over a JSON-RPC server at
	// POST /a2a/{agent}/v1. The a2arpc package translates REST paths to RPC
	// methods and unwraps the response. Skips caching/retry; the server is
	// loopback and rate-limiting/caching is overkill for a single-user CLI.
	if a2arpc.Routes(path) {
		if c.DryRun {
			return c.dryRun(method, targetURL, path, params, bodyBytes, headerOverrides, authHeader)
		}
		result, status, rerr := a2arpc.Route(c.HTTPClient, c.BaseURL, method, path, params, bodyBytes, authHeader)
		if rerr != nil {
			return result, status, &APIError{Method: method, Path: path, StatusCode: status, Body: rerr.Error()}
		}
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "ori-pp-cli/0.1.0")
		}
