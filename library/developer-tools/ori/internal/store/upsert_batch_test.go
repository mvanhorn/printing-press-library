...
func TestUpsertBatch_PopulatesApprovalsTable(t *testing.T) {
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "a2a_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "a2a_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "a2a_id": "test-parent-001"}`),
	}
...
// TestUpsertBatch_PopulatesAgentCardJsonTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed agent_card_json table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesAgentCardJsonTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "well_known_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "well_known_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "well_known_id": "test-parent-001"}`),
	}
	if _, _, err := s.UpsertBatch("agent_card_json", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "agent_card_json").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "agent_card_json")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count agent_card_json: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("agent_card_json count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
