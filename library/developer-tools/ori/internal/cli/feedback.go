...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".ori-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ORI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ORI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ori-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.ori-pp-cli/feedback.jsonl.
When ` + "`ORI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ORI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ori-pp-cli feedback list
  ori-pp-cli feedback list --limit 5
  ori-pp-cli feedback list --json`,
