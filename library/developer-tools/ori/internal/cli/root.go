...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "ori-pp-cli",
		Short: `Ori CLI — Unified ops CLI for the openclaw A2A bridge: chat, tasks, approvals, service kickstart, and FTS5 search across cached t…`,
		Long: `Ori CLI — Unified ops CLI for the openclaw A2A bridge: chat, tasks, approvals, service kickstart, and FTS5 search across cached t…

Highlights (not in the official API docs):
  • doctor   Run every openclaw stack health check in one verb: launchd a2a-server status, /healthz reachability, both agents responding, agents.toml valid, plugin cache env vars commented in compose, codex OAuth…
...
Health check: run 'ori-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("ori-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().DurationVar(&flags.timeout, "timeout", 120*time.Second, "Request timeout (default 120s; raised from 30s on 2026-05-21 — Ori dispatches with wait=true commonly run longer than 30s; pass --timeout 600s or --wait=false for investigation-scale dispatches)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'ori-pp-cli profile list')")
	...
	rootCmd.AddCommand(newA2aCmd(flags))
	rootCmd.AddCommand(newDoctorCmd(flags))
	rootCmd.AddCommand(newKickstartCmd(flags))
	rootCmd.AddCommand(newLogsCmd(flags))
	rootCmd.AddCommand(newWatchCmd(flags))
	rootCmd.AddCommand(newApprovalsRootCmd(flags))
	rootCmd.AddCommand(newTasksRootCmd(flags))
	rootCmd.AddCommand(newContextsRootCmd(flags))
	...
	rootCmd.AddCommand(newWellKnownPromotedCmd(flags))
