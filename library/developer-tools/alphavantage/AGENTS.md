# Alpha Vantage Printed CLI Agent Guide

This directory is a generated `alphavantage-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
alphavantage-pp-cli doctor --json
alphavantage-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
alphavantage-pp-cli which "<capability>" --json
alphavantage-pp-cli <command> --help
```
...
```bash
alphavantage-pp-cli <command> --agent
```
...
```bash
alphavantage-pp-cli <command> --help
alphavantage-pp-cli <command> --dry-run --agent
```
