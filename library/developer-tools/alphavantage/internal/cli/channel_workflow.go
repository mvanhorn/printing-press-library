...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  alphavantage-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  alphavantage-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"commodities", "fundamentals"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  alphavantage-pp-cli workflow status

  # Show status as JSON
  alphavantage-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("alphavantage-pp-cli")
			}
