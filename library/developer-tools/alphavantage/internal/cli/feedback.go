...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".alphavantage-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ALPHAVANTAGE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ALPHAVANTAGE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "alphavantage-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.alphavantage-pp-cli/feedback.jsonl.
When ` + "`ALPHAVANTAGE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ALPHAVANTAGE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  alphavantage-pp-cli feedback list
  alphavantage-pp-cli feedback list --limit 5
  alphavantage-pp-cli feedback list --json`,
