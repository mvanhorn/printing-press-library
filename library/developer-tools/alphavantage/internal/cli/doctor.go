...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  alphavantage-pp-cli doctor
  alphavantage-pp-cli doctor --json
  alphavantage-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export ALPHAVANTAGE_API_KEY=<your-key>"
					report["auth_key_url"] = "https://www.alphavantage.co/support/#api-key"
				} else {
					authConfigured = true
			...
			if os.Getenv("ALPHAVANTAGE_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "ALPHAVANTAGE_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "ALPHAVANTAGE_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					healthPath := "/query?function=MARKET_STATUS"
					...
					if authHeader == "" {
						...
						verifyPath := "/query?function=MARKET_STATUS"
						...
						authHeaders["User-Agent"] = "alphavantage-pp-cli"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("alphavantage-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'alphavantage-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'alphavantage-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'alphavantage-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'alphavantage-pp-cli sync' to refresh."
	}
	return report
}
