...
Exit codes & warnings:
  ...
  alphavantage-pp-cli sync

  # Sync specific resources only
  alphavantage-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  alphavantage-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  alphavantage-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  alphavantage-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  alphavantage-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/alphavantage-pp-cli/data.db)")
