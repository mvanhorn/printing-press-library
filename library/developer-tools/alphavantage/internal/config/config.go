...
type Config struct {
	...
	Path               string            `toml:"-"`
	AlphavantageApiKey string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("ALPHAVANTAGE_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "alphavantage-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("ALPHAVANTAGE_API_KEY"); v != "" {
		cfg.AlphavantageApiKey = v
		cfg.AuthSource = "env:ALPHAVANTAGE_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.AlphavantageApiKey != "" {
		cfg.AuthSource = "config"
	}
