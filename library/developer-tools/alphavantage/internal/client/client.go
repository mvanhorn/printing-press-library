...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "alphavantage-pp-cli", "http")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "alphavantage-pp-cli/0.1.0")
		}
		...
		// Success at HTTP layer, but Alpha Vantage returns 200 with a "Note"
		// or "Information" or "Error Message" field when quota is hit, the
		// function name is unknown, or required params are missing. Surface
		// these as typed errors so callers don't silently process empty data.
		if resp.StatusCode < 400 {
			if msg := detectAlphaVantageBodyError(respBody); msg != "" {
...
// detectAlphaVantageBodyError inspects a 200-status JSON body for the
// service's three documented soft-failure shapes:
//
...
//
// Each indicates the request did not return data despite HTTP 200. Returns
// the message text if detected, empty string otherwise. Order: check whichever
// field is present.
func detectAlphaVantageBodyError(body []byte) string {
	trimmed := bytes.TrimSpace(body)
	if len(trimmed) == 0 || trimmed[0] != '{' {
		return ""
	}
	// Heuristic peek: most rate-limit / error responses are < 1KB and start with
	// one of these field names within the first 40 bytes.
	peek := trimmed
	if len(peek) > 256 {
		peek = peek[:256]
	}
	if !bytes.Contains(peek, []byte(`"Note"`)) &&
		!bytes.Contains(peek, []byte(`"Information"`)) &&
		!bytes.Contains(peek, []byte(`"Error Message"`)) {
		return ""
	}
