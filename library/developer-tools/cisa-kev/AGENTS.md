# Cisa Kev Printed CLI Agent Guide

This directory is a generated `cisa-kev-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
cisa-kev-pp-cli doctor --json
cisa-kev-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
cisa-kev-pp-cli which "<capability>" --json
cisa-kev-pp-cli <command> --help
```
...
```bash
cisa-kev-pp-cli <command> --help
cisa-kev-pp-cli <command> --dry-run --agent
```
