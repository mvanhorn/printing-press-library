...
Resource scoping:
  ...
  cisa-kev-pp-cli sync

  # Sync specific resources only
  cisa-kev-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  cisa-kev-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  cisa-kev-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  cisa-kev-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  cisa-kev-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/cisa-kev-pp-cli/data.db)")
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"sites": "/sites/default/files/feeds/known_exploited_vulnerabilities.json",
	}
