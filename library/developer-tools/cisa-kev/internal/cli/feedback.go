...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "cisa-kev-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CISA_KEV_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CISA_KEV_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "cisa-kev-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/cisa-kev-pp-cli/feedback.jsonl.
When ` + "`CISA_KEV_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CISA_KEV_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cisa-kev-pp-cli feedback list
  cisa-kev-pp-cli feedback list --limit 5
  cisa-kev-pp-cli feedback list --json`,
