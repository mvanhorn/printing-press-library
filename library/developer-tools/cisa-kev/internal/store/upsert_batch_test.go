...
func TestSearchQuotesFTSQuerySyntax(t *testing.T) {
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "ip", "value": "alpha.node.localtest"}`),
		json.RawMessage(`{"id": "cidr", "value": "segment-alpha-range"}`),
		json.RawMessage(`{"id": "host", "value": "host.example.com"}`),
		json.RawMessage(`{"id": "email", "value": "user-at-example"}`),
