# Vibecode Printed CLI Agent Guide

This directory is a generated `vibecode-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
vibecode-pp-cli doctor --json
vibecode-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
vibecode-pp-cli which "<capability>" --json
vibecode-pp-cli <command> --help
```
...
```bash
vibecode-pp-cli <command> --help
vibecode-pp-cli <command> --dry-run --agent
```
