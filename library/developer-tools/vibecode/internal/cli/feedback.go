...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".vibecode-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("VIBECODE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("VIBECODE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "vibecode-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.vibecode-pp-cli/feedback.jsonl.
When ` + "`VIBECODE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`VIBECODE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  vibecode-pp-cli feedback list
  vibecode-pp-cli feedback list --limit 5
  vibecode-pp-cli feedback list --json`,
