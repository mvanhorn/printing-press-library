...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  vibecode-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  vibecode-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"deployments", "projects", "sandboxes"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  vibecode-pp-cli workflow status

  # Show status as JSON
  vibecode-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("vibecode-pp-cli")
			}
