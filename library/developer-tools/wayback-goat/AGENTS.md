# Wayback Goat Printed CLI Agent Guide

This directory is a generated `wayback-goat-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
wayback-goat-pp-cli doctor --json
wayback-goat-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
wayback-goat-pp-cli which "<capability>" --json
wayback-goat-pp-cli <command> --help
```
...
```bash
wayback-goat-pp-cli <command> --agent
```
...
```bash
wayback-goat-pp-cli <command> --help
wayback-goat-pp-cli <command> --dry-run --agent
```
