		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export EXA_API_KEY=<your-key>  # or EXA_TOKEN=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("EXA_TOKEN") != "" || os.Getenv("EXA_API_KEY") != "" {
				key := "EXA_TOKEN"
				if os.Getenv("EXA_API_KEY") != "" {
					key = "EXA_API_KEY"
				}
				authEnvSet = append(authEnvSet, key)
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "EXA_API_KEY (or EXA_TOKEN)")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'exa-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
