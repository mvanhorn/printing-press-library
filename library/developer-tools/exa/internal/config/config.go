...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("EXA_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "exa-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("EXA_TOKEN"); v != "" {
		cfg.ExaToken = v
		cfg.markEnvOverride("ExaToken")
		cfg.AuthSource = "env:EXA_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.ExaToken != "" {
		cfg.AuthSource = "config"
	}
...
func (c *Config) AuthHeader() string {
	...
	if c.ExaToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:EXA_TOKEN"
		}
		return "Bearer " + c.ExaToken
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.ExaToken = ""
	delete(c.envOverrides, "ExaToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["ExaToken"] {
			out.ExaToken = c.fileConfig.ExaToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "ExaToken":
		c.fileConfig.ExaToken = c.ExaToken
	}
}
