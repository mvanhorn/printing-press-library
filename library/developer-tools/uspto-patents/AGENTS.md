# USPTO Patents Printed CLI Agent Guide

This directory is a generated `uspto-patents-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
uspto-patents-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
uspto-patents-pp-cli which "<capability>" --json
uspto-patents-pp-cli <command> --help
```
...
```bash
uspto-patents-pp-cli <command> --agent
```
...
```bash
uspto-patents-pp-cli <command> --help
uspto-patents-pp-cli <command> --dry-run --agent
```
