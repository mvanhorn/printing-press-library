...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".uspto-patents-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("USPTO_PATENTS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("USPTO_PATENTS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "uspto-patents-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.uspto-patents-pp-cli/feedback.jsonl.
When ` + "`USPTO_PATENTS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`USPTO_PATENTS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  uspto-patents-pp-cli feedback list
  uspto-patents-pp-cli feedback list --limit 5
  uspto-patents-pp-cli feedback list --json`,
