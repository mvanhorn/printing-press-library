# cmux Printed CLI Agent Guide

This directory is a generated `cmux-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
cmux-pp-cli doctor --json
cmux-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
cmux-pp-cli which "<capability>" --json
cmux-pp-cli <command> --help
```
...
```bash
cmux-pp-cli <command> --help
cmux-pp-cli <command> --dry-run --agent
```
