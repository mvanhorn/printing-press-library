...
import (
	"bytes"
	"github.com/mvanhorn/printing-press-library/library/developer-tools/cmux/internal/cliutil"
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "cmux-pp-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	// cmux IPC shim: the spec base_url is a placeholder; the printed CLI
	// actually dispatches every read to the local cmux binary via a hidden
	// subprocess call rather than HTTP. See internal/cmuxclient for the shim.
	if strings.Contains(c.BaseURL, "cmux.localhost") {
		if c.DryRun {
			return []byte(`{"dry_run":true,"method":"` + method + `","path":"` + path + `"}`), 200, nil
		}
		data, err := cmuxDispatch(method, path, params)
		if err != nil {
			return nil, 0, err
		}
		return data, 200, nil
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "cmux-pp-cli/0.1.0")
		}
