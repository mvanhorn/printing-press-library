...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  cmux-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  cmux-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"buffers", "capabilities", "hooks", "logs", "notifications", "panes", "status", "surfaces", "surfaces-health", "windows", "workspaces"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  cmux-pp-cli workflow status

  # Show status as JSON
  cmux-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("cmux-pp-cli")
			}
