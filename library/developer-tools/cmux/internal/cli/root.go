...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "cmux-pp-cli",
		Short: `Every cmux feature, plus persisted state, cross-surface search, and a notify-driven event stream no other cmux tool offers.`,
		Long: `Every cmux feature, plus persisted state, cross-surface search, and a notify-driven event stream no other cmux tool offers.
...
Highlights (not in the official API docs):
  ...
  • watch   Long-running stream of cmux notification events as JSONL, with pluggable sinks (stdout, file, exec hook, Slack webhook, generic webhook). Replaces capture-pane polling with id-cursored notification e…
...
Health check: run 'cmux-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("cmux-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'cmux-pp-cli profile list')")
	...
	// cmux-shaped parent: workspaces (carries the novel `card` subcommand).
	workspacesCmd := newWorkspacesCmd(flags)
	workspacesCmd.Hidden = false
	workspacesCmd.AddCommand(newWorkspacesCardCmd(flags))
	rootCmd.AddCommand(workspacesCmd)

	surfacesCmd := newSurfacesCmd(flags)
	surfacesCmd.Hidden = false
	...
	// Replace the generic FTS search with the cmux-shaped multi-source search.
	rootCmd.AddCommand(newCmuxSearchCmd(flags))
	rootCmd.AddCommand(newSyncCmd(flags))
	rootCmd.AddCommand(newTailCmd(flags))
	rootCmd.AddCommand(newAnalyticsCmd(flags))
	rootCmd.AddCommand(newWorkflowCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newBuffersPromotedCmd(flags))
	rootCmd.AddCommand(newCapabilitiesPromotedCmd(flags))
	rootCmd.AddCommand(newHooksPromotedCmd(flags))
	rootCmd.AddCommand(newLogsPromotedCmd(flags))
	rootCmd.AddCommand(newNotificationsPromotedCmd(flags))

	// Panes carries the novel `sample` subcommand alongside `list`.
	panesCmd := newPanesPromotedCmd(flags)
	panesCmd.AddCommand(newPanesSampleCmd(flags))
	rootCmd.AddCommand(panesCmd)

	// Status carries the novel `awaiting`, `timeline`, `stuck`, `changes`,
	// and `icons` subcommands alongside the promoted `list` shortcut.
	statusCmd := newStatusPromotedCmd(flags)
	addNovelStatusSubcommands(statusCmd, flags)
	rootCmd.AddCommand(statusCmd)

	rootCmd.AddCommand(newSnapshotCmd(flags))
	rootCmd.AddCommand(newWatchCmd(flags))
	rootCmd.AddCommand(newAlertCmd(flags))
