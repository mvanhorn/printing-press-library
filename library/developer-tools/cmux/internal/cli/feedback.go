...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".cmux-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CMUX_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CMUX_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "cmux-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.cmux-pp-cli/feedback.jsonl.
When ` + "`CMUX_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CMUX_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cmux-pp-cli feedback list
  cmux-pp-cli feedback list --limit 5
  cmux-pp-cli feedback list --json`,
