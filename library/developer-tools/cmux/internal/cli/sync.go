...
Exit codes & warnings:
  ...
  cmux-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  cmux-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  cmux-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  cmux-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  cmux-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/cmux-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "buffers":
		return db.UpsertBuffers(data)
	case "logs":
		return db.UpsertLogs(data)
	case "notifications":
		return db.UpsertNotifications(data)
	case "panes":
		return db.UpsertPanes(data)
	case "status":
		return db.UpsertStatus(data)
	case "surfaces":
		return db.UpsertSurfaces(data)
	case "windows":
		return db.UpsertWindows(data)
	case "workspaces":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"status":          "/status",
		"surfaces":        "/surfaces",
		"surfaces-health": "/surfaces/health",
		"windows":         "/windows",
