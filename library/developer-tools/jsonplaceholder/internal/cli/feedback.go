...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".jsonplaceholder-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("JSONPLACEHOLDER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("JSONPLACEHOLDER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "jsonplaceholder-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.jsonplaceholder-pp-cli/feedback.jsonl.
When ` + "`JSONPLACEHOLDER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`JSONPLACEHOLDER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
