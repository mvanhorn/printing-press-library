...
In local mode: searches locally synced data only.`,
  ...
  jsonplaceholder-pp-cli search "error timeout"

  # Force local search only
  jsonplaceholder-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  jsonplaceholder-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  jsonplaceholder-pp-cli search "critical" --json --limit 20`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Local FTS search
			if dbPath == "" {
				dbPath = defaultDBPath("jsonplaceholder-pp-cli")
			}

			db, err := store.Open(dbPath)
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'jsonplaceholder-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "comments":
				results, err = db.SearchComments(query, limit)
			case "posts":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchComments(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search comments failed: %w", searchErr)
					}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/jsonplaceholder-pp-cli/data.db)")
