...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  jsonplaceholder-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  jsonplaceholder-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  jsonplaceholder-pp-cli workflow status

  # Show status as JSON
  jsonplaceholder-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("jsonplaceholder-pp-cli")
			}
