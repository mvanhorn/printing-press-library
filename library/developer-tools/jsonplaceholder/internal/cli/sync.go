...
Exit codes & warnings:
  ...
  jsonplaceholder-pp-cli sync

  # Sync specific resources only
  jsonplaceholder-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  jsonplaceholder-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  jsonplaceholder-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  jsonplaceholder-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  jsonplaceholder-pp-cli sync --latest-only`,
