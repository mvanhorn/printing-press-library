# Awwwards Printed CLI Agent Guide

This directory is a generated `awwwards-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
awwwards-pp-cli doctor --json
awwwards-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
awwwards-pp-cli which "<capability>" --json
awwwards-pp-cli <command> --help
```
...
```bash
awwwards-pp-cli <command> --agent
```

This CLI is read-only against awwwards.com (no create/update/delete anywhere). Before running an unfamiliar command, inspect its help and prefer a dry run:

```bash
awwwards-pp-cli <command> --help
awwwards-pp-cli <command> --dry-run --agent
```
