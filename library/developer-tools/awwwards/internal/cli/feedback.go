...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AWWWARDS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AWWWARDS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "awwwards-pp-cli/feedback")
...
When ` + "`AWWWARDS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AWWWARDS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  awwwards-pp-cli feedback list
  awwwards-pp-cli feedback list --limit 5
  awwwards-pp-cli feedback list --json`,
