...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  awwwards-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  awwwards-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"collections", "directory", "websites"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  awwwards-pp-cli workflow status

  # Show status as JSON
  awwwards-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("awwwards-pp-cli")
			}
