...
Run sync first to populate the local search index.`,
  ...
  awwwards-pp-cli search "status"

  # Force local search explicitly
  awwwards-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  awwwards-pp-cli search "status" --type collections --data-source local
  # JSON output for piping
  awwwards-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'awwwards-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
