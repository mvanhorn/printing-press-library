# Excalidraw Mcp Printed CLI Agent Guide

This directory is a generated `excalidraw-mcp-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
excalidraw-mcp-pp-cli doctor --json
excalidraw-mcp-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
excalidraw-mcp-pp-cli which "<capability>" --json
excalidraw-mcp-pp-cli <command> --help
```
...
```bash
excalidraw-mcp-pp-cli <command> --agent
```
...
```bash
excalidraw-mcp-pp-cli <command> --help
excalidraw-mcp-pp-cli <command> --dry-run --agent
```
