...
// TestUpsertBatch_PopulatesCollectionsScenesTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed collections_scenes table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesCollectionsScenesTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("collections_scenes", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "collections_scenes").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "collections_scenes")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count collections_scenes: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("collections_scenes count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandCollectionsScenesGeneric exercises
...
func TestUpsertBatch_TypedFailureDoesNotStrandCollectionsScenesGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	stored, extractFailures, err := s.UpsertBatch("collections_scenes", items)
	if err != nil {
	...
	if typed != 0 {
		t.Fatalf("collections_scenes count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "collections_id")
	}
}
...
func TestUpsertBatch_PopulatesContentTable(t *testing.T) {
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "scenes_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "scenes_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "scenes_id": "test-parent-001"}`),
	}
...
func TestUpsertBatch_TypedFailureDoesNotStrandContentGeneric(t *testing.T) {
	...
	// Items deliberately omit "scenes_id" so the typed NOT NULL
	// constraint fires.
	items := []json.RawMessage{
		json.RawMessage(`{"id": "orphan-001"}`),
		json.RawMessage(`{"id": "orphan-002"}`),
	...
	if typed != 0 {
		t.Fatalf("content count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "scenes_id")
	}
}
