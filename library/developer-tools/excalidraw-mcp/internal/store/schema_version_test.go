...
// TestMigrate_AddsColumnsOnUpgrade_CollectionsScenes verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_CollectionsScenes(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "collections_scenes" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	rows, err := s.DB().Query(`PRAGMA table_info("collections_scenes")`)
	if err != nil {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from collections_scenes after migrate", want)
		}
	}
}
