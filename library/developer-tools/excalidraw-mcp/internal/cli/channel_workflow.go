...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  excalidraw-mcp-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  excalidraw-mcp-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"collections", "elements", "excalidraw-canvas-cloud-health", "files", "invites", "logs", "scenes", "snapshots", "workspace"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  excalidraw-mcp-pp-cli workflow status

  # Show status as JSON
  excalidraw-mcp-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("excalidraw-mcp-pp-cli")
			}
