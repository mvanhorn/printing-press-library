...
Resource scoping:
  ...
  excalidraw-mcp-pp-cli sync

  # Sync specific resources only
  excalidraw-mcp-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  excalidraw-mcp-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  excalidraw-mcp-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  excalidraw-mcp-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  excalidraw-mcp-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/excalidraw-mcp-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "collections_scenes":
		return db.UpsertCollectionsScenes(data)
	case "elements":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"excalidraw-canvas-cloud-health": "/health",
		"files":                          "/api/files",
		"invites":                        "/api/v1/invites",
		"logs":                           "/api/v1/logs",
		"scenes":                         "/api/v1/scenes",
		"snapshots":                      "/api/snapshots",
		"workspace":                      "/api/v1/workspace/users",
	}
