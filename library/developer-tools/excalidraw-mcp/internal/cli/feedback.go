...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".excalidraw-mcp-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("EXCALIDRAW_MCP_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("EXCALIDRAW_MCP_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "excalidraw-mcp-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.excalidraw-mcp-pp-cli/feedback.jsonl.
When ` + "`EXCALIDRAW_MCP_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`EXCALIDRAW_MCP_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  excalidraw-mcp-pp-cli feedback list
  excalidraw-mcp-pp-cli feedback list --limit 5
  excalidraw-mcp-pp-cli feedback list --json`,
