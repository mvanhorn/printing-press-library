...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "excalidraw-mcp-pp-cli",
		Short: "The only Go CLI for Excalidraw — control your canvas, convert Mermaid, diff snapshots, and manage cloud scenes.",
		Long: `The only Go CLI for Excalidraw — control your canvas, convert Mermaid, diff snapshots, and manage cloud scenes.

Canvas commands (elements, snapshots, viewport, convert) talk to the local canvas server at http://127.0.0.1:3000.
Cloud commands (scenes, collections, workspace) require EXCALIDRAW_API_KEY from plus.excalidraw.com.

Start the canvas server: docker run -p 3000:3000 ghcr.io/yctimlin/mcp_excalidraw-canvas:latest
...
Run 'excalidraw-mcp-pp-cli doctor' to verify connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("excalidraw-mcp-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'excalidraw-mcp-pp-cli profile list')")
	...
	rootCmd.AddCommand(newScenesCmd(flags))
	...
	rootCmd.AddCommand(newExcalidrawCanvasCloudExportPromotedCmd(flags))
	rootCmd.AddCommand(newExcalidrawCanvasCloudHealthPromotedCmd(flags))
	rootCmd.AddCommand(newExcalidrawCanvasCloudSyncPromotedCmd(flags))
	rootCmd.AddCommand(newLogsPromotedCmd(flags))
	rootCmd.AddCommand(newViewportPromotedCmd(flags))
	...
	rootCmd.AddCommand(newAgentCanvasContextCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("excalidraw-mcp-pp-cli %s\n", version)
		},
	}
}
