...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  excalidraw-mcp-pp-cli doctor
  excalidraw-mcp-pp-cli doctor --json
  excalidraw-mcp-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export EXCALIDRAW_API_KEY=<your-key>"
				} else {
					authConfigured = true
			...
			if strings.Contains("Excalidraw Plus API key (required for cloud/* commands only). Get from your workspace settings at plus.excalidraw.com.", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "EXCALIDRAW_API_KEY")
				if os.Getenv("EXCALIDRAW_API_KEY") != "" {
					authEnvSet = append(authEnvSet, "EXCALIDRAW_API_KEY")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("EXCALIDRAW_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "EXCALIDRAW_API_KEY")
			} else {
				authEnvInfo = append(authEnvInfo, "EXCALIDRAW_API_KEY optional")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("excalidraw-mcp-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'excalidraw-mcp-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'excalidraw-mcp-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'excalidraw-mcp-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'excalidraw-mcp-pp-cli sync' to refresh."
	}
	return report
}
