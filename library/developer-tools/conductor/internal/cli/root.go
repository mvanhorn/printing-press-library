...
type rootFlags struct {
	...
	// invocation. Mirrors the CONDUCTOR_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "conductor-pp-cli",
		Short: `Conductor CLI — Every Conductor Cloud API primitive, plus bounded session orchestration that handles asynchronous lifecycle races.`,
...
Health check: run 'conductor-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("conductor-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'conductor-pp-cli profile list')")
	...
	rootCmd.AddCommand(newRoundhousePublicSqlPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelDailyReportCmd(flags))
