...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CONDUCTOR_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CONDUCTOR_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "conductor-pp-cli/feedback")
...
When ` + "`CONDUCTOR_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CONDUCTOR_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  conductor-pp-cli feedback list
  conductor-pp-cli feedback list --limit 5
  conductor-pp-cli feedback list --json`,
