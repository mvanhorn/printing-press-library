...
Run sync first to populate the local search index.`,
  ...
  conductor-pp-cli search "status"

  # Force local search explicitly
  conductor-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  conductor-pp-cli search "status" --type me --data-source local
  # JSON output for piping
  conductor-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'conductor-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
