...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/developer-tools/conductor/internal/learn/lookups"
...
Resource scoping:
  ...
  conductor-pp-cli sync

  # Sync specific resources only
  conductor-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  conductor-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  conductor-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  conductor-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  conductor-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("conductor-pp-cli")
			}
...
func determinePaginationDefaults(resource string) paginationDefaults {
	...
	case "projects/projects_workspaces":
		return paginationDefaults{
			cursorParam: "offset",
			cursorType:  "offset",
			limitParam:  "limit",
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "projects_workspaces":
		return db.UpsertProjectsWorkspaces(data)
	case "sessions":
		return db.UpsertSessions(data)
	case "sessions_archive":
		return db.UpsertSessionsArchive(data)
	case "cancel":
		return db.UpsertCancel(data)
	case "sessions_messages":
		return db.UpsertSessionsMessages(data)
	case "sessions_rename":
		return db.UpsertSessionsRename(data)
	case "sessions_status":
		return db.UpsertSessionsStatus(data)
	case "workspaces":
		return db.UpsertWorkspaces(data)
	case "workspaces_archive":
		return db.UpsertWorkspacesArchive(data)
	case "workspaces_rename":
		return db.UpsertWorkspacesRename(data)
	case "workspaces_sessions":
		return db.UpsertWorkspacesSessions(data)
	case "sleep":
		return db.UpsertSleep(data)
	case "workspaces_status":
		return db.UpsertWorkspacesStatus(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		"messages": "/v0/sessions/{sessionId}/messages",
		"projects": "/v0/projects",
		"sessions": "/v0/workspaces/{workspaceId}/sessions",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "projects_workspaces", ParentTable: "projects", ParentIDParam: "projectId", PathTemplate: "/v0/projects/{projectId}/workspaces", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.project", PathParams: []dependentPathParamDef{
...
var reconcileTypedTables = map[string]string{
	...
	"messages":            "messages",
	"projects":            "projects",
	"projects_workspaces": "projects_workspaces",
	"sessions":            "sessions",
	...
	"workspaces_rename":   "workspaces_rename",
	"workspaces_sessions": "workspaces_sessions",
	"workspaces_status":   "workspaces_status",
}
...
func responsePathForResource(resource, path string) []string {
	...
	case "projects\x00/v0/projects":
		return []string{"data"}
	case "roundhouse-public-sql\x00/v0/sql":
		return []string{"rows"}
	}
	return nil
}
