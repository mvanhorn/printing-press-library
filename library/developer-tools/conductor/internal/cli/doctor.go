...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  conductor-pp-cli doctor
  conductor-pp-cli doctor --json
  conductor-pp-cli doctor --fail-on warn
  conductor-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if !configured {
					...
					report["auth_hint"] = "Set one of: export CONDUCTOR_API_KEY=\"your-token-here\" or export CONDUCTOR_BEARER_AUTH=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "CONDUCTOR_API_KEY")
				if os.Getenv("CONDUCTOR_API_KEY") != "" {
					authEnvSet = append(authEnvSet, "CONDUCTOR_API_KEY")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("CONDUCTOR_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "CONDUCTOR_API_KEY")
			} else {
				authEnvInfo = append(authEnvInfo, "CONDUCTOR_API_KEY optional")
			}
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "CONDUCTOR_BEARER_AUTH")
				if os.Getenv("CONDUCTOR_BEARER_AUTH") != "" {
					authEnvSet = append(authEnvSet, "CONDUCTOR_BEARER_AUTH")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("CONDUCTOR_BEARER_AUTH") != "" {
				authEnvSet = append(authEnvSet, "CONDUCTOR_BEARER_AUTH")
			} else {
				authEnvInfo = append(authEnvInfo, "CONDUCTOR_BEARER_AUTH optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "conductor-pp-cli"
						verifyPath := "/me"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'conductor-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'conductor-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		...
		report["hint"] = "No sync recorded; run 'conductor-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'conductor-pp-cli sync' to refresh."
	}
	return report
}
