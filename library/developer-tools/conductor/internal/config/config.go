...
type Config struct {
	...
	ConductorApiKey     string          `toml:"api_key"`
	ConductorBearerAuth string          `toml:"bearer_auth"`
...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.conductor.build",
	}
	...
	if v := os.Getenv("CONDUCTOR_API_KEY"); v != "" {
		...
		cfg.markEnvOverride("ConductorApiKey")
		cfg.AuthSource = "env:CONDUCTOR_API_KEY"
		cfg.CredentialSource = "env:CONDUCTOR_API_KEY"
	}
	if v := os.Getenv("CONDUCTOR_BEARER_AUTH"); v != "" {
		cfg.ConductorBearerAuth = v
		cfg.markEnvOverride("ConductorBearerAuth")
		cfg.AuthSource = "env:CONDUCTOR_BEARER_AUTH"
		cfg.CredentialSource = "env:CONDUCTOR_BEARER_AUTH"
	}
	...
	if v := os.Getenv("CONDUCTOR_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("CONDUCTOR_SESSION_ID")); v != "" {
		cfg.TemplateVars["sessionId"] = v
	} else if verifyMode {
		cfg.TemplateVars["sessionId"] = "sessionId_placeholder"
	}
	if v := strings.TrimSpace(os.Getenv("CONDUCTOR_WORKSPACE_ID")); v != "" {
		cfg.TemplateVars["workspaceId"] = v
	} else if verifyMode {
		cfg.TemplateVars["workspaceId"] = "workspaceId_placeholder"
	}
	return cfg, nil
}
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "conductor-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.ConductorApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:CONDUCTOR_API_KEY"
		}
		return "Bearer " + c.ConductorApiKey
	}
	if c.ConductorBearerAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:CONDUCTOR_BEARER_AUTH"
		}
		return "Bearer " + c.ConductorBearerAuth
	}
	if c.AccessToken != "" {
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		ConductorApiKey:     c.ConductorApiKey,
		ConductorBearerAuth: c.ConductorBearerAuth,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.ConductorApiKey == "" {
		c.ConductorApiKey = creds.ConductorApiKey
	}
	if c.ConductorBearerAuth == "" {
		c.ConductorBearerAuth = creds.ConductorBearerAuth
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.ConductorApiKey = ""
	delete(c.envOverrides, "ConductorApiKey")
	c.updateFileConfigField("ConductorApiKey")
	c.ConductorBearerAuth = ""
	delete(c.envOverrides, "ConductorBearerAuth")
	c.updateFileConfigField("ConductorBearerAuth")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["ConductorApiKey"] {
			out.ConductorApiKey = c.fileConfig.ConductorApiKey
		}
		if c.envOverrides["ConductorBearerAuth"] {
			out.ConductorBearerAuth = c.fileConfig.ConductorBearerAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "ConductorApiKey":
		c.fileConfig.ConductorApiKey = c.ConductorApiKey
	case "ConductorBearerAuth":
		c.fileConfig.ConductorBearerAuth = c.ConductorBearerAuth
	}
}
