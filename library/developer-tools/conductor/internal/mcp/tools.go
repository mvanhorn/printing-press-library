...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/developer-tools/conductor/internal/learn"
	"github.com/mvanhorn/printing-press-library/library/developer-tools/conductor/internal/mcp/bound"
	"github.com/mvanhorn/printing-press-library/library/developer-tools/conductor/internal/mcp/cobratree"
	"github.com/mvanhorn/printing-press-library/library/developer-tools/conductor/internal/platform"
	"github.com/mvanhorn/printing-press-library/library/developer-tools/conductor/internal/store"
)
...
func RegisterTools(s *server.MCPServer) {
	...
	s.AddTool(
		mcplib.NewTool("sql",
			...
			mcplib.WithString("query", mcplib.Required(), mcplib.Description("SQL query (SELECT or WITH...SELECT). Synced records live in resources(resource_type, id, data); filter by resource_type and use json_extract on data, e.g. SELECT json_extract(data,'$.name') FROM resources WHERE resource_type='me'.")),
			mcplib.WithReadOnlyHintAnnotation(true),
			mcplib.WithDestructiveHintAnnotation(false),
		),
		handleSQL,
		...
		if err != nil {
			...
			case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
				return mcpToolError("authentication error: " + cliutil.SanitizeErrorBody(msg) +
					...
					"\n      Set one of: export CONDUCTOR_API_KEY=\"your-token-here\" or export CONDUCTOR_BEARER_AUTH=\"your-token-here\"" +
					"\n      Run 'conductor-pp-cli doctor' to check auth status."), nil
...
func openMCPReadOnlyStore(path string) (*store.Store, *mcplib.CallToolResult) {
	...
	if err != nil {
		return nil, mcplib.NewToolResultError(fmt.Sprintf("opening local data store %s: %v. Run conductor-pp-cli sync to refresh the store, or use live endpoint MCP tools for unsynced data.", path, err))
	}
	return db, nil
}

func mcpMissingStoreMessage(path string) string {
	return fmt.Sprintf("No local data store found at %s. Run conductor-pp-cli sync before using MCP search/sql, or use live endpoint MCP tools for unsynced data.", path)
}
...
func mcpEmptyStoreNextStep() string {
	return "Run conductor-pp-cli sync to populate the local SQLite store before using MCP search/sql."
}
...
func mcpSQLQueryError(err error) string {
	...
	if strings.Contains(strings.ToLower(msg), "no such table") {
		return fmt.Sprintf("query failed: %v. Synced records live in resources(resource_type, id, data), not one SQL table per resource. Filter by resource_type, for example resource_type='me', and read JSON fields with json_extract(data,'$.field').", err)
	}
...
func handleContext(_ context.Context, _ mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	ctx := map[string]any{
		"api":         "conductor",
		"description": "Every Conductor Cloud API primitive, plus bounded session orchestration that handles asynchronous lifecycle races.",
		"archetype":   "communication",
		"tool_count":  21,
		"paths":       paths,
		// tool_surface tells agents which surface a capability lives on.
		"tool_surface": "MCP exposes typed endpoint tools plus a runtime mirror of user-facing CLI commands. Endpoint tools keep typed schemas; command-mirror tools shell out to the companion conductor-pp-cli binary.",
		...
		"resources": []map[string]any{
			...
			{
				"name":        "roundhouse-public-sql",
				"description": "Manage roundhouse public sql",
			...
			{
				"name":        "workspaces.sessions",
				"description": "Manage sessions",
				"endpoints":   []string{"list"},
			},
			{
				"name":        "workspaces.sleep",
				"description": "Manage sleep",
				"endpoints":   []string{"workspace"},
				"writable":    true,
			},
			{
		...
		"command_mirror_capabilities": []map[string]string{
			...
			{"name": "Run", "command": "run", "description": "Launch, monitor, and collect the final transcript and deep link with explicit timeout rules.", "rationale": "Joins asynchronous lifecycle operations into a single bounded receipt without merging or deploying.", "via": "mcp-command-mirror"},
			{"name": "Plan and implement", "command": "plan-implement", "description": "Keep a planner or reviewer session separate from the implementation session in one workspace.", "rationale": "Uses Conductor's multi-session workspace model to preserve role separation and context boundaries.", "via": "mcp-command-mirror"},
			{"name": "Daily report", "command": "daily-report", "description": "Return recent Conductor session rows and mechanical activity totals from transcript search.", "rationale": "The read-only transcript SQL endpoint can assemble cross-workspace activity without the desktop app.", "via": "mcp-command-mirror"},
		},
		"playbook": []map[string]string{
			{"topic": "Launch", "insight": "Combines workspace launch, message delivery, and identifiers into one bounded operator action."},
			{"topic": "Monitor", "insight": "Correct completion needs status history plus transcript changes to avoid Conductor's queued false-idle race."},
			{"topic": "Steer", "insight": "Turns the message primitive into an agent-safe intervention command."},
			{"topic": "Run", "insight": "Joins asynchronous lifecycle operations into a single bounded receipt without merging or deploying."},
			{"topic": "Plan and implement", "insight": "Uses Conductor's multi-session workspace model to preserve role separation and context boundaries."},
			{"topic": "Daily report", "insight": "The read-only transcript SQL endpoint can assemble cross-workspace activity without the desktop app."},
