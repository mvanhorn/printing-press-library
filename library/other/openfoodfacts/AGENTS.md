# Open Food Facts Printed CLI Agent Guide

This directory is a generated `openfoodfacts-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
openfoodfacts-pp-cli doctor --json
openfoodfacts-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
openfoodfacts-pp-cli which "<capability>" --json
openfoodfacts-pp-cli <command> --help
```
...
```bash
openfoodfacts-pp-cli <command> --agent
```
...
```bash
openfoodfacts-pp-cli <command> --help
openfoodfacts-pp-cli <command> --dry-run --agent
```
