...
type Config struct {
	...
	OpenfoodfactsUserAgentAuth string            `toml:"user_agent_auth"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("OPENFOODFACTS_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "openfoodfacts-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("OPENFOODFACTS_USER_AGENT_AUTH"); v != "" {
		cfg.OpenfoodfactsUserAgentAuth = v
		cfg.markEnvOverride("OpenfoodfactsUserAgentAuth")
		cfg.AuthSource = "env:OPENFOODFACTS_USER_AGENT_AUTH"
	}
	...
	if cfg.AuthSource == "" && cfg.OpenfoodfactsUserAgentAuth != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("OPENFOODFACTS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.OpenfoodfactsUserAgentAuth
	if token == "" {
		return ""
	}
	if c.OpenfoodfactsUserAgentAuth == "" {
		return ""
	}
	return token
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.OpenfoodfactsUserAgentAuth = token
	delete(c.envOverrides, "OpenfoodfactsUserAgentAuth")
	c.updateFileConfigField("OpenfoodfactsUserAgentAuth")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["OpenfoodfactsUserAgentAuth"] {
			out.OpenfoodfactsUserAgentAuth = c.fileConfig.OpenfoodfactsUserAgentAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "OpenfoodfactsUserAgentAuth":
		c.fileConfig.OpenfoodfactsUserAgentAuth = c.OpenfoodfactsUserAgentAuth
	}
}
