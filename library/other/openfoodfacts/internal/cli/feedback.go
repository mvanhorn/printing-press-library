...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "openfoodfacts-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("OPENFOODFACTS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("OPENFOODFACTS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "openfoodfacts-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/openfoodfacts-pp-cli/feedback.jsonl.
When ` + "`OPENFOODFACTS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`OPENFOODFACTS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  openfoodfacts-pp-cli feedback list
  openfoodfacts-pp-cli feedback list --limit 5
  openfoodfacts-pp-cli feedback list --json`,
