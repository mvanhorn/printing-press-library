...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  openfoodfacts-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  openfoodfacts-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Mirror the default sync set, which deliberately omits `find`
			// (product search): Open Food Facts rate-limits search and asks
			// clients not to crawl it. Archive the small, stable resources by
			// default; pull the search corpus on demand with `sync --resources find`.
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  openfoodfacts-pp-cli workflow status

  # Show status as JSON
  openfoodfacts-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("openfoodfacts-pp-cli")
			}
