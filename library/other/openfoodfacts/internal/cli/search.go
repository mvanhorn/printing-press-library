...
In local mode: searches locally synced data only.`,
  ...
  openfoodfacts-pp-cli search "error timeout"

  # Force local search only
  openfoodfacts-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  openfoodfacts-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  openfoodfacts-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /api/v2/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/api/v2/search", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'openfoodfacts-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "find":
				results, err = db.SearchFind(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchFind(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search find failed: %w", searchErr)
					}
