...
Resource scoping:
  ...
  openfoodfacts-pp-cli sync

  # Sync specific resources only
  openfoodfacts-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  openfoodfacts-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  openfoodfacts-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  openfoodfacts-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  openfoodfacts-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/openfoodfacts-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "attribute-groups":
		return db.UpsertAttributeGroups(data)
	case "cgi":
		return db.UpsertCgi(data)
	case "find":
		return db.UpsertFind(data)
	case "preferences":
		return db.UpsertPreferences(data)
	default:
...
func defaultSyncResources() []string {
	// `find` (product search) is intentionally excluded from the default set:
	// Open Food Facts rate-limits search to ~10 req/min/IP and explicitly asks
	// clients not to crawl it (use the data dump for bulk). Pulling the full
	// search corpus by default trips the limiter. Sync it on demand with
	// `sync --resources find` (the offline `rank` command guides users there).
	return []string{
		"attribute-groups",
		"cgi",
		"preferences",
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"attribute-groups": "/api/v2/attribute_groups",
		"cgi":              "/cgi/nutrients.pl",
		"find":             "/api/v2/search",
