...
// tailResourceKnown reports whether name is a resource tail can poll.
func tailResourceKnown(name string) bool {
...
native streaming instead of polling.`,
  ...
  openfoodfacts-pp-cli tail --interval 10s

  # Tail a specific resource
  openfoodfacts-pp-cli tail messages --interval 5s

  # Pipe to jq for filtering
  openfoodfacts-pp-cli tail events --interval 30s | jq 'select(.type == "error")'`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !tailResourceKnown(resource) {
				return usageErr(fmt.Errorf("unknown resource %q; known resources: %s", resource, strings.Join(tailKnownResources(), ", ")))
			}
