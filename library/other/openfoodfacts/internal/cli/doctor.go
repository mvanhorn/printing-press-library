...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  openfoodfacts-pp-cli doctor
  openfoodfacts-pp-cli doctor --json
  openfoodfacts-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export OPENFOODFACTS_USER_AGENT_AUTH=<your-key>"
					report["auth_docs_url"] = "https://slack.openfoodfacts.org/"
				} else {
					authConfigured = true
			...
			if os.Getenv("OPENFOODFACTS_USER_AGENT_AUTH") != "" {
				authEnvSet = append(authEnvSet, "OPENFOODFACTS_USER_AGENT_AUTH")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "OPENFOODFACTS_USER_AGENT_AUTH")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "openfoodfacts-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("openfoodfacts-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'openfoodfacts-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'openfoodfacts-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'openfoodfacts-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'openfoodfacts-pp-cli sync' to refresh."
	}
	return report
}
