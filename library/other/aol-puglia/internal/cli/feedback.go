...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "aol-puglia-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AOL_PUGLIA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AOL_PUGLIA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "aol-puglia-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/aol-puglia-pp-cli/feedback.jsonl.
When ` + "`AOL_PUGLIA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AOL_PUGLIA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  aol-puglia-pp-cli feedback list
  aol-puglia-pp-cli feedback list --limit 5
  aol-puglia-pp-cli feedback list --json`,
