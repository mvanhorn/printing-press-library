...
type Config struct {
	...
	AolPugliaBearerAuth string            `toml:"puglia_bearer_auth"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://sanita.puglia.it/AlboOnline/ao",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("AOL_PUGLIA_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "aol-puglia-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("AOL_PUGLIA_BEARER_AUTH"); v != "" {
		cfg.AolPugliaBearerAuth = v
		cfg.markEnvOverride("AolPugliaBearerAuth")
		cfg.AuthSource = "env:AOL_PUGLIA_BEARER_AUTH"
	}
	...
	if cfg.AuthSource == "" && cfg.AolPugliaBearerAuth != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("AOL_PUGLIA_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.AolPugliaBearerAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:AOL_PUGLIA_BEARER_AUTH"
		}
		return "Bearer " + c.AolPugliaBearerAuth
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientID")
	c.updateFileConfigField("ClientSecret")
	c.AolPugliaBearerAuth = ""
	delete(c.envOverrides, "AolPugliaBearerAuth")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["AolPugliaBearerAuth"] {
			out.AolPugliaBearerAuth = c.fileConfig.AolPugliaBearerAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "AolPugliaBearerAuth":
		c.fileConfig.AolPugliaBearerAuth = c.AolPugliaBearerAuth
	}
}
