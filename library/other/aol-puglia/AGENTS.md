# Aol Puglia Printed CLI Agent Guide

This directory is a generated `aol-puglia-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
aol-puglia-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
aol-puglia-pp-cli which "<capability>" --json
aol-puglia-pp-cli <command> --help
```
...
```bash
aol-puglia-pp-cli <command> --agent
```
...
```bash
aol-puglia-pp-cli <command> --help
aol-puglia-pp-cli <command> --dry-run --agent
```
