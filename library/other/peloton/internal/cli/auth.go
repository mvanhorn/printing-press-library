...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	...
	// PATCH(catalog/peloton): register newAuthLoginCmd from auth_login.go
	// for chromedp Auth0 SPA bearer-token harvest. Recorded in
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintf(w, "  peloton-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  peloton-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("PELOTON_USERNAME") != "" {
				envStillSet = "PELOTON_USERNAME"
			}
			if envStillSet == "" && os.Getenv("PELOTON_PASSWORD") != "" {
				envStillSet = "PELOTON_PASSWORD"
			}
			if envStillSet == "" && os.Getenv("PELOTON_TOKEN") != "" {
				envStillSet = "PELOTON_TOKEN"
			}
