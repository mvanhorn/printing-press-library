		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export PELOTON_USERNAME=<your-key>"
				} else {
			...
			if os.Getenv("PELOTON_USERNAME") != "" {
				authEnvSet = append(authEnvSet, "PELOTON_USERNAME")
			} else {
				authEnvInfo = append(authEnvInfo, "PELOTON_USERNAME set during auth login")
			}
			if os.Getenv("PELOTON_PASSWORD") != "" {
				authEnvSet = append(authEnvSet, "PELOTON_PASSWORD")
			} else {
				authEnvInfo = append(authEnvInfo, "PELOTON_PASSWORD set during auth login")
			}
			if os.Getenv("PELOTON_TOKEN") != "" || authHeader != "" {
				authEnvSet = append(authEnvSet, "PELOTON_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "PELOTON_TOKEN set with auth set-token")
			}
			switch {
...
func browserSessionProofStatus(cfg *config.Config, authHeader string) (bool, string) {
	...
	if err != nil {
		return false, "proof not found; run peloton-pp-cli auth login --chrome"
	}
	var proof doctorBrowserSessionProof
	if err := json.Unmarshal(data, &proof); err != nil {
		return false, "proof is not valid JSON; re-run peloton-pp-cli auth login --chrome"
	}
	if proof.APIName != "peloton" {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'peloton-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
