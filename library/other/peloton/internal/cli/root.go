...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "peloton-pp-cli",
		Short: `Workout, ride, and music history from your Peloton account in the terminal. First catalog CLI to harvest the Auth0 SPA bearer token from a browser session, then mirror history into a local SQLite sto…`,
		Long: `Workout, ride, and music history from your Peloton account in the terminal. First catalog CLI to harvest the Auth0 SPA bearer token from a browser session, then mirror history into a local SQLite sto…
...
Run 'peloton-pp-cli doctor' to verify auth and connectivity.`,
	...
	rootCmd.AddCommand(newRidesPromotedCmd(flags))
	// PATCH(catalog/peloton): novel feature command — discoveries.go.
	// Recorded in .printing-press-patches.json.
	rootCmd.AddCommand(newDiscoveriesCmd(flags))
