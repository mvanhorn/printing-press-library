...
// TestMigrate_AddsColumnsOnUpgrade_Rides verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_Rides(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE rides (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
	...
	rows, err := s.DB().Query(`PRAGMA table_info(rides)`)
	if err != nil {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from rides after migrate", want)
		}
	}
}
