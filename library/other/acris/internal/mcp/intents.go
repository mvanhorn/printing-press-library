...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("full_picture_of_one_recording",
			mcplib.WithDescription("Merges the master record, all property legals (BBLs), and all parties for a single document."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Override the recipe's positional id value.")),
		),
		handleFullPictureOfOneRecording,
	)
}

// handleFullPictureOfOneRecording runs the full_picture_of_one_recording recipe intent: Merges the master record, all property legals (BBLs), and all parties for a single document.
func handleFullPictureOfOneRecording(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
