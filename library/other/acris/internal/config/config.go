...
type Config struct {
	...
	AcrisAppToken    string          `toml:"app_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("ACRIS_APP_TOKEN"); v != "" {
		cfg.AcrisAppToken = v
		cfg.markEnvOverride("AcrisAppToken")
		cfg.AuthSource = "env:ACRIS_APP_TOKEN"
		cfg.CredentialSource = "env:ACRIS_APP_TOKEN"
	}
	...
	if v := os.Getenv("ACRIS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("ACRIS_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "acris-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.AcrisAppToken
	if token == "" {
		return ""
	}
	replacements := map[string]string{
		"token":           token,
		"app_token":       c.AcrisAppToken,
		"ACRIS_APP_TOKEN": c.AcrisAppToken,
	}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.AcrisAppToken = creds.AcrisAppToken
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.AcrisAppToken = token
	delete(c.envOverrides, "AcrisAppToken")
	c.updateFileConfigField("AcrisAppToken")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["AcrisAppToken"] {
			out.AcrisAppToken = c.fileConfig.AcrisAppToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "AcrisAppToken":
		c.fileConfig.AcrisAppToken = c.AcrisAppToken
	}
}
