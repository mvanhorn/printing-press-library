...
Resource scoping:
  ...
  acris-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  acris-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  acris-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  acris-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  acris-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "doc-types":
		return db.UpsertDocTypes(data)
	case "documents":
		return db.UpsertDocuments(data)
	case "legals":
		return db.UpsertLegals(data)
	case "parties":
		return db.UpsertParties(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"doc-types": "/7isb-wh4c.json",
		"documents": "/bnx9-e6tj.json",
		"legals":    "/8h5j-fqxa.json",
		"parties":   "/636b-3b5g.json",
	}
