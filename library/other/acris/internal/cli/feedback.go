...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ACRIS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ACRIS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "acris-pp-cli/feedback")
...
When ` + "`ACRIS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ACRIS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  acris-pp-cli feedback list
  acris-pp-cli feedback list --limit 5
  acris-pp-cli feedback list --json`,
