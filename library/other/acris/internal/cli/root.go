...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "acris-pp-cli",
		Short: `Acris CLI — Query NYC property records — deeds, mortgages, parties, and full BBL history — straight from NYC Open Data, with cross-…`,
		Long: `Acris CLI — Query NYC property records — deeds, mortgages, parties, and full BBL history — straight from NYC Open Data, with cross-…
...
Health check: run 'acris-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("acris-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'acris-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelBblCmd(flags))
	rootCmd.AddCommand(newNovelDebtCmd(flags))
	rootCmd.AddCommand(newNovelDocumentCmd(flags))
	rootCmd.AddCommand(newNovelPartySearchCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newDocTypesPromotedCmd(flags))
	rootCmd.AddCommand(newLegalsPromotedCmd(flags))
