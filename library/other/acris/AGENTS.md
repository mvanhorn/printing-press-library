# ACRIS Printed CLI Agent Guide

This directory is a generated `acris-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
acris-pp-cli doctor --json
acris-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
acris-pp-cli which "<capability>" --json
acris-pp-cli <command> --help
```
...
```bash
acris-pp-cli <command> --help
acris-pp-cli <command> --dry-run --agent
```
