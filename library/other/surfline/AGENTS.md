# Surfline Printed CLI Agent Guide

This directory is a generated `surfline-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
surfline-pp-cli doctor --json
surfline-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
surfline-pp-cli which "<capability>" --json
...
```bash
surfline-pp-cli <command> --help
surfline-pp-cli <command> --dry-run --agent
```
