...
Resource scoping:
  ...
  surfline-pp-cli sync

  # Sync specific resources only
  surfline-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  surfline-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  surfline-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  surfline-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  surfline-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Surfline has no bulk-listable resource: every endpoint needs a
			// spotId, subregionId, or search query. A blanket `sync` therefore
			// has nothing to pull. Report that honestly and exit 0 rather than
			// firing a parameterless request the API rejects with HTTP 500.
			if len(resources) == 0 {
				if flags.asJSON || flags.agent {
					fmt.Fprintln(cmd.OutOrStdout(), `{"event":"sync_noop","reason":"no bulk-listable resources"}`)
				} else {
					fmt.Fprintln(cmd.OutOrStdout(), "Nothing to sync: Surfline has no bulk-listable resources.")
					fmt.Fprintln(cmd.OutOrStdout(), "Use 'surfline-pp-cli spots find <name>' for live lookups, or 'surfline-pp-cli journal log <spotId>' to build local history.")
				}
				return nil
			}
...
func defaultSyncResources() []string {
	// Surfline has no parameterless list endpoint, so there is nothing to bulk
	// sync. See the no-op guard in the sync RunE.
	return []string{}
}
