...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  surfline-pp-cli doctor
  surfline-pp-cli doctor --json
  surfline-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export SURFLINE_ACCESS_TOKEN=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("SURFLINE_ACCESS_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "SURFLINE_ACCESS_TOKEN")
				...
				// Auth is optional for Surfline: basic forecasts/search/taxonomy
				// work with no token. A missing token is informational, not an
				// error. (The generator's auth.Optional flag updates the Auth
				// line but not this env-var block — patched here; see retro.)
				authEnvInfo = append(authEnvInfo, "SURFLINE_ACCESS_TOKEN not set (optional — enables 7–17 day forecasts and premium cams)")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					healthPath := "/kbyg/spots/forecasts/wave?spotId=5842041f4e65fad6a7708807&days=1"
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "surfline-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("surfline-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'surfline-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'surfline-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'surfline-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'surfline-pp-cli sync' to refresh."
	}
	return report
}
