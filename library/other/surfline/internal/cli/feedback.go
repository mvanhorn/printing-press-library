...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "surfline-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SURFLINE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SURFLINE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "surfline-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/surfline-pp-cli/feedback.jsonl.
When ` + "`SURFLINE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SURFLINE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  surfline-pp-cli feedback list
  surfline-pp-cli feedback list --limit 5
  surfline-pp-cli feedback list --json`,
