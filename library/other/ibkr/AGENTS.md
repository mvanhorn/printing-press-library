# Ibkr Printed CLI Agent Guide

This directory is a generated `ibkr-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
ibkr-pp-cli doctor --json
ibkr-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
ibkr-pp-cli which "<capability>" --json
ibkr-pp-cli <command> --help
```
...
```bash
ibkr-pp-cli <command> --help
ibkr-pp-cli <command> --dry-run --agent
```
