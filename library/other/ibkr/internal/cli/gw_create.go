...
func newGwCreateCmd(flags *rootFlags) *cobra.Command {
	var bodyApplication string
	var stdinBody bool
	var flagWait bool
	...
	cmd := &cobra.Command{
		Use:   "create",
		Short: "Submit account application. This will create brokerage account for the end user.<br><br>**Scope**:...",
		Example: "  ibkr-pp-cli gw create",
		Annotations: map[string]string{"pp:endpoint": "gw.create", "pp:method": "POST", "pp:path": "/gw/api/v1/accounts"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if stdinBody {
				...
				body = map[string]any{}
				if bodyApplication != "" {
					var parsedApplication any
					if err := json.Unmarshal([]byte(bodyApplication), &parsedApplication); err != nil {
						return fmt.Errorf("parsing --application JSON: %w", err)
					}
					body["application"] = parsedApplication
				}
			}
	...
	cmd.Flags().StringVar(&bodyApplication, "application", "", "Application")
