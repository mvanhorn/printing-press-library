...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  ibkr-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  ibkr-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"forecast", "fyi", "fyi-unreadnumber", "gw", "gw-api-v1-accounts", "gw-api-v1-echo-https", "gw-api-v1-tax-vouchers-active-countries", "gw-api-v1-tax-vouchers-years", "iserver", "iserver-account-allocation-accounts", "iserver-account-allocation-group", "iserver-account-allocation-presets", "iserver-account-orders", "iserver-account-pnl-partitioned", "iserver-account-trades", "iserver-marketdata-unsubscribeall", "iserver-secdef-info", "iserver-watchlists", "portfolio", "portfolio-subaccounts", "portfolio-subaccounts2", "sso",  }
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  ibkr-pp-cli workflow status

  # Show status as JSON
  ibkr-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("ibkr-pp-cli")
			}
