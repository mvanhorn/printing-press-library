...
In local mode: searches locally synced data only.`,
  ...
  ibkr-pp-cli search "error timeout"

  # Force local search only
  ibkr-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  ibkr-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  ibkr-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /iserver/secdef/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post("/iserver/secdef/search", map[string]any{
					"q": query,
					"fund": nil,
					"fundFamilyConidEx": "",
					"more": nil,
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'ibkr-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "gw":
				results, err = db.SearchGw(query, limit)
			case "iserver":
				results, err = db.SearchIserver(query, limit)
			case "":
				// Search all FTS-enabled tables individually to avoid duplicates.
				seen := make(map[string]bool)
				_ = seen // prevent unused error when no FTS tables exist
				{
					partial, searchErr := db.SearchGw(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search gw failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchIserver(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search iserver failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
