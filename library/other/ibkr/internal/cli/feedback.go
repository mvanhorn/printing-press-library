...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".ibkr-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("IBKR_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("IBKR_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ibkr-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.ibkr-pp-cli/feedback.jsonl.
When ` + "`IBKR_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`IBKR_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ibkr-pp-cli feedback list
  ibkr-pp-cli feedback list --limit 5
  ibkr-pp-cli feedback list --json`,
