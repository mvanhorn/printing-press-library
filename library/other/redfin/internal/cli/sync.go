...
func extractPageItems(data json.RawMessage, cursorParam string) ([]json.RawMessage, string, bool) {
	...
	// Deep scan: some APIs nest the items array inside a wrapper object
	// (e.g. Redfin's {"payload":{"homes":[...],"totalRowCount":N}}).
	// Scan one level deeper into each object-valued top-level key and try
	// nestedPageItemKeys. First match wins so ambiguous nested shapes are
	// handled by the nestedPageItemKeys priority order.
	for _, objRaw := range envelope {
		var nestedObj map[string]json.RawMessage
		if err := json.Unmarshal(objRaw, &nestedObj); err != nil {
			continue // not an object
		}
		for _, key := range nestedPageItemKeys {
			if raw, ok := nestedObj[key]; ok {
				var nestedItems []json.RawMessage
				if err := json.Unmarshal(raw, &nestedItems); err == nil && len(nestedItems) > 0 {
					nextCursor, hasMore := extractPaginationFromEnvelope(envelope, cursorParam)
					return nestedItems, nextCursor, hasMore
				}
			}
		}
	}
...
var resourceIDFieldOverrides = map[string]string{
	// Redfin listing objects use listingId as the stable identifier.
	"listings": "listingId",
}
...
// nestedPageItemKeys extends pageItemKeys with domain-specific array names
// that appear inside nested wrapper objects (e.g. Redfin's payload.homes).
var nestedPageItemKeys = []string{
	"homes", "listings", "properties", "units",
	"data", "results", "items", "records", "nodes", "entries",
}
