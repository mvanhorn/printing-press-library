...
In local mode: searches locally synced data only.`,
  ...
  cfpb-complaints-pp-cli search "error timeout"

  # Force local search only
  cfpb-complaints-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  cfpb-complaints-pp-cli search "status" --type data-research --data-source local
  # JSON output for piping
  cfpb-complaints-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /data-research/consumer-complaints/search/api/v1/
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/data-research/consumer-complaints/search/api/v1/", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'cfpb-complaints-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
