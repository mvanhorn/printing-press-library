...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CFPB_COMPLAINTS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CFPB_COMPLAINTS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "cfpb-complaints-pp-cli/feedback")
...
When ` + "`CFPB_COMPLAINTS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CFPB_COMPLAINTS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cfpb-complaints-pp-cli feedback list
  cfpb-complaints-pp-cli feedback list --limit 5
  cfpb-complaints-pp-cli feedback list --json`,
