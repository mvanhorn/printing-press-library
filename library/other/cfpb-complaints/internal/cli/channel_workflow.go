...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  cfpb-complaints-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  cfpb-complaints-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"data-research"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  cfpb-complaints-pp-cli workflow status

  # Show status as JSON
  cfpb-complaints-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("cfpb-complaints-pp-cli")
			}
