...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  fpi-india-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  fpi-india-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"cdsl_reports", "registry", "sector"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  fpi-india-pp-cli workflow status

  # Show status as JSON
  fpi-india-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("fpi-india-pp-cli")
			}
