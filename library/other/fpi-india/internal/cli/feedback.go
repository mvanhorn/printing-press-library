...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FPI_INDIA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FPI_INDIA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "fpi-india-pp-cli/feedback")
...
When ` + "`FPI_INDIA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FPI_INDIA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  fpi-india-pp-cli feedback list
  fpi-india-pp-cli feedback list --limit 5
  fpi-india-pp-cli feedback list --json`,
