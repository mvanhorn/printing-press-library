...
Run sync first to populate the local search index.`,
  ...
  fpi-india-pp-cli search "status"

  # Force local search explicitly
  fpi-india-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  fpi-india-pp-cli search "status" --type auc --data-source local
  # JSON output for piping
  fpi-india-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'fpi-india-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
