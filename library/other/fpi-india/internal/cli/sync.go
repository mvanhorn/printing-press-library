...
Resource scoping:
  ...
  fpi-india-pp-cli sync

  # Sync specific resources only
  fpi-india-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  fpi-india-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  fpi-india-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  fpi-india-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  fpi-india-pp-cli sync --latest-only`,
...
func syncResource(ctx context.Context, c interface {
	...
	// NSDL/CDSL have no API: every resource here is a hand-parsed HTML
	// report page, not a paginated JSON REST collection. nsdlSyncHandlers
	// (internal/cli/nsdl_sync.go) intercepts before the generic REST
	// pagination logic below, which does not apply to these resources.
	if handler, ok := nsdlSyncHandlers[resource]; ok {
		return handler(ctx, c, db, resource, syncEvents, started)
	}
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "limits":
		...
		return db.UpsertLimits(typedData)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		"cdsl_reports": "/Publications/ForeignPortInvestor.html",
		"registry":     "/web/Reports/RegisteredFIISAFPI.aspx",
		"sector":       "/web/Reports/FPI_Fortnightly_Selection.aspx",
	}
