...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://onlineservices.miamidadeclerk.gov/officialrecords",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("MIAMI_DADE_CLERK_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "miami-dade-clerk-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("MIAMI_DADE_CLERK_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
