...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "miami-dade-clerk-pp-cli", "http")
...
// PostSearchWithRecaptcha submits a clerk portal search to one of the
// two reCAPTCHA-protected endpoints (/api/home/standardsearch or
// /api/home/propertysearch). It acquires a fresh single-use reCAPTCHA
// Enterprise v3 token via the headless Chromium driver and attaches it
// as x-recaptcha-token. The clerk's reCAPTCHA tokens are bound to the
// originating site key + page and expire after 2 minutes — we mint one
// per call rather than caching across calls.
//
// path must start with "/api/home/" (caller-chosen between standard or
// property search). params are sent as querystring (matches the portal's
// observed shape — body is empty on submit).
func (c *Client) PostSearchWithRecaptcha(ctx context.Context, path string, params map[string]string) (json.RawMessage, int, error) {
	token, err := GetRecaptchaToken(ctx, ClerkSiteURL, ClerkSiteKey)
	if err != nil {
		return nil, 0, fmt.Errorf("acquiring recaptcha token: %w", err)
	}
	headers := map[string]string{
		"x-recaptcha-token": token,
		// The portal also expects an Origin/Referer that matches the site
		// key's allowed origins. Surf's Chrome impersonation sets a
		// generic Referer; override to the clerk root so token validation
		// on the server side accepts our origin.
		"Referer": ClerkSiteURL,
		"Origin":  "https://onlineservices.miamidadeclerk.gov",
	}
	return c.do("POST", path, params, nil, headers)
}
