...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Miami Dade Clerk",
...
Use --chrome to read cookies from Chrome for onlineservices.miamidadeclerk.gov.
...
profile by name when the installed backend supports it.`,
		Example: `  miami-dade-clerk-pp-cli auth login --chrome
  miami-dade-clerk-pp-cli auth login --browser
  miami-dade-clerk-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !browserFlag {
				fmt.Fprintln(cmd.OutOrStdout(), "Use --chrome or --browser to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  miami-dade-clerk-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  miami-dade-clerk-pp-cli auth login --browser\n")
				return nil
			}
			...
			// Verify-friendly short-circuit: when the printing-press verify/dogfood
			// harness invokes this command with PRINTING_PRESS_VERIFY=1 (or --dry-run
			// for validate-narrative full-examples), skip the real Chrome cookie
			// extraction. Otherwise the harness fails on machines that haven't
			// logged in to the portal in Chrome.
			if cliutil.IsVerifyEnv() || flags.dryRun {
				fmt.Fprintf(w, "would: extract Chrome cookies for %s and save session\n", domain)
				return nil
			}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  miami-dade-clerk-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  miami-dade-clerk-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: onlineservices.miamidadeclerk.gov\n")
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  miami-dade-clerk-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
