...
Resource scoping:
  ...
  miami-dade-clerk-pp-cli sync

  # Sync specific resources only
  miami-dade-clerk-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  miami-dade-clerk-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  miami-dade-clerk-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  miami-dade-clerk-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  miami-dade-clerk-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/miami-dade-clerk-pp-cli/data.db)")
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"doc-types": "/api/home/documentTypes",
	}
