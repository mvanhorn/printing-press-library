...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".miami-dade-clerk-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MIAMI_DADE_CLERK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MIAMI_DADE_CLERK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "miami-dade-clerk-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.miami-dade-clerk-pp-cli/feedback.jsonl.
When ` + "`MIAMI_DADE_CLERK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MIAMI_DADE_CLERK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  miami-dade-clerk-pp-cli feedback list
  miami-dade-clerk-pp-cli feedback list --limit 5
  miami-dade-clerk-pp-cli feedback list --json`,
