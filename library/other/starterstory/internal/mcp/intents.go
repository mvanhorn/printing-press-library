...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("read_one_case_study",
			mcplib.WithDescription("Fetch and extract a single case-study page."),
			mcplib.WithString("slug", mcplib.Required(), mcplib.Description("Override the recipe's positional slug value.")),
		),
		handleReadOneCaseStudy,
	)
	s.AddTool(
		mcplib.NewTool("browse_a_curated_idea_list",
			mcplib.WithDescription("Extract the linked items from a /data category page."),
			mcplib.WithString("slug", mcplib.Required(), mcplib.Description("Override the recipe's positional slug value.")),
		),
		handleBrowseACuratedIdeaList,
	)
}

// handleReadOneCaseStudy runs the read_one_case_study recipe intent: Fetch and extract a single case-study page.
func handleReadOneCaseStudy(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
...
// handleBrowseACuratedIdeaList runs the browse_a_curated_idea_list recipe intent: Extract the linked items from a /data category page.
func handleBrowseACuratedIdeaList(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	if recipeCLIPathErr != nil {
		return mcplib.NewToolResultError(fmt.Sprintf("companion CLI binary not found: %v", recipeCLIPathErr)), nil
	}
