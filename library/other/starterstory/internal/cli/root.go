...
type rootFlags struct {
	...
	// invocation. Mirrors the STARTERSTORY_NO_LEARN env var.
	noLearn       bool
	selectFields  string
	configPath    string
	homePath      string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "starterstory-pp-cli",
		Short: `Every Starter Story case study, idea, and business indexed locally — rank founder stories by revenue, search 13k+ entries offline, and diff what's new, none of which the site itself can do.`,
		Long: `Every Starter Story case study, idea, and business indexed locally — rank founder stories by revenue, search 13k+ entries offline, and diff what's new, none of which the site itself can do.
...
Health check: run 'starterstory-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("starterstory-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'starterstory-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelTopRevenueCmd(flags))
	rootCmd.AddCommand(newNovelWhatsNewCmd(flags))
	rootCmd.AddCommand(newIndexCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newBreakdownsPromotedCmd(flags))
	rootCmd.AddCommand(newBusinessesPromotedCmd(flags))
	...
	rootCmd.AddCommand(newToolsPromotedCmd(flags))
