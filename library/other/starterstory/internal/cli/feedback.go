...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("STARTERSTORY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("STARTERSTORY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "starterstory-pp-cli/feedback")
...
When ` + "`STARTERSTORY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`STARTERSTORY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  starterstory-pp-cli feedback list
  starterstory-pp-cli feedback list --limit 5
  starterstory-pp-cli feedback list --json`,
