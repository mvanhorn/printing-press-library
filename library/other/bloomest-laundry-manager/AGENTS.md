# Bloomest Laundry Manager Printed CLI Agent Guide

This directory is a generated `bloomest-laundry-manager-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
bloomest-laundry-manager-pp-cli doctor --json
bloomest-laundry-manager-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
bloomest-laundry-manager-pp-cli which "<capability>" --json
bloomest-laundry-manager-pp-cli <command> --help
```
...
```bash
bloomest-laundry-manager-pp-cli <command> --agent
```
...
```bash
bloomest-laundry-manager-pp-cli <command> --help
bloomest-laundry-manager-pp-cli <command> --dry-run --agent
```
