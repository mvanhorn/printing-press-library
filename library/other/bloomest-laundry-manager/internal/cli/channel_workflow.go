...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  bloomest-laundry-manager-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  bloomest-laundry-manager-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"accesses", "alarms", "bloomest-laundry-manager-version", "cashbox-updates", "comments", "contacts", "laundries", "loyalties", "message", "notifications", "policy", "reservationsconf", "users"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  bloomest-laundry-manager-pp-cli workflow status

  # Show status as JSON
  bloomest-laundry-manager-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("bloomest-laundry-manager-pp-cli")
			}
