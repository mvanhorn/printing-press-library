...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".bloomest-laundry-manager-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BLOOMEST_LAUNDRY_MANAGER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BLOOMEST_LAUNDRY_MANAGER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "bloomest-laundry-manager-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.bloomest-laundry-manager-pp-cli/feedback.jsonl.
When ` + "`BLOOMEST_LAUNDRY_MANAGER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BLOOMEST_LAUNDRY_MANAGER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "bloomest-laundry-manager-pp-cli",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  bloomest-laundry-manager-pp-cli feedback list
  bloomest-laundry-manager-pp-cli feedback list --limit 5
  bloomest-laundry-manager-pp-cli feedback list --json`,
