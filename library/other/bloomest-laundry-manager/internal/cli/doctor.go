...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  bloomest-laundry-manager-pp-cli doctor
  bloomest-laundry-manager-pp-cli doctor --json
  bloomest-laundry-manager-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export BLOOMEST_LAUNDRY_MANAGER_COOKIE_AUTH=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("BLOOMEST_LAUNDRY_MANAGER_COOKIE_AUTH") != "" || authHeader != "" {
				authEnvSet = append(authEnvSet, "BLOOMEST_LAUNDRY_MANAGER_COOKIE_AUTH")
			} else {
				authEnvInfo = append(authEnvInfo, "BLOOMEST_LAUNDRY_MANAGER_COOKIE_AUTH set with auth set-token")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("bloomest-laundry-manager-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'bloomest-laundry-manager-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'bloomest-laundry-manager-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'bloomest-laundry-manager-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'bloomest-laundry-manager-pp-cli sync' to refresh."
	}
	return report
}
