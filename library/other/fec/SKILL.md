...
- `fec-pp-cli schedules list-schedulef` — Schedule F
- `fec-pp-cli schedules list-scheduleh4` — Schedule H4 filings describe disbursements for allocated federal/nonfederal activity.
- `fec-pp-cli schedules list-scheduleh4-2` — Efiling endpoints provide real-time campaign finance data received from electronic filers.

**state-election-office** — Manage state election office

- `fec-pp-cli state-election-office` — State laws and procedures govern elections for state or local offices as well as how candidates appear on election

**totals** — Manage totals
...
## Auth Setup
Run `fec-pp-cli auth setup` to print the URL and steps for getting a key (add `--launch` to open the URL). Then set:

```bash
export FEC_API_KEY_QUERY_AUTH="<your-key>"
```

To persist credentials, use `fec-pp-cli auth set-token <token>`. Stored secrets live in `credentials.toml` under the data dir, not in `config.toml`.

Run `fec-pp-cli doctor` to verify setup.
...
- **Filterable** — `--select` keeps a subset of fields. Dotted paths descend into nested structures; arrays traverse element-wise. Critical for keeping context small on verbose APIs:
  ...
  ```bash
  fec-pp-cli audit-case --agent --select id,name,status
  ```
...
- Use `--home <dir>` for one invocation, or set `FEC_HOME=<dir>` to relocate all four path kinds under one root.
- Use per-kind env vars only when a specific kind must diverge: `FEC_CONFIG_DIR`, `FEC_DATA_DIR`, `FEC_STATE_DIR`, `FEC_CACHE_DIR`.
- Resolution order is per-kind env var, `--home`, `FEC_HOME`, XDG (`XDG_CONFIG_HOME`, `XDG_DATA_HOME`, `XDG_STATE_HOME`, `XDG_CACHE_HOME`), then platform defaults.
- `config` contains settings like `config.toml` and profiles. `data` contains `credentials.toml`, `data.db`, cookies, and auth sidecars. `state` contains persisted queries, jobs, and `teach.log`. `cache` contains regenerable HTTP/cache files.
- Stored secrets live in `credentials.toml` under the data dir. Existing legacy `config.toml` secrets are read for compatibility and leave `config.toml` on the first auth write.
- Run `fec-pp-cli doctor --fail-on warn` to surface path and credential-location warnings. `agent-context` exposes a schema v4 `paths` block for agents that need the resolved dirs.
...
Fleet precedence: an inherited per-kind env var overrides an explicit `--home` for that kind. Use `FEC_HOME` or per-kind vars as durable fleet levers, and use `--home` only for a single invocation. Relocation is not reversible by unsetting env vars; move files manually before clearing `FEC_HOME`, or `doctor` will not find credentials left under the former root.
...
```json
{
  ...
  "candidates": [
    { "id": 12, "class": "flag_alias | playbook_candidate",
      ...
      "rationale": "...",
      "next_action": ["<trial command>", "fec-pp-cli learnings confirm 12"] }
  ],
  "playbook": {
    "query_family": "...",
    "playbook": {
...
if Playbook present:
    ...
    -> the Playbook's expected_tool_calls is a budget; if you find yourself running
       materially more, record the divergence via `fec-pp-cli playbook amend`
       at end-of-session.
...
- `lookup_refresh_available` (top-level): an entity in the query has no lookup row yet, but synced data could provide one. Run `fec-pp-cli sync` to refresh entity lookups.
...
`fec-pp-cli learnings stats` reports recall hit rate, teach-to-reuse, playbook resolution rate, and candidate confirm/reject counts from the local `learn_events` table. Rates are null until they have a denominator; everything stays on this machine. Use it to check whether the loop is earning its keep for this CLI.

### Disabling learning

- `--no-learn` on a single command short-circuits both `recall` and the `teach` write path. Use for deterministic agent flows or tests that must not be affected by accumulated learnings.
- `FEC_NO_LEARN=true` in the environment globally disables the pipeline.
...
```
fec-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
fec-pp-cli feedback --stdin < notes.txt
fec-pp-cli feedback list --json --limit 10
```

Entries are stored locally as `feedback.jsonl` under the resolved data dir. They are never POSTed unless `FEC_FEEDBACK_ENDPOINT` is set AND either `--send` is passed or `FEC_FEEDBACK_AUTO_SEND=true`. Default behavior is local-only.
...
```
fec-pp-cli profile save briefing --json
fec-pp-cli --profile briefing audit-case
fec-pp-cli profile list --json
fec-pp-cli profile show briefing
fec-pp-cli profile delete briefing --yes
```
...
1. **Empty, `help`, or `--help`** → show `fec-pp-cli --help` output
...
2. Register with Claude Code:
   ```bash
   claude mcp add fec-pp-mcp -- fec-pp-mcp
   ```
3. Verify: `claude mcp list`

## Direct Use

1. Check if installed: `which fec-pp-cli`
   If not found, offer to install (see Prerequisites at the top of this skill).
2. Match the user query to the best command from the Unique Capabilities and Command Reference above.
3. Execute with the `--agent` flag:
   ```bash
   fec-pp-cli <command> [subcommand] [args] --agent
   ```
4. If ambiguous, drill into subcommand help: `fec-pp-cli <command> --help`.
