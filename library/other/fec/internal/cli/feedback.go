...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FEC_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FEC_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "fec-pp-cli/feedback")
...
When ` + "`FEC_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FEC_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  fec-pp-cli feedback list
  fec-pp-cli feedback list --limit 5
  fec-pp-cli feedback list --json`,
