...
// fec-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
const noLearnEnvVar = "FEC_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("fec-pp-cli")
}
...
when learnings exist.`,
		Example: `  fec-pp-cli recall "<question>" --agent
  fec-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  fec-pp-cli learnings list --agent
  fec-pp-cli learnings list --query "<substring>"
  fec-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  fec-pp-cli learnings forget "<question>" --resource <id>
  fec-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  fec-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  fec-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
