...
type Config struct {
	...
	FecApiKeyQueryAuth string          `toml:"api_key_query_auth"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("FEC_API_KEY_QUERY_AUTH"); v != "" {
		cfg.FecApiKeyQueryAuth = v
		cfg.markEnvOverride("FecApiKeyQueryAuth")
		cfg.AuthSource = "env:FEC_API_KEY_QUERY_AUTH"
		cfg.CredentialSource = "env:FEC_API_KEY_QUERY_AUTH"
	}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("FEC_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "fec-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.FecApiKeyQueryAuth
	if token == "" {
		return ""
	}
	if c.FecApiKeyQueryAuth == "" {
		return ""
	}
	return token
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		FecApiKeyQueryAuth: c.FecApiKeyQueryAuth,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.FecApiKeyQueryAuth = creds.FecApiKeyQueryAuth
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.FecApiKeyQueryAuth = token
	delete(c.envOverrides, "FecApiKeyQueryAuth")
	c.updateFileConfigField("FecApiKeyQueryAuth")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["FecApiKeyQueryAuth"] {
			out.FecApiKeyQueryAuth = c.fileConfig.FecApiKeyQueryAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "FecApiKeyQueryAuth":
		c.fileConfig.FecApiKeyQueryAuth = c.FecApiKeyQueryAuth
	}
}
