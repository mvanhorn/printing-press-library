...
func withJournalHome(t *testing.T) string {
	...
	dir := t.TempDir()
	t.Setenv("HOME", dir)
	t.Setenv("FEC_STATE_DIR", "")
	t.Setenv("FEC_HOME", "")
	t.Setenv("XDG_STATE_HOME", "")
	t.Setenv("PRINTING_PRESS_VERIFY", "")
	t.Setenv("PRINTING_PRESS_DOGFOOD", "")
	t.Setenv("FEC_NO_LEARN", "")
	t.Setenv("FEC_LEARN_NO_CAPTURE", "")
	t.Setenv("FEC_LEARN_SESSION", "")
	return dir
}
...
func runCLI(t *testing.T, args ...string) error {
	t.Helper()
	oldArgs := os.Args
	os.Args = append([]string{"fec-pp-cli"}, args...)
...
func TestJournal_SessionKeyFromEnv(t *testing.T) {
	withJournalHome(t)
	t.Setenv("FEC_LEARN_SESSION", "session-abc123")
	if err := runCLI(t, "version"); err != nil {
		t.Fatalf("version: %v", err)
	}
	entries := readAllJournalEntries(t)
...
func TestJournal_DisableSwitchesWriteNothing(t *testing.T) {
	cases := []struct {
		...
		{name: "master no-learn env", env: map[string]string{"FEC_NO_LEARN": "1"}},
		{name: "capture-only env", env: map[string]string{"FEC_LEARN_NO_CAPTURE": "1"}},
