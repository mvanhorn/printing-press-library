...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("RETRACTION_CHECKER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("RETRACTION_CHECKER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "retraction-checker-pp-cli/feedback")
...
When ` + "`RETRACTION_CHECKER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`RETRACTION_CHECKER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  retraction-checker-pp-cli feedback list
  retraction-checker-pp-cli feedback list --limit 5
  retraction-checker-pp-cli feedback list --json`,
