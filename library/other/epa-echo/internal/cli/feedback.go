...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("EPA_ECHO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("EPA_ECHO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "epa-echo-pp-cli/feedback")
...
When ` + "`EPA_ECHO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`EPA_ECHO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  epa-echo-pp-cli feedback list
  epa-echo-pp-cli feedback list --limit 5
  epa-echo-pp-cli feedback list --json`,
