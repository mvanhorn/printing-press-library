...
Resource scoping:
  ...
  epa-echo-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  epa-echo-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  epa-echo-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  epa-echo-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  epa-echo-pp-cli sync --latest-only`,
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		"dfr-rest-services-get-dfr":         "/dfr_rest_services.get_dfr",
		"echo-rest-services-get-facilities": "/echo_rest_services.get_facilities",
		"echo-rest-services-get-qid":        "/echo_rest_services.get_qid",
	}
