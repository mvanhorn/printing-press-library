...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  epa-echo-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  epa-echo-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"echo-rest-services-get-facilities"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  epa-echo-pp-cli workflow status

  # Show status as JSON
  epa-echo-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("epa-echo-pp-cli")
			}
