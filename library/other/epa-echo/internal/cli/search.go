...
In local mode: searches locally synced data only.`,
  ...
  epa-echo-pp-cli search "error timeout"

  # Force local search only
  epa-echo-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  epa-echo-pp-cli search "status" --type dfr-rest-services-get-dfr --data-source local
  # JSON output for piping
  epa-echo-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /echo_rest_services.get_qid
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/echo_rest_services.get_qid", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'epa-echo-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
