...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("order_enforcement_evidence",
			mcplib.WithDescription("Keep reported actions and penalties attached to their source sections."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Override the recipe's positional id value.")),
		),
		handleOrderEnforcementEvidence,
	)
}

// handleOrderEnforcementEvidence runs the order_enforcement_evidence recipe intent: Keep reported actions and penalties attached to their source sections.
func handleOrderEnforcementEvidence(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
