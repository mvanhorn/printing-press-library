...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("EIA_ENERGY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("EIA_ENERGY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "eia-energy-pp-cli/feedback")
...
When ` + "`EIA_ENERGY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`EIA_ENERGY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  eia-energy-pp-cli feedback list
  eia-energy-pp-cli feedback list --limit 5
  eia-energy-pp-cli feedback list --json`,
