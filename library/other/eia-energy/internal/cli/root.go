...
type rootFlags struct {
	...
	// invocation. Mirrors the EIA_ENERGY_NO_LEARN env var.
	noLearn       bool
	selectFields  string
	configPath    string
	homePath      string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "eia-energy-pp-cli",
		Short: "Make EIA's large self-describing hierarchy useful from the terminal without hidden unit conversions, frequency mismatches, silent truncation, forecasts, or causal claims.",
		Long: `Eia Energy CLI
...
Health check: run 'eia-energy-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("eia-energy-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'eia-energy-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelAnomalyCmd(flags))
	rootCmd.AddCommand(newNovelGridCmd(flags))
	rootCmd.AddCommand(newNovelRevisionsCmd(flags))
	rootCmd.AddCommand(newNovelSpreadCmd(flags))
	rootCmd.AddCommand(newNovelStateCmd(flags))
	rootCmd.AddCommand(newNovelWatchCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newElectricityPromotedCmd(flags))
