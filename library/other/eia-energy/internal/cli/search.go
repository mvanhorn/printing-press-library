...
Run sync first to populate the local search index.`,
  ...
  eia-energy-pp-cli search "status"

  # Force local search explicitly
  eia-energy-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  eia-energy-pp-cli search "status" --type electricity --data-source local
  # JSON output for piping
  eia-energy-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'eia-energy-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
