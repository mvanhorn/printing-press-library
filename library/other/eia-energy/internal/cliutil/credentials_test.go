...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"EIA_ENERGY_CONFIG",
		"EIA_ENERGY_CONFIG_DIR",
		"EIA_ENERGY_DATA_DIR",
		"EIA_ENERGY_STATE_DIR",
		"EIA_ENERGY_CACHE_DIR",
		"EIA_ENERGY_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
	...
	return home, filepath.Join(home, ".config", "eia-energy-pp-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "eia-energy-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("EIA_API_KEY", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "eia-energy-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "eia-energy-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "eia-energy-pp-cli", "credentials.toml")
