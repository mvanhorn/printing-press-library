...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".ncbi-entrez-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NCBI_ENTREZ_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NCBI_ENTREZ_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ncbi-entrez-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.ncbi-entrez-pp-cli/feedback.jsonl.
When ` + "`NCBI_ENTREZ_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NCBI_ENTREZ_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ncbi-entrez-pp-cli feedback list
  ncbi-entrez-pp-cli feedback list --limit 5
  ncbi-entrez-pp-cli feedback list --json`,
