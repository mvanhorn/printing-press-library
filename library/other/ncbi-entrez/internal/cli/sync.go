...
Exit codes & warnings:
  ...
  ncbi-entrez-pp-cli sync

  # Sync specific resources only
  ncbi-entrez-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  ncbi-entrez-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  ncbi-entrez-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  ncbi-entrez-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  ncbi-entrez-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/ncbi-entrez-pp-cli/data.db)")
