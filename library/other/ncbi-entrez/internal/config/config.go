...
type Config struct {
	...
	Path          string            `json:"-"`
	NcbiApiKey    string            `json:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("NCBI_ENTREZ_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "ncbi-entrez-pp-cli", "config.json")
	}
	cfg.Path = path
	...
	if v := os.Getenv("NCBI_API_KEY"); v != "" {
		cfg.NcbiApiKey = v
		cfg.AuthSource = "env:NCBI_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.NcbiApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("NCBI_ENTREZ_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
