# NCBI Entrez Printed CLI Agent Guide

This directory is a generated `ncbi-entrez-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
ncbi-entrez-pp-cli doctor --json
ncbi-entrez-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
ncbi-entrez-pp-cli which "<capability>" --json
ncbi-entrez-pp-cli <command> --help
```
...
```bash
ncbi-entrez-pp-cli <command> --agent
```
...
```bash
ncbi-entrez-pp-cli <command> --help
ncbi-entrez-pp-cli <command> --dry-run --agent
```
