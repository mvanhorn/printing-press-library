...
Run sync first to populate the local search index.`,
  ...
  indmoney-pp-cli search "status"

  # Force local search explicitly
  indmoney-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  indmoney-pp-cli search "status" --type indices --data-source local
  # JSON output for piping
  indmoney-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'indmoney-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
