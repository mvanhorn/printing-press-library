...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("INDMONEY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("INDMONEY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "indmoney-pp-cli/feedback")
...
When ` + "`INDMONEY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`INDMONEY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  indmoney-pp-cli feedback list
  indmoney-pp-cli feedback list --limit 5
  indmoney-pp-cli feedback list --json`,
