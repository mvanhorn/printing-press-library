...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Gisis",
	...
	attachAuthPing(cmd, flags) // hand-edit: wire the auth_ping novel subcommand (see auth_ping_impl.go)
...
Use --chrome to read cookies from Chrome for gisis.imo.org.
Use --browser as an alias for --chrome.
Requires a cookie extraction tool (pycookiecheat, cookies, cookie-scoop-cli, or kooky).
...
kooky reads cookies across every installed browser (Chrome, Brave, Edge,
Firefox, Safari), which is the most reliable backend for Cloudflare-Turnstile
gated sites where the GISIS session lives in a non-Chrome browser. Set
GISIS_KOOKY_BROWSER (e.g. "brave") to scope kooky to one browser.`,
		Example: `  gisis-pp-cli auth login --chrome
  gisis-pp-cli auth login --browser
  gisis-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !browserFlag {
				fmt.Fprintln(cmd.OutOrStdout(), "Use --chrome or --browser to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  gisis-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  gisis-pp-cli auth login --browser\n")
				return nil
			}

			w := cmd.OutOrStdout()
			// IMO Web Accounts auth spans imo.org: the SSO token (IMOWEBACC) is
			// set on .imo.org while the GISIS app session (ASP.NET_SessionId) is
			// on gisis.imo.org. Extract at the registrable parent so the cookie
			// tool captures both (kooky's -d is a partial-domain match).
			domain := "imo.org"
			...
			if !fromPressAuth {
				...
				if err != nil {
					...
					if runtime.GOOS == "windows" {
						...
						fmt.Fprintln(w, "  go install github.com/browserutils/kooky/cmd/kooky@latest  # Go, all browsers")
					} else {
			...
			// GISIS auth spans two hosts: the gisis.imo.org app session
			// (ASP.NET_SessionId) and the .imo.org IMO Web Accounts SSO token
			// (IMOWEBACC — the cookie that actually authenticates), plus Azure
			// affinity cookies on .gisis.imo.org. Keep just the GISIS-relevant
			// set; unfiltered, the blob carries every imo.org cookie into the
			// Cookie header, which trips upstream WAFs and shadows the credential.
			// IMOWEBACC is required; the session and affinity cookies are kept
			// when present (a fresh login may not have an app session cookie yet).
			cookieMap := parseCookieString(cookies)
			if _, ok := cookieMap["IMOWEBACC"]; !ok {
				const loginURL = "https://gisis.imo.org/Public/Default.aspx"
				fmt.Fprintf(w, "\n%s IMO Web Accounts cookie (IMOWEBACC) not found for %s.\n", red("ERROR"), domain)
				fmt.Fprintln(w, "")
				fmt.Fprintln(w, "Log in to GISIS in your browser:")
				fmt.Fprintf(w, "\n  %s\n\n", loginURL)
				fmt.Fprintln(w, "Then run this command again:")
				fmt.Fprintf(w, "\n  gisis-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("IMOWEBACC cookie not found for %s — not logged in to IMO Web Accounts", domain))
			}
			keptNames := []string{"IMOWEBACC", "ASP.NET_SessionId", "ARRAffinity", "ARRAffinitySameSite"}
			jarCookies := make(map[string]string, len(keptNames))
			kept := make([]string, 0, len(keptNames))
			...
			// Persist the cookies so client.LoadCookieJar() carries them on
			// subsequent invocations; the SaveTokens blob above feeds the
			// Authorization header path but is invisible to the HTTP client's
			// cookie jar.
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  gisis-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  gisis-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: gisis.imo.org\n")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  gisis-pp-cli auth set-token eyJhbGciOi...",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  gisis-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
...
func detectCookieTool() (cookieTool, error) {
	...
	// Edge, Firefox, Safari) and works on all platforms including Windows. It
	// is the most reliable backend for Cloudflare-Turnstile gated sites like
	// GISIS where the logged-in session often lives in a non-Chrome browser.
	// Detected last so the Chrome-tuned tools above keep priority when present.
	if _, err := exec.LookPath("kooky"); err == nil {
		return cookieTool{name: "kooky"}, nil
	}
	if runtime.GOOS == "windows" {
		return cookieTool{}, fmt.Errorf("no cookie extraction tool found; pycookiecheat does not support Windows. Use `auth login --browser` (live Chrome via CDP), or install cookie-scoop-cli or kooky (go install github.com/browserutils/kooky/cmd/kooky@latest)")
	}
	return cookieTool{}, fmt.Errorf("no cookie extraction tool found; install one: pip install pycookiecheat (or go install github.com/browserutils/kooky/cmd/kooky@latest)")
}
...
// extractViaKooky shells out to `kooky -d <domain> -j` and folds the JSONL
// output into a "name=value; ..." Cookie header. kooky reads cookie stores for
// every installed browser, so unlike the Chrome-only backends it finds the
// GISIS session wherever the user signed in (Brave, Edge, Firefox, Safari).
//
// Browser scope: kooky's -d filter is a partial-domain match across all
// browsers by default. Set GISIS_KOOKY_BROWSER (e.g. "brave") to pin one
// browser when several hold cookies for the same domain and the wrong one wins.
//
// Each JSONL line is one cookie; we only need name/value here because the login
// flow downstream re-filters to the required GISIS cookies and re-derives the
// jar rows. Lines that fail to parse are skipped rather than aborting the whole
// extraction so one malformed row can't sink an otherwise-valid session.
func extractViaKooky(domain string) (string, error) {
	cleanDomain := strings.TrimPrefix(domain, ".")
	args := []string{"-d", cleanDomain, "-j"}
	if browser := strings.TrimSpace(os.Getenv("GISIS_KOOKY_BROWSER")); browser != "" {
		args = append(args, "-b", browser)
	}

	var out bytes.Buffer
	cmd := exec.Command("kooky", args...)
	cmd.Stdout = &out
	cmd.Stderr = os.Stderr
	if err := cmd.Run(); err != nil {
		return "", fmt.Errorf("kooky failed: %w", err)
	}
	return parseKookyJSONL(&out)
}

// parseKookyJSONL folds kooky's `-j` JSON Lines output (one cookie object per
// line) into a "name=value; ..." Cookie header. Only name/value are read; the
// login flow downstream re-filters to the required GISIS cookies. Lines that
// fail to parse or carry an empty name are skipped so one malformed row can't
// sink an otherwise-valid session.
func parseKookyJSONL(r io.Reader) (string, error) {
	var parts []string
	scanner := bufio.NewScanner(r)
	// Cookie values can be long (JWTs, WAF blobs); raise the line cap from the
	// 64 KiB default so a large row isn't silently truncated mid-value.
	...
	if err := scanner.Err(); err != nil {
		return "", fmt.Errorf("reading kooky output: %w", err)
	}
	return strings.Join(parts, "; "), nil
}
