# Polymarket Printed CLI Agent Guide

This directory is a generated `polymarket-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
polymarket-pp-cli doctor --json
polymarket-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
polymarket-pp-cli which "<capability>" --json
polymarket-pp-cli <command> --help
```
...
```bash
polymarket-pp-cli <command> --agent
```
...
```bash
polymarket-pp-cli <command> --help
polymarket-pp-cli <command> --dry-run --agent
```
