...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Polymarket",
	...
	cmd.AddCommand(newAuthDeriveCmd(flags))
...
profile by name when the installed backend supports it.`,
		Example: `  polymarket-pp-cli auth login --chrome
  polymarket-pp-cli auth login --browser
  polymarket-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !browserFlag {
				fmt.Fprintln(cmd.OutOrStdout(), "Use --chrome or --browser to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  polymarket-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  polymarket-pp-cli auth login --browser\n")
				return nil
			}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  polymarket-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  polymarket-pp-cli auth login --chrome\n")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  polymarket-pp-cli auth set-token eyJhbGciOi...",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  polymarket-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
...
func validateComposedAuth(authHeader string) error {
	testURL := "https://gamma-api.polymarket.com"
	req, err := http.NewRequest("HEAD", testURL, nil)
	if err != nil {
		return nil // can't validate, assume OK
	}
	req.Header.Set("Authorization", authHeader)
	req.Header.Set("User-Agent", "polymarket-pp-cli/0.1.0 (+https://github.com/mvanhorn/printing-press-library)")
