...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "polymarket-pp-cli",
		Short: `Polymarket CLI — Every Polymarket feature the official Rust CLI ships, plus six commands no Polymarket tool offers: resolution radar, re…`,
		Long: `Polymarket CLI — Every Polymarket feature the official Rust CLI ships, plus six commands no Polymarket tool offers: resolution radar, re…
...
Highlights (not in the official API docs):
  ...
  • redeem all   Discover every position in resolved markets and report the redeemable set with totals. In this build --broadcast is a documented stub (real on-chain ctf redeem needs go-ethereum + Polygon RPC, wired …

Agent mode: add --agent to any command for JSON output + non-interactive mode.
Health check: run 'polymarket-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("polymarket-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'polymarket-pp-cli profile list')")
	...
	rootCmd.AddCommand(newClobCmd(flags))
	...
	// Hand-written novel features and on-chain primitives (Phase 3).
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "version",
		Short: "Print the polymarket-pp-cli version string",
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("polymarket-pp-cli %s\n", version)
		},
	}
}
