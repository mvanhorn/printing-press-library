...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  polymarket-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  polymarket-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"api_keys", "comments", "events", "markets", "orders", "rewards", "series", "tags", "trades"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  polymarket-pp-cli workflow status

  # Show status as JSON
  polymarket-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("polymarket-pp-cli")
			}
