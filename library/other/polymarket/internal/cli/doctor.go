...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  polymarket-pp-cli doctor
  polymarket-pp-cli doctor --json
  polymarket-pp-cli doctor --verbose   # 4-mode probe: network/auth/rate-limit/clock-skew
  polymarket-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "polymarket-pp-cli auth login --chrome"
				} else {
					authConfigured = true
			...
			if os.Getenv("POLYMARKET_PRIVATE_KEY") != "" {
				authEnvSet = append(authEnvSet, "POLYMARKET_PRIVATE_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "POLYMARKET_PRIVATE_KEY")
			}
			if os.Getenv("POLYMARKET_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "POLYMARKET_API_KEY")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "POLYMARKET_API_KEY")
			}
			if os.Getenv("POLYMARKET_API_SECRET") != "" {
				authEnvSet = append(authEnvSet, "POLYMARKET_API_SECRET")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "POLYMARKET_API_SECRET")
			}
			if os.Getenv("POLYMARKET_API_PASSPHRASE") != "" {
				authEnvSet = append(authEnvSet, "POLYMARKET_API_PASSPHRASE")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "POLYMARKET_API_PASSPHRASE")
			}
			if os.Getenv("POLYMARKET_FUNDER") != "" {
				authEnvSet = append(authEnvSet, "POLYMARKET_FUNDER")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "POLYMARKET_FUNDER")
			}
			if os.Getenv("POLYMARKET_SIGNATURE_TYPE") != "" {
				authEnvSet = append(authEnvSet, "POLYMARKET_SIGNATURE_TYPE")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "POLYMARKET_SIGNATURE_TYPE")
			}
			if os.Getenv("POLYMARKET_CHAIN_ID") != "" {
				authEnvSet = append(authEnvSet, "POLYMARKET_CHAIN_ID")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "POLYMARKET_CHAIN_ID")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "polymarket-pp-cli/0.1.0 (+https://github.com/mvanhorn/printing-press-library)"
						authHeaders["Accept"] = "application/json"
						verifyPath := "https://clob.polymarket.com/rewards/user"
			...
			if verbose {
				report["verbose_probe"] = doctorVerboseProbe(cmd.Context(), cfg)
			}
	...
	cmd.Flags().BoolVar(&verbose, "verbose", false, "4-mode probe: network, auth, rate-limit, clock-skew")
	return cmd
}

// doctorVerboseProbe runs an enriched 4-mode classifier and returns a structured
// report. The 4 modes mirror common production failure classes so an operator
// can tell which knob to turn without re-running multiple commands:
//
//   - network:    can we reach the API host at all? (TCP+TLS+HTTP path)
//   - auth:       does the configured credential pass an authenticated probe?
//   - rate_limit: did the server respond with a 429 or signal throttle headers?
//   - clock_skew: how far does our local clock differ from the server's Date hdr?
//
// Safe to call without credentials — modes degrade to "skipped" rather than
// failing the parent doctor command.
func doctorVerboseProbe(ctx context.Context, cfg *config.Config) map[string]any {
	out := map[string]any{}
	if ctx == nil {
		ctx = context.Background()
	}
	if cfg == nil {
		out["network"] = "skipped: no config loaded"
		out["auth"] = "skipped: no config loaded"
		out["rate_limit"] = "skipped: no config loaded"
		out["clock_skew"] = "skipped: no config loaded"
		return out
	}
	cl := client.New(cfg, 10*time.Second, 0)
	// Network probe via the configured base URL root path.
	probeCtx, cancel := context.WithTimeout(ctx, 10*time.Second)
	defer cancel()
	status, perr := cl.ProbeGet(probeCtx, "/")
	switch {
	case perr != nil:
		out["network"] = fmt.Sprintf("FAIL: %s", perr.Error())
	case status >= 500:
		out["network"] = fmt.Sprintf("WARN: upstream %d", status)
	default:
		out["network"] = fmt.Sprintf("ok (%d)", status)
	}
	// Auth probe — only meaningful when a credential is present.
	if cfg.AuthHeader() == "" {
		out["auth"] = "skipped: no credential configured"
	} else {
		authStatus, aerr := cl.ProbeGet(probeCtx, "/")
		switch {
		case aerr != nil:
			out["auth"] = fmt.Sprintf("FAIL: %s", aerr.Error())
		case authStatus == 401 || authStatus == 403:
			out["auth"] = fmt.Sprintf("FAIL: %d (credential rejected)", authStatus)
		default:
			out["auth"] = fmt.Sprintf("ok (%d)", authStatus)
		}
	}
	// Rate-limit and clock-skew are best-effort. We classify by re-probing and
	// inspecting the recent HTTP response; absent richer client telemetry we
	// surface "not detected" rather than fabricate numbers.
	if status == 429 {
		out["rate_limit"] = "WARN: HTTP 429 observed during probe"
	} else {
		out["rate_limit"] = "not detected"
	}
	out["clock_skew"] = "skipped: server Date header not exposed by client probe (v0.2)"
	// Quiet io import unused warnings (kept for future expansion).
	_ = io.Discard
	_ = sql.ErrNoRows
	_ = errors.New
	return out
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'polymarket-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'polymarket-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'polymarket-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'polymarket-pp-cli sync' to refresh."
	}
	return report
}
