...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".polymarket-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("POLYMARKET_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("POLYMARKET_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "polymarket-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.polymarket-pp-cli/feedback.jsonl.
When ` + "`POLYMARKET_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`POLYMARKET_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  polymarket-pp-cli feedback list
  polymarket-pp-cli feedback list --limit 5
  polymarket-pp-cli feedback list --json`,
