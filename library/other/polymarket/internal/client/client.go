...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "polymarket-pp-cli", "http")
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	// Polymarket has 3 distinct hosts (gamma-api / clob / data-api). Spec
	// endpoints use absolute URLs for the non-default hosts. When path is
	// already an absolute URL, do NOT prepend BaseURL — that produced
	// garbage like "https://gamma-api.polymarket.comhttps://clob...".
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export POLYMARKET_PRIVATE_KEY=<your-token> or polymarket-pp-cli auth set-token <token>")
}
