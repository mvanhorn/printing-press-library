# Global Fishing Watch Printed CLI Agent Guide

This directory is a generated `gfw-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
gfw-pp-cli doctor --json
gfw-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
gfw-pp-cli which "<capability>" --json
gfw-pp-cli <command> --help
```
...
```bash
gfw-pp-cli <command> --help
gfw-pp-cli <command> --dry-run --agent
```
