...
// TestMigrate_AddsColumnsOnUpgrade_DownloadFileUrl verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_DownloadFileUrl(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "download_file_url" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	rows, err := s.DB().Query(`PRAGMA table_info("download_file_url")`)
	if err != nil {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from download_file_url after migrate", want)
		}
	}
}
