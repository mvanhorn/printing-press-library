...
// TestUpsertBatch_PopulatesDownloadFileUrlTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed download_file_url table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesDownloadFileUrlTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "bulk_reports_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "bulk_reports_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "bulk_reports_id": "test-parent-001"}`),
	}
	if _, _, err := s.UpsertBatch("download_file_url", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "download_file_url").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "download_file_url")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count download_file_url: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("download_file_url count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandDownloadFileUrlGeneric exercises
...
func TestUpsertBatch_TypedFailureDoesNotStrandDownloadFileUrlGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// Items deliberately omit "bulk_reports_id" so the typed NOT NULL
	// constraint fires.
	items := []json.RawMessage{
	...
	stored, extractFailures, err := s.UpsertBatch("download_file_url", items)
	if err != nil {
	...
	if typed != 0 {
		t.Fatalf("download_file_url count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "bulk_reports_id")
	}
}
...
func TestUpsertBatch_TypedFailureDoesNotStrandQueryGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("query count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "bulk_reports_id")
	}
}

// TestUpsertBatch_SetsQueryParentID verifies that dependent-resource
...
func TestUpsertBatch_SetsQueryParentID(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	var matchedA int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE parent_id = ?`, "query")
