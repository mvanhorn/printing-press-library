...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  gfw-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  gfw-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"4wings", "4wings-last-report", "4wings-report", "bulk-reports", "datasets", "events", "vessels"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  gfw-pp-cli workflow status

  # Show status as JSON
  gfw-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("gfw-pp-cli")
			}
