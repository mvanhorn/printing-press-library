...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "gfw-pp-cli",
		Short: `Gfw CLI — Every GFW vessel-behavior and risk surface on the command line, plus a local cache that turns one-shot lookups into a c…`,
		Long: `Gfw CLI — Every GFW vessel-behavior and risk surface on the command line, plus a local cache that turns one-shot lookups into a c…
...
Health check: run 'gfw-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("gfw-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'gfw-pp-cli profile list')")
	...
	rootCmd.AddCommand(newV4wingsCmd(flags))
	rootCmd.AddCommand(newBulkReportsCmd(flags))
	rootCmd.AddCommand(newEventsCmd(flags))
	rootCmd.AddCommand(newVesselsCmd(flags))
	...
	rootCmd.AddCommand(newEncountersParentCmd(flags))
	rootCmd.AddCommand(newVesselParentCmd(flags))
	rootCmd.AddCommand(newWatchParentCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newDatasetsPromotedCmd(flags))
