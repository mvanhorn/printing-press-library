...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "gfw-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GFW_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GFW_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "gfw-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/gfw-pp-cli/feedback.jsonl.
When ` + "`GFW_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GFW_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gfw-pp-cli feedback list
  gfw-pp-cli feedback list --limit 5
  gfw-pp-cli feedback list --json`,
