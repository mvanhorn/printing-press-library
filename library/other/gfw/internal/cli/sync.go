...
Resource scoping:
  ...
  gfw-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  gfw-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  gfw-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  gfw-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  gfw-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/gfw-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "download_file_url":
		return db.UpsertDownloadFileUrl(data)
	case "query":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"4wings":             "/v3/4wings/stats",
		"4wings-last-report": "/v3/4wings/last-report",
		"4wings-report":      "/v3/4wings/report",
		"bulk-reports":       "/v3/bulk-reports",
		"datasets":           "/v3/datasets/public-global-fishing-effort:latest",
		"events":             "/v3/events",
		"vessels":            "/v3/vessels",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "query", ParentTable: "bulk-reports", ParentIDParam: "bulk_report_id", PathTemplate: "/v3/bulk-reports/{bulk_report_id}/query", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "bulk_report_id", Field: "id"},
		}},
	}
}
