...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("GFW_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "gfw-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("GFW_TOKEN"); v != "" {
		cfg.GfwToken = v
		cfg.AuthSource = "env:GFW_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.GfwToken != "" {
		cfg.AuthSource = "config"
	}
...
func (c *Config) AuthHeader() string {
	...
	if c.GfwToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:GFW_TOKEN"
		}
		return "Bearer " + c.GfwToken
	}
	if c.AccessToken != "" {
