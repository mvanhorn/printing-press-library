...
// inaturalist-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/other/inaturalist/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/other/inaturalist/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/other/inaturalist/internal/learn/patterns"
...
const noLearnEnvVar = "INATURALIST_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("inaturalist-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  inaturalist-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  inaturalist-pp-cli recall "<question>" --agent
  inaturalist-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List recorded agent learnings with optional filters",
		Example: `  inaturalist-pp-cli learnings list --agent
  inaturalist-pp-cli learnings list --query "<substring>"
  inaturalist-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  inaturalist-pp-cli learnings forget "<question>" --resource <id>
  inaturalist-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  inaturalist-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  inaturalist-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
