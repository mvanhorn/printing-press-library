...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  inaturalist-pp-cli doctor
  inaturalist-pp-cli doctor --json
  inaturalist-pp-cli doctor --fail-on warn
  inaturalist-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set your API key with: export INATURALIST_API_TOKEN=\"your-token-here\""
					report["auth_key_url"] = "https://api.inaturalist.org/v1/"
				} else {
					authConfigured = true
			...
			if os.Getenv("INATURALIST_API_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "INATURALIST_API_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "INATURALIST_API_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "inaturalist-pp-cli"
						verifyPath := "/users/me"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'inaturalist-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'inaturalist-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'inaturalist-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'inaturalist-pp-cli sync' to refresh."
	}
	return report
}
