...
In local mode: searches locally synced data only.`,
  ...
  inaturalist-pp-cli search "error timeout"

  # Force local search only
  inaturalist-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  inaturalist-pp-cli search "status" --type annotations --data-source local
  # JSON output for piping
  inaturalist-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'inaturalist-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
