...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("INATURALIST_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("INATURALIST_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "inaturalist-pp-cli/feedback")
...
When ` + "`INATURALIST_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`INATURALIST_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  inaturalist-pp-cli feedback list
  inaturalist-pp-cli feedback list --limit 5
  inaturalist-pp-cli feedback list --json`,
