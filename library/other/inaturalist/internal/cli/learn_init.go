...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"iconic_taxa": {
			{Canonical: "Aves", Values: []string{"birds", "bird"}},
			{Canonical: "Plantae", Values: []string{"plants", "plant"}},
			{Canonical: "Mammalia", Values: []string{"mammals", "mammal"}},
			{Canonical: "Insecta", Values: []string{"insects", "insect"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: inaturalist-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: inaturalist-pp-cli: learn init: %v\n", err)
		}
	})
}
