...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"INATURALIST_CONFIG",
		"INATURALIST_CONFIG_DIR",
		"INATURALIST_DATA_DIR",
		"INATURALIST_STATE_DIR",
		"INATURALIST_CACHE_DIR",
		"INATURALIST_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
	...
	return home, filepath.Join(home, ".config", "inaturalist-pp-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "inaturalist-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("INATURALIST_API_TOKEN", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "inaturalist-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "inaturalist-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "inaturalist-pp-cli", "credentials.toml")
...
func configCredentialValue(cfg *config.Config) string {
	return cfg.InaturalistApiToken
}
...
func credentialValue(creds *cliutil.Credentials) string {
	return creds.InaturalistApiToken
}

func setCredentialValue(creds *cliutil.Credentials, token string) {
	creds.InaturalistApiToken = token
}
