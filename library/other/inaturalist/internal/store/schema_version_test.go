...
// TestMigrate_AddsColumnsOnUpgrade_ColoredHeatmapYGridJson verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_ColoredHeatmapYGridJson(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "colored_heatmap_y_grid_json" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	rows, err := s.DB().Query(`PRAGMA table_info("colored_heatmap_y_grid_json")`)
	if err != nil {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from colored_heatmap_y_grid_json after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_ColoredHeatmapYPng verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_ColoredHeatmapYPng(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "colored_heatmap_y_png" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("colored_heatmap_y_png")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from colored_heatmap_y_png after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_GridYGridJson verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_GridYGridJson(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "grid_y_grid_json" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("grid_y_grid_json")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from grid_y_grid_json after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_GridYPng verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_GridYPng(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "grid_y_png" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("grid_y_png")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from grid_y_png after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_HeatmapYGridJson verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_HeatmapYGridJson(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "heatmap_y_grid_json" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("heatmap_y_grid_json")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from heatmap_y_grid_json after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_HeatmapYPng verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_HeatmapYPng(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "heatmap_y_png" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("heatmap_y_png")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from heatmap_y_png after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_Identifications verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Identifications(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "identifications" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("identifications")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from identifications after migrate", want)
		}
	}
}
...
// TestMigrate_AddsColumnsOnUpgrade_Observations verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Observations(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from observations after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_Fave verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Fave(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "fave" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("fave")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from fave after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_Quality verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Quality(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "quality" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("quality")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from quality after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_Review verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Review(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "review" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("review")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from review after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_ObservationsSubscriptions verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_ObservationsSubscriptions(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "observations_subscriptions" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("observations_subscriptions")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from observations_subscriptions after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_TaxonSummary verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_TaxonSummary(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "taxon_summary" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("taxon_summary")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from taxon_summary after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_Unfave verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Unfave(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "unfave" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("unfave")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from unfave after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_ViewedUpdates verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_ViewedUpdates(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "viewed_updates" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("viewed_updates")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from viewed_updates after migrate", want)
		}
	}
}
...
// TestMigrate_AddsColumnsOnUpgrade_PlacesYPng verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_PlacesYPng(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "places_y_png" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("places_y_png")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from places_y_png after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_PointsYGridJson verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_PointsYGridJson(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "points_y_grid_json" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("points_y_grid_json")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from points_y_grid_json after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_PointsYPng verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_PointsYPng(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "points_y_png" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("points_y_png")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from points_y_png after migrate", want)
		}
	}
}
...
// TestMigrate_AddsColumnsOnUpgrade_Taxa verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Taxa(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "taxa" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("taxa")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from taxa after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_TaxonPlacesYPng verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_TaxonPlacesYPng(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "taxon_places_y_png" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("taxon_places_y_png")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from taxon_places_y_png after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_TaxonRangesYPng verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_TaxonRangesYPng(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "taxon_ranges_y_png" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("taxon_ranges_y_png")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from taxon_ranges_y_png after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_Mute verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Mute(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "mute" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("mute")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from mute after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_UsersProjects verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_UsersProjects(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "users_projects" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("users_projects")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from users_projects after migrate", want)
		}
	}
}
