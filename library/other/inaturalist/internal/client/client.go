...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "inaturalist-pp-cli/1.3.0")
		}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export INATURALIST_API_TOKEN=<your-token> or inaturalist-pp-cli auth set-token <token>")
}
...
func sanitizeJSONResponse(body []byte) []byte {
	...
	// iNaturalist may include public, obscured, and authenticated private
	// coordinate fields in otherwise ordinary JSON responses. This CLI never
	// emits coordinates, so redact them at the transport boundary before cache,
	// command, MCP, or error paths can observe the response.
	var value any
	if json.Unmarshal(body, &value) == nil {
		redactCoordinateFields(value)
...
func redactCoordinateFields(value any) {
	switch typed := value.(type) {
	case []any:
		for _, item := range typed {
			redactCoordinateFields(item)
		}
	case map[string]any:
		for key := range typed {
			lowerKey := strings.ToLower(key)
			switch lowerKey {
			case "location", "geojson", "coordinates", "geometry", "geometry_geojson", "place_geometry", "centroid", "bbox", "bounding_box", "latitude", "longitude", "lat", "lng":
				delete(typed, key)
			case "private", "private_location", "private_geojson", "private_place_guess", "private_place_ids":
				delete(typed, key)
			default:
				if strings.HasPrefix(lowerKey, "private_") {
					delete(typed, key)
				} else {
					redactCoordinateFields(typed[key])
				}
			}
		}
	}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.InaturalistApiToken)
	}
