...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Care",
	...
	// care.com deviation: persistent Chrome-profile auth (chromedp). care.com's
	// passwordless magic-link + single-active-session model defeats the standard
	// cookie-extraction flow; profile-login keeps a durable logged-in profile and
	// refresh re-extracts cookies headlessly with no re-login. See care_auth.go.
	cmd.AddCommand(newCareAuthProfileLoginCmd(flags))
	cmd.AddCommand(newCareAuthRefreshCmd(flags))
...
Use --chrome to read cookies from Chrome for .care.com.
...
profile by name when the installed backend supports it.`,
		Example: `  care-pp-cli auth login --chrome
  care-pp-cli auth login --browser
  care-pp-cli auth login --cookies-file storage-state.json
  care-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !browserFlag && cookiesFile == "" {
				...
				fmt.Fprintf(cmd.OutOrStdout(), "  care-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  care-pp-cli auth login --browser\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  care-pp-cli auth login --cookies-file storage-state.json\n")
				return nil
			}
			if browserFlag && cookiesFile != "" {
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  care-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  care-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: .care.com\n")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  care-pp-cli auth set-token eyJhbGciOi...",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  care-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
