...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CARE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CARE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "care-pp-cli/feedback")
...
When ` + "`CARE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CARE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  care-pp-cli feedback list
  care-pp-cli feedback list --limit 5
  care-pp-cli feedback list --json`,
