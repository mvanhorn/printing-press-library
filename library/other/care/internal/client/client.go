...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		// care.com deviation: authenticate by session cookie, not a bearer
		// token. The cookie is sourced from the persistent-profile provider
		// (see internal/cli/care_auth.go): CARE_SESSION_COOKIE env wins, else
		// the cached header written by `care-pp-cli auth profile-login/refresh`.
		// care's cookies live in that session-cookie file, not the generated
		// cookies.json, so the jar stays empty for care and this manual header
		// is the single cookie source (no double-ship). careSessionCookie()
		// returns "" unless the file/env is set, so native cookie-jar auth is
		// unaffected when this deviation is not in use.
		if sc := careSessionCookie(); sc != "" {
			req.Header.Set("Cookie", sc)
		}
		req.Header.Set("origin", "https://www.care.com")
		req.Header.Set("apollographql-client-name", "care-web")
		if c.Config != nil {
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "care-pp-cli auth set-token <token>")
}
