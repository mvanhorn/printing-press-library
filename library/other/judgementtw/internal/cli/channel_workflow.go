...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  judgementtw-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  judgementtw-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"find", "judgments", "knowledge"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  judgementtw-pp-cli workflow status

  # Show status as JSON
  judgementtw-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("judgementtw-pp-cli")
			}
