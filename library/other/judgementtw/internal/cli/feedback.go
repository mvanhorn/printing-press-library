...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".judgementtw-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("JUDGEMENTTW_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("JUDGEMENTTW_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "judgementtw-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.judgementtw-pp-cli/feedback.jsonl.
When ` + "`JUDGEMENTTW_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`JUDGEMENTTW_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  judgementtw-pp-cli feedback list
  judgementtw-pp-cli feedback list --limit 5
  judgementtw-pp-cli feedback list --json`,
