...
Exit codes & warnings:
  ...
  judgementtw-pp-cli sync

  # Sync specific resources only
  judgementtw-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  judgementtw-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  judgementtw-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  judgementtw-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  judgementtw-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "judgments":
		return db.UpsertJudgments(data)
	case "knowledge":
		return db.UpsertKnowledge(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"find":      "/FJUD/qryresultlst.aspx",
		"judgments": "/FJUD/recent",
		"knowledge": "/Default.aspx",
	}
