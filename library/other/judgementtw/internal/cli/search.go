...
In local mode: searches locally synced data only.`,
  ...
  judgementtw-pp-cli search "error timeout"

  # Force local search only
  judgementtw-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  judgementtw-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  judgementtw-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /searcher.aspx
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get("/searcher.aspx", map[string]string{
					"query": query,
				})
				if getErr == nil {
			...
			// Local FTS search
			if dbPath == "" {
				dbPath = defaultDBPath("judgementtw-pp-cli")
			}

			db, err := store.OpenWithContext(cmd.Context(), dbPath)
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'judgementtw-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "knowledge":
				results, err = db.SearchKnowledge(query, limit)
			case "":
				// Search all FTS-enabled tables individually to avoid duplicates.
				seen := make(map[string]bool)
				_ = seen // prevent unused error when no FTS tables exist
				{
					partial, searchErr := db.SearchKnowledge(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search knowledge failed: %w", searchErr)
					}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/judgementtw-pp-cli/data.db)")
