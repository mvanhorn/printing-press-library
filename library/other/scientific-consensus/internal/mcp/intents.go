...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("evidence_pyramid_as_a_table",
			mcplib.WithDescription("Study-design distribution from meta-analyses down to case reports."),
			mcplib.WithBoolean("csv", mcplib.Description("Override the recipe's --csv value.")),
		),
		handleEvidencePyramidAsATable,
	)
}

// handleEvidencePyramidAsATable runs the evidence_pyramid_as_a_table recipe intent: Study-design distribution from meta-analyses down to case reports.
func handleEvidencePyramidAsATable(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
