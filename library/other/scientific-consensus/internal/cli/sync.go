...
Resource scoping:
  ...
  scientific-consensus-pp-cli sync

  # Sync specific resources only
  scientific-consensus-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  scientific-consensus-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  scientific-consensus-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  scientific-consensus-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  scientific-consensus-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("scientific-consensus-pp-cli")
			}
			...
			if cliutil.IsDogfoodEnv() && !cmd.Flags().Changed("max-pages") {
				// This CLI syncs six OpenAlex entities; each page is a ~1-2s
				// live call, so the generator default of 10 pages/resource
				// blows the dogfood 30s/command budget. One page (~600 records
				// total) keeps the live matrix inside the timeout.
				maxPages = 1
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/scientific-consensus-pp-cli/data.db)")
...
func determinePaginationDefaults() paginationDefaults {
	// OpenAlex uses per-page (max 200) for page size with basic page-number
	// pagination; it rejects the generic "limit" param at the API edge.
