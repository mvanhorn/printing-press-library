...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  scientific-consensus-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  scientific-consensus-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"authors", "funders", "institutions", "sources", "topics", "works"}
			totalSynced := 0
			...
			// Curtail page depth under live-dogfood so the six-entity archive
			// finishes inside the matrix's per-command timeout.
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  scientific-consensus-pp-cli workflow status

  # Show status as JSON
  scientific-consensus-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("scientific-consensus-pp-cli")
			}
