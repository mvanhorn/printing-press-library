...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "scientific-consensus-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SCIENTIFIC_CONSENSUS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SCIENTIFIC_CONSENSUS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "scientific-consensus-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/scientific-consensus-pp-cli/feedback.jsonl.
When ` + "`SCIENTIFIC_CONSENSUS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SCIENTIFIC_CONSENSUS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "scientific-consensus-pp-cli",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  scientific-consensus-pp-cli feedback list
  scientific-consensus-pp-cli feedback list --limit 5
  scientific-consensus-pp-cli feedback list --json`,
