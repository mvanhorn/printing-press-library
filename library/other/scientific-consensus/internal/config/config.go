...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("SCIENTIFIC_CONSENSUS_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "scientific-consensus-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("SCIENTIFIC_CONSENSUS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
