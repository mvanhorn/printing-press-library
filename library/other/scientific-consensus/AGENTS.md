# Scientific Consensus Printed CLI Agent Guide

This directory is a generated `scientific-consensus-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
scientific-consensus-pp-cli doctor --json
scientific-consensus-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
scientific-consensus-pp-cli which "<capability>" --json
scientific-consensus-pp-cli <command> --help
```
...
```bash
scientific-consensus-pp-cli <command> --agent
```
...
```bash
scientific-consensus-pp-cli <command> --help
scientific-consensus-pp-cli <command> --dry-run --agent
```
