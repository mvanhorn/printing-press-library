# Kakao Transparency Printed CLI Agent Guide

This directory is a generated `kakao-transparency-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
kakao-transparency-pp-cli doctor --json
kakao-transparency-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
kakao-transparency-pp-cli which "<capability>" --json
kakao-transparency-pp-cli <command> --help
```
...
```bash
kakao-transparency-pp-cli <command> --agent
```
...
```bash
kakao-transparency-pp-cli <command> --help
kakao-transparency-pp-cli <command> --dry-run --agent
```
