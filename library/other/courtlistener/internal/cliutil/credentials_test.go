...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		...
		"COURTLISTENER_CONFIG_DIR",
		"COURTLISTENER_DATA_DIR",
		"COURTLISTENER_STATE_DIR",
		"COURTLISTENER_CACHE_DIR",
		"COURTLISTENER_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
		"XDG_CACHE_HOME",
		"COURTLISTENER_TOKEN_AUTH",
	} {
		t.Setenv(name, "")
	}
	...
	return home, filepath.Join(home, ".config", "courtlistener-pp-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "courtlistener-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("COURTLISTENER_TOKEN_AUTH", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "courtlistener-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "courtlistener-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "courtlistener-pp-cli", "credentials.toml")
...
func configCredentialValue(cfg *config.Config) string {
	return cfg.CourtlistenerTokenAuth
}
...
func credentialValue(creds *cliutil.Credentials) string {
	return creds.CourtlistenerTokenAuth
}

func setCredentialValue(creds *cliutil.Credentials, token string) {
	creds.CourtlistenerTokenAuth = token
}
