...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("audit_recap_availability",
			mcplib.WithDescription("Separate available files from metadata-only or unavailable document records."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Override the recipe's positional id value.")),
		),
		handleAuditRecapAvailability,
	)
}

// handleAuditRecapAvailability runs the audit_recap_availability recipe intent: Separate available files from metadata-only or unavailable document records.
func handleAuditRecapAvailability(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
