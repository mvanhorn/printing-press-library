...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("COURTLISTENER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("COURTLISTENER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "courtlistener-pp-cli/feedback")
...
When ` + "`COURTLISTENER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`COURTLISTENER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  courtlistener-pp-cli feedback list
  courtlistener-pp-cli feedback list --limit 5
  courtlistener-pp-cli feedback list --json`,
