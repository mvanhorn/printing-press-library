...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  courtlistener-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  courtlistener-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"attorneys", "docket-entries", "legal_search", "parties", "recap-documents"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  courtlistener-pp-cli workflow status

  # Show status as JSON
  courtlistener-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("courtlistener-pp-cli")
			}
