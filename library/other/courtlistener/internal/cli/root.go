...
type rootFlags struct {
	...
	// invocation. Mirrors the COURTLISTENER_NO_LEARN env var.
	noLearn       bool
	selectFields  string
	configPath    string
	homePath      string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "courtlistener-pp-cli",
		Short: "Search case law publicly and, with a CourtListener token, build bounded docket briefs, relationship maps, filing watches, and RECAP availability audits.",
		Long: `Courtlistener CLI
...
Health check: run 'courtlistener-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("courtlistener-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'courtlistener-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelCounselCmd(flags))
	rootCmd.AddCommand(newNovelDocketCmd(flags))
	rootCmd.AddCommand(newNovelJudgeCmd(flags))
	rootCmd.AddCommand(newNovelNewFilingsCmd(flags))
	rootCmd.AddCommand(newNovelPartyCmd(flags))
	rootCmd.AddCommand(newNovelRecapGapsCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newAttorneysPromotedCmd(flags))
	rootCmd.AddCommand(newDocketEntriesPromotedCmd(flags))
	rootCmd.AddCommand(newDocketsPromotedCmd(flags))
	rootCmd.AddCommand(newLegalSearchPromotedCmd(flags))
	rootCmd.AddCommand(newPartiesPromotedCmd(flags))
	rootCmd.AddCommand(newPeoplePromotedCmd(flags))
	rootCmd.AddCommand(newRecapDocumentsPromotedCmd(flags))
