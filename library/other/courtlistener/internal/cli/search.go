...
In local mode: searches locally synced data only.`,
  ...
  courtlistener-pp-cli search "error timeout"

  # Force local search only
  courtlistener-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  courtlistener-pp-cli search "status" --type attorneys --data-source local
  # JSON output for piping
  courtlistener-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'courtlistener-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
