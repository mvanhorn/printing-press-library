...
type Config struct {
	...
	CourtlistenerTokenAuth string          `toml:"token_auth"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("COURTLISTENER_TOKEN_AUTH"); v != "" {
		cfg.CourtlistenerTokenAuth = v
		cfg.markEnvOverride("CourtlistenerTokenAuth")
		cfg.AuthSource = "env:COURTLISTENER_TOKEN_AUTH"
		cfg.CredentialSource = "env:COURTLISTENER_TOKEN_AUTH"
	}
	...
	if v := os.Getenv("COURTLISTENER_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("COURTLISTENER_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "courtlistener-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.CourtlistenerTokenAuth
	if token == "" {
		return ""
	}
	if c.CourtlistenerTokenAuth == "" {
		return ""
	}
	return token
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		CourtlistenerTokenAuth: c.CourtlistenerTokenAuth,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.CourtlistenerTokenAuth = creds.CourtlistenerTokenAuth
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.CourtlistenerTokenAuth = token
	delete(c.envOverrides, "CourtlistenerTokenAuth")
	c.updateFileConfigField("CourtlistenerTokenAuth")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["CourtlistenerTokenAuth"] {
			out.CourtlistenerTokenAuth = c.fileConfig.CourtlistenerTokenAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "CourtlistenerTokenAuth":
		c.fileConfig.CourtlistenerTokenAuth = c.CourtlistenerTokenAuth
	}
}
