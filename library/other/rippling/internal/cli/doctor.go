...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  rippling-pp-cli doctor
  rippling-pp-cli doctor --json
  rippling-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export RIPPLING_PLATFORM_OAUTH2_PRODUCTION=<your-key>"
				} else {
			...
			if os.Getenv("RIPPLING_PLATFORM_OAUTH2_PRODUCTION") != "" {
				authEnvSet = append(authEnvSet, "RIPPLING_PLATFORM_OAUTH2_PRODUCTION")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "RIPPLING_PLATFORM_OAUTH2_PRODUCTION")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "rippling-pp-cli"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'rippling-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'rippling-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'rippling-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'rippling-pp-cli sync' to refresh."
	}
	return report
}
