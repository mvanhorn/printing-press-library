...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'rippling-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token. Set it with: export RIPPLING_PLATFORM_OAUTH2_PRODUCTION=<your-token>"+
			"\n      or run browser OAuth login: rippling-pp-cli auth login --client-id <client-id> --client-secret <client-secret>"+
			"\n      Run 'rippling-pp-cli doctor' to check auth status.", err))
