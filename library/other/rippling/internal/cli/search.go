...
In local mode: searches locally synced data only.`,
  ...
  rippling-pp-cli search "error timeout"

  # Force local search only
  rippling-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  rippling-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  rippling-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'rippling-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "custom_apps":
				results, err = db.SearchCustomApps(query, limit)
			case "custom_objects":
				results, err = db.SearchCustomObjects(query, limit)
			case "functions":
				results, err = db.SearchFunctions(query, limit)
			case "job_requisitions_write":
				results, err = db.SearchJobRequisitionsWrite(query, limit)
			case "object_categories":
				results, err = db.SearchObjectCategories(query, limit)
			case "":
				// Search all FTS-enabled tables individually to avoid duplicates.
				seen := make(map[string]bool)
				_ = seen // prevent unused error when no FTS tables exist
				{
					partial, searchErr := db.SearchCustomApps(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search custom_apps failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchCustomObjects(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search custom_objects failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchFunctions(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search functions failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchJobRequisitionsWrite(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search job_requisitions_write failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchObjectCategories(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search object_categories failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
