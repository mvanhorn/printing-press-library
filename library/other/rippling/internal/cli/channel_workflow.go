...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  rippling-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  rippling-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"break-types", "business-partner-groups", "business-partners", "candidate-applications", "candidates", "companies", "company-legal-entities", "company-legal-entity-workers", "compensation-bands-details", "compensations", "custom-apps", "custom-fields", "custom-objects", "custom-pages", "custom-settings", "departments", "earning-types", "earnings-inputs", "employment-types", "entitlements", "functions", "headcount-positions", "headcount-priorities", "job-assignments", "job-codes", "job-dimensions", "job-functions", "job-pay-rate-exceptions", "job-requisitions", "kiosk-badges", "leave-accruals", "leave-balances", "leave-requests", "leave-types", "legal-entities", "levels", "location-factors", "metadata", "object-categories", "payroll-runs", "platform-capabilities", "scheduled-job-assignments", "schedules", "shift-inputs", "shiftassignments", "software-deployments", "sso-me", "supergroups", "teams", "time-cards", "time-entries", "titles", "tracks", "unassignedshifts", "users", "work-locations", "worker-time-splits", "workers",  }
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  rippling-pp-cli workflow status

  # Show status as JSON
  rippling-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("rippling-pp-cli")
			}
