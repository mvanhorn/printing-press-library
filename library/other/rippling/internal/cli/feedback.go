...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".rippling-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("RIPPLING_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("RIPPLING_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "rippling-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.rippling-pp-cli/feedback.jsonl.
When ` + "`RIPPLING_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`RIPPLING_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  rippling-pp-cli feedback list
  rippling-pp-cli feedback list --limit 5
  rippling-pp-cli feedback list --json`,
