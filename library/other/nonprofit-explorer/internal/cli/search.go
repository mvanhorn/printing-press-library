...
func newSearchCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		// renamed "search" -> "search-local" so the primary ranked live search
		// in ext_nonprofit.go can own the `search` verb without a runtime
		// RemoveCommand shim.
		Use:   "search-local <query>",
		Short: "Full-text search across locally synced data (FTS5), with live-API fallback",
...
In local mode: searches locally synced data only.`,
  ...
  nonprofit-explorer-pp-cli search-local "food bank"

  # Force local search only
  nonprofit-explorer-pp-cli search-local "food bank" --data-source local
  # Search a specific resource type locally
  nonprofit-explorer-pp-cli search-local "food bank" --type organizations --data-source local
  # JSON output for piping
  nonprofit-explorer-pp-cli search-local "food bank" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'nonprofit-explorer-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
