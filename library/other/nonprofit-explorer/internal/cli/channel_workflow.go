...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  nonprofit-explorer-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  nonprofit-explorer-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  nonprofit-explorer-pp-cli workflow status

  # Show status as JSON
  nonprofit-explorer-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("nonprofit-explorer-pp-cli")
			}
