...
// nonprofit-explorer-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
const noLearnEnvVar = "NONPROFIT_EXPLORER_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("nonprofit-explorer-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  nonprofit-explorer-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  nonprofit-explorer-pp-cli recall "<question>" --agent
  nonprofit-explorer-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  nonprofit-explorer-pp-cli learnings list --agent
  nonprofit-explorer-pp-cli learnings list --query "<substring>"
  nonprofit-explorer-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  nonprofit-explorer-pp-cli learnings forget "<question>" --resource <id>
  nonprofit-explorer-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  nonprofit-explorer-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  nonprofit-explorer-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
