...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NONPROFIT_EXPLORER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NONPROFIT_EXPLORER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "nonprofit-explorer-pp-cli/feedback")
...
When ` + "`NONPROFIT_EXPLORER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NONPROFIT_EXPLORER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  nonprofit-explorer-pp-cli feedback list
  nonprofit-explorer-pp-cli feedback list --limit 5
  nonprofit-explorer-pp-cli feedback list --json`,
