...
type rootFlags struct {
	...
	// invocation. Mirrors the NONPROFIT_EXPLORER_NO_LEARN env var.
	noLearn       bool
	selectFields  string
	configPath    string
	homePath      string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "nonprofit-explorer-pp-cli",
		Short: "Manage nonprofit-explorer resources via the nonprofit-explorer API",
		Long: `Manage nonprofit-explorer resources via the nonprofit-explorer API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'nonprofit-explorer-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("nonprofit-explorer-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'nonprofit-explorer-pp-cli profile list')")
	...
	// hand-authored analytical commands (defined in ext_*.go) are registered
	// directly so the Cobra registration graph — and any static reachability
	// analysis over it — sees them. The novelCommandHooks loop below stays
	// for future markerless extensions.
	rootCmd.AddCommand(newNPSearchCmd(flags))
	rootCmd.AddCommand(newNPOrgCmd(flags))
	rootCmd.AddCommand(newNPFilingsCmd(flags))
	rootCmd.AddCommand(newNPFinancialsCmd(flags))
	rootCmd.AddCommand(newNPCompareCmd(flags))
	rootCmd.AddCommand(newNPPeopleCmd(flags))
