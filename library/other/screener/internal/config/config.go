...
type Config struct {
	...
	ScreenerSessionCookie string          `toml:"session_cookie"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("SCREENER_SESSION_COOKIE"); v != "" {
		cfg.ScreenerSessionCookie = v
		cfg.markEnvOverride("ScreenerSessionCookie")
		cfg.AuthSource = "env:SCREENER_SESSION_COOKIE"
		cfg.CredentialSource = "env:SCREENER_SESSION_COOKIE"
	}
	...
	if v := os.Getenv("SCREENER_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("SCREENER_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "screener-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.ScreenerSessionCookie != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:SCREENER_SESSION_COOKIE"
		}
		return ensureAuthScheme("Bearer", c.ScreenerSessionCookie)
	}
	if c.AccessToken != "" {
		if c.AuthSource == "" {
...
func (c *Config) CookieCredential() string {
	if c.ScreenerSessionCookie != "" {
		return c.ScreenerSessionCookie
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		ScreenerSessionCookie: c.ScreenerSessionCookie,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.ScreenerSessionCookie == "" {
		c.ScreenerSessionCookie = creds.ScreenerSessionCookie
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.ScreenerSessionCookie = ""
	delete(c.envOverrides, "ScreenerSessionCookie")
	c.updateFileConfigField("ScreenerSessionCookie")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["ScreenerSessionCookie"] {
			out.ScreenerSessionCookie = c.fileConfig.ScreenerSessionCookie
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "ScreenerSessionCookie":
		c.fileConfig.ScreenerSessionCookie = c.ScreenerSessionCookie
	}
}
