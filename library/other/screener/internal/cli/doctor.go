...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  screener-pp-cli doctor
  screener-pp-cli doctor --json
  screener-pp-cli doctor --fail-on warn
  screener-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !credentialConfigured {
				...
				report["auth_hint"] = "screener-pp-cli auth login --chrome"
			} else {
				authConfigured = true
				...
				if cfg != nil && cfg.CredentialConfigured() {
					...
					report["auth_domain"] = ".screener.in"
				}
			}
			...
			if os.Getenv("SCREENER_SESSION_COOKIE") != "" {
				authEnvSet = append(authEnvSet, "SCREENER_SESSION_COOKIE")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "SCREENER_SESSION_COOKIE")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					healthPath := "/company/INFY/consolidated/"
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "screener-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("screener-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'screener-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'screener-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		...
		report["hint"] = "No sync recorded; run 'screener-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'screener-pp-cli sync' to refresh."
	}
	return report
}
