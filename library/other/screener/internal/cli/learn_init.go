...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^[A-Z0-9]{3,8}$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: screener-pp-cli: ticker pattern %q failed to compile: %v\n", `^[A-Z0-9]{3,8}$`, err)
	} else {
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"company": {
			{Canonical: "Infosys Ltd", Values: []string{"infy", "infosys", "INFY"}},
			{Canonical: "Tata Steel Ltd", Values: []string{"tata steel", "TATASTEEL", "tata"}},
			{Canonical: "Reliance Industries Ltd", Values: []string{"reliance", "reliance industries", "RELIANCE"}},
			{Canonical: "Tata Motors Ltd", Values: []string{"tata motors", "TATAMOTORS"}},
			{Canonical: "HDFC Bank Ltd", Values: []string{"hdfc bank", "HDFCBANK"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: screener-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: screener-pp-cli: learn init: %v\n", err)
		}
	})
}
