...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SCREENER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SCREENER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "screener-pp-cli/feedback")
...
When ` + "`SCREENER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SCREENER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  screener-pp-cli feedback "the compare table was unclear"
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  screener-pp-cli feedback list
  screener-pp-cli feedback list --limit 5
  screener-pp-cli feedback list --json`,
