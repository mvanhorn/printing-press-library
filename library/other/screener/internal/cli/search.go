...
In local mode: searches locally synced data only.`,
  ...
  screener-pp-cli search "error timeout"

  # Force local search only
  screener-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  screener-pp-cli search "status" --type company --data-source local
  # JSON output for piping
  screener-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /full-text-search/
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/full-text-search/", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'screener-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "company":
				results, err = db.SearchCompany(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchCompany(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search company failed: %w", searchErr)
					}
