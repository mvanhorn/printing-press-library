...
Resource scoping:
  ...
  screener-pp-cli sync
  # Sync specific resources only
  screener-pp-cli sync --resources explore,filings

  # Full resync (ignore previous checkpoint)
  screener-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  screener-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  screener-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  screener-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "screener-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "explore":
		return db.UpsertExplore(data)
	case "filings":
		return db.UpsertFilings(data)
	case "full_text_search":
		return db.UpsertFullTextSearch(data)
	case "ipo":
		return db.UpsertIpo(data)
	case "market":
...
func responsePathForResource(resource, path string) []string {
	...
	case "company\x00/api/company/{company_id}/chart/":
