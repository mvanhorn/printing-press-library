...
var whichIndex = []whichEntry{
	{Command: "compare", Description: "See two to four companies' fundamentals side by side (valuation, margins, growth, insider activity) without tab-switching.", Group: "Local state that compounds", WhyItMatters: "Pick this when an agent needs to decide between candidate stocks on comparable fundamentals instead of fetching pages one at a time."},
	{Command: "qtrend", Description: "Spot whether a company's quarterly profit/sales growth is accelerating or deteriorating, with YOY change and margin drift computed automatically.", Group: "Local state that compounds", WhyItMatters: "Pick this when an agent needs the trend shape of a company's earnings, not just the latest quarter's raw numbers."},
	{Command: "overlap", Description: "Find companies that appear in two or more stock screens in one command, replacing spreadsheet dedup.", Group: "Cross-entity local queries", WhyItMatters: "Pick this when an agent needs the intersection of multiple screening strategies to find high-conviction candidates."},
	{Command: "rank", Description: "Re-score a screen's companies with a composite of fundamentals (P/E, ROCE, growth), sorted by what matters to you.", Group: "Cross-entity local queries", WhyItMatters: "Pick this when an agent should prioritize within a screen by fundamentals or insider conviction rather than the default table order."},
	{Command: "insider-flow", Description: "Answer 'who is net buying the most this month?' with per-company insider buy/sell aggregation.", Group: "Local state that compounds", WhyItMatters: "Pick this when an agent needs insider conviction ranked by net value, not a raw list of individual trades."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  screener-pp-cli which "stale tickets"
  screener-pp-cli which "bottleneck"
  screener-pp-cli which --limit 1 "send message"
  screener-pp-cli which                                # list the full capability index`,
