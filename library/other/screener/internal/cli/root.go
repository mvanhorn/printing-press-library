...
type rootFlags struct {
	...
	// invocation. Mirrors the SCREENER_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "screener-pp-cli",
		Short: `Screener CLI — Every Screener.in feature, plus offline search, screen intersections, and a local fundamentals mirror no other tool has.`,
		Long: `Screener CLI — Every Screener.in feature, plus offline search, screen intersections, and a local fundamentals mirror no other tool has.
...
Health check: run 'screener-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("screener-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select id,name,url)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'screener-pp-cli profile list')")
	...
	rootCmd.PersistentFlags().Float64Var(&flags.rateLimit, "rate-limit", 1.0, "Max requests per second (0 to disable; screener.in throttles burst access, so 1/s is the polite default)")
	...
	rootCmd.AddCommand(newExplorePromotedCmd(flags))
	rootCmd.AddCommand(newFilingsPromotedCmd(flags))
	rootCmd.AddCommand(newFullTextSearchPromotedCmd(flags))
	rootCmd.AddCommand(newIpoPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelInsiderFlowCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelOverlapCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelQtrendCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelRankCmd(flags))
