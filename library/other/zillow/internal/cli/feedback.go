...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ZILLOW_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ZILLOW_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "zillow-pp-cli/feedback")
...
When ` + "`ZILLOW_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ZILLOW_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  zillow-pp-cli feedback list
  zillow-pp-cli feedback list --limit 5
  zillow-pp-cli feedback list --json`,
