...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"metric": {
			{Canonical: "ZHVI", Values: []string{"home value index", "home values"}},
			{Canonical: "ZORI", Values: []string{"observed rent index", "rents"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: zillow-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: zillow-pp-cli: learn init: %v\n", err)
		}
	})
}
