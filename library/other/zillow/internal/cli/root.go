...
type rootFlags struct {
	...
	// invocation. Mirrors the ZILLOW_NO_LEARN env var.
	noLearn       bool
	selectFields  string
	configPath    string
	homePath      string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "zillow-pp-cli",
		Short: "Official Zillow Research data, normalized for explainable housing-market decisions.",
		Long: `Sync public Zillow Research time series into explainable housing-market decisions without scraping Zillow.com.
...
Run 'zillow-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("zillow-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'zillow-pp-cli profile list')")
	...
	rootCmd.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
		...
		if cmd != rootCmd && !noLearnActive(flags) && !shouldSkipLearnHook(cmd.CommandPath()) {
