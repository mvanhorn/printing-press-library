# Betriebsrat Printed CLI Agent Guide

This directory is a generated `betriebsrat-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
betriebsrat-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
betriebsrat-pp-cli which "<capability>" --json
betriebsrat-pp-cli <command> --help
```
...
```bash
betriebsrat-pp-cli <command> --agent
```
...
```bash
betriebsrat-pp-cli <command> --help
betriebsrat-pp-cli <command> --dry-run --agent
```
