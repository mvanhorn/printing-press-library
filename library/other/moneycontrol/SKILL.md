...
description: "Indian market news and quotes without the scraping — offline-searchable and agent-native. Trigger phrases: `moneycontrol news`, `indian market news`, `nifty sensex today`, `stock gainers losers`, `reliance quote`, `what moved the market today`, `use moneycontrol`."
  ...
  openclaw:
    ...
    install:
      - kind: go
        ...
        module: github.com/mvanhorn/printing-press-library/library/other/moneycontrol/cmd/moneycontrol-pp-cli
---
...
Use this CLI when you need Indian stock-market news, index quotes, market-action screens (gainers/losers/52-week/etc.), or per-stock quotes from moneycontrol, especially when an agent or script should consume the output. It shines for portfolio-CLI news ingestion and for 'what happened today' queries.

## Anti-triggers

Do not use this CLI for:
- Do not use for portfolio tracking, watchlists, or price alerts — those need a moneycontrol login.
- Do not use for mutual funds, crypto, or personal-finance content — out of scope.
- Do not use for real-time tick-by-tick streaming data; the pricefeed is a snapshot, not a websocket.
...
This printed CLI owns bounded freshness only for registered store-backed read command paths. In `--data-source auto` mode, those paths check `sync_state` and may run a bounded refresh before reading local data. `--data-source local` never refreshes. `--data-source live` reads the API and does not mutate the local store. Set `MONEYCONTROL_NO_AUTO_REFRESH=1` to skip the freshness hook without changing source selection.
...
- **Filterable** — `--select` keeps a subset of fields. Dotted paths descend into nested structures; arrays traverse element-wise. Critical for keeping context small on verbose APIs:
  ...
  ```bash
  moneycontrol-pp-cli articles get --slug india/tribunals-reforms-bill-2026- --agent --select id,name,status
  ```
...
```bash
moneycontrol-pp-cli recall "<user's question>" --agent
```
...
- `lookup_refresh_available` (top-level): an entity in the query has no lookup row yet, but synced data could provide one. Run `moneycontrol-pp-cli sync` to refresh entity lookups.
...
```bash
moneycontrol-pp-cli teach --query "<user's question>" --resource-type <type> --resource <id1> --resource <id2>
