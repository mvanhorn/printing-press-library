...
var readCommandResources = map[string][]string{
	"moneycontrol-pp-cli trending":        {"trending"},
	"moneycontrol-pp-cli trending list":   {"trending"},
	"moneycontrol-pp-cli trending get":    {"trending"},
	"moneycontrol-pp-cli trending search": {"trending"},
}
...
func cachePolicy() cliutil.Policy {
	...
	envOptOut := "MONEYCONTROL_NO_AUTO_REFRESH"
...
func autoRefreshIfStale(ctx context.Context, flags *rootFlags, resources []string) (meta cliutil.FreshnessMeta) {
	...
	if err := runAutoRefresh(refreshCtx, flags, db, resources); err != nil {
		fmt.Fprintf(os.Stderr, "warning: using stale moneycontrol-pp-cli cache (refresh failed: %v)\n", err)
