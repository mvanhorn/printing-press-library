...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/other/moneycontrol/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/other/moneycontrol/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/other/moneycontrol/internal/learn/patterns"
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  moneycontrol-pp-cli teach --query "<question>" --resource-type <type> --resource <id> --resource <id> &`,
...
when learnings exist.`,
		Example: `  moneycontrol-pp-cli recall "<question>" --agent
  moneycontrol-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
  ...
  moneycontrol-pp-cli learnings list --query "<substring>"
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  moneycontrol-pp-cli learnings forget "<question>" --resource <id>
  moneycontrol-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
