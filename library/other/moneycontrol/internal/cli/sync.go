...
Resource scoping:
  ...
  moneycontrol-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  moneycontrol-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  moneycontrol-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  moneycontrol-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  moneycontrol-pp-cli sync --latest-only`,
...
func responsePathForResource(resource, path string) []string {
	...
	case "indices\x00/pricefeed/notapplicable/inidicesindia/{key}":
		return []string{"data"}
	case "stocks\x00/pricefeed/nse/equitycash/{sc_id}":
		return []string{"data"}
	}
	return nil
}
