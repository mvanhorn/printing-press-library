# ImmoScout24 Printed CLI Agent Guide

This directory is a generated `immoscout24-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
immoscout24-pp-cli doctor --json
immoscout24-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
immoscout24-pp-cli which "<capability>" --json
immoscout24-pp-cli <command> --help
```
...
```bash
immoscout24-pp-cli <command> --agent
```
...
```bash
immoscout24-pp-cli <command> --help
immoscout24-pp-cli <command> --dry-run --agent
```
