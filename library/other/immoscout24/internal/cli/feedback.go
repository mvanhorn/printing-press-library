...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("IMMOSCOUT24_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("IMMOSCOUT24_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "immoscout24-pp-cli/feedback")
...
When ` + "`IMMOSCOUT24_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`IMMOSCOUT24_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  immoscout24-pp-cli feedback list
  immoscout24-pp-cli feedback list --limit 5
  immoscout24-pp-cli feedback list --json`,
