...
Resource scoping:
  ...
  immoscout24-pp-cli sync

  # Sync specific resources only
  immoscout24-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  immoscout24-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  immoscout24-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  immoscout24-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  immoscout24-pp-cli sync --latest-only`,
...
func syncResource(ctx context.Context, c interface {
	...
	for {
		...
		applySyncDefaultParams(resource, params)
		...
		items = normalizeSyncItems(resource, path, items)
...
func applySyncDefaultParams(resource string, params map[string]string) {
	switch resource {
	case "immoscout24-mobile-search":
		setDefaultParam(params, "searchType", "region")
		setDefaultParam(params, "realEstateType", "apartmentrent")
		setDefaultParam(params, "geocodes", "/de/berlin/berlin")
		setDefaultParam(params, "sorting", "standard")
	}
}
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "immoscout24-mobile-search":
		return db.UpsertImmoscout24MobileSearch(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"immoscout24-mobile-search": "/search/map/v3",
	}
...
func responsePathForResource(resource, path string) []string {
	...
	case "expose\x00/expose/{id}":
		return []string{"sections"}
	case "immoscout24-mobile-search\x00/search/list":
		return []string{"resultListItems"}
	case "immoscout24-mobile-search\x00/search/map/v3":
		return []string{"markers"}
	}
	return nil
}

func normalizeSyncItems(resource, path string, items []json.RawMessage) []json.RawMessage {
	if resource != "immoscout24-mobile-search" || path != "/search/map/v3" {
		return items
	}
	...
	for _, raw := range items {
		...
		if _, ok := marker["id"]; !ok {
			if objects, ok := marker["objects"].([]any); ok && len(objects) > 0 {
				if obj, ok := objects[0].(map[string]any); ok {
					if id, ok := obj["id"]; ok {
						marker["id"] = id
					}
				}
