...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "immoscout24-pp-cli",
		Short: `Immoscout24 CLI — Search ImmoScout24 through the replayable mobile JSON API, not brittle blocked HTML.`,
		Long: `Manage immoscout24 resources via the immoscout24 API.
...
Highlights (not in the official API docs):
  ...
  • expose inspect   Extract grouped expose text sections plus deduplicated property image/media URLs and separately detected floorplans.
  • translate-url   Convert between ImmoScout24 web links, mobile API URLs, expose IDs, and CLI commands.

Add --agent to any command for JSON output + non-interactive mode.
Run 'immoscout24-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("immoscout24-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'immoscout24-pp-cli profile list')")
	...
	rootCmd.AddCommand(newImmoscout24MobileSearchCmd(flags))
	...
	rootCmd.AddCommand(newExposePromotedCmd(flags))
	rootCmd.AddCommand(newTranslateURLCmd(flags))
