...
// TestMigrate_AddsColumnsOnUpgrade_Immoscout24MobileSearch verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_Immoscout24MobileSearch(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "immoscout24_mobile_search" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	rows, err := s.DB().Query(`PRAGMA table_info("immoscout24_mobile_search")`)
	if err != nil {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from immoscout24_mobile_search after migrate", want)
		}
	}
}
