...
func TestMCPPathResolutionMatchesCLIResolverWithHomeEnv(t *testing.T) {
	...
	t.Setenv("IMMOSCOUT24_HOME", root)
...
func TestMCPPathResolutionMatchesCLIResolverWithPlatformDefaults(t *testing.T) {
	...
	if want := filepath.Join(home, ".config", "immoscout24-pp-cli", "config.toml"); cfg.Path != want {
	...
	if want := filepath.Join(home, ".local", "share", "immoscout24-pp-cli", "data.db"); gotDB != want {
...
func resetMCPPathEnv(t *testing.T) string {
	...
	for _, name := range []string{
		"IMMOSCOUT24_CONFIG",
		"IMMOSCOUT24_CONFIG_DIR",
		"IMMOSCOUT24_DATA_DIR",
		"IMMOSCOUT24_STATE_DIR",
		"IMMOSCOUT24_CACHE_DIR",
		"IMMOSCOUT24_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
...
func TestMCPRegisterToolsPreservesTypedSpecialTools(t *testing.T) {
	s := server.NewMCPServer("immoscout24", "test")
	RegisterTools(s)
	...
	inspectTool, ok := tools["expose_inspect"]
	if !ok {
		t.Fatalf("expose inspect command mirror missing from registered tools: %#v", tools)
	}
	if !strings.Contains(inspectTool.Tool.Description, "property-facing text sections") {
		t.Fatalf("expose_inspect description = %q", inspectTool.Tool.Description)
	}
}
