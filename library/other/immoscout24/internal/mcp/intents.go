...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("check_install_health",
			mcplib.WithDescription("Run a no-network health check in agent-friendly mode."),
			mcplib.WithBoolean("dry_run", mcplib.Description("Override the recipe's --dry-run value.")),
		),
		handleCheckInstallHealth,
	)
}

// handleCheckInstallHealth runs the check_install_health recipe intent: Run a no-network health check in agent-friendly mode.
func handleCheckInstallHealth(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
