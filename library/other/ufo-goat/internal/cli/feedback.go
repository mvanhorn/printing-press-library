...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".ufo-goat-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("UFO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("UFO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ufo-goat-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.ufo-goat-pp-cli/feedback.jsonl.
When ` + "`UFO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`UFO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ufo-goat-pp-cli feedback list
  ufo-goat-pp-cli feedback list --limit 5
  ufo-goat-pp-cli feedback list --json`,
