...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  ufo-goat-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  ufo-goat-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"agencies", "files"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  ufo-goat-pp-cli workflow status

  # Show status as JSON
  ufo-goat-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("ufo-goat-pp-cli")
			}
