...
Resource scoping:
  ...
  catasto-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  catasto-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  catasto-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  catasto-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  catasto-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/catasto-pp-cli/data.db)")
