...
func TestWhichIndex_CadastralReferenceToGPS(t *testing.T) {
	got := rankWhich(whichIndex, "Comune Foglio Particella Geolocalizzazione GPS", 1)
	if len(got) == 0 {
		t.Fatalf("expected cadastral lookup match, got zero")
	}
	if got[0].Entry.Command != "cadastral" {
		t.Fatalf("top match: want cadastral, got %s", got[0].Entry.Command)
	}
}
