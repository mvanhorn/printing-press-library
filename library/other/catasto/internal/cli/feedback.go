...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".catasto-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CATASTO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CATASTO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "catasto-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.catasto-pp-cli/feedback.jsonl.
When ` + "`CATASTO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CATASTO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  catasto-pp-cli feedback list
  catasto-pp-cli feedback list --limit 5
  catasto-pp-cli feedback list --json`,
