# Catasto Printed CLI Agent Guide

This directory is a generated `catasto-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
catasto-pp-cli doctor --json
catasto-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
catasto-pp-cli which "<capability>" --json
catasto-pp-cli <command> --help
```
...
```bash
catasto-pp-cli <command> --help
catasto-pp-cli <command> --dry-run --agent
```
