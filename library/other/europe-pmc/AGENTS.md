# Europe PMC Printed CLI Agent Guide

This directory is a generated `europe-pmc-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
europe-pmc-pp-cli doctor --json
europe-pmc-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
europe-pmc-pp-cli which "<capability>" --json
europe-pmc-pp-cli <command> --help
```
...
```bash
europe-pmc-pp-cli <command> --agent
```
...
```bash
europe-pmc-pp-cli <command> --help
europe-pmc-pp-cli <command> --dry-run --agent
```
