...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".europe-pmc-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("EUROPE_PMC_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("EUROPE_PMC_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "europe-pmc-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.europe-pmc-pp-cli/feedback.jsonl.
When ` + "`EUROPE_PMC_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`EUROPE_PMC_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  europe-pmc-pp-cli feedback list
  europe-pmc-pp-cli feedback list --limit 5
  europe-pmc-pp-cli feedback list --json`,
