...
// insurance-finder CLI. There is no network auth (this tool talks to no API);
...
const (
	EnvProfile   = "INSURANCE_FINDER_PROFILE"   // path to profile.json
	EnvProviders = "INSURANCE_FINDER_PROVIDERS" // path to providers.json
)

// ProfilePath resolves where the applicant profile lives, preferring an
// explicit flag value, then $INSURANCE_FINDER_PROFILE, then ./profile.json.
...
// $INSURANCE_FINDER_PROVIDERS, else "" (which lets the registry loader fall
