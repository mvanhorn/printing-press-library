...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/other/insurance-finder/internal/config"
...
Typical flow:
  insurance-finder intake          # one-time interview -> profile.json
  insurance-finder match           # ranked shortlist of providers
  insurance-finder guide           # per-provider URL + answer sheet + checklist

Agent mode: add --agent to any command for JSON + non-interactive output.
Health check: insurance-finder doctor`,
		SilenceUsage:  true,
		SilenceErrors: true,
		Version:       version,
	}
	rootCmd.SetVersionTemplate("insurance-finder {{ .Version }}\n")
	...
	pf.StringVar(&f.profilePath, "profile", "", "Path to the applicant profile JSON (default: ./profile.json or $INSURANCE_FINDER_PROFILE)")
