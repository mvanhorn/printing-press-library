...
```bash
./insurance-finder match --json                 # JSON (also the default when piped)
./insurance-finder match --csv                  # CSV for array/table results
./insurance-finder match --plain                # tab-separated values
./insurance-finder match --json --select tier,score
./insurance-finder match --json --compact       # drop verbose fields for fewer tokens
./insurance-finder providers list --quiet       # suppress output; use the exit code
```
...
```bash
insurance-finder --agent match
insurance-finder --agent --profile ./acme.json guide --top 3
```
...
insurance-finder --providers my-providers.json match
```
...
| Symptom | Fix |
|---------|-----|
| `no profile at ...` | Run `insurance-finder intake` first. |
| `no provider with id ...` | Run `insurance-finder providers list` to see valid ids. |
| Color codes in piped output | Color is already off when piped; if forcing `--human-friendly`, drop it or set `NO_COLOR=1`. |
| Registry won't load | Run `insurance-finder doctor`; check your `providers.json` is valid JSON. |
| Profile has PII in git | `profile.json` is gitignored by default — keep it that way. |

## Doctor

```bash
insurance-finder doctor
```
...
```
cmd/insurance-finder-pp-cli/   # main entrypoint
