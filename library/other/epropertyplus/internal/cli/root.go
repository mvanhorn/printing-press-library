...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "epropertyplus-pp-cli",
		Short: `Epropertyplus CLI — Every land bank's public inventory, one command — enumerate, filter, export, and image any ePropertyPlus instance by sl…`,
		Long: `Epropertyplus CLI — Every land bank's public inventory, one command — enumerate, filter, export, and image any ePropertyPlus instance by sl…
...
Health check: run 'epropertyplus-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("epropertyplus-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.instance, "instance", "", "Target ePropertyPlus land-bank slug for this invocation (e.g. --instance kclb); overrides the registry default. See 'instances list'.")
	...
	rootCmd.AddCommand(newInstancesCmd(flags))
	rootCmd.AddCommand(newListCmd(flags))
	rootCmd.AddCommand(newExportCmd(flags))
	rootCmd.AddCommand(newLandExportCmd(flags))
...
func (f *rootFlags) newClient() (*client.Client, error) {
	...
	// Apply the resolved base URL so --instance / env / registry "current"
	// retarget the generated property commands and the novel commands alike.
	// config.Load already applied EPROPERTYPLUS_BASE_URL into cfg.BaseURL; the
	// resolver re-applies the full precedence (flag wins over env-base-url)
	// and is authoritative here.
	cfg.BaseURL = resolveBaseURL(f.instance).BaseURL
...
// activeSlug returns the slug component of the resolved instance for store
// keying. When the base URL comes from EPROPERTYPLUS_BASE_URL (no slug), it
// derives a stable key from the host so per-instance store rows don't collide.
func (f *rootFlags) activeSlug() string {
	res := resolveBaseURL(f.instance)
	if res.Slug != "" {
		return res.Slug
	}
	// env_base_url path: derive a key from the URL so the store stays
