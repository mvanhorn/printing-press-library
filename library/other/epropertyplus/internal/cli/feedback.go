...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "epropertyplus-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("EPROPERTYPLUS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("EPROPERTYPLUS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "epropertyplus-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/epropertyplus-pp-cli/feedback.jsonl.
When ` + "`EPROPERTYPLUS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`EPROPERTYPLUS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  epropertyplus-pp-cli feedback list
  epropertyplus-pp-cli feedback list --limit 5
  epropertyplus-pp-cli feedback list --json`,
