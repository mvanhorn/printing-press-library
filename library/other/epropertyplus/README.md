...
The recommended path installs both the `epropertyplus-pp-cli` binary and the `pp-epropertyplus` agent skill in one shot:

```bash
npx -y @mvanhorn/printing-press install epropertyplus
```
...
```bash
npx skills add mvanhorn/printing-press-library/cli-skills/pp-epropertyplus -g
```

Then invoke `/pp-epropertyplus <query>` in Claude Code. The skill is the most efficient path — Claude Code drives the CLI directly without an MCP server in the middle.
...
```bash
claude mcp add epropertyplus epropertyplus-pp-mcp
```
