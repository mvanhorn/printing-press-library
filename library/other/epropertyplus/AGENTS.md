# ePropertyPlus Printed CLI Agent Guide

This directory is a generated `epropertyplus-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
epropertyplus-pp-cli doctor --json
epropertyplus-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
epropertyplus-pp-cli which "<capability>" --json
epropertyplus-pp-cli <command> --help
```
...
```bash
epropertyplus-pp-cli <command> --agent
```
...
```bash
epropertyplus-pp-cli <command> --help
epropertyplus-pp-cli <command> --dry-run --agent
```
