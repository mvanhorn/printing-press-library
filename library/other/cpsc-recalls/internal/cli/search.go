...
Run sync first to populate the local search index.`,
  ...
  cpsc-recalls-pp-cli search "status"

  # Force local search explicitly
  cpsc-recalls-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  cpsc-recalls-pp-cli search "status" --type cpsc-recall-retrieval-recall --data-source local
  # JSON output for piping
  cpsc-recalls-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'cpsc-recalls-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
