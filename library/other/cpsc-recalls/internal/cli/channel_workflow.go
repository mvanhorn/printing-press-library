...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  cpsc-recalls-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  cpsc-recalls-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"cpsc-recall-retrieval-recall"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  cpsc-recalls-pp-cli workflow status

  # Show status as JSON
  cpsc-recalls-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("cpsc-recalls-pp-cli")
			}
