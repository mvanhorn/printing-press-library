...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CPSC_RECALLS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CPSC_RECALLS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "cpsc-recalls-pp-cli/feedback")
...
When ` + "`CPSC_RECALLS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CPSC_RECALLS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cpsc-recalls-pp-cli feedback list
  cpsc-recalls-pp-cli feedback list --limit 5
  cpsc-recalls-pp-cli feedback list --json`,
