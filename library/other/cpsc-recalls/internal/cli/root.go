...
type rootFlags struct {
	...
	// invocation. Mirrors the CPSC_RECALLS_NO_LEARN env var.
	noLearn       bool
	selectFields  string
	configPath    string
	homePath      string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "cpsc-recalls-pp-cli",
		Short: "Turn CPSC recall records into bounded inventory candidates, change observations, hazard composition tables, and actionable source packets without overstating fuzzy matches.",
		Long: `Cpsc Recalls CLI
...
Health check: run 'cpsc-recalls-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("cpsc-recalls-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'cpsc-recalls-pp-cli profile list')")
	rootCmd.PersistentFlags().StringVar(&flags.deliverSpec, "deliver", "", "Route output to a sink: stdout (default), file:<path>, webhook:<url>")
	rootCmd.PersistentFlags().Float64Var(&flags.rateLimit, "rate-limit", 5, "Max requests per second (0 to disable) (default 5 for CPSC safety)")
	...
	rootCmd.AddCommand(newNovelHazardPulseCmd(flags))
	rootCmd.AddCommand(newNovelInventoryCheckCmd(flags))
	rootCmd.AddCommand(newNovelPacketCmd(flags))
	rootCmd.AddCommand(newNovelWatchCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newCpscRecallRetrievalRecallPromotedCmd(flags))
