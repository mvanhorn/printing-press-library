# Squire Printed CLI Agent Guide

This directory is a generated `squire-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
squire-pp-cli doctor --json
squire-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
squire-pp-cli which "<capability>" --json
squire-pp-cli <command> --help
```
...
```bash
squire-pp-cli <command> --help
squire-pp-cli <command> --dry-run --agent
```
