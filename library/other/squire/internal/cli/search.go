...
In local mode: searches locally synced data only.`,
  ...
  squire-pp-cli search "error timeout"

  # Force local search only
  squire-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  squire-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  squire-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /v1/search/public
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/v1/search/public", map[string]string{
					"term": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'squire-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/squire-pp-cli/data.db)")
