...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "squire-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SQUIRE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SQUIRE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "squire-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/squire-pp-cli/feedback.jsonl.
When ` + "`SQUIRE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SQUIRE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  squire-pp-cli feedback list
  squire-pp-cli feedback list --limit 5
  squire-pp-cli feedback list --json`,
