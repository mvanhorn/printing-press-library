# BSE Filings Printed CLI Agent Guide

This directory is a generated `bse-filings-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
bse-filings-pp-cli doctor --json
bse-filings-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
bse-filings-pp-cli which "<capability>" --json
bse-filings-pp-cli <command> --help
```
...
```bash
bse-filings-pp-cli <command> --agent
```
...
```bash
bse-filings-pp-cli <command> --help
bse-filings-pp-cli <command> --dry-run --agent
```
