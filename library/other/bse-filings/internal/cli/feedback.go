...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".bse-filings-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BSE_FILINGS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BSE_FILINGS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "bse-filings-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.bse-filings-pp-cli/feedback.jsonl.
When ` + "`BSE_FILINGS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BSE_FILINGS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  bse-filings-pp-cli feedback list
  bse-filings-pp-cli feedback list --limit 5
  bse-filings-pp-cli feedback list --json`,
