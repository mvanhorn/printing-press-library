...
In local mode: searches locally synced data only.`,
  ...
  bse-filings-pp-cli search "error timeout"

  # Force local search only
  bse-filings-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  bse-filings-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  bse-filings-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /PeerSmartSearch/w
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get("/PeerSmartSearch/w", map[string]string{
					"text": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'bse-filings-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
