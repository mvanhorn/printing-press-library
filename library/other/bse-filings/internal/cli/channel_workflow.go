...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  bse-filings-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  bse-filings-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("bse-filings-pp-cli")
			}
			...
			// announcements is synced per-holding by the dedicated `sync` command
			// (BSE's announcements feed requires a scrip code; the generic
			// syncResource path has no id and errors). Archive only the resources
			// the generic path can fetch without a scrip.
			resources := []string{"corp_actions", "results_calendar", "results_snapshot", "scrips"}
			totalSynced := 0
			...
			// syncResource streams NDJSON progress events to os.Stdout. Under
			// --json that would interleave with the final summary object and
			// produce invalid JSON, so route the progress stream to stderr (where
			// progress belongs) for the duration of the loop, then restore stdout
			// before emitting the single summary value below.
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/bse-filings-pp-cli/data.db)")
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  bse-filings-pp-cli workflow status

  # Show status as JSON
  bse-filings-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("bse-filings-pp-cli")
			}
