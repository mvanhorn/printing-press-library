...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("batch_approve_pending_transactions",
			mcplib.WithDescription("List everything awaiting approval before approving in bulk."),
			mcplib.WithString("approved", mcplib.Description("Override the recipe's --approved value.")),
		),
		handleBatchApprovePendingTransactions,
	)
}

// handleBatchApprovePendingTransactions runs the batch_approve_pending_transactions recipe intent: List everything awaiting approval before approving in bulk.
func handleBatchApprovePendingTransactions(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	args := []string{}
	args = append(args, "transactions")
	args = append(args, "list")
	var missingApproved bool
	args, missingApproved = appendRecipeStringFlag(args, "approved", input["approved"], "false", false, true)
	if missingApproved {
		return mcplib.NewToolResultError("--approved is required"), nil
	}
