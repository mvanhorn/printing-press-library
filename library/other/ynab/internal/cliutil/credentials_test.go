...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/other/ynab/internal/cliutil/testenv"
...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"YNAB_API_TOKEN",
		"YNAB_ENDPOINTS_TOKEN",
	} {
		t.Setenv(name, "")
	}
	return home, filepath.Join(home, ".config", "ynab-pp-cli", "config.toml")
}
...
func TestExplicitConfigUsesSiblingCredentialsBeforeGlobal(t *testing.T) {
	...
	t.Setenv("YNAB_CONFIG", configPath)
...
func TestCorruptCredentialsDoesNotOverrideLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "ynab-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	home, _ := resetCredentialEnv(t)
	t.Setenv("YNAB_API_TOKEN", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "ynab-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "ynab-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "ynab-pp-cli", "credentials.toml")
