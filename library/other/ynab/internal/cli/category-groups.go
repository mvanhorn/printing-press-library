...
func newCategoryGroupsCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "category-groups",
		Short:       "Create and update category groups",
		Annotations: map[string]string{"mcp:read-only": "true", "pp:parent-group": "true", "pp:api-resource": "true", "pp:typed-exit-codes": "0,2"},
		RunE:        parentNoSubcommandRunE(flags),
	}

	cmd.AddCommand(newCategoryGroupsCreateCmd(flags))
	cmd.AddCommand(newCategoryGroupsUpdateCmd(flags))
	return cmd
}
