...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			"\n      Set one of: export YNAB_API_TOKEN=\"your-token-here\" or export YNAB_ENDPOINTS_TOKEN=\"your-token-here\""+
			"\n      Get a key at: https://api.ynab.com"+
			"\n      Run 'ynab-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token."+
			"\n      Set one of: export YNAB_API_TOKEN=\"your-token-here\" or export YNAB_ENDPOINTS_TOKEN=\"your-token-here\""+
			"\n      Get a key at: https://api.ynab.com"+
			"\n      Run 'ynab-pp-cli doctor' to check auth status.", err))
...
// unwrapYNABEnvelope strips YNAB's own top-level {"data": {...}} wrapper
// before this CLI builds its own "results" envelope around the response.
// unwrapSingleKeyArray (below) only flattens a single-key wrapper whose
// value is itself an array; YNAB's "data" key almost always wraps an
// object instead (e.g. {"data": {"accounts": [...]}}, {"data": {"user":
// {...}}}), which unwrapSingleKeyArray intentionally leaves alone. Without
// this step every generated read/write command emitted results.data.<field>
// instead of results.<field>. Scoped to exactly the "data" key so unrelated
// single-key object responses still pass through unchanged. Local-cache
// writes happen against the original, still-wrapped bytes before this runs,
// so store/offline reads are unaffected.
func unwrapYNABEnvelope(data json.RawMessage) json.RawMessage {
...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Set one of: export YNAB_API_TOKEN=\"your-token-here\" or export YNAB_ENDPOINTS_TOKEN=\"your-token-here\""))
	}
	if len(trimmed) == 0 {
...
// {"results": ..., "meta": {...}}. YNAB's own {"data": {...}} envelope is
// stripped first (see unwrapYNABEnvelope), then single-key array envelopes
// from the API (e.g. {"results": [...]}, {"data": [...]}) are unwrapped so
// the output shape is the same regardless of the API's wrapper key.
func wrapWithProvenance(data json.RawMessage, prov DataProvenance) (json.RawMessage, error) {
	...
	var results any
	if json.Valid(data) {
		results = json.RawMessage(unwrapSingleKeyArray(unwrapYNABEnvelope(data)))
	} else {
