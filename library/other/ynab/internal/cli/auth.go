...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "auth",
		Short:       "Manage authentication for Ynab Endpoints",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  ynab-pp-cli auth setup\n  ynab-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://api.ynab.com")
			fmt.Fprintln(w, "  ynab-pp-cli auth set-token <token>")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Set one of:")
			fmt.Fprintln(w, "  export YNAB_API_TOKEN=\"your-token-here\"")
			fmt.Fprintln(w, "  export YNAB_ENDPOINTS_TOKEN=\"your-token-here\"")
			if !launch {
				return nil
			}
			launchURL := "https://api.ynab.com"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  ynab-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "")
				fmt.Fprintln(w, "Set one of:")
				fmt.Fprintf(w, "  ynab-pp-cli auth set-token <token>\n")
				fmt.Fprintln(w, "")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  ynab-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  ynab-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("YNAB_API_TOKEN") != "" {
				envStillSet = "YNAB_API_TOKEN"
			}
			if envStillSet == "" && os.Getenv("YNAB_ENDPOINTS_TOKEN") != "" {
				envStillSet = "YNAB_ENDPOINTS_TOKEN"
			}
