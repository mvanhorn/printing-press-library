...
type rootFlags struct {
	...
	// invocation. Mirrors the YNAB_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "ynab-pp-cli",
		Short: `Ynab CLI — Every YNAB feature, plus a local mirror that makes reconciliation, payee trend analysis, and a ready-made…`,
		Long: `Ynab CLI — Every YNAB feature, plus a local mirror that makes reconciliation, payee trend analysis, and a ready-made ProjectionLab export instant instead of manual.
...
Health check: run 'ynab-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("ynab-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'ynab-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCategoryGroupsCmd(flags))
	rootCmd.AddCommand(newMonthsCmd(flags))
	rootCmd.AddCommand(newPayeeLocationsCmd(flags))
	rootCmd.AddCommand(newPayeesCmd(flags))
	rootCmd.AddCommand(newPlansCmd(flags))
	rootCmd.AddCommand(newScheduledTransactionsCmd(flags))
	...
	rootCmd.AddCommand(newMoneyMovementGroupsPromotedCmd(flags))
	rootCmd.AddCommand(newMoneyMovementsPromotedCmd(flags))
