...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("YNAB_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("YNAB_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ynab-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  ynab-pp-cli feedback \"the --select flag saved me a ton of tokens here\"",
		Long: `Feedback is captured locally first in the CLI data directory's feedback.jsonl.
When ` + "`YNAB_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`YNAB_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		// Free-text capture: any string is valid input, so there is no
		// "invalid argument" case to reject.
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ynab-pp-cli feedback list
  ynab-pp-cli feedback list --limit 5
  ynab-pp-cli feedback list --json`,
