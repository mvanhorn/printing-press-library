...
type Config struct {
	...
	YnabApiToken       string          `toml:"api_token"`
	YnabEndpointsToken string          `toml:"endpoints_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("YNAB_API_TOKEN"); v != "" {
		cfg.YnabApiToken = v
		cfg.markEnvOverride("YnabApiToken")
		cfg.AuthSource = "env:YNAB_API_TOKEN"
		cfg.CredentialSource = "env:YNAB_API_TOKEN"
	}
	if v := os.Getenv("YNAB_ENDPOINTS_TOKEN"); v != "" {
		cfg.YnabEndpointsToken = v
		cfg.markEnvOverride("YnabEndpointsToken")
		cfg.AuthSource = "env:YNAB_ENDPOINTS_TOKEN"
		cfg.CredentialSource = "env:YNAB_ENDPOINTS_TOKEN"
	}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("YNAB_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "ynab-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.YnabApiToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:YNAB_API_TOKEN"
		}
		return "Bearer " + c.YnabApiToken
	}
	if c.YnabEndpointsToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:YNAB_ENDPOINTS_TOKEN"
		}
		return "Bearer " + c.YnabEndpointsToken
	}
	if c.AccessToken != "" {
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		YnabApiToken:       c.YnabApiToken,
		YnabEndpointsToken: c.YnabEndpointsToken,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.YnabApiToken == "" {
		c.YnabApiToken = creds.YnabApiToken
	}
	if c.YnabEndpointsToken == "" {
		c.YnabEndpointsToken = creds.YnabEndpointsToken
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.YnabApiToken = ""
	delete(c.envOverrides, "YnabApiToken")
	c.updateFileConfigField("YnabApiToken")
	c.YnabEndpointsToken = ""
	delete(c.envOverrides, "YnabEndpointsToken")
	c.updateFileConfigField("YnabEndpointsToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["YnabApiToken"] {
			out.YnabApiToken = c.fileConfig.YnabApiToken
		}
		if c.envOverrides["YnabEndpointsToken"] {
			out.YnabEndpointsToken = c.fileConfig.YnabEndpointsToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "YnabApiToken":
		c.fileConfig.YnabApiToken = c.YnabApiToken
	case "YnabEndpointsToken":
		c.fileConfig.YnabEndpointsToken = c.YnabEndpointsToken
	}
}
