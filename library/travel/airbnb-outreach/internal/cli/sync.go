...
Resource scoping:
  ...
  airbnb-outreach-pp-cli sync

  # Sync specific resources only
  airbnb-outreach-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  airbnb-outreach-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  airbnb-outreach-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  airbnb-outreach-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  airbnb-outreach-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/airbnb-outreach-pp-cli/data.db)")
