...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AIRBNB_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AIRBNB_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "airbnb-outreach-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.airbnb-outreach-pp-cli/feedback.jsonl.
When ` + "`AIRBNB_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AIRBNB_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  airbnb-outreach-pp-cli feedback list
  airbnb-outreach-pp-cli feedback list --limit 5
  airbnb-outreach-pp-cli feedback list --json`,
