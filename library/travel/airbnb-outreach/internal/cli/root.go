...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "airbnb-outreach-pp-cli",
		Short: "Search Airbnb, contact hosts, and archive conversations from the CLI",
		Long: `Script Airbnb's real logged-in surface: search stays, read and send messages,
contact hosts with templated messages and photos, and keep an offline archive of
every conversation. Airbnb has no public API — this drives its internal GraphQL
endpoints and authenticates by importing your logged-in Chrome session (no
password).

Start with 'airbnb-outreach-pp-cli auth login --chrome', then 'me' and 'search'.
...
Add --agent to any command for JSON output + non-interactive mode.`,
	...
	rootCmd.SetVersionTemplate("airbnb-outreach-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'airbnb-outreach-pp-cli profile list')")
	...
	// Airbnb command surface (hand-built against internal/airbnb).
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("airbnb-outreach-pp-cli %s\n", version)
		},
	}
}
