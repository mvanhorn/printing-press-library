# Airbnb Printed CLI Agent Guide

This directory is a generated `airbnb-outreach-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
airbnb-outreach-pp-cli doctor --json
airbnb-outreach-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
airbnb-outreach-pp-cli which "<capability>" --json
airbnb-outreach-pp-cli <command> --help
```
...
```bash
airbnb-outreach-pp-cli <command> --agent
```
...
```bash
airbnb-outreach-pp-cli <command> --help
airbnb-outreach-pp-cli <command> --dry-run --agent
```
