...
func TestRecipeIntentHandlerBuildsRecipeArgs(t *testing.T) {
	...
	req := mcplib.CallToolRequest{Params: mcplib.CallToolParams{Arguments: map[string]any{
		"pickup":  "required-value",
		"dropoff": "required-value",
	}}}
	result, err := handleConfirmTheAggregatorPriceDirectly(context.Background(), req)
	if err != nil {
...
func TestRecipeIntentHandlerOverridesParams(t *testing.T) {
	...
	if !strings.Contains(got, "required-value") {
		t.Fatalf("positional override for pickup missing from %q", got)
	}
	if !strings.Contains(got, "required-value") {
		t.Fatalf("positional override for dropoff missing from %q", got)
	}
}
