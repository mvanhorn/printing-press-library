...
func selftestErr(err error) error  { return &cliError{code: 6, err: err} }
...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	// Type-based throttling detection: a source client that hit HTTP 429/503
	// returns a *cliutil.RateLimitError. Map it to the rate-limit exit code
	// (7) directly, rather than relying on substring matching the message.
	...
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API credentials."+
			"\n      Run 'rentalcarspain-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: permission denied. This API is configured without credentials, so the service may be blocking the request by rate limit, geography, bot protection, or endpoint policy."+
			"\n      Run 'rentalcarspain-pp-cli doctor' to check connectivity.", err))
