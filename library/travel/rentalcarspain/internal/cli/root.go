...
type rootFlags struct {
	...
	// invocation. Mirrors the RENTALCARSPAIN_NO_LEARN env var.
	noLearn bool
	// disabledSuppliers is a comma-separated list of supplier ids the user has
	// opted out of contacting (--disable-supplier), so anyone uneasy about a
	// given supplier's terms can skip it. Matched case-insensitively.
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "rentalcarspain-pp-cli",
		Short: `Find your best rental car, fully insured, at any airport in Spain — zero-excess prices across DoYouSpain, Rentalcars and seven direct suppliers, ranked by real per-airport reviews.`,
		Long: `Find your best rental car, fully insured, at any airport in Spain — zero-excess prices across DoYouSpain, Rentalcars and seven direct suppliers, ranked by real per-airport reviews.

Highlights (not in the official API docs):
  • compare   Compare DoYouSpain's cheapest offer for a supplier against that supplier's own booking site, side by side.
  • drift   Show how a saved search's cheapest full-insurance price has moved across your local snapshots.
  ...
  • saved   Name and store a recurring search so watch and drift can re-run it.
...
Health check: run 'rentalcarspain-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("rentalcarspain-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.currency, "currency", "", "Also show prices converted to this currency (e.g. USD, GBP) — indicative only; rentals are billed in EUR")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'rentalcarspain-pp-cli profile list')")
	rootCmd.PersistentFlags().StringVar(&flags.deliverSpec, "deliver", "", "Route output to a sink: stdout (default), file:<path>, webhook:<url>")
	rootCmd.PersistentFlags().Float64Var(&flags.rateLimit, "rate-limit", 0, "Max requests per second (0 to disable)")
	rootCmd.PersistentFlags().StringVar(&flags.disabledSuppliers, "disable-supplier", "", "Comma-separated suppliers to skip contacting (e.g. doyouspain,rentalcars,goldcar). See README on supplier terms of service.")
	...
	rootCmd.AddCommand(newDelpasoCmd(flags))
	rootCmd.AddCommand(newDirectCmd(flags))
	rootCmd.AddCommand(newBestCmd(flags))
	rootCmd.AddCommand(newRecommendCmd(flags))
	rootCmd.AddCommand(newSelftestCmd(flags))
	rootCmd.AddCommand(newQueryMatrixCmd(flags))
	rootCmd.AddCommand(newNovelCompareCmd(flags))
	rootCmd.AddCommand(newNovelDatesCmd(flags))
	rootCmd.AddCommand(newNovelDriftCmd(flags))
	rootCmd.AddCommand(newNovelSavedCmd(flags))
	rootCmd.AddCommand(newNovelSuppliersCmd(flags))
