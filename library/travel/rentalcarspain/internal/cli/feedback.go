...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("RENTALCARSPAIN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("RENTALCARSPAIN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "rentalcarspain-pp-cli/feedback")
...
When ` + "`RENTALCARSPAIN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`RENTALCARSPAIN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  rentalcarspain-pp-cli feedback list
  rentalcarspain-pp-cli feedback list --limit 5
  rentalcarspain-pp-cli feedback list --json`,
