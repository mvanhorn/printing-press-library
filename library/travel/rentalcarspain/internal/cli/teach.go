...
// rentalcarspain-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/travel/rentalcarspain/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/travel/rentalcarspain/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/travel/rentalcarspain/internal/learn/patterns"
...
const noLearnEnvVar = "RENTALCARSPAIN_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("rentalcarspain-pp-cli")
}
...
func newTeachCmd(flags *rootFlags, learnCfg *entities.Config) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Annotations: map[string]string{"pp:typed-exit-codes": "0,2", "mcp:local-write": "true", "pp:happy-args": "--query=cheap malaga cars;--resource-type=offers;--resource=MAL02"},
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  rentalcarspain-pp-cli teach --query "cheap malaga cars" --resource-type offers --resource MAL02`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// An explicit --json is a request for machine output, so honor it
			// even under --quiet (which only suppresses the human success line).
			if flags.asJSON {
...
when learnings exist.`,
		Example: `  rentalcarspain-pp-cli recall "<question>" --agent
  rentalcarspain-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  rentalcarspain-pp-cli learnings list --agent
  rentalcarspain-pp-cli learnings list --query "<substring>"
  rentalcarspain-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  rentalcarspain-pp-cli learnings forget "<question>" --resource <id>
  rentalcarspain-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  rentalcarspain-pp-cli teach-pattern \
    --query-template "items in {entity}" \
    --resource-template "GROUP-{entity:category}" \
    --resource-type "items" \
    --entity-kind "category" \
    --strategy substitute`,
		Annotations: map[string]string{"pp:happy-args": "--query-template=cheap {entity} cars;--resource-template=GROUP-{entity:location};--resource-type=offers;--entity-kind=location"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  rentalcarspain-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
