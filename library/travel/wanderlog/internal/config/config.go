...
type Config struct {
	...
	WanderlogCookie string            `toml:"cookie"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("WANDERLOG_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "wanderlog-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("WANDERLOG_COOKIE"); v != "" {
		cfg.WanderlogCookie = v
		cfg.markEnvOverride("WanderlogCookie")
		cfg.AuthSource = "env:WANDERLOG_COOKIE"
	}
	...
	if cfg.AuthSource == "" && cfg.WanderlogCookie != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("WANDERLOG_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.WanderlogCookie
	if token == "" {
		return ""
	}
	if c.WanderlogCookie == "" {
		return ""
	}
	replacements := map[string]string{
		"token":            token,
		"cookie":           c.WanderlogCookie,
		"WANDERLOG_COOKIE": c.WanderlogCookie,
	}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.WanderlogCookie = token
	delete(c.envOverrides, "WanderlogCookie")
	c.updateFileConfigField("WanderlogCookie")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["WanderlogCookie"] {
			out.WanderlogCookie = c.fileConfig.WanderlogCookie
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "WanderlogCookie":
		c.fileConfig.WanderlogCookie = c.WanderlogCookie
	}
}
