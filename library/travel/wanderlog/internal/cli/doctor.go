...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  wanderlog-pp-cli doctor
  wanderlog-pp-cli doctor --json
  wanderlog-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export WANDERLOG_COOKIE=<your-key>"
					report["auth_docs_url"] = "https://wanderlog.com"
				} else {
					authConfigured = true
			...
			if os.Getenv("WANDERLOG_COOKIE") != "" {
				authEnvSet = append(authEnvSet, "WANDERLOG_COOKIE")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "WANDERLOG_COOKIE")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "Mozilla/5.0 (compatible; wanderlog-pp-cli/0.1; +https://wanderlog.com)"
						authHeaders["Accept-Language"] = "en"
						verifyPath := "/api/user"
			...
			applyDoctorInstallPath(report, exe, os.Getenv("PATH"), home, os.Getenv("GOPATH"))
			...
			for _, key := range []string{"config_path", "base_url", "auth_source", "version", "binary", "binary_dir"} {
				if v, ok := report[key]; ok {
					fmt.Fprintf(w, "  %s: %v\n", key, v)
				}
			}
			if hint, ok := report["path_hint"]; ok {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("wanderlog-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'wanderlog-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'wanderlog-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'wanderlog-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'wanderlog-pp-cli sync' to refresh."
	}
	return report
}
...
// applyDoctorInstallPath records binary name and a PATH warning when the
// executable lives in ~/.local/bin or GOPATH/bin but that directory is not
// on PATH.
func applyDoctorInstallPath(report map[string]any, exePath, pathEnv, homeDir, goPath string) {
	info := doctorInstallPathReport(exePath, pathEnv, homeDir, goPath)
	report["binary"] = info["binary"]
	report["binary_note"] = info["binary_note"]
	if dir, ok := info["binary_dir"]; ok {
		report["binary_dir"] = dir
	}
	report["path"] = info["path"]
	if hint, ok := info["path_hint"]; ok {
		report["path_hint"] = hint
	}
}

// doctorInstallPathReport is the unit-testable PATH / install-dir check.
//
// PATCH(amend-2026-08-23: warn when ~/.local/bin or GOPATH/bin is off PATH)
func doctorInstallPathReport(exePath, pathEnv, homeDir, goPath string) map[string]any {
	out := map[string]any{
		"binary":      "wanderlog-pp-cli",
		"binary_note": "the binary is wanderlog-pp-cli, not wanderlog",
		"path":        "ok",
	}
	...
	localBin := filepath.Clean(filepath.Join(homeDir, ".local", "bin"))
	goBin := filepath.Clean(filepath.Join(goPath, "bin"))
	if dir != localBin && dir != goBin {
		return out
	}
	if pathEnvContainsDir(pathEnv, dir, homeDir) {
		return out
	}
	out["path"] = fmt.Sprintf("WARN %s is not on PATH", dir)
	out["path_hint"] = fmt.Sprintf("export PATH=\"%s:$PATH\" — invoke wanderlog-pp-cli, not wanderlog", dir)
	return out
}

func pathEnvContainsDir(pathEnv, dir, homeDir string) bool {
	...
	for _, entry := range strings.Split(pathEnv, string(os.PathListSeparator)) {
		...
		if entry == "~" {
			...
			entry = filepath.Join(homeDir, strings.TrimPrefix(entry, "~/"))
		}
