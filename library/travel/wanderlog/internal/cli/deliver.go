...
// PATCH(amend-2026-08-23: file: without stdout unless --also-stdout; webhook keeps MultiWriter)
func deliverOutputWriter(sink DeliverSink, alsoStdout bool, stdout io.Writer, buf *bytes.Buffer) io.Writer {
	if sink.Scheme == "file" && !alsoStdout {
		return buf
	}
	return io.MultiWriter(stdout, buf)
}

// Deliver routes a captured output buffer to the configured sink. stdout
// is a no-op. For --deliver file:<path>, PersistentPreRunE captures into
// the buffer only (no stdout) unless --also-stdout is set. Webhook sinks
// still capture and default to mirroring stdout; --also-stdout keeps that
// MultiWriter path.
...
func deliverWebhook(url string, body []byte, compact bool) error {
	...
	req.Header.Set("User-Agent", "wanderlog-pp-cli/deliver")
