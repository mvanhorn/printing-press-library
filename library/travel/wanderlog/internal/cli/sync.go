...
Resource scoping:
  ...
  wanderlog-pp-cli sync

  # Sync specific resources only
  wanderlog-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  wanderlog-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  wanderlog-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  wanderlog-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  wanderlog-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "wanderlog-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/wanderlog-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "geos":
		...
		return db.UpsertGeos(typedData)
	case "guides":
		typedData, err := typedSingleObjectData(data, obj, id, forceTypedID)
		if err != nil {
			return err
		}
		return db.UpsertGuides(typedData)
	case "places":
		typedData, err := typedSingleObjectData(data, obj, id, forceTypedID)
		if err != nil {
			return err
		}
		return db.UpsertPlaces(typedData)
	case "trips":
		typedData, err := typedSingleObjectData(data, obj, id, forceTypedID)
		if err != nil {
			return err
		}
		return db.UpsertTrips(typedData)
	default:
