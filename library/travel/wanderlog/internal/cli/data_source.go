...
func detectUpstreamFailure(data json.RawMessage) error {
	var obj map[string]json.RawMessage
	if err := json.Unmarshal(data, &obj); err != nil {
		return nil
	}
	rawSuccess, ok := obj["success"]
	if !ok {
		return nil
	}
	var success bool
	...
	message := "upstream API reported failure"
	if raw := obj["error"]; len(raw) > 0 {
		var s string
	...
	if raw := obj["messages"]; len(raw) > 0 {
		var messages []string
		if json.Unmarshal(raw, &messages) == nil && len(messages) > 0 && strings.TrimSpace(messages[0]) != "" {
			message = messages[0]
		}
	}
...
func checkedLiveRead(ctx context.Context, c *client.Client, path string, params map[string]string, headers map[string]string) (json.RawMessage, error) {
	data, err := c.GetWithHeaders(ctx, path, params, headers)
	if err != nil {
		return nil, err
	}
	if err := detectUpstreamFailure(data); err != nil {
		return nil, err
	}
	return data, nil
}
...
func resolveReadWithStrategy(ctx context.Context, c *client.Client, flags *rootFlags, strategy string, resourceType string, isList bool, path string, params map[string]string, headers map[string]string, hintWriter io.Writer) (json.RawMessage, DataProvenance, error) {
	...
	if strategy == "live" {
		data, err := checkedLiveRead(ctx, c, path, params, headers)
		if err != nil {
			return nil, DataProvenance{}, err
		}
	...
	default: // "auto"
		...
		if fallbackErr != nil {
			return nil, DataProvenance{}, fmt.Errorf("API unreachable and no local data. Run 'wanderlog-pp-cli sync' to enable offline access.\n\nOriginal error: %w", err)
		}
...
func writeThroughCache(ctx context.Context, resourceType string, data json.RawMessage) {
	db, err := store.OpenWithContext(ctx, defaultDBPath("wanderlog-pp-cli"))
	if err != nil {
		return
	}
	defer db.Close()
...
func resolveLocal(ctx context.Context, flags *rootFlags, hintWriter io.Writer, resourceType string, isList bool, path string, params map[string]string, reason string) (json.RawMessage, DataProvenance, error) {
	db, err := openStoreForRead(ctx, "wanderlog-pp-cli")
	if err != nil {
		return nil, DataProvenance{}, fmt.Errorf("opening local database: %w\nRun 'wanderlog-pp-cli sync' first.", err)
	}
	if db == nil {
		return nil, DataProvenance{}, fmt.Errorf("no local data. Run 'wanderlog-pp-cli sync' first")
	}
	defer db.Close()
	...
	if isList {
		...
		if len(items) == 0 {
			return nil, DataProvenance{}, fmt.Errorf("no local data for %q. Run 'wanderlog-pp-cli sync' first", resourceType)
		}
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, DataProvenance{}, fmt.Errorf("resource %q with ID %q not found in local store. Run 'wanderlog-pp-cli sync' first", resourceType, id)
		}
		return nil, DataProvenance{}, fmt.Errorf("querying local store: %w", err)
	}
