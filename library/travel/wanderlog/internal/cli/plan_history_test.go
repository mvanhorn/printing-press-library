...
func TestPickJournalRecordEmptyPlan(t *testing.T) {
	_, _, err := pickJournalRecord(nil, "omxsrbpstldoniqa", "", "undo")
	if err == nil || !strings.Contains(err.Error(), "no applied journal entry") {
		t.Fatalf("empty undo: %v", err)
	}
	_, _, err = pickJournalRecord([]planEditJournalRecord{{TargetKey: "omxsrbpstldoniqa", Status: "applied"}}, "omxsrbpstldoniqa", "", "redo")
