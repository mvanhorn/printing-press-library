...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "wanderlog-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("WANDERLOG_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("WANDERLOG_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "wanderlog-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/wanderlog-pp-cli/feedback.jsonl.
When ` + "`WANDERLOG_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`WANDERLOG_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  wanderlog-pp-cli feedback list
  wanderlog-pp-cli feedback list --limit 5
  wanderlog-pp-cli feedback list --json`,
