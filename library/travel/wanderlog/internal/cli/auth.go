...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Wanderlog (there is no auth login; use setup and set-token)",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:     "setup",
		Short:   "Print how to copy connect.sid and save it with auth set-token (no auth login)",
		Example: "  wanderlog-pp-cli auth setup\n  wanderlog-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			// PATCH(amend-2026-08-23: connect.sid recipe; never paste cookies; no auth login)
			fmt.Fprintln(w, "There is no auth login command. Save a browser cookie locally.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "1. Open https://wanderlog.com and log in.")
			fmt.Fprintln(w, "2. DevTools → Application → Cookies → wanderlog.com → copy the connect.sid value.")
			fmt.Fprintln(w, "   Do not paste the cookie into chat. Keep it in your terminal only.")
			fmt.Fprintln(w, "3. Save it in Cookie-header form (connect.sid=<value>) with set-token's positional argument:")
			fmt.Fprintln(w, "     wanderlog-pp-cli auth set-token 'connect.sid=<value>'")
			fmt.Fprintln(w, "   or:")
			fmt.Fprintln(w, "     export WANDERLOG_COOKIE='connect.sid=<value>'")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then run: wanderlog-pp-cli auth status")
			if !launch {
				return nil
			}
			launchURL := "https://wanderlog.com"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  wanderlog-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.asJSON {
				...
				if authed {
					out["note"] = authVerifiedFalseNote
				}
			...
			if !authed {
				...
				fmt.Fprintln(w, "")
				fmt.Fprintln(w, "There is no auth login. Run: wanderlog-pp-cli auth setup")
				fmt.Fprintln(w, "Then: wanderlog-pp-cli auth set-token 'connect.sid=<value>'")
				return authErr(fmt.Errorf("no credentials configured"))
			}
			...
			fmt.Fprintf(w, "  %s\n", authVerifiedFalseNote)
			return nil
		},
	}
}

// authVerifiedFalseNote explains status.verified when a cookie is present.
// PATCH(amend-2026-08-23: verified:false is optional; config cookie is enough)
const authVerifiedFalseNote = "verified:false is an optional extra check; a config cookie is enough for API calls"

func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		Use:     "set-token <token>",
		Short:   "Save an API token to the config file",
		Example: "  wanderlog-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  wanderlog-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("WANDERLOG_COOKIE") != "" {
				envStillSet = "WANDERLOG_COOKIE"
			}
