...
- User-provided Okinawa shared plan: `https://wanderlog.com/plan/omxsrbpstldoniqa/trip-to-okinawa-prefecture/shared`, verified via `/api/tripPlans/omxsrbpstldoniqa?clientSchemaVersion=2`.
...
| 4 | Get public guide/shared plan by key | wanderlog-mcp get_guide + live probes | (generated endpoint) guides get | Fetches full tripPlan/resources with clientSchemaVersion=2, including user-provided Okinawa plan |
...
| 21 | Preview copyable shared plan contents | User priority + Okinawa probe | wanderlog-pp-cli plan preview | Shows sections, dates, blocks, resources, and clone warnings before mutation |
...
| 3 | Clone preview and copyability report | `plan preview --source-url <url>` | 8/10 | hand-code | Reads the source plan and emits title, dates, section counts, block counts, resource coverage, and clone warnings without credentials or mutations. | Required safeguard for the user-provided Okinawa example and for dry-run dogfood when no authenticated fixture is available. | Use this command to inspect what a shared plan contains before cloning/filling. Do NOT use it to write a trip; use `plan clone` or `plan fill` with `--apply`. |
