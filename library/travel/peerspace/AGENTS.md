# Peerspace Printed CLI Agent Guide

This directory is a generated `peerspace-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
peerspace-pp-cli doctor --json
peerspace-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
peerspace-pp-cli which "<capability>" --json
peerspace-pp-cli <command> --help
```
...
```bash
peerspace-pp-cli <command> --agent
```
...
```bash
peerspace-pp-cli <command> --help
peerspace-pp-cli <command> --dry-run --agent
```
...
3. After answering, always fire `peerspace-pp-cli teach --query "<question>" --resource <id> --resource-type <type> &` in the background - teaching is unconditional and is the anchor that triggers playbook synthesis. Teach the structural question with identifiers stripped (no names, emails, phone numbers, account ids); the CLI warns on obvious PII shapes but does not block.
...
Disable the loop with `--no-learn` per-invocation or `PEERSPACE_NO_LEARN=true` for the whole session - useful for deterministic agent flows that don't want a learning row to silently change subsequent query results.

For install, auth, examples, and longer product guidance, read `README.md` and `SKILL.md`. This file intentionally stays small so repo-local agents get invariant local guidance without duplicating the generated docs.

## Guest planning flow (this CLI)

Typical agent choreography for tech-event venue scouting:

1. `venues list` (search cards; light)
2. `shortlist create-board` / `shortlist add` (cookie + PSAccess; body `project` = board id string)
3. `shortlist hydrate` (opt-in full page blocks into SQLite — not during search)
4. `calendar availability-*` then `contracts inquiry-quote` / `inquiry-send` (**`--yes` required on send**; contacts hosts)
5. `shortlist export --collaborator-id … --board-id …` (richer after hydrate)

Do not auto-hydrate every search hit. Do not send host inquiries without explicit user approval. See `CHANGELOG.md` for the 2026-07-16 local amend surface.
