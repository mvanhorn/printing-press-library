...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PEERSPACE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PEERSPACE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "peerspace-pp-cli/feedback")
...
When ` + "`PEERSPACE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PEERSPACE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  peerspace-pp-cli feedback list
  peerspace-pp-cli feedback list --limit 5
  peerspace-pp-cli feedback list --json`,
