...
type rootFlags struct {
	...
	// invocation. Mirrors the PEERSPACE_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "peerspace-pp-cli",
		Short: `Tech-event venue scout for Peerspace: search, shortlist, hydrate full listing details, check availability, message hosts, and export proposal packs.`,
		Long: `Tech-event venue scout for Peerspace: search, shortlist, hydrate full listing details, check availability, message hosts, and export proposal packs.
...
Highlights (not in the official API docs):
  • shortlist hydrate   Fetch full listing page details (about, rules, parking, amenities) into SQLite after shortlisting.
  • venues get   Live full listing detail (GET /v1/listings/{id}) with write-through cache.
  • contracts inquiry-send   Message a listing host with dates, guest count, and a custom note (requires --yes).
  • contracts inquiry-quote   Price a host-inquiry form without submitting.
  • contracts guest-quote   Price a request-to-book window (prepare-only quote).
  • calendar availability-start/end/month   Day and month availability for a space.
  • verification listing   Check if guest ID verification is required before booking.
  • shortlist create-board / add   Cookie-auth favorite board writes.
  • shortlist compare / export   Offline shortlist table + markdown with About/Rules/Parking after hydrate.
  • scout budget / capacity / gaps / multi-city   Market and fit intelligence on synced venues.
  • markets pulse / neighborhoods   Cross-city and neighborhood rollups.
  • messages unread   Inbox unread count (v2).
  • spaces faqs-event   Event FAQs for a space.
  • pulse   Cookie-auth inbox + favorites + profile snapshot.

Agent mode: add --agent to any command for JSON output + non-interactive mode.
Health check: run 'peerspace-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("peerspace-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'peerspace-pp-cli profile list')")
	...
	rootCmd.AddCommand(newVerificationCmd(flags))
	...
	rootCmd.AddCommand(newNovelMarketsCmd(flags))
	rootCmd.AddCommand(newNovelPulseCmd(flags))
	rootCmd.AddCommand(newNovelScoutCmd(flags))
