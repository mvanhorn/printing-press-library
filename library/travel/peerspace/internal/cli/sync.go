...
Resource scoping:
  ...
  peerspace-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  peerspace-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  peerspace-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  peerspace-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  peerspace-pp-cli sync --latest-only`,
	...
	// Novel subcommand: offline coverage report over local store metadata.
