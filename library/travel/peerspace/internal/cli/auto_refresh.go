...
var readCommandResources = map[string][]string{
	"peerspace-pp-cli venues":        {"venues"},
	"peerspace-pp-cli venues list":   {"venues"},
	"peerspace-pp-cli venues get":    {"venues"},
	"peerspace-pp-cli venues search": {"venues"},
}
...
func cachePolicy() cliutil.Policy {
	...
	envOptOut := "PEERSPACE_NO_AUTO_REFRESH"
...
func autoRefreshIfStale(ctx context.Context, flags *rootFlags, resources []string) (meta cliutil.FreshnessMeta) {
	...
	if err := runAutoRefresh(refreshCtx, flags, db, resources); err != nil {
		fmt.Fprintf(os.Stderr, "warning: using stale peerspace-pp-cli cache (refresh failed: %v)\n", err)
