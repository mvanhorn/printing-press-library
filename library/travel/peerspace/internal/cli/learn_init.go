...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"activity": {
			{Canonical: "meetup", Values: []string{"meeting", "meetings", "reunion", "réunion"}},
			{Canonical: "party", Values: []string{"fete", "fête", "celebration", "soirée", "soiree"}},
			{Canonical: "film_shoot", Values: []string{"film", "photo", "production", "shoot", "video"}},
			{Canonical: "wedding", Values: []string{"mariage", "ceremony"}},
			{Canonical: "workshop", Values: []string{"atelier", "class", "training"}},
		},
		"city": {
			{Canonical: "Paris", Values: []string{"paris", "paris france", "idf"}},
			{Canonical: "Los Angeles", Values: []string{"la", "los angeles", "l.a."}},
			{Canonical: "New York", Values: []string{"nyc", "new york city", "brooklyn"}},
			{Canonical: "San Francisco", Values: []string{"sf", "bay area"}},
			{Canonical: "London", Values: []string{"ldn", "london uk"}},
		},
		"space_type": {
			{Canonical: "daylight_studio", Values: []string{"studio", "daylight studio", "photo studio"}},
			{Canonical: "film_photo_loft", Values: []string{"loft", "photo loft"}},
			{Canonical: "apartment", Values: []string{"flat", "appartement"}},
			{Canonical: "flex_space", Values: []string{"flex", "flexible space"}},
			{Canonical: "event_space", Values: []string{"event venue", "venue", "salle"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		dbPath := defaultDBPath("peerspace-pp-cli")
		s, err := store.OpenWithContext(ctx, dbPath)
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: peerspace-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: peerspace-pp-cli: learn init: %v\n", err)
		}
	})
}
