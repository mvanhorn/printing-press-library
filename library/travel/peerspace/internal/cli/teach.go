...
// peerspace-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/travel/peerspace/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/travel/peerspace/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/travel/peerspace/internal/learn/patterns"
...
const noLearnEnvVar = "PEERSPACE_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("peerspace-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  peerspace-pp-cli teach --query "venues in paris" --resource-type venues --resource 68d468bb44492187e415d4a6`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// --json always emits a machine envelope even when --quiet defaults on
			// (background LLM fire-and-forget still needs agent-readable output when asked).
			if flags.asJSON {
				return printJSONFiltered(cmd.OutOrStdout(), map[string]any{
					"recorded":   true,
					"query":      query,
...
when learnings exist.`,
		Example: `  peerspace-pp-cli recall "<question>" --agent
  peerspace-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List taught query-to-resource learnings from the local store",
		Example: `  peerspace-pp-cli learnings list --agent
  peerspace-pp-cli learnings list --query "<substring>"
  peerspace-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  peerspace-pp-cli learnings forget "<question>" --resource <id>
  peerspace-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  peerspace-pp-cli teach-pattern --query-template "venues in {entity}" --resource-template "GROUP-{entity:category}" --entity-kind city --resource-type venues`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  peerspace-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
