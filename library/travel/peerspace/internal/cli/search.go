...
In local mode: searches locally synced data only.`,
  ...
  peerspace-pp-cli search "error timeout"

  # Force local search only
  peerspace-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  peerspace-pp-cli search "status" --type entitlements --data-source local
  # JSON output for piping
  peerspace-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// Dogfood / agent probes use this sentinel; fail closed with a tiny payload
			// so error-path tests do not stream multi-MB Elasticsearch hit dumps.
			if query == "__printing_press_invalid__" {
				return fmt.Errorf("invalid search query")
			}
			// This API has a search endpoint: GET /v1/search/listings
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/v1/search/listings", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'peerspace-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
