...
agents can record playbooks without a file on disk.`,
		Example: `  peerspace-pp-cli teach-playbook --query "venues for tech meetup paris" --playbook-json '{"steps":[{"cmd":"venues list"}]}'`,
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  peerspace-pp-cli playbook amend --query "venues for tech meetup paris" --add-note "prefer venues recommend after sync"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			cmd.SilenceUsage = true
			cmd.SilenceErrors = true
			if noLearnActive(flags) {
...
func newPlaybookListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example:     `  peerspace-pp-cli playbook list --agent`,
