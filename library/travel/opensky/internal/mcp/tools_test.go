...
func TestMCPPathResolutionMatchesCLIResolverWithPlatformDefaults(t *testing.T) {
	...
	if want := filepath.Join(home, ".config", "opensky-pp-cli", "config.toml"); cfg.Path != want {
	...
	if want := filepath.Join(home, ".local", "share", "opensky-pp-cli", "data.db"); gotDB != want {
...
func resetMCPPathEnv(t *testing.T) string {
	...
	// Windows-specific home vars too or the real user state dirs leak into the
	// suite (same fix as the greenhouse/lever prints).
...
func TestMCPRegisterToolsPreservesTypedSpecialTools(t *testing.T) {
	s := server.NewMCPServer("opensky", "test")
	RegisterTools(s)
