...
Run sync first to populate the local search index.`,
  ...
  opensky-pp-cli search "status"

  # Force local search explicitly
  opensky-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  opensky-pp-cli search "status" --type flights --data-source local
  # JSON output for piping
  opensky-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'opensky-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
