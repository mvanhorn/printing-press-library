...
// opensky-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
const noLearnEnvVar = "OPENSKY_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("opensky-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  opensky-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  opensky-pp-cli recall "<question>" --agent
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  opensky-pp-cli learnings list --agent
  opensky-pp-cli learnings list --query "<substring>"
  opensky-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
a whole family.`,
		Example: `  opensky-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  opensky-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
