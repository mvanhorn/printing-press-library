...
func withTempLearnHome(t *testing.T) string {
	...
	dir := t.TempDir()
	t.Setenv("HOME", dir)
	// Windows: os.UserHomeDir() reads USERPROFILE, not HOME, so isolate the
	// Windows home vars too or the real state dir leaks into the suite (same
	// fix as the greenhouse/lever prints).
	...
	t.Setenv("OPENSKY_STATE_DIR", "")
	t.Setenv("OPENSKY_HOME", "")
...
func TestSkipLearnHook_FrameworkCommandsSkipped(t *testing.T) {
	cases := []string{
		"opensky-pp-cli auth login",
		"opensky-pp-cli doctor",
		"opensky-pp-cli help items",
		"opensky-pp-cli sync",
		"opensky-pp-cli profile list",
		"opensky-pp-cli feedback list",
		"opensky-pp-cli which",
		"opensky-pp-cli agent-context",
		"opensky-pp-cli completion bash",
		"opensky-pp-cli version",
	}
...
func TestSkipLearnHook_NovelCommandsNotSkipped(t *testing.T) {
	novel := []string{
		...
		"opensky-pp-cli recall",
		"opensky-pp-cli learnings",
		"opensky-pp-cli teach-pattern",
		"opensky-pp-cli teach-lookup",
		"opensky-pp-cli items list",
	}
...
func TestSkipLearnHook_FrameworkCommandsDoNotCreateDefaultDB(t *testing.T) {
	...
	for _, args := range cases {
		...
		if _, err := os.Stat(defaultDBPath("opensky-pp-cli")); err == nil {
