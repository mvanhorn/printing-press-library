...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("OPENSKY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("OPENSKY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/travel/opensky/feedback")
...
When ` + "`OPENSKY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`OPENSKY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  opensky-pp-cli feedback list
  opensky-pp-cli feedback list --limit 5
  opensky-pp-cli feedback list --json`,
