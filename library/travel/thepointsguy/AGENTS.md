# The Points Guy Printed CLI Agent Guide

This directory is a generated `thepointsguy-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
thepointsguy-pp-cli doctor --json
thepointsguy-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
thepointsguy-pp-cli which "<capability>" --json
thepointsguy-pp-cli <command> --help
```
...
```bash
thepointsguy-pp-cli <command> --agent
```
...
```bash
thepointsguy-pp-cli <command> --help
thepointsguy-pp-cli <command> --dry-run --agent
```
