...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("compare_two_cards",
			mcplib.WithDescription("Side-by-side annual fee, APRs, welcome bonus, and rewards from the local mirror."),
		...
		handleCompareTwoCards,
	)
}

// handleCompareTwoCards runs the compare_two_cards recipe intent: Side-by-side annual fee, APRs, welcome bonus, and rewards from the local mirror.
func handleCompareTwoCards(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
