...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "thepointsguy-pp-cli",
		Short: `Thepointsguy CLI — The first CLI and local database for The Points Guy: search every article, look up any card's real terms, and turn poin…`,
		Long: `Thepointsguy CLI — The first CLI and local database for The Points Guy: search every article, look up any card's real terms, and turn poin…
...
See README.md or the bundled SKILL.md for recipes.`,
	...
	rootCmd.SetVersionTemplate("thepointsguy-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'thepointsguy-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelRedeemCheckCmd(flags))
	rootCmd.AddCommand(newNovelSinceCmd(flags))
	rootCmd.AddCommand(newNovelValuationsCmd(flags))
	rootCmd.AddCommand(newNovelWorthCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newArticlesPromotedCmd(flags))
	// cards: attach hand-authored subcommands (list/get/best) to the generated
	// parent alongside its built-in `cards <slug>` and `cards compare`.
	cardsCmd := newCardsPromotedCmd(flags)
	cardsCmd.AddCommand(newCardsListCmd(flags))
	cardsCmd.AddCommand(newCardsGetCmd(flags))
	cardsCmd.AddCommand(newCardsBestCmd(flags))
	rootCmd.AddCommand(cardsCmd)
	// Hand-authored absorbed commands over the discovered TPG surfaces.
