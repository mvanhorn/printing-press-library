...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("THEPOINTSGUY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("THEPOINTSGUY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "thepointsguy-pp-cli/feedback")
...
When ` + "`THEPOINTSGUY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`THEPOINTSGUY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  thepointsguy-pp-cli feedback list
  thepointsguy-pp-cli feedback list --limit 5
  thepointsguy-pp-cli feedback list --json`,
