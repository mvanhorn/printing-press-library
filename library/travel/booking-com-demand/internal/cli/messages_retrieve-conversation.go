...
func newMessagesRetrieveConversationCmd(flags *rootFlags) *cobra.Command {
	var flagBodyJSON string
	var stdinBody bool

	cmd := &cobra.Command{
		Use:         "retrieve-conversation",
		Short:       "Retrieves a conversation accessible to the authenticated user, including message history and participants.",
		Example:     "  booking-com-demand-pp-cli messages retrieve-conversation",
		Annotations: map[string]string{"pp:endpoint": "messages.retrieve-conversation", "pp:method": "POST", "pp:path": "/messages/conversations"},
