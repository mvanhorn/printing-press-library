...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".booking-com-demand-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BOOKING_COM_DEMAND_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BOOKING_COM_DEMAND_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "booking-com-demand-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.booking-com-demand-pp-cli/feedback.jsonl.
When ` + "`BOOKING_COM_DEMAND_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BOOKING_COM_DEMAND_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  booking-com-demand-pp-cli feedback list
  booking-com-demand-pp-cli feedback list --limit 5
  booking-com-demand-pp-cli feedback list --json`,
