...
description: "The only Go-native, agent-native, MCP-exposed UK rail CLI; live boards, journey planning, and a local SQLite store... Trigger phrases: `next train from <station>`, `departures from <station>`, `trains london to <destination>`, `is the X.YY to <station> on time`, `which platform is the <service> on`, `use uk-train-goat`, `run uk-train-goat-pp-cli`."
author: "Sachin Ahuja"
license: "Apache-2.0"
...
metadata:
  openclaw:
    ...
    install:
      - kind: go
        bins: [uk-train-goat-pp-cli]
        module: github.com/mvanhorn/printing-press-library/library/travel/uk-train-goat/cmd/uk-train-goat-pp-cli
---
...
Multi-origin fan-out + dotted-path field selection; tight payload for an LLM context window.
...
Runs the agent eval suite; exits non-zero if pass rate drops below 80%.
...
Entries are stored locally at `~/.uk-train-goat-pp-cli/feedback.jsonl`. They are never POSTed unless `TRAIN_GOAT_FEEDBACK_ENDPOINT` is set AND either `--send` is passed or `TRAIN_GOAT_FEEDBACK_AUTO_SEND=true`. Default behavior is local-only.
