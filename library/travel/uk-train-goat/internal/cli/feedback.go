...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TRAIN_GOAT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TRAIN_GOAT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "uk-train-goat-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.uk-train-goat-pp-cli/feedback.jsonl.
When ` + "`TRAIN_GOAT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TRAIN_GOAT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
