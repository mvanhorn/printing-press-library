...
// defaultSyncResources returns the default list of resources to sync.
// uk-train-goat is a wrapper-only CLI: live OpenLDBWS data is fetched on
// every command, and the static UK station list (~2,580 entries) is
// populated automatically on the first call to `stations --search`
// from the embedded wrapper map. There is no HTTP-paged resource to
// sync, so this returns an empty list. The sync command remains
// available for forward-compatibility (v0.2 may add cacheable resources
// such as historical performance stats).
