...
// sea-airport-parking-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/travel/sea-airport-parking/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/travel/sea-airport-parking/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/travel/sea-airport-parking/internal/learn/patterns"
...
const noLearnEnvVar = "SEA_AIRPORT_PARKING_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("sea-airport-parking-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  sea-airport-parking-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  sea-airport-parking-pp-cli recall "<question>" --agent
  sea-airport-parking-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  sea-airport-parking-pp-cli learnings list --agent
  sea-airport-parking-pp-cli learnings list --query "<substring>"
  sea-airport-parking-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  sea-airport-parking-pp-cli learnings forget "<question>" --resource <id>
  sea-airport-parking-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  sea-airport-parking-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  sea-airport-parking-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
