...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SEA_AIRPORT_PARKING_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SEA_AIRPORT_PARKING_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "sea-airport-parking-pp-cli/feedback")
...
When ` + "`SEA_AIRPORT_PARKING_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SEA_AIRPORT_PARKING_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "sea-airport-parking-pp-cli",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  sea-airport-parking-pp-cli feedback list
  sea-airport-parking-pp-cli feedback list --limit 5
  sea-airport-parking-pp-cli feedback list --json`,
