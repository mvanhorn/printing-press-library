...
type rootFlags struct {
	...
	// invocation. Mirrors the SEA_AIRPORT_PARKING_NO_LEARN env var.
	noLearn       bool
	selectFields  string
	configPath    string
	homePath      string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "sea-airport-parking-pp-cli",
		Short: `Sea Airport Parking CLI — Check, track, and get alerted on SEA's official on-airport Reserved garage — with a local price/availability history…`,
		Long: `Sea Airport Parking CLI — Check, track, and get alerted on SEA's official on-airport Reserved garage — with a local price/availability history and sold-out watch that no site or aggregator has.
...
Health check: run 'sea-airport-parking-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("sea-airport-parking-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'sea-airport-parking-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelSweepCmd(flags))
	rootCmd.AddCommand(newNovelWatchCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newParkingPromotedCmd(flags))
