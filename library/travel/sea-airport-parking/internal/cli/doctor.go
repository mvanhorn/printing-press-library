...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  sea-airport-parking-pp-cli doctor
  sea-airport-parking-pp-cli doctor --json
  sea-airport-parking-pp-cli doctor --fail-on warn
  sea-airport-parking-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					healthPath := "/book/SEA/Parking?parkingCmd=collectParkingDetails"
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "sea-airport-parking-pp-cli", suggestion)
						} else {
