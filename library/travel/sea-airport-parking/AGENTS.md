# SEA Airport Parking Printed CLI Agent Guide

This directory is a generated `sea-airport-parking-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
sea-airport-parking-pp-cli doctor --json
sea-airport-parking-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
sea-airport-parking-pp-cli which "<capability>" --json
sea-airport-parking-pp-cli <command> --help
```
...
```bash
sea-airport-parking-pp-cli <command> --agent
```
...
```bash
sea-airport-parking-pp-cli <command> --help
sea-airport-parking-pp-cli <command> --dry-run --agent
```
...
3. After answering, always fire `sea-airport-parking-pp-cli teach --query "<question>" --resource <id> --resource-type <type> &` in the background - teaching is unconditional and is the anchor that triggers playbook synthesis. Teach the structural question with identifiers stripped (no names, emails, phone numbers, account ids); the CLI warns on obvious PII shapes but does not block.
...
Disable the loop with `--no-learn` per-invocation or `SEA_AIRPORT_PARKING_NO_LEARN=true` for the whole session - useful for deterministic agent flows that don't want a learning row to silently change subsequent query results.
