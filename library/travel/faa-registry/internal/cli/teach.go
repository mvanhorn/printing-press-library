...
// faa-registry-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/travel/faa-registry/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/travel/faa-registry/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/travel/faa-registry/internal/learn/patterns"
...
const noLearnEnvVar = "FAA_REGISTRY_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("faa-registry-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  faa-registry-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  faa-registry-pp-cli recall "<question>" --agent
  faa-registry-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  faa-registry-pp-cli learnings list --agent
  faa-registry-pp-cli learnings list --query "<substring>"
  faa-registry-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  faa-registry-pp-cli learnings forget "<question>" --resource <id>
  faa-registry-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  faa-registry-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  faa-registry-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
