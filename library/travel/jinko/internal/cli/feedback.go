...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "jinko-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("JINKO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("JINKO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/travel/jinko/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/jinko-pp-cli/feedback.jsonl.
When ` + "`JINKO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`JINKO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  jinko-pp-cli feedback list
  jinko-pp-cli feedback list --limit 5
  jinko-pp-cli feedback list --json`,
