...
In local mode: searches locally synced data only.`,
  ...
  jinko-pp-cli search "error timeout"

  # Force local search only
  jinko-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  jinko-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  jinko-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /api/v1/flights/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/api/v1/flights/search", map[string]any{
					"q":                 query,
					"allow_live_shop":   nil,
					"currency":          "",
					...
					"price_constraints": map[string]any{},
					"sort":              "lowest",
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'jinko-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
