...
Resource scoping:
  ...
  jinko-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  jinko-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  jinko-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  jinko-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  jinko-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("jinko-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/jinko-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "devplatform":
		return db.UpsertDevplatform(data)
	case "flights":
		return db.UpsertFlights(data)
	case "hotels":
		return db.UpsertHotels(data)
	case "payment":
		return db.UpsertPayment(data)
	case "shop":
		return db.UpsertShop(data)
	case "quoted":
		return db.UpsertQuoted(data)
	case "shopping-cart":
		return db.UpsertShoppingCart(data)
	case "travelers":
		return db.UpsertTravelers(data)
	default:
...
func defaultSyncResources() []string {
	return []string{
		"devplatform",
		"devplatform-users",
		"devplatform-users-me",
		"devplatform-users-me-keys",
		"devplatform-users-me-usage",
		"travelers",
	}
}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"devplatform":                "/api/v1/devplatform/orgs",
		"devplatform-users":          "/api/v1/devplatform/users",
		"devplatform-users-me":       "/api/v1/devplatform/users/me",
		"devplatform-users-me-keys":  "/api/v1/devplatform/users/me/keys",
		"devplatform-users-me-usage": "/api/v1/devplatform/users/me/usage",
		"travelers":                  "/travelers",
	}
