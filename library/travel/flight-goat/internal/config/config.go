...
type Config struct {
	...
	FlightGoatApiKey string          `toml:"goat_api_key"`
...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://{env}.flightaware.com/aeroapi",
	}
	...
	// Env var overrides
	if v := os.Getenv("FLIGHT_GOAT_API_KEY"); v != "" {
		cfg.FlightGoatApiKey = v
		cfg.markEnvOverride("FlightGoatApiKey")
		cfg.AuthSource = "env:FLIGHT_GOAT_API_KEY"
		cfg.CredentialSource = "env:FLIGHT_GOAT_API_KEY"
	}
	...
	if v := strings.TrimSpace(os.Getenv("FLIGHT_GOAT_ENV")); v != "" {
		cfg.TemplateVars["env"] = normalizeEndpointTemplateValue(v)
	} else {
		cfg.TemplateVars["env"] = "aeroapi"
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("FLIGHT_GOAT_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "flight-goat-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.FlightGoatApiKey
	if token == "" {
		return ""
	}
	if c.FlightGoatApiKey == "" {
		return ""
	}
	return token
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		FlightGoatApiKey: c.FlightGoatApiKey,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.FlightGoatApiKey = creds.FlightGoatApiKey
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.FlightGoatApiKey = token
	delete(c.envOverrides, "FlightGoatApiKey")
	c.updateFileConfigField("FlightGoatApiKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["FlightGoatApiKey"] {
			out.FlightGoatApiKey = c.fileConfig.FlightGoatApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "FlightGoatApiKey":
		c.fileConfig.FlightGoatApiKey = c.FlightGoatApiKey
	}
}
