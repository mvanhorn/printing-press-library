...
In local mode: searches locally synced data only.`,
  ...
  flight-goat-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  flight-goat-pp-cli search "status" --type aircraft --data-source local
  ...
  flight-goat-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.dataSource != "local" {
				...
				data, getErr := c.Get(cmd.Context(), "/flights/search", map[string]string{
					"query": query,
				})
				if getErr == nil {
			...
			case "":
				...
				{
					partial, searchErr := db.SearchAirports(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search airports failed: %w", searchErr)
					}
