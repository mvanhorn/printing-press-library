...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"FLIGHT_GOAT_CONFIG",
		"FLIGHT_GOAT_CONFIG_DIR",
		"FLIGHT_GOAT_DATA_DIR",
		"FLIGHT_GOAT_STATE_DIR",
		"FLIGHT_GOAT_CACHE_DIR",
		"FLIGHT_GOAT_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
	...
	return home, filepath.Join(home, ".config", "flight-goat-pp-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "flight-goat-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("FLIGHT_GOAT_API_KEY", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "flight-goat-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "flight-goat-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "flight-goat-pp-cli", "credentials.toml")
