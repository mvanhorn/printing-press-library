...
func TestOpenHardensSQLiteFilePermissions(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// No -wal/-shm assertions: this store uses journal_mode(DELETE),
			// not WAL (see the DSN comment in OpenWithContext), so those
			// sidecar files are never created. DELETE mode's own rollback
			// journal ("-journal") is created and removed within each
			// transaction, so none is left over to assert on once Open
			// has returned.
		})
	}
}
...
func TestOpenWithRelativePathDoesNotChmodWorkingDirectory(t *testing.T) {
	...
	// No -wal/-shm assertions -- this store uses journal_mode(DELETE), not
	// WAL; see the DSN comment in OpenWithContext.
}
...
//
// journal_mode is "delete", not "wal": this CLI is invoked as many
// short-lived processes, and WAL's shared-memory "-shm" sidecar file crashed
// with SIGBUS (not a catchable SQLITE_BUSY) when several such processes
// raced to create or reopen it. See the DSN comment in OpenWithContext.
func TestOpenAppliesPragmas(t *testing.T) {
	...
	defer s.Close()
	requirePragma(t, s.DB(), "journal_mode", "delete")
...
// TestMigrate_AddsColumnsOnUpgrade_Hotel verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Hotel(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "hotel" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("hotel")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from hotel after migrate", want)
		}
	}
}
