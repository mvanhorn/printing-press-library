...
type Config struct {
	...
	TravelclickToken string          `toml:"token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.travelclick.com",
	}
	...
	// Env var overrides
	if v := strings.ToLower(strings.TrimSpace(os.Getenv("TRAVELCLICK_SKIP_TLS_VERIFY"))); v == "1" || v == "true" || v == "yes" || v == "on" {
		cfg.SetSkipTLSVerify(true)
	}
	if v := os.Getenv("TRAVELCLICK_TOKEN"); v != "" {
		cfg.TravelclickToken = v
		cfg.markEnvOverride("TravelclickToken")
		cfg.AuthSource = "env:TRAVELCLICK_TOKEN"
		cfg.CredentialSource = "env:TRAVELCLICK_TOKEN"
	}
	...
	if v := os.Getenv("TRAVELCLICK_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("TRAVELCLICK_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "travelclick-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.TravelclickToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:TRAVELCLICK_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"token":             c.TravelclickToken,
			"TRAVELCLICK_TOKEN": c.TravelclickToken,
			"access_token":      c.TravelclickToken,
		})
	}
	if c.AccessToken != "" {
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.TravelclickToken != "" {
		parts = append(parts, "token="+c.TravelclickToken)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		TravelclickToken: c.TravelclickToken,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.TravelclickToken == "" {
		c.TravelclickToken = creds.TravelclickToken
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.TravelclickToken = ""
	delete(c.envOverrides, "TravelclickToken")
	c.updateFileConfigField("TravelclickToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	...
	if c.fileConfig != nil {
		...
		if c.envOverrides["TravelclickToken"] {
			out.TravelclickToken = c.fileConfig.TravelclickToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "TravelclickToken":
		c.fileConfig.TravelclickToken = c.TravelclickToken
	}
}
