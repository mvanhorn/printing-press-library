...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("check_a_corporate_code_before_searching",
			mcplib.WithDescription("Confirms a rate-access code is live for this hotel before building a search around it."),
		...
		handleCheckACorporateCodeBeforeSearching,
	)
}

// handleCheckACorporateCodeBeforeSearching runs the check_a_corporate_code_before_searching recipe intent: Confirms a rate-access code is live for this hotel before building a search around it.
func handleCheckACorporateCodeBeforeSearching(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
