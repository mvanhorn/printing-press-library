...
var whichIndex = []whichEntry{
	{Command: "rates compare", Description: "Check several boutique hotels for the same dates in one call, ranked by lowest fee-inclusive total.", Group: "Cross-hotel local queries", WhyItMatters: "Reach for this when the user names more than one property for the same trip instead of calling 'rates search' repeatedly."},
	{Command: "rates cheapest-night", Description: "Scan several hotels' calendars at once and return the single best hotel+date combination.", Group: "Cross-hotel local queries", WhyItMatters: "Reach for this when the user is flexible on both hotel and date, not just date."},
	{Command: "hotels alias", Description: "Give a memorable name to a TravelClick hotel ID instead of memorizing a 6-digit number.", Group: "Local state that compounds", WhyItMatters: "Reach for this before repeated lookups against the same property so later commands can take the alias instead of the raw ID."},
	{Command: "analytics price-drift", Description: "Track how a hotel's rates move over time from your own saved search history.", Group: "Local state that compounds", WhyItMatters: "Reach for this after the user has run a few 'rates search --save' calls and wants to know if a rate went up or down."},
	{Command: "codes check-all", Description: "Test one corporate or group code against every saved hotel at once.", Group: "Cross-hotel local queries", WhyItMatters: "Reach for this when the user has a code but isn't sure which of their saved hotels honors it."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  travelclick-pp-cli which "stale tickets"
  travelclick-pp-cli which "bottleneck"
  travelclick-pp-cli which --limit 1 "send message"
  travelclick-pp-cli which                                # list the full capability index`,
