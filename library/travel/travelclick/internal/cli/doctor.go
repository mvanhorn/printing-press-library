...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  travelclick-pp-cli doctor
  travelclick-pp-cli doctor --json
  travelclick-pp-cli doctor --fail-on warn
  travelclick-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "Set it with: travelclick-pp-cli auth set-token <token> or export TRAVELCLICK_TOKEN=\"your-token-here\""
						report["auth_docs_url"] = "https://api.travelclick.com"
					} else {
						authConfigured = true
			...
			if os.Getenv("TRAVELCLICK_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "TRAVELCLICK_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "TRAVELCLICK_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "travelclick-pp-cli", suggestion)
						} else {
