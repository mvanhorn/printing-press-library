...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TRAVELCLICK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TRAVELCLICK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "travelclick-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  travelclick-pp-cli feedback \"the --check-in flag name wasn't obvious from --help alone\"",
		Long: `Feedback is captured locally first in the CLI data directory's feedback.jsonl.
When ` + "`TRAVELCLICK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TRAVELCLICK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  travelclick-pp-cli feedback list
  travelclick-pp-cli feedback list --limit 5
  travelclick-pp-cli feedback list --json`,
