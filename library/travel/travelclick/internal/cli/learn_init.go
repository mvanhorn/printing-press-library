...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^[0-9]{4,7}$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: travelclick-pp-cli: ticker pattern %q failed to compile: %v\n", `^[0-9]{4,7}$`, err)
	} else {
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"hotel_id": {
			{Canonical: "102306", Values: []string{"made hotel", "made hotel nyc", "made", "made nyc"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: travelclick-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: travelclick-pp-cli: learn init: %v\n", err)
		}
	})
}
