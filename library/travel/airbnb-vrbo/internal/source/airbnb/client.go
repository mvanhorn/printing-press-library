...
const (
	...
	geoUA      = "airbnb-vrbo-pp-cli/0.1.0 (+https://github.com/mvanhorn/airbnb-vrbo)"
)
...
func (c *Client) allowedByRobots(ctx context.Context, path string) error {
	if strings.EqualFold(os.Getenv("AIRBNB_VRBO_IGNORE_ROBOTS_TXT"), "true") || os.Getenv("AIRBNB_VRBO_IGNORE_ROBOTS_TXT") == "1" {
		return nil
	}
	c.mu.Lock()
	if allowed, ok := c.robots[path]; ok {
		c.mu.Unlock()
		if !allowed {
			return fmt.Errorf("blocked by Airbnb robots.txt for %s; set AIRBNB_VRBO_IGNORE_ROBOTS_TXT=true to override", path)
		}
		return nil
	}
	c.mu.Unlock()
