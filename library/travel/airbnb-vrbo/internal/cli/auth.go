...
profile by name when the installed backend supports it.`,
		Example: `  airbnb-vrbo-pp-cli auth login --chrome
  airbnb-vrbo-pp-cli auth login --browser
  airbnb-vrbo-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !browserFlag {
				fmt.Fprintln(cmd.OutOrStdout(), "Use --chrome or --browser to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  airbnb-vrbo-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  airbnb-vrbo-pp-cli auth login --browser\n")
				return nil
			}
...
func saveAirbnbCookieJSON(cookies string) error {
	...
	dir := filepath.Join(home, ".config", "airbnb-vrbo-pp-cli")
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  airbnb-vrbo-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  airbnb-vrbo-pp-cli auth login --chrome\n")
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  airbnb-vrbo-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
