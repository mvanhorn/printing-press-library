...
Exit codes & warnings:
  ...
  airbnb-vrbo-pp-cli sync

  # Sync specific resources only
  airbnb-vrbo-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  airbnb-vrbo-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  airbnb-vrbo-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  airbnb-vrbo-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  airbnb-vrbo-pp-cli sync --latest-only`,
