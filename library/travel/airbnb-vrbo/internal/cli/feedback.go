...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".airbnb-vrbo-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AIRBNB_VRBO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AIRBNB_VRBO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/travel/airbnb-vrbo/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.airbnb-vrbo-pp-cli/feedback.jsonl.
When ` + "`AIRBNB_VRBO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AIRBNB_VRBO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
