...
var readCommandResources = map[string][]string{
	"airbnb-vrbo-pp-cli airbnb_wishlist":        {"airbnb_wishlist"},
	"airbnb-vrbo-pp-cli airbnb_wishlist list":   {"airbnb_wishlist"},
	"airbnb-vrbo-pp-cli airbnb_wishlist get":    {"airbnb_wishlist"},
	"airbnb-vrbo-pp-cli airbnb_wishlist search": {"airbnb_wishlist"},
}
...
func cachePolicy() cliutil.Policy {
	...
	envOptOut := "AIRBNB_VRBO_NO_AUTO_REFRESH"
...
func autoRefreshIfStale(ctx context.Context, flags *rootFlags, resources []string) (meta cliutil.FreshnessMeta) {
	...
	if err := runAutoRefresh(refreshCtx, flags, db, resources); err != nil {
		fmt.Fprintf(os.Stderr, "warning: using stale airbnb-vrbo-pp-cli cache (refresh failed: %v)\n", err)
