...
### Subscriber support lookup

```bash
postscript-pp-cli subscribers list --phone-number-contains 555 --json
```

### Flow event intake

```bash
postscript-pp-cli events --type BackInStock --email customer@example.com --external-id evt_123 --json
```
