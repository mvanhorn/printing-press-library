# Postscript Printed CLI Agent Guide

This directory is a generated `postscript-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
postscript-pp-cli doctor --json
postscript-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
postscript-pp-cli which "<capability>" --json
postscript-pp-cli <command> --help
```
...
```bash
postscript-pp-cli <command> --agent
```
...
```bash
postscript-pp-cli <command> --help
postscript-pp-cli <command> --dry-run --agent
```
