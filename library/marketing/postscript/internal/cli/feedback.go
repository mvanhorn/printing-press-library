...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "postscript-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("POSTSCRIPT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("POSTSCRIPT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "postscript-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/postscript-pp-cli/feedback.jsonl.
When ` + "`POSTSCRIPT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`POSTSCRIPT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  postscript-pp-cli feedback list
  postscript-pp-cli feedback list --limit 5
  postscript-pp-cli feedback list --json`,
