...
Resource scoping:
  ...
  postscript-pp-cli sync

  # Sync specific resources only
  postscript-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  postscript-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  postscript-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  postscript-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  postscript-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/postscript-pp-cli/data.db)")
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"subscribers": "/api/v2/subscribers",
	}
