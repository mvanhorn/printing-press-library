...
type Config struct {
	...
	PostscriptApiKey string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("POSTSCRIPT_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "postscript-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("POSTSCRIPT_API_KEY"); v != "" {
		cfg.PostscriptApiKey = v
		cfg.markEnvOverride("PostscriptApiKey")
		cfg.AuthSource = "env:POSTSCRIPT_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.PostscriptApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("POSTSCRIPT_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	replacements := map[string]string{
		...
		"POSTSCRIPT_API_KEY": c.PostscriptApiKey,
	}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.PostscriptApiKey = token
	delete(c.envOverrides, "PostscriptApiKey")
	c.updateFileConfigField("PostscriptApiKey")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["PostscriptApiKey"] {
			out.PostscriptApiKey = c.fileConfig.PostscriptApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "PostscriptApiKey":
		c.fileConfig.PostscriptApiKey = c.PostscriptApiKey
	}
}
