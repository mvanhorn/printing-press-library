...
**Amplitude read workflows for agents that need product analytics context.**

Wraps Amplitude read APIs with agent defaults, local SQLite sync, search, analytics, command discovery, and MCP tools so agents can inspect events, users, annotations, exports, funnels, retention, revenue, and segmentation without hand-building HTTP calls.
...
- **`search`** — Search live or locally synced Amplitude records from one agent-friendly command.
