...
type Config struct {
	...
	BingAdsCustomerAccountId               string          `toml:"ads_customer_account_id"`
	MicrosoftAdvertisingBingDeveloperToken string          `toml:"advertising_bing_developer_token"`
	MicrosoftAdvertisingBingEntraBearer    string          `toml:"advertising_bing_entra_bearer"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://campaign.api.bingads.microsoft.com",
	}
	...
	// Env var overrides
	if v := os.Getenv("BING_ADS_CUSTOMER_ACCOUNT_ID"); v != "" {
		cfg.BingAdsCustomerAccountId = v
		cfg.markEnvOverride("BingAdsCustomerAccountId")
		cfg.AuthSource = "env:BING_ADS_CUSTOMER_ACCOUNT_ID"
		cfg.CredentialSource = "env:BING_ADS_CUSTOMER_ACCOUNT_ID"
	}
	...
	if v := os.Getenv("MICROSOFT_ADVERTISING_BING_DEVELOPER_TOKEN"); v != "" {
		cfg.MicrosoftAdvertisingBingDeveloperToken = v
		cfg.markEnvOverride("MicrosoftAdvertisingBingDeveloperToken")
	}
	// Sibling-scheme per-call credential sent on every request alongside the
	// primary auth. AuthSource intentionally not stamped: the primary auth
	// remains the canonical surface for doctor/auth-status reporting.
	if v := os.Getenv("MICROSOFT_ADVERTISING_BING_ENTRA_BEARER"); v != "" {
		cfg.MicrosoftAdvertisingBingEntraBearer = v
		cfg.markEnvOverride("MicrosoftAdvertisingBingEntraBearer")
	}
	...
	if v := os.Getenv("BING_ADS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("BING_ADS_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "bing-ads-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.BingAdsCustomerAccountId
	if token == "" {
		return ""
	}
	if c.BingAdsCustomerAccountId == "" {
		return ""
	}
	return token
}
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.MicrosoftAdvertisingBingDeveloperToken != "" {
		parts = append(parts, "sibling:header:DeveloperToken="+c.MicrosoftAdvertisingBingDeveloperToken)
	}
	if c.MicrosoftAdvertisingBingEntraBearer != "" {
		parts = append(parts, "sibling:header:Authorization="+c.MicrosoftAdvertisingBingEntraBearer)
	}
	if len(parts) > 0 {
	...
	if c.BingAdsCustomerAccountId != "" {
		parts = append(parts, "ads_customer_account_id="+c.BingAdsCustomerAccountId)
	}
	if c.MicrosoftAdvertisingBingDeveloperToken != "" {
		parts = append(parts, "advertising_bing_developer_token="+c.MicrosoftAdvertisingBingDeveloperToken)
	}
	if c.MicrosoftAdvertisingBingEntraBearer != "" {
		parts = append(parts, "advertising_bing_entra_bearer="+c.MicrosoftAdvertisingBingEntraBearer)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) clearCredentialFields() {
	...
	c.ClientID = ""
	c.ClientSecret = ""
	c.BingAdsCustomerAccountId = ""
	c.MicrosoftAdvertisingBingDeveloperToken = ""
	c.MicrosoftAdvertisingBingEntraBearer = ""
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		BingAdsCustomerAccountId:               c.BingAdsCustomerAccountId,
		MicrosoftAdvertisingBingDeveloperToken: c.MicrosoftAdvertisingBingDeveloperToken,
		MicrosoftAdvertisingBingEntraBearer:    c.MicrosoftAdvertisingBingEntraBearer,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.BingAdsCustomerAccountId == "" {
		c.BingAdsCustomerAccountId = creds.BingAdsCustomerAccountId
	}
	if c.MicrosoftAdvertisingBingDeveloperToken == "" {
		c.MicrosoftAdvertisingBingDeveloperToken = creds.MicrosoftAdvertisingBingDeveloperToken
	}
	if c.MicrosoftAdvertisingBingEntraBearer == "" {
		c.MicrosoftAdvertisingBingEntraBearer = creds.MicrosoftAdvertisingBingEntraBearer
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.BingAdsCustomerAccountId = token
	delete(c.envOverrides, "BingAdsCustomerAccountId")
	c.updateFileConfigField("BingAdsCustomerAccountId")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("BingAdsCustomerAccountId")
	c.MicrosoftAdvertisingBingDeveloperToken = ""
	delete(c.envOverrides, "MicrosoftAdvertisingBingDeveloperToken")
	c.updateFileConfigField("MicrosoftAdvertisingBingDeveloperToken")
	c.MicrosoftAdvertisingBingEntraBearer = ""
	delete(c.envOverrides, "MicrosoftAdvertisingBingEntraBearer")
	c.updateFileConfigField("MicrosoftAdvertisingBingEntraBearer")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["BingAdsCustomerAccountId"] {
			out.BingAdsCustomerAccountId = c.fileConfig.BingAdsCustomerAccountId
		}
		if c.envOverrides["MicrosoftAdvertisingBingDeveloperToken"] {
			out.MicrosoftAdvertisingBingDeveloperToken = c.fileConfig.MicrosoftAdvertisingBingDeveloperToken
		}
		if c.envOverrides["MicrosoftAdvertisingBingEntraBearer"] {
			out.MicrosoftAdvertisingBingEntraBearer = c.fileConfig.MicrosoftAdvertisingBingEntraBearer
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "BingAdsCustomerAccountId":
		c.fileConfig.BingAdsCustomerAccountId = c.BingAdsCustomerAccountId
	case "MicrosoftAdvertisingBingDeveloperToken":
		c.fileConfig.MicrosoftAdvertisingBingDeveloperToken = c.MicrosoftAdvertisingBingDeveloperToken
	case "MicrosoftAdvertisingBingEntraBearer":
		c.fileConfig.MicrosoftAdvertisingBingEntraBearer = c.MicrosoftAdvertisingBingEntraBearer
	}
}
