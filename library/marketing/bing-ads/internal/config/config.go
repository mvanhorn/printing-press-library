...
func Load(configPath string) (*Config, error) {
	...
	if explicitConfigFile {
		...
		if real, evalErr := filepath.EvalSymlinks(path); evalErr == nil {
			credentialPermErr := cliutil2.VerifyCredsPerms(real)
			parsed := *cfg
			if err := readConfigFile(path, &parsed, "config-kind path"); err != nil {
				...
				if credentialPermErr != nil && parsed.hasCredentialFields() {
					parsed.addCredentialRefusal(cliutil2.CredentialRefusal{
		...
		data, sourcePath, err := cliutil2.ReadFileWithLegacyFallback(path, legacyPath)
		if err != nil {
			if !os.IsNotExist(err) {
				return nil, err
			}
	...
	if cfg.AgentcookieManagedByExternalStore() {
		...
		var creds *cliutil2.Credentials
		var ok bool
		credentialsRefused := false
		if !cfg.hasCompleteCredentialFields() {
			if explicitConfigFile {
				var status cliutil2.CredentialLoadStatus
				creds, status, err = cliutil2.LoadCredentialsForConfigWithStatus(path)
				if err != nil {
					return nil, err
				}
				ok = status.Loaded
			...
			if (!ok || creds == nil || !creds.HasValues()) && !credentialsRefused {
				var status cliutil2.CredentialLoadStatus
				creds, status, err = cliutil2.LoadCredentialsWithStatus()
				if err != nil {
					return nil, err
				}
				ok = status.Loaded
