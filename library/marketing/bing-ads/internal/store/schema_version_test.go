...
// TestMigrate_AddsColumnsOnUpgrade_AdInsight verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_AdInsight(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "ad_insight" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	rows, err := s.DB().Query(`PRAGMA table_info("ad_insight")`)
	if err != nil {
	...
	for _, want := range []string{
		"currency",
		"is_impression_too_specific",
		"is_privacy_check_passed",
		"suggested_bid",
		"events_lost_to_bid",
		...
		"aar_opt_in_status",
		"recommendation_type",
		"ad_group_bid_landscape_type",
		"ad_group_id",
		"campaign_bid_landscape_type",
		"campaign_id",
		"keyword_id",
		"current_bid",
		"estimated_increase_in_clicks",
		"estimated_increase_in_cost",
		"estimated_increase_in_impressions",
		"match_type",
		"opportunity_key",
		"budget_type",
		"current_budget",
		"increase_in_clicks",
		"increase_in_impressions",
		"percentage_increase_in_clicks",
		"percentage_increase_in_impressions",
		"recommended_budget",
		"bid",
		"category_name",
		"coverage",
		...
		"current_cost",
		"current_impressions",
		"estimated_increase_in_conversions",
		"recommendation_hash",
		"recommendation_id",
		"type",
		"confidence_score",
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from ad_insight after migrate", want)
		}
	}
}
...
// TestMigrate_AddsColumnsOnUpgrade_CampaignManagement verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_CampaignManagement(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "campaign_management" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("campaign_management")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		"final_url_suffix",
		"language",
		"multimedia_ads_bid_adjustment",
		"name",
		"network",
		"privacy_status",
		"status",
		"tracking_url_template",
		"use_optimized_targeting",
		"use_predictive_targeting",
		"is_account_opt_out",
		"is_customer_opt_out",
		"is_customer_opt_out_of_everything",
		"justification",
		"asset_group_id",
		"asset_group_listing_type",
		"is_excluded",
		"listing_group_path",
		"parent_listing_group_id",
		"business_name",
		"campaign_id",
		"size",
		"bid_strategy_id",
		...
		"value",
		"type",
		"file_import_upload_url",
		"file_url",
		"file_url_expiry_time_utc",
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from campaign_management after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_CustomerBilling verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_CustomerBilling(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "customer_billing" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("customer_billing")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		"purchase_order",
		"reference_id",
		"series_frequency_type",
		"series_name",
		"spend_cap_amount",
	} {
		if !hasColumn[want] {
			t.Fatalf("%s column missing from customer_billing after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_CustomerManagement verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_CustomerManagement(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "customer_management" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("customer_management")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		"time_stamp",
		"time_zone",
		"client_entity_customer_number",
		"client_entity_id",
		"client_entity_name",
		"client_entity_number",
		"inviter_email",
		"inviter_name",
		"inviter_phone",
		"is_bill_to_client",
		"last_modified_date_time",
		"managing_customer_id",
		"managing_customer_name",
		"managing_customer_number",
		"note",
		"start_date",
		"status",
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from customer_management after migrate", want)
		}
	}
}
