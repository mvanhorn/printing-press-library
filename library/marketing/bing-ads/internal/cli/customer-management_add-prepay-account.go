...
func newCustomerManagementAddPrepayAccountCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "add-prepay-account",
		Short:       "add_prepay_account",
		Example:     "  bing-ads-pp-cli customer-management add-prepay-account",
		Annotations: map[string]string{"pp:endpoint": "customer-management.add-prepay-account", "pp:method": "POST", "pp:path": "/CustomerManagement/v13/PrepayAccount"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.asJSON || (!isTerminal(cmd.OutOrStdout()) && !flags.csv && !flags.quiet && !flags.plain) {
				...
				if flags.selectFields != "" {
					...
					filtered = compactFields(filtered, map[string]bool{"AccountId": true, "AccountNumber": true, "CreateTime": true})
				}
				if len(filtered) > 0 {
					var parsed any
