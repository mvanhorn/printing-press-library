...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  bing-ads-pp-cli doctor
  bing-ads-pp-cli doctor --json
  bing-ads-pp-cli doctor --fail-on warn
  bing-ads-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "Set your API key with: export BING_ADS_CUSTOMER_ACCOUNT_ID=\"your-token-here\""
					} else {
						authConfigured = true
			...
			if os.Getenv("BING_ADS_CUSTOMER_ACCOUNT_ID") != "" {
				authEnvSet = append(authEnvSet, "BING_ADS_CUSTOMER_ACCOUNT_ID")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "BING_ADS_CUSTOMER_ACCOUNT_ID")
			}
			{
				configuredValue := ""
				if cfg != nil {
					configuredValue = cfg.MicrosoftAdvertisingBingDeveloperToken
				}
				recordAdditionalAuthEnv("MICROSOFT_ADVERTISING_BING_DEVELOPER_TOKEN", configuredValue)
			}
			{
				configuredValue := ""
				if cfg != nil {
					configuredValue = cfg.MicrosoftAdvertisingBingEntraBearer
				}
				recordAdditionalAuthEnv("MICROSOFT_ADVERTISING_BING_ENTRA_BEARER", configuredValue)
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "bing-ads-pp-cli", suggestion)
						} else {
