...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BING_ADS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BING_ADS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "bing-ads-pp-cli/feedback")
...
When ` + "`BING_ADS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BING_ADS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  bing-ads-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
  bing-ads-pp-cli feedback --stdin < notes.txt
  bing-ads-pp-cli feedback list --limit 10`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  bing-ads-pp-cli feedback list
  bing-ads-pp-cli feedback list --limit 5
  bing-ads-pp-cli feedback list --json`,
