...
func newCampaignManagementGetCampaignsByIdsCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "get-campaigns-by-ids",
		Short:       "get_campaigns_by_ids",
		Example:     "  bing-ads-pp-cli campaign-management get-campaigns-by-ids",
		Annotations: map[string]string{"pp:endpoint": "campaign-management.get-campaigns-by-ids", "pp:method": "POST", "pp:path": "/CampaignManagement/v13/Campaigns/QueryByIds", "mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if isDryRunResponse(c.IsDryRun(), data) {
				if flags.asJSON || (!isTerminal(cmd.OutOrStdout()) && !flags.csv && !flags.quiet && !flags.plain) {
					return printOutputWithFlagsMeta(cmd.OutOrStdout(), data, flags, map[string]any{"source": "dry-run"}, map[string]bool{"Campaigns": true, "PartialErrors": true})
				}
				return nil
			}
			_ = statusCode
			...
			return printOutputWithFlagsMeta(cmd.OutOrStdout(), formatData, flags, map[string]any{"source": "live"}, map[string]bool{"Campaigns": true, "PartialErrors": true})
		},
	}
