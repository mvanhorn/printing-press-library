...
type rootFlags struct {
	...
	// invocation. Mirrors the BING_ADS_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "bing-ads-pp-cli",
		Short: "Manage bing-ads resources via the bing-ads API",
		Long: `Manage bing-ads resources via the bing-ads API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'bing-ads-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("bing-ads-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select Result)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'bing-ads-pp-cli profile list')")
	...
	rootCmd.AddCommand(newAdInsightCmd(flags))
	rootCmd.AddCommand(newBulkCmd(flags))
	rootCmd.AddCommand(newCampaignManagementCmd(flags))
	rootCmd.AddCommand(newCustomerBillingCmd(flags))
	rootCmd.AddCommand(newCustomerManagementCmd(flags))
