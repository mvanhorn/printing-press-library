...
func newTeachPlaybookCmd(flags *rootFlags, learnCfg *entities2.Config) *cobra.Command {
	var query string
...
agents can record playbooks without a file on disk.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			family := learn2.QueryFamily(normalized)
			if family == "" {
				return fmt.Errorf("teach-playbook: query normalized to empty family")
			}

			id, inserted, err := s.UpsertPlaybook(store2.UpsertPlaybookInput{
				QueryFamily:  family,
			...
			if evErr := s.InsertLearnEvent(store2.LearnEventTeachPlaybook,
				learn2.FamilyHash(family), 0, false, store2.LearnEventSurface()); evErr != nil {
...
func newPlaybookAmendCmd(flags *rootFlags, learnCfg *entities2.Config) *cobra.Command {
	var query string
	var addNote string
	var dbPath string
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if evErr := s.InsertLearnEvent(store2.LearnEventAmend,
				learn2.FamilyHash(family), 0, false, store2.LearnEventSurface()); evErr != nil {
