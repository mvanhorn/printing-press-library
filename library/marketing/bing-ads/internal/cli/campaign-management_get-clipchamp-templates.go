			...
			if stdinBody {
				...
				}
				{
					...
					if cmd.Flags().Changed("template-filter-aspect-ratios") {
						parsedTemplateFilterAspectRatios, parseErr := cliutil2.ParseStringList(bodyTemplateFilterAspectRatios)
						if parseErr != nil {
					...
					if cmd.Flags().Changed("template-filter-template-ids") {
						parsedTemplateFilterTemplateIds, parseErr := cliutil2.ParseStringList(bodyTemplateFilterTemplateIds)
						if parseErr != nil {
					...
					if cmd.Flags().Changed("template-filter-themes") {
						parsedTemplateFilterThemes, parseErr := cliutil2.ParseStringList(bodyTemplateFilterThemes)
						if parseErr != nil {
