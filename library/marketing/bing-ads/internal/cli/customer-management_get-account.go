...
func newCustomerManagementGetAccountCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "get-account",
		Short:       "get_account",
		Example:     "  bing-ads-pp-cli customer-management get-account",
		Annotations: map[string]string{"pp:endpoint": "customer-management.get-account", "pp:method": "POST", "pp:path": "/CustomerManagement/v13/Account/Query", "mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if isDryRunResponse(c.IsDryRun(), data) {
				if flags.asJSON || (!isTerminal(cmd.OutOrStdout()) && !flags.csv && !flags.quiet && !flags.plain) {
					return printOutputWithFlagsMeta(cmd.OutOrStdout(), data, flags, map[string]any{"source": "dry-run"}, map[string]bool{"Account": true})
				}
				return nil
			}
			_ = statusCode
			...
			return printOutputWithFlagsMeta(cmd.OutOrStdout(), formatData, flags, map[string]any{"source": "live"}, map[string]bool{"Account": true})
		},
	}
