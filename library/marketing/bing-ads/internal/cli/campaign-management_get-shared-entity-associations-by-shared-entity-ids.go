...
func newCampaignManagementGetSharedEntityAssociationsBySharedEntityIdsCmd(flags *rootFlags) *cobra.Command {
	var bodyEntityType string
	var bodyReturnAdditionalFields string
	var bodySharedEntityIds string
	var bodySharedEntityScope string
	var bodySharedEntityType string
	var stdinBody bool

	cmd := &cobra.Command{
		Use:         "get-shared-entity-associations-by-shared-entity-ids",
		Short:       "get_shared_entity_associations_by_shared_entity_ids",
		Example:     "  bing-ads-pp-cli campaign-management get-shared-entity-associations-by-shared-entity-ids",
		Annotations: map[string]string{"pp:endpoint": "campaign-management.get-shared-entity-associations-by-shared-entity-ids", "pp:method": "POST", "pp:path": "/CampaignManagement/v13/SharedEntityAssociations/QueryBySharedEntityIds", "mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if stdinBody {
				...
				if cmd.Flags().Changed("shared-entity-ids") {
					parsedSharedEntityIds, parseErr := cliutil.ParseStringList(bodySharedEntityIds)
					if parseErr != nil {
						return fmt.Errorf("parsing --shared-entity-ids list: %w", parseErr)
					}
					bodyMap["SharedEntityIds"] = parsedSharedEntityIds
				}
			...
			if isDryRunResponse(c.IsDryRun(), data) {
				if flags.asJSON || (!isTerminal(cmd.OutOrStdout()) && !flags.csv && !flags.quiet && !flags.plain) {
					return printOutputWithFlagsMeta(cmd.OutOrStdout(), data, flags, map[string]any{"source": "dry-run"}, map[string]bool{"Associations": true, "PartialErrors": true})
				}
				return nil
			}
			_ = statusCode
			...
			return printOutputWithFlagsMeta(cmd.OutOrStdout(), formatData, flags, map[string]any{"source": "live"}, map[string]bool{"Associations": true, "PartialErrors": true})
		},
	}
	...
	cmd.Flags().StringVar(&bodySharedEntityIds, "shared-entity-ids", "", "Shared entity ids")
