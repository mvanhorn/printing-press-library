...
import (
	...
	learn2 "github.com/mvanhorn/printing-press-library/library/marketing/bing-ads/internal/learn"
	"github.com/mvanhorn/printing-press-library/library/marketing/bing-ads/internal/learn/entities"
	entities2 "github.com/mvanhorn/printing-press-library/library/marketing/bing-ads/internal/learn/entities"
	...
	patterns2 "github.com/mvanhorn/printing-press-library/library/marketing/bing-ads/internal/learn/patterns"
	"github.com/mvanhorn/printing-press-library/library/marketing/bing-ads/internal/store"
	store2 "github.com/mvanhorn/printing-press-library/library/marketing/bing-ads/internal/store"
)
...
func newTeachCmd(flags *rootFlags, learnCfg *entities2.Config) *cobra.Command {
	var query string
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, rule := range learn2.ScanPII(query) {
			...
			s, err := store2.OpenWithContext(cmd.Context(), dbPath)
			if err != nil {
			...
			normalized := learn2.Normalize(query, learnCfg)
			...
			resolver := learn2.NewCanonicalResolver(cmd.Context(), s.DB())
			normalized = learn2.PromoteEntities(normalized, resolver)
			...
			for _, rid := range resources {
				...
				learningID, _, uerr := s.UpsertLearning(cmd.Context(), store2.UpsertLearningInput{
					Query:         query,
					QueryEntities: normalized.Entities,
					ResourceID:    rid,
					ResourceType:  resourceType,
					Venue:         venueArg,
					Source:        store2.LearningSourceTaught,
					Notes:         notes,
				})
				if uerr != nil {
				...
				if !noValidate {
					for _, w := range learn2.ValidateResourceShape(cmd.Context(), s.DB(), learnCfg, query, rid, resourceType, nil) {
						if logErr := learn2.AppendTeachLogWarning("teach", query, w); logErr != nil {
			...
			teachFamHash := learn2.FamilyHash(learn2.QueryFamily(normalized))
			teachSurface := store2.LearnEventSurface()
			for _, learningID := range taughtRowIDs {
				if evErr := s.InsertLearnEvent(store2.LearnEventTeach, teachFamHash,
			...
			if _, exErr := patterns2.Extract(s.DB(), nil); exErr != nil {
			...
			if promoErr := promoteCandidateOnTeach(s, learn2.QueryFamily(normalized)); promoErr != nil {
			...
			if _, _, synthErr := learn2.SynthesizePlaybookCandidate(s, learn2.QueryFamily(normalized), learn2.JournalSessionKey()); synthErr != nil {
...
func newRecallCmd(flags *rootFlags, learnCfg *entities2.Config) *cobra.Command {
	var minConf int
	var limit int
	var dbPath string
...
when learnings exist.`,
  ...
  bing-ads-pp-cli recall "<question>" --agent --min-confidence 2`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			result, err := learn2.Recall(cmd.Context(), s.DB(), query, learn2.Opts{
			...
			learn2.SetJournalLearnContext(result.Family, result.UnresolvedEntities)
  ...
  bing-ads-pp-cli learnings list --warnings --agent`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if warningsOnly {
				...
				entries, err := learn2.ReadTeachLogWarnings(filterIDs...)
				if err != nil {
					return fmt.Errorf("learnings list --warnings: %w", err)
				}
				if entries == nil {
					entries = []learn2.TeachLogEntry{}
				}
				if flags.asJSON || !isTerminal(cmd.OutOrStdout()) {
					return printJSONFiltered(cmd.OutOrStdout(), map[string]any{
						"warnings": entries,
			...
			rows, err := listLearningsRows(cmd.Context(), s, store2.ListLearningsFilter{
				Query:         queryFilter,
				Source:        sourceFilter,
...
func newLearningsForgetCmd(flags *rootFlags, learnCfg *entities2.Config) *cobra.Command {
...
Requires at least one of --resource, --action, or --all.`,
  ...
  bing-ads-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			n, err := forgetLearningsRows(cmd.Context(), s, query, store2.ForgetLearningsFilter{
			...
			normalized := learn2.PromoteEntities(
				learn2.Normalize(query, learnCfg),
				learn2.NewCanonicalResolver(cmd.Context(), s.DB()))
			if n > 0 {
				famHash := learn2.FamilyHash(learn2.QueryFamily(normalized))
				...
				if evErr := s.InsertLearnEvent(store2.LearnEventForget, famHash,
					0, false, store2.LearnEventSurface()); evErr != nil {
			...
			_ = appendLearningsAudit(map[string]any{
				"action":       "forget",
				"query_hash":   learn2.QueryHash(query),
				"normalized":   learn2.RedactPII(normalized.NonEntityNormalized),
