		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if stdinBody {
				...
				if cmd.Flags().Changed("audio-filter-audio-names") {
					parsedAudioFilterAudioNames, parseErr := cliutil2.ParseStringList(bodyAudioFilterAudioNames)
					if parseErr != nil {
				...
				if cmd.Flags().Changed("audio-filter-categories") {
					parsedAudioFilterCategories, parseErr := cliutil2.ParseStringList(bodyAudioFilterCategories)
					if parseErr != nil {
