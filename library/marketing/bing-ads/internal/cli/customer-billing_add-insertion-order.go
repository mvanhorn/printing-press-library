...
func newCustomerBillingAddInsertionOrderCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "add-insertion-order",
		Aliases:     []string{"create"},
		Short:       "add_insertion_order",
		Example:     "  bing-ads-pp-cli customer-billing add-insertion-order",
		Annotations: map[string]string{"pp:endpoint": "customer-billing.add-insertion-order", "pp:method": "POST", "pp:path": "/CustomerBilling/v13/InsertionOrder"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.asJSON || (!isTerminal(cmd.OutOrStdout()) && !flags.csv && !flags.quiet && !flags.plain) {
				...
				if flags.selectFields != "" {
					...
					filtered = compactFields(filtered, map[string]bool{"CreateTime": true, "InsertionOrderId": true})
				}
				if len(filtered) > 0 {
					var parsed any
