...
func newCampaignManagementCreateAssetGroupRecommendationCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "create-asset-group-recommendation",
		Short:       "create_asset_group_recommendation",
		Example:     "  bing-ads-pp-cli campaign-management create-asset-group-recommendation",
		Annotations: map[string]string{"pp:endpoint": "campaign-management.create-asset-group-recommendation", "pp:method": "POST", "pp:path": "/CampaignManagement/v13/AssetGroupRecommendation/Create"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.asJSON || (!isTerminal(cmd.OutOrStdout()) && !flags.csv && !flags.quiet && !flags.plain) {
				...
				if flags.selectFields != "" {
					...
					filtered = compactFields(filtered, map[string]bool{"AssetLink": true, "Image": true, "ImageMetadata": true, "ImageUrl": true, "Internal_Template": true})
				}
				if len(filtered) > 0 {
					var parsed any
