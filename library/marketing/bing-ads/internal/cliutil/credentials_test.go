...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/marketing/bing-ads/internal/cliutil/testenv"
...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"BING_ADS_CUSTOMER_ACCOUNT_ID",
	} {
		t.Setenv(name, "")
	}
	return home, filepath.Join(home, ".config", "bing-ads-pp-cli", "config.toml")
}
...
func TestExplicitConfigUsesSiblingCredentialsBeforeGlobal(t *testing.T) {
	...
	t.Setenv("BING_ADS_CONFIG", configPath)
...
func TestCorruptCredentialsDoesNotOverrideLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "bing-ads-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	home, _ := resetCredentialEnv(t)
	t.Setenv("BING_ADS_CUSTOMER_ACCOUNT_ID", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "bing-ads-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "bing-ads-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "bing-ads-pp-cli", "credentials.toml")
...
func legacyCredentialKey() string {
	return "ads_customer_account_id"
}
...
func configCredentialValue(cfg *config.Config) string {
	return cfg.BingAdsCustomerAccountId
}
...
func credentialValue(creds *cliutil.Credentials) string {
	return creds.BingAdsCustomerAccountId
}

func setCredentialValue(creds *cliutil.Credentials, token string) {
	creds.BingAdsCustomerAccountId = token
}
