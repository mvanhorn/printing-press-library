# Umami Printed CLI Agent Guide

This directory is a generated `umami-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
umami-pp-cli doctor --json
umami-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
umami-pp-cli which "<capability>" --json
umami-pp-cli <command> --help
```
...
```bash
umami-pp-cli <command> --help
umami-pp-cli <command> --dry-run --agent
```
