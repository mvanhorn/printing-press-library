...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  umami-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  umami-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"admin", "admin-users", "admin-websites", "boards", "links", "me", "me-websites", "pixels", "reports", "teams", "websites"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  umami-pp-cli workflow status

  # Show status as JSON
  umami-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("umami-pp-cli")
			}
