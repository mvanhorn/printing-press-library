...
Resource scoping:
  ...
  umami-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  umami-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  umami-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  umami-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  umami-pp-cli sync --latest-only`,
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"admin-websites": "/api/admin/websites",
		"boards":         "/api/boards",
		"links":          "/api/links",
		"me":             "/api/me/teams",
		"me-websites":    "/api/me/websites",
		"pixels":         "/api/pixels",
		"reports":        "/api/reports",
		"teams":          "/api/teams",
...
func responsePathForResource(resource, path string) []string {
	...
	case "admin\x00/api/admin/teams":
		return []string{"data"}
	case "admin\x00/api/admin/users":
		return []string{"data"}
	case "admin\x00/api/admin/websites":
		return []string{"data"}
	case "boards\x00/api/boards":
		return []string{"data"}
	case "event-data\x00/api/websites/{website_id}/event-data-pivot":
		return []string{"data"}
	case "events\x00/api/websites/{website_id}/events":
		return []string{"data"}
	case "links\x00/api/links":
		return []string{"data"}
	case "me\x00/api/me/teams":
		return []string{"data"}
	case "me\x00/api/me/websites":
		return []string{"data"}
	case "pixels\x00/api/pixels":
		return []string{"data"}
	case "replays\x00/api/websites/{website_id}/replays":
		return []string{"data"}
	case "replays\x00/api/websites/{website_id}/replays/saved":
		return []string{"data"}
	case "reports\x00/api/websites/{website_id}/reports":
		return []string{"data"}
	case "reports\x00/api/reports":
		return []string{"data"}
	case "sessions\x00/api/websites/{website_id}/sessions":
		return []string{"data"}
	case "shares\x00/api/websites/{website_id}/shares":
		return []string{"data"}
	case "teams\x00/api/teams":
		return []string{"data"}
	case "teams\x00/api/teams/{team_id}/users":
		return []string{"data"}
	case "teams\x00/api/teams/{team_id}/websites":
		return []string{"data"}
	case "users\x00/api/users/{user_id}/teams":
		return []string{"data"}
	case "users\x00/api/users/{user_id}/websites":
		return []string{"data"}
	case "websites\x00/api/websites":
		return []string{"data"}
	}
	return nil
}
