...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "umami-pp-cli",
		Short: `Umami CLI — Every Umami v3 API surface, plus the portfolio rollup, SEO deltas, and anomaly watch that self-hosted Umami never shipp…`,
		Long: `Umami CLI — Every Umami v3 API surface, plus the portfolio rollup, SEO deltas, and anomaly watch that self-hosted Umami never shipp…
...
Health check: run 'umami-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("umami-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'umami-pp-cli profile list')")
	...
	rootCmd.AddCommand(newEventDataCmd(flags))
	rootCmd.AddCommand(newEventsCmd(flags))
	rootCmd.AddCommand(newLinksCmd(flags))
	rootCmd.AddCommand(newMeCmd(flags))
	rootCmd.AddCommand(newPixelsCmd(flags))
	rootCmd.AddCommand(newReplaysCmd(flags))
	reportsCmd := newReportsCmd(flags)
	fixReportRunnerFilters(reportsCmd)
	rootCmd.AddCommand(reportsCmd)
	rootCmd.AddCommand(newRevenueCmd(flags))
	rootCmd.AddCommand(newSegmentsCmd(flags))
	rootCmd.AddCommand(newSessionDataCmd(flags))
	sessionsCmd := newSessionsCmd(flags)
	addQualifiedSessionsFlag(sessionsCmd)
	rootCmd.AddCommand(sessionsCmd)
	rootCmd.AddCommand(newSharesCmd(flags))
	rootCmd.AddCommand(newTeamsCmd(flags))
	rootCmd.AddCommand(newUsersCmd(flags))
	rootCmd.AddCommand(newWebsitesCmd(flags))
	rootCmd.AddCommand(newDoctorCmd(flags))
	authCmd := newAuthCmd(flags)
	authCmd.AddCommand(newAuthLoginCmd(flags))
	authCmd.AddCommand(newAuthSetTokenCmd(flags))
	...
	rootCmd.AddCommand(newNovelNewReferrersCmd(flags))
	rootCmd.AddCommand(newNovelPaceCmd(flags))
	rootCmd.AddCommand(newNovelSeoCmd(flags))
	...
	rootCmd.AddCommand(newPeakHoursCmd(flags))
	...
	sendCmd := newSendPromotedCmd(flags)
	sendCmd.AddCommand(newSendBatchCmd(flags))
	rootCmd.AddCommand(sendCmd)
	rootCmd.AddCommand(newVersionCmd())
	addPeriodFlags(rootCmd)
	addVersionGateHints(rootCmd)
