...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("UMAMI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("UMAMI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "umami-pp-cli/feedback")
...
When ` + "`UMAMI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`UMAMI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  umami-pp-cli feedback list
  umami-pp-cli feedback list --limit 5
  umami-pp-cli feedback list --json`,
