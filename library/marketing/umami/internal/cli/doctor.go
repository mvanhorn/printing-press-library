...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  umami-pp-cli doctor
  umami-pp-cli doctor --json
  umami-pp-cli doctor --fail-on warn
  umami-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set credentials with: export UMAMI_TOKEN=\"your-token-here\" UMAMI_API_KEY=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if os.Getenv("UMAMI_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "UMAMI_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "UMAMI_TOKEN")
			}
			if os.Getenv("UMAMI_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "UMAMI_API_KEY")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "UMAMI_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "umami-pp-cli"
						verifyPath := "/api/me"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'umami-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'umami-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'umami-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'umami-pp-cli sync' to refresh."
	}
	return report
}
