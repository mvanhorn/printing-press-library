...
type Config struct {
	...
	UmamiToken       string          `toml:"token"`
	UmamiApiKey      string          `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("UMAMI_TOKEN"); v != "" {
		cfg.UmamiToken = v
		cfg.markEnvOverride("UmamiToken")
		cfg.AuthSource = "env:UMAMI_TOKEN"
		cfg.CredentialSource = "env:UMAMI_TOKEN"
	}
	if v := os.Getenv("UMAMI_API_KEY"); v != "" {
		cfg.UmamiApiKey = v
		cfg.markEnvOverride("UmamiApiKey")
		cfg.AuthSource = "env:UMAMI_API_KEY"
		cfg.CredentialSource = "env:UMAMI_API_KEY"
	}
	...
	if v := os.Getenv("UMAMI_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("UMAMI_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "umami-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.UmamiToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:UMAMI_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"token":       c.UmamiToken,
			"UMAMI_TOKEN": c.UmamiToken,
		})
	}
	if c.UmamiApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:UMAMI_API_KEY"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"api_key":       c.UmamiApiKey,
			"UMAMI_API_KEY": c.UmamiApiKey,
			"token":         c.UmamiApiKey,
		})
	}
	if c.AccessToken != "" {
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		UmamiToken:    c.UmamiToken,
		UmamiApiKey:   c.UmamiApiKey,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.UmamiToken = creds.UmamiToken
	c.UmamiApiKey = creds.UmamiApiKey
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.UmamiToken = ""
	delete(c.envOverrides, "UmamiToken")
	c.updateFileConfigField("UmamiToken")
	c.UmamiApiKey = ""
	delete(c.envOverrides, "UmamiApiKey")
	c.updateFileConfigField("UmamiApiKey")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["UmamiToken"] {
			out.UmamiToken = c.fileConfig.UmamiToken
		}
		if c.envOverrides["UmamiApiKey"] {
			out.UmamiApiKey = c.fileConfig.UmamiApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "UmamiToken":
		c.fileConfig.UmamiToken = c.UmamiToken
	case "UmamiApiKey":
		c.fileConfig.UmamiApiKey = c.UmamiApiKey
	}
}
