...
type Config struct {
	...
	JudgemeApiKey      string          `toml:"api_key"`
	JudgemeOauthApikey string          `toml:"oauth_apikey"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("JUDGEME_API_KEY"); v != "" {
		cfg.JudgemeApiKey = v
		cfg.markEnvOverride("JudgemeApiKey")
		cfg.AuthSource = "env:JUDGEME_API_KEY"
		cfg.CredentialSource = "env:JUDGEME_API_KEY"
	}
	if v := os.Getenv("JUDGEME_OAUTH_APIKEY"); v != "" {
		cfg.JudgemeOauthApikey = v
		cfg.markEnvOverride("JudgemeOauthApikey")
		cfg.AuthSource = "env:JUDGEME_OAUTH_APIKEY"
		cfg.CredentialSource = "env:JUDGEME_OAUTH_APIKEY"
	}
	...
	if v := os.Getenv("JUDGEME_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("JUDGEME_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "judgeme-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.JudgemeApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:JUDGEME_API_KEY"
		}
		// Preserve the generated credential-provider contract; the Judge.me
		// client converts this value to X-Api-Token before sending.
		return "Bearer " + c.JudgemeApiKey
	}
	if c.JudgemeOauthApikey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:JUDGEME_OAUTH_APIKEY"
		}
		return "Bearer " + c.JudgemeOauthApikey
	}
	if c.AccessToken != "" {
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		JudgemeApiKey:      c.JudgemeApiKey,
		JudgemeOauthApikey: c.JudgemeOauthApikey,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.JudgemeApiKey == "" {
		c.JudgemeApiKey = creds.JudgemeApiKey
	}
	if c.JudgemeOauthApikey == "" {
		c.JudgemeOauthApikey = creds.JudgemeOauthApikey
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.JudgemeApiKey = ""
	delete(c.envOverrides, "JudgemeApiKey")
	c.updateFileConfigField("JudgemeApiKey")
	c.JudgemeOauthApikey = ""
	delete(c.envOverrides, "JudgemeOauthApikey")
	c.updateFileConfigField("JudgemeOauthApikey")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["JudgemeApiKey"] {
			out.JudgemeApiKey = c.fileConfig.JudgemeApiKey
		}
		if c.envOverrides["JudgemeOauthApikey"] {
			out.JudgemeOauthApikey = c.fileConfig.JudgemeOauthApikey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "JudgemeApiKey":
		c.fileConfig.JudgemeApiKey = c.JudgemeApiKey
	case "JudgemeOauthApikey":
		c.fileConfig.JudgemeOauthApikey = c.JudgemeOauthApikey
	}
}
