...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("JUDGEME_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("JUDGEME_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "judgeme-pp-cli/feedback")
...
When ` + "`JUDGEME_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`JUDGEME_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  judgeme-pp-cli feedback list
  judgeme-pp-cli feedback list --limit 5
  judgeme-pp-cli feedback list --json`,
