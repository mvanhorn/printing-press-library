...
// judgeme-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/marketing/judgeme/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/marketing/judgeme/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/marketing/judgeme/internal/learn/patterns"
...
const noLearnEnvVar = "JUDGEME_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("judgeme-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  judgeme-pp-cli teach --query "<question>" --resource-type <type> --resource <id> --resource <id> &`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dryRunOK(flags) {
				if flags.asJSON {
					return printJSONFiltered(cmd.OutOrStdout(), map[string]any{"command": "teach", "dry_run": true}, flags)
				}
				return nil
			}
...
when learnings exist.`,
		Example: `  judgeme-pp-cli recall "<question>" --agent
  judgeme-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  judgeme-pp-cli learnings list --agent
  judgeme-pp-cli learnings list --query "<substring>"
  judgeme-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  judgeme-pp-cli learnings forget "<question>" --resource <id>
  judgeme-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  judgeme-pp-cli teach-pattern --query-template "items in {entity}" --resource-template "GROUP-{entity:category}" --resource-type "items" --entity-kind "category" --strategy substitute`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				if flags.asJSON {
					return printJSONFiltered(cmd.OutOrStdout(), map[string]any{"command": "teach-pattern", "dry_run": true}, flags)
				}
				return nil
			}
			if noLearnActive(flags) {
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  judgeme-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				if flags.asJSON {
					return printJSONFiltered(cmd.OutOrStdout(), map[string]any{"command": "teach-lookup", "dry_run": true}, flags)
				}
				return nil
			}
			if noLearnActive(flags) {
