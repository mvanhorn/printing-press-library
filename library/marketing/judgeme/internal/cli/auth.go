...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Judgeme",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  judgeme-pp-cli auth setup\n  judgeme-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://help.judge.me/en/articles/8278390-integrating-with-judge-me")
			fmt.Fprintln(w, "  judgeme-pp-cli auth set-token <token>")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Set one of:")
			fmt.Fprintln(w, "  export JUDGEME_API_KEY=\"your-token-here\"")
			fmt.Fprintln(w, "  export JUDGEME_OAUTH_APIKEY=\"your-token-here\"")
			if !launch {
				return nil
			}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  judgeme-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "")
				fmt.Fprintln(w, "Set one of:")
				fmt.Fprintf(w, "  judgeme-pp-cli auth set-token <token>\n")
				fmt.Fprintln(w, "")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  judgeme-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  judgeme-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("JUDGEME_API_KEY") != "" {
				envStillSet = "JUDGEME_API_KEY"
			}
			if envStillSet == "" && os.Getenv("JUDGEME_OAUTH_APIKEY") != "" {
				envStillSet = "JUDGEME_OAUTH_APIKEY"
			}
