...
Run sync first to populate the local search index.`,
  ...
  judgeme-pp-cli search "status"

  # Force local search explicitly
  judgeme-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  judgeme-pp-cli search "status" --type private-replies --data-source local
  # JSON output for piping
  judgeme-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'judgeme-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
