...
Resource scoping:
  ...
  judgeme-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  judgeme-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  judgeme-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  judgeme-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  judgeme-pp-cli sync --latest-only`,
...
func responsePathForResource(resource, path string) []string {
	...
	case "reviews\x00/reviews":
		return []string{"reviews"}
	case "webhooks\x00/webhooks":
