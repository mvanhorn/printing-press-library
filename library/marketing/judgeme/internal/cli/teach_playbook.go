...
agents can record playbooks without a file on disk.`,
		Example: `  judgeme-pp-cli teach-playbook --query "<question that anchors the family>" --playbook-file ~/playbooks/recipe.json --notes-file ~/playbooks/recipe-notes.md`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dryRunOK(flags) {
				if flags.asJSON {
					return printJSONFiltered(cmd.OutOrStdout(), map[string]any{"command": "teach-playbook", "dry_run": true}, flags)
				}
				return nil
			}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  judgeme-pp-cli playbook amend --query "<exact recall query>" --add-note "summary endpoint envelope: data lives at .results.header, not .header"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			cmd.SilenceUsage = true
			cmd.SilenceErrors = true
			if noLearnActive(flags) {
			...
			if dryRunOK(flags) {
				if flags.asJSON {
					return printJSONFiltered(cmd.OutOrStdout(), map[string]any{"command": "playbook amend", "dry_run": true}, flags)
				}
				return nil
			}
			if strings.TrimSpace(query) == "" {
...
func newPlaybookListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example:     `  judgeme-pp-cli playbook list --agent`,
