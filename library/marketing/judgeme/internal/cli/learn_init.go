...
func newLearnConfig() *entities.Config {
	...
	learnSynonyms := map[string]string{
		"storefront visible": "published",
		"unique reviews":     "unique bodies",
	}
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"review_status": {
			{Canonical: "published", Values: []string{"live", "storefront-visible"}},
			{Canonical: "pending", Values: []string{"to-curate", "awaiting-moderation"}},
			{Canonical: "hidden", Values: []string{"unpublished", "spam"}},
			{Canonical: "archived", Values: []string{"removed-from-dashboard", "archive"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: judgeme-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: judgeme-pp-cli: learn init: %v\n", err)
		}
	})
}
