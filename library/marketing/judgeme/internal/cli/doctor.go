...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  judgeme-pp-cli doctor
  judgeme-pp-cli doctor --json
  judgeme-pp-cli doctor --fail-on warn
  judgeme-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" && cfg.JudgemeApiKey == "" {
					report["auth"] = "not configured"
					report["auth_hint"] = "Set JUDGEME_API_KEY to the private token and JUDGEME_SHOP_DOMAIN to the myshopify.com domain"
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "JUDGEME_API_KEY")
				if os.Getenv("JUDGEME_API_KEY") != "" {
					authEnvSet = append(authEnvSet, "JUDGEME_API_KEY")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("JUDGEME_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "JUDGEME_API_KEY")
			} else {
				authEnvInfo = append(authEnvInfo, "JUDGEME_API_KEY optional")
			}
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "JUDGEME_OAUTH_APIKEY")
				if os.Getenv("JUDGEME_OAUTH_APIKEY") != "" {
					authEnvSet = append(authEnvSet, "JUDGEME_OAUTH_APIKEY")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("JUDGEME_OAUTH_APIKEY") != "" {
				authEnvSet = append(authEnvSet, "JUDGEME_OAUTH_APIKEY")
			} else {
				authEnvInfo = append(authEnvInfo, "JUDGEME_OAUTH_APIKEY optional")
			}
			if os.Getenv("JUDGEME_SHOP_DOMAIN") != "" {
				authEnvSet = append(authEnvSet, "JUDGEME_SHOP_DOMAIN")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "JUDGEME_SHOP_DOMAIN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "judgeme-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'judgeme-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'judgeme-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'judgeme-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'judgeme-pp-cli sync' to refresh."
	}
	return report
}
