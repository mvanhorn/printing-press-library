...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/marketing/judgeme/internal/cliutil/testenv"
...
func TestMCPPathResolutionMatchesCLIResolverWithPlatformDefaults(t *testing.T) {
	...
	if want := filepath.Join(home, ".config", "judgeme-pp-cli", "config.toml"); cfg.Path != want {
	...
	if want := filepath.Join(home, ".local", "share", "judgeme-pp-cli", "data.db"); gotDB != want {
...
func TestMCPRegisterToolsPreservesTypedSpecialTools(t *testing.T) {
	s := server.NewMCPServer("judgeme", "test")
	RegisterTools(s)
...
func TestMCPMutationRequiresExplicitApplyBeforeClientConstruction(t *testing.T) {
	handler := makeAPIHandler("PUT", "/reviews/{id}", false, false, nil, mcpPageConfig{}, nil, []string{"id"})
	...
	if result == nil || !result.IsError {
		t.Fatalf("result IsError = %v, want true", result != nil && result.IsError)
	}
	if text := mcpTextContent(t, result); !strings.Contains(text, "apply=true") {
		t.Fatalf("mutation gate response = %q", text)
	}
}

func TestFilterJudgeMeMCPReviewPopulation(t *testing.T) {
	data := json.RawMessage(`[
		{"id":1,"published":true,"hidden":false},
		{"id":2,"published":false,"hidden":true},
		{"id":3,"published":false,"hidden":false}
	]`)
	for population, wantID := range map[string]float64{
		"published": 1,
		"hidden":    2,
		"pending":   3,
	} {
		filtered, err := filterJudgeMeMCPReviewPopulation(data, population)
		if err != nil {
			t.Fatal(err)
		}
		...
		if len(rows) != 1 || rows[0]["id"] != wantID {
			t.Fatalf("%s rows = %#v", population, rows)
		}
	}
}
