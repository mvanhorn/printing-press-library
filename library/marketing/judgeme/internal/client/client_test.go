...
func TestJudgeMePrivateAuthUsesHeaderAndInjectsShopDomain(t *testing.T) {
	t.Setenv("JUDGEME_SHOP_DOMAIN", "shop.example.myshopify.com")
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if got := r.Header.Get("Authorization"); got != "" {
			t.Fatalf("Authorization must be empty for a private Judge.me key, got %q", got)
		}
		if got := r.URL.Query().Get("shop_domain"); got != "shop.example.myshopify.com" {
			t.Fatalf("shop_domain = %q", got)
		}
		if strings.Contains(r.URL.RawQuery, token) {
			t.Fatal("private token leaked into request URL")
		}
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"ok":true}`))
	}))
	...
	c := New(&config.Config{BaseURL: server.URL, JudgemeApiKey: token}, time.Second, 0)
	c.HTTPClient = server.Client()
	c.NoCache = true
	if _, err := c.Get(context.Background(), "/reviews/count", nil); err != nil {
		t.Fatal(err)
	}
}
