...
var readCommandResources = map[string][]string{
	"sensortower-pp-cli categories":        {"categories"},
	"sensortower-pp-cli categories list":   {"categories"},
	"sensortower-pp-cli categories get":    {"categories"},
	"sensortower-pp-cli categories search": {"categories"},
}
...
func cachePolicy() cliutil.Policy {
	...
	envOptOut := "SENSORTOWER_NO_AUTO_REFRESH"
...
func autoRefreshIfStale(ctx context.Context, flags *rootFlags, resources []string) (meta cliutil.FreshnessMeta) {
	...
	if err := runAutoRefresh(refreshCtx, flags, db, resources); err != nil {
		fmt.Fprintf(os.Stderr, "warning: using stale sensortower-pp-cli cache (refresh failed: %v)\n", err)
