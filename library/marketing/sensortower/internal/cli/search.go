...
In local mode: searches locally synced data only.`,
  ...
  sensortower-pp-cli search "error timeout"

  # Force local search only
  sensortower-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  sensortower-pp-cli search "status" --type apps --data-source local
  # JSON output for piping
  sensortower-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /api/autocomplete_search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/api/autocomplete_search", map[string]string{
					"term": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'sensortower-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
