...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Sensortower",
...
func requiredAuthCookies() []string {
	raw := []string{"sensor_tower_session"}
	out := raw[:0]
...
Use --chrome to read cookies from Chrome for .sensortower.com.
...
profile by name when the installed backend supports it.`,
		Example: `  sensortower-pp-cli auth login --chrome
  sensortower-pp-cli auth login --browser
  sensortower-pp-cli auth login --cookies-file storage-state.json
  sensortower-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !browserFlag && cookiesFile == "" {
				...
				fmt.Fprintf(cmd.OutOrStdout(), "  sensortower-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  sensortower-pp-cli auth login --browser\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  sensortower-pp-cli auth login --cookies-file storage-state.json\n")
				return nil
			}
			if browserFlag && cookiesFile != "" {
			...
			requiredCookies := []string{"sensor_tower_session"}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  sensortower-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  sensortower-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: .sensortower.com\n")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  sensortower-pp-cli auth set-token eyJhbGciOi...",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  sensortower-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
