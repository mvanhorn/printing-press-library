...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SENSORTOWER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SENSORTOWER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "sensortower-pp-cli/feedback")
...
When ` + "`SENSORTOWER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SENSORTOWER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  sensortower-pp-cli feedback list
  sensortower-pp-cli feedback list --limit 5
  sensortower-pp-cli feedback list --json`,
