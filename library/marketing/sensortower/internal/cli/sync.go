...
Resource scoping:
  ...
  sensortower-pp-cli sync

  # Sync specific resources only
  sensortower-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  sensortower-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  sensortower-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  sensortower-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  sensortower-pp-cli sync --latest-only`,
...
func responsePathForResource(resource, path string) []string {
	...
	case "apps\x00/api/android/apps":
		return []string{"apps"}
	case "apps\x00/api/ios/apps":
		return []string{"apps"}
	case "apps\x00/api/unified/apps":
		return []string{"apps"}
	case "categories\x00/api/docs/static/category_ids.json":
		return []string{"category_ids"}
	case "find\x00/api/autocomplete_search":
		return []string{"data.entities"}
	case "publishers\x00/api/ios/publishers/{publisher_id}/apps":
		return []string{"data"}
	case "publishers\x00/api/unified/publishers":
		return []string{"publishers"}
	case "rankings\x00/api/android/category_rankings":
		return []string{"data"}
	case "rankings\x00/api/ios/category_rankings":
		return []string{"data"}
	}
	return nil
}
