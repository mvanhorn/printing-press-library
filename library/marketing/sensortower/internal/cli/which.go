...
var whichIndex = []whichEntry{
	{Command: "movers", Description: "See which apps climbed, fell, or newly appeared in a category chart since the previous 'movers' run.", Group: "History the dashboard never kept", WhyItMatters: "Reach for this when the question is 'who is new or moving in this category', which the dashboard cannot answer because it keeps no history."},
	{Command: "teardown", Description: "Align an app's release timeline against its rank history to see which shipped version moved the needle.", Group: "History the dashboard never kept", WhyItMatters: "Reach for this when a competitor spikes and you need to know whether a release, not luck, explains it."},
	{Command: "watch digest", Description: "Print every tracked app's current rank per chart plus its delta since the previous 'movers' run.", Group: "History the dashboard never kept", WhyItMatters: "Reach for this for the recurring Monday check across a whole tracked set, instead of re-reading ranks app by app."},
	{Command: "divergence", Description: "Find monetization outliers in a category by comparing each app's free-chart rank against its grossing-chart rank.", Group: "Exact ranks over fuzzy money", WhyItMatters: "Reach for this to spot install-rich but monetization-poor apps (or the inverse) without trusting Sensor Tower's coarse revenue estimates."},
	{Command: "compare", Description: "Compare one product's iOS and Android standing side by side after resolving cross-platform identity.", Group: "Cross-platform", WhyItMatters: "Reach for this when asked which platform is winning for the same app; requires a session cookie via auth login --chrome."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  sensortower-pp-cli which "stale tickets"
  sensortower-pp-cli which "bottleneck"
  sensortower-pp-cli which --limit 1 "send message"
  sensortower-pp-cli which                                # list the full capability index`,
