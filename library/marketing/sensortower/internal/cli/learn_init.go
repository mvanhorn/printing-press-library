...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^[0-9]{8,12}$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: sensortower-pp-cli: ticker pattern %q failed to compile: %v\n", `^[0-9]{8,12}$`, err)
	} else {
		cfg.RegisterTickerPattern(re)
	}
	if re, err := regexp.Compile(`^[a-z][a-z0-9_]*(\.[a-z0-9_]+)+$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: sensortower-pp-cli: ticker pattern %q failed to compile: %v\n", `^[a-z][a-z0-9_]*(\.[a-z0-9_]+)+$`, err)
	} else {
		cfg.RegisterTickerPattern(re)
	}
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"chart_type": {
			{Canonical: "topfreeapplications", Values: []string{"free", "top free", "free apps"}},
			{Canonical: "toppaidapplications", Values: []string{"paid", "top paid", "paid apps"}},
			{Canonical: "topgrossingapplications", Values: []string{"grossing", "top grossing", "revenue"}},
		},
		"platform": {
			{Canonical: "ios", Values: []string{"iphone", "apple", "itunes", "app store"}},
			{Canonical: "android", Values: []string{"google", "play", "google play"}},
			{Canonical: "unified", Values: []string{"both", "cross-platform", "combined"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: sensortower-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: sensortower-pp-cli: learn init: %v\n", err)
		}
	})
}
