...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("see_which_release_moved_the_chart",
			mcplib.WithDescription("Aligns the versions timeline against locally stored rank history so you can see whether a ship date precedes a climb."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Override the recipe's positional id value.")),
		),
		handleSeeWhichReleaseMovedTheChart,
	)
	s.AddTool(
		mcplib.NewTool("compare_the_same_product_across_platforms",
			mcplib.WithDescription("Resolves cross-platform identity via the cookie-gated unified endpoint, then shows both rank ladders side by side."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Override the recipe's positional id value.")),
			mcplib.WithString("domain", mcplib.Required(), mcplib.Description("Override the recipe's positional domain value.")),
		),
		handleCompareTheSameProductAcrossPlatforms,
	)
}

// handleSeeWhichReleaseMovedTheChart runs the see_which_release_moved_the_chart recipe intent: Aligns the versions timeline against locally stored rank history so you can see whether a ship date precedes a climb.
func handleSeeWhichReleaseMovedTheChart(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
...
// handleCompareTheSameProductAcrossPlatforms runs the compare_the_same_product_across_platforms recipe intent: Resolves cross-platform identity via the cookie-gated unified endpoint, then shows both rank ladders side by side.
func handleCompareTheSameProductAcrossPlatforms(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	if recipeCLIPathErr != nil {
		return mcplib.NewToolResultError(fmt.Sprintf("companion CLI binary not found: %v", recipeCLIPathErr)), nil
	}
