...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "google-reviews-pp-cli",
		Short: "Google Reviews CLI with review fetching, rating summaries, and local sync",
		Long: `Google Reviews CLI with review fetching, rating summaries, and local sync.

Fetch reviews and rating data from Google Maps businesses — no API key required.
Add --agent to any command for JSON output + non-interactive mode.
Run 'google-reviews-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("google-reviews-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'google-reviews-pp-cli profile list')")
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("google-reviews-pp-cli %s\n", version)
		},
	}
}
