...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GOOGLE_REVIEWS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GOOGLE_REVIEWS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "google-reviews-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.google-reviews-pp-cli/feedback.jsonl.
When ` + "`GOOGLE_REVIEWS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GOOGLE_REVIEWS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  google-reviews-pp-cli feedback list
  google-reviews-pp-cli feedback list --limit 5
  google-reviews-pp-cli feedback list --json`,
