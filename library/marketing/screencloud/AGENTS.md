# ScreenCloud Printed CLI Agent Guide

This directory is a generated `screencloud-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
screencloud-pp-cli doctor --json
screencloud-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
screencloud-pp-cli which "<capability>" --json
screencloud-pp-cli <command> --help
```
...
```bash
screencloud-pp-cli <command> --agent
```
...
```bash
screencloud-pp-cli <command> --help
screencloud-pp-cli <command> --dry-run --agent
```
...
3. After answering, always fire `screencloud-pp-cli teach --query "<question>" --resource <id> --resource-type <type> &` in the background - teaching is unconditional and is the anchor that triggers playbook synthesis. Teach the structural question with identifiers stripped (no names, emails, phone numbers, account ids); the CLI warns on obvious PII shapes but does not block.
