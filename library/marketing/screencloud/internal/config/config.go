...
type Config struct {
	...
	ScreencloudApiKey string          `toml:"api_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://graphql.us.screencloud.com",
	}
	...
	// Env var overrides
	if v := os.Getenv("SCREENCLOUD_API_KEY"); v != "" {
		cfg.ScreencloudApiKey = v
		cfg.markEnvOverride("ScreencloudApiKey")
		cfg.AuthSource = "env:SCREENCLOUD_API_KEY"
		cfg.CredentialSource = "env:SCREENCLOUD_API_KEY"
	}
	...
	if v := os.Getenv("SCREENCLOUD_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("SCREENCLOUD_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "screencloud-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.ScreencloudApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:SCREENCLOUD_API_KEY"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"api_key":             c.ScreencloudApiKey,
			"SCREENCLOUD_API_KEY": c.ScreencloudApiKey,
			"token":               c.ScreencloudApiKey,
		})
	}
	if c.AccessToken != "" {
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		ScreencloudApiKey: c.ScreencloudApiKey,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.ScreencloudApiKey = creds.ScreencloudApiKey
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.ScreencloudApiKey = ""
	delete(c.envOverrides, "ScreencloudApiKey")
	c.updateFileConfigField("ScreencloudApiKey")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["ScreencloudApiKey"] {
			out.ScreencloudApiKey = c.fileConfig.ScreencloudApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "ScreencloudApiKey":
		c.fileConfig.ScreencloudApiKey = c.ScreencloudApiKey
	}
}
