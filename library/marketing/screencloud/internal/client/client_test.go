...
func TestNoCacheClientDoesNotPersistFreshResponses(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"private":"playgrounds-source"}`))
	}))
	t.Cleanup(server.Close)
	c := New(&config.Config{BaseURL: server.URL}, time.Second, 0)
	c.HTTPClient = server.Client()
	...
	c.NoCache = true
	if _, err := c.GetWithHeadersNoCache(context.Background(), "/files/app-1", nil, nil); err != nil {
		t.Fatal(err)
	}
	...
	if len(entries) != 0 {
		t.Fatalf("private response was cached: %v", entries)
	}
}

func TestCrossHostRedirectStripsAllConfiguredCredentials(t *testing.T) {
	...
	c := New(&config.Config{
		BaseURL:       source.URL,
		AuthHeaderVal: "Bearer studio-secret",
	...
	c.NoCache = true
	if _, err := c.Get(context.Background(), "/redirect", nil); err != nil {
		t.Fatal(err)
	}
	for _, key := range []string{"Authorization", "X-Custom-Secret", "Cookie", "Proxy-Authorization"} {
		if got := received.Get(key); got != "" {
			t.Fatalf("cross-host redirect leaked %s=%q", key, got)
		}
	}
}

func TestHTTPSRedirectDowngradeIsRejected(t *testing.T) {
	targetHit := false
	target := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		targetHit = true
	}))
	...
	t.Cleanup(source.Close)
	c := New(&config.Config{BaseURL: source.URL, AuthHeaderVal: "Bearer studio-secret"}, time.Second, 0)
	...
	if err == nil || !strings.Contains(err.Error(), "HTTPS-to-HTTP") {
		t.Fatalf("credential downgrade was not rejected: %v", err)
	}
	if targetHit {
		t.Fatal("downgrade redirect reached the cleartext target")
	}
}

func TestCredentialLikeConfiguredHeadersAreMasked(t *testing.T) {
	c := New(&config.Config{Headers: map[string]string{
		"X-API-Key":      "custom-api-secret",
		"X-Signature":    "signed-secret",
		"X-Harmless-Tag": "public-value",
	}}, time.Second, 0)
	masked := c.maskCredentialText("custom-api-secret signed-secret public-value")
	if strings.Contains(masked, "custom-api-secret") || strings.Contains(masked, "signed-secret") {
		t.Fatalf("configured credential header value leaked: %s", masked)
	}
	if !strings.Contains(masked, "public-value") {
		t.Fatalf("non-credential header was unnecessarily masked: %s", masked)
	}
}
