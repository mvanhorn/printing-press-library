...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	httpClient.CheckRedirect = func(req *http.Request, via []*http.Request) error {
		...
		if strings.EqualFold(via[0].URL.Scheme, "https") && strings.EqualFold(req.URL.Scheme, "http") {
			return errors.New("refusing HTTPS-to-HTTP redirect because it would downgrade credential transport")
		}
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/marketing/screencloud/0.1.0")
		}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export SCREENCLOUD_API_KEY=<your-token> or screencloud-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.ScreencloudApiKey)
		for name, value := range c.Config.Headers {
			if credentialLikeHeaderName(name) {
				addCredential(value)
			}
		}
	}
...
func credentialLikeHeaderName(name string) bool {
	normalized := strings.NewReplacer("-", "", "_", "").Replace(strings.ToLower(strings.TrimSpace(name)))
	for _, marker := range []string{"authorization", "apikey", "token", "secret", "cookie", "signature", "credential"} {
