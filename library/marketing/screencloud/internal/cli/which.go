...
var whichIndex = []whichEntry{
	{Command: "playgrounds impact", Description: "Map the synchronized spaces, channels, playlists, and screens related to a reviewed Playgrounds working copy before publishing.", Group: "Release confidence", WhyItMatters: "Use this before a publish when an agent must explain the known synchronized placement graph and its completeness, not merely show a file fingerprint."},
	{Command: "playgrounds readiness", Description: "Find missing, inactive, outdated, dangling, and inconsistent Playgrounds deployments across the organization.", Group: "Fleet intelligence", WhyItMatters: "Use this instead of separate list calls when an agent needs an actionable organization-wide health verdict."},
	{Command: "playgrounds config-drift", Description: "Detect structurally divergent Playgrounds configurations without storing or revealing private values.", Group: "Fleet intelligence", WhyItMatters: "Use this when an agent must compare a fleet safely without pulling sensitive configuration values into context."},
	{Command: "playgrounds create-reconcile", Description: "Turn a partial create receipt into a resume or cleanup plan; a no-op conclusion requires live verification.", Group: "Safe recovery", WhyItMatters: "Use this after an interrupted two-service create workflow instead of guessing which mutation is safe to repeat. The example uses a shipped fixture; use your own redacted receipt path for real recovery."},
	{Command: "playgrounds contract-check", Description: "Verify the current management and viewer content contracts after minting ephemeral scoped JWTs, without changing Playgrounds content.", Group: "Release confidence", WhyItMatters: "Use this before automation relies on the bundle-derived Playgrounds contract or after an unexpected response shape."},
	{Command: "playgrounds preview-drift", Description: "Find unpublished previews, production-ahead conflicts, and preview work that has waited too long.", Group: "Release confidence", WhyItMatters: "Use this when an agent needs a fleet-level preview queue rather than inspecting preview and production workspaces one at a time."},
	{Command: "auth capabilities", Description: "Explain whether the current identity appears able to run a supported mapped command without exposing token material or raw effective grants.", Group: "Safe automation", WhyItMatters: "Use this before automation or a guarded mutation needs a least-privilege preflight; it is diagnostic evidence, not authorization or a guarantee of mutation success."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  screencloud-pp-cli which "stale tickets"
  screencloud-pp-cli which "bottleneck"
  screencloud-pp-cli which --limit 1 "send message"
  screencloud-pp-cli which                                # list the full capability index`,
