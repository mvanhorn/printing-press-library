...
// screencloud-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/marketing/screencloud/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/marketing/screencloud/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/marketing/screencloud/internal/learn/patterns"
...
const noLearnEnvVar = "SCREENCLOUD_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("screencloud-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  screencloud-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  screencloud-pp-cli recall "<question>" --agent
  screencloud-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "list",
		Short: "Search and filter recorded agent learnings",
		Example: `  screencloud-pp-cli learnings list --agent
  screencloud-pp-cli learnings list --query "<substring>"
  screencloud-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  screencloud-pp-cli learnings forget "<question>" --resource <id>
  screencloud-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  screencloud-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  screencloud-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
