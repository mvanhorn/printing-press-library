...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  screencloud-pp-cli doctor
  screencloud-pp-cli doctor --json
  screencloud-pp-cli doctor --fail-on warn
  screencloud-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set it with: screencloud-pp-cli auth set-token <token> or export SCREENCLOUD_API_KEY=\"your-token-here\""
					report["auth_docs_url"] = "https://graphql.us.screencloud.com"
				} else {
					authConfigured = true
			...
			if os.Getenv("SCREENCLOUD_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "SCREENCLOUD_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "SCREENCLOUD_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "screencloud-pp-cli", suggestion)
						} else {
