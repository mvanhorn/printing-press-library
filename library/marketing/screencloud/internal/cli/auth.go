...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for ScreenCloud",
	...
	cmd.AddCommand(newAuthInspectCmd(flags))
	cmd.AddCommand(newNovelAuthCapabilitiesCmd(flags))
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  screencloud-pp-cli auth setup\n  screencloud-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "See API docs: https://graphql.us.screencloud.com")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export SCREENCLOUD_API_KEY=\"your-token-here\"")
			fmt.Fprintln(w, "  screencloud-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://graphql.us.screencloud.com"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  screencloud-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export SCREENCLOUD_API_KEY=\"your-token-here\"")
				fmt.Fprintf(w, "  screencloud-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  screencloud-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  screencloud-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("SCREENCLOUD_API_KEY") != "" {
				envStillSet = "SCREENCLOUD_API_KEY"
			}
