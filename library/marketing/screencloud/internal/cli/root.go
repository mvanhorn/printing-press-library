...
type rootFlags struct {
	...
	// invocation. Mirrors the SCREENCLOUD_NO_LEARN env var.
	noLearn       bool
	selectFields  string
	configPath    string
	homePath      string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "screencloud-pp-cli",
		Short: `ScreenCloud CLI — A Playgrounds release and fleet-operations CLI that understands both Studio and the app content service.`,
		Long: `ScreenCloud CLI — A Playgrounds release and fleet-operations CLI that understands both Studio and the app content service.
...
Agent mode: add --agent for JSON output + non-interactive mode; live mutations still require --yes.
Health check: run 'screencloud-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("screencloud-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().BoolVar(&flags.agent, "agent", false, "Set safe agent-friendly defaults (--json --compact --no-input --no-color); mutations still require --yes")
	rootCmd.PersistentFlags().BoolVar(&flags.noLearn, "no-learn", false, "Disable the teach/recall learning loop for this invocation")
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'screencloud-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelPlaygroundsCmd(flags))
