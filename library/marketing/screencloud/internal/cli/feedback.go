...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SCREENCLOUD_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SCREENCLOUD_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/marketing/screencloud/feedback")
...
When ` + "`SCREENCLOUD_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SCREENCLOUD_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  screencloud-pp-cli feedback list
  screencloud-pp-cli feedback list --limit 5
  screencloud-pp-cli feedback list --json`,
