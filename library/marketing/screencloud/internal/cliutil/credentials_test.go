...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"SCREENCLOUD_CONFIG",
		"SCREENCLOUD_CONFIG_DIR",
		"SCREENCLOUD_DATA_DIR",
		"SCREENCLOUD_STATE_DIR",
		"SCREENCLOUD_CACHE_DIR",
		"SCREENCLOUD_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
	...
	return home, filepath.Join(home, ".config", "screencloud-pp-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "screencloud-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("SCREENCLOUD_API_KEY", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "screencloud-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "screencloud-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "screencloud-pp-cli", "credentials.toml")
