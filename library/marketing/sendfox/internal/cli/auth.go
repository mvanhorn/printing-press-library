...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			setupURL := "https://sendfox.com/account/oauth"
			fmt.Fprintln(w, "Get your Personal Access Token at:")
			fmt.Fprintln(w, "  "+setupURL)
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then run the interactive setup wizard:")
			fmt.Fprintln(w, "  sendfox-pp-cli setup")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Or set the token directly:")
			fmt.Fprintln(w, "  export SENDFOX_TOKEN=\"<your-token>\"")
			fmt.Fprintln(w, "  sendfox-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			fmt.Fprintf(w, "Opening %s...\n", setupURL)
			fmt.Fprintln(w, "(copy the URL above and open it in your browser)")
			return nil
		},
	}
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export SENDFOX_TOKEN=\"your-token-here\"")
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("SENDFOX_TOKEN") != "" {
				envStillSet = "SENDFOX_TOKEN"
			}
