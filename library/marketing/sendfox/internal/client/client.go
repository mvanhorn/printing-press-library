...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "sendfox-pp-cli")
...
func (c *Client) writeCache(path string, params map[string]string, data json.RawMessage) {
	...
	// Tighten pre-existing directory permissions for users upgrading from
	// before the 0o700 fix. MkdirAll does not change modes on existing dirs.
	os.Chmod(c.cacheDir, 0o700)
	cacheFile := filepath.Join(c.cacheDir, c.cacheKey(path, params)+".json")
	os.WriteFile(cacheFile, []byte(data), 0o600)
	// Tighten pre-existing file permissions for the same reason.
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "sendfox-pp-cli/1.3.0")
		}
