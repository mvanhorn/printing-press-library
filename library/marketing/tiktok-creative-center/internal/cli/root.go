...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "tiktok-creative-center-pp-cli",
		Short: `Trending TikTok hashtags (with popularity curves + audience demographics) and the Top Ads competitor library — synced to a local store, with cross-entity niche reports and trend velocity no web UI of…`,
		Long: `Trending TikTok hashtags (with popularity curves + audience demographics) and the Top Ads competitor library — synced to a local store, with cross-entity niche reports and trend velocity no web UI of…
...
Highlights (not in the official API docs):
  ...
  • decide   Given a niche, output a concrete recommendation: which hashtags to ride, what content formats are working, where the saturation gaps are, how competitors are positioned, and what account angle is sti…

Agent mode: add --agent to any command for JSON output + non-interactive mode.
Health check: run 'tiktok-creative-center-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("tiktok-creative-center-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'tiktok-creative-center-pp-cli profile list')")
	...
	rootCmd.AddCommand(newTopAdsCmd(flags))
	...
	rootCmd.AddCommand(newNovelCompetitorCmd(flags))
	rootCmd.AddCommand(newNovelContentCmd(flags))
	rootCmd.AddCommand(newNovelDecideCmd(flags))
	rootCmd.AddCommand(newNovelNicheCmd(flags))
	...
	rootCmd.AddCommand(newNovelViralCmd(flags))
