...
import (
	...
	"encoding/json"
	"fmt"
	"github.com/mvanhorn/printing-press-library/library/marketing/tiktok-creative-center/internal/cliutil"
...
Resource scoping:
  ...
  tiktok-creative-center-pp-cli sync

  # Sync specific resources only
  tiktok-creative-center-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  tiktok-creative-center-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  tiktok-creative-center-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  tiktok-creative-center-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  tiktok-creative-center-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("tiktok-creative-center-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/tiktok-creative-center-pp-cli/data.db)")
...
func upsertResourceBatch(db *store.Store, resource string, items []json.RawMessage) (int, int, error) {
	// Hand-fix: top-ads keys on the nested itemInfo.itemID; the store's
	// ID extraction is top-level only. Hoist it to a top-level id so rows
	// cache. See internal/cli/topads_flatten.go.
	if resource == "top-ads" {
		items = flattenTopAdsIDs(items)
	}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"hashtags": "/CreativeOne/KnowledgeAPI/GetHashtagList",
		// top-ads declares a resource-level base_url override
		// (ads.us.tiktok.com, not the client default ads.tiktok.com) in the
		// spec; the generated top-ads_list command bakes the absolute URL
		// in for the same reason. Without the absolute URL here, sync hit
		// the wrong host and got "invalid page or limit" from every request.
		"top-ads": "https://ads.us.tiktok.com/CreativeOne/Report/CreativeCenterGetTopContentsList",
	}
...
var pageItemKeys = []string{
	...
	"Data", "Results", "Items", "Records", "Nodes", "Entries", "Features",
	// TikTok Creative Center Top Contents wraps its item array in
	// "entityInfos" (spec response_path: entityInfos); the generic detector
	// above misses it, so sync stored the whole response blob as one row.
	"entityInfos",
}

var dataEnvelopeKeys = []string{"data", "Data", "result", "Result",
	// TikTok Creative Center wraps its payload one level deeper than the
	// generic "data"/"result" convention: {meta, results:{BaseResp,
	// entityInfos:[...]}}. Without "results" here, extractPageItems never
	// descends into the nested entityInfos array.
	"results", "Results",
}
