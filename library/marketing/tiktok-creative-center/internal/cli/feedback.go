...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "tiktok-creative-center-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TIKTOK_CREATIVE_CENTER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TIKTOK_CREATIVE_CENTER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "tiktok-creative-center-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/tiktok-creative-center-pp-cli/feedback.jsonl.
When ` + "`TIKTOK_CREATIVE_CENTER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TIKTOK_CREATIVE_CENTER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "tiktok-creative-center-pp-cli",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  tiktok-creative-center-pp-cli feedback list
  tiktok-creative-center-pp-cli feedback list --limit 5
  tiktok-creative-center-pp-cli feedback list --json`,
