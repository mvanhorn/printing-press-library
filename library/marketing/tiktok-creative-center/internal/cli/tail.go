...
native streaming instead of polling.`,
  ...
  tiktok-creative-center-pp-cli tail --interval 10s

  # Tail a specific resource
  tiktok-creative-center-pp-cli tail messages --interval 5s

  # Pipe to jq for filtering
  tiktok-creative-center-pp-cli tail events --interval 30s | jq 'select(.type == "error")'`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Initial fetch. In one-shot mode (!follow) this is the only chance
			// to report failure, so an invalid resource must surface as a
			// non-zero exit instead of a warning; in follow mode a transient
			// failure shouldn't kill a long-running poll loop.
