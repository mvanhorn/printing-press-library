...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("TIKTOK_CREATIVE_CENTER_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "tiktok-creative-center-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("TIKTOK_CREATIVE_CENTER_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
