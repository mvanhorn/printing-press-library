...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	var failOn string
	var probeFamilies bool
	cmd := &cobra.Command{
		Use:   "doctor",
		Short: "Check CLI health",
		Example: `  appsflyer-pp-cli doctor
  appsflyer-pp-cli doctor --json
  appsflyer-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export APPSFLYER_API_TOKEN=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("APPSFLYER_API_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "APPSFLYER_API_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "APPSFLYER_API_TOKEN")
			}
			switch {
			...
			// AppsFlyer-specific extras: dotenv path, daily call budget, and
			// optional per-family permission probes (each probe consumes one
			// API call from the daily budget, so they are opt-in).
			{
				var afClient *client.Client
				if cfg != nil {
					afClient, _ = flags.newClient()
				}
				reportAppsFlyerExtras(report, afClient, probeFamilies)
			}
			...
			for _, key := range []string{"config_path", "base_url", "auth_source", "dotenv_path", "version"} {
				if v, ok := report[key]; ok {
					fmt.Fprintf(w, "  %s: %v\n", key, v)
				}
			}
			// AppsFlyer extras: dotenv status (with optional hint) and budget block.
			if dot, ok := report["dotenv"].(string); ok {
				if hint, hok := report["dotenv_hint"].(string); hok && dot != "loaded" {
					fmt.Fprintf(w, "  dotenv: %s\n  hint: %s\n", dot, hint)
				} else {
					fmt.Fprintf(w, "  dotenv: %s\n", dot)
				}
			}
			if bud, ok := report["budget"].(map[string]any); ok {
				fmt.Fprintf(w, "  budget: %v used / %v remaining (limit %v, date %v)\n", bud["used"], bud["remaining"], bud["limit"], bud["date"])
			}
			if probes, ok := report["permission_probes"]; ok {
				switch v := probes.(type) {
				case string:
					fmt.Fprintf(w, "  permission_probes: %s\n", v)
				case map[string]string:
					for family, result := range v {
						fmt.Fprintf(w, "  permission_probes.%s: %s\n", family, result)
					}
				}
			}
	...
	cmd.Flags().BoolVar(&probeFamilies, "probe-families", false, "Probe each API report family with one call (consumes one budget call per family)")
	return cmd
}
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("appsflyer-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'appsflyer-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'appsflyer-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'appsflyer-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'appsflyer-pp-cli sync' to refresh."
	}
	return report
}
