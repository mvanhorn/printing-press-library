...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".appsflyer-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("APPSFLYER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("APPSFLYER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "appsflyer-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.appsflyer-pp-cli/feedback.jsonl.
When ` + "`APPSFLYER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`APPSFLYER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  appsflyer-pp-cli feedback list
  appsflyer-pp-cli feedback list --limit 5
  appsflyer-pp-cli feedback list --json`,
