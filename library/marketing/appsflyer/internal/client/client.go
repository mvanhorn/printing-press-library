...
// SetBudget attaches a daily-call-budget tracker. Successful API calls then
// decrement the budget; dry-run requests do not charge. Pass nil to clear.
func (c *Client) SetBudget(t *budget.Tracker) { c.budget = t }

// Budget returns the attached budget tracker, or nil when none is configured.
func (c *Client) Budget() *budget.Tracker { return c.budget }
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "appsflyer-pp-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "appsflyer-pp-cli/0.1.0")
		}
