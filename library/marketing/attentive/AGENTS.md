# Attentive Printed CLI Agent Guide

This directory is a generated `attentive-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
attentive-pp-cli doctor --json
attentive-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
attentive-pp-cli which "<capability>" --json
attentive-pp-cli <command> --help
```
...
```bash
attentive-pp-cli <command> --agent
```
...
```bash
attentive-pp-cli <command> --help
attentive-pp-cli <command> --dry-run --agent
```
