...
type Config struct {
	...
	AttentiveBearerAuth string            `toml:"bearer_auth"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.attentivemobile.com",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("ATTENTIVE_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "attentive-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("ATTENTIVE_BEARER_AUTH"); v != "" {
		cfg.AttentiveBearerAuth = v
		cfg.markEnvOverride("AttentiveBearerAuth")
		cfg.AuthSource = "env:ATTENTIVE_BEARER_AUTH"
	}
	...
	if cfg.AuthSource == "" && cfg.AttentiveBearerAuth != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("ATTENTIVE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.AttentiveBearerAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:ATTENTIVE_BEARER_AUTH"
		}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["AttentiveBearerAuth"] {
			out.AttentiveBearerAuth = c.fileConfig.AttentiveBearerAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "AttentiveBearerAuth":
		c.fileConfig.AttentiveBearerAuth = c.AttentiveBearerAuth
	}
}
