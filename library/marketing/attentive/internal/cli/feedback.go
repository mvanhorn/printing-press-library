...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "attentive-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ATTENTIVE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ATTENTIVE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "attentive-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/attentive-pp-cli/feedback.jsonl.
When ` + "`ATTENTIVE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ATTENTIVE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  attentive-pp-cli feedback list
  attentive-pp-cli feedback list --limit 5
  attentive-pp-cli feedback list --json`,
