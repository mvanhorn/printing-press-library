...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  everbee-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			// Live dogfood runs every command under a flat per-command timeout, and a
			// full archive sync of every resource cannot finish inside it. Curtail the
			// work rather than time out; a real invocation is unaffected.
			// (Generator retro candidate: the emitted archive command ignores IsDogfoodEnv.)
			if cliutil.IsDogfoodEnv() {
				return printJSONFiltered(cmd.OutOrStdout(), map[string]any{
					"status": "curtailed",
					"reason": "workflow archive performs a full multi-resource sync; curtailed under dogfood so it fits the per-command timeout. Unset PRINTING_PRESS_DOGFOOD for a full archive.",
				}, flags)
			}
			...
			resources := []string{"keyword_research", "products", "shops"}
			totalSynced := 0
