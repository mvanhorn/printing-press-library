...
agents can record playbooks without a file on disk.`,
		Example: `  everbee-pp-cli teach-playbook --query "dad shirt niche" --notes "seed with research niche before drilling into subniches"
  # attach a full playbook: --playbook-file ~/playbooks/recipe.json --notes-file ~/playbooks/recipe-notes.md`,
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  everbee-pp-cli playbook amend --query "dad shirt niche" --add-note "summary endpoint envelope: data lives at .results.header, not .header"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			cmd.SilenceUsage = true
			cmd.SilenceErrors = true
			if noLearnActive(flags) {
...
func newPlaybookListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example:     `  everbee-pp-cli playbook list --agent`,
