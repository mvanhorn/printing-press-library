...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"product_type": {
			{Canonical: "physical", Values: []string{"apparel", "shirt", "t-shirt", "tee", "sweatshirt", "hoodie", "mug", "poster", "print", "physical product"}},
			{Canonical: "digital", Values: []string{"svg", "png", "download", "digital download", "clipart", "cut file", "printable", "digital product"}},
		},
		"sort_field": {
			{Canonical: "est_mo_revenue", Values: []string{"revenue", "monthly revenue", "est revenue", "earnings"}},
			{Canonical: "est_mo_sales", Values: []string{"sales", "monthly sales", "units sold"}},
			{Canonical: "new_volume", Values: []string{"volume", "search volume", "demand"}},
			{Canonical: "competition", Values: []string{"competing listings", "competition", "saturation"}},
		},
		"time_range": {
			{Canonical: "last_1_month", Values: []string{"last month", "past month", "30 days", "monthly"}},
			{Canonical: "last_3_months", Values: []string{"last 3 months", "past quarter", "quarterly"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: everbee-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: everbee-pp-cli: learn init: %v\n", err)
		}
	})
}
