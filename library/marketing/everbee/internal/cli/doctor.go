...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
  ...
  everbee-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set your API key with: export EVERBEE_ACCESS_TOKEN=\"your-token-here\""
					report["auth_docs_url"] = "https://api.everbee.com"
					report["auth_instructions"] = "Obtain a token after Google SSO with `everbee-pp-cli auth setup`, then store it with `everbee-pp-cli auth set-token <token>`."
				} else {
					authConfigured = true
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "everbee-pp-cli"
						verifyPath := "/users/show"
			...
			report["cache"] = collectCacheReport(cmd.Context(), "15m")
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'everbee-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
