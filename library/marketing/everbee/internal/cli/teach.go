...
// everbee-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/marketing/everbee/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/marketing/everbee/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/marketing/everbee/internal/learn/patterns"
...
const noLearnEnvVar = "EVERBEE_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("everbee-pp-cli")
}
...
func newTeachCmd(flags *rootFlags, learnCfg *entities.Config) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Annotations: map[string]string{"pp:typed-exit-codes": "0,2", "mcp:local-write": "true", "pp:happy-args": "--query=dad shirt niche;--resource-type=keywords;--resource=dad shirt"},
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  everbee-pp-cli teach --query "dad shirt niche" --resource-type keywords --resource "dad shirt"
  # fire-and-forget from an agent: append & to background; silent on success`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Explicit --json overrides the silent default: machine callers
			// asking for JSON need an ack envelope, not empty stdout.
			if flags.asJSON {
...
when learnings exist.`,
		Example: `  everbee-pp-cli recall "<question>" --agent
  everbee-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  everbee-pp-cli learnings list --agent
  everbee-pp-cli learnings list --query "<substring>"
  everbee-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  everbee-pp-cli learnings forget "<question>" --resource <id>
  everbee-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
func newTeachPatternCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		// dogfood cannot synthesize --query-template; declare a fixture so the
		// happy-path probe can run. (Generator retro candidate: required-flag
		// framework commands ship without pp:happy-args.)
		Annotations: map[string]string{"pp:happy-args": "--query-template=low competition {niche};--resource-template=products;--resource-type=products;--entity-kind=niche"},
		Use:         "teach-pattern",
...
a whole family.`,
		Example: `  everbee-pp-cli teach-pattern --query-template "items in {entity}" --resource-template "GROUP-{entity:category}" --resource-type "items" --entity-kind "category" --strategy substitute`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  everbee-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
