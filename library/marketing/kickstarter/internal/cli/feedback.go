...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".kickstarter-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("KICKSTARTER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("KICKSTARTER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "kickstarter-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.kickstarter-pp-cli/feedback.jsonl.
When ` + "`KICKSTARTER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`KICKSTARTER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  kickstarter-pp-cli feedback list
  kickstarter-pp-cli feedback list --limit 5
  kickstarter-pp-cli feedback list --json`,
