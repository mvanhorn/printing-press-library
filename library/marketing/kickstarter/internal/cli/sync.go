...
func newSyncCmd(flags *rootFlags) *cobra.Command {
	...
	var latestOnly bool
	var strict bool
	// --since-last and --diff are hand-written extensions added by the
	// novel-commands phase. When both are set, sync emits only the delta
	// (new + changed projects) since the last successful sync_runs row,
	// as JSONL one-per-line, and writes a project_snapshots row per
	// project so funding-rank can later compute velocity deltas.
	var sinceLast bool
	var diffMode bool
...
Exit codes & warnings:
  ...
  kickstarter-pp-cli sync

  # Sync specific resources only
  kickstarter-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  kickstarter-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  kickstarter-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  kickstarter-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  kickstarter-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("kickstarter-pp-cli")
			}
			...
			defer db.Close()
			// Ensure v2 hand-written tables exist (sync_runs,
			// project_snapshots, vertical_match, magazine_articles,
			// magazine_fts). Idempotent — no-op on already-current DBs.
			if err := db.EnsureV2Schema(cmd.Context()); err != nil {
				return fmt.Errorf("ensuring v2 schema: %w", err)
			}

			// Capture the "since-last" watermark BEFORE this run completes
			// so the delta-emission step below can compare new state
			// against the prior watermark, not the watermark this run is
			// about to write.
			var sinceLastWatermark time.Time
			if sinceLast || diffMode {
				w, werr := db.LastSyncRun("")
				if werr == nil {
					sinceLastWatermark = w
				}
			}
			...
			// Diff emission + snapshot capture. Runs only when both
			// --since-last and --diff are set, after the regular sync
			// finished. Emits one JSONL "delta" event per project that
			// is newer than the prior sync_runs watermark, then writes a
			// project_snapshots row per project so funding-rank can
			// compute the velocity delta on the next sync pass.
			now := time.Now()
			if sinceLast && diffMode {
				emitSyncDelta(cmd, db, sinceLastWatermark, now)
			}
			// Always record the sync run so subsequent --since-last
			// invocations have a watermark, even when --diff was not set.
			for _, r := range resources {
				_ = db.RecordSyncRun(r, totalSynced, now)
			}
			return nil
		},
	}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/kickstarter-pp-cli/data.db)")
	...
	cmd.Flags().BoolVar(&sinceLast, "since-last", false, "Only consider records newer than the last successful sync (uses sync_runs watermark).")
	cmd.Flags().BoolVar(&diffMode, "diff", false, "When combined with --since-last, emit one JSONL `delta` event per new/changed project on stdout, and snapshot every project into project_snapshots.")

	return cmd
}

// emitSyncDelta scans the local store for projects newer or changed
// since the prior watermark and emits one JSONL line per project to the
// command's stdout. Also writes a project_snapshots row per project at
// the snapshot timestamp so funding-rank can later compute deltas.
//
// Lightweight by design: a single scan over all discover rows, no extra
// network. Failures during snapshot write are logged to stderr but do
// not fail the sync run — the sync_runs watermark is still authoritative
// for "did we run".
func emitSyncDelta(cmd *cobra.Command, db *store.Store, watermark, snapshotAt time.Time) {
	// Expand page-level envelopes into per-project rows first so the
	// snapshot pass below covers everything the sync just observed,
	// not only previously-seen projects.
	if _, err := db.ExpandDiscoverProjectsFromEnvelopes(); err != nil {
		fmt.Fprintf(cmd.ErrOrStderr(), "warn: expanding discover envelopes: %v\n", err)
	}
	projects, err := db.LoadDiscoverProjects(0)
	if err != nil {
		fmt.Fprintf(cmd.ErrOrStderr(), "warn: loading projects for diff: %v\n", err)
		return
	}
	enc := json.NewEncoder(cmd.OutOrStdout())
	wmUnix := watermark.Unix()
	deltaCount := 0
	for _, p := range projects {
		isNewOrChanged := false
		if wmUnix == 0 {
			isNewOrChanged = true // first-ever sync: every project is a delta
		} else {
			if p.LaunchedAt > wmUnix || p.StateChangedAt > wmUnix {
				isNewOrChanged = true
			}
		}
		if isNewOrChanged {
			...
			_ = enc.Encode(map[string]any{
				...
				"name":        p.Name,
				"state":       p.State,
				"launched_at": p.LaunchedAt,
			})
		}
		// Snapshot every project regardless of delta status — funding-
		// rank wants a complete time series, not only the new ones.
		if err := db.UpsertProjectSnapshot(p.ID, snapshotAt, p.Pledged, p.BackersCount, p.State); err != nil {
			fmt.Fprintf(cmd.ErrOrStderr(), "warn: snapshot project %d: %v\n", p.ID, err)
		}
	}
	_ = enc.Encode(map[string]any{
		"event":     "delta_summary",
