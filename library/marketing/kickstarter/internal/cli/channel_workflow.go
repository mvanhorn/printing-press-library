...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  kickstarter-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  kickstarter-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"discover", "magazine"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  kickstarter-pp-cli workflow status

  # Show status as JSON
  kickstarter-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("kickstarter-pp-cli")
			}
