...
// TestMigrate_AddsColumnsOnUpgrade_ListLeadsJson verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_ListLeadsJson(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "list_leads_json" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	rows, err := s.DB().Query(`PRAGMA table_info("list_leads_json")`)
	if err != nil {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from list_leads_json after migrate", want)
		}
	}
}
