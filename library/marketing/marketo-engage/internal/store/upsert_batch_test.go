...
// TestUpsertBatch_PopulatesListLeadsJsonTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed list_leads_json table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesListLeadsJsonTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("list_leads_json", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "list_leads_json").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "list_leads_json")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count list_leads_json: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("list_leads_json count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandListLeadsJsonGeneric exercises
...
func TestUpsertBatch_TypedFailureDoesNotStrandListLeadsJsonGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	stored, extractFailures, err := s.UpsertBatch("list_leads_json", items)
	if err != nil {
	...
	if typed != 0 {
		t.Fatalf("list_leads_json count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "list_id")
	}
}

// TestUpsertBatch_KeysListLeadsJsonByChildAndParent verifies dependent
...
func TestUpsertBatch_KeysListLeadsJsonByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("list_leads_json count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "list_id" = ?`, "list_leads_json")
