...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "marketo-engage-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MARKETO_ENGAGE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MARKETO_ENGAGE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "marketo-engage-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/marketo-engage-pp-cli/feedback.jsonl.
When ` + "`MARKETO_ENGAGE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MARKETO_ENGAGE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  marketo-engage-pp-cli feedback list
  marketo-engage-pp-cli feedback list --limit 5
  marketo-engage-pp-cli feedback list --json`,
