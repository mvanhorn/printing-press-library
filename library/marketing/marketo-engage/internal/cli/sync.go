...
Resource scoping:
  ...
  marketo-engage-pp-cli sync

  # Sync specific resources only
  marketo-engage-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  marketo-engage-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  marketo-engage-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  marketo-engage-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  marketo-engage-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("marketo-engage-pp-cli")
			}
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "marketo-engage-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/marketo-engage-pp-cli/data.db)")
...
func resourceSupportsPagination(resource string) bool {
	...
	case "asset":
		return true
	case "asset-v1-emails-json":
		return true
	case "asset-v1-folders-json":
		return true
	case "asset-v1-landing-pages-json":
		return true
	case "asset-v1-programs-json":
		return true
	case "asset-v1-smart-lists-json":
		return true
	}
	return false
}
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "list_leads_json":
		return db.UpsertListLeadsJson(data)
	default:
...
func knownSyncResourceNames() []string {
	names := []string{
		...
		"asset-v1-landing-pages-json",
		"asset-v1-programs-json",
		"asset-v1-smart-lists-json",
		"campaigns-json",
		"leads-json",
		"lists-json",
	}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"activities":                  "/v1/activities/types.json",
		"activities-json":             "/v1/activities.json",
		"asset":                       "/asset/v1/forms.json",
		"asset-v1-emails-json":        "/asset/v1/emails.json",
		"asset-v1-folders-json":       "/asset/v1/folders.json",
		"asset-v1-landing-pages-json": "/asset/v1/landingPages.json",
		"asset-v1-programs-json":      "/asset/v1/programs.json",
		"asset-v1-smart-lists-json":   "/asset/v1/smartLists.json",
		"campaigns-json":              "/v1/campaigns.json",
		"leads-json":                  "/v1/leads.json",
		"lists-json":                  "/v1/lists.json",
	}
