...
type Config struct {
	...
	MarketoEngageBearerAuth string            `toml:"engage_bearer_auth"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://example.mktorest.com/rest",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("MARKETO_ENGAGE_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "marketo-engage-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("MARKETO_ENGAGE_BEARER_AUTH"); v != "" {
		cfg.MarketoEngageBearerAuth = v
		cfg.markEnvOverride("MarketoEngageBearerAuth")
		cfg.AuthSource = "env:MARKETO_ENGAGE_BEARER_AUTH"
	}
	...
	if cfg.AuthSource == "" && cfg.MarketoEngageBearerAuth != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("MARKETO_ENGAGE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.MarketoEngageBearerAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:MARKETO_ENGAGE_BEARER_AUTH"
		}
		return "Bearer " + c.MarketoEngageBearerAuth
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientID")
	c.updateFileConfigField("ClientSecret")
	c.MarketoEngageBearerAuth = ""
	delete(c.envOverrides, "MarketoEngageBearerAuth")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["MarketoEngageBearerAuth"] {
			out.MarketoEngageBearerAuth = c.fileConfig.MarketoEngageBearerAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "MarketoEngageBearerAuth":
		c.fileConfig.MarketoEngageBearerAuth = c.MarketoEngageBearerAuth
	}
}
