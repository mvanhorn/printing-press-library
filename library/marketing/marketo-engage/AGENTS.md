# Marketo Engage Printed CLI Agent Guide

This directory is a generated `marketo-engage-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
marketo-engage-pp-cli doctor --json
marketo-engage-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
marketo-engage-pp-cli which "<capability>" --json
marketo-engage-pp-cli <command> --help
```
...
```bash
marketo-engage-pp-cli <command> --agent
```
...
```bash
marketo-engage-pp-cli <command> --help
marketo-engage-pp-cli <command> --dry-run --agent
```
