...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "semrush-pp-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		// SEMrush public API returns semicolon-delimited CSV by default;
		// auto-convert to a JSON array so downstream --json/--select/--compact
		// see structured fields instead of an opaque CSV string. No-op for
		// JSON, HTML, or empty bodies.
		respBody = cliutil.MaybeConvertSemrushCSV(respBody)
