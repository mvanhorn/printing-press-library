# Seranking Printed CLI Agent Guide

This directory is a generated `seranking-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
seranking-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
seranking-pp-cli which "<capability>" --json
seranking-pp-cli <command> --help
```
...
```bash
seranking-pp-cli <command> --agent
```
...
```bash
seranking-pp-cli <command> --help
seranking-pp-cli <command> --dry-run --agent
```
