...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".seranking-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SERANKING_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SERANKING_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "seranking-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.seranking-pp-cli/feedback.jsonl.
When ` + "`SERANKING_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SERANKING_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  seranking-pp-cli feedback list
  seranking-pp-cli feedback list --limit 5
  seranking-pp-cli feedback list --json`,
