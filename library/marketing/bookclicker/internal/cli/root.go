...
type rootFlags struct {
	...
	// invocation. Mirrors the BOOKCLICKER_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "bookclicker-pp-cli",
		Short: `Bookclicker CLI — Every Bookclicker workflow, plus a local mirror of the newsletter marketplace that makes launch planning a single query.`,
		Long: `Bookclicker CLI — Every Bookclicker workflow, plus a local mirror of the newsletter marketplace that makes launch planning a single query.
...
Health check: run 'bookclicker-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("bookclicker-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'bookclicker-pp-cli profile list')")
	...
	rootCmd.AddCommand(newConfirmPromosCmd(flags))
	rootCmd.AddCommand(newExternalReservationsCmd(flags))
	rootCmd.AddCommand(newInventoriesCmd(flags))
	rootCmd.AddCommand(newListsCmd(flags))
	rootCmd.AddCommand(newPenNamesCmd(flags))
	...
	rootCmd.AddCommand(newBookingCalendarsPromotedCmd(flags))
	...
	rootCmd.AddCommand(newMyListsPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelConfirmDueCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelDriftCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelLaunchCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelPartnerRoiCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelPlanCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelStaleCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSwapBalanceCmd(flags))
