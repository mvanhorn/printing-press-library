...
Resource scoping:
  ...
  bookclicker-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  bookclicker-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  bookclicker-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  bookclicker-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  bookclicker-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "booking_calendars":
		return db.UpsertBookingCalendars(data)
	case "books":
	...
	case "external_reservations":
		return db.UpsertExternalReservations(data)
	case "inventories":
		return db.UpsertInventories(data)
	case "lists":
		return db.UpsertLists(data)
	case "my_lists":
		return db.UpsertMyLists(data)
	case "pen_names":
		return db.UpsertPenNames(data)
	case "reservations":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		...
		"pen_names-list-for-buyer": "/api/pen_names/list_for_buyer",
	}
...
func responsePathForResource(resource, path string) []string {
	...
	case "lists\x00/api/lists":
		return []string{"lists"}
	case "my_lists\x00/api/my_lists":
		return []string{"lists"}
	case "pen_names\x00/api/pen_names/list_for_buyer":
		return []string{"pen_names"}
	case "pen_names\x00/api/pen_names":
		return []string{"pen_names"}
	}
	return nil
}
