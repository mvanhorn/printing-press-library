...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  bookclicker-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  bookclicker-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"books", "lists", "my_lists", "pen_names", "pen_names-list-for-buyer"}
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  bookclicker-pp-cli workflow status

  # Show status as JSON
  bookclicker-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("bookclicker-pp-cli")
			}
