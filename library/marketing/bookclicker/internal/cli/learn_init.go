...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^[0-9]{4,7}$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: bookclicker-pp-cli: ticker pattern %q failed to compile: %v\n", `^[0-9]{4,7}$`, err)
	} else {
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"inv_type": {
			{Canonical: "solo", Values: []string{"solo promo", "solo send", "dedicated"}},
			{Canonical: "feature", Values: []string{"featured", "feature spot"}},
			{Canonical: "mention", Values: []string{"mentions", "link mention"}},
		},
		"platform": {
			{Canonical: "mailerlite", Values: []string{"mailer lite", "ml"}},
			{Canonical: "mailchimp", Values: []string{"mail chimp", "mc"}},
			{Canonical: "convertkit", Values: []string{"convert kit", "kit", "ck"}},
			{Canonical: "activecampaign", Values: []string{"active campaign", "ac"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: bookclicker-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: bookclicker-pp-cli: learn init: %v\n", err)
		}
	})
}
