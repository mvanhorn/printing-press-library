...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BOOKCLICKER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BOOKCLICKER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "bookclicker-pp-cli/feedback")
...
When ` + "`BOOKCLICKER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BOOKCLICKER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  bookclicker-pp-cli feedback list
  bookclicker-pp-cli feedback list --limit 5
  bookclicker-pp-cli feedback list --json`,
