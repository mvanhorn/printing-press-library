...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  bookclicker-pp-cli doctor
  bookclicker-pp-cli doctor --json
  bookclicker-pp-cli doctor --fail-on warn
  bookclicker-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if !configured {
					...
					report["auth_hint"] = "Set your API key with: export BOOKCLICKER_SESSION=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if os.Getenv("BOOKCLICKER_SESSION") != "" {
				authEnvSet = append(authEnvSet, "BOOKCLICKER_SESSION")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "BOOKCLICKER_SESSION")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "bookclicker-pp-cli"
						verifyPath := "/api/user"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'bookclicker-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'bookclicker-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		...
		report["hint"] = "No sync recorded; run 'bookclicker-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'bookclicker-pp-cli sync' to refresh."
	}
	return report
}
