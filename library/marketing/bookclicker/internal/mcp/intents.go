...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("find_partners_who_cancel_swaps",
			mcplib.WithDescription("Lists counterparties who agreed to a swap and then cancelled or declined it."),
			mcplib.WithBoolean("flaky", mcplib.Description("Override the recipe's --flaky value.")),
		),
		handleFindPartnersWhoCancelSwaps,
	)
}

// handleFindPartnersWhoCancelSwaps runs the find_partners_who_cancel_swaps recipe intent: Lists counterparties who agreed to a swap and then cancelled or declined it.
func handleFindPartnersWhoCancelSwaps(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	args := []string{}
	args = append(args, "swap-balance")
	args = appendRecipeBoolFlag(args, "flaky", input["flaky"], true)
