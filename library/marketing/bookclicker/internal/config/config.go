...
type Config struct {
	...
	BookclickerSession string          `toml:"session"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://www.bookclicker.com",
	}
	...
	// Env var overrides
	if v := os.Getenv("BOOKCLICKER_SESSION"); v != "" {
		cfg.BookclickerSession = v
		cfg.markEnvOverride("BookclickerSession")
		cfg.AuthSource = "env:BOOKCLICKER_SESSION"
		cfg.CredentialSource = "env:BOOKCLICKER_SESSION"
	}
	...
	if v := os.Getenv("BOOKCLICKER_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("BOOKCLICKER_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "bookclicker-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.BookclickerSession
	if token == "" {
		return ""
	}
	if c.BookclickerSession == "" {
		return ""
	}
	replacements := map[string]string{
		"token":               token,
		"session":             c.BookclickerSession,
		"BOOKCLICKER_SESSION": c.BookclickerSession,
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		BookclickerSession: c.BookclickerSession,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.BookclickerSession == "" {
		c.BookclickerSession = creds.BookclickerSession
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.BookclickerSession = token
	delete(c.envOverrides, "BookclickerSession")
	c.updateFileConfigField("BookclickerSession")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["BookclickerSession"] {
			out.BookclickerSession = c.fileConfig.BookclickerSession
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "BookclickerSession":
		c.fileConfig.BookclickerSession = c.BookclickerSession
	}
}
