# Syndicate Links Printed CLI Agent Guide

This directory is a generated `syndicate-links-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
syndicate-links-pp-cli doctor --json
syndicate-links-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
syndicate-links-pp-cli which "<capability>" --json
syndicate-links-pp-cli <command> --help
```
...
```bash
syndicate-links-pp-cli <command> --agent
```
...
```bash
syndicate-links-pp-cli <command> --help
syndicate-links-pp-cli <command> --dry-run --agent
```
