...
import (
	...
	"runtime"
	"os"
	"github.com/mvanhorn/printing-press-library/library/marketing/syndicate-links/internal/cliutil"
...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Syndicate Links",
	}
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  syndicate-links-pp-cli auth setup\n  syndicate-links-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "See API docs: https://syndicatelinks.co")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export SYNDICATE_LINKS_BEARER_AUTH=\"<your-token>\"")
			fmt.Fprintln(w, "  syndicate-links-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://syndicatelinks.co"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  syndicate-links-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export SYNDICATE_LINKS_BEARER_AUTH=\"your-token-here\"")
				fmt.Fprintf(w, "  syndicate-links-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  syndicate-links-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  syndicate-links-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("SL_API_KEY") != "" {
				envStillSet = "SL_API_KEY"
			} else if envStillSet == "" && os.Getenv("SYNDICATE_LINKS_BEARER_AUTH") != "" {
				envStillSet = "SYNDICATE_LINKS_BEARER_AUTH"
			}
