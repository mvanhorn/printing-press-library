...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".syndicate-links-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SYNDICATE_LINKS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SYNDICATE_LINKS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/marketing/syndicate-links/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.syndicate-links-pp-cli/feedback.jsonl.
When ` + "`SYNDICATE_LINKS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SYNDICATE_LINKS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  syndicate-links-pp-cli feedback list
  syndicate-links-pp-cli feedback list --limit 5
  syndicate-links-pp-cli feedback list --json`,
