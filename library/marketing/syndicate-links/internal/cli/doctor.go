...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/marketing/syndicate-links/internal/client"
	"github.com/mvanhorn/printing-press-library/library/marketing/syndicate-links/internal/config"
...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  syndicate-links-pp-cli doctor
  syndicate-links-pp-cli doctor --json
  syndicate-links-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export SYNDICATE_LINKS_BEARER_AUTH=<your-key>"
					report["auth_docs_url"] = "https://syndicatelinks.co"
				} else {
					authConfigured = true
			...
			if os.Getenv("SYNDICATE_LINKS_BEARER_AUTH") != "" {
				authEnvSet = append(authEnvSet, "SYNDICATE_LINKS_BEARER_AUTH")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "SYNDICATE_LINKS_BEARER_AUTH")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'syndicate-links-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'syndicate-links-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'syndicate-links-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'syndicate-links-pp-cli sync' to refresh."
	}
	return report
}
