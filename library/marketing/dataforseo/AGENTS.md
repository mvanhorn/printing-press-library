# DataForSEO Printed CLI Agent Guide

This directory is a generated `dataforseo-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
dataforseo-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
dataforseo-pp-cli which "<capability>" --json
dataforseo-pp-cli <command> --help
```
...
```bash
dataforseo-pp-cli <command> --agent
```
...
```bash
dataforseo-pp-cli <command> --help
dataforseo-pp-cli <command> --dry-run --agent
```
