...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  dataforseo-pp-cli doctor
  dataforseo-pp-cli doctor --json
  dataforseo-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export DATAFORSEO_LOGIN=<your-login> DATAFORSEO_PASSWORD=<your-password>"
				} else {
					authConfigured = true
			...
			if v := os.Getenv("DATAFORSEO_LOGIN"); v != "" {
				_ = v
				authEnvSet = append(authEnvSet, "DATAFORSEO_LOGIN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "DATAFORSEO_LOGIN")
			}
			if v := os.Getenv("DATAFORSEO_PASSWORD"); v != "" {
				_ = v
				authEnvSet = append(authEnvSet, "DATAFORSEO_PASSWORD")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "DATAFORSEO_PASSWORD")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'dataforseo-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'dataforseo-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'dataforseo-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'dataforseo-pp-cli sync' to refresh."
	}
	return report
}
