...
In local mode: searches locally synced data only.`,
  ...
  dataforseo-pp-cli search "error timeout"

  # Force local search only
  dataforseo-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  dataforseo-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  dataforseo-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /v3/content_analysis/search/live
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post("/v3/content_analysis/search/live", map[string]any{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'dataforseo-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()

			// PATCH: An empty type searches all locally indexed resource types.
