...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".dataforseo-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DATAFORSEO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DATAFORSEO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "dataforseo-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.dataforseo-pp-cli/feedback.jsonl.
When ` + "`DATAFORSEO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DATAFORSEO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  dataforseo-pp-cli feedback list
  dataforseo-pp-cli feedback list --limit 5
  dataforseo-pp-cli feedback list --json`,
