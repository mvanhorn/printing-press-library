...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"ZOHO_CAMPAIGNS_CONFIG",
		"ZOHO_CAMPAIGNS_CONFIG_DIR",
		"ZOHO_CAMPAIGNS_DATA_DIR",
		"ZOHO_CAMPAIGNS_STATE_DIR",
		"ZOHO_CAMPAIGNS_CACHE_DIR",
		"ZOHO_CAMPAIGNS_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
	...
	return home, filepath.Join(home, ".config", "zoho-campaigns-pp-cli", "config.toml")
}
...
func TestCredentialsFileWinsWhenLegacyConfigAlsoHasSecrets(t *testing.T) {
	...
	if err := cliutil.RestrictPrivateFile(configPath); err != nil {
		t.Fatalf("restrict fixture config: %v", err)
	}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "zoho-campaigns-pp-cli", "credentials.toml")
	...
	if err := cliutil.RestrictPrivateFile(credentialsPath); err != nil {
		t.Fatalf("restrict fixture credentials: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	if err := cliutil.RestrictPrivateFile(defaultConfigPath); err != nil {
		t.Fatalf("restrict fixture config: %v", err)
	}
	...
	activeConfigPath := filepath.Join(newConfigDir, "zoho-campaigns-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "zoho-campaigns-pp-cli", "credentials.toml")
