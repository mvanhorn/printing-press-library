...
func LoadCredentials() (*Credentials, bool, error) {
	...
	if permErr := VerifyCredsPerms(real); permErr != nil {
		// Self-heal before refusing — but only when the offending ACEs are all
		// INHERITED: a same-owner copy (backup restore, sandbox seeding,
		// profile migration) silently drops the protected owner-only ACL and
		// inherits the destination directory's ACEs. Explicit broad grants are
		// deliberate broadening (the token may be compromised) and keep the
		// fail-closed refusal. Without healing the copy case, every invocation
		// falls back to a fresh token mint — a thundering herd that trips
		// OAuth token-endpoint rate limits.
		healed := false
		if CredsPermsIssueIsInheritedOnly(real) {
			if healErr := RestrictPrivateFile(real); healErr == nil {
				healed = VerifyCredsPerms(real) == nil
			}
		}
		if !healed {
