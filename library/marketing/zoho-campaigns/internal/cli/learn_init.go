...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^3z[0-9a-f]{40,}$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: zoho-campaigns-pp-cli: ticker pattern %q failed to compile: %v\n", `^3z[0-9a-f]{40,}$`, err)
	} else {
		cfg.RegisterTickerPattern(re)
	}
	if re, err := regexp.Compile(`^[0-9]{16,19}$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: zoho-campaigns-pp-cli: ticker pattern %q failed to compile: %v\n", `^[0-9]{16,19}$`, err)
	} else {
		cfg.RegisterTickerPattern(re)
	}
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"campaign_status": {
			{Canonical: "sent", Values: []string{"delivered", "sent out"}},
			{Canonical: "drafts", Values: []string{"draft", "unsent"}},
			{Canonical: "scheduled", Values: []string{"queued", "upcoming"}},
			{Canonical: "inprogress", Values: []string{"sending", "in progress"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: zoho-campaigns-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: zoho-campaigns-pp-cli: learn init: %v\n", err)
		}
	})
}
