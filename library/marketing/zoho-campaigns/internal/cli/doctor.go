...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  zoho-campaigns-pp-cli doctor
  zoho-campaigns-pp-cli doctor --json
  zoho-campaigns-pp-cli doctor --fail-on warn
  zoho-campaigns-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				if cfg.RefreshToken == "" {
					...
					report["auth_hint"] = "export ZOHO_CAMPAIGNS_REFRESH_TOKEN=<your-oauth-refresh-value>"
			...
			if os.Getenv("ZOHO_CAMPAIGNS_CLIENT_ID") != "" {
				authEnvSet = append(authEnvSet, "ZOHO_CAMPAIGNS_CLIENT_ID")
			} else {
				authEnvInfo = append(authEnvInfo, "ZOHO_CAMPAIGNS_CLIENT_ID set during auth login")
			}
			if os.Getenv("ZOHO_CAMPAIGNS_CLIENT_SECRET") != "" {
				authEnvSet = append(authEnvSet, "ZOHO_CAMPAIGNS_CLIENT_SECRET")
			} else {
				authEnvInfo = append(authEnvInfo, "ZOHO_CAMPAIGNS_CLIENT_SECRET set during auth login")
			}
			if os.Getenv("ZOHO_CAMPAIGNS_REFRESH_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "ZOHO_CAMPAIGNS_REFRESH_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "ZOHO_CAMPAIGNS_REFRESH_TOKEN set during auth login")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					healthPath := "/recentcampaigns?resfmt=JSON"
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "zoho-campaigns-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'zoho-campaigns-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'zoho-campaigns-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'zoho-campaigns-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'zoho-campaigns-pp-cli sync' to refresh."
	}
	return report
}
