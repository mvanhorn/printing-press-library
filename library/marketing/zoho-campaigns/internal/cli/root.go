...
type rootFlags struct {
	...
	// invocation. Mirrors the ZOHO_CAMPAIGNS_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "zoho-campaigns-pp-cli",
		Short: `Every Zoho Campaigns operation, plus report history, offline search, and headless self-refreshing OAuth no other tool has.`,
		Long: `Every Zoho Campaigns operation, plus report history, offline search, and headless self-refreshing OAuth no other tool has.
...
Health check: run 'zoho-campaigns-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("zoho-campaigns-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'zoho-campaigns-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelBounceAuditCmd(flags))
	rootCmd.AddCommand(newNovelDeltaCmd(flags))
	rootCmd.AddCommand(newNovelDigestCmd(flags))
	rootCmd.AddCommand(newNovelEngagementCmd(flags))
	rootCmd.AddCommand(newNovelGrowthCmd(flags))
	rootCmd.AddCommand(newNovelJourneyCmd(flags))
