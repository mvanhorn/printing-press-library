...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ZOHO_CAMPAIGNS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ZOHO_CAMPAIGNS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "zoho-campaigns-pp-cli/feedback")
...
When ` + "`ZOHO_CAMPAIGNS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ZOHO_CAMPAIGNS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  zoho-campaigns-pp-cli feedback list
  zoho-campaigns-pp-cli feedback list --limit 5
  zoho-campaigns-pp-cli feedback list --json`,
