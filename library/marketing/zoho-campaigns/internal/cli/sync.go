...
Resource scoping:
  ...
  zoho-campaigns-pp-cli sync

  # Sync specific resources only
  zoho-campaigns-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  zoho-campaigns-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  zoho-campaigns-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  zoho-campaigns-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  zoho-campaigns-pp-cli sync --latest-only`,
...
func knownSyncResourceNames() []string {
	names := []string{
		"campaigns",
		"contacts",
		"contacts-getlistadvanceddetails?resfmt=json",
		"contacts-getlistsubscribers?resfmt=json",
		"tags",
	}
	return names
}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		...
		"contacts-getlistadvanceddetails?resfmt=json": "/getlistadvanceddetails?resfmt=JSON",
		"contacts-getlistsubscribers?resfmt=json":     "/getlistsubscribers?resfmt=JSON",
...
func responsePathForResource(resource, path string) []string {
	...
	case "campaigns\x00/recentcampaigns?resfmt=JSON":
		return []string{"recent_campaigns"}
	case "contacts\x00/getmailinglists?resfmt=JSON":
		return []string{"list_of_details"}
	case "contacts\x00/getlistsubscribers?resfmt=JSON":
		return []string{"list_of_details"}
	}
	return nil
}
