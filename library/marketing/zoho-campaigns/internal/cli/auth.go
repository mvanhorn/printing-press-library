...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Zoho Campaigns",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  zoho-campaigns-pp-cli auth setup\n  zoho-campaigns-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "No setup URL is configured for this CLI; check the API's docs.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export ZOHO_CAMPAIGNS_CLIENT_ID=\"your-token-here\"")
			fmt.Fprintln(w, "  export ZOHO_CAMPAIGNS_CLIENT_SECRET=\"your-token-here\"")
			fmt.Fprintln(w, "  export ZOHO_CAMPAIGNS_REFRESH_TOKEN=\"your-token-here\"")
			fmt.Fprintln(w, "")
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  zoho-campaigns-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export ZOHO_CAMPAIGNS_CLIENT_ID=\"your-token-here\" # OAuth client ID.")
				fmt.Fprintln(w, "  export ZOHO_CAMPAIGNS_CLIENT_SECRET=\"your-token-here\" # OAuth client secret.")
				fmt.Fprintln(w, "  export ZOHO_CAMPAIGNS_REFRESH_TOKEN=\"your-token-here\" # OAuth refresh token.")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  zoho-campaigns-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  zoho-campaigns-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("ZOHO_CAMPAIGNS_CLIENT_ID") != "" {
				envStillSet = "ZOHO_CAMPAIGNS_CLIENT_ID"
			}
			if envStillSet == "" && os.Getenv("ZOHO_CAMPAIGNS_CLIENT_SECRET") != "" {
				envStillSet = "ZOHO_CAMPAIGNS_CLIENT_SECRET"
			}
			if envStillSet == "" && os.Getenv("ZOHO_CAMPAIGNS_REFRESH_TOKEN") != "" {
				envStillSet = "ZOHO_CAMPAIGNS_REFRESH_TOKEN"
			}
