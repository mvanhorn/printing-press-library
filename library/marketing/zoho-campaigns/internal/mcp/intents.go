...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("find_the_dead_weight",
			mcplib.WithDescription("Contacts who never opened anything across all synced campaigns — prune candidates."),
			mcplib.WithBoolean("never_opened", mcplib.Description("Override the recipe's --never-opened value.")),
		),
		handleFindTheDeadWeight,
	)
}

// handleFindTheDeadWeight runs the find_the_dead_weight recipe intent: Contacts who never opened anything across all synced campaigns — prune candidates.
func handleFindTheDeadWeight(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	args = appendRecipeBoolFlag(args, "never-opened", input["never_opened"], true)
