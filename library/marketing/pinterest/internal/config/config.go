...
type Config struct {
	...
	Path                     string `toml:"-"`
	PinterestPinterestOauth2 string `toml:"pinterest_oauth2"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("PINTEREST_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "pinterest-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("PINTEREST_ACCESS_TOKEN"); v != "" {
		cfg.AccessToken = v
		cfg.AuthSource = "env:PINTEREST_ACCESS_TOKEN"
	}
	if v := os.Getenv("PINTEREST_PINTEREST_OAUTH2"); v != "" {
		cfg.PinterestPinterestOauth2 = v
		cfg.AuthSource = "env:PINTEREST_PINTEREST_OAUTH2"
	}
	...
	if cfg.AuthSource == "" && cfg.PinterestPinterestOauth2 != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("PINTEREST_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("PINTEREST_AUTHORIZATION_URL"); v != "" {
		cfg.AuthorizationURL = v
	}
	if v := os.Getenv("PINTEREST_TOKEN_URL"); v != "" {
		cfg.TokenURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.AccessToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:PINTEREST_ACCESS_TOKEN"
		}
		return "Bearer " + c.AccessToken
	}
	if c.PinterestPinterestOauth2 != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:PINTEREST_PINTEREST_OAUTH2"
		}
		return "Bearer " + c.PinterestPinterestOauth2
	}
	if c.AccessToken != "" {
