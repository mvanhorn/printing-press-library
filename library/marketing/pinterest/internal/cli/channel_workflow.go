...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  pinterest-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  pinterest-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"ad-accounts", "boards", "catalogs", "catalogs-feeds", "catalogs-product-groups", "integrations", "media", "pins", "pinterest-search", "resources", "resources-ad-account-countries", "user-account", "user-account-businesses", "user-account-followers", "user-account-following", "user-account-following-boards"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  pinterest-pp-cli workflow status

  # Show status as JSON
  pinterest-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("pinterest-pp-cli")
			}
