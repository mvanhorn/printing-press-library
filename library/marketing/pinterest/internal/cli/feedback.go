...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "pinterest-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PINTEREST_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PINTEREST_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "pinterest-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/pinterest-pp-cli/feedback.jsonl.
When ` + "`PINTEREST_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PINTEREST_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pinterest-pp-cli feedback list
  pinterest-pp-cli feedback list --limit 5
  pinterest-pp-cli feedback list --json`,
