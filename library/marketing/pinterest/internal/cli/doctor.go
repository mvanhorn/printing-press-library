...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pinterest-pp-cli doctor
  pinterest-pp-cli doctor --json
  pinterest-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "pinterest-pp-cli login"
					report["auth_docs_url"] = "https://developers.pinterest.com/"
				} else {
					authConfigured = true
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "PINTEREST_ACCESS_TOKEN")
				if os.Getenv("PINTEREST_ACCESS_TOKEN") != "" {
					authEnvSet = append(authEnvSet, "PINTEREST_ACCESS_TOKEN")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("PINTEREST_ACCESS_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "PINTEREST_ACCESS_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "PINTEREST_ACCESS_TOKEN optional")
			}
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "PINTEREST_PINTEREST_OAUTH2")
				if os.Getenv("PINTEREST_PINTEREST_OAUTH2") != "" {
					authEnvSet = append(authEnvSet, "PINTEREST_PINTEREST_OAUTH2")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("PINTEREST_PINTEREST_OAUTH2") != "" {
				authEnvSet = append(authEnvSet, "PINTEREST_PINTEREST_OAUTH2")
			} else {
				authEnvInfo = append(authEnvInfo, "PINTEREST_PINTEREST_OAUTH2 optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "pinterest-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'pinterest-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'pinterest-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'pinterest-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'pinterest-pp-cli sync' to refresh."
	}
	return report
}
