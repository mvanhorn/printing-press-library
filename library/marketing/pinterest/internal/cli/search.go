...
In local mode: searches locally synced data only.`,
  ...
  pinterest-pp-cli search "error timeout"

  # Force local search only
  pinterest-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  pinterest-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  pinterest-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /search/pins
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/search/pins", map[string]string{
					"query": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'pinterest-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "pins":
				results, err = db.SearchPins(query, limit)
			case "resources":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchPins(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search pins failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
