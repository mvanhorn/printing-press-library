# Pinterest Printed CLI Agent Guide

This directory is a generated `pinterest-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
pinterest-pp-cli doctor --json
pinterest-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
pinterest-pp-cli which "<capability>" --json
pinterest-pp-cli <command> --help
```
...
```bash
pinterest-pp-cli <command> --agent
```
...
```bash
pinterest-pp-cli <command> --help
pinterest-pp-cli <command> --dry-run --agent
```
