# Naver Blog Printed CLI Agent Guide

This directory is a generated `naver-blog-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
naver-blog-pp-cli doctor --json
naver-blog-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
naver-blog-pp-cli which "<capability>" --json
naver-blog-pp-cli <command> --help
```
...
```bash
naver-blog-pp-cli <command> --agent
```
...
```bash
naver-blog-pp-cli <command> --help
naver-blog-pp-cli <command> --dry-run --agent
```
