...
func mockProbeClient(handler http.Handler) *http.Client {
	return &http.Client{
...
func TestProbeReachable_200(t *testing.T) {
	client := mockProbeClient(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte("hello"))
	}))

	status, code, err := ProbeReachable(context.Background(), client, "https://probe.test/")
	if err != nil {
...
func TestProbeReachable_NilClient_UsesDefault(t *testing.T) {
	old := newDefaultProbeClient
	newDefaultProbeClient = func() *http.Client {
		return mockProbeClient(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.WriteHeader(http.StatusOK)
		}))
	}
	t.Cleanup(func() { newDefaultProbeClient = old })

	status, _, err := ProbeReachable(context.Background(), nil, "https://probe.test/")
	if err != nil {
		t.Fatalf("unexpected err: %v", err)
	}
	if status != ReachabilityReachable {
...
// fallback is constructed with a bounded timeout rather than the
// unbounded http.DefaultClient.
func TestProbeReachable_NilClient_HasTimeout(t *testing.T) {
	client := newDefaultProbeClient()
	if client == nil {
		t.Fatal("newDefaultProbeClient returned nil")
	}
	if client.Timeout != defaultProbeTimeout {
		t.Errorf("timeout: want %s, got %s", defaultProbeTimeout, client.Timeout)
	}
}
...
func TestProbeReachable_SendsRangeHeader(t *testing.T) {
	...
	_, _, err := ProbeReachable(context.Background(), client, "https://probe.test/")
	if err != nil {
		t.Fatalf("unexpected err: %v", err)
	}
