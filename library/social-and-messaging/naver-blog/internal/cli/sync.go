...
Resource scoping:
  ...
  naver-blog-pp-cli sync

  # Sync specific resources only
  naver-blog-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  naver-blog-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  naver-blog-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  naver-blog-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  naver-blog-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/naver-blog-pp-cli/data.db)")
