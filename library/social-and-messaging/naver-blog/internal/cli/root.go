...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "naver-blog-pp-cli",
		Short: `Search, fetch, and aggregate public Naver Blog posts — title, body, tags, likes (공감), comments — with a local SQLite store and offline search.`,
		Long: `Search, fetch, and aggregate public Naver Blog posts — title, body, tags, likes (공감), comments — with a local SQLite store and offline search.
...
Highlights (not in the official API docs):
  ...
  • bundle   Runs a list of keyword/hashtag queries from a YAML/JSONL file, enriches every result with engagement, dedupes by canonical URL, and streams a sheet-ready TSV — one binary replaces the Hermes web_sear…
  ...
  • batch   Accepts CSV / JSON column / newline-delimited URLs in any mix of m.blog.naver.com / blog.naver.com / PostView.naver shapes; canonicalizes each; runs concurrent fetches with adaptive rate limiting; pr…
...
Health check: run 'naver-blog-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("naver-blog-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().BoolVar(&flags.plain, "plain", false, "Output as plain tab-separated rows without a header")
	rootCmd.PersistentFlags().BoolVar(&flags.quiet, "quiet", false, "Bare output, one value per line")
	rootCmd.PersistentFlags().StringVar(&flags.configPath, "config", "", "Config file path")
	rootCmd.PersistentFlags().DurationVar(&flags.timeout, "timeout", 30*time.Second, "Request timeout")
	rootCmd.PersistentFlags().BoolVar(&flags.dryRun, "dry-run", false, "Show request without sending")
	rootCmd.PersistentFlags().BoolVar(&flags.noCache, "no-cache", false, "Bypass response cache")
	rootCmd.PersistentFlags().DurationVar(&flags.cacheTTL, "cache-ttl", 5*time.Minute, "Maximum age for cached HTTP responses (0 forces refresh)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'naver-blog-pp-cli profile list')")
	...
	rootCmd.AddCommand(newBlogsPromotedCmd(flags))
	rootCmd.AddCommand(newBlogsInfoCmd(flags))
	rootCmd.AddCommand(newPostsPromotedCmd(flags))
	rootCmd.AddCommand(newReactionsPromotedCmd(flags))
	// Pass 2 novel commands (transcendence layer).
	rootCmd.AddCommand(newPostsDiffCmd(flags))
	rootCmd.AddCommand(newBlogsHealthCmd(flags))
	...
	rootCmd.AddCommand(newPostsCommentsCmd(flags))
	// Pass 2 Gate-1 aliases (thin wrappers; MCP-hidden to avoid tool
	// duplication with the canonical commands).
	rootCmd.AddCommand(newPostAliasCmd(flags))
	rootCmd.AddCommand(newPostBatchAliasCmd(flags))
	rootCmd.AddCommand(newFeedAliasCmd(flags))
	rootCmd.AddCommand(newSearchAliasCmd(flags))
	rootCmd.AddCommand(newHashtagAliasCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("naver-blog-pp-cli %s\n", version)
		},
	}
}
