...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "naver-blog-pp-cli", "http")
...
// Limiter exposes the client's adaptive limiter so off-host helpers
// (e.g. reactionapi, sponsored detection, SERP fetches) that bypass
// the client's PerformRequest path can still share the same outbound
// rate budget. Returns the limiter directly — callers may pass nil
// to disable rate-limit coordination.
func (c *Client) Limiter() *cliutil.AdaptiveLimiter {
	return c.limiter
}
...
func (c *Client) readCache(path string, params map[string]string) (json.RawMessage, bool) {
	...
	if cacheExpired(cacheFile, c.CacheTTL) {
		return nil, false
	}
...
// cacheExpired reports whether path is missing or stale beyond maxAge.
// A non-positive TTL is treated as immediately expired so callers can force
// fresh reads without disabling cache writes.
func cacheExpired(path string, ttl time.Duration) bool {
	if ttl <= 0 {
		return true
	}
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "naver-blog-pp-cli/0.1.0")
		}
