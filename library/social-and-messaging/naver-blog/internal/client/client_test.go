...
func TestCacheExpired(t *testing.T) {
	...
	if !cacheExpired(missing, time.Minute) {
		t.Fatalf("missing cache file must be expired")
	}

	cacheFile := filepath.Join(t.TempDir(), "entry.json")
	if err := os.WriteFile(cacheFile, []byte(`{"ok":true}`), 0o644); err != nil {
		t.Fatalf("write cache file: %v", err)
	}
	if cacheExpired(cacheFile, time.Hour) {
		t.Fatalf("fresh cache file must not be expired")
	}
	if !cacheExpired(cacheFile, 0) {
		t.Fatalf("zero TTL must force expiry")
	}
	...
	if !cacheExpired(cacheFile, time.Hour) {
		t.Fatalf("old cache file must be expired")
	}
}
