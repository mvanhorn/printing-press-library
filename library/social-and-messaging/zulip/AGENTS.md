# Zulip Printed CLI Agent Guide

This directory is a generated `zulip-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
zulip-pp-cli doctor --json
zulip-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
zulip-pp-cli which "<capability>" --json
zulip-pp-cli <command> --help
```
...
```bash
zulip-pp-cli <command> --help
zulip-pp-cli <command> --dry-run --agent
```
