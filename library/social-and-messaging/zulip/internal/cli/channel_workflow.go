...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  zulip-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  zulip-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"attachments", "bot-storage", "channel-folders", "dev-list-users", "drafts", "invites", "messages", "navigation-views", "realm", "realm-domains", "realm-linkifiers", "realm-presence", "realm-profile-fields", "reminders", "saved-snippets", "scheduled-messages", "server-settings", "streams", "user-groups", "users", "users-me", "users-me-alert-words", "users-me-subscriptions", "zulip-export", "zulip-export-consents"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  zulip-pp-cli workflow status

  # Show status as JSON
  zulip-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("zulip-pp-cli")
			}
