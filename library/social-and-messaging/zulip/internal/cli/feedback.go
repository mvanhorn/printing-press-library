...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".zulip-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ZULIP_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ZULIP_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "zulip-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.zulip-pp-cli/feedback.jsonl.
When ` + "`ZULIP_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ZULIP_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  zulip-pp-cli feedback list
  zulip-pp-cli feedback list --limit 5
  zulip-pp-cli feedback list --json`,
