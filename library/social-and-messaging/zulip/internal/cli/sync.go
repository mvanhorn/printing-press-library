...
import (
	...
	"sync/atomic"
	"time"
	"github.com/mvanhorn/printing-press-library/library/social-and-messaging/zulip/internal/cliutil"
...
Resource scoping:
  ...
  zulip-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  zulip-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  zulip-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  zulip-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  zulip-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("zulip-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/zulip-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "bot-storage":
		return db.UpsertBotStorage(data)
	case "api_key":
		return db.UpsertApiKey(data)
	case "calls":
		return db.UpsertCalls(data)
	case "channel-folders":
		return db.UpsertChannelFolders(data)
	case "default-streams":
		return db.UpsertDefaultStreams(data)
	case "dev-fetch-api-key":
		return db.UpsertDevFetchApiKey(data)
	case "dev-list-users":
		return db.UpsertDevListUsers(data)
	case "drafts":
		return db.UpsertDrafts(data)
	case "events":
		return db.UpsertEvents(data)
	case "fetch-api-key":
		return db.UpsertFetchApiKey(data)
	case "get-stream-id":
		return db.UpsertGetStreamId(data)
	case "invites":
		return db.UpsertInvites(data)
	case "resend":
		return db.UpsertResend(data)
	case "jwt":
		return db.UpsertJwt(data)
	case "messages":
	...
	case "read_receipts":
		return db.UpsertReadReceipts(data)
	case "report":
		return db.UpsertReport(data)
	case "messages_typing":
		return db.UpsertMessagesTyping(data)
	case "mobile-push":
		return db.UpsertMobilePush(data)
	case "navigation-views":
		return db.UpsertNavigationViews(data)
	case "realm":
		return db.UpsertRealm(data)
	case "register":
		return db.UpsertRegister(data)
	case "register-client-device":
		return db.UpsertRegisterClientDevice(data)
	case "reminders":
		return db.UpsertReminders(data)
	case "saved-snippets":
		return db.UpsertSavedSnippets(data)
	case "scheduled-messages":
		return db.UpsertScheduledMessages(data)
	case "server-settings":
		return db.UpsertServerSettings(data)
	case "streams":
		return db.UpsertStreams(data)
	case "delete_topic":
		return db.UpsertDeleteTopic(data)
	case "email_address":
		return db.UpsertEmailAddress(data)
	case "streams_members":
		return db.UpsertStreamsMembers(data)
	case "user-groups":
		return db.UpsertUserGroups(data)
	case "deactivate":
		return db.UpsertDeactivate(data)
	case "user_groups_members":
		return db.UpsertUserGroupsMembers(data)
	case "subgroups":
		return db.UpsertSubgroups(data)
	case "user-uploads":
		return db.UpsertUserUploads(data)
	case "users":
		return db.UpsertUsers(data)
	case "users_channels":
		return db.UpsertUsersChannels(data)
	case "presence":
	...
	case "zulip-export":
		return db.UpsertZulipExport(data)
	case "zulip-outgoing-webhook":
		return db.UpsertZulipOutgoingWebhook(data)
	default:
...
func defaultSyncResources() []string {
	return []string{
		...
		"users",
		"users-me",
		"users-me-alert-words",
		"users-me-subscriptions",
		"zulip-export",
		"zulip-export-consents",
	}
}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"bot-storage":            "/bot_storage",
		"channel-folders":        "/channel_folders",
		"dev-list-users":         "/dev_list_users",
		"drafts":                 "/drafts",
		"invites":                "/invites",
		"messages":               "/messages",
		"navigation-views":       "/navigation_views",
		"realm":                  "/realm/emoji",
		"realm-domains":          "/realm/domains",
		"realm-linkifiers":       "/realm/linkifiers",
		"realm-presence":         "/realm/presence",
		"realm-profile-fields":   "/realm/profile_fields",
		"reminders":              "/reminders",
		"saved-snippets":         "/saved_snippets",
		...
		"users":                  "/users",
		"users-me":               "/users/me",
		"users-me-alert-words":   "/users/me/alert_words",
		"users-me-subscriptions": "/users/me/subscriptions",
		"zulip-export":           "/export/realm",
		"zulip-export-consents":  "/export/realm/consents",
	}
