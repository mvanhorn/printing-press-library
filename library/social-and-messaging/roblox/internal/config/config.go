...
type Config struct {
	...
	RobloxCookies    string          `toml:"cookies"`
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("ROBLOX_COOKIES"); v != "" {
		cfg.RobloxCookies = v
		cfg.markEnvOverride("RobloxCookies")
		cfg.AuthSource = "env:ROBLOX_COOKIES"
		cfg.CredentialSource = "env:ROBLOX_COOKIES"
	}
	...
	if v := os.Getenv("ROBLOX_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("ROBLOX_COLLECTIBLE_ITEM_ID")); v != "" {
		cfg.TemplateVars["collectibleItemId"] = v
	} else if verifyMode {
		cfg.TemplateVars["collectibleItemId"] = "collectibleItemId_placeholder"
	}
	if v := strings.TrimSpace(os.Getenv("ROBLOX_TARGET_USER_ID")); v != "" {
		cfg.TemplateVars["targetUserId"] = v
	} else if verifyMode {
		cfg.TemplateVars["targetUserId"] = "targetUserId_placeholder"
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("ROBLOX_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "roblox-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.RobloxCookies != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:ROBLOX_COOKIES"
		}
		return ensureAuthScheme("Bearer", c.RobloxCookies)
	}
	if c.AccessToken != "" {
		if c.AuthSource == "" {
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.RobloxCookies = creds.RobloxCookies
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.RobloxCookies = ""
	delete(c.envOverrides, "RobloxCookies")
	c.updateFileConfigField("RobloxCookies")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["RobloxCookies"] {
			out.RobloxCookies = c.fileConfig.RobloxCookies
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "RobloxCookies":
		c.fileConfig.RobloxCookies = c.RobloxCookies
	}
}
