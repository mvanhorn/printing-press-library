...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ROBLOX_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ROBLOX_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "roblox-pp-cli/feedback")
...
When ` + "`ROBLOX_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ROBLOX_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  roblox-pp-cli feedback list
  roblox-pp-cli feedback list --limit 5
  roblox-pp-cli feedback list --json`,
