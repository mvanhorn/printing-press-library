...
In local mode: searches locally synced data only.`,
  ...
  roblox-pp-cli search "error timeout"

  # Force local search only
  roblox-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  roblox-pp-cli search "status" --type asset-thumbnail-animated --data-source local
  # JSON output for piping
  roblox-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /v1/users/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/v1/users/search", map[string]string{
					"keyword": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'roblox-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
