...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  roblox-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  roblox-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"asset-to-category", "asset-to-subcategory", "avatar", "avatar-2", "avatar-metadata", "avatar-rules", "badges-2", "birthdate", "catalog-search", "categories", "description", "economy-user", "gender", "groups-2", "groups-2-metadata", "groups-user", "metadata", "my", "my-friends-requests", "my-new-friend-requests-count", "my-trusted-friends-count", "my-trusted-friends-requests", "subcategories", "user", "user-trusted-friend-requests-count", "users", "users-authenticated-age-bracket", "users-authenticated-country-code", "users-authenticated-roles"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  roblox-pp-cli workflow status

  # Show status as JSON
  roblox-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("roblox-pp-cli")
			}
