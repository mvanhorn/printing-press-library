# Substack Notes Printed CLI Agent Guide

This directory is a generated `substack-notes-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
substack-notes-pp-cli doctor --json
substack-notes-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
substack-notes-pp-cli which "<capability>" --json
substack-notes-pp-cli <command> --help
```
...
```bash
substack-notes-pp-cli <command> --agent
```
...
```bash
substack-notes-pp-cli <command> --help
substack-notes-pp-cli <command> --dry-run --agent
```
