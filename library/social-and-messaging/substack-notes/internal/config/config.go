...
type Config struct {
	...
	Path                    string            `toml:"-"`
	SubstackNotesCookieAuth string            `toml:"notes_cookie_auth"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("SUBSTACK_NOTES_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "substack-notes-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("SUBSTACK_NOTES_COOKIE_AUTH"); v != "" {
		cfg.SubstackNotesCookieAuth = v
		cfg.AuthSource = "env:SUBSTACK_NOTES_COOKIE_AUTH"
	}
	...
	if cfg.AuthSource == "" && cfg.SubstackNotesCookieAuth != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("SUBSTACK_NOTES_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.SubstackNotesCookieAuth
	if token == "" {
		return ""
	}
	if c.SubstackNotesCookieAuth == "" {
		return ""
	}
	return token
}
...
func (c *Config) SaveCredential(token string) error {
	c.AuthHeaderVal = ""
	c.AccessToken = ""
	c.SubstackNotesCookieAuth = token
	return c.save()
}
