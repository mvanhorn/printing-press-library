...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Substack Notes",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:     "setup",
		Short:   "Print safe setup steps for local Substack auth",
		Example: "  substack-notes-pp-cli auth setup\n  substack-notes-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Sign in to Substack in a supported local browser, then run:")
			fmt.Fprintln(w, "  substack-notes-pp-cli auth login --browser chrome")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Supported browsers: chrome, brave, arc")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Advanced fallback:")
			fmt.Fprintln(w, "  export SUBSTACK_NOTES_COOKIE_AUTH=\"<cookie-header>\"")
			fmt.Fprintln(w, "  substack-notes-pp-cli auth set-token \"<cookie-header>\"")
			if !launch {
				return nil
			}
			launchURL := "https://substack.com"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  substack-notes-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "")
				fmt.Fprintln(w, "Set up local auth:")
				fmt.Fprintln(w, "  substack-notes-pp-cli auth login --browser chrome")
				fmt.Fprintln(w, "  substack-notes-pp-cli auth setup")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		Use:     "set-token <token>",
		Short:   "Save a Cookie header value to the config file",
		Example: "  substack-notes-pp-cli auth set-token \"cookie_name=cookie_value; ...\"",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  substack-notes-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("SUBSTACK_NOTES_COOKIE_AUTH") != "" {
				envStillSet = "SUBSTACK_NOTES_COOKIE_AUTH"
			}
