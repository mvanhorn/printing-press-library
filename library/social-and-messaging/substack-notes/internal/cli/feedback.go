...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "substack-notes-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SUBSTACK_NOTES_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SUBSTACK_NOTES_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "substack-notes-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/substack-notes-pp-cli/feedback.jsonl.
When ` + "`SUBSTACK_NOTES_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SUBSTACK_NOTES_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  substack-notes-pp-cli feedback list
  substack-notes-pp-cli feedback list --limit 5
  substack-notes-pp-cli feedback list --json`,
