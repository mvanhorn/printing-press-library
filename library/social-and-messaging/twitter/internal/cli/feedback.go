...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".twitter-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TWITTER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TWITTER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "twitter-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.twitter-pp-cli/feedback.jsonl.
When ` + "`TWITTER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TWITTER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
