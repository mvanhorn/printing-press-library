...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("PRODUCT_HUNT_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "product-hunt-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if cfg.AuthSource == "" && cfg.ProductHuntToken != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("PRODUCT_HUNT_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.ProductHuntToken != "" {
		c.AuthSource = "env:PRODUCT_HUNT_TOKEN"
