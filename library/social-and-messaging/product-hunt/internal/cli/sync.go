...
Exit codes & warnings:
  ...
  product-hunt-pp-cli sync

  # Sync specific resources only
  product-hunt-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  product-hunt-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  product-hunt-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  product-hunt-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  product-hunt-pp-cli sync --latest-only`,
			...
			for i := 0; i < concurrency; i++ {
				...
				go func() {
					...
					for resource := range work {
						res := syncGQLResource(ctx, phc, db, resource, sinceTS, full, maxPages)
						results <- res
					}
				}()
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/product-hunt-pp-cli/data.db)")
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"collections": "/graphql/collections",
		"posts": "/graphql",
		"topics": "/graphql/topics",
	}
