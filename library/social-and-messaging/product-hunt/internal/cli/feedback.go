...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".product-hunt-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PRODUCT_HUNT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PRODUCT_HUNT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "product-hunt-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.product-hunt-pp-cli/feedback.jsonl.
When ` + "`PRODUCT_HUNT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PRODUCT_HUNT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  product-hunt-pp-cli feedback list
  product-hunt-pp-cli feedback list --limit 5
  product-hunt-pp-cli feedback list --json`,
