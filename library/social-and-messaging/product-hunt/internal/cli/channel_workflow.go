...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  product-hunt-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  product-hunt-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"collections", "posts", "topics",  }
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  product-hunt-pp-cli workflow status

  # Show status as JSON
  product-hunt-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("product-hunt-pp-cli")
			}
