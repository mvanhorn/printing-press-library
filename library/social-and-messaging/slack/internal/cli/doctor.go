...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  slack-pp-cli doctor
  slack-pp-cli doctor --json
  slack-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export SLACK_USER_TOKEN=<your-key>"
					report["auth_docs_url"] = "https://api.slack.com/support"
				} else {
					authConfigured = true
			...
			if os.Getenv("SLACK_USER_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "SLACK_USER_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "SLACK_USER_TOKEN")
			}
			if strings.Contains("Optional bot token (xoxb-...) for post-only flows. Set this OR SLACK_USER_TOKEN.", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "SLACK_BOT_TOKEN")
				if os.Getenv("SLACK_BOT_TOKEN") != "" {
					authEnvSet = append(authEnvSet, "SLACK_BOT_TOKEN")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("SLACK_BOT_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "SLACK_BOT_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "SLACK_BOT_TOKEN optional")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'slack-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'slack-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'slack-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'slack-pp-cli sync' to refresh."
	}
	return report
}
