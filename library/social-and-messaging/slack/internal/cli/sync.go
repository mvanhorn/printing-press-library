...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "admin-conversations-create":
		return db.UpsertAdminConversationsCreate(data)
	case "auth-test":
		return db.UpsertAuthTest(data)
	case "chat-schedule-message":
		return db.UpsertChatScheduleMessage(data)
	case "chat-update":
		return db.UpsertChatUpdate(data)
	case "conversations-history":
		return db.UpsertConversationsHistory(data)
	case "dnd-end-snooze":
		return db.UpsertDndEndSnooze(data)
	case "dnd-info":
		return db.UpsertDndInfo(data)
	case "files-info":
		return db.UpsertFilesInfo(data)
	case "migration-exchange":
		return db.UpsertMigrationExchange(data)
	case "users-get-presence":
		return db.UpsertUsersGetPresence(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"admin-invite-requests-approved-list": "/admin.inviteRequests.approved.list",
		"admin-invite-requests-denied-list":   "/admin.inviteRequests.denied.list",
		"admin-invite-requests-list":          "/admin.inviteRequests.list",
		"admin-teams-list":                    "/admin.teams.list",
		"apps-uninstall":                      "/apps.uninstall",
		"chat-scheduled-messages-list":        "/chat.scheduledMessages.list",
