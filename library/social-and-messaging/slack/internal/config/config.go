...
func Load(configPath string) (*Config, error) {
	...
	if path == "" {
		...
		path = filepath.Join(home, ".config", "slack-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if cfg.AuthSource == "" && cfg.SlackUserToken != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.SlackBotToken != "" {
		cfg.AuthSource = "config"
	}
...
func (c *Config) AuthHeader() string {
	...
	// Slack expects `Authorization: Bearer <token>`. The generator emits the
	// apiKey security scheme without the Bearer prefix, so add it here. Prefer
	// the xoxp user token; fall back to an xoxb bot token for post-only flows.
	token := c.SlackUserToken
	if token == "" {
