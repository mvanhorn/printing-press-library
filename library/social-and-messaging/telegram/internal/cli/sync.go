...
Exit codes & warnings:
  ...
  telegram-pp-cli sync

  # Sync specific resources only
  telegram-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  telegram-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  telegram-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  telegram-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  telegram-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/telegram-pp-cli/data.db)")
