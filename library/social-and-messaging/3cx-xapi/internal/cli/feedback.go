...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("API_3CX_XAPI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("API_3CX_XAPI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "3cx-xapi-pp-cli/feedback")
...
When ` + "`API_3CX_XAPI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`API_3CX_XAPI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  3cx-xapi-pp-cli feedback list
  3cx-xapi-pp-cli feedback list --limit 5
  3cx-xapi-pp-cli feedback list --json`,
