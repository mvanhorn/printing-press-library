...
Auto and local modes search the local mirror. Live mode is rejected because
3CX exposes directory-search settings mutations, not a read-only search endpoint.`,
		Example: `  # Search the local mirror
  3cx-xapi-pp-cli search "error timeout"

  # Force local search only
  3cx-xapi-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  3cx-xapi-pp-cli search "status" --type active-calls --data-source local
  # JSON output for piping
  3cx-xapi-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.dataSource == "live" {
				return usageErr(fmt.Errorf("search has no read-only live endpoint; use --data-source local or auto after sync"))
			}
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun '3cx-xapi-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "aisettings":
				results, err = db.SearchAisettings(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchAisettings(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search aisettings failed: %w", searchErr)
					}
