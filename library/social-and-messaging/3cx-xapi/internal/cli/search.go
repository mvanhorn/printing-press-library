...
In local mode: searches locally synced data only.`,
  ...
  3cx-xapi-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /Contacts/Pbx.SetDirSearchSettings
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/Contacts/Pbx.SetDirSearchSettings", map[string]any{
					"q":                 query,
					"dirSearchSettings": map[string]any{},
				})
				if getErr == nil {
