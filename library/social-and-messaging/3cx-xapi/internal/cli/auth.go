...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Xapi",
...
Mint an OAuth2 bearer token via POST to your instance's /connect/token endpoint
with grant_type=client_credentials and cache it on disk. Subsequent commands
auto-refresh the token before expiry.

Point the CLI at your instance with TCX_FQDN (e.g. your-company.3cx.com.au).
Credentials default to TCX_CLIENT_ID (Client ID) and TCX_CLIENT_SECRET (Client Secret) environment variables.
`),
		Example: strings.Trim(`
  # Use env vars
  3cx-xapi-pp-cli auth login

  # Explicit credentials
  3cx-xapi-pp-cli auth login --client-id <id> --client-secret <secret>
`, "\n"),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if clientID == "" {
				clientID = strings.TrimSpace(os.Getenv("TCX_CLIENT_ID"))
			}
			if clientSecret == "" {
				clientSecret = strings.TrimSpace(os.Getenv("TCX_CLIENT_SECRET"))
			}
			if clientID == "" || clientSecret == "" {
				return authErr(fmt.Errorf("client ID and secret required (set --client-id/--client-secret or TCX_CLIENT_ID/TCX_CLIENT_SECRET)"))
			}
			...
			if tokenURL == "" {
				if u, perr := url.Parse(cfg.BaseURL); perr == nil && u.Scheme != "" && u.Host != "" {
					tokenURL = u.Scheme + "://" + u.Host + "/connect/token"
				}
			}
			if tokenURL == "" {
				return configErr(fmt.Errorf("no 3CX instance configured: set TCX_FQDN (e.g. your-company.3cx.com.au) or base_url in config"))
			}
			tok, err := mintClientCredentialsToken(&http.Client{Timeout: flags.timeout}, tokenURL, clientID, clientSecret)
			if err != nil {
				return authErr(err)
			}
	...
	cmd.Flags().StringVar(&clientID, "client-id", "", "OAuth2 Client ID; defaults to $TCX_CLIENT_ID")
	cmd.Flags().StringVar(&clientSecret, "client-secret", "", "OAuth2 Client Secret; defaults to $TCX_CLIENT_SECRET")
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  3cx-xapi-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export TCX_CLIENT_ID=\"<client-id>\"")
				fmt.Fprintln(w, "  export TCX_CLIENT_SECRET=\"<client-secret>\"")
				fmt.Fprintf(w, "  3cx-xapi-pp-cli auth login\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  3cx-xapi-pp-cli auth set-token <bearer-jwt>",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  3cx-xapi-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("TCX_CLIENT_ID") != "" {
				envStillSet = "TCX_CLIENT_ID"
			}
			if envStillSet == "" && os.Getenv("TCX_CLIENT_SECRET") != "" {
				envStillSet = "TCX_CLIENT_SECRET"
			}
