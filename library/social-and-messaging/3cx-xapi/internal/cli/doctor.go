...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  3cx-xapi-pp-cli doctor
  3cx-xapi-pp-cli doctor --json
  3cx-xapi-pp-cli doctor --fail-on warn
  3cx-xapi-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				// OAuth2 client-credentials: the Bearer token is minted on the
				// first API call, so AuthHeader() is empty until then. Treat the
				// presence of a client id + secret (from env or config) as
				// configured auth, not just an already-minted token.
				hasClientCreds := strings.TrimSpace(cfg.ClientID) != "" && strings.TrimSpace(cfg.ClientSecret) != ""
				if header == "" && !hasClientCreds {
					report["auth"] = "not configured"
					report["auth_hint"] = "Set credentials with: export TCX_CLIENT_ID=\"your-token-here\" TCX_CLIENT_SECRET=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if os.Getenv("TCX_CLIENT_ID") != "" {
				authEnvSet = append(authEnvSet, "TCX_CLIENT_ID")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "TCX_CLIENT_ID")
			}
			if os.Getenv("TCX_CLIENT_SECRET") != "" {
				authEnvSet = append(authEnvSet, "TCX_CLIENT_SECRET")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "TCX_CLIENT_SECRET")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "3cx-xapi-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run '3cx-xapi-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run '3cx-xapi-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run '3cx-xapi-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run '3cx-xapi-pp-cli sync' to refresh."
	}
	return report
}
