...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"API_3CX_XAPI_CONFIG",
		"API_3CX_XAPI_CONFIG_DIR",
		"API_3CX_XAPI_DATA_DIR",
		"API_3CX_XAPI_STATE_DIR",
		"API_3CX_XAPI_CACHE_DIR",
		"API_3CX_XAPI_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
	...
	return home, filepath.Join(home, ".config", "3cx-xapi-pp-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "3cx-xapi-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("TCX_CLIENT_ID", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "3cx-xapi-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "3cx-xapi-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "3cx-xapi-pp-cli", "credentials.toml")
