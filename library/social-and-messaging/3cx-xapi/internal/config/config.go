...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		// No baked instance: the base URL comes from TCX_FQDN (or base_url
		// config / API_3CX_XAPI_BASE_URL). Every 3CX deployment has its own
		// FQDN, so there is no sensible default.
		BaseURL: "",
	}
	...
	// Env var overrides
	if v := os.Getenv("TCX_CLIENT_ID"); v != "" {
		cfg.ClientID = v
		cfg.markEnvOverride("ClientID")
		cfg.AuthSource = "env:TCX_CLIENT_ID"
		cfg.CredentialSource = "env:TCX_CLIENT_ID"
	}
	if v := os.Getenv("TCX_CLIENT_SECRET"); v != "" {
		cfg.ClientSecret = v
		cfg.markEnvOverride("ClientSecret")
		cfg.AuthSource = "env:TCX_CLIENT_SECRET"
		cfg.CredentialSource = "env:TCX_CLIENT_SECRET"
	}
	...
	// TCX_FQDN is the ecosystem-standard way to point the CLI at an instance.
	// Accept a bare host, a host with scheme, or one with a trailing path; derive
	// "https://<host>/xapi/v1". The more specific API_3CX_XAPI_BASE_URL below wins.
	if v := strings.TrimSpace(os.Getenv("TCX_FQDN")); v != "" {
		host := v
		...
		if host != "" {
			cfg.BaseURL = "https://" + host + "/xapi/v1"
		}
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("API_3CX_XAPI_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("API_3CX_XAPI_TOKEN_URL"); v != "" {
		cfg.TokenURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("3CX_XAPI_AFTER")); v != "" {
		cfg.TemplateVars["after"] = v
	} else if verifyMode {
		cfg.TemplateVars["after"] = "after_placeholder"
	}
	if v := strings.TrimSpace(os.Getenv("3CX_XAPI_ID")); v != "" {
		cfg.TemplateVars["id"] = v
	} else if verifyMode {
		cfg.TemplateVars["id"] = "id_placeholder"
	}
	if v := strings.TrimSpace(os.Getenv("3CX_XAPI_LIMIT")); v != "" {
		cfg.TemplateVars["limit"] = v
	} else if verifyMode {
		cfg.TemplateVars["limit"] = "limit_placeholder"
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("API_3CX_XAPI_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "3cx-xapi-pp-cli", "config.toml"), nil
}
