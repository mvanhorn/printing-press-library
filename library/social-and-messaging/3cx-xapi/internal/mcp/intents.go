...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("trace_why_an_extension_is_not_ringing",
			mcplib.WithDescription("Shows every inbound rule, ring group, queue, DID, and forwarding profile that routes to extension 214."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Override the recipe's positional id value.")),
		),
		handleTraceWhyAnExtensionIsNotRinging,
	)
}

// handleTraceWhyAnExtensionIsNotRinging runs the trace_why_an_extension_is_not_ringing recipe intent: Shows every inbound rule, ring group, queue, DID, and forwarding profile that routes to extension 214.
func handleTraceWhyAnExtensionIsNotRinging(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
