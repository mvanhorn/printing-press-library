...
func newHTTPClient(timeout time.Duration, jar http.CookieJar) *http.Client {
	// Instagram requires HTTP/2 — do not disable TLSNextProto.
	// With HTTP/1.1 forced, the API returns 400 for most endpoints.
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "instagram-pp-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		// Instagram cookie auth: access_token holds the full Cookie header string.
		// authHeader() returns access_token (the cookie blob); send it as Cookie.
		if authHeader != "" {
			req.Header.Set("Cookie", authHeader)
			// Extract csrftoken for the CSRF header Instagram requires
			req.Header.Set("X-CSRFToken", extractCookieValue(authHeader, "csrftoken"))
		}
		// Instagram-required headers for private API access
		...
		// Use a non-headless Chrome UA — headless UA is blocked by some endpoints
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	...
	if authHeader != "" {
		fmt.Fprintf(os.Stderr, "  %s: %s\n", "X-CSRFToken", maskToken(authHeader))
	}
...
// extractCookieValue extracts the value of a named cookie from a Cookie header string.
