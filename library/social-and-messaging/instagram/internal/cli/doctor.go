		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "instagram-pp-cli auth login --chrome"
				} else {
					authConfigured = true
					report["auth"] = "configured (browser session)"
					report["auth_source"] = cfg.AuthSource
					report["auth_domain"] = ".instagram.com"
				}
			}
			...
			if os.Getenv("INSTAGRAM_SESSION_ID") != "" {
				authEnvSet = append(authEnvSet, "INSTAGRAM_SESSION_ID")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "INSTAGRAM_SESSION_ID")
			}
			if os.Getenv("INSTAGRAM_CSRF_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "INSTAGRAM_CSRF_TOKEN")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "INSTAGRAM_CSRF_TOKEN")
			}
			if os.Getenv("INSTAGRAM_USER_ID") != "" {
				authEnvSet = append(authEnvSet, "INSTAGRAM_USER_ID")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "INSTAGRAM_USER_ID")
			}
			switch {
