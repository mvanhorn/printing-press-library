...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("RESPONDIO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("RESPONDIO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "respondio-pp-cli/feedback")
...
When ` + "`RESPONDIO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`RESPONDIO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  respondio-pp-cli feedback list
  respondio-pp-cli feedback list --limit 5
  respondio-pp-cli feedback list --json`,
