...
Run sync first to populate the local search index.`,
  ...
  respondio-pp-cli search "status"

  # Force local search explicitly
  respondio-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  respondio-pp-cli search "status" --type comment --data-source local
  # JSON output for piping
  respondio-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'respondio-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "space":
				results, err = db.SearchSpace(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchSpace(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search space failed: %w", searchErr)
					}
