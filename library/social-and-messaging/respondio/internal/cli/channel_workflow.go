...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  respondio-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  respondio-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"contact", "space", "space-channel", "space-custom-field"}
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  respondio-pp-cli workflow status

  # Show status as JSON
  respondio-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("respondio-pp-cli")
			}
