...
type Config struct {
	...
	RespondIoApiToken string          `toml:"io_api_token"`
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("RESPOND_IO_API_TOKEN"); v != "" {
		cfg.RespondIoApiToken = v
		cfg.markEnvOverride("RespondIoApiToken")
		cfg.AuthSource = "env:RESPOND_IO_API_TOKEN"
		cfg.CredentialSource = "env:RESPOND_IO_API_TOKEN"
	}
	...
	if v := os.Getenv("RESPONDIO_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("RESPONDIO_IDENTIFIER")); v != "" {
		cfg.TemplateVars["identifier"] = v
	} else if verifyMode {
		cfg.TemplateVars["identifier"] = "identifier_placeholder"
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("RESPONDIO_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "respondio-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.RespondIoApiToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:RESPOND_IO_API_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"io_api_token":         c.RespondIoApiToken,
			"RESPOND_IO_API_TOKEN": c.RespondIoApiToken,
			"token":                c.RespondIoApiToken,
			"access_token":         c.RespondIoApiToken,
		})
	}
	if c.AccessToken != "" {
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		RespondIoApiToken: c.RespondIoApiToken,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.RespondIoApiToken == "" {
		c.RespondIoApiToken = creds.RespondIoApiToken
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.RespondIoApiToken = ""
	delete(c.envOverrides, "RespondIoApiToken")
	c.updateFileConfigField("RespondIoApiToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["RespondIoApiToken"] {
			out.RespondIoApiToken = c.fileConfig.RespondIoApiToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "RespondIoApiToken":
		c.fileConfig.RespondIoApiToken = c.RespondIoApiToken
	}
}
