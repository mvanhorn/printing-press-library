...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("where_customers_talk_to_you",
			mcplib.WithDescription("Show the channel-source distribution across synced contacts."),
			mcplib.WithString("slug", mcplib.Required(), mcplib.Description("Override the recipe's positional slug value.")),
		),
		handleWhereCustomersTalkToYou,
	)
}

// handleWhereCustomersTalkToYou runs the where_customers_talk_to_you recipe intent: Show the channel-source distribution across synced contacts.
func handleWhereCustomersTalkToYou(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
