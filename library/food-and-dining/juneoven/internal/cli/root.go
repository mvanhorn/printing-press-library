...
type rootFlags struct {
	...
	// invocation. Mirrors the JUNEOVEN_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "juneoven-pp-cli",
		Short: `Control a June oven from the terminal, plus a durable local cook history and repeatable named cooks that June's live-only cloud never keeps.`,
		Long: `Control a June oven from the terminal, plus a durable local cook history and repeatable named cooks that June's live-only cloud never keeps.
...
Run 'juneoven-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("juneoven-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'juneoven-pp-cli profile list')")
	...
	rootCmd.AddCommand(newPairCmd(flags))
	rootCmd.AddCommand(newStatusCmd(flags))
	rootCmd.AddCommand(newPreheatCmd(flags))
	rootCmd.AddCommand(newTempCmd(flags))
	rootCmd.AddCommand(newTimerCmd(flags))
	rootCmd.AddCommand(newCancelCmd(flags))
	rootCmd.AddCommand(newWatchCmd(flags))
	rootCmd.AddCommand(newCamCmd(flags))
	// Novel features (local history + derived insight the June cloud never keeps)
	rootCmd.AddCommand(newReadyCmd(flags))
	rootCmd.AddCommand(newRecordCmd(flags))
	rootCmd.AddCommand(newLogCmd(flags))
	rootCmd.AddCommand(newCurveCmd(flags))
	rootCmd.AddCommand(newPreheatStatsCmd(flags))
	rootCmd.AddCommand(newEtaCmd(flags))
	rootCmd.AddCommand(newRepeatCmd(flags))
