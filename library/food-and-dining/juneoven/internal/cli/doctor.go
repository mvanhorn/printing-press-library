...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  juneoven-pp-cli doctor
  juneoven-pp-cli doctor --json
  juneoven-pp-cli doctor --fail-on warn
  juneoven-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set it with: juneoven-pp-cli auth set-token <token> or export JUNE_ACCESS_TOKEN=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if os.Getenv("JUNE_ACCESS_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "JUNE_ACCESS_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "JUNE_ACCESS_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "juneoven-pp-cli", suggestion)
						} else {
