...
// forkable-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/food-and-dining/forkable/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/food-and-dining/forkable/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/food-and-dining/forkable/internal/learn/patterns"
...
const noLearnEnvVar = "FORKABLE_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("forkable-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  forkable-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  forkable-pp-cli recall "<question>" --agent
  forkable-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  forkable-pp-cli learnings list --agent
  forkable-pp-cli learnings list --query "<substring>"
  forkable-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  forkable-pp-cli learnings forget "<question>" --resource <id>
  forkable-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  forkable-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  forkable-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
