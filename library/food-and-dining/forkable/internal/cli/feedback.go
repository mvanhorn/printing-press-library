...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FORKABLE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FORKABLE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "forkable-pp-cli/feedback")
...
When ` + "`FORKABLE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FORKABLE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  forkable-pp-cli feedback list
  forkable-pp-cli feedback list --limit 5
  forkable-pp-cli feedback list --json`,
