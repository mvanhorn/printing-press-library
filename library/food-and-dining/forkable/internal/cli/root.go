...
type rootFlags struct {
	...
	// invocation. Mirrors the FORKABLE_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "forkable-pp-cli",
		Short: `Forkable CLI — A CLI and MCP server for your Forkable office-lunch program, with a local database and history, spend, and preference queries the web app cannot answer, plus commands to set, confirm, and skip meal orders (dry-run by default; --confirm to apply).`,
		Long: `Forkable CLI — A CLI and MCP server for your Forkable office-lunch program, with a local database and history, spend, and preference queries the web app cannot answer, plus commands to set, confirm, and skip meal orders (dry-run by default; pass --confirm to apply).
...
Health check: run 'forkable-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("forkable-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'forkable-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelAllowanceBurnCmd(flags))
	rootCmd.AddCommand(newNovelPreferenceDriftCmd(flags))
	rootCmd.AddCommand(newNovelServedHistoryCmd(flags))
	rootCmd.AddCommand(newNovelSpendTrendCmd(flags))
	rootCmd.AddCommand(newNovelUpcomingDigestCmd(flags))
	rootCmd.AddCommand(newNovelVenueRotationCmd(flags))
	rootCmd.AddCommand(newNovelWhyPickedCmd(flags))
	rootCmd.AddCommand(newMealCmd(flags))
	rootCmd.AddCommand(newNovelReorderCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newAccountPromotedCmd(flags))
	rootCmd.AddCommand(newBuffetAddressesPromotedCmd(flags))
	rootCmd.AddCommand(newClubsPromotedCmd(flags))
	rootCmd.AddCommand(newCsrfPromotedCmd(flags))
	rootCmd.AddCommand(newMealScoresPromotedCmd(flags))
	rootCmd.AddCommand(newMenusPromotedCmd(flags))
	rootCmd.AddCommand(newNotificationsPromotedCmd(flags))
	rootCmd.AddCommand(newVenueUsagePromotedCmd(flags))
