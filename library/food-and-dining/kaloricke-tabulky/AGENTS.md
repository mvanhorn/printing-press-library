# Kalorické Tabulky Printed CLI Agent Guide

This directory is a generated `kaloricke-tabulky-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
kaloricke-tabulky-pp-cli doctor --json
kaloricke-tabulky-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
kaloricke-tabulky-pp-cli which "<capability>" --json
kaloricke-tabulky-pp-cli <command> --help
```
...
```bash
kaloricke-tabulky-pp-cli <command> --agent
```
...
```bash
kaloricke-tabulky-pp-cli <command> --help
kaloricke-tabulky-pp-cli <command> --dry-run --agent
```
