...
In local mode: searches locally synced data only.`,
  ...
  kaloricke-tabulky-pp-cli search "error timeout"

  # Force local search only
  kaloricke-tabulky-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  kaloricke-tabulky-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  kaloricke-tabulky-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /autocomplete/meal
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get("/autocomplete/meal", map[string]string{
					"query": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'kaloricke-tabulky-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "achievements":
				results, err = db.SearchAchievements(query, limit)
			case "notifications":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchAchievements(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search achievements failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchNotifications(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search notifications failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
