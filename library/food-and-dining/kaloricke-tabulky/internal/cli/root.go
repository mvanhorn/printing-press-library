...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "kaloricke-tabulky-pp-cli",
		Short: `Kaloricke Tabulky CLI — Every Kalorické Tabulky feature plus offline FTS5 search, macro-gap windowing, and weight regression — agent-ready Czec…`,
		Long: `Kaloricke Tabulky CLI — Every Kalorické Tabulky feature plus offline FTS5 search, macro-gap windowing, and weight regression — agent-ready Czec…
...
Health check: run 'kaloricke-tabulky-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("kaloricke-tabulky-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'kaloricke-tabulky-pp-cli profile list')")
	...
	authCmd.AddCommand(newAuthPasswordLoginCmd(flags))
	...
	rootCmd.AddCommand(newAchievementsPromotedCmd(flags))
	activityCmd := newActivityPromotedCmd(flags)
	activityCmd.AddCommand(newKTActivityGetCmd(flags))
	rootCmd.AddCommand(activityCmd)
	rootCmd.AddCommand(newFindPromotedCmd(flags))
	foodCmd := newFoodPromotedCmd(flags)
	foodCmd.AddCommand(newKTFoodGetCmd(flags))
	foodCmd.AddCommand(newKTFoodAllergensCmd(flags))
	foodCmd.AddCommand(newKTFoodSubstitutesCmd(flags))
	rootCmd.AddCommand(foodCmd)
	rootCmd.AddCommand(newMealPromotedCmd(flags))
	recipeCmd := newRecipePromotedCmd(flags)
	recipeCmd.AddCommand(newKTRecipeGetCmd(flags))
	rootCmd.AddCommand(recipeCmd)
	rootCmd.AddCommand(newSessionPromotedCmd(flags))
	rootCmd.AddCommand(newStreakPromotedCmd(flags))
	rootCmd.AddCommand(newSummaryPromotedCmd(flags))
	weightCmd := newWeightPromotedCmd(flags)
	weightCmd.AddCommand(newKTWeightRegressionCmd(flags))
	rootCmd.AddCommand(weightCmd)

	// Hand-coded transcendence top-level commands.
	rootCmd.AddCommand(newKTMacrosCmd(flags))
	rootCmd.AddCommand(newKTEnergyCmd(flags))
	rootCmd.AddCommand(newKTBMRCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("kaloricke-tabulky-pp-cli %s\n", version)
		},
	}
}
