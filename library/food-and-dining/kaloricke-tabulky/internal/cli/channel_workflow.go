...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  kaloricke-tabulky-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  kaloricke-tabulky-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"achievements", "favorite", "favorite-foodstuff", "meal", "notifications", "notifications-inapp", "session", "stats", "stats-count"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  kaloricke-tabulky-pp-cli workflow status

  # Show status as JSON
  kaloricke-tabulky-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("kaloricke-tabulky-pp-cli")
			}
