...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".kaloricke-tabulky-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("KALORICKE_TABULKY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("KALORICKE_TABULKY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "kaloricke-tabulky-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.kaloricke-tabulky-pp-cli/feedback.jsonl.
When ` + "`KALORICKE_TABULKY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`KALORICKE_TABULKY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  kaloricke-tabulky-pp-cli feedback list
  kaloricke-tabulky-pp-cli feedback list --limit 5
  kaloricke-tabulky-pp-cli feedback list --json`,
