...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Kaloricke Tabulky",
...
Use --chrome to read cookies from Chrome for .kaloricketabulky.cz.
...
profile by name when the installed backend supports it.`,
		Example: `  kaloricke-tabulky-pp-cli auth login --chrome
  kaloricke-tabulky-pp-cli auth login --browser
  kaloricke-tabulky-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !browserFlag {
				fmt.Fprintln(cmd.OutOrStdout(), "Use --chrome or --browser to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  kaloricke-tabulky-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  kaloricke-tabulky-pp-cli auth login --browser\n")
				return nil
			}

			w := cmd.OutOrStdout()
			domain := ".kaloricketabulky.cz"
			...
			requiredCookies := []string{"JSESSIONID", "kaloricketabulky_token"}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  kaloricke-tabulky-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  kaloricke-tabulky-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: .kaloricketabulky.cz\n")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  kaloricke-tabulky-pp-cli auth set-token eyJhbGciOi...",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  kaloricke-tabulky-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
