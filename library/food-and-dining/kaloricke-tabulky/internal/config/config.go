...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://www.kaloricketabulky.cz",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("KALORICKE_TABULKY_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "kaloricke-tabulky-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("KALORICKE_TABULKY_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
