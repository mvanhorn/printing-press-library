...
- R14. Bug-report reproductions and ambiguity-coverage fixtures pass as integration tests:
  ...
  - **F4**: `restaurants list --query 'pasta' --location portland` returns MEDIUM-tier response (Portland OR picked by popularity prior + Portland ME in `location_warning.alternates`). Agent contract (SKILL.md) surfaces both to the user.
