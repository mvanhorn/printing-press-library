...
// table-reservation-goat-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/food-and-dining/table-reservation-goat/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/food-and-dining/table-reservation-goat/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/food-and-dining/table-reservation-goat/internal/learn/patterns"
...
const noLearnEnvVar = "TABLE_RESERVATION_GOAT_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("table-reservation-goat-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  table-reservation-goat-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  table-reservation-goat-pp-cli recall "<question>" --agent
  table-reservation-goat-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  table-reservation-goat-pp-cli learnings list --agent
  table-reservation-goat-pp-cli learnings list --query "<substring>"
  table-reservation-goat-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  table-reservation-goat-pp-cli learnings forget "<question>" --resource <id>
  table-reservation-goat-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  table-reservation-goat-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  table-reservation-goat-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
