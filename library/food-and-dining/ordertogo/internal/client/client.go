...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		// The live web client sends BOTH the Cookie and the Authorization token
		// on the order POST; a wire capture showed the server's bot/abuse gate
		// 400s requests that look non-browser-like (no cookie, stale UA, missing
		// client hints). So keep the cookie (requires a fresh `auth login`) and
		// match a current Chrome UA.
