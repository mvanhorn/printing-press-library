...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Ordertogo",
	}
...
Use --chrome to read cookies from Chrome for .ordertogo.com.
Use --browser as an alias for --chrome.
Uses github.com/browserutils/kooky to decrypt Chrome cookies and stores them
in ~/.config/ordertogo-pp-cli/cookies.json.`,
		Example: `  ordertogo-pp-cli auth login --chrome
  ordertogo-pp-cli auth login --browser
  ordertogo-pp-cli auth login --chrome --chrome-profile "Profile 1"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !chromeFlag {
				...
				fmt.Fprintf(cmd.OutOrStdout(), "  ordertogo-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  ordertogo-pp-cli auth login --browser\n")
				return nil
			}

			w := cmd.OutOrStdout()
			domain := ".ordertogo.com"

			// PATCH: Verification runs must not touch Chrome's live SQLite DB.
			if os.Getenv("PRINTING_PRESS_VERIFY") == "1" {
				result := map[string]any{
					...
					"cookie_store":   config.CookieStorePath(""),
				}
				return printJSONFiltered(w, result, flags)
			}

			cookies, err := importChromeCookies(cmd.Context(), domain, userDataDir, chromeProfile)
			if err != nil {
				return authErr(err)
			}
			if len(cookies) == 0 {
				return authErr(fmt.Errorf("Chrome profile %q has no cookies for %s", chromeProfile, domain))
			}
			...
			header, err := config.CookieHeaderFromStore("")
			if err != nil {
				return configErr(err)
			}
			if err := cfg.SaveTokens("", "", header, "", time.Time{}); err != nil {
				return configErr(fmt.Errorf("saving cookies: %w", err))
			}

			fmt.Fprintf(w, "%s Imported %d cookies for %s\n", green("OK"), len(cookies), domain)
			fmt.Fprintf(w, "Cookies saved to %s\n", config.CookieStorePath(""))
			return nil
		},
	}

	cmd.Flags().BoolVar(&chromeFlag, "chrome", false, "Read cookies from Chrome")
	...
	cmd.Flags().StringVar(&chromeProfile, "profile", "Default", "Alias for --chrome-profile")
	cmd.Flags().StringVar(&userDataDir, "user-data-dir", "", "Chrome user data directory")
	return cmd
}

func importChromeCookies(ctx context.Context, domain, userDataDir, profile string) ([]config.StoredCookie, error) {
	if userDataDir == "" {
		var err error
		userDataDir, err = chromeDataDir()
		if err != nil {
			return nil, err
		}
	}
	...
	rawCookies, err := chrome.ReadCookies(ctx, cookiesPath, kooky.Valid, kooky.DomainHasSuffix(strings.TrimPrefix(domain, ".")))
	if err != nil {
		return nil, fmt.Errorf("reading Chrome cookies from %s: %w", cookiesPath, err)
	}
	out := make([]config.StoredCookie, 0, len(rawCookies))
	for _, cookie := range rawCookies {
		if cookie == nil || cookie.Name == "" {
			continue
		}
		out = append(out, config.StoredCookie{
			Name:     cookie.Name,
			Value:    cookie.Value,
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  ordertogo-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  ordertogo-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: .ordertogo.com\n")
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  ordertogo-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
