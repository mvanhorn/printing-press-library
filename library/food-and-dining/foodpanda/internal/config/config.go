...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://disco.deliveryhero.io",
	}
	...
	// Env var overrides
	if v := strings.ToLower(strings.TrimSpace(os.Getenv("FOODPANDA_SKIP_TLS_VERIFY"))); v == "1" || v == "true" || v == "yes" || v == "on" {
	...
	// Env-var session credential. foodpanda's customer endpoints authenticate
	// with the `token` JWT cookie that a browser login sets; until now the only
	// way to supply it was `auth login --chrome`, which needs a desktop Chrome
	// profile. That makes the customer-scoped commands unreachable from CI, a
	// container, or an agent sandbox. Reading the credential from the
	// environment is the headless equivalent of that login.
	//
	// Env wins over the config file, matching AuthHeader's documented
	// precedence. Accepts either the bare JWT or a full cookie-jar string
	// ("token=eyJ...; other=..."), because both are what a user actually has
	// on hand: the former from devtools, the latter from a cookie export.
	if v := strings.TrimSpace(os.Getenv("FOODPANDA_TOKEN")); v != "" {
		cfg.AccessToken = extractSessionToken(v)
		cfg.AuthSource = "env:FOODPANDA_TOKEN"
		cfg.CredentialSource = "FOODPANDA_TOKEN environment variable"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("FOODPANDA_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}

// extractSessionToken normalises what FOODPANDA_TOKEN may contain. A cookie-jar
// string is reduced to the `token` cookie's value; anything else is returned
// trimmed and used as-is, so a bare JWT still works. Returning the bare value
// keeps CookieCredential and AuthHeader consistent: both read AccessToken, and
// neither should have to know which of the two shapes the user pasted.
func extractSessionToken(raw string) string {
	...
	for _, part := range strings.Split(raw, ";") {
		...
		if found && strings.EqualFold(strings.TrimSpace(name), "token") {
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("FOODPANDA_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "foodpanda-pp-cli", "config.toml"), nil
}
