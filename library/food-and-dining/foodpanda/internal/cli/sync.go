...
Resource scoping:
  ...
  foodpanda-pp-cli sync

  # Sync specific resources only
  foodpanda-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  foodpanda-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  foodpanda-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  foodpanda-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  foodpanda-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "foodpanda-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
...
func syncResource(ctx context.Context, c interface {
	...
	// Record the scope this sync ran with. foodpanda's listing endpoints are
	// geo-scoped, so this is what makes an unattended refresh reproducible;
	// without it --refresh-stale can only report that data is stale. Failure
	// is not fatal: the sync itself succeeded, and a missing scope only costs
	// the refresh path, which degrades to a skip-with-hint.
	if userParams != nil && len(userParams.trueGlobal) > 0 {
		_ = db.SaveSyncParams(resource, userParams.trueGlobal)
	}
...
func responsePathForResource(resource, path string) []string {
	...
	case "menu\x00/api/v5/vendors/{vendor_code}":
		return []string{"data"}
	case "reviews\x00/reviews/vendor/{vendor_code}":
		return []string{"data"}
	case "vendors\x00/listing/api/v1/pandora/vendors":
		return []string{"data.items"}
	case "vendors\x00/listing/api/v1/pandora/search":
		return []string{"data.items"}
	}
	return nil
}
