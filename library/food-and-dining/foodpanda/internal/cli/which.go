...
var whichIndex = []whichEntry{
	{Command: "home", Description: "Rank every restaurant near your saved home address by what delivery actually costs you.", Group: "Local state that compounds", WhyItMatters: "Reach for this when the question is 'what is cheapest to get delivered to me', not 'what is this one restaurant's fee'."},
	{Command: "dish", Description: "Find which nearby restaurant sells a specific dish cheapest, searching every synced menu at once.", Group: "Local state that compounds", WhyItMatters: "Use this for item-level price hunting; use search when you want restaurants by name instead."},
	{Command: "menu-diff", Description: "Show what changed in a restaurant's menu and prices between two syncs.", Group: "Local state that compounds", WhyItMatters: "Use this to catch price rises, removed items, or newly added deals over time."},
	{Command: "posture", Description: "Rank vendors by advertising and placement signals: CPC ad participation, promoted and premium status, and ranking score.", Group: "Competitive intelligence", WhyItMatters: "Use this for competitive analysis of who buys placement. It does not report merchant commission rates, which are not exposed in any consumer surface."},
	{Command: "coverage", Description: "Determine which vendors actually deliver to an arbitrary point using each vendor's published delivery radius.", Group: "Competitive intelligence", WhyItMatters: "Use this before assuming a restaurant is orderable from an address you have not tried."},
	{Command: "fees", Description: "Compare the full cost structure across an area: delivery fee, minimum order, service fee and VAT together.", Group: "Competitive intelligence", WhyItMatters: "Use this when headline delivery fee is misleading because service fee or minimum order dominates."},
	{Command: "digest", Description: "Split a restaurant's blended star rating into per-topic scores so food quality and delivery quality are separated.", Group: "Signal the site throws away", WhyItMatters: "Use this to tell 'the food is bad' apart from 'the delivery is bad' before trusting a rating."},
	{Command: "market-compare", Description: "Run the same query across every foodpanda market and compare vendor counts, ratings and fees side by side.", Group: "Signal the site throws away", WhyItMatters: "Use this for regional benchmarking; use vendors list when you only care about one city."},
	{Command: "find", Description: "Search vendors live upstream and label how strongly each result actually matched the query.", Group: "Signal the site throws away", WhyItMatters: "Use this for live upstream search where match quality matters; use 'search' instead for offline full-text search over already-synced data."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  foodpanda-pp-cli which "stale tickets"
  foodpanda-pp-cli which "bottleneck"
  foodpanda-pp-cli which --limit 1 "send message"
  foodpanda-pp-cli which                                # list the full capability index`,
