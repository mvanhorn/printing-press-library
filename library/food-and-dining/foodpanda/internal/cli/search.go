...
In local mode: searches locally synced data only.`,
  ...
  foodpanda-pp-cli search "error timeout"

  # Force local search only
  foodpanda-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  foodpanda-pp-cli search "status" --type menu --data-source local
  # JSON output for piping
  foodpanda-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /listing/api/v1/pandora/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/listing/api/v1/pandora/search", map[string]string{
					"query": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'foodpanda-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
