...
type rootFlags struct {
	...
	// invocation. Mirrors the FOODPANDA_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "foodpanda-pp-cli",
		Short: `Foodpanda CLI — Every foodpanda restaurant, menu and price in a local database — with cross-restaurant dish search, price history and…`,
		Long: `Foodpanda CLI — Every foodpanda restaurant, menu and price in a local database — with cross-restaurant dish search, price history and fee comparison the app cannot do.
...
Health check: run 'foodpanda-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("foodpanda-pp-cli {{ .Version }}\n")

	rootCmd.PersistentFlags().BoolVar(&flags.refreshStale, "refresh-stale", false,
		"Re-sync local data older than the staleness threshold before running (off by default; also FOODPANDA_AUTO_REFRESH=1)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'foodpanda-pp-cli profile list')")
	...
	rootCmd.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
		...
		// Opt-in staleness refresh. Reuses learnHookSkipList, which was
		// shipped for exactly this consumer: the framework commands that
		// must never touch the local store are the same set a refresh
		// must never fire for. Progress goes to stderr so a refresh
		// notice never contaminates --json stdout.
		if !shouldSkipLearnHook(cmd.CommandPath()) {
			autoRefreshIfStale(cmd.Context(), flags, cmd.ErrOrStderr())
		}
		return nil
	}
	...
	addNovelCommandIfAbsent(rootCmd, newNovelDishCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelFeesCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelFindCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelHomeCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelMarketCompareCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelMenuDiffCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelPostureCmd(flags))
