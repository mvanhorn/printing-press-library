...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/food-and-dining/foodpanda/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/food-and-dining/foodpanda/internal/learn/lookups"
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"city": {
			{Canonical: "Karachi", Values: []string{"khi", "karach"}},
			{Canonical: "Lahore", Values: []string{"lhr", "lahor"}},
			{Canonical: "Islamabad", Values: []string{"isb", "isl"}},
			{Canonical: "Rawalpindi", Values: []string{"pindi", "rwp"}},
			{Canonical: "Faisalabad", Values: []string{"fsd", "lyallpur"}},
			{Canonical: "Multan", Values: []string{"mux"}},
			{Canonical: "Peshawar", Values: []string{"pesh"}},
		},
		"cuisine": {
			{Canonical: "Biryani", Values: []string{"biriyani", "biryani rice", "pulao rice"}},
			{Canonical: "Pizza", Values: []string{"pizzas", "italian pizza"}},
			{Canonical: "Burgers", Values: []string{"burger", "smash burger", "zinger"}},
			{Canonical: "BBQ", Values: []string{"bar b q", "barbeque", "barbecue", "tikka"}},
			{Canonical: "Desserts", Values: []string{"dessert", "sweets", "mithai"}},
			{Canonical: "Cakes & Bakery", Values: []string{"bakery", "cake", "pastry"}},
			{Canonical: "Beverages", Values: []string{"drinks", "juice", "shakes"}},
			{Canonical: "Fast Food", Values: []string{"fastfood", "quick bites"}},
			{Canonical: "Chinese", Values: []string{"chinese food", "indo chinese"}},
			{Canonical: "Pakistani", Values: []string{"desi", "local", "traditional"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: foodpanda-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: foodpanda-pp-cli: learn init: %v\n", err)
		}
	})
}
