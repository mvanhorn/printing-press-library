...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FOODPANDA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FOODPANDA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "foodpanda-pp-cli/feedback")
...
When ` + "`FOODPANDA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FOODPANDA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  foodpanda-pp-cli feedback list
  foodpanda-pp-cli feedback list --limit 5
  foodpanda-pp-cli feedback list --json`,
