...
Run sync first to populate the local search index.`,
  ...
  cookunity-pp-cli search "status"

  # Force local search explicitly
  cookunity-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  cookunity-pp-cli search "status" --type meals --data-source local
  # JSON output for piping
  cookunity-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'cookunity-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "meals":
				results, err = db.SearchMeals(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchMeals(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search meals failed: %w", searchErr)
					}
