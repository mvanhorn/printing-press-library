...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/food-and-dining/cookunity/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/food-and-dining/cookunity/internal/learn/lookups"
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"category": {
			{Canonical: "Meals", Values: []string{"lunch", "dinner", "entree", "entrees"}},
			{Canonical: "Breakfast", Values: []string{"morning", "brunch"}},
			{Canonical: "Bundles", Values: []string{"bundle", "better together"}},
			{Canonical: "Proteins only", Values: []string{"protein only", "protein please"}},
			{Canonical: "Drinks", Values: []string{"beverages", "smart sips"}},
			{Canonical: "Treats", Values: []string{"dessert", "desserts", "little delights"}},
		},
		"cuisine": {
			{Canonical: "american", Values: []string{"american comfort", "southwest american"}},
			{Canonical: "italian", Values: []string{"pasta", "risotto"}},
			{Canonical: "asian", Values: []string{"pan-asian", "thai", "authentic thai", "traditional chinese", "chinese"}},
			{Canonical: "latin american", Values: []string{"latin", "mexican"}},
			{Canonical: "indian", Values: []string{}},
			{Canonical: "caribbean", Values: []string{}},
			{Canonical: "middle eastern", Values: []string{"israeli", "israeli and middle eastern"}},
		},
		"diet": {
			{Canonical: "high protein", Values: []string{"high-protein", "protein"}},
			{Canonical: "gluten free", Values: []string{"gluten-free", "gf", "no gluten"}},
			{Canonical: "dairy free", Values: []string{"dairy-free", "df", "no dairy"}},
			{Canonical: "keto", Values: []string{"ketogenic", "low carb", "low-carb"}},
			{Canonical: "paleo", Values: []string{}},
			{Canonical: "vegan", Values: []string{"plant based", "plant-powered", "plant powered"}},
			{Canonical: "vegetarian", Values: []string{"veggie", "meatless"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: cookunity-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: cookunity-pp-cli: learn init: %v\n", err)
		}
	})
}
