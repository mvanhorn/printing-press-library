...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("COOKUNITY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("COOKUNITY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "cookunity-pp-cli/feedback")
...
When ` + "`COOKUNITY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`COOKUNITY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cookunity-pp-cli feedback list
  cookunity-pp-cli feedback list --limit 5
  cookunity-pp-cli feedback list --json`,
