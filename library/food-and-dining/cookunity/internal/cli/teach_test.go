...
func withTempLearnHome(t *testing.T) string {
	...
	dir := t.TempDir()
	t.Setenv("HOME", dir)
	// Go resolves the user home dir via USERPROFILE on Windows (not HOME), so
	// redirect it too or defaultDBPath (~/.local/share/...) leaks to the real
	// user profile and cross-test data.db pollution fails isolation checks.
	t.Setenv("USERPROFILE", dir)
	t.Setenv(noLearnEnvVar, "")
	t.Setenv("COOKUNITY_STATE_DIR", "")
	t.Setenv("COOKUNITY_HOME", "")
...
func TestSkipLearnHook_FrameworkCommandsSkipped(t *testing.T) {
	cases := []string{
		"cookunity-pp-cli auth login",
		"cookunity-pp-cli doctor",
		"cookunity-pp-cli help items",
		"cookunity-pp-cli sync",
		"cookunity-pp-cli profile list",
		"cookunity-pp-cli feedback list",
		"cookunity-pp-cli which",
		"cookunity-pp-cli agent-context",
		"cookunity-pp-cli completion bash",
		"cookunity-pp-cli version",
	}
...
func TestSkipLearnHook_NovelCommandsNotSkipped(t *testing.T) {
	novel := []string{
		...
		"cookunity-pp-cli recall",
		"cookunity-pp-cli learnings",
		"cookunity-pp-cli teach-pattern",
		"cookunity-pp-cli teach-lookup",
		"cookunity-pp-cli items list",
	}
...
func TestSkipLearnHook_FrameworkCommandsDoNotCreateDefaultDB(t *testing.T) {
	...
	for _, args := range cases {
		...
		if _, err := os.Stat(defaultDBPath("cookunity-pp-cli")); err == nil {
