...
// TestMigrate_AddsColumnsOnUpgrade_Meals verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_Meals(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "meals" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	rows, err := s.DB().Query(`PRAGMA table_info("meals")`)
	if err != nil {
	...
	for _, want := range []string{
		...
		"public_url",
		"image",
		"heat_instructions_microwave",
		"heat_instructions_oven",
		"ingredients",
		"weight",
		"delivery_date",
	} {
		if !hasColumn[want] {
			t.Fatalf("%s column missing from meals after migrate", want)
		}
	}
}
