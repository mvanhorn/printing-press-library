...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("IFOOD_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("IFOOD_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ifood-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example:     "  ifood-pp-cli feedback \"Product search wording was unclear\" --dry-run",
...
When ` + "`IFOOD_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`IFOOD_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if flags.dryRun {
				return writeDryRun(cmd.OutOrStdout(), flags, "record feedback locally")
			}
			var text string
			if useStdin {
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ifood-pp-cli feedback list
  ifood-pp-cli feedback list --limit 5
  ifood-pp-cli feedback list --json`,
