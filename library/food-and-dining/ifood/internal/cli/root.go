...
type rootFlags struct {
	...
	// invocation. Mirrors the IFOOD_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "ifood-pp-cli",
		Short: `Ifood CLI — Agent-ready iFood grocery quoting with browser-owned authentication and confirmation-gated cart planning.`,
		Long: `Ifood CLI — Agent-ready iFood grocery quoting with browser-owned authentication and confirmation-gated cart planning.
...
Highlights (not in the official API docs):
  ...
  • cart build   Resolve product terms into a read-only cart-item preview; cart add owns the guarded write.

Agent mode: add --agent to any command for JSON output + non-interactive mode.
Health check: run 'ifood-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("ifood-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'ifood-pp-cli profile list')")
	...
	rootCmd.AddCommand(newBmPromotedCmd(flags))
	rootCmd.AddCommand(newIfoodWebReadSearchPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelBrowserCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelCartCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelQuoteCmd(flags))
