...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ifood-pp-cli doctor
  ifood-pp-cli doctor --json
  ifood-pp-cli doctor --fail-on warn
  ifood-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if !configured {
					...
					report["auth_hint"] = "Set it with: ifood-pp-cli auth set-token <token> or export IFOOD_BEARER_AUTH=\"your-token-here\""
					report["auth_docs_url"] = "https://www.ifood.com.br"
				} else {
					authConfigured = true
			...
			if os.Getenv("IFOOD_BEARER_AUTH") != "" {
				authEnvSet = append(authEnvSet, "IFOOD_BEARER_AUTH")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "IFOOD_BEARER_AUTH")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "ifood-pp-cli", suggestion)
						} else {
