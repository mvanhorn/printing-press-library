...
var whichIndex = []whichEntry{
	{Command: "browser plan", Description: "Emit the complete browser-owned quotation and cart workflow without exporting credentials.", Group: "Browser-backed agent workflow", WhyItMatters: "Agents get deterministic requirements while credentials remain inside the browser."},
	{Command: "browser validate-quote", Description: "Validate complete product observations from at least three markets meeting a configurable rating floor.", Group: "Browser-backed agent workflow", WhyItMatters: "The CLI fails closed on missing, unavailable, unnamed, or unpriced products."},
	{Command: "browser cart-plan", Description: "Produce the exact selected merchant, products, quantities, and expected total before any cart interaction.", Group: "Guarded cart boundary", WhyItMatters: "Agents can request approval for a concrete plan and stop on substitutions or price changes."},
	{Command: "quote", Description: "Compare requested grocery terms across eligible markets and return complete price estimates.", Group: "Direct API composition", WhyItMatters: "Agents can inspect the direct-API composition when a controlled session supports it."},
	{Command: "cart build", Description: "Resolve product terms into an exact read-only cart-item preview.", Group: "Guarded cart boundary", WhyItMatters: "Product resolution stays separate from the confirmation-gated cart add write, and checkout is never attempted."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  ifood-pp-cli which "stale tickets"
  ifood-pp-cli which "bottleneck"
  ifood-pp-cli which --limit 1 "send message"
  ifood-pp-cli which                                # list the full capability index`,
