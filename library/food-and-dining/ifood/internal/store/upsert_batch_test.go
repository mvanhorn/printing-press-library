...
func TestUpsertBatch_TypedFailureDoesNotStrandPromotionsGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("promotions count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "merchants_id")
	}
}

// TestUpsertBatch_KeysPromotionsByChildAndParent verifies dependent
...
func TestUpsertBatch_KeysPromotionsByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("promotions count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "merchants_id" = ?`, "promotions")
