...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("inspect_direct_cart_composition_safely",
			mcplib.WithDescription("Shows preview-first cart composition options without running a mutation."),
			mcplib.WithBoolean("help", mcplib.Description("Override the recipe's --help value.")),
		),
		handleInspectDirectCartCompositionSafely,
	)
}

// handleInspectDirectCartCompositionSafely runs the inspect_direct_cart_composition_safely recipe intent: Shows preview-first cart composition options without running a mutation.
func handleInspectDirectCartCompositionSafely(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	args = appendRecipeBoolFlag(args, "help", input["help"], true)
