...
type Config struct {
	...
	IfoodBearerAuth  string          `toml:"bearer_auth"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://www.ifood.com.br/site-api",
	}
	...
	// Env var overrides
	if v := strings.ToLower(strings.TrimSpace(os.Getenv("IFOOD_SKIP_TLS_VERIFY"))); v == "1" || v == "true" || v == "yes" || v == "on" {
		cfg.SetSkipTLSVerify(true)
	}
	if v := os.Getenv("IFOOD_BEARER_AUTH"); v != "" {
		cfg.IfoodBearerAuth = v
		cfg.markEnvOverride("IfoodBearerAuth")
		cfg.AuthSource = "env:IFOOD_BEARER_AUTH"
		cfg.CredentialSource = "env:IFOOD_BEARER_AUTH"
	}
	...
	if v := os.Getenv("IFOOD_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("IFOOD_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "ifood-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.IfoodBearerAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:IFOOD_BEARER_AUTH"
		}
		return "Bearer " + c.IfoodBearerAuth
	}
	if c.AccessToken != "" {
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		IfoodBearerAuth: c.IfoodBearerAuth,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.IfoodBearerAuth == "" {
		c.IfoodBearerAuth = creds.IfoodBearerAuth
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.IfoodBearerAuth = ""
	delete(c.envOverrides, "IfoodBearerAuth")
	c.updateFileConfigField("IfoodBearerAuth")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	...
	if c.fileConfig != nil {
		...
		if c.envOverrides["IfoodBearerAuth"] {
			out.IfoodBearerAuth = c.fileConfig.IfoodBearerAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "IfoodBearerAuth":
		c.fileConfig.IfoodBearerAuth = c.IfoodBearerAuth
	}
}
