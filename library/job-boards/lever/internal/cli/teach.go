...
// lever-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/job-boards/lever/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/job-boards/lever/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/job-boards/lever/internal/learn/patterns"
...
const noLearnEnvVar = "LEVER_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("lever-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  lever-pp-cli teach --query "who owns acme" --resource acme --resource-type company`,
...
when learnings exist.`,
		Example: `  lever-pp-cli recall "<question>" --agent
  lever-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  lever-pp-cli learnings list --agent
  lever-pp-cli learnings list --query "<substring>"
  lever-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  lever-pp-cli learnings forget "<question>" --resource <id>
  lever-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  lever-pp-cli teach-pattern --query-template "items in {entity}" --resource-template "GROUP-{entity:category}" --resource-type items --entity-kind category --strategy substitute`,
