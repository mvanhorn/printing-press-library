...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("LEVER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("LEVER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/job-boards/lever/feedback")
...
When ` + "`LEVER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`LEVER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  lever-pp-cli feedback list
  lever-pp-cli feedback list --limit 5
  lever-pp-cli feedback list --json`,
