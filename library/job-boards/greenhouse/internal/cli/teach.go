...
// greenhouse-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/job-boards/greenhouse/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/job-boards/greenhouse/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/job-boards/greenhouse/internal/learn/patterns"
...
const noLearnEnvVar = "GREENHOUSE_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("greenhouse-pp-cli")
}
...
when learnings exist.`,
		Example: `  greenhouse-pp-cli recall "<question>" --agent
  greenhouse-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  greenhouse-pp-cli learnings list --agent
  greenhouse-pp-cli learnings list --query "<substring>"
  greenhouse-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  greenhouse-pp-cli learnings forget "<question>" --resource <id>
  greenhouse-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  greenhouse-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
