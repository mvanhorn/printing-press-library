...
func withTempLearnHome(t *testing.T) string {
	...
	if runtime.GOOS == "windows" {
		// os.UserHomeDir() reads %USERPROFILE% (or HOMEDRIVE+HOMEPATH) on
		// Windows, not $HOME. Isolate the platform-native home vars too,
		// otherwise the real user state dir leaks into these tests.
	...
	t.Setenv("GREENHOUSE_STATE_DIR", "")
	t.Setenv("GREENHOUSE_HOME", "")
...
func TestSkipLearnHook_FrameworkCommandsSkipped(t *testing.T) {
	cases := []string{
		"greenhouse-pp-cli auth login",
		"greenhouse-pp-cli doctor",
		"greenhouse-pp-cli help items",
		"greenhouse-pp-cli sync",
		"greenhouse-pp-cli profile list",
		"greenhouse-pp-cli feedback list",
		"greenhouse-pp-cli which",
		"greenhouse-pp-cli agent-context",
		"greenhouse-pp-cli completion bash",
		"greenhouse-pp-cli version",
	}
...
func TestSkipLearnHook_NovelCommandsNotSkipped(t *testing.T) {
	novel := []string{
		...
		"greenhouse-pp-cli recall",
		"greenhouse-pp-cli learnings",
		"greenhouse-pp-cli teach-pattern",
		"greenhouse-pp-cli teach-lookup",
		"greenhouse-pp-cli items list",
	}
...
func TestSkipLearnHook_FrameworkCommandsDoNotCreateDefaultDB(t *testing.T) {
	...
	for _, args := range cases {
		...
		if _, err := os.Stat(defaultDBPath("greenhouse-pp-cli")); err == nil {
