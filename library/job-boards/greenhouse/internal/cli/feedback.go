...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GREENHOUSE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GREENHOUSE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/job-boards/greenhouse/feedback")
...
When ` + "`GREENHOUSE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GREENHOUSE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  greenhouse-pp-cli feedback list
  greenhouse-pp-cli feedback list --limit 5
  greenhouse-pp-cli feedback list --json`,
