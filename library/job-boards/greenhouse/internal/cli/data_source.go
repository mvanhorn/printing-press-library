...
func resolveReadWithStrategyResponsePathAndJSONGuard(ctx context.Context, c *client.Client, flags *rootFlags, strategy string, resourceType string, isList bool, path string, params map[string]string, headers map[string]string, responsePath string, guardLiveJSON bool, hintWriter io.Writer) (json.RawMessage, DataProvenance, error) {
	...
	default: // "auto"
		...
		if fallbackErr != nil {
			return nil, DataProvenance{}, fmt.Errorf("API unreachable and no local data. Run 'greenhouse-pp-cli sync' to enable offline access.\n\nOriginal error: %w", err)
		}
...
func writeThroughCache(ctx context.Context, resourceType string, data json.RawMessage) {
	db, err := store.OpenWithContext(ctx, defaultDBPath("greenhouse-pp-cli"))
	if err != nil {
		return
	}
	defer db.Close()
...
// boardScopedType namespaces local-store rows per board token. The sync
// command writes under "jobs:stripe", "departments:stripe", etc., so
// syncing several companies never mixes rows: a local list for one board
// must not return another board's cached data. Reads without a board
// token in the path stay unscoped.
func boardScopedType(resourceType, path string) string {
	parts := strings.Split(strings.Trim(path, "/"), "/")
	if len(parts) < 2 {
		return resourceType
	}
	switch resourceType {
	case "jobs", "departments", "offices":
...
func resolveLocal(ctx context.Context, flags *rootFlags, hintWriter io.Writer, resourceType string, isList bool, path string, params map[string]string, reason string) (json.RawMessage, DataProvenance, error) {
	db, err := openStoreForRead(ctx, "greenhouse-pp-cli")
	if err != nil {
		return nil, DataProvenance{}, fmt.Errorf("opening local database: %w\nRun 'greenhouse-pp-cli sync' first.", err)
	}
	if db == nil {
		return nil, DataProvenance{}, fmt.Errorf("no local data. Run 'greenhouse-pp-cli sync' first")
	}
	defer db.Close()

	scoped := boardScopedType(resourceType, path)

	if flags != nil {
		emitSyncHints(hintWriter, db, scoped, flags.maxAge)
	}

	prov := localProvenance(db, scoped, reason)
	...
	if isList {
		raw, err := db.List(scoped, 0) // 0 = no limit, return all synced data
		if err != nil {
		...
		if len(items) == 0 {
			return nil, DataProvenance{}, fmt.Errorf("no local data for %q. Run 'greenhouse-pp-cli sync' first", resourceType)
		}
	...
	item, err := db.Get(scoped, id)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, DataProvenance{}, fmt.Errorf("resource %q with ID %q not found in local store. Run 'greenhouse-pp-cli sync' first", resourceType, id)
		}
		return nil, DataProvenance{}, fmt.Errorf("querying local store: %w", err)
	}
