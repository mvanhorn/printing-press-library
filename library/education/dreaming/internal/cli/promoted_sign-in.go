...
func newSignInPromotedCmd(flags *rootFlags) *cobra.Command {
	var bodyEmail string
	var bodyPassword string

	cmd := &cobra.Command{
		Use:         "sign-in",
		Short:       "Exchange email/password for a bearer token",
		Long:        "Exchange email/password for a bearer token",
		Example:     "  dreaming-pp-cli sign-in --email user@example.com",
		Annotations: map[string]string{"pp:endpoint": "sign-in.sign_in", "pp:method": "POST", "pp:path": "/signIn"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !flags.dryRun && statusCode >= 200 && statusCode < 300 && (partialFailure == nil || flags.allowPartialFailure) {
				writeMutationResponseToStore(cmd.Context(), "sign-in", data, "")
			}
