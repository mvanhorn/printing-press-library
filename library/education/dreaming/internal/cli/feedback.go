...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "dreaming-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DREAMING_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DREAMING_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "dreaming-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/dreaming-pp-cli/feedback.jsonl.
When ` + "`DREAMING_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DREAMING_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  dreaming-pp-cli feedback list
  dreaming-pp-cli feedback list --limit 5
  dreaming-pp-cli feedback list --json`,
