...
Resource scoping:
  ...
  dreaming-pp-cli sync

  # Sync specific resources only
  dreaming-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  dreaming-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  dreaming-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  dreaming-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  dreaming-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("dreaming-pp-cli")
			}
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "dreaming-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/dreaming-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "day-watched-time":
		return db.UpsertDayWatchedTime(data)
	case "external-time":
		return db.UpsertExternalTime(data)
	case "playlist":
		return db.UpsertPlaylist(data)
	case "user":
...
func defaultSyncResources() []string {
	// These four list endpoints populate the local store that the offline
	// analytics/corpus commands query. (user is a single object, refreshed
	// lazily by stats/roadmap/plan rather than batch-synced.)
	return []string{"videos", "day-watched-time", "external-time", "playlist"}
}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"day-watched-time": "/dayWatchedTime",
		"external-time":    "/externalTime",
		"playlist":         "/playlist",
		"videos":           "/videos",
	}
...
var pageItemKeys = []string{
	...
	"Data", "Results", "Items", "Records", "Nodes", "Entries", "Features",
	// Dreaming envelopes: /videos -> {videos:[...], guides:[...]} (two arrays,
	// so the single-array fallback can't disambiguate); /externalTime ->
	// {externalTimes:[...]}. Naming the collection keys explicitly resolves it.
	"videos", "Videos", "externalTimes", "ExternalTimes",
}
