# Dreaming Printed CLI Agent Guide

This directory is a generated `dreaming-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
dreaming-pp-cli doctor --json
dreaming-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
dreaming-pp-cli which "<capability>" --json
dreaming-pp-cli <command> --help
```
...
```bash
dreaming-pp-cli <command> --help
dreaming-pp-cli <command> --dry-run --agent
```
