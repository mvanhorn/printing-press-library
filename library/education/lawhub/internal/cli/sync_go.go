...
type instancePayload struct {
	State struct {
		Sections []instanceSection `json:"sections"`
	} `json:"state"`
}
type instanceSection struct {
	FormID                             string `json:"formId"`
	StartTime, EndTime, LastUpdateTime string
	TimeRemaining                      int
	IsSectionExposed                   bool
...
type answerOption struct {
	...
	OptionState int    `json:"optionState"`
}
...
func browserPage() (browser *rod.Browser, page *rod.Page, err error) {
	...
	launcher := launcher.New().Bin(browserExe).Headless(true).NoSandbox(true)
	...
	for _, c := range st.Cookies {
		protoCookie := &proto.NetworkCookieParam{Name: c.Name, Value: c.Value, Domain: c.Domain, Path: c.Path, HTTPOnly: c.HTTPOnly, Secure: c.Secure}
		_ = p.SetCookies([]*proto.NetworkCookieParam{protoCookie})
	}
...
func syncHistoryCmd() *cobra.Command {
	...
	c := &cobra.Command{Use: "history", RunE: func(cmd *cobra.Command, args []string) error {
		...
		for _, m := range mods {
			api := fmt.Sprintf("https://app.lawhub.org/api/request/v2/api/user/%s/history/%s?PageNumber=1&SortOrder=desc&SortField=startDate&PageSize=%d", url.PathEscape(userID), url.PathEscape(m), pageSize)
			raw := p.MustEval(`async u => JSON.stringify(await fetch(u).then(r=>r.json()))`, api).String()
			var hr historyResp
			if err := json.Unmarshal([]byte(raw), &hr); err != nil {
				details[m] = map[string]any{"error": err.Error()}
				continue
			}
			details[m] = map[string]any{"items": len(hr.Items), "recordCount": hr.RecordCount}
...
func upsertReportQuestions(db *sql.DB, attemptID string, rows []reportRow) int {
	...
	for _, r := range rows {
		res, _ := db.Exec(`UPDATE questions SET question_type=?, chosen_answer=?, correct_answer=?, is_correct=?, time_spent_seconds=COALESCE(?,time_spent_seconds), flagged=?, source_url=?, answered=?, difficulty=? WHERE attempt_id=? AND section_index=? AND question_number=?`,
			r.QuestionType, r.ChosenAnswer, nullIfEmpty(r.CorrectAnswer), r.IsCorrect, r.TimeSpentSeconds, r.Flagged, fmt.Sprintf("https://app.lawhub.org/question/%s/Section%%20%d", attemptID, r.SectionIndex), boolInt(r.ChosenAnswer != ""), nullIfZero(r.Difficulty), attemptID, r.SectionIndex, r.QuestionNumber)
...
func normalizeReportRows(rows []reportRow) []reportRow {
	for i := range rows {
		...
		rows[i].Flagged = 1
		if strings.HasPrefix(strings.ToLower(rows[i].FlaggedText), "not") {
			rows[i].Flagged = 0
		}
...
func updateReportRows(db *sql.DB, id string, rows []reportRow) int {
	updated := 0
	for _, r := range rows {
		res, _ := db.Exec(`UPDATE questions SET question_type=?, chosen_answer=?, correct_answer=?, is_correct=?, time_spent_seconds=COALESCE(?,time_spent_seconds), flagged=?, difficulty=?, answered=? WHERE attempt_id=? AND section_index=? AND question_number=?`, r.QuestionType, r.ChosenAnswer, nullIfEmpty(r.CorrectAnswer), r.IsCorrect, r.TimeSpentSeconds, r.Flagged, nullIfZero(r.Difficulty), boolInt(r.ChosenAnswer != ""), id, r.SectionIndex, r.QuestionNumber)
		n, _ := res.RowsAffected()
		if n == 0 {
