...
After attaching to the user's logged-in main Chrome profile over CDP on port `55521`, the account dashboard live-smoke passed against managed domain pages. A broader browser-backed audit on 2026-05-14 checked five domains through the logged-in browser context and through the installed CLI using the same session cookie.
...
May 14 multi-domain browser audit:

```text
Domains checked: 5
Browser page surfaces per domain: overview, DNS settings, email, billing
Browser API reads per domain: byName, permissions, registrar-info, custom-record-set,
presets, email-forwarding, email-forwarding MX conflict check, forwarding-presets,
is-unparked, website-domains, billing eligibility, billing valid terms
Result: all core browser API reads returned 200 or 204
Observed dashboard quirk: /api/account/1/domains/<domain-id>/category returned 500
for every checked domain in the browser context, so it is not used as a health gate.
```
...
```text
Domains checked: 5
Commands per domain: get, permissions, registrar-info, custom-records, presets,
email-forwarding, email-mx-conflicts, billing-eligibility, billing-valid-terms
Result: 45/45 CLI calls succeeded
Email forwarding: active forwarding payloads returned for four domains; one domain
returned HTTP 204/no-content, matching the browser API response.
Billing: valid renewal terms returned for all five domains.
```
...
Redacted local evidence files from the May 14 audit:

```text
/Users/zaydk/Desktop/Squarespace CLI Project/evidence/browser-live-account-audit.json
/Users/zaydk/Desktop/Squarespace CLI Project/evidence/live-cli-account-audit.json
```

Those files store response status, response shape, top-level keys, counts, and surface signals only. They do not store cookies, request headers, raw private API payloads, or account-specific internal ids.
