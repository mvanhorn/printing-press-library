# Amazon Ae Printed CLI Agent Guide

This directory is a generated `amazon-ae-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
amazon-ae-pp-cli doctor --json
amazon-ae-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
amazon-ae-pp-cli which "<capability>" --json
amazon-ae-pp-cli <command> --help
```
...
```bash
amazon-ae-pp-cli <command> --agent
```
...
```bash
amazon-ae-pp-cli <command> --help
amazon-ae-pp-cli <command> --dry-run --agent
```
