...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Open Woot All Deals to capture a current key: https://www.woot.com/alldeals"+
			"\n      Run 'woot-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API key."+
			" Set your API key with: export D24QG5ZSX8XDC4_CLOUDFRONT_API_KEY=\"your-token-here\""+
			"\n      Open Woot All Deals to capture a current key: https://www.woot.com/alldeals"+
			"\n      Run 'woot-pp-cli doctor' to check auth status.", err))
...
func printOutputWithFlagsMeta(w io.Writer, data json.RawMessage, flags *rootFlags, agentMeta map[string]any) error {
	if flags.agent && flags.asJSON && !flags.csv && !flags.plain && !flags.quiet {
		data, agentMeta = unwrapAgentEnvelope(data, agentMeta)
	}
...
// unwrapAgentEnvelope prevents commands that already return {meta,results}
// from being nested inside a second generic agent envelope. Existing
// provenance wins; fallback metadata fills only missing keys.
func unwrapAgentEnvelope(data json.RawMessage, fallback map[string]any) (json.RawMessage, map[string]any) {
	var envelope struct {
		Meta    map[string]any  `json:"meta"`
		Results json.RawMessage `json:"results"`
	}
	if err := json.Unmarshal(data, &envelope); err != nil || envelope.Results == nil || envelope.Meta == nil {
		return data, fallback
	}
	for key, value := range fallback {
		if _, exists := envelope.Meta[key]; !exists {
			envelope.Meta[key] = value
		}
	}
	return envelope.Results, envelope.Meta
}
...
type DataProvenance struct {
	...
	Incomplete   bool       `json:"incomplete,omitempty"`    // local snapshot stopped before the reported catalog boundary
	ResumeCursor string     `json:"resume_cursor,omitempty"` // cursor retained for the next sync invocation
...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Set your API key with: export D24QG5ZSX8XDC4_CLOUDFRONT_API_KEY=\"your-token-here\""))
	}
	if len(trimmed) == 0 {
...
func wrapWithProvenance(data json.RawMessage, prov DataProvenance) (json.RawMessage, error) {
	...
	if prov.ResumeCursor != "" {
		meta["resume_cursor"] = prov.ResumeCursor
	}
