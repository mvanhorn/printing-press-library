...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("WOOT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("WOOT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "woot-pp-cli/feedback")
...
When ` + "`WOOT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`WOOT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  woot-pp-cli feedback list
  woot-pp-cli feedback list --limit 5
  woot-pp-cli feedback list --json`,
