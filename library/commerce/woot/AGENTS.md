# Woot Printed CLI Agent Guide

This directory is a generated `woot-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
woot-pp-cli doctor --json
woot-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
woot-pp-cli which "<capability>" --json
woot-pp-cli <command> --help
```
...
```bash
woot-pp-cli <command> --help
woot-pp-cli <command> --dry-run --agent
```
