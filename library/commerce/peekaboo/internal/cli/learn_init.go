...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"category": {
			{Canonical: "Food", Values: []string{"food", "restaurants", "eat", "dining"}},
			{Canonical: "Lifestyle", Values: []string{"lifestyle"}},
			{Canonical: "Health", Values: []string{"health", "medical"}},
			{Canonical: "Entertainment", Values: []string{"entertainment", "fun"}},
			{Canonical: "Grocery", Values: []string{"grocery", "groceries"}},
			{Canonical: "Hotels", Values: []string{"hotels", "hotel", "stay"}},
		},
		"city": {
			{Canonical: "Lahore", Values: []string{"lahore", "lhr"}},
			{Canonical: "Karachi", Values: []string{"karachi", "khi"}},
			{Canonical: "Islamabad", Values: []string{"islamabad", "isb"}},
			{Canonical: "Rawalpindi", Values: []string{"rawalpindi", "pindi"}},
			{Canonical: "Faisalabad", Values: []string{"faisalabad"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		dbPath := defaultDBPath("peekaboo-pp-cli")
		s, err := store.OpenWithContext(ctx, dbPath)
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: peekaboo-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: peekaboo-pp-cli: learn init: %v\n", err)
		}
	})
}
