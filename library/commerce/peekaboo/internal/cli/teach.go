...
// peekaboo-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/commerce/peekaboo/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/commerce/peekaboo/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/commerce/peekaboo/internal/learn/patterns"
...
const noLearnEnvVar = "PEEKABOO_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("peekaboo-pp-cli")
}
...
func newTeachCmd(flags *rootFlags, learnCfg *entities.Config) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Annotations: map[string]string{"pp:typed-exit-codes": "0,2", "mcp:local-write": "true", "pp:happy-args": "--query=food deals;--resource-type=places;--resource=places"},
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  peekaboo-pp-cli teach --query "<question>" --resource-type <type> \
    --resource <id> --resource <id> &`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// An explicit --json/--agent should always emit JSON, even under
			// the quiet-by-default background posture. Only a bare quiet
			// invocation (no --json) stays silent.
			if flags.asJSON {
...
when learnings exist.`,
		Example: `  peekaboo-pp-cli recall "<question>" --agent
  peekaboo-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  peekaboo-pp-cli learnings list --agent
  peekaboo-pp-cli learnings list --query "<substring>"
  peekaboo-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  peekaboo-pp-cli learnings forget "<question>" --resource <id>
  peekaboo-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
func newTeachPatternCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Annotations: map[string]string{"mcp:local-write": "true", "pp:happy-args": "--query-template=deals in {entity};--resource-template=places {entity:city};--resource-type=places;--entity-kind=city"},
...
a whole family.`,
		Example: `  peekaboo-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  peekaboo-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
