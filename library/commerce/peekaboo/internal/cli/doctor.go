...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  peekaboo-pp-cli doctor
  peekaboo-pp-cli doctor --json
  peekaboo-pp-cli doctor --fail-on warn
  peekaboo-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if os.Getenv("PEEKABOO_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "PEEKABOO_TOKEN")
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "peekaboo-pp-cli", suggestion)
						} else {
