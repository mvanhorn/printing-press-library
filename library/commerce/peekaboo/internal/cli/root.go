...
type rootFlags struct {
	...
	// invocation. Mirrors the PEEKABOO_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "peekaboo-pp-cli",
		Short: `Peekaboo CLI — Every Peekaboo deal, branch, and bank-card offer from the terminal — plus city-wide deal ranking, bank-card reverse…`,
		Long: `Peekaboo CLI — Every Peekaboo deal, branch, and bank-card offer from the terminal — plus city-wide deal ranking, bank-card reverse lookup, and bulk branch map-link export the Peekaboo site has no button for.

Highlights (not in the official API docs):
  • directions   Get a ready-to-open Google Maps directions link for every branch of a restaurant at once.
  • nearest   Find the single closest branch of a restaurant to a city or coordinates, with its directions link.
  • wallet   List every merchant in a city that honors a given bank card's deal (card to merchants).
  • top-deals   Rank a city's merchants by discount percentage and return the biggest current discounts.
  • expiring   List deals whose validity window closes within N days.
  • open-now   Keep only merchants with a branch open at the current local time.

Agent mode: add --agent to any command for JSON output + non-interactive mode.
Health check: run 'peekaboo-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("peekaboo-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.dataSource, "data-source", "auto", "Data source for read commands: auto (live with local fallback), live (API only), local (previous live reads only)")
	rootCmd.PersistentFlags().DurationVar(&flags.maxAge, "max-age", 30*time.Minute, "Maximum acceptable age of local-store data before a stderr hint suggests a live refresh; 0 disables")
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'peekaboo-pp-cli profile list')")
	...
	rootCmd.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
		...
		// Zero-config auth: ensure the public guest token is cached before any
		// command that hits an authenticated endpoint (hand-authored; see
		// internal/cli/peekaboo_ext.go).
		maybeEnsureGuestToken(cmd, flags)
		return nil
	}
	...
	rootCmd.AddCommand(newNovelDirectionsCmd(flags))
	rootCmd.AddCommand(newNovelExpiringCmd(flags))
	rootCmd.AddCommand(newNovelNearestCmd(flags))
	rootCmd.AddCommand(newNovelOpenNowCmd(flags))
	rootCmd.AddCommand(newNovelTopDealsCmd(flags))
	rootCmd.AddCommand(newNovelWalletCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newAmenitiesPromotedCmd(flags))
	rootCmd.AddCommand(newBranchesPromotedCmd(flags))
	rootCmd.AddCommand(newBrandsPromotedCmd(flags))
	rootCmd.AddCommand(newCardsPromotedCmd(flags))
