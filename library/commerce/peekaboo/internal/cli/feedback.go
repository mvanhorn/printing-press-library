...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PEEKABOO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PEEKABOO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "peekaboo-pp-cli/feedback")
...
When ` + "`PEEKABOO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PEEKABOO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  peekaboo-pp-cli feedback list
  peekaboo-pp-cli feedback list --limit 5
  peekaboo-pp-cli feedback list --json`,
