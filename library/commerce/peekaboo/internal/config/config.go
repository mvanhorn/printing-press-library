...
type Config struct {
	...
	PeekabooToken    string          `toml:"token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://peekaboo.guru/api",
	}
	...
	// Env var overrides
	if v := os.Getenv("PEEKABOO_TOKEN"); v != "" {
		cfg.PeekabooToken = v
		cfg.markEnvOverride("PeekabooToken")
		cfg.AuthSource = "env:PEEKABOO_TOKEN"
		cfg.CredentialSource = "env:PEEKABOO_TOKEN"
	}
	...
	if v := os.Getenv("PEEKABOO_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("PEEKABOO_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "peekaboo-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.PeekabooToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:PEEKABOO_TOKEN"
		}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.PeekabooToken = creds.PeekabooToken
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.PeekabooToken = ""
	delete(c.envOverrides, "PeekabooToken")
	c.updateFileConfigField("PeekabooToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["PeekabooToken"] {
			out.PeekabooToken = c.fileConfig.PeekabooToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "PeekabooToken":
		c.fileConfig.PeekabooToken = c.PeekabooToken
	}
}
