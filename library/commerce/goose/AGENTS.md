# Goose Printed CLI Agent Guide

This directory is a generated `goose-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
goose-pp-cli doctor --json
goose-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
goose-pp-cli which "<capability>" --json
goose-pp-cli <command> --help
```
...
```bash
goose-pp-cli <command> --help
goose-pp-cli <command> --dry-run --agent
```
