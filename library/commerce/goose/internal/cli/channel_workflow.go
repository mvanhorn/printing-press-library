...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  goose-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  goose-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"bookings", "contracts", "conversations", "customers", "dashboard", "notes", "reports", "services", "species", "staff", "vouchers"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  goose-pp-cli workflow status

  # Show status as JSON
  goose-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("goose-pp-cli")
			}
