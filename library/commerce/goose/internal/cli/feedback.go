...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".goose-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GOOSE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GOOSE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "goose-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.goose-pp-cli/feedback.jsonl.
When ` + "`GOOSE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GOOSE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  goose-pp-cli feedback list
  goose-pp-cli feedback list --limit 5
  goose-pp-cli feedback list --json`,
