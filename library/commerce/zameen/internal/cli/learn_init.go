...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"category": {
			{Canonical: "Homes", Values: []string{"home", "house", "houses", "flat", "flats", "apartment", "apartments"}},
			{Canonical: "Plots", Values: []string{"plot", "plots", "land", "file", "files"}},
			{Canonical: "Commercial", Values: []string{"commercial", "office", "shop", "warehouse"}},
			{Canonical: "Rentals", Values: []string{"rent", "rental", "rentals", "for-rent"}},
		},
		"city": {
			{Canonical: "Islamabad", Values: []string{"isb", "isl", "islamabad-3", "capital"}},
			{Canonical: "Lahore", Values: []string{"lhr", "lahore-1"}},
			{Canonical: "Karachi", Values: []string{"khi", "karachi-2"}},
			{Canonical: "Rawalpindi", Values: []string{"rwp", "pindi", "rawalpindi-41"}},
			{Canonical: "Faisalabad", Values: []string{"fsd", "faisalabad-16"}},
			{Canonical: "Multan", Values: []string{"mux", "multan-15"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: zameen-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: zameen-pp-cli: learn init: %v\n", err)
		}
	})
}
