...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ZAMEEN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ZAMEEN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "zameen-pp-cli/feedback")
...
When ` + "`ZAMEEN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ZAMEEN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  zameen-pp-cli feedback list
  zameen-pp-cli feedback list --limit 5
  zameen-pp-cli feedback list --json`,
