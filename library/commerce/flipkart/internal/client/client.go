...
func newTierLimiters(rateLimit float64) map[string]*cliutil.AdaptiveLimiter {
	return map[string]*cliutil.AdaptiveLimiter{
		"affiliate": cliutil.NewAdaptiveLimiter(rateLimit),
		"none":      cliutil.NewAdaptiveLimiter(rateLimit),
	}
}
...
func (c *Client) baseURLForRequest() string {
	...
	case "affiliate":
		return strings.TrimRight("https://affiliate-api.flipkart.net/affiliate", "/")
	case "none":
		return c.BaseURL
	default:
		return c.BaseURL
...
func (c *Client) authForRequest() (requestAuth, error) {
	switch c.requestTier {
	case "affiliate":
		tierValue0 := os.Getenv("FLIPKART_AFFILIATE_ID")
		if tierValue0 == "" {
			return requestAuth{In: "header", Name: "Fk-Affiliate-Id"}, nil
		}
		value := tierValue0
		return requestAuth{Value: value, In: "header", Name: "Fk-Affiliate-Id"}, nil
	case "none":
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "flipkart-pp-cli", "http")
