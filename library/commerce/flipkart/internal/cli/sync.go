...
Resource scoping:
  ...
  flipkart-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  flipkart-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  flipkart-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  flipkart-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  flipkart-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/flipkart-pp-cli/data.db)")
