...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".flipkart-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FLIPKART_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FLIPKART_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/commerce/flipkart/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.flipkart-pp-cli/feedback.jsonl.
When ` + "`FLIPKART_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FLIPKART_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  flipkart-pp-cli feedback list
  flipkart-pp-cli feedback list --limit 5
  flipkart-pp-cli feedback list --json`,
