...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "flipkart-pp-cli",
		Short: `Search, track prices, and compare products on Flipkart from the terminal.`,
		Long: `Search, track prices, and compare products on Flipkart from the terminal.
...
Health check: run 'flipkart-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("flipkart-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'flipkart-pp-cli profile list')")
	...
	// feed and catalog are generated (DO NOT EDIT) commands; their new
	// hand-written subcommands (feed digest, catalog diff) are wired here
	// via AddCommand on the returned *cobra.Command rather than by editing
	// feed.go / promoted_catalog.go directly.
	feedCmd := newFeedCmd(flags)
	feedCmd.AddCommand(newFeedDigestCmd(flags))
	...
	catalogCmd := newCatalogPromotedCmd(flags)
	catalogCmd.AddCommand(newCatalogDiffCmd(flags))
	rootCmd.AddCommand(catalogCmd)

	rootCmd.AddCommand(newVersionCliCmd())

	// Phase 3 hand-written commands (product/watch/compare/deals/offers).
