...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  flipkart-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  flipkart-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, resource := range resources {
				res := syncResource(syncClientForResource(c, resource), s, resource, "", full, 100, false, nil)
				if res.Err != nil {
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  flipkart-pp-cli workflow status

  # Show status as JSON
  flipkart-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("flipkart-pp-cli")
			}
