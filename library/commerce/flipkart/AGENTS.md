# Flipkart Printed CLI Agent Guide

This directory is a generated `flipkart-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
flipkart-pp-cli doctor --json
flipkart-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
flipkart-pp-cli which "<capability>" --json
flipkart-pp-cli <command> --help
```
...
```bash
flipkart-pp-cli <command> --help
flipkart-pp-cli <command> --dry-run --agent
```
