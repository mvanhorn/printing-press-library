...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Harris Teeter",
	}
...
Use --chrome to read cookies from Chrome for .harristeeter.com.
Use --browser as an alias for --chrome.
Uses pycookiecheat, cookies, or cookie-scoop-cli when available; otherwise
falls back to a live Chrome session through agent-browser, browser-use, or CDP.
...
profile by name when the installed backend supports it.`,
		Example: `  harris-teeter-pp-cli auth login --chrome
  harris-teeter-pp-cli auth login --browser
  harris-teeter-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !browserFlag {
				fmt.Fprintln(cmd.OutOrStdout(), "Use --chrome or --browser to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  harris-teeter-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  harris-teeter-pp-cli auth login --browser\n")
				return nil
			}
			...
			if err != nil {
				tool = ""
				fmt.Fprintln(w, "No standalone cookie extraction tool found; trying the live Chrome session...")
			}

			if profileFlag != "" && tool == "" {
				return authErr(fmt.Errorf("--profile requires pycookiecheat or cookie-scoop-cli; live browser fallback uses the currently connected Chrome session"))
			}
			...
			if tool == "" {
				fmt.Fprintf(w, "Reading cookies from the live browser session for %s...\n", domain)
			} else if profileDir != "" {
			...
			if tool == "" {
				...
				if err != nil || cookies == "" {
					if err != nil {
						fmt.Fprintf(w, "Cookie tool failed (%v); trying the live Chrome session...\n", err)
					} else {
						fmt.Fprintln(w, "Cookie tool returned no cookies; trying the live Chrome session...")
					}
					cookies, err = extractLiveCookies(domain)
				}
			}
			if err != nil {
				fmt.Fprintln(w, "")
				fmt.Fprintln(w, "Install a cookie extraction tool if live browser fallback is unavailable:")
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  harris-teeter-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  harris-teeter-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: .harristeeter.com\n")
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  harris-teeter-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
...
// extractLiveCookies reads cookies from a live Chrome session.
...
func extractLiveCookies(domain string) (string, error) {
	...
	// 2. Try agent-browser --auto-connect.
...
func extractViaAgentBrowser(targetURL string) (string, error) {
	...
	// Read cookies through the browser automation API so httpOnly cookies are
	// included. document.cookie misses several auth/session cookies.
	var out bytes.Buffer
	cookiesCmd := exec.Command("agent-browser", "cookies", "get", "--json")
	cookiesCmd.Stdout = &out
	cookiesCmd.Stderr = os.Stderr
	if err := cookiesCmd.Run(); err != nil {
		closeCmd := exec.Command("agent-browser", "close")
		_ = closeCmd.Run()
		return "", fmt.Errorf("agent-browser cookies get: %w", err)
	}
	...
	header, err := cookieHeaderFromAgentBrowserJSON(out.Bytes(), targetURL)
	if err != nil {
		return "", err
	}
	return header, nil
...
func cookieHeaderFromAgentBrowserJSON(data []byte, targetURL string) (string, error) {
	var result struct {
	...
	if err := json.Unmarshal(data, &result); err != nil {
		return "", fmt.Errorf("parsing agent-browser cookies: %w", err)
	}
	if !result.Success {
		return "", fmt.Errorf("agent-browser cookies failed: %v", result.Error)
	}
	...
	for _, cookie := range result.Data.Cookies {
		...
		domain := strings.TrimPrefix(strings.TrimPrefix(cookie.Domain, "."), "www.")
		if domain != "" && !strings.HasSuffix(trimmedHost, domain) {
			continue
		}
