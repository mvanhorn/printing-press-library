...
func AdoptVerifiedLegacyDatabase(ctx context.Context, session *Session, state *LegacyMigrationState) error {
	...
	if err := source.PingContext(ctx); err != nil {
		markLegacyDatabaseQuarantined(state)
		state.Reason = state.Reason + ": " + err.Error()
		return nil
	}
	...
	if err := securePrivateFile(targetPath); err != nil {
		_ = os.Remove(targetPath)
...
func BackupLegacyArtifacts(state *LegacyMigrationState) error {
	...
	for _, artifact := range artifacts {
		...
		if err := securePrivateFile(backupPath); err != nil {
...
func readOnlySQLiteDSN(path string) string {
	...
	// the leading slash, modernc SQLite parses the drive letter as URI syntax
	// and fails with a misleading "out of memory" SQL error.
	if runtime.GOOS == "windows" && !strings.HasPrefix(slashPath, "/") {
		slashPath = "/" + slashPath
	}
	return (&url.URL{Scheme: "file", Path: slashPath, RawQuery: "mode=ro"}).String()
}
