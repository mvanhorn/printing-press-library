...
func newEmployeesCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "employees",
		Short:       "List and get employees",
		Annotations: map[string]string{"mcp:read-only": "true", "pp:api-resource": "true", "pp:typed-exit-codes": "0,2"},
		RunE:        parentNoSubcommandRunE(flags),
	}

	cmd.AddCommand(newEmployeesListCmd(flags))
	cmd.AddCommand(newEmployeesRetrieveCmd(flags))
	return cmd
}
