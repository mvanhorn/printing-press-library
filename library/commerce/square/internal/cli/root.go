...
type rootFlags struct {
	...
	// invocation. Mirrors the SQUARE_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "square-pp-cli",
		Short: `Square CLI — The current Square v2 API surface, plus local reconciliation and history the remote API cannot provide by itself.`,
		Long: `Square CLI — The current Square v2 API surface, plus local reconciliation and history the remote API cannot provide by itself.
...
Health check: run 'square-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("square-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'square-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCashDrawersCmd(flags))
	...
	rootCmd.AddCommand(newTeamMembersCmd(flags))
	rootCmd.AddCommand(newTerminalsCmd(flags))
	rootCmd.AddCommand(newTransferOrdersCmd(flags))
	...
	rootCmd.AddCommand(newApplePayPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelCustomerCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelReconcileCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelRequestCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelServiceCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelWebhookCmd(flags))
