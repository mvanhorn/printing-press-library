...
var whichIndex = []whichEntry{
	{Command: "reconcile close", Description: "Explain the difference between sales, collected payments, refunds, disputes, and payouts for each location.", Group: "Operational reconciliation", WhyItMatters: "Use this when an operator needs an explainable close instead of several unrelated API responses."},
	{Command: "inventory drift", Description: "See which prices, variations, availability settings, and location counts changed since an earlier snapshot.", Group: "Operational reconciliation", WhyItMatters: "Use this before opening or publishing a menu when current-state API calls cannot explain what changed."},
	{Command: "customer timeline", Description: "Follow one customer's orders, payments, refunds, loyalty activity, invoices, and bookings in chronological order.", Group: "Connected customer operations", WhyItMatters: "Use this for customer-service investigations that otherwise require carrying IDs between services."},
	{Command: "webhook health", Description: "Measure duplicate locally ingested webhook deliveries, ordering problems, delivery lag, gaps, and subscription changes.", Group: "Integration confidence", WhyItMatters: "Use this before releases or during incidents when a subscription listing cannot reveal delivery behavior."},
	{Command: "service review", Description: "Review completed, unpaid, and missed appointments alongside staff and location utilization.", Group: "Connected customer operations", WhyItMatters: "Use this for a service business's weekly operating review instead of assembling separate reports."},
	{Command: "request check", Description: "Validate a planned Square request, environment, API version, mutation policy, and idempotency before anything is sent.", Group: "Integration confidence", WhyItMatters: "Use this before a risky sandbox or production call when schema validity alone is not enough."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  square-pp-cli which "stale tickets"
  square-pp-cli which "bottleneck"
  square-pp-cli which --limit 1 "send message"
  square-pp-cli which                                # list the full capability index`,
