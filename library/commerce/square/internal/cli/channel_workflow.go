...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  square-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  square-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"bookings-business-booking-profile", "catalog", "online-checkout-merchant-settings", "reporting"}
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  square-pp-cli workflow status

  # Show status as JSON
  square-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("square-pp-cli")
			}
