...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SQUARE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SQUARE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "square-pp-cli/feedback")
...
When ` + "`SQUARE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SQUARE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  square-pp-cli feedback list
  square-pp-cli feedback list --limit 5
  square-pp-cli feedback list --json`,
