...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/commerce/square/internal/learn/lookups"
...
Resource scoping:
  ...
  square-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  square-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  square-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  square-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  square-pp-cli sync --latest-only`,
...
type syncPageClient interface {
	Get(context.Context, string, map[string]string) (json.RawMessage, error)
	RateLimit() float64
}

type syncPostQueryClient interface {
	PostQueryWithParams(context.Context, string, map[string]string, any) (json.RawMessage, int, error)
}

// fetchSyncResourcePage keeps the generated GET-based sync machinery intact,
// while providing the handful of Square collection APIs that are POST-only
// with their wire-correct method and body shape.
func fetchSyncResourcePage(ctx context.Context, c syncPageClient, resource, path string, params map[string]string) (json.RawMessage, error) {
	if syncResourceMethod(resource) == "GET" {
		return c.Get(ctx, path, params)
	}
	postClient, ok := c.(syncPostQueryClient)
	if !ok {
		return nil, fmt.Errorf("sync client does not support POST query requests required by %s", resource)
	}
	body, queryParams, err := syncResourcePOSTBody(resource, params)
	if err != nil {
		return nil, err
	}
	data, _, err := postClient.PostQueryWithParams(ctx, path, queryParams, body)
	return data, err
}

func syncResourceMethod(resource string) string {
	switch resource {
	case "catalog", "orders", "inventory", "team-members":
		return "POST"
	default:
		return "GET"
	}
...
func syncResourcePOSTBody(resource string, params map[string]string) (map[string]any, map[string]string, error) {
	body := map[string]any{}
	query := make(map[string]string, len(params))
	for key, value := range params {
		query[key] = value
	}
	moveString := func(key string) {
	...
	moveInt := func(key string) error {
		...
		if err != nil {
			return fmt.Errorf("invalid %s sync parameter %s=%q: expected integer", resource, key, value)
		}
		body[key] = parsed
		delete(query, key)
		return nil
	}
	moveList := func(key string) error {
		value, ok := query[key]
		if !ok || strings.TrimSpace(value) == "" {
			delete(query, key)
			return nil
		}
		parsed, err := cliutil.ParseStringList(value)
		if err != nil {
			return fmt.Errorf("invalid %s sync parameter %s=%q: %w", resource, key, value, err)
		}
		body[key] = parsed
		delete(query, key)
		return nil
	}
	moveBool := func(key string) error {
		value, ok := query[key]
		if !ok || value == "" {
			delete(query, key)
			return nil
		...
		if err != nil {
			return fmt.Errorf("invalid %s sync parameter %s=%q: expected boolean", resource, key, value)
		}
		body[key] = parsed
		delete(query, key)
		return nil
	...
	moveString("cursor")
	if err := moveInt("limit"); err != nil {
		return nil, nil, err
	}
	switch resource {
	case "catalog":
		if err := moveBool("include_deleted_objects"); err != nil {
			return nil, nil, err
		}
		// ListCatalog used the query name `types`; SearchCatalogObjects uses
		// `object_types` in its JSON request. Accept either operator spelling,
		// but send only the spec-correct body field.
		typesValue := query["object_types"]
		if typesValue == "" {
			typesValue = query["types"]
		}
		delete(query, "types")
		delete(query, "object_types")
		if strings.TrimSpace(typesValue) != "" {
			parsed, err := cliutil.ParseStringList(typesValue)
			if err != nil {
				return nil, nil, fmt.Errorf("invalid catalog sync parameter types=%q: %w", typesValue, err)
			}
			body["object_types"] = parsed
		}
	case "orders":
		if err := moveList("location_ids"); err != nil {
			return nil, nil, err
		}
		if since, ok := query["updated_at_begin_time"]; ok && since != "" {
			body["query"] = map[string]any{"filter": map[string]any{"date_time_filter": map[string]any{"updated_at": map[string]any{"start_at": since}}}}
			delete(query, "updated_at_begin_time")
		}
	case "inventory":
		for _, key := range []string{"catalog_object_ids", "location_ids", "states"} {
			if err := moveList(key); err != nil {
				return nil, nil, err
			}
		}
		moveString("updated_after")
	case "team-members":
		filter := map[string]any{}
		if value, ok := query["location_ids"]; ok {
			parsed, err := cliutil.ParseStringList(value)
			if err != nil {
				return nil, nil, fmt.Errorf("invalid team-members sync parameter location_ids=%q: %w", value, err)
			}
			filter["location_ids"] = parsed
			delete(query, "location_ids")
		}
		if value, ok := query["status"]; ok {
			filter["status"] = value
			delete(query, "status")
		}
		if value, ok := query["is_owner"]; ok {
			parsed, err := strconv.ParseBool(value)
			if err != nil {
				return nil, nil, fmt.Errorf("invalid team-members sync parameter is_owner=%q: expected boolean", value)
			}
			filter["is_owner"] = parsed
			delete(query, "is_owner")
		}
		if len(filter) > 0 {
			body["query"] = map[string]any{"filter": filter}
		}
	}
	return body, query, nil
}

func syncResource(ctx context.Context, c syncPageClient, db *store.Store, resource, sinceTS string, full bool, maxPages int, latestOnly bool, prune bool, userParams *syncUserParams, syncEvents io.Writer) syncResult {
	started := time.Now()
	if syncEvents == nil {
	...
	for {
		...
		applySquareSyncDefaultParams(resource, params)
		...
		data, err := fetchSyncResourcePage(ctx, c, resource, path, params)
		if err != nil {
		...
		items = normalizeSquareSyncItems(resource, items)
...
func determinePaginationDefaults(resource string) paginationDefaults {
	...
	case "bookings-custom-attribute-definitions":
		return paginationDefaults{
			cursorParam: "cursor",
			cursorType:  "cursor",
			limitParam:  "limit",
	...
	case "bookings-location-booking-profiles":
		return paginationDefaults{
			cursorParam: "cursor",
			cursorType:  "cursor",
			limitParam:  "limit",
	...
	case "bookings-team-member-booking-profiles":
		return paginationDefaults{
			cursorParam: "cursor",
			cursorType:  "cursor",
			limitParam:  "limit",
	...
	case "bookings/bookings_custom_attributes":
		return paginationDefaults{
			cursorParam: "cursor",
			cursorType:  "cursor",
			limitParam:  "limit",
	...
	case "customers-custom-attribute-definitions":
		return paginationDefaults{
			cursorParam: "cursor",
			cursorType:  "cursor",
			limitParam:  "limit",
	...
	case "customers/customers_custom_attributes":
		return paginationDefaults{
			cursorParam: "cursor",
			cursorType:  "cursor",
			limitParam:  "limit",
	...
	case "locations-custom-attribute-definitions":
		return paginationDefaults{
			cursorParam: "cursor",
			cursorType:  "cursor",
			limitParam:  "limit",
	...
	case "locations/locations_custom_attributes":
		return paginationDefaults{
			cursorParam: "cursor",
			cursorType:  "cursor",
			limitParam:  "limit",
	...
	case "merchants-custom-attribute-definitions":
		return paginationDefaults{
			cursorParam: "cursor",
			cursorType:  "cursor",
			limitParam:  "limit",
	...
	case "merchants/merchants_custom_attributes":
		return paginationDefaults{
			cursorParam: "cursor",
			cursorType:  "cursor",
			limitParam:  "limit",
	...
	case "orders/orders_custom_attributes":
		return paginationDefaults{
			cursorParam: "cursor",
			cursorType:  "cursor",
			limitParam:  "limit",
...
// InventoryCount has no standalone `id`; its identity is the catalog object,
// location, and state tuple. Add that stable composite before the generic store
// extractor runs so real count rows are retained instead of discarded as
// metadata-shaped objects.
func normalizeSquareSyncItems(resource string, items []json.RawMessage) []json.RawMessage {
	if resource != "inventory" && resource != "changes" {
		return items
	}
	...
	for _, item := range items {
		...
		if resource == "inventory" && store.ExtractResourceID(resource, object) == "" {
			catalogObjectID := fmt.Sprint(object["catalog_object_id"])
			locationID := fmt.Sprint(object["location_id"])
			state := fmt.Sprint(object["state"])
			if catalogObjectID != "" && catalogObjectID != "<nil>" && locationID != "" && locationID != "<nil>" {
				object["id"] = catalogObjectID + "\x00" + locationID + "\x00" + state
		...
		if resource == "changes" {
			normalizeSquareInventoryChange(object)
			if encoded, err := json.Marshal(object); err == nil {
				item = encoded
			}
		}
...
func normalizeSquareInventoryChange(object map[string]any) {
	changeType := strings.ToLower(fmt.Sprint(object["type"]))
	nestedKey := ""
	if _, ok := object["physical_count"].(map[string]any); ok {
		nestedKey = "physical_count"
	} else if _, ok := object["adjustment"].(map[string]any); ok {
		nestedKey = "adjustment"
	}
	if nestedKey == "" {
		return
	}
	nested := object[nestedKey].(map[string]any)
	if changeType == "" || changeType == "<nil>" {
		changeType = nestedKey
	}
	...
	id := copyString("id")
	catalogObjectID := copyString("catalog_object_id")
	locationID := copyString("location_id")
	copyString("reference_id")
	if id == "" && catalogObjectID != "" {
		parts := []string{changeType, catalogObjectID, locationID}
		for _, key := range []string{"state", "from_state", "to_state", "occurred_at", "calculated_at", "created_at", "updated_at", "reference_id", "quantity"} {
			parts = append(parts, fmt.Sprint(nested[key]))
		}
		object["id"] = strings.Join(parts, "\x00")
	}
}

func applySquareSyncDefaultParams(resource string, params map[string]string) {
	if resource == "catalog" {
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "apple-pay":
		return db.UpsertApplePay(data)
	case "bank-accounts":
		return db.UpsertBankAccounts(data)
	case "bank_accounts_disable":
		return db.UpsertBankAccountsDisable(data)
	case "bookings":
		return db.UpsertBookings(data)
	case "bookings_cancel":
		return db.UpsertBookingsCancel(data)
	case "bookings_custom_attributes":
		return db.UpsertBookingsCustomAttributes(data)
	case "cards":
		return db.UpsertCards(data)
	case "cards_disable":
		return db.UpsertCardsDisable(data)
	case "cash-drawers":
		return db.UpsertCashDrawers(data)
	case "catalog":
	...
	case "customers_cards":
		return db.UpsertCustomersCards(data)
	case "customers_custom_attributes":
		return db.UpsertCustomersCustomAttributes(data)
	case "groups":
		return db.UpsertGroups(data)
	case "devices":
		return db.UpsertDevices(data)
	case "disputes":
		return db.UpsertDisputes(data)
	case "accept":
		return db.UpsertAccept(data)
	case "evidence":
		return db.UpsertEvidence(data)
	case "evidence_files":
		return db.UpsertEvidenceFiles(data)
	case "evidence_text":
		return db.UpsertEvidenceText(data)
	case "submit_evidence":
		return db.UpsertSubmitEvidence(data)
	case "employees":
		return db.UpsertEmployees(data)
	case "gift-cards":
		return db.UpsertGiftCards(data)
	case "link_customer":
		return db.UpsertLinkCustomer(data)
	case "unlink_customer":
		return db.UpsertUnlinkCustomer(data)
	case "inventory":
	...
	case "invoices_cancel":
		return db.UpsertInvoicesCancel(data)
	case "publish":
		return db.UpsertPublish(data)
	case "labor":
		return db.UpsertLabor(data)
	case "locations":
		return db.UpsertLocations(data)
	case "checkouts":
		return db.UpsertCheckouts(data)
	case "locations_custom_attributes":
		return db.UpsertLocationsCustomAttributes(data)
	case "transactions":
		return db.UpsertTransactions(data)
	case "loyalty":
		return db.UpsertLoyalty(data)
	case "merchants":
		return db.UpsertMerchants(data)
	case "merchants_custom_attributes":
		return db.UpsertMerchantsCustomAttributes(data)
	case "oauth2":
		return db.UpsertOauth2(data)
	case "online-checkout":
		return db.UpsertOnlineCheckout(data)
	case "orders":
		return db.UpsertOrders(data)
	case "orders_custom_attributes":
		return db.UpsertOrdersCustomAttributes(data)
	case "pay":
		return db.UpsertPay(data)
	case "payments":
		return db.UpsertPayments(data)
	case "payments_cancel":
		return db.UpsertPaymentsCancel(data)
	case "complete":
		return db.UpsertComplete(data)
	case "payouts":
		return db.UpsertPayouts(data)
	case "payout_entries":
		return db.UpsertPayoutEntries(data)
	case "refunds":
		return db.UpsertRefunds(data)
	case "snippet":
		return db.UpsertSnippet(data)
	case "subscriptions":
		return db.UpsertSubscriptions(data)
	case "actions":
		return db.UpsertActions(data)
	case "billing_anchor":
		return db.UpsertBillingAnchor(data)
	case "subscriptions_cancel":
		return db.UpsertSubscriptionsCancel(data)
	case "subscriptions_events":
		return db.UpsertSubscriptionsEvents(data)
	case "pause":
		return db.UpsertPause(data)
	case "resume":
		return db.UpsertResume(data)
	case "swap_plan":
		return db.UpsertSwapPlan(data)
	case "team-members":
		return db.UpsertTeamMembers(data)
	case "wage_setting":
		return db.UpsertWageSetting(data)
	case "terminals":
		return db.UpsertTerminals(data)
	case "transfer-orders":
		return db.UpsertTransferOrders(data)
	case "transfer_orders_cancel":
		return db.UpsertTransferOrdersCancel(data)
	case "receive":
		return db.UpsertReceive(data)
	case "start":
...
func defaultSyncResources() []string {
	return []string{
		...
		"catalog",
		"online-checkout-merchant-settings",
		"reporting",
	}
}
...
func knownSyncResourceNames() []string {
	names := []string{
		...
		"bookings",
		"bookings-business-booking-profile",
		"bookings-custom-attribute-definitions",
		...
		"channels",
		"customers",
		"customers-custom-attribute-definitions",
		"customers-groups",
		"customers-segments",
		"devices",
		"devices-codes",
		...
		"labor-team-member-wages",
		"labor-workweek-configs",
		"locations",
		"locations-custom-attribute-definitions",
		"loyalty",
		"merchants",
		"merchants-custom-attribute-definitions",
		"online-checkout",
		"online-checkout-merchant-settings",
		"orders",
		"payments",
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		"bank-accounts":                          "/v2/bank-accounts",
		"bookings":                               "/v2/bookings",
		"bookings-business-booking-profile":      "/v2/bookings/business-booking-profile",
		"bookings-custom-attribute-definitions":  "/v2/bookings/custom-attribute-definitions",
		"bookings-location-booking-profiles":     "/v2/bookings/location-booking-profiles",
		"bookings-team-member-booking-profiles":  "/v2/bookings/team-member-booking-profiles",
		"cards":                                  "/v2/cards",
		"cash-drawers":                           "/v2/cash-drawers/shifts",
		"catalog":                                "/v2/catalog/search",
		"channels":                               "/v2/channels",
		"customers":                              "/v2/customers",
		"customers-custom-attribute-definitions": "/v2/customers/custom-attribute-definitions",
		"customers-groups":                       "/v2/customers/groups",
		"customers-segments":                     "/v2/customers/segments",
		"devices":                                "/v2/devices",
		"devices-codes":                          "/v2/devices/codes",
		"disputes":                               "/v2/disputes",
		"employees":                              "/v2/employees",
		"events":                                 "/v2/events/types",
		"gift-cards":                             "/v2/gift-cards",
		"gift-cards-activities":                  "/v2/gift-cards/activities",
		"inventory":                              "/v2/inventory/counts/batch-retrieve",
		"invoices":                               "/v2/invoices",
		"labor":                                  "/v2/labor/break-types",
		"labor-employee-wages":                   "/v2/labor/employee-wages",
		"labor-team-member-wages":                "/v2/labor/team-member-wages",
		"labor-workweek-configs":                 "/v2/labor/workweek-configs",
		"locations":                              "/v2/locations",
		"locations-custom-attribute-definitions": "/v2/locations/custom-attribute-definitions",
		"loyalty":                                "/v2/loyalty/programs",
		"merchants":                              "/v2/merchants",
		"merchants-custom-attribute-definitions": "/v2/merchants/custom-attribute-definitions",
		"online-checkout":                        "/v2/online-checkout/payment-links",
		"online-checkout-merchant-settings":      "/v2/online-checkout/merchant-settings",
		"orders":                                 "/v2/orders/search",
		"payments":                               "/v2/payments",
		"payouts":                                "/v2/payouts",
		"refunds":                                "/v2/refunds",
		"reporting":                              "/reporting/v1/meta",
		"sites":                                  "/v2/sites",
		"team-members":                           "/v2/team-members/search",
		"webhooks":                               "/v2/webhooks/event-types",
		"webhooks-subscriptions":                 "/v2/webhooks/subscriptions",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "bookings_custom_attributes", ParentTable: "bookings", ParentIDParam: "booking_id", PathTemplate: "/v2/bookings/{booking_id}/custom-attributes", KeyField: "", ReconcileMode: "none", GenericScopeJSONPath: "", PathParams: []dependentPathParamDef{
			{Param: "booking_id", Field: "id"},
		}},
		{Name: "changes", ParentTable: "inventory", ParentIDParam: "catalog_object_id", PathTemplate: "/v2/inventory/{catalog_object_id}/changes", KeyField: "", ReconcileMode: "none", GenericScopeJSONPath: "", PathParams: []dependentPathParamDef{
			{Param: "catalog_object_id", Field: "catalog_object_id"},
		}},
		{Name: "customers_custom_attributes", ParentTable: "customers", ParentIDParam: "customer_id", PathTemplate: "/v2/customers/{customer_id}/custom-attributes", KeyField: "", ReconcileMode: "none", GenericScopeJSONPath: "", PathParams: []dependentPathParamDef{
			{Param: "customer_id", Field: "id"},
		}},
		{Name: "evidence", ParentTable: "disputes", ParentIDParam: "dispute_id", PathTemplate: "/v2/disputes/{dispute_id}/evidence", KeyField: "", ReconcileMode: "none", GenericScopeJSONPath: "", PathParams: []dependentPathParamDef{
			{Param: "dispute_id", Field: "id"},
		}},
		{Name: "locations_custom_attributes", ParentTable: "locations", ParentIDParam: "location_id", PathTemplate: "/v2/locations/{location_id}/custom-attributes", KeyField: "", ReconcileMode: "none", GenericScopeJSONPath: "", PathParams: []dependentPathParamDef{
			{Param: "location_id", Field: "id"},
		}},
		{Name: "merchants_custom_attributes", ParentTable: "merchants", ParentIDParam: "merchant_id", PathTemplate: "/v2/merchants/{merchant_id}/custom-attributes", KeyField: "", ReconcileMode: "none", GenericScopeJSONPath: "", PathParams: []dependentPathParamDef{
			{Param: "merchant_id", Field: "id"},
		}},
		{Name: "orders_custom_attributes", ParentTable: "orders", ParentIDParam: "order_id", PathTemplate: "/v2/orders/{order_id}/custom-attributes", KeyField: "", ReconcileMode: "none", GenericScopeJSONPath: "", PathParams: []dependentPathParamDef{
			{Param: "order_id", Field: "id"},
		}},
		{Name: "payout_entries", ParentTable: "payouts", ParentIDParam: "payout_id", PathTemplate: "/v2/payouts/{payout_id}/payout-entries", KeyField: "", ReconcileMode: "none", GenericScopeJSONPath: "", PathParams: []dependentPathParamDef{
			{Param: "payout_id", Field: "id"},
		}},
		{Name: "transactions", ParentTable: "locations", ParentIDParam: "location_id", PathTemplate: "/v2/locations/{location_id}/transactions", KeyField: "", ReconcileMode: "none", GenericScopeJSONPath: "", PathParams: []dependentPathParamDef{
			{Param: "location_id", Field: "id"},
		}},
	}
}
...
func syncOneParent(
	...
	for {
		...
		items = normalizeSquareSyncItems(dep.Name, items)
...
var reconcileTypedTables = map[string]string{
	...
	"bank_accounts_disable":       "bank_accounts_disable",
	"billing_anchor":              "billing_anchor",
	"bookings":                    "bookings",
	"bookings_cancel":             "bookings_cancel",
	"bookings_custom_attributes":  "bookings_custom_attributes",
	"cards":                       "cards",
	"cards_disable":               "cards_disable",
	...
	"customers_custom_attributes": "customers_custom_attributes",
	"devices":                     "devices",
	"disputes":                    "disputes",
	"employees":                   "employees",
	"evidence":                    "evidence",
	...
	"locations":                   "locations",
	"locations_custom_attributes": "locations_custom_attributes",
	"loyalty":                     "loyalty",
	"merchants":                   "merchants",
	"merchants_custom_attributes": "merchants_custom_attributes",
	"oauth2":                      "oauth2",
	"online-checkout":             "online_checkout",
	"orders":                      "orders",
	"orders_custom_attributes":    "orders_custom_attributes",
	"pause":                       "pause",
	"pay":                         "pay",
	"payments":                    "payments",
	...
	"subscriptions_events":        "subscriptions_events",
	"swap_plan":                   "swap_plan",
	"team-members":                "team_members",
	"terminals":                   "terminals",
	"transactions":                "transactions",
	"transfer-orders":             "transfer_orders",
	"transfer_orders_cancel":      "transfer_orders_cancel",
...
func responsePathForResource(resource, path string) []string {
	...
	case "catalog\x00/v2/catalog/search":
		return []string{"objects"}
	case "orders\x00/v2/orders/search":
		return []string{"orders"}
	case "inventory\x00/v2/inventory/counts/batch-retrieve":
		return []string{"counts"}
	case "team-members\x00/v2/team-members/search":
		return []string{"team_members"}
	case "apple-pay\x00/v2/apple-pay/domains":
		return []string{"errors"}
	case "bank-accounts\x00/v2/bank-accounts":
		return []string{"errors"}
	case "bank-accounts\x00/v2/bank-accounts/{bank_account_id}":
		return []string{"errors"}
	case "bank-accounts\x00/v2/bank-accounts/by-v1-id/{v1_bank_account_id}":
		return []string{"errors"}
	case "bookings\x00/v2/bookings/custom-attributes/bulk-delete":
		return []string{"errors"}
	case "bookings\x00/v2/bookings/bulk-retrieve":
		return []string{"errors"}
	case "bookings\x00/v2/bookings/team-member-booking-profiles/bulk-retrieve":
		return []string{"errors"}
	case "bookings\x00/v2/bookings/custom-attributes/bulk-upsert":
		return []string{"errors"}
	case "bookings\x00/v2/bookings":
		return []string{"errors"}
	case "bookings\x00/v2/bookings/custom-attribute-definitions":
		return []string{"errors"}
	case "bookings\x00/v2/bookings/custom-attribute-definitions/{key}":
		return []string{"errors"}
	case "bookings\x00/v2/bookings/{booking_id}":
		return []string{"errors"}
	case "bookings\x00/v2/bookings/business-booking-profile":
		return []string{"errors"}
	case "bookings\x00/v2/bookings/location-booking-profiles/{location_id}":
		return []string{"errors"}
	case "bookings\x00/v2/bookings/team-member-booking-profiles/{team_member_id}":
		return []string{"errors"}
	case "cards\x00/v2/cards":
		return []string{"errors"}
	case "cards\x00/v2/cards/{card_id}":
		return []string{"errors"}
	case "cash-drawers\x00/v2/cash-drawers/shifts/{shift_id}":
		return []string{"errors"}
	case "catalog\x00/v2/catalog/images":
		return []string{"errors"}
	case "catalog\x00/v2/catalog/info":
		return []string{"errors"}
	case "catalog\x00/v2/catalog/images/{image_id}":
		return []string{"errors"}
	case "catalog\x00/v2/catalog/update-item-modifier-lists":
		return []string{"errors"}
	case "catalog\x00/v2/catalog/update-item-taxes":
		return []string{"errors"}
	case "channels\x00/v2/channels/bulk-retrieve":
		return []string{"errors"}
	case "channels\x00/v2/channels/{channel_id}":
		return []string{"errors"}
	case "customers\x00/v2/customers/bulk-create":
		return []string{"errors"}
	case "customers\x00/v2/customers/bulk-delete":
		return []string{"errors"}
	case "customers\x00/v2/customers/bulk-retrieve":
		return []string{"errors"}
	case "customers\x00/v2/customers/bulk-update":
		return []string{"errors"}
	case "customers\x00/v2/customers/custom-attributes/bulk-upsert":
		return []string{"errors"}
	case "customers\x00/v2/customers":
		return []string{"errors"}
	case "customers\x00/v2/customers/custom-attribute-definitions":
		return []string{"errors"}
	case "customers\x00/v2/customers/groups":
		return []string{"errors"}
	case "customers\x00/v2/customers/{customer_id}":
		return []string{"errors"}
	case "customers\x00/v2/customers/custom-attribute-definitions/{key}":
		return []string{"errors"}
	case "customers\x00/v2/customers/groups/{group_id}":
		return []string{"errors"}
	case "customers\x00/v2/customers/segments/{segment_id}":
		return []string{"errors"}
	case "devices\x00/v2/devices/codes":
		return []string{"errors"}
	case "devices\x00/v2/devices/{device_id}":
		return []string{"errors"}
	case "devices\x00/v2/devices/codes/{id}":
		return []string{"errors"}
	case "disputes\x00/v2/disputes/{dispute_id}":
		return []string{"errors"}
	case "employees\x00/v2/employees/{id}":
		return []string{"errors"}
	case "events\x00/v2/events/disable":
		return []string{"errors"}
	case "events\x00/v2/events/enable":
		return []string{"errors"}
	case "gift-cards\x00/v2/gift-cards":
		return []string{"errors"}
	case "gift-cards\x00/v2/gift-cards/activities":
		return []string{"errors"}
	case "gift-cards\x00/v2/gift-cards/{id}":
		return []string{"errors"}
	case "gift-cards\x00/v2/gift-cards/from-gan":
		return []string{"errors"}
	case "gift-cards\x00/v2/gift-cards/from-nonce":
		return []string{"errors"}
	case "inventory\x00/v2/inventory/adjustment-reasons/create":
		return []string{"errors"}
	case "inventory\x00/v2/inventory/adjustment-reasons/delete":
		return []string{"errors"}
	case "inventory\x00/v2/inventory/adjustment/{adjustment_id}":
		return []string{"errors"}
	case "inventory\x00/v2/inventory/physical-count/{physical_count_id}":
		return []string{"errors"}
	case "inventory\x00/v2/inventory/adjustment-reasons/restore":
		return []string{"errors"}
	case "inventory\x00/v2/inventory/adjustments/{adjustment_id}":
		return []string{"errors"}
	case "inventory\x00/v2/inventory/adjustment-reasons/retrieve":
		return []string{"errors"}
	case "inventory\x00/v2/inventory/physical-counts/{physical_count_id}":
		return []string{"errors"}
	case "inventory\x00/v2/inventory/adjustments/update":
		return []string{"errors"}
	case "inventory\x00/v2/inventory/adjustment-reasons/update":
		return []string{"errors"}
	case "invoices\x00/v2/invoices":
		return []string{"errors"}
	case "invoices\x00/v2/invoices/{invoice_id}":
		return []string{"errors"}
	case "labor\x00/v2/labor/scheduled-shifts/bulk-publish":
		return []string{"errors"}
	case "labor\x00/v2/labor/break-types":
		return []string{"errors"}
	case "labor\x00/v2/labor/scheduled-shifts":
		return []string{"errors"}
	case "labor\x00/v2/labor/shifts":
		return []string{"errors"}
	case "labor\x00/v2/labor/timecards":
		return []string{"errors"}
	case "labor\x00/v2/labor/break-types/{id}":
		return []string{"errors"}
	case "labor\x00/v2/labor/shifts/{id}":
		return []string{"errors"}
	case "labor\x00/v2/labor/timecards/{id}":
		return []string{"errors"}
	case "labor\x00/v2/labor/employee-wages/{id}":
		return []string{"errors"}
	case "labor\x00/v2/labor/team-member-wages/{id}":
		return []string{"errors"}
	case "labor\x00/v2/labor/scheduled-shifts/{id}/publish":
		return []string{"errors"}
	case "labor\x00/v2/labor/scheduled-shifts/{id}":
		return []string{"errors"}
	case "labor\x00/v2/labor/workweek-configs/{id}":
		return []string{"errors"}
	case "locations\x00/v2/locations/custom-attributes/bulk-delete":
		return []string{"errors"}
	case "locations\x00/v2/locations/custom-attributes/bulk-upsert":
		return []string{"errors"}
	case "locations\x00/v2/locations":
		return []string{"errors"}
	case "locations\x00/v2/locations/custom-attribute-definitions":
		return []string{"errors"}
	case "locations\x00/v2/locations/custom-attribute-definitions/{key}":
		return []string{"errors"}
	case "locations\x00/v2/locations/{location_id}":
		return []string{"errors"}
	case "loyalty\x00/v2/loyalty/accounts/{account_id}/adjust":
		return []string{"errors"}
	case "loyalty\x00/v2/loyalty/programs/{program_id}/calculate":
		return []string{"errors"}
	case "loyalty\x00/v2/loyalty/programs/{program_id}/promotions/{promotion_id}/cancel":
		return []string{"errors"}
	case "loyalty\x00/v2/loyalty/accounts":
		return []string{"errors"}
	case "loyalty\x00/v2/loyalty/programs/{program_id}/promotions":
		return []string{"errors"}
	case "loyalty\x00/v2/loyalty/rewards":
		return []string{"errors"}
	case "loyalty\x00/v2/loyalty/rewards/{reward_id}":
		return []string{"errors"}
	case "loyalty\x00/v2/loyalty/rewards/{reward_id}/redeem":
		return []string{"errors"}
	case "loyalty\x00/v2/loyalty/accounts/{account_id}":
		return []string{"errors"}
	case "loyalty\x00/v2/loyalty/programs/{program_id}":
		return []string{"errors"}
	case "loyalty\x00/v2/loyalty/programs/{program_id}/promotions/{promotion_id}":
		return []string{"errors"}
	case "merchants\x00/v2/merchants/custom-attributes/bulk-delete":
		return []string{"errors"}
	case "merchants\x00/v2/merchants/custom-attributes/bulk-upsert":
		return []string{"errors"}
	case "merchants\x00/v2/merchants/custom-attribute-definitions":
		return []string{"errors"}
	case "merchants\x00/v2/merchants/custom-attribute-definitions/{key}":
		return []string{"errors"}
	case "merchants\x00/v2/merchants/{merchant_id}":
		return []string{"errors"}
	case "oauth2\x00/oauth2/token":
		return []string{"errors"}
	case "oauth2\x00/oauth2/revoke":
		return []string{"errors"}
	case "online-checkout\x00/v2/online-checkout/payment-links":
		return []string{"errors"}
	case "online-checkout\x00/v2/online-checkout/payment-links/{id}":
		return []string{"errors"}
	case "online-checkout\x00/v2/online-checkout/location-settings/{location_id}":
		return []string{"errors"}
	case "online-checkout\x00/v2/online-checkout/merchant-settings":
		return []string{"errors"}
	case "orders\x00/v2/orders/custom-attributes/bulk-delete":
		return []string{"errors"}
	case "orders\x00/v2/orders/custom-attributes/bulk-upsert":
		return []string{"errors"}
	case "orders\x00/v2/orders/calculate":
		return []string{"errors"}
	case "orders\x00/v2/orders/clone":
		return []string{"errors"}
	case "orders\x00/v2/orders":
		return []string{"errors"}
	case "orders\x00/v2/orders/custom-attribute-definitions":
		return []string{"errors"}
	case "orders\x00/v2/orders/custom-attribute-definitions/{key}":
		return []string{"errors"}
	case "orders\x00/v2/orders/{order_id}":
		return []string{"errors"}
	case "payments\x00/v2/payments/cancel":
		return []string{"errors"}
	case "payments\x00/v2/payments":
		return []string{"errors"}
	case "payments\x00/v2/payments/{payment_id}":
		return []string{"errors"}
	case "payouts\x00/v2/payouts/{payout_id}":
		return []string{"errors"}
	case "refunds\x00/v2/refunds/{refund_id}":
		return []string{"errors"}
	case "refunds\x00/v2/refunds":
		return []string{"errors"}
	case "subscriptions\x00/v2/subscriptions/bulk-swap-plan":
		return []string{"errors"}
	case "subscriptions\x00/v2/subscriptions":
		return []string{"errors"}
	case "subscriptions\x00/v2/subscriptions/{subscription_id}":
		return []string{"errors"}
	case "team-members\x00/v2/team-members/bulk-create":
		return []string{"errors"}
	case "team-members\x00/v2/team-members/bulk-update":
		return []string{"errors"}
	case "team-members\x00/v2/team-members":
		return []string{"errors"}
	case "team-members\x00/v2/team-members/jobs":
		return []string{"errors"}
	case "team-members\x00/v2/team-members/{team_member_id}":
		return []string{"errors"}
	case "team-members\x00/v2/team-members/jobs/{job_id}":
		return []string{"errors"}
	case "terminals\x00/v2/terminals/actions/{action_id}/cancel":
		return []string{"errors"}
	case "terminals\x00/v2/terminals/checkouts/{checkout_id}/cancel":
		return []string{"errors"}
	case "terminals\x00/v2/terminals/refunds/{terminal_refund_id}/cancel":
		return []string{"errors"}
	case "terminals\x00/v2/terminals/actions":
		return []string{"errors"}
	case "terminals\x00/v2/terminals/checkouts":
		return []string{"errors"}
	case "terminals\x00/v2/terminals/refunds":
		return []string{"errors"}
	case "terminals\x00/v2/terminals/actions/{action_id}/dismiss":
		return []string{"errors"}
	case "terminals\x00/v2/terminals/checkouts/{checkout_id}/dismiss":
		return []string{"errors"}
	case "terminals\x00/v2/terminals/refunds/{terminal_refund_id}/dismiss":
		return []string{"errors"}
	case "terminals\x00/v2/terminals/actions/{action_id}":
		return []string{"errors"}
	case "terminals\x00/v2/terminals/checkouts/{checkout_id}":
		return []string{"errors"}
	case "terminals\x00/v2/terminals/refunds/{terminal_refund_id}":
		return []string{"errors"}
	case "transfer-orders\x00/v2/transfer-orders":
		return []string{"errors"}
	case "transfer-orders\x00/v2/transfer-orders/{transfer_order_id}":
		return []string{"errors"}
	case "vendors\x00/v2/vendors/bulk-create":
		return []string{"errors"}
	case "vendors\x00/v2/vendors/bulk-retrieve":
		return []string{"errors"}
	case "vendors\x00/v2/vendors/bulk-update":
		return []string{"errors"}
	case "vendors\x00/v2/vendors/create":
		return []string{"errors"}
	case "vendors\x00/v2/vendors/{vendor_id}":
		return []string{"errors"}
	case "webhooks\x00/v2/webhooks/subscriptions":
		return []string{"errors"}
	case "webhooks\x00/v2/webhooks/subscriptions/{subscription_id}":
		return []string{"errors"}
	case "webhooks\x00/v2/webhooks/subscriptions/{subscription_id}/test":
		return []string{"errors"}
	case "webhooks\x00/v2/webhooks/subscriptions/{subscription_id}/signature-key":
		return []string{"errors"}
	}
	return nil
}
