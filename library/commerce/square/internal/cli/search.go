...
In local mode: searches locally synced data only.`,
  ...
  square-pp-cli search "error timeout"

  # Force local search only
  square-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  square-pp-cli search "status" --type apple-pay --data-source local
  # JSON output for piping
  square-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /v2/events
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/v2/events", map[string]any{
					"query":  query,
					"cursor": "",
					"limit":  nil,
				})
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'square-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
