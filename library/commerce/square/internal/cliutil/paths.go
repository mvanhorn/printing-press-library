...
const appName = "square-pp-cli"
const envPrefix = "SQUARE"
...
func AtomicWritePrivateFile(path string, data []byte, fileMode, dirMode os.FileMode) error {
	// Windows cannot atomically replace a destination while another writer is
	// publishing it. Serializing the small critical section also prevents a
	// reader from observing competing credential-file renames.
	privateFileWriteMu.Lock()
	defer privateFileWriteMu.Unlock()
