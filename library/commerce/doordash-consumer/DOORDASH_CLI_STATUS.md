# DoorDash Consumer CLI — Printing Press Prototype Status
...
Important install note: the active PATH commands are intentionally Node/CycleTLS wrappers. Stale generated Go binaries in active PATH locations caused 403s against DoorDash in browser-facing calls. They were backed up under `/home/hermes/.hermes/backups/doordash-consumer-pathfix-*`, `/home/hermes/.hermes/backups/doordash-consumer-profile-pathfix-*`, or `/home/hermes/backups/doordash-consumer-pp-cli-install/`, then replaced with wrappers to the canonical clean repo build:

```text
/home/hermes/projects/doordash-consumer-pp-cli/active-wrapper/dist/cli.js
/home/hermes/projects/doordash-consumer-pp-cli/active-wrapper/dist/index.js
```
...
```text
/home/hermes/projects/doordash-consumer-pp-cli/active-wrapper
```

Printing Press-generated skeleton and curated GraphQL spec artifacts:

```text
/home/hermes/printing-press/library/doordash-consumer-pp-cli
/home/hermes/printing-press/library/doordash-consumer-pp-cli/spec.yaml
/home/hermes/printing-press/library/doordash-consumer-pp-cli/references/doordash-consumer-graphql-spec.yaml
/home/hermes/printing-press/library/doordash-consumer-pp-cli/references/doordash-consumer-graphql-spec-manifest.json
```
...
Session file: ~/.config/doordash-consumer-pp-cli/session_cookies.json (0600, values never printed)
...
```text
Active wrapper command: doordash-consumer-pp-cli checkout preview --cart-id <id>
Generated skeleton command: doordash-consumer-pp-cli cart preview --variables '{...}'
...
```text
Smoke script: ./scripts/smoke-doordash-consumer-cli.sh
...
Verifier caveat: `printing-press verify --dir .` currently fails before testing because verifier assumes Go files directly under `cmd/`; this repo uses nested `cmd/doordash-consumer-pp-cli` and `cmd/doordash-consumer-pp-mcp`. Manual Go/smoke checks above are the Phase 8 shipcheck substitute until verifier supports nested cmd layout.
Install: go install refreshed /home/hermes/go/bin/doordash-consumer-pp-cli and /home/hermes/go/bin/doordash-consumer-pp-mcp.
...
Changed active Node/CycleTLS wrapper files: src/api/orders.ts, src/api/menu.ts, src/cli.ts and rebuilt dist/* with npm run build. Generated Go skeleton verification also passed with /home/hermes/.local/bin/go: go test -vet=off ./..., go build ./cmd/doordash-consumer-pp-cli, and go build ./cmd/doordash-consumer-pp-mcp.
...
- `doordash-consumer-pp-cli --help` as runtime command truth
- `doordash-consumer-pp-cli login --json` / `verify --code ... --json` for approved interactive login flow; do not paste secrets in chat
...
```bash
doordash-consumer-pp-cli doctor --json
doordash-consumer-pp-cli login --json
doordash-consumer-pp-cli verify --code 123456 --json
```

Low-level GraphQL skeleton:

```bash
doordash-consumer-pp-cli graphql --help
doordash-consumer-pp-cli graphql create-add-cart-item --help
doordash-consumer-pp-cli graphql create-autocomplete-facet-feed --help
doordash-consumer-pp-cli graphql create-checkout --help
doordash-consumer-pp-cli graphql create-consumer-order-cart --help
doordash-consumer-pp-cli graphql create-convenience-search-query --help
doordash-consumer-pp-cli graphql create-create-order-from-cart --help
doordash-consumer-pp-cli graphql create-delete-cart --help
doordash-consumer-pp-cli graphql create-detailed-cart-items --help
doordash-consumer-pp-cli graphql create-get-has-new-notifications --help
doordash-consumer-pp-cli graphql create-get-open-carts-count --help
doordash-consumer-pp-cli graphql create-item-page --help
doordash-consumer-pp-cli graphql create-list-carts --help
doordash-consumer-pp-cli graphql create-poll-order-payment-status --help
doordash-consumer-pp-cli graphql create-promo-sticky-footer --help
doordash-consumer-pp-cli graphql create-remove-cart-item-v2 --help
doordash-consumer-pp-cli graphql create-storepage-feed --help
doordash-consumer-pp-cli graphql create-total-fee-tally --help
doordash-consumer-pp-cli graphql create-update-cart-item-v2 --help
doordash-consumer-pp-cli graphql create-validate-consumer-address-with-address-link-id --help
```

Cart/order safety gate:

```bash
doordash-consumer-pp-cli order place --cart-id <id> --confirm "PLACE ORDER" --max-total-cents <cents> --max-delivery-fee-cents <cents> --max-tip-cents <cents>   # still blocked without ALLOW_DOORDASH_ORDERING=1
ALLOW_DOORDASH_ORDERING=1 doordash-consumer-pp-cli order place \
  --cart-id <id> \
  --confirm "PLACE ORDER" \
  --tip-cents <cents> \
...
/tmp/doordash-consumer-pp-cli-current doctor --json
/tmp/doordash-consumer-pp-cli-current auth status --json   # safe session metadata only; no cookie values
```
...
```text
/home/hermes/printing-press/library/doordash-consumer-pp-cli/PHASE5_PAUSE_HANDOFF.md
```
