...
func TestLoadDoorDashSessionEnvOverrides(t *testing.T) {
	t.Setenv("DOORDASH_CONSUMER_COOKIE", "a=b")
	t.Setenv("DOORDASH_CONSUMER_CSRF_TOKEN", "csrf")
	t.Setenv("DOORDASH_CONSUMER_SESSION_FILE", "~/custom-session.json")
	t.Setenv("DOORDASH_CONSUMER_USER_AGENT", "ua")
	...
	if cfg.AuthSource != "env:DOORDASH_CONSUMER_COOKIE" {
