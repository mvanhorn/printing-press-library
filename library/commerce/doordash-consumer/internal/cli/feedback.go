...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DOORDASH_CONSUMER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DOORDASH_CONSUMER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "doordash-consumer-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.doordash-consumer-pp-cli/feedback.jsonl.
When ` + "`DOORDASH_CONSUMER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DOORDASH_CONSUMER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  doordash-consumer-pp-cli feedback list
  doordash-consumer-pp-cli feedback list --limit 5
  doordash-consumer-pp-cli feedback list --json`,
