...
Once synced, use the 'search' command for instant full-text search.`,
  ...
  thevc-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  thevc-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  thevc-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  thevc-pp-cli sync --concurrency 8`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("thevc-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/thevc-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "information":
		return db.UpsertInformation(data)
	default:
...
func syncResourcePath(resource string) string {
	paths := map[string]string{
		"information": "/api/information/organizations/latest-registered",
	}
