---
name: pp-priority
description: "The entire Priority REST API — every form, subform, batch, and attachment — plus offline sync, schema intelligence, and throttle-safe bulk operations no other Priority tool has. Trigger phrases: `query priority orders`, `check customer aging in priority`, `sync priority erp data`, `which priority forms are api licensed`, `find a field in the priority schema`, `use priority`, `run priority`."
author: "alon-auto"
license: "Apache-2.0"
...
metadata:
  openclaw:
    ...
    install:
      - kind: go
        bins: [priority-pp-cli]
        module: github.com/mvanhorn/printing-press-library/library/commerce/priority/cmd/priority-pp-cli
---

# Priority Software — Printing Press CLI

## Prerequisites: Install the CLI

This skill drives the `priority-pp-cli` binary. **You must verify the CLI is installed before invoking any command from this skill.** If it is missing, install it first:

1. Install via the Printing Press installer. It defaults binaries to `$HOME/.local/bin` on macOS/Linux and `%LOCALAPPDATA%\Programs\PrintingPress\bin` on Windows:
   ```bash
   npx -y @mvanhorn/printing-press-library install priority --cli-only
   ```
2. Verify: `priority-pp-cli --version`
...
Use this CLI whenever an agent or script needs Priority ERP data or writes: querying any form with OData options, creating or amending business documents (orders, invoices) including subform lines, bulk loads via $batch, attachment and text handling, and tenant schema discovery. The local mirror makes it the right choice for repeated lookups, search, aging/debtor/shortage analytics, and any workflow that would otherwise burn the 100-requests-per-minute cap.

## Anti-triggers

Do not use this CLI for:
- Automating the Priority web UI or form engine flows (use priority-web-sdk — this CLI speaks the OData REST API only)
- Running Priority procedures or printing reports (the OData REST API does not expose procedure execution)
- Administering users, licenses, or tenant configuration (not exposed via the REST API)
- Real-time event push — the API has no streaming; use 'priority-pp-cli tail' for near-real-time polling
...
**invoices** — AR invoices (AINVOICES form; composite key IVNUM+IVTYPE+DEBIT — use 'entity get' for single records)

- `priority-pp-cli invoices` — List AR invoices

**meta** — Service metadata and version probes
...
```bash
priority-pp-cli which "<capability in your own words>"
```
...
Run `priority-pp-cli doctor` to verify setup.
...
- **Filterable** — `--select` keeps a subset of fields. Dotted paths descend into nested structures; arrays traverse element-wise. Critical for keeping context small on verbose APIs:
  ...
  ```bash
  priority-pp-cli customers list --agent --select id,name,status
  ```
...
- Use `--home <dir>` for one invocation, or set `PRIORITY_HOME=<dir>` to relocate all four path kinds under one root.
- Use per-kind env vars only when a specific kind must diverge: `PRIORITY_CONFIG_DIR`, `PRIORITY_DATA_DIR`, `PRIORITY_STATE_DIR`, `PRIORITY_CACHE_DIR`.
- Resolution order is per-kind env var, `--home`, `PRIORITY_HOME`, XDG (`XDG_CONFIG_HOME`, `XDG_DATA_HOME`, `XDG_STATE_HOME`, `XDG_CACHE_HOME`), then platform defaults.
- `config` contains settings like `config.toml` and profiles. `data` contains `credentials.toml`, `data.db`, cookies, and auth sidecars. `state` contains persisted queries, jobs, and `teach.log`. `cache` contains regenerable HTTP/cache files.
- Stored secrets live in `credentials.toml` under the data dir. Existing legacy `config.toml` secrets are read for compatibility and leave `config.toml` on the first auth write.
- Run `priority-pp-cli doctor --fail-on warn` to surface path and credential-location warnings. `agent-context` exposes a schema v4 `paths` block for agents that need the resolved dirs.
...
Fleet precedence: an inherited per-kind env var overrides an explicit `--home` for that kind. Use `PRIORITY_HOME` or per-kind vars as durable fleet levers, and use `--home` only for a single invocation. Relocation is not reversible by unsetting env vars; move files manually before clearing `PRIORITY_HOME`, or `doctor` will not find credentials left under the former root.
...
```bash
priority-pp-cli recall "<user's question>" --agent
```
...
```json
{
  ...
  "candidates": [
    { "id": 12, "class": "flag_alias | playbook_candidate",
      ...
      "rationale": "...",
      "next_action": ["<trial command>", "priority-pp-cli learnings confirm 12"] }
  ],
  "playbook": {
    "query_family": "...",
    "playbook": {
...
if Playbook present:
    ...
    -> the Playbook's expected_tool_calls is a budget; if you find yourself running
       materially more, record the divergence via `priority-pp-cli playbook amend`
       at end-of-session.
...
- `lookup_refresh_available` (top-level): an entity in the query has no lookup row yet, but synced data could provide one. Run `priority-pp-cli sync` to refresh entity lookups.
...
```bash
priority-pp-cli teach --query "<user's question>" --resource-type <type> --resource <id1> --resource <id2>
...
priority-pp-cli teach-playbook \
  --query "<user's question>" \
  --playbook-file ~/playbooks/<shape>.json \
...
```bash
priority-pp-cli playbook amend \
...
`priority-pp-cli learnings stats` reports recall hit rate, teach-to-reuse, playbook resolution rate, and candidate confirm/reject counts from the local `learn_events` table. Rates are null until they have a denominator; everything stays on this machine. Use it to check whether the loop is earning its keep for this CLI.

### Disabling learning

- `--no-learn` on a single command short-circuits both `recall` and the `teach` write path. Use for deterministic agent flows or tests that must not be affected by accumulated learnings.
- `PRIORITY_NO_LEARN=true` in the environment globally disables the pipeline.
...
```
priority-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
priority-pp-cli feedback --stdin < notes.txt
priority-pp-cli feedback list --json --limit 10
```

Entries are stored locally as `feedback.jsonl` under the resolved data dir. They are never POSTed unless `PRIORITY_FEEDBACK_ENDPOINT` is set AND either `--send` is passed or `PRIORITY_FEEDBACK_AUTO_SEND=true`. Default behavior is local-only.
...
```
priority-pp-cli profile save briefing --json
priority-pp-cli --profile briefing customers list
priority-pp-cli profile list --json
priority-pp-cli profile show briefing
priority-pp-cli profile delete briefing --yes
```
...
| 6 | Partial failure — some batch/aggregate operations failed |
...
1. **Empty, `help`, or `--help`** → show `priority-pp-cli --help` output
...
2. Register with Claude Code:
   ```bash
   claude mcp add priority-pp-mcp -- priority-pp-mcp
   ```
3. Verify: `claude mcp list`

## Direct Use

1. Check if installed: `which priority-pp-cli`
