...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PRIORITY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PRIORITY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "priority-pp-cli/feedback")
...
When ` + "`PRIORITY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PRIORITY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  priority-pp-cli feedback list
  priority-pp-cli feedback list --limit 5
  priority-pp-cli feedback list --json`,
