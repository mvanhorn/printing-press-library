...
// priority-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/commerce/priority/internal/learn/patterns"
...
const noLearnEnvVar = "PRIORITY_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("priority-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  priority-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  priority-pp-cli recall "<question>" --agent
  priority-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  priority-pp-cli learnings list --agent
  priority-pp-cli learnings list --query "<substring>"
  priority-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  priority-pp-cli learnings forget "<question>" --resource <id>
  priority-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  priority-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  priority-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
