...
type rootFlags struct {
	...
	// invocation. Mirrors the PRIORITY_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "priority-pp-cli",
		Short: `Priority CLI — The entire Priority REST API — every form, subform, batch, and attachment — plus offline sync, schema intelligence,…`,
		Long: `Priority CLI — The entire Priority REST API — every form, subform, batch, and attachment — plus offline sync, schema intelligence, and throttle-safe bulk operations no other Priority tool has.
...
Health check: run 'priority-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("priority-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'priority-pp-cli profile list')")
	...
	rootCmd.AddCommand(newPartsCmd(flags))
	rootCmd.AddCommand(newPordersCmd(flags))
	rootCmd.AddCommand(newSuppliersCmd(flags))
	...
	rootCmd.AddCommand(newNovelCustomerCmd(flags))
	rootCmd.AddCommand(newNovelFormsCmd(flags))
	...
	rootCmd.AddCommand(newWarehousesPromotedCmd(flags))
