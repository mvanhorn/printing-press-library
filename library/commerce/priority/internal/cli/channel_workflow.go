...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  priority-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  priority-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"customers", "invoices", "orders", "parts", "porders", "suppliers", "warehouses"}
			maxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  priority-pp-cli workflow status

  # Show status as JSON
  priority-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("priority-pp-cli")
			}
