...
var whichIndex = []whichEntry{
	{Command: "forms search", Description: "Find any form, field, or subform on your tenant by name fragment, instantly and offline.", Group: "Tenant schema intelligence", WhyItMatters: "Reach for this before any entity call on an unfamiliar tenant — field names differ per install and guessing burns the 100/min rate cap."},
	{Command: "forms diff", Description: "Snapshot your tenant's schema and see exactly which forms and fields changed after an upgrade or customization.", Group: "Tenant schema intelligence", WhyItMatters: "Run after tenant upgrades to find silent field changes before integrations start failing with 400s."},
	{Command: "forms licensed", Description: "Discover which forms are actually API-enabled on this tenant instead of finding out via 400 errors.", Group: "Tenant schema intelligence", WhyItMatters: "\"API Cannot Be Run for This Form\" is the most common real-world Priority API failure; probe once, know forever."},
	{Command: "batch resume", Description: "Re-run only the failed operations from a partially-failed $batch — Priority batches have no rollback.", Group: "Throttle-safe bulk operations", WhyItMatters: "After a half-failed bulk load, this is the difference between surgical recovery and manually diffing response JSON."},
	{Command: "reconcile", Description: "Verify your local mirror matches the live tenant using windowed probes — the API has no $count.", Group: "Throttle-safe bulk operations", WhyItMatters: "Answers \"did last night's sync actually get everything\" without a full re-pull."},
	{Command: "shortage", Description: "See which parts will run out given the open order book — open demand vs on-hand stock, optionally netting inbound POs.", Group: "Offline ERP analytics", WhyItMatters: "Replaces the weekly Excel VLOOKUP between order lines and stock; sync orders with --resource-param \"orders:$expand=ORDERITEMS_SUBFORM\" first, and expect on_hand null when the tenant exposes no balance fields."},
	{Command: "customer summary", Description: "One command for a customer's whole picture: invoiced totals, age buckets by invoice date, open orders, recent invoices.", Group: "Offline ERP analytics", WhyItMatters: "The pre-collections-call briefing, at terminal speed, without burning the rate cap."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  priority-pp-cli which "stale tickets"
  priority-pp-cli which "bottleneck"
  priority-pp-cli which --limit 1 "send message"
  priority-pp-cli which                                # list the full capability index`,
