...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Priority",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  priority-pp-cli auth setup\n  priority-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "No setup URL is configured for this CLI; check the API's docs.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export PRIORITY_API_USERNAME=\"your-username\"")
			fmt.Fprintln(w, "  export PRIORITY_API_PASSWORD=\"your-password\"")
			fmt.Fprintln(w, "  priority-pp-cli auth set-credentials your-username your-password")
			if !launch {
				return nil
			}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  priority-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export PRIORITY_API_USERNAME=\"your-username\"")
				fmt.Fprintln(w, "  export PRIORITY_API_PASSWORD=\"your-password\"")
				fmt.Fprintf(w, "  priority-pp-cli auth set-credentials your-username your-password\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetCredentialsCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		Use:     "set-credentials <api_username> <api_password>",
		Short:   "Save Basic auth credentials to the credentials file",
		Example: "  priority-pp-cli auth set-credentials your-username your-password",
		Args: func(cmd *cobra.Command, args []string) error {
			if len(args) != 2 {
				return fmt.Errorf("requires exactly 2 args: <api_username> <api_password>")
			}
			if args[0] == "" {
				return fmt.Errorf("PRIORITY_API_USERNAME is required")
			}
			if args[1] == "" {
				return fmt.Errorf("PRIORITY_API_PASSWORD is required")
			}
			return nil
		},
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  priority-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("PRIORITY_API_USERNAME") != "" {
				envStillSet = "PRIORITY_API_USERNAME"
			}
			if envStillSet == "" && os.Getenv("PRIORITY_API_PASSWORD") != "" {
				envStillSet = "PRIORITY_API_PASSWORD"
			}
