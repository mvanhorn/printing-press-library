...
Resource scoping:
  ...
  priority-pp-cli sync

  # Sync specific resources only
  priority-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  priority-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  priority-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  priority-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  priority-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "porders":
		return db.UpsertPorders(data)
	case "suppliers":
		return db.UpsertSuppliers(data)
	default:
...
func responsePathForResource(resource, path string) []string {
	...
	case "customers\x00/CUSTOMERS":
		return []string{"value"}
	case "entity\x00/{form}":
		return []string{"value"}
	case "entity\x00/{form}({keyspec})/{subform}":
		return []string{"value"}
	case "invoices\x00/AINVOICES":
		return []string{"value"}
	case "orders\x00/ORDERS":
		return []string{"value"}
	case "parts\x00/LOGPART":
		return []string{"value"}
	case "porders\x00/PORDERS":
		return []string{"value"}
	case "suppliers\x00/SUPPLIERS":
		return []string{"value"}
	case "warehouses\x00/WAREHOUSES":
		return []string{"value"}
	}
	return nil
}
