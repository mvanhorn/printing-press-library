...
type Config struct {
	...
	PriorityApiUsername string          `toml:"api_username"`
	PriorityApiPassword string          `toml:"api_password"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://t.eu.priority-connect.online/odata/Priority/tabbtd38.ini,3/usdemo",
	}
	...
	// Env var overrides
	if v := os.Getenv("PRIORITY_API_USERNAME"); v != "" {
		cfg.PriorityApiUsername = v
		cfg.markEnvOverride("PriorityApiUsername")
		cfg.AuthSource = "env:PRIORITY_API_USERNAME"
		cfg.CredentialSource = "env:PRIORITY_API_USERNAME"
	}
	if v := os.Getenv("PRIORITY_API_PASSWORD"); v != "" {
		cfg.PriorityApiPassword = v
		cfg.markEnvOverride("PriorityApiPassword")
		cfg.AuthSource = "env:PRIORITY_API_PASSWORD"
		cfg.CredentialSource = "env:PRIORITY_API_PASSWORD"
	}
	...
	if v := os.Getenv("PRIORITY_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("PRIORITY_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "priority-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.PriorityApiUsername == "" {
		return ""
	}
	if c.PriorityApiPassword == "" {
		return ""
	}
	credentials := basicAuthPayload("Basic {username}:{password}", map[string]string{
		"api_username":          c.PriorityApiUsername,
		"PRIORITY_API_USERNAME": c.PriorityApiUsername,
		"username":              c.PriorityApiUsername,
		"user":                  c.PriorityApiUsername,
		"api_password":          c.PriorityApiPassword,
		"PRIORITY_API_PASSWORD": c.PriorityApiPassword,
		"password":              c.PriorityApiPassword,
		"secret":                c.PriorityApiPassword,
		"token":                 c.PriorityApiPassword,
		"access_token":          c.PriorityApiPassword,
	}, false)
	if credentials == "" {
		return ""
	}
...
func (c *Config) clearCredentialFields() {
	...
	c.ClientID = ""
	c.ClientSecret = ""
	c.PriorityApiUsername = ""
	c.PriorityApiPassword = ""
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		PriorityApiUsername: c.PriorityApiUsername,
		PriorityApiPassword: c.PriorityApiPassword,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.PriorityApiUsername = creds.PriorityApiUsername
	c.PriorityApiPassword = creds.PriorityApiPassword
}
...
func (c *Config) SaveCredentials(value0, value1 string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.PriorityApiUsername = value0
	delete(c.envOverrides, "PriorityApiUsername")
	c.updateFileConfigField("PriorityApiUsername")
	c.PriorityApiPassword = value1
	delete(c.envOverrides, "PriorityApiPassword")
	c.updateFileConfigField("PriorityApiPassword")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["PriorityApiUsername"] {
			out.PriorityApiUsername = c.fileConfig.PriorityApiUsername
		}
		if c.envOverrides["PriorityApiPassword"] {
			out.PriorityApiPassword = c.fileConfig.PriorityApiPassword
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "PriorityApiUsername":
		c.fileConfig.PriorityApiUsername = c.PriorityApiUsername
	case "PriorityApiPassword":
		c.fileConfig.PriorityApiPassword = c.PriorityApiPassword
	}
}
