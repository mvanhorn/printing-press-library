# IQOS Printed CLI Agent Guide

This directory is a generated `iqos-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
iqos-pp-cli doctor --json
iqos-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
iqos-pp-cli which "<capability>" --json
iqos-pp-cli <command> --help
```
...
```bash
iqos-pp-cli <command> --help
iqos-pp-cli <command> --dry-run --agent
```
