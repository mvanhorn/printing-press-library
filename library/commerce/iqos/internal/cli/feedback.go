...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".iqos-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("IQOS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("IQOS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "iqos-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.iqos-pp-cli/feedback.jsonl.
When ` + "`IQOS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`IQOS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  iqos-pp-cli feedback list
  iqos-pp-cli feedback list --limit 5
  iqos-pp-cli feedback list --json`,
