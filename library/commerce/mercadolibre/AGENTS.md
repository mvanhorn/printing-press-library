# Mercadolibre Printed CLI Agent Guide

This directory is a generated `mercadolibre-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
mercadolibre-pp-cli which "<capability>" --json
mercadolibre-pp-cli <command> --help
```
...
```bash
mercadolibre-pp-cli <command> --agent
```
...
```bash
mercadolibre-pp-cli <command> --help
mercadolibre-pp-cli <command> --dry-run --agent
```
