...
func (c *Client) graphqlWithThrottle(ctx context.Context, req GraphQLRequest, post func(context.Context, GraphQLRequest) (json.RawMessage, int, error)) (json.RawMessage, error) {
	...
	for attempt := 0; ; attempt++ {
		...
		// pp:patch shopify-dry-run-graphql-stub — Client.dryRun returns a
		// REST-shaped `{"dry_run": true}` stub, not a `{"data": ...}` GraphQL
		// envelope. Unmarshaling it into GraphQLResponse below would silently
		// yield a nil Data with no error, which callers then fail to render
		// as JSON. Pass the stub straight through under dry-run.
		if c.DryRun {
			return raw, nil
		}
