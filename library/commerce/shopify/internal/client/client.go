...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	httpClient.CheckRedirect = func(req *http.Request, via []*http.Request) error {
		...
		if req.URL.Host == via[0].URL.Host {
			...
			req.Header.Del("X-Shopify-Access-Token")
		}
		return nil
	}
	return c
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	if urlErr != nil {
		// pp:patch shopify-dry-run-template-vars — let --dry-run preview the
		// request shape with placeholder vars instead of hard-failing before
		// ever reaching the DryRun branch below. The live path is unaffected:
		// a real request still fails buildURL's validation immediately.
		if _, isTemplateErr := urlErr.(*TemplateVarError); isTemplateErr && c.DryRun {
			targetURL = buildPreviewURL(c.BaseURL, path, endpointVars)
		} else {
			return nil, 0, urlErr
		}
	}
