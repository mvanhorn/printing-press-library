...
func TestUpsertBatch_PopulatesInventoryItemsTable(t *testing.T) {
	...
	if _, _, err := s.UpsertBatch("inventory_items", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "inventory_items").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
...
func TestUpsertBatch_PopulatesFulfillmentOrdersTable(t *testing.T) {
	...
	if _, _, err := s.UpsertBatch("fulfillment_orders", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "fulfillment_orders").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
