...
func TestSearchAbandonedCheckoutsQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if stored, failed, err := s.UpsertBatch("abandoned-checkouts", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	} else if failed != 0 || stored != len(items) {
		t.Fatalf("UpsertBatch stored=%d failed=%d, want stored=%d failed=0", stored, failed, len(items))
	}
	...
	for _, query := range []string{
		...
		results, err := s.SearchAbandonedCheckouts(query, 10)
		if err != nil {
			t.Fatalf("SearchAbandonedCheckouts(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchAbandonedCheckouts(%q) returned no results", query)
		}
	}
}
...
func TestSearchOrdersQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if stored, failed, err := s.UpsertBatch("orders", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	} else if failed != 0 || stored != len(items) {
		t.Fatalf("UpsertBatch stored=%d failed=%d, want stored=%d failed=0", stored, failed, len(items))
	}
	...
	for _, query := range []string{
		...
		results, err := s.SearchOrders(query, 10)
		if err != nil {
			t.Fatalf("SearchOrders(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchOrders(%q) returned no results", query)
		}
	}
}
