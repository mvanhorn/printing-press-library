...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("support_ticket_one_shot",
			mcplib.WithDescription("Order + fulfillment + inventory availability in one call instead of three."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Override the recipe's positional id value.")),
		),
		handleSupportTicketOneShot,
	)
}

// handleSupportTicketOneShot runs the support_ticket_one_shot recipe intent: Order + fulfillment + inventory availability in one call instead of three.
func handleSupportTicketOneShot(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
