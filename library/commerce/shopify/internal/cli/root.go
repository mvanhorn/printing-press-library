...
type rootFlags struct {
	...
	// invocation. Mirrors the SHOPIFY_NO_LEARN env var.
	noLearn       bool
	selectFields  string
	configPath    string
	homePath      string
	...
	freshnessMeta any
	throttleMode  string
	// reportDBPath overrides the SQLite store path for the hand-authored
	// report/growth/ops/merchandising analytics command families, bound via
	// each family's own PersistentFlags --db registration.
	reportDBPath string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "shopify-pp-cli",
		Short: `Operate a Shopify store from the terminal with curated Admin GraphQL commands, local sync, analytics, and bulk exports.`,
		Long: `Operate a Shopify store from the terminal with curated Admin GraphQL commands, local sync, analytics, and bulk exports.
...
Health check: run 'shopify-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
	...
	doctorCmd.AddCommand(newNovelDoctorThrottleCmd(flags))
	...
	rootCmd.AddCommand(newAbandonedCheckoutsPromotedCmd(flags))
	customersCmd := newCustomersPromotedCmd(flags)
	customersCmd.AddCommand(newCustomersTagCmd(flags))
	rootCmd.AddCommand(customersCmd)
	rootCmd.AddCommand(newFulfillmentOrdersPromotedCmd(flags))
	rootCmd.AddCommand(newInventoryItemsPromotedCmd(flags))
	ordersCmd := newOrdersPromotedCmd(flags)
