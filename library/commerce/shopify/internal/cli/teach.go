...
// shopify-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/commerce/shopify/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/commerce/shopify/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/commerce/shopify/internal/learn/patterns"
...
const noLearnEnvVar = "SHOPIFY_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("shopify-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  shopify-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  shopify-pp-cli recall "<question>" --agent
  shopify-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  shopify-pp-cli learnings list --agent
  shopify-pp-cli learnings list --query "<substring>"
  shopify-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  shopify-pp-cli learnings forget "<question>" --resource <id>
  shopify-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  shopify-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  shopify-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
