...
Run sync first to populate the local search index.`,
  ...
  shopify-pp-cli search "status"

  # Force local search explicitly
  shopify-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  shopify-pp-cli search "status" --type abandoned-checkouts --data-source local
  # JSON output for piping
  shopify-pp-cli search "status" --json --limit 20`,
