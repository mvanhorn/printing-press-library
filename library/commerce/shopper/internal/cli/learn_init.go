...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"store": {
			{Canonical: "Compra Programada", Values: []string{"programada", "mensal", "monthly", "programada mensal"}},
			{Canonical: "Programada Fresh", Values: []string{"fresh", "fresco", "fresh shopper"}},
			{Canonical: "Compra Única", Values: []string{"unica", "única", "pontual", "one-time"}},
			{Canonical: "Pet.Shopper", Values: []string{"pet", "pet shopper", "animais"}},
			{Canonical: "Shopper Now", Values: []string{"now", "rapido", "expresso", "ultrafast"}},
			{Canonical: "Now Bebidas", Values: []string{"now-bebidas", "bebidas", "now bebidas", "drinks"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: shopper-pp-cli: learn init: %v\n", err)
		}
	})
}
