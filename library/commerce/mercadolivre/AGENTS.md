# Mercado Livre Printed CLI Agent Guide

This directory is a generated `mercadolivre-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
mercadolivre-pp-cli doctor --json
mercadolivre-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
mercadolivre-pp-cli which "<capability>" --json
mercadolivre-pp-cli <command> --help
```
...
```bash
mercadolivre-pp-cli <command> --agent
```
...
```bash
mercadolivre-pp-cli <command> --help
mercadolivre-pp-cli <command> --dry-run --agent
```
