...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MERCADOLIVRE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MERCADOLIVRE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "mercadolivre-pp-cli/feedback")
...
When ` + "`MERCADOLIVRE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MERCADOLIVRE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  mercadolivre-pp-cli feedback list
  mercadolivre-pp-cli feedback list --limit 5
  mercadolivre-pp-cli feedback list --json`,
