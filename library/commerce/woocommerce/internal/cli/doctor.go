...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  woocommerce-pp-cli doctor
  woocommerce-pp-cli doctor --json
  woocommerce-pp-cli doctor --fail-on warn
  woocommerce-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set Basic credentials with: export WOOCOMMERCE_CONSUMER_KEY=\"your-token-here\" WOOCOMMERCE_CONSUMER_SECRET=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if os.Getenv("WOOCOMMERCE_CONSUMER_KEY") != "" {
				authEnvSet = append(authEnvSet, "WOOCOMMERCE_CONSUMER_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "WOOCOMMERCE_CONSUMER_KEY")
			}
			if os.Getenv("WOOCOMMERCE_CONSUMER_SECRET") != "" {
				authEnvSet = append(authEnvSet, "WOOCOMMERCE_CONSUMER_SECRET")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "WOOCOMMERCE_CONSUMER_SECRET")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "woocommerce-pp-cli", suggestion)
						} else {
...
func collectCredentialsLocationReport(report map[string]any, cfg *config.Config) {
	...
	if credsPresent && len(locations) > 1 {
		if legacySecretsElsewhere != "" {
			report["credentials_location_warning"] = "WARN credentials stored in more than one location; legacy secrets remain at " + legacySecretsElsewhere + "; run auth set-credentials or auth logout to consolidate and remove legacy secrets"
		} else {
			report["credentials_location_warning"] = "WARN credentials stored in more than one location; current reads use credentials file; run auth set-credentials or auth logout to consolidate"
		}
	}
}
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'woocommerce-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'woocommerce-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'woocommerce-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'woocommerce-pp-cli sync' to refresh."
	}
	return report
}
