...
// woocommerce-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/commerce/woocommerce/internal/learn/patterns"
...
const noLearnEnvVar = "WOOCOMMERCE_NO_LEARN"
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  woocommerce-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  woocommerce-pp-cli recall "<question>" --agent
  woocommerce-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  woocommerce-pp-cli learnings list --agent
  woocommerce-pp-cli learnings list --query "<substring>"
  woocommerce-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  woocommerce-pp-cli learnings forget "<question>" --resource <id>
  woocommerce-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  woocommerce-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  woocommerce-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
