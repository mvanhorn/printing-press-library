...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  woocommerce-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  woocommerce-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"all-variations", "brands", "catalog", "catalog-reviews", "catalog-taxonomy", "catalog-taxonomy-attributes", "catalog-taxonomy-brands", "catalog-taxonomy-categories", "categories", "coupons", "customers", "orders", "payment-gateways", "product-attributes", "products", "products-custom-fields-names", "products-suggested-products", "refunds", "reports", "reviews", "settings", "shipping-classes", "shipping-methods", "shipping-zones", "store-data", "store-data-continents", "store-data-countries", "store-data-currencies", "system-status", "tags", "tax-classes", "taxes", "webhooks"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  woocommerce-pp-cli workflow status

  # Show status as JSON
  woocommerce-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("woocommerce-pp-cli")
			}
