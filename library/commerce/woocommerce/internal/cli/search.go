...
Run sync first to populate the local search index.`,
  ...
  woocommerce-pp-cli search "status"

  # Force local search explicitly
  woocommerce-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  woocommerce-pp-cli search "status" --type all-variations --data-source local
  # JSON output for piping
  woocommerce-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'woocommerce-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
