...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("WOOCOMMERCE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("WOOCOMMERCE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "woocommerce-pp-cli/feedback")
...
When ` + "`WOOCOMMERCE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`WOOCOMMERCE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  woocommerce-pp-cli feedback list
  woocommerce-pp-cli feedback list --limit 5
  woocommerce-pp-cli feedback list --json`,
