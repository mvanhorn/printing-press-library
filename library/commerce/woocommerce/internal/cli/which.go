...
var whichIndex = []whichEntry{
	{Command: "orders triage", Description: "Splits the order queue into actionable buckets and flags payment gateways whose failure rate is climbing.", Group: "Local state that compounds", WhyItMatters: "Reach for this instead of listing orders by status when you need to know what is stuck and whether a gateway is degrading."},
	{Command: "stock velocity", Description: "Units sold per day per variation divided into current stock, producing days-of-cover and a reorder list sorted by stockout date.", Group: "Local state that compounds", WhyItMatters: "Use this to answer 'what do I reorder' - it is the one number the WooCommerce admin never shows."},
	{Command: "revenue explain", Description: "Breaks a revenue change between two periods into order-count, basket-size, per-product, refund, and coupon contributions.", Group: "Local state that compounds", WhyItMatters: "Pick this over a sales report when the question is why a number moved, not what the number is."},
	{Command: "refund-rate", Description: "Refunded units over sold units for every product and variation, so return-prone items surface by rate rather than raw count.", Group: "Local state that compounds", WhyItMatters: "Use this to find margin leaks that a top-sellers report actively hides."},
	{Command: "customers ltv", Description: "Per-customer lifetime value and order count, plus repeat-purchase retention bucketed by first-order month.", Group: "Local state that compounds", WhyItMatters: "Reach for this when the question is about customer quality over time rather than a single order."},
	{Command: "catalog audit", Description: "Deterministic scan for publishable defects: missing images, empty descriptions, absent SKUs, zero prices, and unpriced or stockless variations.", Group: "Catalog intelligence", WhyItMatters: "Run this before a campaign to catch broken listings while they are still cheap to fix."},
	{Command: "catalog watch", Description: "Records a timestamped price, sale, and stock snapshot of any public WooCommerce store, with no credentials of any kind.", Group: "Catalog intelligence", WhyItMatters: "Use this on stores you do not own; every other WooCommerce tool assumes you hold admin keys."},
	{Command: "catalog diff", Description: "Compares two recorded snapshots to report added and delisted products, price moves with magnitude, sale windows, and stock flips.", Group: "Catalog intelligence", WhyItMatters: "This is the payoff of catalog watch: what actually changed, rather than what is true right now."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  woocommerce-pp-cli which "stale tickets"
  woocommerce-pp-cli which "bottleneck"
  woocommerce-pp-cli which --limit 1 "send message"
  woocommerce-pp-cli which                                # list the full capability index`,
