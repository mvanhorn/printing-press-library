...
var (
	...
	// querySynonymMu guards the two globals above. The comment on
	// registration claims it is one-shot at startup, but the generated
	// concurrency test calls it from several goroutines, and
	// compileQuerySynonyms iterates the same map RegisterQuerySynonyms
	// writes. Without this lock that is a fatal concurrent map
	// iteration/write, reproducible under -race.
