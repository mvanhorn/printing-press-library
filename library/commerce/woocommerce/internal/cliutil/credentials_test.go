...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"WOOCOMMERCE_CONFIG",
		"WOOCOMMERCE_CONFIG_DIR",
		"WOOCOMMERCE_STATE_DIR",
		"WOOCOMMERCE_CACHE_DIR",
		"WOOCOMMERCE_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
	...
	return home, filepath.Join(home, ".config", "woocommerce-pp-cli", "config.toml")
}

func TestLegacyConfigWinsWhenCredentialsFileAlsoHasSecrets(t *testing.T) {
	...
	assertBasicAuthCredential(t, cfg.AuthHeader(), "legacy-secret")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, "data", "woocommerce-pp-cli", "credentials.toml")
	...
	if stderr != "" {
		t.Fatalf("stderr %q, want no credentials-file warning when config credentials win", stderr)
	}
}
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	home, _ := resetCredentialEnv(t)
	t.Setenv("WOOCOMMERCE_CONSUMER_KEY", "env-secret")
	t.Setenv("WOOCOMMERCE_CONSUMER_SECRET", "env-secret-secret")
	credentialsPath := filepath.Join(home, "data", "woocommerce-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
	...
	stderr := captureCredentialStderr(t, func() {
		...
		assertBasicAuthCredential(t, cfg.AuthHeader(), "env-secret")
	})
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "woocommerce-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "woocommerce-pp-cli", "credentials.toml")
...
func testCredentials(token string) *cliutil.Credentials {
	...
	creds.WoocommerceConsumerSecret = token + "-secret"
	return creds
}
...
func saveConfigCredential(cfg *config.Config, token string) error {
	return cfg.SaveCredentials(token, token+"-secret")
}

func assertBasicAuthCredential(t *testing.T, header, key string) {
	t.Helper()
	const prefix = "Basic "
	if !strings.HasPrefix(header, prefix) {
		t.Fatalf("AuthHeader() = %q, want Basic header", header)
	}
	payload, err := base64.StdEncoding.DecodeString(strings.TrimPrefix(header, prefix))
	if err != nil {
		t.Fatalf("decode AuthHeader(): %v", err)
	}
	if got, want := string(payload), key+":"+key+"-secret"; got != want {
		t.Fatalf("decoded AuthHeader() = %q, want %q", got, want)
	}
}
...
func configCredentialValue(cfg *config.Config) string {
	return cfg.WoocommerceConsumerKey
}
...
func credentialValue(creds *cliutil.Credentials) string {
	return creds.WoocommerceConsumerKey
}

func setCredentialValue(creds *cliutil.Credentials, token string) {
	creds.WoocommerceConsumerKey = token
}
