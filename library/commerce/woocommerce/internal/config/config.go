...
type Config struct {
	...
	WoocommerceConsumerKey    string          `toml:"consumer_key"`
	WoocommerceConsumerSecret string          `toml:"consumer_secret"`
...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://woocommerce.com/wp-json",
	}
	...
	if cfg.AgentcookieManagedByExternalStore() {
		...
		// A config file that carries its own credentials wins over the shared
		// credentials file. Without this, every config points at the same
		// global key pair: per-store credentials are silently ignored, and a
		// command aimed at store B sends store A's keys to B's server. That is
		// a cross-tenant credential leak for anyone running more than one store.
		if cfg.hasCredentialFields() {
			cfg.AuthSource = "config"
			cfg.CredentialSource = "config file"
		} else {
	...
	// Env var overrides
	if v := os.Getenv("WOOCOMMERCE_CONSUMER_KEY"); v != "" {
		cfg.WoocommerceConsumerKey = v
		cfg.markEnvOverride("WoocommerceConsumerKey")
		cfg.AuthSource = "env:WOOCOMMERCE_CONSUMER_KEY"
		cfg.CredentialSource = "env:WOOCOMMERCE_CONSUMER_KEY"
	}
	if v := os.Getenv("WOOCOMMERCE_CONSUMER_SECRET"); v != "" {
		cfg.WoocommerceConsumerSecret = v
		cfg.markEnvOverride("WoocommerceConsumerSecret")
		cfg.AuthSource = "env:WOOCOMMERCE_CONSUMER_SECRET"
		cfg.CredentialSource = "env:WOOCOMMERCE_CONSUMER_SECRET"
	}
	...
	if v := os.Getenv("WOOCOMMERCE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("WOOCOMMERCE_ATTRIBUTE_ID")); v != "" {
		cfg.TemplateVars["attribute_id"] = v
	} else if verifyMode {
		cfg.TemplateVars["attribute_id"] = "attribute_id_placeholder"
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("WOOCOMMERCE_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "woocommerce-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.WoocommerceConsumerKey == "" {
		return ""
	}
	if c.WoocommerceConsumerSecret == "" {
		return ""
	}
	credentials := basicAuthPayload("Basic {consumer_key}:{consumer_secret}", map[string]string{
		"consumer_key":                c.WoocommerceConsumerKey,
		"WOOCOMMERCE_CONSUMER_KEY":    c.WoocommerceConsumerKey,
		"username":                    c.WoocommerceConsumerKey,
		"user":                        c.WoocommerceConsumerKey,
		"consumer_secret":             c.WoocommerceConsumerSecret,
		"WOOCOMMERCE_CONSUMER_SECRET": c.WoocommerceConsumerSecret,
		"password":                    c.WoocommerceConsumerSecret,
		"secret":                      c.WoocommerceConsumerSecret,
		"token":                       c.WoocommerceConsumerSecret,
		"access_token":                c.WoocommerceConsumerSecret,
	}, false)
	if credentials == "" {
		return ""
	}
...
func basicAuthPayload(format string, replacements map[string]string, appendColonForSingleToken bool) string {
	payload := strings.TrimSpace(format)
	if scheme, rest, ok := strings.Cut(payload, " "); ok && strings.EqualFold(strings.TrimSpace(scheme), "Basic") {
		payload = strings.TrimSpace(rest)
	}
	payload = applyAuthFormat(payload, replacements)
	if payload == "" {
		return ""
	}
	if appendColonForSingleToken && !strings.Contains(payload, ":") {
		payload += ":"
	}
	return payload
}
...
func (c *Config) clearCredentialFields() {
	...
	c.ClientID = ""
	c.ClientSecret = ""
	c.WoocommerceConsumerKey = ""
	c.WoocommerceConsumerSecret = ""
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		WoocommerceConsumerKey:    c.WoocommerceConsumerKey,
		WoocommerceConsumerSecret: c.WoocommerceConsumerSecret,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.WoocommerceConsumerKey = creds.WoocommerceConsumerKey
	c.WoocommerceConsumerSecret = creds.WoocommerceConsumerSecret
}
...
// SaveCredentials persists a two-part request credential pair to the fields
// AuthHeader() consults. It is used for Basic auth pairs, where set-token
// cannot represent the username/password shape.
func (c *Config) SaveCredentials(value0, value1 string) error {
	c.AuthHeaderVal = ""
	c.AccessToken = ""
	...
	c.updateFileConfigField("AccessToken")
	c.WoocommerceConsumerKey = value0
	delete(c.envOverrides, "WoocommerceConsumerKey")
	c.updateFileConfigField("WoocommerceConsumerKey")
	c.WoocommerceConsumerSecret = value1
	delete(c.envOverrides, "WoocommerceConsumerSecret")
	c.updateFileConfigField("WoocommerceConsumerSecret")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AuthHeaderVal")
	c.updateFileConfigField("AccessToken")
	c.WoocommerceConsumerKey = token
	delete(c.envOverrides, "WoocommerceConsumerKey")
	c.updateFileConfigField("WoocommerceConsumerKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["WoocommerceConsumerKey"] {
			out.WoocommerceConsumerKey = c.fileConfig.WoocommerceConsumerKey
		}
		if c.envOverrides["WoocommerceConsumerSecret"] {
			out.WoocommerceConsumerSecret = c.fileConfig.WoocommerceConsumerSecret
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "WoocommerceConsumerKey":
		c.fileConfig.WoocommerceConsumerKey = c.WoocommerceConsumerKey
	case "WoocommerceConsumerSecret":
		c.fileConfig.WoocommerceConsumerSecret = c.WoocommerceConsumerSecret
	}
}
