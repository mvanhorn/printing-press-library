...
func walk(cmd *cobra.Command, path []string, visit func(*cobra.Command, []string)) {
	for _, child := range cmd.Commands() {
		// mcp:hidden is the explicit MCP opt-out: prune the whole subtree.
		if isMCPHidden(child) {
			continue
		}
		childPath := append(append([]string{}, path...), child.Name())
		// Cobra's Hidden only curates the CLI's root help; it is not an MCP
		// directive. Don't register the curated group itself, but keep
		// descending so novel leaves hosted under an endpoint-mirror group
		// (e.g. `orders triage`) stay reachable from the MCP surface. The
		// group's endpoint children still classify as commandEndpoint and are
		// skipped by the visitor.
		if !child.Hidden {
			visit(child, childPath)
		}
		if kind := classify(child); kind != commandFramework {
