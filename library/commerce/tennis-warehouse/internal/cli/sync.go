...
Resource scoping:
  ...
  tennis-warehouse-pp-cli sync

  # Sync specific resources only
  tennis-warehouse-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  tennis-warehouse-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  tennis-warehouse-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  tennis-warehouse-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  tennis-warehouse-pp-cli sync --latest-only`,
