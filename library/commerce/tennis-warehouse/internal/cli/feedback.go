...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".tennis-warehouse-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TENNIS_WAREHOUSE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TENNIS_WAREHOUSE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "tennis-warehouse-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.tennis-warehouse-pp-cli/feedback.jsonl.
When ` + "`TENNIS_WAREHOUSE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TENNIS_WAREHOUSE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  tennis-warehouse-pp-cli feedback list
  tennis-warehouse-pp-cli feedback list --limit 5
  tennis-warehouse-pp-cli feedback list --json`,
