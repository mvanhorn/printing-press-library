...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  tennis-warehouse-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  tennis-warehouse-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// syncResource emits NDJSON sync events directly to os.Stdout.
			// When --json is set, the caller expects a single JSON envelope on
			// stdout; we redirect stdout to stderr for the duration of the
			// sync loop so the streaming events don't corrupt the final
			// envelope. Restored after the loop.
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/tennis-warehouse-pp-cli/data.db)")
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  tennis-warehouse-pp-cli workflow status

  # Show status as JSON
  tennis-warehouse-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("tennis-warehouse-pp-cli")
			}
