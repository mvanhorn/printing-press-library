# Tennis Warehouse Printed CLI Agent Guide

This directory is a generated `tennis-warehouse-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
tennis-warehouse-pp-cli doctor --json
tennis-warehouse-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
tennis-warehouse-pp-cli which "<capability>" --json
tennis-warehouse-pp-cli <command> --help
```
...
```bash
tennis-warehouse-pp-cli <command> --agent
```
...
```bash
tennis-warehouse-pp-cli <command> --help
tennis-warehouse-pp-cli <command> --dry-run --agent
```
