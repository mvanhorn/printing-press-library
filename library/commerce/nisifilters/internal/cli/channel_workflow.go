...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  nisifilters-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  nisifilters-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"authors", "categories", "comments", "media", "pages", "posts", "product_categories", "products", "tags"}
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  nisifilters-pp-cli workflow status

  # Show status as JSON
  nisifilters-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("nisifilters-pp-cli")
			}
