...
var whichIndex = []whichEntry{
	{Command: "shop", Description: "List filters joined with their categories — name, price, stock (in / backorder / out) — sorted by price.", Group: "Local joins that compound", WhyItMatters: "Use to scan the filter catalog with prices and categories in one view."},
	{Command: "variants", Description: "List a variable product's WooCommerce variations — variation id, attributes, price, stock — invisible via wp-json/wp/v2.", Group: "Local joins that compound", WhyItMatters: "Use to get the exact variation_id and stock for a size (e.g. 82mm vs 95mm adapter ring) before building cart links."},
	{Command: "filters", Description: "Discover available filter sizes/types from product attributes and find matching products.", Group: "Local joins that compound", WhyItMatters: "Reach for this to answer 'which sizes exist' or 'which products are 82mm' without clicking the storefront."},
	{Command: "read", Description: "Print one post, page, or product as title plus plain-text content, stripping the WordPress envelope.", Group: "Agent-native plumbing", WhyItMatters: "Use to read Academy guides without burning context on WordPress envelope fields."},
	{Command: "digest", Description: "Summarize the mirror: product count, price range, in-stock vs out, top categories, and content counts.", Group: "Local joins that compound", WhyItMatters: "Use for a one-shot overview of the catalog and site without paging every collection."},
	{Command: "since", Description: "List posts and pages modified within a time window, across those types at once.", Group: "Agent-native plumbing", WhyItMatters: "Use to answer 'what is new on the site' without checking each collection separately."},
	{Command: "image", Description: "Resolve the featured image of a post or page, or a product's inline images, to full-resolution URLs.", Group: "Agent-native plumbing", WhyItMatters: "Use when you have a content item or product and need its actual image URL."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  nisifilters-pp-cli which "stale tickets"
  nisifilters-pp-cli which "bottleneck"
  nisifilters-pp-cli which --limit 1 "send message"
  nisifilters-pp-cli which                                # list the full capability index`,
