...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NISIFILTERS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NISIFILTERS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "nisifilters-pp-cli/feedback")
...
When ` + "`NISIFILTERS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NISIFILTERS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  nisifilters-pp-cli feedback list
  nisifilters-pp-cli feedback list --limit 5
  nisifilters-pp-cli feedback list --json`,
