...
In local mode: searches locally synced data only.`,
  ...
  nisifilters-pp-cli search "error timeout"

  # Force local search only
  nisifilters-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  nisifilters-pp-cli search "status" --type authors --data-source local
  # JSON output for piping
  nisifilters-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'nisifilters-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "authors":
				results, err = db.SearchAuthors(query, limit)
			case "categories":
				results, err = db.SearchCategories(query, limit)
			case "product_categories":
				results, err = db.SearchProductCategories(query, limit)
			case "products":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchAuthors(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search authors failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchProductCategories(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search product_categories failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
