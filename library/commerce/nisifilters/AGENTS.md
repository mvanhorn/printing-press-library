# NiSi Italia Printed CLI Agent Guide

This directory is a generated `nisifilters-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
nisifilters-pp-cli doctor --json
nisifilters-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
nisifilters-pp-cli which "<capability>" --json
nisifilters-pp-cli <command> --help
```
...
```bash
nisifilters-pp-cli <command> --agent
```
...
```bash
nisifilters-pp-cli <command> --help
nisifilters-pp-cli <command> --dry-run --agent
```
