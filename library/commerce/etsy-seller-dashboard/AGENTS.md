# Etsy Seller Dashboard Printed CLI Agent Guide

This directory is a generated `etsy-seller-dashboard-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
etsy-seller-dashboard-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
etsy-seller-dashboard-pp-cli which "<capability>" --json
etsy-seller-dashboard-pp-cli <command> --help
```
...
```bash
etsy-seller-dashboard-pp-cli <command> --agent
```
...
```bash
etsy-seller-dashboard-pp-cli <command> --help
etsy-seller-dashboard-pp-cli <command> --dry-run --agent
```
...
Disable the loop with `--no-learn` per-invocation or `ETSY_SELLER_DASHBOARD_NO_LEARN=true` for the whole session - useful for deterministic agent flows that don't want a learning row to silently change subsequent query results.
