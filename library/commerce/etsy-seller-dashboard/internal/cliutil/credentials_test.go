...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"ETSY_SELLER_DASHBOARD_CONFIG",
		"ETSY_SELLER_DASHBOARD_CONFIG_DIR",
		"ETSY_SELLER_DASHBOARD_DATA_DIR",
		"ETSY_SELLER_DASHBOARD_STATE_DIR",
		"ETSY_SELLER_DASHBOARD_CACHE_DIR",
	...
	return home, filepath.Join(home, ".config", "etsy-seller-dashboard-pp-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "etsy-seller-dashboard-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("ETSY_COOKIE", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "etsy-seller-dashboard-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "etsy-seller-dashboard-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "etsy-seller-dashboard-pp-cli", "credentials.toml")
