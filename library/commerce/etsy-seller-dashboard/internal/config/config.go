...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := strings.ToLower(strings.TrimSpace(os.Getenv("ETSY_SELLER_DASHBOARD_SKIP_TLS_VERIFY"))); v == "1" || v == "true" || v == "yes" || v == "on" {
		cfg.SetSkipTLSVerify(true)
	}
	if v := os.Getenv("ETSY_COOKIE"); v != "" {
		cfg.EtsyCookie = v
		cfg.markEnvOverride("EtsyCookie")
		cfg.AuthSource = "env:ETSY_COOKIE"
		cfg.CredentialSource = "env:ETSY_COOKIE"
	}
	...
	if v := os.Getenv("ETSY_SELLER_DASHBOARD_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("ETSY_SELLER_DASHBOARD_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "etsy-seller-dashboard-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.EtsyCookie != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:ETSY_COOKIE"
		}
		return ensureAuthScheme("Bearer", c.EtsyCookie)
	}
	if c.AccessToken != "" {
		if c.AuthSource == "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.EtsyCookie = ""
	delete(c.envOverrides, "EtsyCookie")
	c.updateFileConfigField("EtsyCookie")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	...
	if c.fileConfig != nil {
		...
		if c.envOverrides["EtsyCookie"] {
			out.EtsyCookie = c.fileConfig.EtsyCookie
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "EtsyCookie":
		c.fileConfig.EtsyCookie = c.EtsyCookie
	}
}
