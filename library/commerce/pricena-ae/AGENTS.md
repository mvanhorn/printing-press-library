# Pricena Ae Printed CLI Agent Guide

This directory is a generated `pricena-ae-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
pricena-ae-pp-cli doctor --json
pricena-ae-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
pricena-ae-pp-cli which "<capability>" --json
pricena-ae-pp-cli <command> --help
```
...
```bash
pricena-ae-pp-cli <command> --agent
```
...
```bash
pricena-ae-pp-cli <command> --help
pricena-ae-pp-cli <command> --dry-run --agent
```
