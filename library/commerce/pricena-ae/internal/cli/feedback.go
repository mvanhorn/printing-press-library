...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "pricena-ae-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PRICENA_AE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PRICENA_AE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "pricena-ae-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/pricena-ae-pp-cli/feedback.jsonl.
When ` + "`PRICENA_AE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PRICENA_AE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pricena-ae-pp-cli feedback list
  pricena-ae-pp-cli feedback list --limit 5
  pricena-ae-pp-cli feedback list --json`,
