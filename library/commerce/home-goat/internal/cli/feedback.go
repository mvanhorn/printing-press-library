...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "home-goat-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("HOME_GOAT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HOME_GOAT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/commerce/home-goat/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/home-goat-pp-cli/feedback.jsonl.
When ` + "`HOME_GOAT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`HOME_GOAT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  home-goat-pp-cli feedback list
  home-goat-pp-cli feedback list --limit 5
  home-goat-pp-cli feedback list --json`,
