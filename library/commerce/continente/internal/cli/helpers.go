...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API credentials."+
			"\n      Run 'continente-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: permission denied. This API is configured without credentials, so the service may be blocking the request by rate limit, geography, bot protection, or endpoint policy."+
			"\n      Run 'continente-pp-cli doctor' to check connectivity.", err))
...
func compactListFields(items []map[string]any) json.RawMessage {
	keepFields := map[string]bool{
		// Identity
		"id": true, "name": true, "brand": true, "title": true, "identifier": true,
...
type DataProvenance struct {
	...
	Store        *config.PreferredStore `json:"preferred_store,omitempty"`
}

func resolvedMetaMode(flags *rootFlags) string {
	if flags == nil || flags.metaMode == "" || flags.metaMode == "auto" {
		if flags != nil && flags.agent {
			return "minimal"
		}
		return "full"
	}
	return flags.metaMode
}
...
func printProvenance(cmd *cobra.Command, count int, prov DataProvenance) {
	...
	if prov.Source == "live" {
		if prov.Store != nil {
			fmt.Fprintf(cmd.ErrOrStderr(), "%d results (live, store %s)\n", count, provenanceStoreLabel(prov.Store))
			return
		}
	...
	if prov.Store != nil {
		fmt.Fprintf(cmd.ErrOrStderr(), "%s%d results (cached, synced %s, store %s)\n", prefix, count, age, provenanceStoreLabel(prov.Store))
		return
	}
	fmt.Fprintf(cmd.ErrOrStderr(), "%s%d results (cached, synced %s)\n", prefix, count, age)
}

func provenanceStoreLabel(store *config.PreferredStore) string {
	if store == nil {
		return ""
	}
...
func wrapWithProvenance(data json.RawMessage, prov DataProvenance, mode string) (json.RawMessage, error) {
	if mode == "none" {
		if json.Valid(data) {
			return data, nil
		}
	...
	if mode != "minimal" {
		...
		if prov.Store != nil {
			meta["preferred_store"] = prov.Store
		}
	}
	var results any
	if json.Valid(data) {
