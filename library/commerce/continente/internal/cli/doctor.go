...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  continente-pp-cli doctor
  continente-pp-cli doctor --json
  continente-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				...
				report["cookie_jar_path"] = cfg.CookieJarPath
				if cfg.PreferredStore != nil {
					report["preferred_store"] = cfg.PreferredStore
				}
			}
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if cfg.CookieJarPath != "" {
						if count, countErr := storedCookieCount(cfg.CookieJarPath); countErr == nil {
							report["cookie_jar_cookies"] = count
						}
					}
					report["contract"] = acquisition.Discover(cmd.Context(), c).DoctorSummary()
					if status, statusErr := buildAuthStatusPayload(cmd.Context(), c); statusErr == nil {
						report["auth_status"] = status.Summary
						report["auth_cookie_count"] = status.CookieCount
						if cfg.SessionMode == "imported" {
							report["auth"] = "session-backed"
						} else if status.CookieCount > 0 {
							report["auth"] = "guest cookies present"
						}
					}
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "continente-pp-cli", suggestion)
						} else {
				...
				report["contract"] = map[string]any{
					"mode":   "unknown",
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("continente-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'continente-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'continente-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'continente-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'continente-pp-cli sync' to refresh."
	}
	return report
}
