...
func TestResolvedMetaMode(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		{name: "nil flags default full", flags: nil, want: "full"},
		{name: "auto defaults full", flags: &rootFlags{metaMode: "auto"}, want: "full"},
		{name: "agent auto becomes minimal", flags: &rootFlags{metaMode: "auto", agent: true}, want: "minimal"},
		{name: "explicit none wins", flags: &rootFlags{metaMode: "none", agent: true}, want: "none"},
	}
	for _, tc := range cases {
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			if got := resolvedMetaMode(tc.flags); got != tc.want {
				t.Fatalf("resolvedMetaMode(...) = %q, want %q", got, tc.want)
			}
		})
	}
}
