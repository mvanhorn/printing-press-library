...
func TestCompactWhichMatches(t *testing.T) {
	t.Parallel()

	compact := compactWhichMatches([]whichMatch{{
		Entry: whichEntry{
			Command:      "checkout slots",
			Description:  "List slots",
			WhyItMatters: "Lets the agent pick a pickup window",
		},
		Score: 7,
	}})
	raw, err := json.Marshal(compact)
	if err != nil {
		t.Fatalf("json.Marshal(compact): %v", err)
	}
	if strings.Contains(string(raw), "score") {
		t.Fatalf("compact which output should omit score: %s", string(raw))
	}
	if strings.Contains(string(raw), "why_it_matters") {
		t.Fatalf("compact which output should omit why_it_matters: %s", string(raw))
	}
}
