...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "continente-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CONTINENTE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CONTINENTE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/commerce/continente/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/github.com/mvanhorn/printing-press-library/library/commerce/continente/feedback.jsonl.
When ` + "`CONTINENTE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CONTINENTE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  continente-pp-cli feedback list
  continente-pp-cli feedback list --limit 5
  continente-pp-cli feedback list --json`,
