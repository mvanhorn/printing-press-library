...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "continente-pp-cli", "http")
	jar, _ := newPersistedCookieJar(cfg.CookieJarPath)
...
func (c *Client) ClearCookieDomainSuffix(domainSuffix string) error {
	if c == nil || c.HTTPClient == nil || c.HTTPClient.Jar == nil {
		return nil
	}
	if jar, ok := c.HTTPClient.Jar.(*persistedCookieJar); ok {
		return jar.ClearDomainSuffix(domainSuffix)
	}
	return nil
}
...
func (c *Client) PostForm(ctx context.Context, path string, form url.Values) (json.RawMessage, int, error) {
	return c.doForm(ctx, "POST", path, form, nil)
}

func (c *Client) PostFormWithHeaders(ctx context.Context, path string, form url.Values, headers map[string]string) (json.RawMessage, int, error) {
	return c.doForm(ctx, "POST", path, form, headers)
}
...
func (c *Client) doForm(ctx context.Context, method, path string, form url.Values, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	if c.DryRun {
		return c.dryRun(method, targetURL, path, formValuesPreview(form), []byte(form.Encode()), mergeHeaderMaps(map[string]string{
			"Content-Type": "application/x-www-form-urlencoded; charset=UTF-8",
		}, headerOverrides), authHeader)
	}
...
func mergeHeaderMaps(left, right map[string]string) map[string]string {
...
func formValuesPreview(form url.Values) map[string]string {
	if len(form) == 0 {
		return nil
	}
