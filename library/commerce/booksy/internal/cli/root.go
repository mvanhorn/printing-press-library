...
type rootFlags struct {
	...
	// invocation. Mirrors the BOOKSY_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "booksy-pp-cli",
		Short: `Booksy CLI — Book a haircut in Poland from the terminal — search barbers, compare prices and reviews, check open slots, and book,…`,
		Long: `Booksy CLI — Book a haircut in Poland from the terminal — search barbers, compare prices and reviews, check open slots, and book, with a dry-run-by-default booking guard.
...
See README.md or the bundled SKILL.md for recipes.`,
	...
	rootCmd.SetVersionTemplate("booksy-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select id,name,slug)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'booksy-pp-cli profile list')")
	...
	addNovelCommandIfAbsent(rootCmd, newNovelAvailabilityCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelBookCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelCheapestCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelCompareCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelEarliestCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelServicesCmd(flags))
