...
func newLearnConfig() *entities.Config {
	...
	learnSynonyms := map[string]string{
		"beard":          "broda",
		"haircut":        "strzyżenie",
		"mens haircut":   "strzyżenie męskie",
		"next available": "earliest",
		"soonest":        "earliest",
	}
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"category": {
			{Canonical: "Barber Shop", Values: []string{"barber", "barbershop", "fryzjer meski", "barbers"}},
			{Canonical: "Fryzjer", Values: []string{"hairdresser", "salon fryzjerski", "hair salon", "fryzjerstwo"}},
			{Canonical: "Paznokcie", Values: []string{"nails", "manicure", "nail salon"}},
			{Canonical: "Kosmetyczka", Values: []string{"beauty salon", "kosmetyka", "cosmetician"}},
		},
		"city": {
			{Canonical: "Warszawa", Values: []string{"warsaw", "warszawie", "waw"}},
			{Canonical: "Kraków", Values: []string{"krakow", "cracow", "krakowie"}},
			{Canonical: "Wrocław", Values: []string{"wroclaw", "wroclawiu"}},
			{Canonical: "Poznań", Values: []string{"poznan", "poznaniu"}},
			{Canonical: "Gdańsk", Values: []string{"gdansk", "gdansku"}},
			{Canonical: "Łódź", Values: []string{"lodz", "lodzi"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: booksy-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: booksy-pp-cli: learn init: %v\n", err)
		}
	})
}
