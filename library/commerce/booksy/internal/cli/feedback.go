...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BOOKSY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BOOKSY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "booksy-pp-cli/feedback")
...
When ` + "`BOOKSY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BOOKSY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
  ...
  booksy-pp-cli feedback "services didn't surface the addon price"
  echo "availability was slow for a 30-day window" | booksy-pp-cli feedback --stdin
`, "\n"),
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  booksy-pp-cli feedback list
  booksy-pp-cli feedback list --limit 5
  booksy-pp-cli feedback list --json`,
