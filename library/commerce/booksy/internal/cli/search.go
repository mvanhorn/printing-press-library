...
In local mode: searches locally synced data only.`,
  ...
  booksy-pp-cli search "error timeout"

  # Force local search only
  booksy-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  booksy-pp-cli search "status" --type businesses --data-source local
  # JSON output for piping
  booksy-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /core/v2/customer_api/businesses
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/core/v2/customer_api/businesses", map[string]string{
					"query": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'booksy-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "businesses":
				results, err = db.SearchBusinesses(query, limit)
			case "discover":
				results, err = db.SearchDiscover(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchBusinesses(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search businesses failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchDiscover(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search discover failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
