...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  booksy-pp-cli doctor
  booksy-pp-cli doctor --json
  booksy-pp-cli doctor --fail-on warn
  booksy-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "Set your API key with: export BOOKSY_ACCESS_TOKEN=\"your-token-here\""
					} else {
						authConfigured = true
			...
			if os.Getenv("BOOKSY_ACCESS_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "BOOKSY_ACCESS_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "BOOKSY_ACCESS_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					healthPath := "/core/v2/customer_api/search/recommended_treatments/"
					...
					if authHeader == "" {
						...
						authHeaders["x-api-key"] = "web-e3d812bf-d7a2-445d-ab38-55589ae6a121"
						authHeaders["x-app-version"] = "3.0"
						authHeaders["accept-language"] = "pl"
						authHeaders["x-fingerprint"] = "92b1b36d-e6fc-4d91-ada3-80cab96ec2f6"
						authHeaders["User-Agent"] = "booksy-pp-cli"
						verifyPath := "/core/v2/customer_api/me"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("booksy-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'booksy-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'booksy-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		...
		report["hint"] = "No sync recorded; run 'booksy-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'booksy-pp-cli sync' to refresh."
	}
	return report
}
