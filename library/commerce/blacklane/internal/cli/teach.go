...
// blacklane-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/commerce/blacklane/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/commerce/blacklane/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/commerce/blacklane/internal/learn/patterns"
...
const noLearnEnvVar = "BLACKLANE_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("blacklane-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  blacklane-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  blacklane-pp-cli recall "<question>" --agent
  blacklane-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  blacklane-pp-cli learnings list --agent
  blacklane-pp-cli learnings list --query "<substring>"
  blacklane-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  blacklane-pp-cli learnings forget "<question>" --resource <id>
  blacklane-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  blacklane-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  blacklane-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
