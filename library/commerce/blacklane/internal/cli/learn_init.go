...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"service_class": {
			{Canonical: "business", Values: []string{"business", "business class", "sedan", "standard", "e-class", "saloon"}},
			{Canonical: "first", Values: []string{"first", "first class", "luxury", "s-class", "premium"}},
			{Canonical: "van", Values: []string{"van", "suv", "business van", "business suv", "mpv", "v-class", "minibus", "group"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: blacklane-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: blacklane-pp-cli: learn init: %v\n", err)
		}
	})
}
