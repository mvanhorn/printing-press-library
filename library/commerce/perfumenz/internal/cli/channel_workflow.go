...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  perfumenz-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  perfumenz-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"collections", "products-json"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  perfumenz-pp-cli workflow status

  # Show status as JSON
  perfumenz-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("perfumenz-pp-cli")
			}
