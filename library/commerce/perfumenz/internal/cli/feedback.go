...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "perfumenz-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PERFUMENZ_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PERFUMENZ_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "perfumenz-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/perfumenz-pp-cli/feedback.jsonl.
When ` + "`PERFUMENZ_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PERFUMENZ_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  perfumenz-pp-cli feedback list
  perfumenz-pp-cli feedback list --limit 5
  perfumenz-pp-cli feedback list --json`,
