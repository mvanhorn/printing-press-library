...
import (
	...
	"net/url"
	"os"
	"perfumenz-pp-cli/internal/cliutil"
...
func newSyncCmd(flags *rootFlags) *cobra.Command {
	...
	var seedFile string // hand-authored: allow seeding full catalog + note parse from a local products.json (research capture)
...
Resource scoping:
  ...
  perfumenz-pp-cli sync

  # Sync specific resources only
  perfumenz-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  perfumenz-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  perfumenz-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  perfumenz-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  perfumenz-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// hand-authored Perfumenz seed path (full catalog + note parsing from research capture)
			if seedFile != "" {
				return seedPerfumenzFromFile(cmd, db, seedFile, humanFriendly)
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/perfumenz-pp-cli/data.db)")
	...
	// hand-authored for Perfumenz: seed full catalog (with note parsing) from a captured products.json when live public feed is limited
	cmd.Flags().StringVar(&seedFile, "seed", "", "Seed the store from a local full products.json (research capture). Parses Top/Heart/Base notes and bypasses live fetch.")
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"collections":   "/collections/all.json",
...
// seedPerfumenzFromFile is hand-authored for Perfumenz to bootstrap the full catalog
// (the public /products.json feed often returns a limited page; the research capture has the complete set).
// It loads the JSON, parses Top/Heart/Base notes via ParseNotes, and upserts into the
// resources table (resource_type "perfumes") with enriched note arrays in the JSON data.
func seedPerfumenzFromFile(cmd *cobra.Command, db *store.Store, path string, humanFriendly bool) error {
	f, err := os.Open(path)
	if err != nil {
	...
	if err := json.NewDecoder(f).Decode(&payload); err != nil {
		...
		if err2 := json.NewDecoder(f).Decode(&arr); err2 != nil {
			return fmt.Errorf("decoding seed JSON (expected {products:[]} or []): %w / %w", err, err2)
		}
		payload.Products = arr
	}
	if len(payload.Products) == 0 {
		return fmt.Errorf("no products found in seed file")
	}
	count := 0
	for _, p := range payload.Products {
		body := ""
		if v, ok := p["body_html"].(string); ok {
			body = v
		}
		t, h, b := ParseNotes(body)
		if len(t) > 0 || len(h) > 0 || len(b) > 0 {
			p["top_notes"] = t
			p["heart_notes"] = h
			p["base_notes"] = b
		}
		// choose stable id: id or handle
		id := ""
		if v, ok := p["id"]; ok {
			id = fmt.Sprintf("%v", v)
		}
		if id == "" {
			if v, ok := p["handle"].(string); ok {
				id = v
			}
		}
		if id == "" {
		...
		if err := db.Upsert("perfumes", id, dataBytes); err != nil {
			return fmt.Errorf("upsert %s: %w", id, err)
		}
		count++
	}
	if humanFriendly {
		fmt.Fprintf(cmd.OutOrStdout(), "seeded %d perfumes from %s (notes parsed for %d)\n", count, path, count)
	}
	return nil
}
