# Perfume NZ Printed CLI Agent Guide

This directory is a generated `perfumenz-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
perfumenz-pp-cli doctor --json
perfumenz-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
perfumenz-pp-cli which "<capability>" --json
perfumenz-pp-cli <command> --help
```
...
```bash
perfumenz-pp-cli <command> --agent
```
...
```bash
perfumenz-pp-cli <command> --help
perfumenz-pp-cli <command> --dry-run --agent
```
