...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "noon-ae-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NOON_AE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NOON_AE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "noon-ae-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/noon-ae-pp-cli/feedback.jsonl.
When ` + "`NOON_AE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NOON_AE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  noon-ae-pp-cli feedback list
  noon-ae-pp-cli feedback list --limit 5
  noon-ae-pp-cli feedback list --json`,
