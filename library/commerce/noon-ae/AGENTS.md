# Noon Ae Printed CLI Agent Guide

This directory is a generated `noon-ae-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
noon-ae-pp-cli doctor --json
noon-ae-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
noon-ae-pp-cli which "<capability>" --json
noon-ae-pp-cli <command> --help
```
...
```bash
noon-ae-pp-cli <command> --help
noon-ae-pp-cli <command> --dry-run --agent
```
