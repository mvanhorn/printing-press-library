  ...
  openclaw:
    ...
    install:
      - kind: go
        ...
        module: github.com/mvanhorn/printing-press-library/library/ai/huggingface/cmd/huggingface-pp-cli
---
...
```bash
go install github.com/mvanhorn/printing-press-library/library/ai/huggingface/cmd/huggingface-pp-cli@latest
```

If `--version` reports "command not found" after install, the install step did not put the binary on `$PATH`. Do not proceed with skill commands until verification succeeds.

Closes the loop between Hugging Face discovery and operational decisions: should this model replace what runs today, will it load on the chosen backend, has it already been benched. Every command is multi-runtime safe (TTY-agnostic, --json + stable schemas, exit-code semantics, flock-guarded state) so the same binary serves an interactive terminal and a containerized agent without per-environment forks.

## When to Use This CLI

Reach for this CLI whenever the answer to "should I switch from what I run today?" would otherwise require a WebFetch dance through model cards, config.json files, GitHub PRs, and local config. The discovery half (find-feature, find-quants, model-card, backend-check, trending, derivatives, uploader-rep, compare-quants, doctor, schema, local) is universally useful with no setup. The stack-coupled half (vs-current, bench-history, eval-candidates, watch, watch-poll) lights up when the caller has a local agent config (`data/openclaw.json` shape) and a `model-eval-harness` layout — set `HF_OPENCLAW_ROOT` or pass `--config-path` / `--harness` to point at your own paths.
...
- **`watch`** — State-file subscription with cron-callable poll; matches emit through the MC API alert pipeline as structured events.
  ...
  ```bash
  huggingface-pp-cli watch add unsloth --kind uploader --notify jarvis
  ```
...
- **`find-quants`** — Surface GGUF quant variants of a base model with uploader rep, effective size, and quant-type classification — sorted by trusted-uploader allowlist + size filter.
  ...
  _Use when picking a quant for a target hardware budget; encodes a curated trusted-uploader heuristic (unsloth, bartowski, mradermacher)._
...
1. Install the MCP server:
   ```bash
   go install github.com/mvanhorn/printing-press-library/library/ai/huggingface/cmd/huggingface-pp-mcp@latest
   ```
