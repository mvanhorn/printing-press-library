---
name: pp-airbnb-vrbo
description: "Skip the platform fee. Find the host's direct booking site for any Airbnb or VRBO listing. Trigger phrases: `find the direct booking site`, `skip the airbnb fee`, `compare airbnb and vrbo`, `vacation rental cheapest`, `book direct`, `use airbnb-vrbo`, `run airbnb-vrbo`."
argument-hint: "<command> [args] | install cli|mcp"
allowed-tools: "Read Bash"
metadata: '{"openclaw":{"requires":{"bins":["airbnb-vrbo-pp-cli"]},"install":[{"id":"go","kind":"shell","command":"go install github.com/mvanhorn/printing-press-library/library/other/airbnb-vrbo-pp-cli/cmd/airbnb-vrbo-pp-cli@latest","bins":["airbnb-vrbo-pp-cli"],"label":"Install via go install"}]}}'
---
...
- `airbnb-vrbo-pp-cli airbnb_listing get` — Get full Airbnb listing detail (amenities, house rules, location, highlights, description, policies, host) via SSR...
- `airbnb-vrbo-pp-cli airbnb_listing search` — Search Airbnb listings by location, dates, and guest count via the public SSR HTML page (openbnb pattern). Walks...
...
- `airbnb-vrbo-pp-cli host extract` — Extract the host's brand or display name from a listing URL across both platforms. Uses propertyManagement.name...
...
- `airbnb-vrbo-pp-cli vrbo_listing get` — Get full VRBO property detail via the propertyDetail GraphQL operation (operation name discovered at runtime). Falls...
...
This printed CLI owns bounded freshness only for registered store-backed read command paths. In `--data-source auto` mode, those paths check `sync_state` and may run a bounded refresh before reading local data. `--data-source local` never refreshes. `--data-source live` reads the API and does not mutate the local store. Set `AIRBNB_VRBO_NO_AUTO_REFRESH=1` to skip the freshness hook without changing source selection.
...
```bash
airbnb-vrbo-pp-cli which "<capability in your own words>"
```
...
```bash
airbnb-vrbo-pp-cli cheapest 'https://www.airbnb.com/rooms/37124493' --checkin 2026-05-16 --checkout 2026-05-19 --json --select savings,direct.url,direct.total,airbnb.total
```
...
```bash
airbnb-vrbo-pp-cli watch add 'https://www.airbnb.com/rooms/37124493' --max-price 350 && airbnb-vrbo-pp-cli watch check
```
...
```bash
airbnb-vrbo-pp-cli host portfolio 'Vacasa' --city Austin --agent --select listings.title,listings.direct_url
```
...
```bash
airbnb-vrbo-pp-cli airbnb get 37124493 --agent --select listing.title,listing.coordinate.latitude,listing.coordinate.longitude,pricingQuote.structuredStayDisplayPrice.primaryLine.price,pricingQuote.structuredStayDisplayPrice.secondaryLine.price
```
...
Run `airbnb-vrbo-pp-cli doctor` to verify setup.
...
- **Filterable** — `--select` keeps a subset of fields. Dotted paths descend into nested structures; arrays traverse element-wise. Critical for keeping context small on verbose APIs:
  ...
  ```bash
  airbnb-vrbo-pp-cli airbnb_listing get --agent --select id,name,status
  ```
...
```
airbnb-vrbo-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
airbnb-vrbo-pp-cli feedback --stdin < notes.txt
airbnb-vrbo-pp-cli feedback list --json --limit 10
```

Entries are stored locally at `~/.airbnb-vrbo-pp-cli/feedback.jsonl`. They are never POSTed unless `AIRBNB_VRBO_FEEDBACK_ENDPOINT` is set AND either `--send` is passed or `AIRBNB_VRBO_FEEDBACK_AUTO_SEND=true`. Default behavior is local-only.
...
```
airbnb-vrbo-pp-cli profile save briefing --json
airbnb-vrbo-pp-cli --profile briefing airbnb_listing get
airbnb-vrbo-pp-cli profile list --json
airbnb-vrbo-pp-cli profile show briefing
airbnb-vrbo-pp-cli profile delete briefing --yes
```
...
1. **Empty, `help`, or `--help`** → show `airbnb-vrbo-pp-cli --help` output
...
2. Install:
   ```bash
   go install github.com/mvanhorn/printing-press-library/library/other/airbnb-vrbo-pp-cli/cmd/airbnb-vrbo-pp-cli@latest
   
   # If `@latest` installs a stale build (Go module proxy cache lag), install from main:
   GOPRIVATE='github.com/mvanhorn/*' GOFLAGS=-mod=mod \
     go install github.com/mvanhorn/printing-press-library/library/other/airbnb-vrbo-pp-cli/cmd/airbnb-vrbo-pp-cli@main
   ```
3. Verify: `airbnb-vrbo-pp-cli --version`
...
1. Install the MCP server:
   ```bash
   go install github.com/mvanhorn/printing-press-library/library/other/airbnb-vrbo-pp-cli/cmd/airbnb-vrbo-pp-mcp@latest
   
   # If `@latest` installs a stale build (Go module proxy cache lag), install from main:
   GOPRIVATE='github.com/mvanhorn/*' GOFLAGS=-mod=mod \
     go install github.com/mvanhorn/printing-press-library/library/other/airbnb-vrbo-pp-cli/cmd/airbnb-vrbo-pp-mcp@main
   ```
2. Register with Claude Code:
   ```bash
   claude mcp add airbnb-vrbo-pp-mcp -- airbnb-vrbo-pp-mcp
   ```
3. Verify: `claude mcp list`

## Direct Use

1. Check if installed: `which airbnb-vrbo-pp-cli`
   If not found, offer to install (see CLI Installation above).
2. Match the user query to the best command from the Unique Capabilities and Command Reference above.
3. Execute with the `--agent` flag:
   ```bash
   airbnb-vrbo-pp-cli <command> [subcommand] [args] --agent
   ```
4. If ambiguous, drill into subcommand help: `airbnb-vrbo-pp-cli <command> --help`.
