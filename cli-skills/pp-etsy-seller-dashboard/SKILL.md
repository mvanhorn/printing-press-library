---
name: pp-etsy-seller-dashboard
description: "Printing Press CLI for Etsy Seller Dashboard. Read-only Etsy Shop Manager demand, advertising, offsite attribution, and promotion intelligence"
author: "horknfbr"
license: "Apache-2.0"
...
metadata:
  openclaw:
    requires:
      bins:
        - etsy-seller-dashboard-pp-cli
    install:
      - kind: go
        bins: [etsy-seller-dashboard-pp-cli]
        module: github.com/mvanhorn/printing-press-library/library/ecommerce/etsy-seller-dashboard/cmd/etsy-seller-dashboard-pp-cli
---
<!-- GENERATED FILE — DO NOT EDIT.
     This file is a verbatim mirror of library/commerce/etsy-seller-dashboard/SKILL.md,
     regenerated post-merge by tools/generate-skills/. Hand-edits here are
     silently overwritten on the next regen. Edit the library/ source instead.
     See the repository agent guide, section "Generated artifacts: registry.json, cli-skills/". -->

# Etsy Seller Dashboard — Printing Press CLI

## Prerequisites: Install the CLI

This skill drives the `etsy-seller-dashboard-pp-cli` binary. **You must verify the CLI is installed before invoking any command from this skill.** If it is missing, install it first:

1. Install via the Printing Press installer. It defaults binaries to `$HOME/.local/bin` on macOS/Linux and `%LOCALAPPDATA%\Programs\PrintingPress\bin` on Windows:
   ```bash
   npx -y @mvanhorn/printing-press-library install etsy-seller-dashboard --cli-only
   ```
2. Verify: `etsy-seller-dashboard-pp-cli --version`
...
## Unsupported or Unobserved Data

- Marketplace Insights country filters and keyword clicks/CTR are unavailable in the captured seller surface.
- Etsy Ads search terms are unavailable; never infer keyword revenue from listing titles.
- Promotion order count, item count, and average order value are optional and may be absent.
- Etsy-side mutations are outside this CLI's scope.
...
**ads** — Review read-only Etsy Ads listing and account performance

- `etsy-seller-dashboard-pp-cli ads <shop_id>` — List Etsy Ads performance by listing

**marketplace-insights** — Research first-party Etsy search demand, competition, trends, and saved terms
...
**offsite-ads** — Review read-only Offsite Ads attribution, traffic, channels, listings, and fees
...
**promotions** — Review read-only Etsy sales, promo codes, bundles, and targeted offers
...
```bash
etsy-seller-dashboard-pp-cli which "<capability in your own words>"
```
...
Run `etsy-seller-dashboard-pp-cli doctor` to verify setup.
...
- **Filterable** — `--select` keeps a subset of fields. Dotted paths descend into nested structures; arrays traverse element-wise. Critical for keeping context small on verbose APIs:
  ...
  ```bash
  etsy-seller-dashboard-pp-cli marketplace-insights search mock-value --agent --select id,name,status
  ```
...
- Use `--home <dir>` for one invocation, or set `ETSY_SELLER_DASHBOARD_HOME=<dir>` to relocate all four path kinds under one root.
- Use per-kind env vars only when a specific kind must diverge: `ETSY_SELLER_DASHBOARD_CONFIG_DIR`, `ETSY_SELLER_DASHBOARD_DATA_DIR`, `ETSY_SELLER_DASHBOARD_STATE_DIR`, `ETSY_SELLER_DASHBOARD_CACHE_DIR`.
- Resolution order is per-kind env var, `--home`, `ETSY_SELLER_DASHBOARD_HOME`, XDG (`XDG_CONFIG_HOME`, `XDG_DATA_HOME`, `XDG_STATE_HOME`, `XDG_CACHE_HOME`), then platform defaults.
- `config` contains settings like `config.toml` and profiles. `data` contains `credentials.toml`, `data.db`, cookies, and auth sidecars. `state` contains persisted queries, jobs, and `teach.log`. `cache` contains regenerable HTTP/cache files.
- Stored secrets live in `credentials.toml` under the data dir. Existing legacy `config.toml` secrets are read for compatibility and leave `config.toml` on the first auth write.
- Run `etsy-seller-dashboard-pp-cli doctor --fail-on warn` to surface path and credential-location warnings. `agent-context` exposes a schema v4 `paths` block for agents that need the resolved dirs.
...
Fleet precedence: an inherited per-kind env var overrides an explicit `--home` for that kind. Use `ETSY_SELLER_DASHBOARD_HOME` or per-kind vars as durable fleet levers, and use `--home` only for a single invocation. Relocation is not reversible by unsetting env vars; move files manually before clearing `ETSY_SELLER_DASHBOARD_HOME`, or `doctor` will not find credentials left under the former root.
...
```bash
etsy-seller-dashboard-pp-cli recall "<user's question>" --agent
```
...
```json
{
  ...
  "candidates": [
    { "id": 12, "class": "flag_alias | playbook_candidate",
      ...
      "rationale": "...",
      "next_action": ["<trial command>", "etsy-seller-dashboard-pp-cli learnings confirm 12"] }
  ],
  "playbook": {
    "query_family": "...",
    "playbook": {
...
if Playbook present:
    ...
    -> the Playbook's expected_tool_calls is a budget; if you find yourself running
       materially more, record the divergence via `etsy-seller-dashboard-pp-cli playbook amend`
       at end-of-session.
...
```bash
etsy-seller-dashboard-pp-cli teach --query "<user's question>" --resource-type <type> --resource <id1> --resource <id2>
...
etsy-seller-dashboard-pp-cli teach-playbook \
  --query "<user's question>" \
  --playbook-file ~/playbooks/<shape>.json \
...
```bash
etsy-seller-dashboard-pp-cli playbook amend \
...
- `ETSY_SELLER_DASHBOARD_NO_LEARN=true` in the environment globally disables the pipeline.
...
```
etsy-seller-dashboard-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
etsy-seller-dashboard-pp-cli feedback --stdin < notes.txt
etsy-seller-dashboard-pp-cli feedback list --json --limit 10
```

Entries are stored locally as `feedback.jsonl` under the resolved data dir. They are never POSTed unless `ETSY_SELLER_DASHBOARD_FEEDBACK_ENDPOINT` is set AND either `--send` is passed or `ETSY_SELLER_DASHBOARD_FEEDBACK_AUTO_SEND=true`. Default behavior is local-only.
...
```
etsy-seller-dashboard-pp-cli profile save briefing --json
etsy-seller-dashboard-pp-cli --profile briefing marketplace-insights search mock-value
etsy-seller-dashboard-pp-cli profile list --json
etsy-seller-dashboard-pp-cli profile show briefing
etsy-seller-dashboard-pp-cli profile delete briefing --yes
```
...
1. **Empty, `help`, or `--help`** → show `etsy-seller-dashboard-pp-cli --help` output
...
2. Register with Claude Code:
   ```bash
   claude mcp add etsy-seller-dashboard-pp-mcp -- etsy-seller-dashboard-pp-mcp
   ```
3. Verify: `claude mcp list`

## Direct Use

1. Check if installed: `which etsy-seller-dashboard-pp-cli`
   If not found, offer to install (see Prerequisites at the top of this skill).
2. Match the user query to the best command from the Unique Capabilities and Command Reference above.
3. Execute with the `--agent` flag:
   ```bash
   etsy-seller-dashboard-pp-cli <command> [subcommand] [args] --agent
   ```
4. If ambiguous, drill into subcommand help: `etsy-seller-dashboard-pp-cli <command> --help`.
