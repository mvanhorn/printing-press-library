...
description: "Every land bank's public inventory, one command — enumerate, filter, export Trigger phrases: `list land bank properties`, `export land bank parcels`, `sync epropertyplus inventory`, `find vacant lots in a land bank`, `use epropertyplus`, `run epropertyplus`."
author: "startos00"
license: "Apache-2.0"
...
<!-- GENERATED FILE — DO NOT EDIT.
     This file is a verbatim mirror of library/other/epropertyplus/SKILL.md,
...
- `epropertyplus-pp-cli property get` — Returns the full property record (parcel number, address, geometry, zoning, potential use, structure type, occupancy
- `epropertyplus-pp-cli property get-custom-field-configs` — Returns the instance's custom-field configuration so the cryptic s_custom_*/n_custom_* keys on a Property can be mapped
- `epropertyplus-pp-cli property get-image` — Returns the binary image.
...
Entries are stored locally at `~/.local/share/epropertyplus-pp-cli/feedback.jsonl`. They are never POSTed unless `EPROPERTYPLUS_FEEDBACK_ENDPOINT` is set AND either `--send` is passed or `EPROPERTYPLUS_FEEDBACK_AUTO_SEND=true`. Default behavior is local-only.
