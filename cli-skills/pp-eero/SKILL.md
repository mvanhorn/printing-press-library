---
name: pp-eero
description: "Inspect an eero mesh network from the terminal with agent-friendly output for networks, nodes, connected devices, diagnostics, and speed tests."
author: "Erik Rogne"
license: "Apache-2.0"
...
metadata:
  openclaw:
    ...
    install:
      - kind: go
        bins: [eero-pp-cli]
        module: github.com/mvanhorn/printing-press-library/library/devices/eero/cmd/eero-pp-cli
---
...
`eero-pp-cli learnings stats` reports recall hit rate, teach-to-reuse, playbook resolution rate, and candidate confirm/reject counts from the local `learn_events` table. Rates are null until they have a denominator; everything stays on this machine. Use it to check whether the loop is earning its keep for this CLI.

### Disabling learning

- `--no-learn` on a single command short-circuits both `recall` and the `teach` write path. Use for deterministic agent flows or tests that must not be affected by accumulated learnings.
- `EERO_NO_LEARN=true` in the environment globally disables the pipeline.
...
```
eero-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
eero-pp-cli feedback --stdin < notes.txt
eero-pp-cli feedback list --json --limit 10
```

Entries are stored locally as `feedback.jsonl` under the resolved data dir. They are never POSTed unless `EERO_FEEDBACK_ENDPOINT` is set AND either `--send` is passed or `EERO_FEEDBACK_AUTO_SEND=true`. Default behavior is local-only.
...
```
eero-pp-cli profile save briefing --json
eero-pp-cli --profile briefing account
eero-pp-cli profile list --json
eero-pp-cli profile show briefing
eero-pp-cli profile delete briefing --yes
```
...
1. **Empty, `help`, or `--help`** → show `eero-pp-cli --help` output
...
2. Register with Claude Code:
   ```bash
   claude mcp add eero-pp-mcp -- eero-pp-mcp
   ```
...
3. Execute with the `--agent` flag:
   ```bash
   eero-pp-cli <command> [subcommand] [args] --agent
   ```
4. If ambiguous, drill into subcommand help: `eero-pp-cli <command> --help`.
