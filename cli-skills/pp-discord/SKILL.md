---
name: pp-discord
description: "Printing Press CLI for Discord. Preview of the Discord v10 HTTP API specification. See https://discord.com/developers/docs for more details."
...
metadata:
  openclaw:
    ...
    install:
      - kind: go
        bins: [discord-pp-cli]
        module: github.com/mvanhorn/printing-press-library/library/productivity/discord/cmd/discord-pp-cli
---

<!-- GENERATED FILE — DO NOT EDIT.
     This file is a verbatim mirror of library/productivity/discord/SKILL.md,
...
1. Install via the Printing Press installer. It defaults binaries to `$HOME/.local/bin` on macOS/Linux and `%LOCALAPPDATA%\Programs\PrintingPress\bin` on Windows:
   ...
   ```
2. Verify: `discord-pp-cli --version`
...
**partner-sdk** — Manage partner sdk
...
**soundboard-default-sounds** — Manage soundboard default sounds

- `discord-pp-cli soundboard-default-sounds` — Get

**stage-instances** — Manage stage instances
...
**sticker-packs** — Manage sticker packs
...
```bash
discord-pp-cli which "<capability in your own words>"
```

`which` resolves a natural-language capability query to the best matching command from this CLI's curated feature index. Exit code `0` means at least one match; exit code `2` means no confident match — fall back to `--help` or use a narrower query.

## Auth Setup
Run `discord-pp-cli auth setup` to print the URL and steps for getting a key (add `--launch` to open the URL). Then set:

```bash
export DISCORD_BOT_TOKEN="<your-key>"
```

To persist credentials, use `discord-pp-cli auth set-token <token>`. Stored secrets live in `credentials.toml` under the data dir, not in `config.toml`.

Run `discord-pp-cli doctor` to verify setup.
...
- **Filterable** — `--select` keeps a subset of fields. Dotted paths descend into nested structures; arrays traverse element-wise. Critical for keeping context small on verbose APIs:
  ...
  ```bash
  discord-pp-cli applications get mock-value --agent --select id,name,status
  ```
...
- Use `--home <dir>` for one invocation, or set `DISCORD_HOME=<dir>` to relocate all four path kinds under one root.
- Use per-kind env vars only when a specific kind must diverge: `DISCORD_CONFIG_DIR`, `DISCORD_DATA_DIR`, `DISCORD_STATE_DIR`, `DISCORD_CACHE_DIR`.
- Resolution order is per-kind env var, `--home`, `DISCORD_HOME`, XDG (`XDG_CONFIG_HOME`, `XDG_DATA_HOME`, `XDG_STATE_HOME`, `XDG_CACHE_HOME`), then platform defaults.
- `config` contains settings like `config.toml` and profiles. `data` contains `credentials.toml`, `data.db`, cookies, and auth sidecars. `state` contains persisted queries, jobs, and `teach.log`. `cache` contains regenerable HTTP/cache files.
- Stored secrets live in `credentials.toml` under the data dir. Existing legacy `config.toml` secrets are read for compatibility and leave `config.toml` on the first auth write.
- Run `discord-pp-cli doctor --fail-on warn` to surface path and credential-location warnings. `agent-context` exposes a schema v4 `paths` block for agents that need the resolved dirs.
...
Fleet precedence: an inherited per-kind env var overrides an explicit `--home` for that kind. Use `DISCORD_HOME` or per-kind vars as durable fleet levers, and use `--home` only for a single invocation. Relocation is not reversible by unsetting env vars; move files manually before clearing `DISCORD_HOME`, or `doctor` will not find credentials left under the former root.
...
```bash
discord-pp-cli recall "<user's question>" --agent
```
...
```json
{
  ...
  "candidates": [
    { "id": 12, "class": "flag_alias | playbook_candidate",
      ...
      "rationale": "...",
      "next_action": ["<trial command>", "discord-pp-cli learnings confirm 12"] }
  ],
  "playbook": {
    "query_family": "...",
    "playbook": {
...
if Playbook present:
    ...
    -> the Playbook's expected_tool_calls is a budget; if you find yourself running
       materially more, record the divergence via `discord-pp-cli playbook amend`
       at end-of-session.
...
- `lookup_refresh_available` (top-level): an entity in the query has no lookup row yet, but synced data could provide one. Run `discord-pp-cli sync` to refresh entity lookups.
...
```bash
discord-pp-cli teach --query "<user's question>" --resource-type <type> --resource <id1> --resource <id2>
...
`discord-pp-cli learnings stats` reports recall hit rate, teach-to-reuse, playbook resolution rate, and candidate confirm/reject counts from the local `learn_events` table. Rates are null until they have a denominator; everything stays on this machine. Use it to check whether the loop is earning its keep for this CLI.

### Disabling learning

- `--no-learn` on a single command short-circuits both `recall` and the `teach` write path. Use for deterministic agent flows or tests that must not be affected by accumulated learnings.
- `DISCORD_NO_LEARN=true` in the environment globally disables the pipeline.
...
```
discord-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
discord-pp-cli feedback --stdin < notes.txt
discord-pp-cli feedback list --json --limit 10
```

Entries are stored locally as `feedback.jsonl` under the resolved data dir. They are never POSTed unless `DISCORD_FEEDBACK_ENDPOINT` is set AND either `--send` is passed or `DISCORD_FEEDBACK_AUTO_SEND=true`. Default behavior is local-only.
...
```
discord-pp-cli profile save briefing --json
discord-pp-cli --profile briefing applications get mock-value
discord-pp-cli profile list --json
discord-pp-cli profile show briefing
discord-pp-cli profile delete briefing --yes
```
...
1. **Empty, `help`, or `--help`** → show `discord-pp-cli --help` output
...
2. Register with Claude Code:
   ```bash
   claude mcp add discord-pp-mcp -- discord-pp-mcp
   ```
3. Verify: `claude mcp list`

## Direct Use

1. Check if installed: `which discord-pp-cli`
   If not found, offer to install (see Prerequisites at the top of this skill).
2. Match the user query to the best command from the Unique Capabilities and Command Reference above.
3. Execute with the `--agent` flag:
   ```bash
   discord-pp-cli <command> [subcommand] [args] --agent
   ```
4. If ambiguous, drill into subcommand help: `discord-pp-cli <command> --help`.
