...
| R6 | If the dogfood matrix fails the bar (any Tier-1 command produces null/error on a known-good listing), the plan executes the deletion path rather than leaving the CLI shipped-but-broken | User: "prove that this thing works, or we need to delete it from the library" |
...
| Bot-challenge sentinel | New `BotChallengeError struct { ChallengeType string; Remediation string }` as a sibling of `RateLimitError` in `internal/cliutil/errors.go` (or local to `source/airbnb` if cliutil is shared template) | Symmetry with `RateLimitError`. Lets callers `errors.As` and surface the remediation hint in CLI output without coupling to detection logic. |
...
- Update `BookingPrice`, `WishlistList`, `WishlistItems` call sites to read the resolver instead of the constant. The hash appears twice per call (URL path segment + `extensions` query param sha256Hash field) — update both occurrences.
- Reuse the existing `fetchHomepage`-equivalent helper that PR #712 added for the API key (or generalize it once and call from both API-key and hash resolvers).
...
**Approach:**
- Replace any existing bare `time.Sleep(cliutil.Backoff(attempt))` calls with `time.Sleep(cliutil.Backoff(attempt) + time.Duration(rand.Int63n(int64(cliutil.Backoff(attempt)/4))))` (25% jitter).
- Guard against `cliutil.Backoff(attempt) == 0` for edge cases (`rand.Int63n` panics on zero).
...
- Each subtest writes a JSON entry to a temp results file with: `{name, tier, exit_code, ok, stdout_shape, stderr_summary, duration_ms, observed_error_type}`.
